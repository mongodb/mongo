    ...
    const WT_ITEM *src, WT_ITEM *dst, int *compression_failed)
{
	...
	/*
	 * dst->size was computed in wt_snappy_pre_size, so we know it's big
	 * enough.  Skip past the space we'll use to store the compressed buf
	 * size.
	 */
	...
	snret = snappy_compress(
	    (char *)src->data, src->size, snapbuf, &snaplen);
...
static int
wt_snappy_decompress(WT_COMPRESSOR *compressor,
    WT_SESSION *session, const WT_ITEM *src, WT_ITEM *dst)
{
	snappy_status snret;
	size_t dstlen, snaplen;
	...
	dstlen = dst->size;
	snret = snappy_uncompress((char *)src->data + sizeof(size_t),
	    snaplen, (char *)dst->data, &dstlen);

	if (snret == SNAPPY_OK) {
		dst->size = (uint32_t)dstlen;
		return (0);
	}
	return (wt_snappy_error(session, "snappy_uncompress", snret));
}

static int
wt_snappy_pre_size(WT_COMPRESSOR *compressor,
    WT_SESSION *session, const WT_ITEM *src, WT_ITEM *dst)
{
	size_t snaplen;
