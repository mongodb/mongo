...
function runTests(db, coll) {
    ...
    // $merge
    // In 4.2, performing $merge against the collection being aggregated is prohibited. As such,
    // we output to an arbitrary collection.
    const mergeCollName = "mergeColl";
    checkCounters(() => coll.aggregate([{
                                $merge: {
                                    into: mergeCollName,
                                    whenMatched: [{$set: {a: {$multiply: ["$price", 2]}}}]
                                }
                            }])
                            .toArray(),
                  ['$merge', "$set"]);
    ...
    checkCounters(() => coll.update(
                      {price: {$gte: 0}}, [{$addFields: {a: {$add: ['$_id', 1]}}}], {multi: true}),
                  ["$addFields"],
                  ["$set"]);
