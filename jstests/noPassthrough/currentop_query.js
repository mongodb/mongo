...
(function() {
    ...
    const merizosDB = merizosConn.getDB("currentop_query");
    const merizosColl = merizosDB.currentop_query;
    ...
    st.ensurePrimaryShard(merizosDB.getName(), rsConn.name);
    ...
    function dropAndRecreateTestCollection() {
        ...
        assert.commandWorked(merizosDB.adminCommand(
            {shardCollection: merizosColl.getFullName(), key: {_id: "hashed"}}));
    }
     ...
     * merizoS itself rather than on the shards.
     */
    function runTests({conn, readMode, currentOp, truncatedOps, localOps}) {
                ...
                function() {
                    ...
                    if (result.inprog.length > 0) {
                        result.inprog.forEach((op) => {
                            assert.eq(op.appName, "MerizoDB Shell", tojson(result));
        ...
        function runStandardTests() {
            ...
            for (let cmdName in originatingCommands) {
                ...
                // If this is a non-localOps test running via merizoS, then the cursorID we obtained
                // above is the ID of the merizoS cursor, and will not match the IDs of any of the
    ...
    for (let connType of[rsConn, merizosConn]) {
