...
function setParameterTest(conn) {
    ...
    // proxyUnixSocketMaximumHeaderSize can be btwn kDefaultHeaderSize and
    // kMaxConfigurableProxyUnixSocketHeaderSize.
    ...
    // proxyUnixSocketMaximumHeaderSize should be smaller than
    // kMaxConfigurableProxyUnixSocketHeaderSize.
...
function assertConnectWithProxyProtocolHeader(size, shouldSucceed) {
    ...
    assertion(
        () => new Mongo(uri),
        [],
        `Expected connection with proxy protocol size of ${size} ${
            shouldSucceed ? "to succeed" : "to throw"}`,
    );
}
...
function runConnectTest(conn) {
    ...
    // On the proxy unix socket, the largest we can read by default is
    // kDefaultProxyUnixSocketHeaderSize.
    ...
    // Test that we can configure the proxy unix socket max header size and have the previous header
    // succeed.
    setParameterWithAssert(
        conn, {proxyUnixSocketMaximumHeaderSize: kDefaultProxyUnixSocketHeaderSize + 1000});
