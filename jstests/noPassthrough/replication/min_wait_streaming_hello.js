...
describe("minWaitForStreamingHelloMillis", function() {
    ...
    it("clamps maxAwaitTimeMS to minimum when below threshold for unauthenticated client",
       function() {
           ...
           assert.gte(
               elapsed,
               1800,
               "Expected command to wait at least 1800ms due to clamping, " +
                   "but it completed in " + elapsed + "ms",
           );
       });
    ...
    it("allows maxAwaitTimeMS at or above the minimum without clamping", function() {
        ...
        assert.gte(
            elapsed,
            1800,
            "Expected command to wait at least 1800ms, " +
                "but it completed in " + elapsed + "ms",
        );
    });
    ...
    it("can update minWaitForStreamingHelloMillis at runtime", function() {
        ...
        assert.gte(
            elapsed,
            400,
            "Expected command to wait at least 400ms due to clamping, " +
                "but it completed in " + elapsed + "ms",
        );
