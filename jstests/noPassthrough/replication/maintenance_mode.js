...
(function () {
    ...
    function assertShardingNotEnabled(node) {
        ...
        assert.commandFailedWithCode(node.getDB("test").runCommand(findCmd), ErrorCodes.ShardingStateNotInitialized);
    }

    {
        ...
        assert(!config.hasOwnProperty("configsvr"), tojson(config)); // possible race here... cause it fails sometimes
...

    {
        ...
        assert.commandWorked(node.getDB("test").runCommand({insert: "testColl", documents: [{x: 1}]}));
        const res = assert.commandWorked(node.getDB("test").runCommand({count: "testColl", query: {x: 1}}));
        assert.eq(res.n, 1);
