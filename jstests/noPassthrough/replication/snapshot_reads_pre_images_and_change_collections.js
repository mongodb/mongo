...
collAndErrors.forEach(({coll, error, conn}) => {
    ...
    assert.commandFailedWithCode(coll.runCommand({find: collName, readConcern: {level: "snapshot"}}), error);

    assert.commandFailedWithCode(
        coll.runCommand({aggregate: collName, pipeline: [], cursor: {}, readConcern: {level: "snapshot"}}),
        error,
    );

    assert.commandFailedWithCode(
        coll.runCommand({distinct: collName, key: "_id", readConcern: {level: "snapshot"}}),
        error,
    );
