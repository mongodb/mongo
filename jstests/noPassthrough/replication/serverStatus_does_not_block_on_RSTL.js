...
const st = new ShardingTest({
    ...
    config: 1,
    shards: 1,
    rs: {nodes: 1, setParameter: {watchdogPeriodSeconds: 60}},
});
...
try {
    ...
    const serverStatusResult = assert.commandWorked(
        testDB.adminCommand({serverStatus: 1, all: 1, maxTimeMS: 20 * 1000}),
    );
