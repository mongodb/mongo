/**
 * Container ops (ci/cu/cd) are internal-only. _applyOps() gates them based on
 * oplogApplicationMode:
 *   - oplogApplicationMode == kApplyOpsCmd (a direct applyOps invocation, e.g. from a client):
 *     featureFlagContainerWrites and test commands must both be enabled. This branch is covered by
 *     the ContainerOpsRequireFeatureFlagAndTestCommands unit test, not here;
 ...
 * oplogApplicationMode to any non-kApplyOpsCmd value (e.g. "Secondary") to skip the
 * flag/testCommands check entirely and issue a raw storage write. It exercises all three container
 * op types (ci/cu/cd), since the gate covers each of them.
 */
...
// The three internal-only container op types the gate covers: insert, update, delete.
const CONTAINER_OP_TYPES = ["ci", "cu", "cd"];
...
// attempted, so the ident never has to exist. The "o" field matches each op's schema (insert and
// update carry a value; update also carries a version; delete carries only the key).
function makeContainerOp(opType) {
    ...
    switch (opType) {
        case "ci":
            ...
            break;
        case "cu":
            op.o = {k: key, v: value, $v: NumberLong(1)};
            break;
        case "cd":
            op.o = {k: key};
            break;
...
// Asserts that every container op type (ci/cu/cd) sent via applyOps is rejected under every replay
...
// includeDefaultMode is false -- pass false when the feature flag and test commands are both
// enabled, since kApplyOpsCmd is then legitimately allowed through (see the top-of-file comment).
...
describe("applyOps container-ops enablement gate", function () {
    ...
    describe("when the container-writes feature is disabled", function () {
        it("rejects every user-issued container op for every oplogApplicationMode", function () {
            ...
            if (FeatureFlagUtil.isPresentAndEnabled(db, "ContainerWrites")) {
                jsTest.log.info(
                    "ContainerWrites enabled by default; skipping feature-disabled case",
                );
                return;
            }
    ...
    describe("when the container-writes feature is enabled", function () {
        it("rejects every user-issued container op for every oplogApplicationMode", function () {
            this.conn = MongoRunner.runMongod({setParameter: {featureFlagContainerWrites: true}});
            const db = this.conn.getDB("admin");

            // If this build cannot enable the flag, there is nothing to assert.
            if (!FeatureFlagUtil.isPresentAndEnabled(db, "ContainerWrites")) {
                jsTest.log.info(
                    "ContainerWrites could not be enabled; skipping feature-enabled case",
                );
                return;
            }

            // MongoRunner enables test commands by default, so with the feature flag also on, the
            // default (kApplyOpsCmd) mode is legitimately allowed through here -- exercise only
            // the replay modes, which is what this regression test is actually about.
