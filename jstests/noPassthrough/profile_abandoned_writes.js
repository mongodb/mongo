...
function assertNoteDocExists(db) {
    ...
    assert(noteDoc.hasOwnProperty("internalQueryGlobalProfilingLockDeadlineMs"),
...
// Test: disables profiling automatically after exceeding per-second threshold.
(function testAutoDisable() {
    ...
    assert.gte(finalAbandonedWrites - initialAbandonedWrites,
    ...
    assert.eq(
        countAfter, countBefore, "Expected no new profile entries after profiling was disabled");
})();

// Test: can be re-enabled after automatic disable.
(function testManualReenable() {
    ...
    assert.gt(countAfterReEnable,
              countBeforeReEnable,
              "Expected profiling to be re-enabled after setting level back to 2");
})();

// Test: applies policy per database.
(function testPerDatabasePolicy() {
    ...
    assert.eq(stats3.dbsPastThreshold,
              initialDbsPastThreshold + 2,
              "Expected dbsPastThreshold to increment by 2 total");
})();

// Test: can be disabled multiple times.
(function testMultipleDisables() {
...
(function testParameterValidation() {
    ...
    assert.commandFailed(
        result, "Expected setting internalProfilingMaxAbandonedWritesPerSecondPerDb to -1 to fail");
...
// Test: disables profiling immediately with zero tolerance threshold.
(function testZeroThreshold() {
