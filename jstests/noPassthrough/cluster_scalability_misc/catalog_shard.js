...

{
    ...
    const finishRemoveThread = new Thread(function(mongosHost) {
        ...
        // transitionToDedicatedConfigServer handles draining, orphan cleanup delay, and
        // finalization in a single call. Retry if it returns before reaching finalization
        // (e.g. "pendingDataCleanup" while orphanCleanupDelaySecs hasn't elapsed).
        let res;
        assert.soon(() => {
            res = mongos.adminCommand({transitionToDedicatedConfigServer: 1});
            return !res.ok || res.state === "completed";
        }, "transitionToDedicatedConfigServer did not complete within the timeout");
        return res;
    }, st.s.host);
...

{
    ...
    const timeseriesDB = st.s.getDB(timeseriesDbName);
    const bucketColl =
        getTimeseriesCollForDDLOps(timeseriesDB, timeseriesDB[timeseriesShardedCollName]);
    ...
    assert.commandWorked(
        st.s.getCollection(sessionsNs).insert({_id: testSessionId, lastUse: new Date()}));
    assert.eq(
        0, st.configRS.getPrimary().getCollection(sessionsNs).countDocuments({_id: testSessionId}));
    assert.eq(
        1, newShardRS.getPrimary().getCollection(sessionsNs).countDocuments({_id: testSessionId}));
