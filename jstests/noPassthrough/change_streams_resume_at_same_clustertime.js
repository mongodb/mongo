 ...
 * clusterTime is identical, differing only by documentKey, without causing the PBRT sent to merizoS
 * to go back-in-time.
...
(function() {
    ...
    const merizosDB = st.s.startSession({causalConsistency: true}).getDatabase(jsTestName());
    ...
    st.shardColl(merizosColl, {_id: 1}, {_id: 0}, {_id: 1}, merizosDB.getName(), true);

    // Write one document to each shard.
    assert.commandWorked(merizosColl.insert({_id: -10}));
    assert.commandWorked(merizosColl.insert({_id: 10}));

    // Open a change stream cursor to listen for subsequent events.
    let csCursor = merizosColl.watch([], {cursor: {batchSize: 1}});
    ...
    // updates.
    assert.commandWorked(merizosColl.update({}, {$set: {updated: 1}}, {multi: true}));
    assert.commandWorked(merizosColl.update({}, {$set: {updatedAgain: 1}}, {multi: true}));
    ...
    assert.commandWorked(merizosColl.update({}, {$set: {updatedYetAgain: 1}}, {multi: true}));

    // Resume from the second update, and confirm that we only see events starting with the third
    // and fourth updates. We use batchSize:1 to induce merizoD to send each individual event to the
    // merizoS when resuming, rather than scanning all the way to the most recent point in its oplog.
    csCursor = merizosColl.watch([], {resumeAfter: updateEvent._id, cursor: {batchSize: 1}});
