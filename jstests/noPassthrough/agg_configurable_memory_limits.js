...
(function testInternalQueryMaxMemoryIntensiveExpressions() {
    ...
    const originalVal = assert.commandWorked(db.adminCommand({setParameter: 1, internalQueryMaxMemoryIntensiveExpressions: 8})).was;
    assert.throwsWithCode(() => coll.aggregate(pipeline), 12876600);
    assert.commandWorked(db.adminCommand({setParameter: 1, internalQueryMaxMemoryIntensiveExpressions: originalVal}));
})();
