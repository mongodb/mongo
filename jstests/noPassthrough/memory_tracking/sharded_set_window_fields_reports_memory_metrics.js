 ...
 * and explain("executionStats") do not report on the merger part of a split pipeline, and
 * $setWindowFields is forced to be run on the merging node, so we cannot see memory tracking
 * information memory tracking statistics.
 */
...
runShardedMemoryStatsTest({
    ...
    // the merging node - so we skip explain, and the numShards parameter is unused
    skipExplain: true
});
st.stop();
