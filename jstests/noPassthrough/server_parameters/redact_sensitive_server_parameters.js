...
function assertLogEntry(logFile, logId, attrs) {
    assert(
        checkLog.checkContainsWithAtLeastCountJson(
            logFile, logId, attrs, 1, null, /*isRelaxed=*/ true),
...
function assertNoLeak(logFile) {
    assert(!checkLog.checkContainsOnce(logFile, kSensitiveValue),
...
function testSensitiveParameterIsRedacted(conn) {
    ...
    let hasSuccess =
        hasEntry(logFile, kSetSuccessWithOldValueId, "ldapQueryPassword", kRedactedMask);
    if (!hasSuccess) {
...
function testNonSensitiveFieldsPreservedInSlowQueryLog(conn) {
    ...
    withSlowMs(admin, 0, () => {
        ...
        assert(
            checkLog.checkContainsOnce(logFile,
                                       '"internalQueryStatsCacheSize":"' + kCacheSize3 + '"'),
