...
(function() {
    ...
    function executeTest(db, isMongos) {
        ...
        (function simplePositiveCase() {
            ...
            // TODO SERVER-32565: The error should always be MaxTimeMSExpired, but there are rare
            // cases where interrupting javascript execution on the server with a stepdown or
            // timeout results in an error code of InternalError or Interrupted instead, so we also
            // accept those here.
        ...
        //
        // Simple negative test for query: a ~300ms query with a 10s time limit should not hit the
        // time limit.
        //
        ...
        // second batch takes ~15 seconds, so the getmore should be aborted.
        //
        ...
        // second batch takes only ~2 seconds, so both the query and getmore should not hit the
        // time limit.
        //
        ...
        // - Issue a many-batch find() with a 6-second time limit where the results take 10 seconds
        // to generate; one of the later getmore ops should be aborted.
        //
        ...
        //
        // Many-batch negative test for getmore: - Issue a many-batch find() with a 20-second time
        // limit where the results take 10 seconds to generate; the find() should not hit the time
        // limit.
        //
        (function manyBatchNegativeGetMore() {
            ...
            assert.doesNotThrow(function() {
                // SERVER-40305: Add some additional logging here in case this fails to help us
                // track down why it failed.
        ...
        //
        // Simple negative test for commands: a ~300ms command with a 10s time limit should not hit
        // the time limit.
        //
        ...
        (function checkValidation() {
            ...
            assert.commandFailedWithCode(db.runCommand({ping: 1, maxTimeMS: -1}),
                                         ErrorCodes.BadValue);
            ...
            assert.commandFailedWithCode(db.runCommand({ping: 1, maxTimeMS: maxValue + 1}),
                                         ErrorCodes.BadValue);
            assert.commandFailedWithCode(
                db.runCommand({ping: 1, maxTimeMS: NumberInt(maxValue + 1)}), ErrorCodes.BadValue);
            assert.commandFailedWithCode(
                db.runCommand({ping: 1, maxTimeMS: NumberLong(maxValue + 1)}), ErrorCodes.BadValue);
        ...
        (function checkFailPointsWork() {
            ...
            // maxTimeNeverTimeOut positive test for command. Don't run on mongos because there's
            // no sleep command.
            if (!isMongos) {
                try {
                    assert.commandWorked(db.adminCommand(
