...
const MissingIndexIdent = class {
    ...
    static run() {
        ...
        const uri = assert.commandWorked(testDB().runCommand({collStats: coll().getName()}))
                        .indexDetails.a_1.uri.substring('statistics:'.length);

        replTest.restart(primary(), {setParameter: {logComponentVerbosity: '{storage: 1}'}});

        MissingIndexIdent.checkRecoveryLogs(primary(), ts, uri);
        ...
        return {replTest, dbpath};
    }

    /**
     * Checks that the index was recovered.
     */
    static checkRecoveryLogs(conn, ts, uri) {
        ...
        // be dropped.
        checkLog.containsJson(conn, 22338, {uri: uri});
    }
};
