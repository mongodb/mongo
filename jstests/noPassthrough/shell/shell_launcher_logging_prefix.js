...
const pidWithDefaultLoggingPrefix = _startMongoProgram({
    args: ["echo", "hello with default logging prefix"],
});
jsTest.log.info(
    `Started program with default logging prefix, PID: ${tojson(pidWithDefaultLoggingPrefix)}`,
);
assert(
    waitProgram(pidWithDefaultLoggingPrefix) == 0,
...
assert(
    waitProgram(pidWithCustomLoggingPrefix) == 0,
