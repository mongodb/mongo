/**
 * Insert 20MB of docs in EU zone (shard 0) and 10MB in US zone (shards 1 and 2), then make sure US
 * zone gets properly balanced (configured "balancing unit" 1MB). This is a regression test for
 * SERVER-115962.
 */
...
assert.soonNoExcept(() => {
    const dataDistribution =
        mongos.getDB("admin")
            ...
            .next();
    dataDistribution.shards.forEach(
        (shard) => assert.eq(expectedDistribution[shard.shardName], shard.numOwnedDocuments),
    );
    return true;
});
