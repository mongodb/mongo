...
function runTests({label, makeFixture, teardown, getRouter, getShardRs, getMetricsConn, getDefaultsColl}) {
    ...
    try {
        const router = getRouter(fixture);
        const shardRs = getShardRs(fixture);
        const metricsConn = getMetricsConn(fixture);
        const testDB = router.getDB(dbName);
        ...
        for (const {name, cmd} of makeBlockingCommands(collName)) {
            jsTestLog(`-- ${label}: blocks ${name} with explicit afterClusterTime and no level`);
        ...
        jsTestLog(`-- ${label}: reflects merged majority level in serverStatus readConcern metrics`);
        {
            const getCounters = () => {
        ...
        jsTestLog(`-- ${label}: promotes a CWRWC level of available to local when afterClusterTime is present`);
        {
            const getCounters = () => {
                const status = assert.commandWorked(
                    metricsConn.adminCommand({serverStatus: 1, defaultRWConcern: false}),
            ...
            assert.commandWorked(
                router.adminCommand({setDefaultRWConcern: 1, defaultReadConcern: {level: "available"}}),
            );
            try {
                const insertRes = assert.commandWorked(
                    testDB.runCommand({
                ...
                const beforeLocal =
                    (beforeCounters.noneInfo && beforeCounters.noneInfo.CWRC && beforeCounters.noneInfo.CWRC.local) || 0;
                const afterLocal =
                ...
                assert.commandWorked(
                    router.adminCommand({setDefaultRWConcern: 1, defaultReadConcern: {level: "majority"}}),
                );
            }
        }

        if (getDefaultsColl) {
            jsTestLog(`-- ${label}: rejects the read when a directly-written default level cannot be merged`);
            const insertRes = assert.commandWorked(
                testDB.runCommand({
                    insert: collName,
                    documents: [{_id: nextId()}],
            ...
            // (available + afterClusterTime) is handled by the promotion above. A direct write to
            // the persisted defaults document bypasses the command-level check — a state the server
            // explicitly tolerates (see observeDirectWriteToConfigSettings) — and is the only way to
            // reach the post-merge uassert. This limits the failure to a per-request InvalidOptions
            // rather than an invalid {afterClusterTime, level: "linearizable"} combination reaching
            // the read path.
            assert.commandWorked(
        ...
        jsTestLog(`-- ${label}: blocks a causal-consistency session read when CWRWC level is majority`);
        {
            stopReplicationOnSecondaries(shardRs, false);
            let session;
            try {
...
runTests({
    ...
    // The mongos dispatcher shares the same merge code, but a direct write to the config server's
    // defaults document is not promptly visible to mongos (periodic cache refresh), so the
    // direct-write case runs only against the replica set.
