...
// should support reads and writes on both mongos via main ports
{
    for (const mongos of [st.s0, st.s1]) {
...
// should support reads and writes on the mongos secondary port
{
    // TODO SERVER-121779 hostNoPort is not set on MongoRunner.runMongos
    const host = st.s0.host.split(":")[0];
    ...
    const mainDoc = coll.findOne({_id: "main_" + st.s0.port});
    assert.neq(
        mainDoc, null, "document from main port should be readable via mongos secondary port");

    conn.close();
}

// should respond to hello on the config server secondary port
{
    const configNode = st.configRS.nodes[0];
    ...
    assert(helloRes.isWritablePrimary,
           "config server should report isWritablePrimary on secondary port");
    assert(helloRes.hasOwnProperty("setName"),
           "config server hello should include setName on secondary port");

    conn.close();
}

// should respond to hello on shard 0 secondary port
{
    const shardNode = st.rs0.nodes[0];
...
// should respond to hello on shard 1 secondary port
{
    const shardNode = st.rs1.nodes[0];
