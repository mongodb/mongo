...
(function() {
    ...
    function testWithGoodTimeZoneDir(tz_good_path) {
        ...
        assert.doesNotThrow(
            () => coll.aggregate(
                [{$project: {x_parts: {$dateToParts: {date: "$x", timezone: "GMT"}}}}]));
        assert.doesNotThrow(() => coll.aggregate([
            {$project: {x_parts: {$dateToParts: {date: "$x", timezone: "America/Sao_Paulo"}}}}
        ]));

        // Confirm that attempt to use a non-existent timezone in an expression fails.
        const err = assert.throws(
            () => coll.aggregate(
                [{$project: {x_parts: {$dateToParts: {date: "$x", timezone: "Unknown"}}}}]));
        assert.eq(err.code, 40485);

        // Test some dates which specifically exercise slim-format timezone files. The test-case
        // below
        // produced incorrect output using `$dateToParts` on `timelib-2018.01` with slim timezone
        // format
        // and the "America/New York" timezone, since timelib was unable to correctly extrapolate
        // DST
        ...
        for (let i = 0; i < test_dates.length; ++i) {
            assert.commandWorked(
                corner_coll.insert({_id: i, x: new ISODate(test_dates[i].test_date)}));
        }
        ...
        let res =
            corner_coll
                .aggregate([
                    {
                      $project:
                          {x_parts: {$dateToParts: {date: "$x", timezone: "America/New_York"}}}
                    },
                    {$sort: {_id: 1}}
                ])
                .toArray();
