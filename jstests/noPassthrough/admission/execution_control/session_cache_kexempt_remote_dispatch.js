...
// --- Test 1: session refresh (update) from secondary with primary write tickets = 0 ---
{
    ...
    const cacheStats =
        secondary.getDB("admin").serverStatus().logicalSessionRecordCache;
...
// --- Test 2: session delete (removeRecords) from secondary with primary write tickets = 0 ---
{
    ...
    assert.commandWorked(
        secondary.getDB("admin").runCommand({endSessions: [startRes.id]}));
...
// --- Test 3: findRemovedSessions (read) from secondary with primary read tickets = 0 ---
{
    ...
    const cursor = cursorSession.getDatabase("config")
                       .getCollection("system.sessions")
                       .find({})
                       .batchSize(1);
    assert(cursor.hasNext(),
           "expected at least one session document so the cursor stays open");

    const openCursorsBefore =
        secondary.getDB("admin").serverStatus().metrics.cursor.open.total;
