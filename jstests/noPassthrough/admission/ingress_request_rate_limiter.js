...
function testRateLimiterMetrics(conn, exemptConn) {
    ...
    // Ensure metrics.commands.insert is populated.
...
function testRateLimiterUnauthenticated(noAuthClient, exemptConn) {
    ...
    // Check that we added as much exempt requests as the amount of ping we ran, plus a few from the
    // exempted client. As no requests should be rate limited, running a command should only
    // increment the exempt counter.
...
function runTestCompressed() {
    ...
    // Ensure metrics.commands.insert is populated.
    exemptConn.getDB(`${jsTest.name()}_db`)[`${jsTest.name()}_coll`].insertOne({});
