...
testCommand(
    {findAndModify: kCollName, query: {x: 1}, remove: true},
    {
...
testCommand(
    {findAndModify: kCollName, query: {x: 1}, update: {$set: {x: 2}}},
    {
        "command.findAndModify": kCollName,
...
testCommand(
    {insert: kCollName, documents: [{x: 1}]},
    {"command.insert": kCollName, "command.readConcern.level": "snapshot"},
    false /*write is expected to fail*/,
);
testCommand(
    {update: kCollName, updates: [{q: {x: 1}, u: {$set: {x: 2}}}]},
