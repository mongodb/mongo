...
// Only include metadata field names that exist on v7.0 and that are still stripped by
// Document::toBsonStrippingMetadata. $sortKey is intentionally excluded on v7.0 — the
...
// $score, $scoreDetails, $stream, and $changeStreamControlEvent were added post-v7.0 and
...
const sortKeyNoSortResults =
    injectColl
        ...
        .toArray();
assert.eq(
    sortKeyNoSortResults.length, 10, `Expected 10 results, got: ${tojson(sortKeyNoSortResults)}`);
for (const r of sortKeyNoSortResults) {
    assert(!("$sortKey" in r), `$sortKey should be stripped by mongos, got: ${tojson(r)}`);
}
...
const sortKeyErr = assert.throws(
    () =>
        injectColl
            .aggregate([
                {
