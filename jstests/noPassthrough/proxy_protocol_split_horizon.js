...
// =============================================================================
jsTestLog("Test 2: SNI matching horizon through proxy → horizon hostnames");
{
    const result = helloViaProxy(proxy0, kProxyIngressPort0, kHorizonSNI);
    jsTestLog("hello response (horizon SNI): " + tojson(result));
    ...
    assert.eq(
        result.me, node0HorizonHost, `Expected 'me' to be ${node0HorizonHost}, got: ${result.me}`);
}

// =============================================================================
// Test 3: SNI matching the horizon on the second node → same horizon, different 'me'.
// =============================================================================
jsTestLog("Test 3: SNI matching horizon on node 1 → horizon hostnames, me=node1");
{
    const result = helloViaProxy(proxy1, kProxyIngressPort1, kHorizonSNI);
    jsTestLog("hello response (horizon SNI, node 1): " + tojson(result));
    ...
    assert.eq(
        result.me, node1HorizonHost, `Expected 'me' to be ${node1HorizonHost}, got: ${result.me}`);
}

// =============================================================================
// Test 4: Unknown SNI → falls back to default horizon.
// =============================================================================
jsTestLog("Test 4: Unknown SNI through proxy → default horizon (fallback)");
{
    const result = helloViaProxy(proxy0, kProxyIngressPort0, kUnknownSNI);
    jsTestLog("hello response (unknown SNI): " + tojson(result));
...
// =============================================================================
jsTestLog("Test 5: Reconfigure horizons, verify proxy SNI uses new config");
{
    ...
    assert(
        ...
        `After reconfig: expected hosts to contain ${node0HorizonHost}, got: ${
            tojson(result1.hosts)}`,
    );
    assert.eq(
        result1.me,
        node0HorizonHost,
    ...
    assert(
        ...
        `After reconfig: expected hosts to contain ${node0SecondHorizonHost}, got: ${
            tojson(result2.hosts)}`,
    );
    assert(
        result2.hosts.includes(node1SecondHorizonHost),
        `After reconfig: expected hosts to contain ${node1SecondHorizonHost}, got: ${
            tojson(result2.hosts)}`,
    );
    assert.eq(
        result2.me,
