...
assert.commandWorked(
    st.s
        .getDB(dbName)[foreignColl].insert({_id: "K", value: "joined"}, {writeConcern: {w: "majority"}}),
);
...
// therefore wrapped in withRetryOnTransientTxnError, which restarts the whole transaction the way
// a driver would; the refresh completes in the background and the retry then finds fresh routing
// info.
jsTest.log(
    "Steps 1-3: insert into local (shard0), $lookup so shard0 sub-routes to shard1 as a brand-new participant, then commit.",
);
// Each attempt uses a fresh session, so there is nothing to clean up between them.
let aggRes;
withRetryOnTransientTxnError(() => {
    ...
    jsTest.log("commit response: " + tojson({commitRes}));
});

jsTest.log("aggregate succeeded: " + tojson({firstBatch: aggRes.cursor.firstBatch}));
