    ...
    chunks.forEach((chunk) => {
        shards.forEach((shard) => {
            ...
            assert.eq(
                0,
                nRangeDelDocs,
                "Found unexpected range deletion docs on " +
                    shard.shardName +
                    " for collection " +
                    fullNs,
            );
...
function shardCollectionAndRecreateRangeDeletions(fullNs, shardKey, skipEmptyRanges) {
    ...
    assert.commandWorked(
        testDb.runCommand({recreateRangeDeletionTasks: collName, skipEmptyRanges: skipEmptyRanges}),
    );
...
shardCollectionAndRecreateRangeDeletions(
    compoundHashedShardKeyNs,
    {h: "hashed", sk0: 1, sk1: 1},
    skipEmptyRanges,
);
...
function testSkipEmptyRanges() {
    ...
    assert.commandWorked(
        testDb.runCommand({
            recreateRangeDeletionTasks: initiallyEmptyCollName,
