...
describe("mergeAllChunksOnShard scoped per-shard", function () {
    before(() => {
        ...
        assert.commandWorked(this.st.s.adminCommand({moveChunk: this.ns, find: {x: 15}, to: this.st.shard1.shardName}));
        assert.commandWorked(this.st.s.adminCommand({moveChunk: this.ns, find: {x: 35}, to: this.st.shard2.shardName}));
        assert.commandWorked(this.st.s.adminCommand({moveChunk: this.ns, find: {x: 45}, to: this.st.shard1.shardName}));
        assert.commandWorked(this.st.s.adminCommand({moveChunk: this.ns, find: {x: 55}, to: this.st.shard2.shardName}));

        this.countChunksOnShard = (shardName) =>
            findChunksUtil.findChunksByNs(this.st.s.getDB("config"), this.ns, {shard: shardName}).itcount();
    ...
    it("is a no-op against shard2 (no contiguous shard2-owned pair)", () => {
        ...
        assert.commandWorked(this.st.s.adminCommand({mergeAllChunksOnShard: this.ns, shard: this.st.shard2.shardName}));
    ...
    it("after the full sweep, exactly one merge happened (total chunk count = original - 1)", () => {
        ...
        const totalChunks = findChunksUtil.findChunksByNs(this.st.s.getDB("config"), this.ns).itcount();
