 ...
 */
const testCollModConvertUniqueWithSideWrites = function (
    initialDocs,
    performCrudOpsFunc,
    duplicateDoc = {
        _id: 100,
    ...
    try {
        ...
        if (!expectedViolations) {
            awaitCollMod = assertCommandWorkedInParallelShell(primary, testDB, {
                collMod: collName,
...
testCollModConvertUniqueWithSideWrites(initialDocsUnique, (coll) => {
    ...
    jsTestLog(
        "Successfully inserted a non-conflicting document containing an unindexed field. " +
...
testCollModConvertUniqueWithSideWrites(
    initialDocsDuplicate,
    (coll) => {
        jsTestLog("Inserting additional documents after collMod completed index scan.");
        assert.commandFailedWithCode(coll.insert({_id: 1000, a: 100}), ErrorCodes.DuplicateKey);
        jsTestLog("Failed to insert documents. Resuming collMod index conversion.");
    },
    {_id: 1000, a: 100} /* duplicateDoc */,
    [{ids: [1, 2]}, {ids: [3, 4]}] /* expectedViolations */,
);
