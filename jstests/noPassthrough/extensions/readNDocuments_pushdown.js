...
function runAllTests(coll) {
    ...
    {
        const result = coll
            .aggregate([{$readNDocuments: {numDocs: 10}}, {$match: {_id: {$gte: 5}}}])
            .toArray();
    ...
    {
        const result = coll
            .aggregate([{$readNDocuments: {numDocs: 10}}, {$match: {_id: {$gt: 5}}}])
            .toArray();
    ...
    {
        const result = coll
            .aggregate([{$readNDocuments: {numDocs: 5}}, {$match: {_id: {$gte: 10}}}])
            .toArray();
    ...
    {
        const result = coll
            .aggregate([{$readNDocuments: {numDocs: 10}}, {$match: {value: {$gte: 10}}}])
            .toArray();
    ...
    {
        const result = coll
            .aggregate([{$readNDocuments: {numDocs: 5}}, {$project: {_id: 1}}])
            .toArray();
        assert.eq(result.length, 5, "$project pushdown: expected 5 docs");
        for (const doc of result) {
            assert(!doc.hasOwnProperty("value"), "$project pushdown: unexpected value field", {
                doc,
            });
            assert(!doc.hasOwnProperty("label"), "$project pushdown: unexpected label field", {
                doc,
            });
        }
    }
    ...
    {
        const result = coll
            .aggregate([{$readNDocuments: {numDocs: 5}}, {$project: {_id: 1, value: 1}}])
            .toArray();
        assert.eq(result.length, 5, "selective suppression (value): expected 5 docs");
        for (const doc of result) {
            assert(doc.hasOwnProperty("value"), "selective suppression (value): missing value", {
                doc,
            });
            assert.eq(doc.value, doc._id * 2, "selective suppression (value): wrong value", {doc});
            assert(
                !doc.hasOwnProperty("label"),
                "selective suppression (value): unexpected label",
                {doc},
            );
        }
    }
    ...
    {
        const result = coll
            .aggregate([{$readNDocuments: {numDocs: 5}}, {$project: {_id: 1, label: 1}}])
            .toArray();
        assert.eq(result.length, 5, "selective suppression (label): expected 5 docs");
        for (const doc of result) {
            assert(doc.hasOwnProperty("label"), "selective suppression (label): missing label", {
                doc,
            });
            assert.eq(doc.label, `doc_${doc._id}`, "selective suppression (label): wrong label", {
                doc,
            });
            assert(
                !doc.hasOwnProperty("value"),
                "selective suppression (label): unexpected value",
                {doc},
            );
        }
    }
...
function runTests(conn, shardingTest) {
    ...
    assert.commandWorked(
        coll.insertMany(
            Array.from({length: 10}, (_, i) => ({_id: i, value: i * 2, label: `doc_${i}`})),
        ),
    );
