...
const testKillOnClientDisconnect = (conn) => {
    ...
    // The isConnected check will succeed on Windows because there is still data on the socket to
    // read, and the markKillOnClientDisconnect logic on Windows checks that rather than polling the
    // socket state.
    if (!_isWindows()) {
