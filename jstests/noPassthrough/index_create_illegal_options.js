...
const illegalIndexTypes = [
    ...
    {
        type: "queryable_encrypted_range",
        codes: [ErrorCodes.IndexOptionsConflict],
    },
    {type: "wildcard", codes: [7246202, ErrorCodes.CannotCreateIndex]},
    {
        type: "columnstore",
        codes: [ErrorCodes.NotImplemented, ErrorCodes.CannotCreateIndex],
    },
...
illegalIndexTypes.forEach((args) => {
    ...
    assert.commandFailedWithCode(test[testCollName].createIndex({foo: indexType}),
                                 expectedErrorCodes);
    // TODO(SERVER-114308): Primary-driven index builds eagerly create the collection, this
    // assertion will fail on those build variants. Reenable this check.
    //   assert.doesNotContain(
    //     test.getCollectionNames(),
    //     [testCollName],
    //     `The ${testCollName} collection should not be implicitly created upon failing to create
    //     the index.`
    //   );
...
legalIndexTypes.forEach(function(indexType) {
    testCollName = collName + "." + indexType;
