...
try {
    ...
    const nameField =
        FeatureFlagUtil.isPresentAndEnabled(testDB, 'featureFlagReplicateLocalCatalogIdentifiers')
        ? 'o.spec.name'
        : 'o.name';
    const ops = rst.dumpOplog(
        primary, {op: 'c', ns: cmdNs, 'o.createIndexes': emptyColl.getName(), [nameField]: 'b_1'});
    assert.eq(1,
              ops.length,
