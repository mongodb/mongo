...
describe("retryable writes during primary-driven index build", function () {
    ...
    before(function () {
        ...
        IndexBuildTest.pauseIndexBuilds(this.primary);
        ...
        IndexBuildTest.waitForIndexBuildToStart(this.primaryDB, collName, "x_1");
        ...
        IndexBuildTest.resumeIndexBuilds(this.primary);
        awaitIndex();
    ...
    it("retries work after failover", function () {
        ...
        // Step the secondary up with skipDryRun rather than ReplSetTest.stepUp(). The default
        // replSetStepUp runs a dry-run election that a priority-0 standby (e.g. under disaggregated
        // storage) can never win; skipDryRun promotes the target directly and works for both an
        // ordinary secondary and a priority-0 standby.
        jsTest.log.info("Stepping up the secondary");
        assert.soonNoExcept(() => {
            return this.secondary.adminCommand({replSetStepUp: 1, skipDryRun: true}).ok;
        }, "failed to step up the secondary");
        this.rst.awaitNodesAgreeOnPrimary();
        const newPrimary = this.rst.getPrimary();
        assert.neq(
            newPrimary.host,
            this.primary.host,
