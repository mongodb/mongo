...

{
    ...
    assert.gte(oplogWriterOp.length,
               1,
...
// killOp() on secondary.
//
jsTestLog("Kill ops on secondary");
...
try {
    ...
    assert.soon(
        () => rawMongoProgramOutput("11227300.*Not killing exempt op")
                  .match('.*"opId":' + oplogWriterOp[0].opid),
