...
// Suspend range deletions on shards in order to inspect the persisted range deletion documents in a
// stable state
shards.forEach((shard) => {
    assert.commandWorked(
        shard.adminCommand({configureFailPoint: "suspendRangeDeletion", mode: "alwaysOn"}));
});
...
shards.forEach((shard) => {
    assert.commandWorked(
        shard.adminCommand({configureFailPoint: "suspendRangeDeletion", mode: "off"}));
});
