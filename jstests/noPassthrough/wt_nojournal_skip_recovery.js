...
(function() {
    ...
    var conn = MerizoRunner.runMerizod({
        ...
        // executed and before the merizod is terminated. This is necessary to ensure that exactly 90
    ...
    // prior to the merizod being terminated.
    var awaitShell = startParallelShell(function() {
        for (let loopNum = 1; true; ++loopNum) {
            ...
            // Create a checkpoint slightly before the merizod is terminated.
            if (loopNum === 90) {
    ...
    // After some journaled write operations have been performed against the merizod, send a SIGKILL
    ...
    assert.eq(null, conn, 'merizod should not have started up because it requires recovery');
    ...
    assert.neq(null, conn, 'merizod was unable to start up after removing the journal directory');
