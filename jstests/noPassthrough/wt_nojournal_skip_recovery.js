...
(function() {
    ...
    var dbpath = BongoRunner.dataPath + 'wt_nojournal_skip_recovery';
    resetDbpath(dbpath);
    ...
    var conn = BongoRunner.runBongod({
        ...
        // executed and before the bongod is terminated. This is necessary to ensure that exactly 90
    ...
    // prior to the bongod being terminated.
    var awaitShell = startParallelShell(function() {
        for (let loopNum = 1; true; ++loopNum) {
            ...
            // Create a checkpoint slightly before the bongod is terminated.
            if (loopNum === 90) {
    ...
    // After some journaled write operations have been performed against the bongod, send a SIGKILL
    ...
    // Restart the bongod with journaling disabled, but configure it to error if the database needs
    // recovery.
    ...
    assert.eq(null, conn, 'bongod should not have started up because it requires recovery');

    // Remove the journal files.
    assert(removeFile(dbpath + '/journal'), 'failed to remove the journal directory');

    // Restart the bongod with journaling disabled again.
    conn = BongoRunner.runBongod({
        dbpath: dbpath,
        noCleanData: true,
        nojournal: '',
    ...
    assert.neq(null, conn, 'bongod was unable to start up after removing the journal directory');
