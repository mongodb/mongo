...
const ddlNs = bucketsColl.getFullName();
...
const dupes =
    coll.aggregate([{$group: {_id: "$_id", n: {$sum: 1}}}, {$match: {n: {$gt: 1}}}]).toArray();
...
assert.eq(
    numDocsInserted, numDocsToInsert, "Inserted a different number of measurements than expected");
