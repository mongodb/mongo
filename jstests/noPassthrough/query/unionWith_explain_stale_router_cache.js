...
assert.commandWorked(st.s.adminCommand({enableSharding: "union_with", primaryShard: st.shard0.shardName}));
...
assert.commandWorked(st.s.adminCommand({moveCollection: "union_with.A", toShard: st.shard2.shardName}));
...
assert.commandWorked(st.s.adminCommand({moveCollection: "union_with.B", toShard: st.shard1.shardName}));
