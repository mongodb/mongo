...
++totalShapes;
coll.find({x: {$regex: ".*"}}, {x: "$$ROOT", y: {$bsonSize: ["$$ROOT"]}})
    .limit(5)
    .toArray();
...
assert.sameMembers(
    untransformedKeyHashes,
    transformedKeyHashes,
    `untransformedEntries = ${tojson(untransformedEntries)}, transformedEntries = ${tojson(transformedEntries)}`,
);
assert.sameMembers(
