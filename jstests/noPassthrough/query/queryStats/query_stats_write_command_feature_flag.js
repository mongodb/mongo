...
const conn = MongoRunner.runMongod({
    setParameter: {featureFlagQueryStatsWriteCommand: false},
});
assert.neq(null, conn, "failed to start mongod");
const testDB = conn.getDB("test");

assert(
    FeatureFlagUtil.isPresentAndDisabled(testDB, "QueryStatsWriteCommand"),
    "featureFlagQueryStatsWriteCommand is undefined or enabled",
);
