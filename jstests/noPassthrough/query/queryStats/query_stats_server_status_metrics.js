...
function addApprox2MBOfStatsData(testDB, coll) {
    ...
    assert.gte(
        4 * testDB.serverStatus().metrics.queryStats.numEntries,
        nIterations,
...
function queryStatsStoreSizeEstimateTest(conn, testDB, coll, testOptions) {
    ...
    assert(
        fullSize >= halfWayPointSize * 1.95 && fullSize <= halfWayPointSize * 2.05,
 ...
 */
runTestWithMongodOptions({setParameter: noRateLimit}, countRateLimitedRequestsTest, {
    samplingRate: 2147483647,
    numRequests: 20,
});
...
runTestWithMongodOptions(
    {setParameter: {...noRateLimit, internalQueryStatsCacheSize: "0.00001MB"}},
 ...
 */
runTestWithMongodOptions(
    {setParameter: {...noRateLimit, internalQueryStatsCacheSize: "2MB"}},
