...
function updateTokenizationTests(ctxFn) {
    ...
    it("should tokenize pipeline update with constants", function () {
        ...
        assert.commandWorked(
            testDB.runCommand({
                update: collName,
                updates: [{q: {v: 1}, u: [{$set: {v: "$$newVal"}}], c: {newVal: 500}}],
...
runTokenizationTestsForTopology(
    ...
    {collName, initialDocs: [{v: 1}]},
    updateTokenizationTests,
);

runTokenizationTestsForTopology(
    "update command one way tokenization (Sharded)",
    () => {
