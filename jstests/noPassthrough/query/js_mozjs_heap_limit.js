...
function concurrentWorker(host, dbName, iterations) {
    ...
    for (let i = 0; i < iterations; i++) {
        try {
            ...
            if (res.ok === 1) {
                ok++;
            } else if (res.code === ErrorCodes.ExceededMemoryLimit) {
                oom++;
            } else {
                other++;
            }
        } catch (e) {
            if (e.code === ErrorCodes.ExceededMemoryLimit ||
                e.code === ErrorCodes.JSInterpreterFailure) {
                oom++;
            } else {
                other++;
            }
...
for (const legacy of [false, true]) {
    jsTest.log(`Phase 1: concurrent $where, jsUseLegacyMemoryTracking=${legacy}`);
...
jsTest.log("Phase 2: scope pool reuse, sequential mmap_bytes accumulation check");
                        ...
                        {$limit: 1},
                        {
                            $project: {
                                r: {
                                    $function: {
                                        body: function() {
                                            // ArrayBuffer backing store -> BufferAllocator -> mmap
                                            // -> record_mmap_alloc. Freed via UnmapPages ->
                                            // record_mmap_free when the GC collects this buffer on
                                            // scope return.
                            ...
                            r: {
                                $function: {
                                    body: function() {
                                        ...
                                        for (let j = 0; j < 100; j++)
                                            s.push({x: j});
                                        // Typed array backing store: BufferAllocator mmap
                                        // (mmap_bytes).
...
jsTest.log("Phase 3: concurrent GC pressure + BufferAllocator mmap");
assert.commandWorked(testDB.adminCommand({setParameter: 1, jsUseLegacyMemoryTracking: false}));
const mixedTs =
    Array.from({length: 8}, () => new Thread(mixedWorker, conn.host, testDB.getName(), 3));
                    ...
                    {$limit: 1},
                    {
                        $project: {
                            r: {
                                $function: {
                                    body: function() {
                                        // Long strings: char data -> StringBufferArena -> js_malloc
                                        // -> malloc_bytes. 10,000 x 1000-char Latin-1 strings = ~10
                                        // MB of malloc_bytes.
                                        let strs = [];
...
jsTest.log("Phase 4: concurrent malloc_bytes stress via long string allocations");
assert.commandWorked(testDB.adminCommand({setParameter: 1, jsUseLegacyMemoryTracking: false}));
const mallocTs =
    Array.from({length: 8}, () => new Thread(mallocWorker, conn.host, testDB.getName(), 3));
...
assert.eq(
    mallocResult.oom, 0, `malloc stress test: unexpected OOM errors: ${tojson(mallocResult)}`);
