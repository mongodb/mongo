...
for (const legacy of [false, true]) {
    jsTestLog(`Phase 1: concurrent $where, jsUseLegacyMemoryTracking=${legacy}`);
...
jsTestLog("Phase 2: scope pool reuse, sequential mmap_bytes accumulation check");
...
jsTestLog("Phase 3: concurrent GC pressure + BufferAllocator mmap");
assert.commandWorked(testDB.adminCommand({setParameter: 1, jsUseLegacyMemoryTracking: false}));
const mixedTs =
...
jsTestLog("Phase 4: concurrent malloc_bytes stress via long string allocations");
assert.commandWorked(testDB.adminCommand({setParameter: 1, jsUseLegacyMemoryTracking: false}));
const mallocTs =
