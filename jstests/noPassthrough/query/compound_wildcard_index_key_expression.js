...
assert.sameMembers(
    getKeys({key: {a: 1, other: 1, "$**": 1}, name: "t3d", wildcardProjection: {a: 0, other: 0}}), [
        {a: 1, other: 9, "b.x": 2},
        {a: 1, other: 9, "b.y": 3},
    ]);

assert.sameMembers(
    getKeys({key: {"b.x": 1, "$**": 1, "b.y": 1}, name: "t3d", wildcardProjection: {b: 0}}), [
        {"b.x": 2, "a": 1, "b.y": 3},
        {"b.x": 2, "other": 9, "b.y": 3},
    ]);

assert.sameMembers(getKeys({key: {"$**": 1}, name: "t3d", wildcardProjection: {a: 1, other: 1}}),
                   [{a: 1}, {other: 9}]);

assert.sameMembers(getKeys({key: {"$**": 1}, name: "t3d", wildcardProjection: {b: 1}}),
                   [{"b.x": 2}, {"b.y": 3}]);
