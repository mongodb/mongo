...
try {
    ...
    // ========================
    // Mixed plan ranking tests.
    // ========================

    coll.getPlanCache().clear();

    const nonProductiveFilterWithMultipleSolutions = {
        nonexistentField: {$exists: true},
        a: 1,
        b: 1,
    };
    ...
    // ---------------------------------------------------------------------------
    {
        ...
        try {
            ...
            assert.commandWorked(
                db.adminCommand({
                    setParameter: 1,
                    internalQueryPlanEvaluationWorks: originalKnobValue,
                }),
            );
        }
    }
    ...
    // ---------------------------------------------------------------------------
    {
        ...
        try {
            ...
            assert.commandWorked(
                coll.find({a: {$gt: 0}, b: {$gt: 0}, c: {$gt: 0}, d: 0}).explain(),
            );
