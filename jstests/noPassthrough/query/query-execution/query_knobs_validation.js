...
function assertDefaultParameterValues() {
    ...
    for (let paramName in expectedParamDefaults) {
        ...
        const getParamRes = assert.commandWorked(
            testDB.adminCommand({getParameter: 1, [paramName]: 1}),
        );
...
function assertSetParameterFails(paramName, value) {
    assert.commandFailedWithCode(
        testDB.adminCommand({setParameter: 1, [paramName]: value}),
        ErrorCodes.BadValue,
    );
}

const getParamRes = assert.commandWorked(
    testDB.adminCommand({getParameter: 1, internalPipelineLengthLimit: 1}),
);
