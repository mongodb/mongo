...
const updateQueryPredicatesCase = {
    ...
    command: (collName, constants, q = rootRolePredicate) => withRuntimeConstants(
        {
            update: collName,
...
const updateExplainCase = {
    name: "update explain",
    command: (collName, constants, q = rootRolePredicate) => explainWithRuntimeConstants(
        {
            update: collName,
            updates: [{q, u: {$set: {v: 9}}, multi: true}],
        },
...
const findAndModifyUpdateCase = {
    ...
    command: (collName, constants, query = rootRolePredicate) => withRuntimeConstants(
        {
...
const findAndModifyExplainCase = {
    ...
    command: (collName, constants, query = rootRolePredicate) => explainWithRuntimeConstants(
        {
            findAndModify: collName,
            query,
            update: {$set: {v: 9}},
...
const aggregateCase = {
    name: "aggregate",
    command: (collName, constants) => withRuntimeConstants(
        {
            aggregate: collName,
    ...
    // Unlike newer branches, mongod on 7.0 does not reject external 'runtimeConstants' for
    // aggregate outright; it only rejects a client-supplied 'userRoles'.
    mongodCode: kUserRolesNotPermitted,
    mongosCode: 51143,
...

{
    ...
    try {
        ...
        assert.commandWorked(
            adminDB.runCommand({createUser: "admin", pwd: "password", roles: ["root"]}));
        assert(adminDB.auth("admin", "password"), "admin auth failed");

        const attackerDB = conn.getDB(dbName);
        assert.commandWorked(
            attackerDB.runCommand(readWriteUserCommand(attackerUser, attackerPassword, dbName)));
        ...
        jsTest.log(
            "Test: uses the authenticated user's real roles when runtimeConstants is absent");
        {
            const noMatch = assert.commandWorked(
                attackerDB.runCommand({find: collName, filter: rootRolePredicate}));
            assert.eq([],
                      noMatch.cursor.firstBatch,
                      "root predicate should not match for a readWrite user");

            const match = assert.commandWorked(
                attackerDB.runCommand(
                    {find: collName, filter: rolePredicate("readWrite"), sort: {_id: 1}}),
            );
            assert.eq(
                seedDocs,
        ...
        for (const testCase of testCases) {
            ...
            assert.commandFailedWithCode(attackerDB.runCommand(testCase.command(collName)),
                                         testCase.mongodCode);
        }

        // On 7.0 every command, including aggregate, accepts runtime constants without
        // 'userRoles' from an external client.
        for (const testCase of testCases) {
...

{
    ...
    try {
        ...
        for (const testCase of testCases) {
            jsTest.log(
                `Test: accepts runtimeConstants.userRoles for ${testCase.name}`,
            );
            reseed();
            const command = {
                ...testCase.command(collName),
                ...testCase.internalClientFields,
            };
...

{
    ...
    try {
        ...
        for (const testCase of testCases) {
            ...
            assert.commandFailedWithCode(testDB.runCommand(testCase.command(collName)),
                                         testCase.mongosCode);
        }
        ...
        assert.commandWorked(
            testDB.runCommand(deleteCase.command(collName, benignRuntimeConstants(), {_id: -1})));
    } finally {
        st.stop();
    }
}

{
    ...
    try {
        ...
        jsTest.log(
            "Test: evaluates $$USER_ROLES against the user's real roles when routed through mongos");
        {
            const noMatch = assert.commandWorked(
