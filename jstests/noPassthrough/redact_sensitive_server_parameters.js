...
/**
 * Reads the log file and returns an array of parsed JSON log entry objects.
 */
...
/**
 * Internal helper: recursively checks that object1 satisfies object2's fields.
 * object1 may contain additional fields not in object2.
 */
function _deepEqual(object1, object2) {
    ...
    for (const key of keys2) {
        ...
        if (bothObjects) {
            if (!_deepEqual(val1, val2)) return false;
        } else {
...
/**
 * Returns true when the log file contains at least one entry whose
 * `id` matches and whose `attr` object satisfies every key in `attrsDict`.
 * Extra keys in the log entry's `attr` are ignored.
 */
function logFileContainsJson(logFile, id, attrsDict) {
    const lines = readLogLines(logFile);
    for (const obj of lines) {
        if (obj.id !== id) continue;
        if (_deepEqual(obj.attr || {}, attrsDict)) return true;
    }
    return false;
}

/**
 * Returns true if `msg` appears as a substring anywhere in the raw log file text.
 */
function logFileContainsOnce(logFile, msg) {
    return cat(logFile).includes(msg);
}
...
function assertLogEntry(logFile, logId, attrs) {
    assert(
        logFileContainsJson(logFile, logId, attrs),
...
function hasEntry(logFile, logID, parameterName, expectedValue) {
    ...
    return logFileContainsJson(logFile, logID, attrs);
}
...
function assertNoLeak(logFile) {
    assert(!logFileContainsOnce(logFile, kSensitiveValue), `Sensitive value found unredacted in log`);
}
...
function testNonSensitiveFieldsPreservedInSlowQueryLog(conn) {
    ...
    withSlowMs(admin, 0, () => {
        ...
        assert(
            logFileContainsOnce(logFile, '"internalQueryStatsCacheSize":"' + kCacheSize3 + '"'),
