...
assert.commandWorked(st.s.adminCommand({setParameter: 1, logComponentVerbosity: {transaction: {verbosity: 3}}}));
...
assert.commandWorked(st.s.getDB(dbName)[localColl].insert({_id: 1, key: "K"}, {writeConcern: {w: "majority"}}));
// Pre-populate `foreign` (sharded, on shard1) so the $lookup actually returns something.
assert.commandWorked(
    st.s.getDB(dbName)[foreignColl].insert({_id: "K", value: "joined"}, {writeConcern: {w: "majority"}}),
);
...
jsTest.log("Step 5: verify the mongos actually recorded shard1's term on first observation (log id 12812800).");
checkLog.containsJson(st.s, 12812800, {shardId: st.shard1.shardName});

jsTest.log("Step 6: drive a second transaction; the participant primaries are unchanged, so it also commits.");
const lsid2 = {id: UUID()};
