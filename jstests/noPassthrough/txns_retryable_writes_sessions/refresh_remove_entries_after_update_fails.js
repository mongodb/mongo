/**
 * Test that when refreshing the timestamp in the sessions collection fails due to errors (like
 * ShardNotFound or WriteConcernErrors), the sessions pending to be ended are still processed.
 */
...
jsTest.log.info("Session cache stats after creating 10 entries: " +
                tojson(statsAfterCreatingSessions));
assert.eq(statsAfterCreatingSessions.lastSessionsCollectionJobEntriesEnded,
          0,
...
assert.commandWorked(admin.runCommand({endSessions: endSessionsBatch1}),
...
assert.commandWorked(admin.runCommand({endSessions: endSessionsBatch2}),
...
assert.commandWorked(admin.runCommand({endSessions: endSessionsBatch3}),
...
assert.eq(statsAfter.lastSessionsCollectionJobEntriesEnded,
          2,
          "should have ended at least 8 accumulated sessions");
assert.eq(statsAfter.lastSessionsCollectionJobEntriesRefreshed,
          1,
