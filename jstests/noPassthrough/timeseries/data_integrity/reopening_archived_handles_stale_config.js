 ...
 * inserting measurements one at a time, and one of these inserts reopens an archived
...
const awaitInsert = startParallelShell(
    funWithArgs(
        function (dbName, collName, batch) {
            assert.commandWorked(db.getSiblingDB(dbName).getCollection(collName).insertMany(batch, {ordered: true}));
        },
        dbName,
        collName,
        batchToInsert,
...
const dupes = coll.aggregate([{$group: {_id: "$_id", n: {$sum: 1}}}, {$match: {n: {$gt: 1}}}]).toArray();
...
assert.eq(numDocsInserted, numDocsToInsert, "Inserted a different number of measurements than expected");
