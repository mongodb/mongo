/**
 * Tests a repair mechanism for timeseries buckets that fail strict validation. A bucket that is
 * rejected by strict validation can still be decompressed by $_internalUnpackBucket in some cases.
 * The repair uses an aggregation pipeline with $_internalUnpackBucket to extract the measurements
 * and $out to write them to a new valid timeseries collection.
 *
...
// Create a timeseries collection and insert two measurements with known values, this gives us a
// valid bucket to base the test on.
const sourceColl = db.source;
assert.commandWorked(
    db.createCollection(sourceColl.getName(), {timeseries: {timeField, metaField}}));
const sourceBucketsColl = db["system.buckets." + sourceColl.getName()];
...
const sourceBuckets = sourceBucketsColl.find().toArray();
assert.eq(1, sourceBuckets.length, "Expected one bucket: " + tojson(sourceBuckets));
const validBucket = sourceBuckets[0];

// Build an invalid bucket by replacing the _id with an OID whose embedded timestamp does not match
// control.min.t. This bucket can no longer be inserted into a timeseries collection.
...
const verifyResult = db.runCommand({
    insert: sourceBucketsColl.getName(),
...
const repairedDocs = repairedColl.find().sort({[timeField]: 1}).toArray();
