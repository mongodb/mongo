...
(function testUnixEpoch() {
    ...
    assert.eq(1, getTimeseriesCollForRawOps(db, coll).find().rawData().itcount());
})();
...
(function testUnixEpoch() {
    ...
    assert.commandWorked(coll.insert({m: 1, t: ISODate("1970-01-01T01:00:00.000Z")}));
    assert.eq(2, getTimeseriesCollForRawOps(db, coll).find().rawData().itcount());
    ...
    assert.eq(3, getTimeseriesCollForRawOps(db, coll).find().rawData().itcount());
})();
