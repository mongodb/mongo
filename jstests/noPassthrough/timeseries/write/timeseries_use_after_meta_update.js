...
assert.eq(getTimeseriesCollForRawOps(db, coll).find({meta: 1}).rawData().toArray().length, 1);
...
assert.eq(getTimeseriesCollForRawOps(db, coll).find({meta: 1}).rawData().toArray().length, 0);
assert.eq(coll.find({m: 2}).toArray().length, 1);
assert.eq(getTimeseriesCollForRawOps(db, coll).find({meta: 2}).rawData().toArray().length, 1);
...
assert.eq(getTimeseriesCollForRawOps(db, coll).find({meta: 3}).rawData().toArray().length, 1);

// Update correctly changes meta field for existing bucket.
assert.commandWorked(coll.updateMany({m: 3}, {$set: {m: 4}}));
assert.eq(coll.find({m: 3}).toArray().length, 0);
assert.eq(getTimeseriesCollForRawOps(db, coll).find({meta: 3}).rawData().toArray().length, 0);
assert.eq(coll.find({m: 4}).toArray().length, 1);
assert.eq(getTimeseriesCollForRawOps(db, coll).find({meta: 4}).rawData().toArray().length, 1);
...
assert.eq(getTimeseriesCollForRawOps(db, coll).find({meta: 5}).rawData().toArray().length, 1);
assert.commandWorked(coll.updateMany({m: 5}, {$set: {m: 6}}));
assert.eq(coll.find({m: 5}).toArray().length, 0);
assert.eq(getTimeseriesCollForRawOps(db, coll).find({meta: 5}).rawData().toArray().length, 0);
assert.eq(coll.find({m: 6}).toArray().length, 1);
assert.eq(getTimeseriesCollForRawOps(db, coll).find({meta: 6}).rawData().toArray().length, 1);
...
assert.eq(getTimeseriesCollForRawOps(db, coll).find({meta: 7}).rawData().toArray().length, 1);
assert.commandWorked(coll.updateMany({m: 7}, {$set: {m: 8}}));
assert.eq(coll.find({m: 7}).toArray().length, 0);
assert.eq(getTimeseriesCollForRawOps(db, coll).find({meta: 7}).rawData().toArray().length, 0);
assert.eq(coll.find({m: 8}).toArray().length, 1);
assert.eq(getTimeseriesCollForRawOps(db, coll).find({meta: 8}).rawData().toArray().length, 1);
