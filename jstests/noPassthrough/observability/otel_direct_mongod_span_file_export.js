 ...
 * are exported via the OTel JSONL file exporter. Covers both a write and a read command.
 *
...
describe("OTel direct-to-mongod span file export", function () {
    before(function () {
        ...
        assert(
            getEffectiveTraceDir(this.mongod),
            "mongod has no opentelemetryTraceDirectory configured",
        );
    ...
    function rootSpansForCommand(test, commandName) {
        return readClusterSpans([test.mongod]).filter(
            (span) =>
    ...
    it("exports a root span for a read command", function () {
        assert.soon(
            () => {
                assert.commandWorked(this.db.runCommand({find: this.coll.getName(), filter: {}}));
