 ...
 *
 * @tags: [requires_scripting, requires_fcv_70]
 */
(function() {
"use strict";
...
contextCases.forEach(function(tc) {
    assert.commandFailedWithCode(
        db.runCommand(tc.cmd), ErrorCodes.InvalidBSONFromJavaScript, tc.desc);
});
...
// --------------------------------------------------------------------------
{
    ...
    assert.commandWorked(
        db.createCollection(tsColl.getName(), {timeseries: {timeField: "t", metaField: "m"}}));
    ...
    assert.eq(bsonWoCompare(bucket.data.t, bucketAfter.data.t),
              0,
