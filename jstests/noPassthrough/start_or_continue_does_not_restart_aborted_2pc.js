...
st.s.getDB(dbName)[localColl]
    .aggregate([{$lookup: {from: foreignColl, localField: "key", foreignField: "_id", as: "joined"}}])
    .toArray();
...
assert(
    ...
    "refusal must carry TransientTransactionError so the whole transaction is retried: " + tojson(aggRes),
);
...
const localIds = st.s.getDB(dbName)[localColl]
    .find({}, {_id: 1})
    .sort({_id: 1})
    .toArray()
    .map((d) => d._id);
const foreignIds = st.s.getDB(dbName)[foreignColl]
    .find({}, {_id: 1})
    .sort({_id: 1})
    .toArray()
    .map((d) => d._id);
assert(!localIds.includes("X"), "X must NOT be committed (the whole transaction aborted): " + tojson(localIds));
assert(!foreignIds.includes("Y"), "Y must NOT be committed: " + tojson(foreignIds));
