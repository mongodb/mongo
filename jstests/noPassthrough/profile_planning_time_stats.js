...
load(
    "jstests/noPassthrough/explain_and_profile_optimization_stats_util.js");  // for
                                                                              // runWithFailpoint
                                                                              // and
...
function runTest(db) {
    ...
    testProfileEntryContainsPlanningTime(
        db,
        {
            ...
            failpointName: "sleepWhilePlanning",
...
const setParam = {
    setParameter: 'internalQueryDisablePlanCache=1'
};
jsTestLog("Testing standalone");
(function testStandalone() {
    const conn = MongoRunner.runMongod(setParam);
...
(function testReplicaSet() {
    const rst = new ReplSetTest({nodes: 2, nodeOptions: setParam});
    rst.startSet();
    rst.initiate();
...
(function testShardedCluster() {
    const st = new ShardingTest(
        {shards: 2, mongos: 1, config: 1, other: {enableBalancer: false, shardOptions: setParam}});
