...
/*
 * Moves the chunk that matches the given query to toShard. Forces fromShard to skip the recipient
 * metadata refresh post-migration commit.
 */
let moveChunkNotRefreshRecipient = function(mongos, ns, fromShard, toShard, findQuery) {
    assert.commandWorked(fromShard.adminCommand(
...
assert.soon(
    () => {
        moveChunkNotRefreshRecipient(st.s, ns2, st.shard0, st.shard1, {_id: 0});
        sleep(2000);
        moveChunkNotRefreshRecipient(st.s, ns2, st.shard1, st.shard0, {_id: 0});
        sleep(2000);
