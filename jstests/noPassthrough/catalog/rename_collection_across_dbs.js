...
let srcNsRegex,
    ...
    numNewDocs,
    testDescription,
    largeNumberOfDocsToExceedBatchCountLimit,
    srcDb,
    dstDb,
    srcColl,
    testNum,
...
opEntries = db
    .getSiblingDB("local")
    .oplog.rs.find({op: "c", ns: "admin.$cmd", "o.applyOps": {$exists: true}}, {"o.applyOps": 1})
    .sort({$natural: -1})
    .toArray();
...
for (let i = 0; i < opEntries.length; i++) {
    ...
    assert.eq(
        opEntries[i]["o"]["applyOps"].length,
        maxInsertsCount,
        "Each batch should contain the maximum size of " + maxInsertsCount + "entries.",
    );
}
assert.eq(
    largeNumberOfDocsToExceedBatchCountLimit / maxInsertsCount,
    opEntries.length,
    "Should be " +
        largeNumberOfDocsToExceedBatchCountLimit +
        "/" +
...
rst = new ReplSetTest({
    ...
    nodeOptions: {
        setParameter: {
            maxNumberOfInsertsBatchInsertsForRenameAcrossDatabases: BSONObjMaxUserSize, // ensure size limit is hit, not count
        },
    },
});
rst.startSet();
