...
jsTest.log("Start a transaction, insert X, read it back, then abort it.");
...
assert(
    ...
    "rejection should carry the TransientTransactionError label so the driver retries: " + tojson(res),
);

jsTest.log("No silent data loss: X was discarded by the abort and Y was rejected, so " + "neither persisted.");
const docs = st.s.getDB(dbName)[collName].find({}, {_id: 1}).sort({_id: 1}).toArray();
assert.eq([], docs, "collection should be empty: " + tojson(docs));
