...
function mockPlanShardedSearchFromMongos(count) {
    for (let i = 0; i < count; i++) {
        mockPlanShardedSearchResponse(
...
function runRequiresSearchMetaCursorTest({
    ...
    subpipelineIsLookup = false
}) {
    // Each pipeline will run planShardedSearch from mongos once for the top-level $search and once
    // for the inner-pipeline search initially. There will be one more redundant planShardedSearch
    // (which has been optimized away on versions past 7.0); the
    // subPipelinePlanShardedSearchDispatchedToShards arguments indicates if the final
    // planShardedSearch is run from mongos or from a shard. By default, we mock the shards twice,
    // once per $search. In a $lookup subpipeline, we get one more planShardedSearch from mongos
    // (also optimized away on versions past 7.0) and one more real $search query from the shards.
    let numPlanShardedSearchFromMongos = 1;
    let numSearchQueriesPerShard = 2;
    if (subPipelinePlanShardedSearchDispatchedToShards) {
        mockPlanShardedSearchFromShards();
    } else {
        numPlanShardedSearchFromMongos++;
    }
    if (subpipelineIsLookup) {
        numPlanShardedSearchFromMongos++;
        numSearchQueriesPerShard++;
    }
    mockPlanShardedSearchFromMongos(numPlanShardedSearchFromMongos);
    mockShards(numSearchQueriesPerShard);
    ...
    for (let shardDB of [shard0DB, shard1DB]) {
        ...
        if (subpipelineIsLookup) {
            // For $lookup, each shard will have two $search queries for the inner pipeline.
            assert.eq(3, res.length, res);
        } else {
        ...
        // meta cursor.
        assert.eq(true, res[1].command.pipeline[0].$search.requiresSearchMetaCursor, res);
        if (subpipelineIsLookup) {
            assert.eq(true, res[2].command.pipeline[0].$search.requiresSearchMetaCursor, res);
        }
    }
}
...
// Because SERVER-63816 was not backported to 7.0, we have to mock planShardedSearch extra times for
// $lookup. These tests use {$limit: 1} in the $lookup sub-pipeline to avoid needing to mock even
// more.
runRequiresSearchMetaCursorTest({
    pipeline: [
        {$search: mongotQuery},
