...
let productsIndexName = "productsIndex";
let categoriesIndexName = "categoriesIndex";
let suppliersIndexName = "suppliersIndex";
let customersIndexName = "customersIndex";
let ordersIndexName = "ordersIndex";
createSearchIndex(productsView, {name: productsIndexName, definition: {mappings: {dynamic: true}}});
createSearchIndex(categoriesView,
                  {name: categoriesIndexName, definition: {mappings: {dynamic: true}}});
createSearchIndex(suppliersView,
                  {name: suppliersIndexName, definition: {mappings: {dynamic: true}}});
createSearchIndex(customersView,
                  {name: customersIndexName, definition: {mappings: {dynamic: true}}});
createSearchIndex(ordersView, {name: ordersIndexName, definition: {mappings: {dynamic: true}}});
 ...
 * - Categories related to "coffee" that have suppliers in Japan OR premium customers with delivered
 * orders
 */
const unionWithFirstPipeline = [
    {$search: {index: productsIndexName, text: {query: "premium", path: "tags"}}},
    {
        $unionWith: {
...
// Assert that the top level view definition the first level $unionWith view
// definition is applied in the explain output. We don't currently have the testing infrastructure
// to check for inner $unionWith view definitions.
const unionWithFirstExplain =
    assert.commandWorked(productsView.explain().aggregate(unionWithFirstPipeline));
assertUnionWithSearchPipelinesApplyViews(unionWithFirstExplain,
                                         productsViewPipeline,
                                         categories,
                                         categoriesView,
                                         categoriesViewPipeline);

const unionWithFirstResults = productsView.aggregate(unionWithFirstPipeline).toArray();
...
assert.eq(
    unionWithFirstResults,
    unionWithFirstExpected,
);
...
const lookupFirstPipeline = [
    {$search: {index: productsIndexName, text: {query: "organic", path: "tags"}}},
    {
        $lookup: {
...
const lookupFirstExplain =
    assert.commandWorked(productsView.explain().aggregate(lookupFirstPipeline));
assertViewAppliedCorrectly(lookupFirstExplain, lookupFirstPipeline, productsViewPipeline);

const lookupFirstResults = productsView.aggregate(lookupFirstPipeline).toArray();
...
assert.eq(lookupFirstResults, lookupFirstExpected);

// Clean up search indexes.
dropSearchIndex(productsView, {name: productsIndexName});
dropSearchIndex(categoriesView, {name: categoriesIndexName});
dropSearchIndex(suppliersView, {name: suppliersIndexName});
dropSearchIndex(customersView, {name: customersIndexName});
dropSearchIndex(ordersView, {name: ordersIndexName});