...
const testUnionWithStoredSource = function() {
    ...
    let stateColorViewPipeline = [{
        "$addFields":
            {"facts.official_state_colors": {$ifNull: ["$facts.official_state_colors", []]}}
    }];
        ...
        $addFields: {
            "facts.top_halloween_candy": {
                $cond: [
                    ...
                    "reese's peanut butter cups",  // Any state not in the array will be assigned
                                                   // this.
    ...
    indexDef = {mappings: {dynamic: true}, storedSource: {exclude: ["facts.state_bird"]}};
    createSearchIndex(halloweenCandyView, {name: "excludeStateBird", definition: indexDef});

    /**
     * This query runs the outer $search query on a view and the inner unionWith.$search subpipeline
     * on a view.
     */
    let pipeline = [
        {
        ...
        {$sort: {source: 1, state: 1}}
    ];

    let explain = officialStateColorsView.explain().aggregate(pipeline);

    // Confirm top-level aggregation doesn't include view stages as it's a stored source query,
    // mongot should be returning the transformed fields.
    ...
    assertViewNotApplied(unionWithExplain, halloweenCandyViewPipeline);
