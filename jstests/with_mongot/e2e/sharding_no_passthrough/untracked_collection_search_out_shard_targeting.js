...
assert.eq(
    ...
    "Output collection should have same number of documents as search results. outputResults: " +
        tojson(outputResults),
);
assert.eq(
    outputResults.length,
    2,
    "$out should have written exactly 2 documents. outputResults: " + tojson(outputResults),
);
...
assert.eq(
    outputIds,
    searchIds,
    "Output collection documents should match search results. outputIds: " + tojson(outputIds) +
        ", searchIds: " + tojson(searchIds),
);
