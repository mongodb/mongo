 ...
 * internal routing fields (e.g. mongotQuery) and the recursive internal-field check rejects the
 * query for external clients with 5491300 — even though the user never typed those fields.
 *
...
before(function () {
    ...
    assert.commandWorked(testDb.createView(kNestedViewName, kSearchViewName, [{$match: {score: {$gt: 0}}}]));
...
describe("hybrid search as an external client", function () {
    // Case 1: canonical view-resolution path. If serializeForReparse is dropped, the resolved
    // pipeline carries mongotQuery and the recursive internal-field check rejects with 5491300.
        ...
        const pipeline = [
            {
                $rankFusion: {
                    input: {
                        pipelines: {
                            a: [{$search: {index: kSearchIndexName, text: {query: "hello", path: "body"}}}],
                            b: [{$search: {index: kSearchIndexName, text: {query: "world", path: "body"}}}],
                        },
                    },
                },
            },
    ...
    // serializeForReparse for the final reparse to see user-form $search.
