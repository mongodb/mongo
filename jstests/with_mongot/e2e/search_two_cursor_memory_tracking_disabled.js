/**
 * Confirm that $search queries that use two cursors opt out of per-operation memory tracking.
 * TODO SERVER-132669: enable memory tracking for $$SEARCH_META queries and update/delete this test.
 *
 * @tags: [
 *   requires_sharding,
...
describe("$$SEARCH_META memory tracking on a sharded collection", function () {
        ...
        shardDBs.forEach((shardDB, i) => {
            ...
            assert.commandWorked(
                shardDB.adminCommand({
                    setParameter: 1,
                    internalQueryMaxWriteToCurOpMemoryUsageBytes:
                        prevMaxWriteToCurOpMemoryUsageBytes[i],
                }),
            );
        });
    ...
    function runAndProfileShards({pipeline, runOn = searchColl}) {
        ...
        return {
            results,
            entriesFor: (coll) =>
                entriesByShard.map((entries) =>
                    entries.filter((entry) => entry.ns === coll.getFullName()),
                ),
        };
    }
     ...
     * not the shard half of a search query. With featureFlagSearchExtension on, mongos dispatches
     * the search as an extension stage wrapped in the internal document-results-and-metadata
     * container, whose 'returnCursor' is what 'requiresSearchMetaCursor' is on classic $search.
     */
    function metaCursorFlagOf(entry) {
        ...
        if (Object.hasOwn(stage, kDocumentResultsAndMetadataStage)) {
            return stage[kDocumentResultsAndMetadataStage].returnCursor;
        }
        return null;
    }
    ...
    function assertRequiresSearchMetaCursor(entriesByShard, expected) {
        ...
        const searchEntries = entriesByShard
            .flat()
            .filter((entry) => metaCursorFlagOf(entry) !== null);
    ...
    it("still reports memory metrics when only the results cursor is needed", function () {
        // The same query without the $$SEARCH_META reference: no metadata cursor, no opt-out.
        const {results, entriesFor} = runAndProfileShards({
            pipeline: [{$search: kSearchQuery}, kGroup],
        });
        assert.eq(kNumDocs, results.length, "expected one group per distinct 'k'");

        const entries = entriesFor(searchColl);
        assertRequiresSearchMetaCursor(entries, false);
        assertReportsMemoryMetrics(entries);
    });
});
