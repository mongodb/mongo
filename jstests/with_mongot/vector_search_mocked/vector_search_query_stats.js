...
function runTest(conn) {
...

    {
        ...
        assert.docEq({sum: 0, min: 0, max: 0, sumOfSquares: 0}, vectorSearchMetrics.numCandidatesLimitRatio, entry);
    }

    {
        // Second shape, with one execution.
        const entry = stats[1];
        assert.docEq({$vectorSearch: {filter: {a: {$gt: "?number"}}}}, entry.key.queryShape.pipeline[0], entry);
        assert.eq(entry.metrics.execCount, 1, entry);
