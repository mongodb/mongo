...
outputAggregationPlanAndResults(coll, [
    {$group: {_id: "$d", accum: {$top: {sortBy: {d: -1}, output: "$c"}}}},
]);
outputAggregationPlanAndResults(coll, [
    {$sort: {d: -1}},
    {$group: {_id: "$d", accum: {$first: "$c"}}},
]);
...
outputAggregationPlanAndResults(coll, [
    {$group: {_id: "$a", accumB: {$first: "$b"}, accumC: {$first: "$c"}}},
]);
