...
try {
    ...
    // We only support "complex renames" where the field path is 2 components long. In this case, the
    // field path has three components, so we don't expect the match to be pushed down.
    queries = [{zip: 42}];
    runFindOnPipeline(pipeline, queries);
    ...
    // it should be pushed down past the first second $lookup-$unwind pair and into the subpipeline of
    // the first $lookup-$unwind pair.
