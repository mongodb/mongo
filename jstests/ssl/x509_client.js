...
if (cmdOut.ok) {
    TestData.authMechanism = "BONGODB-X509";  // SERVER-10353
}
...
var SERVER_USER = "C=US,ST=New York,L=New York City,O=BongoDB,OU=Kernel,CN=server";
var INTERNAL_USER = "C=US,ST=New York,L=New York City,O=BongoDB,OU=Kernel,CN=internal";
var CLIENT_USER = "C=US,ST=New York,L=New York City,O=BongoDB,OU=KernelUser,CN=client";
var INVALID_CLIENT_USER = "C=US,ST=New York,L=New York City,O=BongoDB,OU=KernelUser,CN=invalid";

function authAndTest(bongo) {
    external = bongo.getDB("$external");
    test = bongo.getDB("test");
    ...
    assert(!external.auth({user: INVALID_CLIENT_USER, mechanism: 'BONGODB-X509'}),
    ...
    assert(external.auth({mechanism: 'BONGODB-X509'}),
           "authentication with valid client cert and no user field failed");
    assert(external.runCommand({authenticate: 1, mechanism: 'BONGODB-X509', user: CLIENT_USER}).ok,
           "runCommand authentication with valid client cert and user field failed");
    assert(external.runCommand({authenticate: 1, mechanism: 'BONGODB-X509'}).ok,
...
print("1. Testing x.509 auth to bongod");
var x509_options = {sslMode: "requireSSL", sslPEMKeyFile: SERVER_CERT, sslCAFile: CA_CERT};

var bongo = BongoRunner.runBongod(Object.merge(x509_options, {auth: ""}));

authAndTest(bongo);
BongoRunner.stopBongod(bongo.port);

print("2. Testing x.509 auth to bongos");
...
authAndTest(new Bongo("localhost:" + st.s0.port));
