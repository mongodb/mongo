(function() {
    ...
    var checkShellOkay = function(url) {
        ...
        var argv = ['./bongo', url, '--eval', ('db.runCommand({replSetGetStatus: 1})')];
        if (!_isWindows()) {
        ...
        return runBongoProgram(...argv);
    };

    var noMentionSSLURL = `bongodb://${nodeList}/admin?replicaSet=${replTest.name}`;
    ...
    var useSSLURL = `bongodb://${nodeList}/admin?replicaSet=${replTest.name}&ssl=true`;
    jsTestLog(`Replica set url (uses SSL): ${useSSLURL}`);
    assert.eq(checkShellOkay(useSSLURL), 0, "successfully connected with SSL");

    var disableSSLURL = `bongodb://${nodeList}/admin?replicaSet=${replTest.name}&ssl=false`;
