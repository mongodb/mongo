...
try:
    ...
    OBJ_create(
        b"1.3.6.1.4.1.34601.2.1.1", b"mongoRoles", b"Sequence of MongoDB Database Roles"
    )
    OBJ_create(
        b"1.3.6.1.4.1.34601.2.1.2",
...
MUST_STAPLE_VALUE_STR = (
    "DER:30:03:02:01:05"  # ASN.1 value: SEQUENCE { INTEGER 0x05 (5 decimal) }
)
...
def load_authority_file(issuer):
    ...
    if ca_cert:
        ...
        signing_key = OpenSSL.crypto.load_privatekey(
            OpenSSL.crypto.FILETYPE_PEM, pem, passphrase=passphrase
        )
...
def set_general_list_extension(x509, exts, cert, name, values):
    ...
    exts.append(
        OpenSSL.crypto.X509Extension(
            name.encode("utf-8"), critical, ",".join(tags).encode("utf-8"), subject=x509
        )
    )
...
def set_ocsp_extension(x509, exts, cert):
    ...
    exts.append(
        OpenSSL.crypto.X509Extension(
            b"authorityInfoAccess", False, ocsp.encode("utf-8"), subject=x509
        )
    )
...
def set_no_check_extension(x509, exts, cert):
    ...
    exts.append(
        OpenSSL.crypto.X509Extension(
            b"noCheck", False, "this-value-ignored".encode("utf8"), subject=x509
        )
    )
...
def set_tls_feature_extension(x509, exts, cert):
    ...
    exts.append(
        OpenSSL.crypto.X509Extension(
            MUST_STAPLE_KEY, False, MUST_STAPLE_VALUE, subject=x509
        )
    )
...
def set_san_extension(x509, exts, cert):
    ...
    for typ, vals in san.items():
        ...
        if not typ in [
            "IP",
            "DNS",
        ]:  # Other things can live here, but this is all we use.
    ...
    exts.append(
        OpenSSL.crypto.X509Extension(
            b"subjectAltName", critical, ",".join(sans).encode("utf-8"), subject=x509
        )
    )
...
def enable_subject_key_identifier_extension(x509, exts, cert):
    ...
    exts.append(
        OpenSSL.crypto.X509Extension(
            b"subjectKeyIdentifier", ident == "hash-critical", b"hash", subject=x509
        )
    )
...
def to_der_sequence_pair(name, value):
    ...
    return (
        b"\x30" + to_der_varint(len(bin_name) + len(bin_value)) + bin_name + bin_value
    )
...
def set_mongo_roles_extension(exts, cert):
    ...
    value = (
        b"DER:31" + binascii.hexlify(to_der_varint(len(pair))) + binascii.hexlify(pair)
    )
...
def set_crl_distribution_point_extension(exts, cert):
    ...
    exts.append(
        OpenSSL.crypto.X509Extension(
            b"crlDistributionPoints", False, (",".join(uris)).encode("utf-8")
        )
    )
...
def set_extensions(x509, cert):
    ...
    exts = []
    set_general_dict_extension(
        x509, exts, cert, "basicConstraints", {"CA": False, "pathlen": 0}
    )
    set_general_list_extension(
        x509,
        exts,
        cert,
        "keyUsage",
    ...
    if ns_comment:
        exts.append(
            OpenSSL.crypto.X509Extension(
                b"nsComment", False, ns_comment.encode("utf-8")
            )
        )
...
def get_header_comment(cert):
    ...
    comment = (
        comment + "# Generate using jstests/ssl/x509/mkcert.py --config " + CONFIGFILE
    )
...
def create_cert(cert):
    ...
    if cert.get("keyfile", False) and cert.get("crtfile", False):
        ...
        open(make_filename(crt_path_dict), "wt").write(
            header
            + OpenSSL.crypto.dump_certificate(OpenSSL.crypto.FILETYPE_PEM, x509).decode(
                "ascii"
            )
        )
        ...
        open(make_filename(key_path_dict), "wt").write(
            ...
            + OpenSSL.crypto.dump_privatekey(
                OpenSSL.crypto.FILETYPE_PEM, key, cipher=cipher, passphrase=passphrase
            ).decode("ascii")
        )
...
def check_special_case_keys(cert):
    ...
    if not keys.issubset(allowed_keys):
        ...
        raise ValueError(
            "Unexpected fields in special entry: "
    ...
    if not required_keys.issubset(keys):
        ...
        raise ValueError(
            "Missing fields in special entry: " + ", ".join(map(str, missing_keys))
        )
...
def process_ecdsa_cert(cert, pem, key, dest, filename, split_pem=True):
    ...
    open(dest, "wt").write(
        open(pem, "rt").read() + open(temp_key_filename, "rt").read()
    )
...
def process_ecdsa_ca(cert):
    ...
    serial = str(random.randint(1000, 0x7FFFFFFF))
    subject = (
        "/C=US/ST=New York/L=New York City/O=MongoDB/OU=Kernel/CN=Kernel Test ESCDA CA/"
    )
    ...
    ecparamargs = (
        ["openssl", "ecparam", "-name", "prime256v1", "-genkey", "-out", key, "-noout"]
...
def process_cert(cert):
    ...
    if cert["name"] in [
        "ecdsa-client.pem",
        "ecdsa-server.pem",
        "ecdsa-server-ocsp.pem",
        "ecdsa-server-ocsp-mustStaple.pem",
        "ecdsa-ocsp-responder.crt",
    ]:
    ...
    for append_cert in append_certs:
        ...
        if not x509:
            raise ValueError(
                "Unable to find certificate '" + append_cert + "' to append"
            )
        header = (
            "# Certificate from " + append_cert + "\n"
            if cert.get("include_header", True)
            else ""
        )
...
def parse_command_line():
    ...
    parser.add_argument(
        "--config", help="Certificate definition file", type=str, default=CONFIGFILE
    )
    parser.add_argument(
        "cert", nargs="*", help="Certificate to generate (blank for all)"
    )
        ...
        ret.update(
            {
                cert["name"]: cert
                for cert in CONFIG["certs"]
                if [True for append in cert.get("append_cert", []) if append in names]
            }
        )
...
def sort_items(items):
    ...
    while len(ret) != len(items):
        for cert in items:
            ...
            has_issuer = (
                (issuer == "self")
                or (issuer in processed_names)
                or (issuer not in all_names)
            )
