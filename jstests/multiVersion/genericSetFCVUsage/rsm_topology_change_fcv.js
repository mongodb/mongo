...
function runTest(downgradeFCV) {
    ...
    const st = new ShardingTest({
        mongos: [{setParameter: {replicaSetMonitorProtocol: "sdam"}}],
        config: 1,
        shards: 0,
    });
    ...
    const downgradedWireVersion =
        downgradeFCV === lastContinuousFCV
            ? latestWireVersion - 1
            : latestWireVersion - numVersionsSinceLastLTS;
    const downgradeRegex = makeTopologyChangeLogMsgRegex(
        st.configRS,
        downgradedWireVersion,
        latestWireVersion,
    );
    const latestRegex = makeTopologyChangeLogMsgRegex(
        st.configRS,
        latestWireVersion,
        latestWireVersion,
    );
