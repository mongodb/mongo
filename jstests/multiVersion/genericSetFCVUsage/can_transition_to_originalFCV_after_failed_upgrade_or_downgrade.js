...
function testConfigFails(originalFCV, targetFCV, adminDBs) {
    ...
    assert.commandWorked(adminDBs.config.runCommand(
...
function testMixedFailures(originalFCV, targetFCV, adminDBs) {
    ...
    assert.commandWorked(
        adminDBs.shard0.runCommand({configureFailPoint: failPointName, mode: "alwaysOn"}));
    assert.commandWorked(adminDBs.shard1.runCommand(
...
function runShardingTests() {
    ...
    if (FeatureFlagUtil.isPresentAndEnabled(adminDBs.mongos, "UpgradingToDowngrading")) {
        testCases.push({originalFCV: lastLTSFCV, targetFCV: latestFCV},
