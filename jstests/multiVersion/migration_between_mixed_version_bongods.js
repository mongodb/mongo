//
// Testing migrations between latest and last-stable bongod versions, where the
...
(function() {
    ...
    var options = {
        ...
        bongos: 1,
        other: {bongosOptions: {binVersion: "last-stable"}}
    };
    ...
    var bongos = st.s0, admin = bongos.getDB('admin'),
        ...
        fooDB = "fooTest", fooNS = fooDB + ".foo", fooColl = bongos.getCollection(fooNS),
        ...
        barDB = "barTest", barNS = barDB + ".foo", barColl = bongos.getCollection(barNS),
    ...
    assert.eq(1,
              ...
              "Foo collection migration failed. " +
                  "Last-stable -> latest bongod version migration failure.");
    assert.eq(1,
              fooDonorColl.count(),
    ...
    assert.eq(1,
              ...
              "Bar collection migration failed. " +
                  "Latest -> last-stable bongod version migration failure.");
    assert.eq(1,
              barDonorColl.count(),
