...
// FixtureHelpers (e.g. numberOfShardsForCollection) consults the global `db` handle rather than
// using coll.getDB(), so ensure it points at this test DB on the provided connection.
...
export function wrapOptionsWithExtensionsInsideHybridSearchEnabled(baseOptions) {
    return {
        ...baseOptions,
        setParameter: {
            ...baseOptions.setParameter,
...
export function assertSearchViewCreationAllowed(primaryConn, isSearchMeta = false) {
    ...
    if (!isSearchExtensionEnabled(primaryConn)) {
        assert.throwsWithCode(
            () => db[searchViewName].aggregate([]).toArray(),
            ErrorCodes.SearchNotEnabled,
        );
    ...
    // On a sharded cluster, extension $search/$searchMeta on views requires
    // featureFlagExtensionsInsideHybridSearch. When that flag is off, the mongos view resolution
    // path triggers an IFR kickback to the legacy parser, which fails without mongot. On a replica
    // set the view resolution is inline and does not hit this kickback.
    if (
        FixtureHelpers.isMongos(db) &&
        !FeatureFlagUtil.isPresentAndEnabled(primaryConn, "ExtensionsInsideHybridSearch")
    ) {
        assert.throwsWithCode(
            () => db[searchViewName].aggregate([]).toArray(),
            ErrorCodes.SearchNotEnabled,
...
function isSearchExtensionEnabled(primaryConn) {
    const searchExtFlag = primaryConn.getDB("admin").runCommand({
        getParameter: 1,
        featureFlagSearchExtension: 1,
    }).featureFlagSearchExtension;
    return searchExtFlag && searchExtFlag.currentlyEnabled;
}
...
export function assertOnlyRouterHasIFRFlagAndExtensionSearchUsed(
    ...
    assertFullExtensionSearchBehavior(primaryConn, isSearchMeta);
}
...
export function assertOnlyShardHasIFRFlagAndLegacySearchUsed(
    ...
    assertFullLegacySearchBehavior(primaryConn, isSearchMeta);
}
