 ...
 *
 * This test must live in the multiVersion suite rather than under jstests/sharding/: it starts a
 * last-lts mongos, and the last-lts binaries are only provisioned in the multiversion suites. In
 * the generic sharding suites (which install only 'latest' binaries), the shell cannot find the
 * requested binary ('mongos-9.0' on master, 'mongos-8.0' on v9.0) and ShardingTest setup fails
 * (BF-46297). The multiVersion suite fixtures provision the downgrade binary, so the cluster comes
 * up there.
 *
...
describe("$graphLookup views across shards when the hybrid-search flag is disabled", function () {
    before(function () {
        // last-lts mongos over latest shards. On a branch whose last-lts is < 9.0 (e.g. v9.0, where
        // last-lts is 8.0), the mongos sends no ifrFlags, so the shards install
        // featureFlagExtensionsInsideHybridSearch as disabled and exercise the flag-disabled
        // serialization path. On master last-lts is already 9.0, so the mongos sends the flag toggled
        // on and the test passes trivially as a regression guard.
