...
function setupCollection(primaryConnection, shardingTest = null) {
    ...
    // shard 1: {_id: 6, ..., 11}
    if (shardingTest) {
        shardingTest.shardColl(coll, {_id: 1} /*key*/, {_id: docs.length} /*split*/);
        ...
        assert.eq(
            ...
            docs.length,
            `Expected ${docs.length} docs on shard 0 in data distribution` + tojson(shardedDataDistribution),
        );
        assert.eq(
            shardedDataDistribution[0].shards[1].numOwnedDocuments,
            docs.length,
            `Expected ${docs.length} docs on shard 1 in data distribution` + tojson(shardedDataDistribution),
        );
    }
}
