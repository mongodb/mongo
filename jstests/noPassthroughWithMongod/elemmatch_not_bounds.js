...
assert.commandWorked(c.createIndex({"th.sa": 1}));
...
// Able to use index bounds on th.sa with another $not query.
runTest(
    {
        "$and": [
            {th: {"$elemMatch": {sa: {"$not": {"$eq": "ABBA"}}}}},
        ],
    },
    [
        {
            "th.sa": ["[MinKey, \"ABBA\")", "(\"ABBA\", MaxKey]"],
        },
    ],
);

// Change the index.
assert.commandWorked(c.dropIndex({"th.sa": 1}));
