...
// "setClientRPCProtocols" method on the Merizo object.
...
(function() {
    ...
    function runInShell(rpcProtocol, func) {
        assert(0 == _runMerizoProgram("merizo",
    ...
    runInShell(RPC_PROTOCOLS.OP_QUERY, function() {
        assert(db.getMerizo().getClientRPCProtocols() === "opQueryOnly");
    ...
    runInShell(RPC_PROTOCOLS.OP_MSG, function() {
        assert(db.getMerizo().getClientRPCProtocols() === "opMsgOnly");
    ...
    runInShell(RPC_PROTOCOLS.OP_MSG, function() {
        assert(db.getMerizo().getClientRPCProtocols() === "opMsgOnly");
        db.getMerizo().setClientRPCProtocols("opQueryOnly");
        assert(db.getMerizo().getClientRPCProtocols() === "opQueryOnly");
    ...
    runInShell(RPC_PROTOCOLS.OP_QUERY, function() {
        assert(db.getMerizo().getClientRPCProtocols() === "opQueryOnly");
        db.getMerizo().setClientRPCProtocols("opMsgOnly");
        assert(db.getMerizo().getClientRPCProtocols() === "opMsgOnly");
