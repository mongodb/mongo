...
// Test listIndex cursors.
{
    ...
    const listIndCursorId1 = assert.commandWorked(
        db.runCommand({listIndexes: coll.getName(), cursor: {batchSize: 2}}),
    ).cursor.id;
    ...
    const listIndCursorId2 = assert.commandWorked(
        db.runCommand({listIndexes: coll.getName(), cursor: {batchSize: 2}}),
    ).cursor.id;
...
// Test listCollections cursors.
{
    ...
    const listCollsCursorId1 = assert.commandWorked(
        db.runCommand({listCollections: 1, cursor: {batchSize: 2}}),
    ).cursor.id;
    ...
    // Use this cursor with a getMore.
    const listCollsCursorId2 = assert.commandWorked(
        db.runCommand({listCollections: 1, cursor: {batchSize: 2}}),
    ).cursor.id;
    assert.commandWorked(
