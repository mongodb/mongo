...
assert.eq(output.nInvalidDocuments, 1, "validate returned an invalid number of invalid documents: " + tojson(output));
assert.eq(output.nrecords, 1, "validate returned an invalid number of records: " + tojson(output));
assert.eq(output.corruptRecords, [NumberLong(1)], "validate returned an invalid corruptRecords: " + tojson(output));
