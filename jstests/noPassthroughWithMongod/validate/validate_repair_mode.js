...
describe("It detects corrupt documents in validation", () => {
    ...
    it("Ensures validate detects the corrupt document", () => {
        ...
        assert.eq(
            output.valid,
            false,
            "validate returned valid true when expected false: " + tojson(output),
        );
        assert.eq(
            output.repaired,
            false,
        ...
        assert.eq(
            output.nrecords,
            1,
            "validate returned an invalid number of records: " + tojson(output),
        );
        assert.eq(
    ...
    it(
        ...
        () => {
            ...
            assert.eq(
                output.valid,
                true,
                "validate returned valid false when expected true" + tojson(output),
            );
            assert.eq(
                output.repaired,
                true,
                "validate returned repaired false when expected true" + tojson(output),
            );
            assert.eq(
                output.nInvalidDocuments,
                0,
            ...
            assert.eq(
                ...
                1,
                "validate returned an invalid number of removed extra index entries" +
                    tojson(output),
            );
            assert.eq(
                output.keysPerIndex._id_,
                0,
                "expected 0 keys in index _id: " + tojson(output),
            );
            assert.eq(
            ...
            assert.eq(
                revalidateOutput.valid,
                true,
                "validate returned valid false when expected true",
            );
            assert.eq(
                revalidateOutput.repaired,
                false,
                "validate returned repaired true when expected false",
            );
        },
    );
});
