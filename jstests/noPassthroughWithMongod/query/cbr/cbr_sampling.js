...
try {
    ...
    assertAllPlansUseSampling(
        {b: {$gte: 0}, c: {$exists: true}, $expr: {$divide: [42, "$a"]}},
        494.949494949495,
    );
    ...
    // order, and that the costs of the two candidate plans are strictly positive and distinct.
    {
        ...
        function assertCompoundIndexWinsExactZero() {
            ...
            assert.gt(
                ...
                0,
                "expected at least one rejected plan to compare costs",
            );
    ...
    assert.commandWorked(
        db.adminCommand({
            ...
            internalQuerySamplingBySequentialScan:
                prevSamplingConfig.internalQuerySamplingBySequentialScan,
