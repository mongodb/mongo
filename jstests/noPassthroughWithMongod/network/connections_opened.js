...
function createPersistentConnection() {
    assert.soon(
        ...
        "Timed out waiting for persistent connection to connect",
        30000,
        5000,
    );
}
...
function waitForConnections(expectedCurrentConnections, expectedTotalConnections) {
    assert.soon(
        ...
        () => {
            return (
                "Incorrect connection numbers. Expected " +
                expectedCurrentConnections +
...
waitForConnections(
    originalConnInfo.current + numPerTypeToCreate * 2,
