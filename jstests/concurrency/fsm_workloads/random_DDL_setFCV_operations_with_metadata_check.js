...
export const $config = extendWorkload($baseConfig, function($config, $super) {
    ...
    // Number of maximum number of move collections to db primary attempts before throwing an error.
    $config.data.kNumMoveCollsToPrimaryAttemps = 10;
    $config.data.kAcceptedFindErrors = [ErrorCodes.Interrupted];
    ...
    $config.data.kAcceptedMoveCollectionErrors = [
        ...
        // setFCV might abort ongoing resharding operations.
    ...
    // Auxiliary function to move all collection to their database primary shard.
    $config.data.moveCollectionsToDbPrimary = function(db) {
        jsTestLog('Trying to move collections to Db Primary in moveCollectionsToDbPrimary');
        for (let i = 0; i < $config.data.kNumMoveCollsToPrimaryAttemps; ++i) {
            try {
                db.getSiblingDB('config').databases.find().toArray().forEach((dbObj) => {
                    let collections = db.getSiblingDB('config')
                                          .collections
                                          .find({
                                              _id: new RegExp(dbObj._id),
                                              unsplittable: true,
                                              reshardingFields: {$exists: false}
                                          })
                                          .toArray();
                    collections.forEach((collObj) => {
                        db.getSiblingDB('config')
                            .chunks.find({uuid: collObj.uuid, shard: {$ne: dbObj.primary}})
                            .toArray()
                            .forEach((chunk) => {
                                $config.data.moveCollectionHelper(db, collObj._id, dbObj.primary);
                            });
                    });
                });
                return;
            ...
            // Backoff, sleep before retrying the entire operation.
            sleep(500);
            jsTestLog('Retrying moving collections to dbPrimary attempt ' + i);
        }
    };
    ...
    $config.states.moveCollection = function(db, collName, connCache) {
        ...
        try {
            ...
            if (!$config.data.kAcceptedFindErrors.includes(e.code)) {
                throw e;
            }
        }
    ...
    // This jstest adds new errors that can occur due to concurrent execution with resharding.
