...
import {$config as $baseConfig} from "jstests/concurrency/fsm_workloads/random_moveChunk/random_moveChunk_timeseries_meta_updates.js";
...
export const $config = extendWorkload($baseConfig, function ($config, $super) {
    ...
    $config.states.arbitraryUpdate = function (db, collName, connCache) {
        ...
        const res = assert.commandWorked(shardedColl.update({}, {$inc: {"updateCount": 1}}, {multi: true}));
    ...
    $config.data.validateCollection = function validate(db, collName) {
        ...
        const loggedUpdateCount = db[logCollection].aggregate(updateCountAggregation).toArray()[0].count;
    ...
    $config.transitions = {
        ...
        bucketLevelUpdate: {insert: 0.2, moveChunk: 0.1, bucketLevelUpdate: 0.2, arbitraryUpdate: 0.5},
        arbitraryUpdate: {insert: 0.2, moveChunk: 0.1, bucketLevelUpdate: 0.2, arbitraryUpdate: 0.5},
