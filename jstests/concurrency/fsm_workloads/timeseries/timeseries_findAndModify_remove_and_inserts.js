...
import {$config as $baseConfig} from "jstests/concurrency/fsm_workloads/query/timeseries/timeseries_deletes_and_inserts.js";
    ...
    $config.states.findAndRemove = function findAndRemove(db, collName) {
        ...
        for (let sensorId = 0; sensorId < this.nSensors; ++sensorId) {
            ...
            if (res.lastErrorObject.n) {
                const errMsg = `Deleted measurement ${res.value} should match the query predicate ${tojson({
                    readingNo: readingNo,
                    sensorId: sensorId,
                })} and all fields`;
