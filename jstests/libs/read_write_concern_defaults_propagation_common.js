export var ReadWriteConcernDefaultsPropagation = (function () {
    ...
    function setDefaultsAndVerifyPropagation(setConn, checkConns, inMemory) {
        ...
        let initialSetConnDefaults = assert.commandWorked(
            setConn.adminCommand({getDefaultRWConcern: 1}),
        );
        ...
        assert.soon(
            () => {
                ...
                return initialCheckConnsDefaults.every((checkConnDefaults) =>
                    kSetFields.every((field) =>
                        friendlyEqual(checkConnDefaults[field], initialSetConnDefaults[field]),
                    ),
                );
            },
            () =>
        ...
        assert.hasFields(
            newDefaultsRes,
            kExtraFields,
            "missing field in result of setDefaultRWConcern",
        );
        ...
        assert.soon(
            () =>
                checkConns.every((checkConn) => {
                    ...
                    const unexpectedFields = kDefaultRWCFields
                        .concat(kExtraSetFields)
                        .filter((field) => {
                            return (
