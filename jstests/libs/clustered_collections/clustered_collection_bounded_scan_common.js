...
export const testClusteredCollectionBoundedScan = function(coll, clusterKey, checkProfile) {
    ...
    function assertForwardBackwardConsistency(filter, comment) {
        const forward = coll.find(filter).sort({[clusterKeyFieldName]: 1}).toArray();
        const backward = coll.find(filter).sort({[clusterKeyFieldName]: -1}).toArray();
    ...
    function testBackwardBoundedScans() {
        ...
        assertForwardBackwardConsistency({[clusterKeyFieldName]: {$gt: 20, $lt: 40}},
                                         "backward $gt/$lt");

        // Both bounds inclusive.
        assertForwardBackwardConsistency({[clusterKeyFieldName]: {$gte: 20, $lte: 40}},
                                         "backward $gte/$lte");

        // Mixed bounds.
        assertForwardBackwardConsistency({[clusterKeyFieldName]: {$gte: 20, $lt: 40}},
                                         "backward $gte/$lt");
        assertForwardBackwardConsistency({[clusterKeyFieldName]: {$gt: 20, $lte: 40}},
                                         "backward $gt/$lte");
    }
