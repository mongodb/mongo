 ...
 *
 * This suite contains only cases that are behaviorally identical between the two copies. Cases
 * whose expected output depends on the shell-only colorized patchDiff formatting (absent in the
 * server copy) live in shell_assertions.js. The shared helpers and `kAttr` they reuse are
 * exported below.
 */
...
/**
 * Runs 'assertFailureTriggerFn' and asserts it throws an error whose message equals 'msg'.
 */
export function assertThrowsError(assertFailureTriggerFn, msg) {
    try {
...
/**
 * Like 'assertThrowsError', but forces JSON log format and additionally checks that the thrown
 * error's 'extraAttr' matches 'attr'.
 */
export function assertThrowsErrorWithJson(assertFailureTriggerFn, {msg, attr}) {
