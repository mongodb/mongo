 ...
 * de-modularized server copy (src/mongo/scripting/mozjs/server/assert.js).
 *
 * `colorizedDiff` gates the cases that assert on the shell-only colorized patchDiff failure
 * messages (absent in the server copy); all pass/fail/throw behavior runs unconditionally.
 */
...
export function runAssertionTests({colorizedDiff = true} = {}) {
    ...
    // The colorized patchDiff failure-message formatting is shell-only; the server copy of
    // assert.js omits it, so these message-format cases run only when colorizedDiff is set.
    if (colorizedDiff) {
