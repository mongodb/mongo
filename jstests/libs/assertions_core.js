/**
 * Shared assertion-library test suite. Exercises the global `assert`/`doassert`/`sortDoc`/
 * `formatErrorMsg` -- whichever copy is installed by the caller. shell_assertions.js runs it
 * against the shell's module assert.js; server_assertions.js runs it against the
 * de-modularized server copy (src/mongo/scripting/mozjs/server/server_assert.js).
 */
export function runAssertionTests() {
    const tests = [];
    ...
    tests.push(
        function callingDoAssertWithStringPassedAsFunctionThrowsException() {
            const expectedError = "hello world";
            const actualError = assert.throws(() => {
                doassert(() => {
    ...
    tests.push(function callingDoAssertCorrectlyAttachesWriteErrors() {
        ...
        let error = assert.throws(
            () => doassert(errorMessage, new BulkWriteError(bulkResult)),
        );
        assert.eq(error.writeErrors[0].code, bulkResult.writeErrors[0].code);

        error = assert.throws(
            () => doassert(errorMessage, new BulkWriteResult(bulkResult)),
        );
        assert.eq(error.writeErrors[0].code, bulkResult.writeErrors[0].code);

        error = assert.throws(
            () => doassert(errorMessage, new WriteResult(bulkResult)),
        );
        assert.eq(error.writeErrors[0].code, bulkResult.writeErrors[0].code);
    ...
    tests.push(function callingDoAssertCorrectlyAttachesWriteConcernError() {
        ...
        assert.eq(
            error.writeConcernError.code,
            bulkResult.writeConcernErrors[0].code,
        );
    ...
    tests.push(
        function assertShouldThrowExceptionForFalseWithDefaultMessagePrefix() {
            ...
            assert.neq(
                -1,
                err.message.indexOf(prefix),
                "assert message should should contain prefix",
            );
            assert.neq(
                -1,
    ...
    tests.push(function sameMembersFailsWithDifferentObjects() {
        assert.throws(
            () => assert.sameMembers([{_id: 0, a: 0}], [{_id: 0, a: 1}]),
        );
        assert.throws(
            () => assert.sameMembers([{_id: 1, a: 0}], [{_id: 0, a: 0}]),
        );
        assert.throws(() => {
            assert.sameMembers(
                [{a: [{b: 0, c: 0}], _id: 0}],
    ...
    tests.push(function sameMembersFailsWithDifferentBSONTypes() {
        ...
        assert.throws(
            () => assert.sameMembers([new Timestamp(0, 1)], [new Timestamp(0, 2)]),
        );
    });
    ...
    tests.push(function assertEqJsonFormat() {
        assertThrowsErrorWithJson(
            ...
            {
                msg: "[{a}] and [{b}] are not equal : lorem ipsum",
                attr: {a: 5, b: 4},
            },
        );
        assertThrowsErrorWithJson(
    ...
    tests.push(function assertDocEqJsonFormat() {
        assertThrowsErrorWithJson(
            ...
            },
            {
                ...
                attr: {
                    expectedDoc: {msg: "hello"},
                    actualDoc: {msg: "goodbye"},
                    ...kAttr,
                },
            },
        );
    ...
    tests.push(function assertSetEqJsonFormat() {
        assertThrowsErrorWithJson(
            ...
            {
                msg:
                    "expected set {expectedSet} and actual set {actualSet} are not equal : lorem ipsum",
    ...
    tests.push(function assertNeqJsonFormat() {
        assertThrowsErrorWithJson(
            ...
            {
                msg: "[{a}] and [{b}] are equal : Oops!",
                attr: {a: 42, b: 42, ...kAttr},
            },
        );
    });
    ...
    tests.push(function assertDoesNotContainJsonFormat() {
        assertThrowsErrorWithJson(
            ...
            {
                msg: "{element} is in {arr} : Oops!",
                attr: {element: 3, arr: [3, 23], ...kAttr},
            },
        );
    });
    ...
    tests.push(function assertSoonJsonFormat() {
        assertThrowsErrorWithJson(
            ...
            {
                msg: "assert.soon failed (timeout 20ms), msg : Oops!",
                attr: {...kAttr},
            },
        );
    });
            ...
            },
            {
                ...
                attr: {
                    res: {
                        ...
                        codeName: ErrorCodeStrings[ErrorCodes.BadValue],
                        errmsg: "unexpected error",
                        _mongo: "connection to localhost:20000",  // Will not be seen when 'res' is
                                                                  // BSON.
    ...
    tests.push(function assertWriteOKJsonFormat() {
        assertThrowsErrorWithJson(
            ...
            {
                msg: "unknown type of write result, cannot check ok: {res} : Oops!",
                attr: {res: {ok: 0}},
            },
        );
    });
        ...
        assertThrowsErrorWithJson(
            () => {
                const res = new WriteResult(
                    {nRemoved: 0, writeErrors: [], upserted: []},
                    3,
                    {w: "majority"},
                );
    ...
    tests.push(function assertWriteErrorWithCodeJsonFormat() {
        ...
        assertThrowsErrorWithJson(
            () => {
                const res = new WriteResult(
                    {nRemoved: 0, writeErrors: [writeError], upserted: []},
                    3,
                    {w: "majority"},
                );
    ...
    tests.push(function assertIsNullJsonFormat() {
        assertThrowsErrorWithJson(
            ...
            {
                msg: "supposed to be null, was: {value} : Oops!",
                attr: {value: {ok: 1}, ...kAttr},
            },
        );
    });
    ...
    tests.push(function assertLTJsonFormat() {
        assertThrowsErrorWithJson(
            ...
            {
                msg: "{a} is not less than {b} : Oops!",
                attr: {a: 41, b: 18, ...kAttr},
            },
        );
    });
    ...
    tests.push(function assertCloseWithinMSJsonFormat() {
        ...
        assertThrowsErrorWithJson(
            ...
            {
                msg:
                    "86399999 is not equal to 86400009 within 1 millis, actual delta: 10 millis : Oops!",
                attr: {
                    a: dateForLog(date1),
                    b: dateForLog(date2),
                    deltaMS: 1,
