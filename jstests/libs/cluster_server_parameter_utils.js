...
// _id
function checkGetClusterParameterMatch(db, tenantId, name, expectedValue) {
    const cps =
        assert.commandWorked(db.runCommand(tenantCommand({getClusterParameter: name}, tenantId)))
            .clusterParameters;
...
export function testGetClusterParameterStar(conn, tenantId) {
    ...
    const allParameters =
        assert
            .commandWorked(adminDB.runCommand(tenantCommand({getClusterParameter: '*'}, tenantId)))
            .clusterParameters;
    for (const param of allParameters) {
        assert(param.hasOwnProperty("_id"),
               'Entry in {getClusterParameter: "*"} result is missing _id key:\n' + tojson(param));
        const name = param["_id"];
        checkGetClusterParameterMatch(adminDB, tenantId, name, param);
    }
}
