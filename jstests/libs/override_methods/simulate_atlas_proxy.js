...
assert(
    !!TestData.tenantId || (TestData.tenantIds && TestData.tenantIds.length > 0),
...
function assertRoutingConnection(conn) {
    if (conn !== initialConn) {
        assert.eq(null, conn._internalRoutingConnection, "Expected connection to have no internal routing connection.");
    }
}
...
function removeSuccessfulOpIndexesExceptForUpserted(resObj, indexMap, ordered) {
    ...
    for (let index in Object.keys(indexMap)) {
        if (
            !indexSetForUpserted.has(parseInt(index)) &&
 ...
 */
function runCommandWithTenantId(conn, securityToken, dbNameWithTenantId, cmdObjWithTenantId, options) {
...
Mongo.prototype.runCommand = function (dbName, cmdObj, options) {
    ...
    const resObj = runCommandWithTenantId(this, securityToken, dbNameWithTenantId, cmdObjWithTenantId, options);
    ...
    let checkPrefixOptions = !useResponsePrefixChecking
        ...
        : {
              ...
              debugLog:
                  ...
                  tojsononeline(cmdObjWithTenantId),
          };
...
["toggleAutoEncryption", "unsetAutoEncryption", "setAutoEncryption"].forEach((methodName) => {
    const $method = Mongo.prototype[methodName];
    Mongo.prototype[methodName] = function () {
...
OverrideHelpers.prependOverrideInParallelShell("jstests/libs/override_methods/simulate_atlas_proxy.js");
