...
ReplSetTest[kOverrideConstructorForRST] = class MultitenantChangeStreamReplSetTest extends ReplSetTest {
    constructor(opts) {
        ...
        const newOpts =
            typeof opts !== "string" && !TestData.hasOwnProperty("setParametersMongos")
    ...
    initiate(cfg, initCmd) {
        ...
        assert.eq(assert.commandWorked(adminDb.runCommand({getChangeStreamState: 1})).enabled, true);

        // Verify that the change stream cursor is getting opened in the change collection.
        const explain = assert.commandWorked(
            adminDb.getSiblingDB("test").runCommand({
                aggregate: 1,
                pipeline: [{$changeStream: {}}],
                explain: true,
            }),
        );
        assert.eq(explain.stages[0].$cursor.queryPlanner.namespace, "config.system.change_collection");
    }
};

ShardingTest[kOverrideConstructorForST] = class MultitenantChangeStreamShardingTest extends ShardingTest {
    constructor(params) {
        super(params);
