...
(function() {
    ...
    ShardingTest = function ShardingTestWithContinuousConfigPrimaryStepdown() {
        ...
        this.startBalancer = function(timeoutMs, interval) {
            ...
            var result = this.configRS.getPrimary().adminCommand(
                {_configsvrControlBalancer: 'start', maxTimeMS: timeoutMs});
        ...
        this.stopBalancer = function(timeoutMs, interval) {
            timeoutMs = timeoutMs || 60000;
            var result = this.configRS.getPrimary().adminCommand(
                {_configsvrControlBalancer: 'stop', maxTimeMS: timeoutMs});
            if (result.code === ErrorCodes.CommandNotFound) {
                return originalStopBalancer.call(this, [timeoutMs, interval]);
            }
