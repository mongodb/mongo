...
export function assertQueryShapeHashStability(conn, dbName, explainCmd) {
    ...
    const isRawOperationOnLegacyTimeseries = (() => {
        ...
        return explainResults.some(explainRes =>
                                       isSystemBucketsNamespace(explainRes.queryPlanner.namespace));
    })();

    // TODO SERVER-103295 remove this if once 9.0 becomes last LTS. By then we won't have anymore
    // legacy timeseries collections (system.bucksts.)
    ...
    const firstQueryShapeHash = explainResults.queryShapeHash;
    assert(
