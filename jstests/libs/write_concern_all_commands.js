...
let additionalCRUDOps = {
        ...
        success: {
            ...
            confirmFunc: (res, coll, cluster, clusterType) => {
                ...
                if (clusterType != "sharded" || !writeWithoutSkOrId) {
                    ...
                    // The two phase write path returns WriteConcernTimeout in the write errors
                    // array.
                    assert.commandWorkedIgnoringWriteErrorsAndWriteConcernErrors(res);
    ...
    "orderedBatch": {
        ...
        failure: {
            ...
            // last update should not be executed. The first and second should still return WCEs.
            req: {
                update: collName,
                updates: [
                    {q: {a: -20}, u: {$set: {b: 3}}},
    ...
    "bulkWriteOrdered": {
        ...
        noop: {
            ...
            confirmFunc: (res, coll, cluster, clusterType) => {
                ...
                if (clusterType != "sharded" || !writeWithoutSkOrId) {
                    ...
                    // The write without shard key will execute a transaction and fail the write
                    // with WriteConcernTimeout, so the insert will not execute.
                    assert.eq(res.cursor.firstBatch.length, 2);
        ...
        // The second update will fail, and this is an ordered batch so the final update should
        // not exeucte.
        failure: {
            req: {
                bulkWrite: 1,
...
function stopSecondaries(cluster, clusterType) {
    if (clusterType == "rs") {
        ...
        shards.forEach((rs) => {
            ...
            // other secondary to ensure waiting for write concern will still fail, so we track the
            // other secondary.
...
/**
 * Assert that the result contains a write concern error. Typically we expect WCEs to be reported in
 * the WriteConcernErrors field, but some commands nest the WCE and other actually return the error
 * as a top-level command error.
 */
...
export function checkWriteConcernBehaviorForAllCommands(
    conn, cluster, clusterType, preSetup, shardedCollection) {
    jsTestLog("Checking write concern behavior for all commands");
    let commandsList = AllCommandsTest.checkCommandCoverage(conn, wcCommandsTests);
    ...
    if (clusterType == "rs") {
        ...
        executeWriteConcernBehaviorTests(
            conn, coll, cluster, clusterType, preSetup, commandsList, wcCommandsTests);
    ...
    // In a sharded cluster, some commands are coordinated through the configsvr, and others through
    // the shard(s). For commands that target the configsvr, we'll prevent replication on the
    // configsvr. Otherwise, we'll prevent replication on the shard(s). To speed up the execution of
    // this test, we'll run all commands that target the configsvr first and then those that shards
    // target shards so that we don't need to stop and restart nodes for each test case.
    let cmdsTargetConfigServer = [];
    let cmdsTargetShards = [];
    commandsList.forEach((command) => {
        let cmd = wcCommandsTests[command];
