...
(function () {
    ...
    assert.commandWorked(db.adminCommand({appendOplogNote: 1, data: {msg: "dummy write to advance cluster time"}}));
    ...
    for (const stage of allowedStages) {
        ...
        const changeStreamCursor = testColl.watch(changeStreamPipeline, {startAtOperationTime: startClusterTime});
        assert.soon(
