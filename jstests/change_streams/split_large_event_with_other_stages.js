...
(function () {
    ...
    const allowedStages = [
        ...
        {$project: {"fullDocument.first_name": 1, "fullDocument.last_name": 1}},
        {
            $replaceRoot: {
                newRoot: {
                    full_name: {
                        $concat: ["$fullDocument.first_name", " ", "$fullDocument.last_name"],
                    },
                    _id: "$_id",
                },
            },
        },
        {
            $replaceWith: {
                _id: "$_id",
                pets: {
                    $mergeObjects: [{dogs: 0, cats: 0, birds: 0, fish: 0}, "$fullDocument.pets"],
                },
            },
        },
    ...
    for (const stage of allowedStages) {
        ...
        const changeStreamCursor = testColl.watch(changeStreamPipeline, {
            startAtOperationTime: startClusterTime,
        });
        assert.soon(
            () => changeStreamCursor.hasNext(),
            "Unexpected lack of events for the change stream pipeline " +
                tojson(changeStreamPipeline),
        );
