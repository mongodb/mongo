...
function assertUpdateLookupResultBeforeOp(op, changeStreamOptions) {
    ...
    const options = Object.assign({}, changeStreamOptions, {fullDocument: "updateLookup"});
    let cursor =
        cst.startWatchingChanges({pipeline: [{$changeStream: options}], collection: collNameA});
    ...
    const updatedDocInCollA = Object.assign({}, doc, {a: 2});
    ...
    if (op === 'renameCollection') {
        ...
        const newDocInNewCollA =
            Object.assign({}, doc, {b: "extra field in the new document in the new collection"});
