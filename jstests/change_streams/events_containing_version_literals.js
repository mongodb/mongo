...
setUp();
jsTestLog("tests that updates using $v inside an object literal fail");
...
setUp();
jsTestLog(
    "tests that updates using $v inside an object literal succeed with upsert when the source documents do not exist");
["test", 1, 2, "1", "2"].forEach((v, i) => {
...
setUp();
jsTestLog("tests that updates using $v inside $set and an object literal fail");
["test", 1, 2, "1", "2"].forEach((v, i) => {
    assert.commandFailedWithCode(
...
setUp();
jsTestLog(
    "tests that updates using $v inside $set and an object literal succeed with upsert when the source documents do not exist");
...
setUp();
jsTestLog("tests that pipeline updates using $v inside $replaceWith and $literal succeed");
["test", 1, 2, "1", "2"].forEach((v, i) => {
...
setUp();
jsTestLog(
    "tests that pipeline updates using $v inside using $replaceWith and an object literal fail");
["test", 1, 2, "1", "2"].forEach((v, i) => {
    assert.commandFailedWithCode(db[kCollName].update({_id: i}, [{$replaceWith: {_id: i, $v: v}}]),
                                 16410);
});

setUp();
jsTestLog("tests that pipeline updates using $v inside using $replaceRoot and $literal succeed");
["test", 1, 2, "1", "2"].forEach((v, i) => {
    assert.commandWorked(
...
setUp();
jsTestLog("tests that pipeline updates using $v using $replaceRoot and an object literal fail");
["test", 1, 2, "1", "2"].forEach((v, i) => {
    assert.commandFailedWithCode(
...
setUp();
jsTestLog("tests that update using $v inside $inc fails");
["test", 1, 2, "1", "2"].forEach((v, i) => {
...
setUp();
jsTestLog("tests that updates using $v inside $addFields fails");
["test", 1, 2, "1", "2"].forEach((v, i) => {
...
setUp();
jsTestLog(
    "tests that updates using $v inside $replaceWith and $setField and an object literal succeed");
["test", 1, 2, "1", "2"].forEach((v, i) => {
    assert.commandWorked(
        db[kCollName].update(
            {_id: i},
