...
(function() {
    ...
    // Correct ordering treats {a: undefined} and {a: []} as the same value (undefined) and treats
    // {a:
    // null} and missing "a" as the same value (null). When we sort by the "a" field, we expect the
    // sort
    // to put all the undefined-like documents in one group (1, 3, 6, 8) and all null-like documents
    // in
    // another (2, 4, 5, 7). We break the ties within these two groups by sorting on the "_id"
    // field.
    ...
    function checkFindAndAggSorts(sortPattern, expectedOrder) {
        ...
        const aggResult = coll.aggregate({$sort: sortPattern}).toArray();
    ...
    // Repeat the above test, but this time we sort on a path that traverse an array, forcing the
    // sort
    // key generator to use its general path, instead of the fast path for sorting on non-array
    // paths.
    assert(coll.drop());
    assert.writeOK(coll.insert([
        {_id: 1, a: [{b: undefined}]},
