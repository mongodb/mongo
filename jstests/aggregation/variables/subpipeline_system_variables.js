/**
 * Tests that $$NOW and $$CLUSTER_TIME have the same value in subpipelines as in the parent
 * pipeline.
 * @tags: [
...
// Because we are testing time-based variables, we run the test multiple times to reduce the chance
// of a false positive.
...
for (let testAttempt = 0; testAttempt < TEST_COUNT; testAttempt++) {
    jsTestLog(`Starting test attempt ${testAttempt} / ${TEST_COUNT}`);
    runTests("NOW");
    if (!isStandalone) {
