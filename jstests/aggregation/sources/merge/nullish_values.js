...
for (let mergeMode of ["merge", "replace", "keepExisting"]) {
    let mergePipeline = [{$merge: {into: outputColl.getName(), on: "mergeOn", whenMatched: mergeMode}}];
...
assert.commandWorked(
    inputColl.insertMany([{merge: {on: null}, a: 1}, {merge: {}, b: 2}, {c: 3}, {merge: "scalar", d: 4}]),
);
assert.doesNotThrow(() =>
    inputColl.aggregate([{$project: {_id: 0}}, {$merge: {into: outputColl.getName(), on: "merge.on"}}]),
);
