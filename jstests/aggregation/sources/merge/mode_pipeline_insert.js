...
(function testMergeIntoNonExistentCollection() {
    ...
    assert.doesNotThrow(() =>
        source.aggregate(
            makeMergePipeline({target: target.getName(), updatePipeline: [{$addFields: {x: 1}}]}),
        ),
    );
    assertArrayEq({
    ...
    assert.doesNotThrow(() =>
        source.aggregate(
            makeMergePipeline({
                ...
                updatePipeline: [{$addFields: {x: 1, y: 2}}],
            }),
        ),
    );
...
(function testMergeWithSupportedUpdatePipelineStages() {
    ...
    assert.doesNotThrow(() =>
        source.aggregate(
            makeMergePipeline({
                target: target.getName(),
                updatePipeline: [{$project: {x: {$add: ["$b", 1]}}}],
            }),
        ),
    );
    assertArrayEq({
    ...
    assert.doesNotThrow(() =>
        source.aggregate(
            makeMergePipeline({target: target.getName(), updatePipeline: [{$replaceWith: "$c"}]}),
        ),
    );
    assertArrayEq({
        actual: target.find().toArray(),
    ...
    assert.doesNotThrow(() =>
        source.aggregate(
            makeMergePipeline({
                target: target.getName(),
                updatePipeline: [{$replaceRoot: {newRoot: "$c"}}],
            }),
        ),
    );
    assertArrayEq({
...
(function testMergePresrvesIndexesAndOptions() {
    ...
    assert.doesNotThrow(() =>
        source.aggregate(
            makeMergePipeline({target: target.getName(), updatePipeline: [{$addFields: {z: 1}}]}),
        ),
    );
    assertArrayEq({
        actual: target.find().toArray(),
