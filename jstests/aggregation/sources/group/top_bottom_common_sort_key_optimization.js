...
function verifyOptimizedForSbe(pipeline, explain) {
    ...
    assert.eq(
        projectStages.length,
        1,
        `W/ the optimization, $project or PROJECTION_DEFAULT should appear in the plan: ${D(pipeline, explain)}`,
    );
}
...
(function testOptimizableTopNsAndBottomNsWithMissingFields() {
    runTestCase({
        docs: [doc_t1_sD_x3_y10, doc_t1_sD_adotb_0_, doc_t2_sA_x10_y5],
        pipeline: [
            {
                $group: {
                    _id: "$s",
...
(function testOptimizableTopNsAndBottomNsWithDottedPathsAsMissingFields() {
    runTestCase({
        docs: [doc_t1_sD_adotb_0_, doc_t2_sA_adotb_1_99_],
        pipeline: [
            {
                $group: {
                    _id: "$s",
