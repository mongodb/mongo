...
// Run test on a collection without any collation. The collation is passed to the aggregate query instead.
const runTestCollectionNoCollation = () => {
    const coll = db.collation_bucket;
    coll.drop();
...
// Run test on a collection with a collation.
const runTestCollectionWithCollation = () => {
    // The collection is recreated using a different name to avoid problems with outdated router
    // caches in multi-router setups.
    const coll = db.collation_bucket_collation;
    coll.drop();
