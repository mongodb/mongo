...
const testCommand = function (cmd, cmdObj) {
    ...
    assert.commandWorked(
        testDB.runCommand({create: viewName, viewOn: coll.getName(), pipeline: [], writeConcern: {w: "majority"}}),
    );
    ...
    jsTestLog("The command '" + cmd + "' succeeds on timeseries collection when no UUID is provided.");
    const tsCollName = "ts_coll";
    assert.commandWorked(
        testDB.runCommand({create: tsCollName, timeseries: {timeField: "time"}, writeConcern: {w: "majority"}}),
    );
