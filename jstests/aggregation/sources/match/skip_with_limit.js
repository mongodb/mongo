...
count = coll
    .aggregate([{$match: {x: 4}}, {$skip: 10}, {$addFields: {y: 1}}, {$limit: 5}])
    .itcount();
assert.eq(count, 5);

count = coll
    .aggregate([{$match: {x: 4}}, {$skip: 10}, {$group: {_id: "$y"}}, {$limit: 5}])
    .itcount();
assert.eq(count, 5);
...
count = coll
    .aggregate([{$match: {x: 4}}, {$limit: 10}, {$addFields: {y: 1}}, {$skip: 5}])
    .itcount();
assert.eq(count, 5);

count = coll
    .aggregate([{$match: {x: 4}}, {$limit: 10}, {$group: {_id: "$y"}}, {$skip: 5}])
    .itcount();
assert.eq(count, 5);
...
count = coll
    .aggregate([{$match: {x: 4}}, {$skip: 18}, {$addFields: {y: 1}}, {$limit: 5}])
    .itcount();
assert.eq(count, 2);

count = coll
    .aggregate([{$match: {x: 4}}, {$skip: 18}, {$group: {_id: "$y"}}, {$limit: 5}])
    .itcount();
assert.eq(count, 2);
...
count = coll
    .aggregate([{$match: {x: 4}}, {$sort: {x: 1}}, {$skip: 10}, {$skip: 8}, {$limit: 5}])
    .itcount();
assert.eq(count, 2);

count = coll
    .aggregate([{$match: {x: 4}}, {$skip: 5}, {$limit: 10}, {$skip: 5}, {$limit: 4}])
    .itcount();
assert.eq(count, 4);
...
assert.eq(count, 4);
count = coll
    .aggregate([{$match: {x: 4}}, {$sort: {y: -1}}, {$skip: 7}, {$skip: 4}, {$limit: 4}])
    .itcount();
assert.eq(count, 4);
count = coll.aggregate([{$match: {x: 4}}, {$skip: 7}, {$skip: 10}, {$limit: 4}]).itcount();
assert.eq(count, 3);
count = coll
    .aggregate([{$match: {x: 4}}, {$sort: {y: -1}}, {$skip: 7}, {$skip: 10}, {$limit: 4}])
    .itcount();
assert.eq(count, 3);
