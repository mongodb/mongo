...
pipeline = [
    {
        $lookup: {
            let: {var1: "$_id"},
            pipeline: [{$match: {$expr: {$lt: ["$_id", "$$var1"]}}}],
            from: "from",
            as: "c",
        },
    },
...
pipeline = [
    {
        $lookup: {
            let: {var1: {$mod: ["$x", 3]}},
            pipeline: [{$project: {var1Mod3TimesForeignId: {$multiply: ["$$var1", "$_id"]}}}],
            from: "from",
            as: "c",
        },
    },
...
pipeline = [
    {
        $lookup: {
            ...
            pipeline: [
                {
                    $match: {
                        $expr: {$eq: ["FIELD-IS-NULL", {$ifNull: ["$$var1.y.z", "FIELD-IS-NULL"]}]},
                    },
                },
            ],
            from: "from",
...
pipeline = [
    {
        $lookup: {
            let: {var1: "$x"},
            pipeline: [{$match: {$expr: {$eq: ["$$var1", [1, 2, 3]]}}}],
            from: "from",
            as: "as",
        },
    },
...
pipeline = [
    {
        $lookup: {
            let: {var1: "$$ROOT.x.y.z"},
            pipeline: [{$match: {$expr: {$eq: ["$$var1", "$$ROOT.x.y.z"]}}}, {$project: {_id: 0}}],
            from: "lookUp",
            as: "as",
        },
    },
