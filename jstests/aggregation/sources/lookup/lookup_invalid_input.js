...
assert.commandWorked(
    db.runCommand({
        ...
        pipeline: [
            {$lookup: {from: from.getName(), localField: "x", foreignField: "y", as: "result"}},
        ],
        cursor: {},
    }),
);
...
// Fails when neither localField/foreignField nor pipeline is provided.
...
assertErrorCode(
    coll,
    [{$lookup: {from: from.getName(), localField: "x", as: "result"}}],
    ErrorCodes.FailedToParse,
);
...
assertErrorCode(
    coll,
    [{$lookup: {from: from.getName(), foreignField: "y", as: "result"}}],
    ErrorCodes.FailedToParse,
);
...
assertErrorCode(
    coll,
    [{$lookup: {localField: "x", foreignField: "y", as: "result"}}],
    ErrorCodes.FailedToParse,
);
