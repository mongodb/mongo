...
testWithProject({
    ...
    percentileSpec:
        {$percentile: {p: [0.5], input: ["$x", "$x1", "$x2", "$x3"], algorithm: "approximate"}},
    expectedResult: [1],
...
testWithProject({
    ...
    percentileSpec:
        {$percentile: {p: [0.5, 0.9], input: ["$x", "$x1", "$x2"], algorithm: "approximate"}},
    expectedResult: null,
...
testWithProject({
    doc: {x: 10, x1: 5, x2: 27},
    percentileSpec: {$percentile: {p: [1], input: ["$x", "$x1", "$x2"], algorithm: "approximate"}},
    expectedResult: [27],
...
testWithProject({
    doc: {x: 0, x1: 1, x2: 2},
    percentileSpec:
        {$percentile: {p: [0.5, 0.9, 0.1], input: ["$x", "$x1", "$x2"], algorithm: "approximate"}},
    expectedResult: [1, 2, 0],
...
testWithProject({
    ...
    percentileSpec: {$percentile: {p: [0.5, 0.9, 0.1], input: "$x1", algorithm: "approximate"}},
    expectedResult: null,
