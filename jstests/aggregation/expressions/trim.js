...
for (const op of ["$trim", "$ltrim", "$rtrim"]) {
    ...
    assert.eq(
        coll
            .aggregate([{$project: {exp: {[op]: {input: "$caseTest", chars: "$chars"}}}}], {
...
assert.eq(
    coll
        .aggregate([
            {$project: {proof: {$rtrim: {input: "$proof", chars: "∎"}}}},
            {$sort: {_id: 1}},
        ])
        .toArray(),
    [
...
assert.eq(
    coll
        .aggregate([
            {$project: {proof: {$rtrim: {input: "$proof", chars: "$extra"}}}},
            {$sort: {_id: 1}},
        ])
        .toArray(),
    [
...
for (const op of ["$trim", "$ltrim", "$rtrim"]) {
    ...
    assertErrorCode(
        coll,
        [{$project: {x: {[op]: {input: {$add: ["$constantNum", "$constantNum"]}}}}}],
        50699,
    );
...
for (const op of ["$trim", "$ltrim", "$rtrim"]) {
    assertErrorCode(
        coll,
        [{$project: {x: {[op]: {input: "$str", chars: " ".repeat(4097)}}}}],
        12066800,
    );
}
