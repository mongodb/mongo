 ...
 * @tags: [
 *   # BinData $convert numeric was added in v8.2.
 *   requires_fcv_82,
...
function createBindataVectorBitArray(dataTypeByte, vector, numPaddingBits) {
    ...
    let paddingBitArray = dataTypeByte == kPackedBitByte ? int8VectorToBitArray([numPaddingBits])
                                                         : int8VectorToBitArray([0]);
    let arrayElemsBitArray = dataTypeByte == kFloat32Byte ? float32VectorToBitArray(vector)
...
testCases.forEach((testCase) => {
    ...
    let doc = {
        ...
        approx:
            dtype == kFloat32Byte  // BSON does not support FLOAT32, so results may not be exact.
    };
    ...
    bindataToBsonResult.forEach(doc => {
        if (doc.approx) {
            ...
            for (let i = 0; i < doc.output.length; i++) {
                if (doc.output[i] == Number.NEGATIVE_INFINITY ||
    ...
    if (intArrayCanConvertToPackedBit) {
        ...
        expectedBindataVector =
            BinData(kBindataVectorSubtype,
                    bitArrayToBase64String(createBindataVectorBitArray(
                        kPackedBitByte, bitArrayToByteArray(arrayFilledWithZeros), numZeros)));
    }

    // BSON arrays that only contain integer values from [-128, 127] will convert to INT8
    // arrays.
    let canBeRepresentedAsIntArray =
        array_elems.every(n => (Number.isInteger(n) && n <= 127 && n >= -128));
...
let binToBsonErrorCases = [
    ...
    {
        invalid_bindata_vector: kInt8Byte + "01" +
            "01",
        error_code: 10506606
    },
    // Invalid padding - should only exist for PACKED_BIT
    {
        invalid_bindata_vector: kFloat32Byte + "01" +
            "11111111",
        error_code: 10506606
    },
    // Not enough bytes for float
    {
        invalid_bindata_vector: kFloat32Byte + "00" +
            "001100",
        error_code: 10506602
    },
];
...
binToBsonErrorCases.forEach((testCase) => {
    ...
    let bindataToBsonPipeline = [{
        $project:
            {_id: 0, output: {$convert: {to: {type: "array"}, input: "$bindata_array_base64"}}}
    }];
...
bsonToBinErrorCases.forEach((testCase) => {
    ...
    let bsonToBindataPipeline = [{
        $project: {
            ...
            output: {
                $convert:
                    {to: {type: "binData", subtype: 9}, input: "$bson_array", format: "base64"}
            }
        }
    }];

    assert.throwsWithCode(() => coll.aggregate(bsonToBindataPipeline).toArray(),
                          testCase.error_code);
});
...
(function bsonArrayWithLargePositiveIntFailsToBeConverted() {
    ...
    let bsonToBindataPipeline = [{
        $project:
            {_id: 0, output: {$convert: {to: {type: "binData", subtype: 9}, input: "$bson_array"}}}
    }];
