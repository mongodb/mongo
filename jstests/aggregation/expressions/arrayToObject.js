...
(function() {
    ...
    assert.writeOK(coll.insert({_id: 8, expanded: [{"k": "price", "v": 24}, ["item", "apple"]]}));
    assertErrorCode(coll, [{$match: {_id: 8}}, array_to_object_expr], 40391);

    assert.writeOK(coll.insert({_id: 9, expanded: [["item", "apple"], {"k": "price", "v": 24}]}));
    assertErrorCode(coll, [{$match: {_id: 9}}, array_to_object_expr], 40396);
    ...
    assert.writeOK(coll.insert({_id: 12, expanded: NumberLong(0)}));
    assertErrorCode(coll, [{$match: {_id: 12}}, array_to_object_expr], 40386);

    assert.writeOK(coll.insert({_id: 13, expanded: [0]}));
    assertErrorCode(coll, [{$match: {_id: 13}}, array_to_object_expr], 40398);

    assert.writeOK(coll.insert({_id: 14, expanded: [["missing_value"]]}));
    assertErrorCode(coll, [{$match: {_id: 14}}, array_to_object_expr], 40397);

    assert.writeOK(coll.insert({_id: 15, expanded: [[321, 12]]}));
    assertErrorCode(coll, [{$match: {_id: 15}}, array_to_object_expr], 40395);

    assert.writeOK(coll.insert({_id: 16, expanded: [["key", "value", "offset"]]}));
    assertErrorCode(coll, [{$match: {_id: 16}}, array_to_object_expr], 40397);

    assert.writeOK(coll.insert({_id: 17, expanded: {y: []}}));
    assertErrorCode(coll, [{$match: {_id: 17}}, array_to_object_expr], 40386);

    assert.writeOK(coll.insert({_id: 18, expanded: [{y: "x", x: "y"}]}));
    assertErrorCode(coll, [{$match: {_id: 18}}, array_to_object_expr], 40393);

    assert.writeOK(coll.insert({_id: 19, expanded: [{k: "missing"}]}));
    assertErrorCode(coll, [{$match: {_id: 19}}, array_to_object_expr], 40392);

    assert.writeOK(coll.insert({_id: 20, expanded: [{k: 24, v: "string"}]}));
    assertErrorCode(coll, [{$match: {_id: 20}}, array_to_object_expr], 40394);

    assert.writeOK(coll.insert({_id: 21, expanded: [{y: "ignored", k: "item", v: "pear"}]}));
    assertErrorCode(coll, [{$match: {_id: 21}}, array_to_object_expr], 40392);

    assert.writeOK(coll.insert({_id: 22, expanded: NaN}));
    assertErrorCode(coll, [{$match: {_id: 22}}, array_to_object_expr], 40386);
}());
