    ...
    assert.eq(
        coll
            .aggregate([
                {
                    $project: {
                        result: {$replaceAll: {input: "$input", find: "$find", replacement: "$replacement"}},
                    },
                },
            ])
            .toArray()[0].result,
...
function runAndAssertThrows(args, code) {
    const error = assert.throws(() => coll.aggregate([{$project: {args, result: {$replaceAll: args}}}]).toArray());
    assert.commandFailedWithCode(error, code);
}

// Test find all.
runAndAssert({input: "albatross", find: "ross", replacement: "rachel"}, "albatrachel");
runAndAssert({input: "albatross", find: "", replacement: "one "}, "one aone lone bone aone tone rone oone sone sone ");
...
runAndAssert({input: "an antelope is an animal", find: "an", replacement: "this"}, "this thistelope is this thisimal");
