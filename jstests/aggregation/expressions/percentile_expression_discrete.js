/**
 * Tests for the discrete percentile expression semantics.
 * @tags: [
 *   requires_fcv_81,
...
testWithProject({
    ...
    percentileSpec: {$percentile: {p: [0.5], input: ["$x", "$x1", "$x2", "$x3", "$x4"], method: "discrete"}},
    expectedResult: [1],
...
testWithProject({
    ...
    percentileSpec: {$percentile: {p: [0.5, 0.9], input: ["$x", "$x1", "$x2"], method: "discrete"}},
...
testWithProject({
    ...
    percentileSpec: {$percentile: {p: [0.1, 0.5], input: "$x", method: "discrete"}},
...
testWithProject({
    coll: coll,
    doc: {x: 10, x1: 5, x2: 27},
    percentileSpec: {$percentile: {p: [1], input: ["$x", "$x1", "$x2"], method: "discrete"}},
    expectedResult: [27],
...
testWithProject({
    coll: coll,
    doc: {x: 0, x1: 1, x2: 2},
    percentileSpec: {$percentile: {p: [0.5, 0.9, 0.1], input: ["$x", "$x1", "$x2"], method: "discrete"}},
    expectedResult: [1, 2, 0],
...
testWithProject({
    ...
    percentileSpec: {$percentile: {p: [0.5, 0.9, 0.1], input: "$x1", method: "discrete"}},
 ...
 */
testLargeInput(coll, "discrete");

testLargeNonNumericInput(coll, "discrete");
