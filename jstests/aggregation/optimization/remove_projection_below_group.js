...
describe("remove_projection_below_group optimizer rule", function () {
    ...
    it("mixed projection (computed + passthrough) before group that only reads the passthrough field: correct result", function () {
        assertArrayEq({
            actual: coll
                .aggregate([{$project: {k0: {$add: ["$a", "$b"]}, b: 1}}, {$group: {_id: "$b"}}])
                .toArray(),
            expected: [{_id: 1}, {_id: 2}, {_id: 4}, {_id: 7}, {_id: 11}],
        });
    });

    it("mixed projection where the group reads the computed field: correct result", function () {
        assertArrayEq({
            actual: coll
                .aggregate([{$project: {k0: {$add: ["$a", "$b"]}, b: 1}}, {$group: {_id: "$k0"}}])
                .toArray(),
            expected: [{_id: 1}, {_id: 3}, {_id: 7}, {_id: 13}, {_id: 21}],
        });
    });

    it("projection that renames a field the group reads must not be eliminated", function () {
        assertArrayEq({
            actual: coll.aggregate([{$project: {k1: "$b"}}, {$group: {_id: "$k1"}}]).toArray(),
            expected: [{_id: 1}, {_id: 2}, {_id: 4}, {_id: 7}, {_id: 11}],
        });
    });

    it("two chained renaming projections before a group get the correct result", function () {
        assertArrayEq({
            actual: coll
                .aggregate([
                    {$project: {k1: "$b"}},
