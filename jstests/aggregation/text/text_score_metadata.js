...
function runProjectionTest({forceProjectionOnMerger}) {
    ...
    for (let result of results) {
        assert(
            result.hasOwnProperty(kTextScoreMetadataArg),
            `Failed to find ${kTextScoreMetadataArg} in document: ${tojson(result)}`,
        );
        assert(
            result.hasOwnProperty(kScoreMetadataArg),
...
(function testSortOnScore() {
    const results = coll
        .aggregate([textMatchStage, {$sort: {[kScoreMetadataArg]: {$meta: kScoreMetadataArg}}}])
        .toArray();
    assert.eq(results, [
        {_id: 2, x: "test test"},
        {_id: 0, x: "test"},
...
(function testGroupOnScore() {
    ...
    const results = coll
        .aggregate([textMatchStage, {$group: {_id: {$meta: kScoreMetadataArg}, count: {$sum: 1}}}])
        .toArray();
