// Test --host.
(function() {
    // This "inner_mode" method of spawning a bongod and re-running was copied from
    // ipv6_connection_string_validation.js
    if ("undefined" == typeof inner_mode) {
        // Start a bongod with --ipv6
        jsTest.log("Outer mode test starting bongod with --ipv6");
        ...
        // tested manually), so 127.0.0.1 is also present so the test bongo shell can connect
        // with that address.
        var bongod = BongoRunner.runBongod({ipv6: "", bind_ip: "::1,127.0.0.1"});
        var args = [
            "bongo",
            "--nodb",
            "--ipv6",
            ...
            bongod.port,
            "--eval",
            "inner_mode=true;port=" + bongod.port + ";",
            "jstests/noPassthroughWithBongod/host_connection_string_validation.js"
        ];
        var exitCode = _runBongoProgram.apply(null, args);
    ...
    var testHost = function(host, shouldSucceed) {
        var exitCode = runBongoProgram('bongo', '--ipv6', '--eval', ';', '--host', host);
        if (shouldSucceed) {
            if (exitCode !== 0) {
    ...
    function runUriTestFor(i, connectionString, isGood) {
        ...
        print("    * testing bongodb://" + connectionString);
        testHost("bongodb://" + connectionString, isGood);
    }
