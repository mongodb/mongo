...
// We need to create a new bongod to ensure no one else is talking to us in the background, and
// will screw up our stats.
var bongo = BongoRunner.runBongod({});
...
function createPersistentConnection() {
    assert.soon(function() {
        try {
            permConns.push(new Bongo(db.getBongo().host));
            return true;
        } catch (x) {
            return false;
        }
...
function createTemporaryConnection() {
    ...
    var pollString = "var conn = null;" + "assert.soon(function() {" + "try { conn = new Bongo(\"" +
        db.getBongo().host + "\"); return conn" + "} catch (x) {return false;}}, " +
