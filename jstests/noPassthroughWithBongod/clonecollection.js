...
var fromBongod = BongoRunner.runBongod({bind_ip: "127.0.0.1"});
var toBongod = BongoRunner.runBongod({bind_ip: "127.0.0.1"});
var f = fromBongod.getDB(baseName);
var t = toBongod.getDB(baseName);
...
assert.commandWorked(t.cloneCollection("localhost:" + fromBongod.port, "a"));
assert.eq(1000, t.a.find().count(), "A2");

t.a.drop();

assert.commandWorked(
    t.cloneCollection("localhost:" + fromBongod.port, "a", {i: {$gte: 10, $lt: 20}}));
...
assert.commandFailed(t.cloneCollection("localhost:" + fromBongod.port, "system.profile"));
...
assert.commandFailedWithCode(t.cloneCollection("localhost:" + fromBongod.port, "viewA"),
