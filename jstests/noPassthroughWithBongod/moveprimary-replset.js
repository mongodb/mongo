// This test ensures that data we add on a replica set is still accessible via bongos when we add it
...
(function() {
    ...
    jsTest.log("Geting connection to bongos for the cluster");
    ...
    jsTest.log(
        "Updating the data via bongos and making sure all documents are updated and present");
    testDB[testCollName].update({}, {$set: {y: 'hello'}}, false /*upsert*/, true /*multi*/);
    assert.eq(testDB[testCollName].count({y: 'hello'}),
              numDocs,
              'updating and counting docs via bongos failed');

    jsTest.log("Adding replSet2 as second shard");
    bongosConn.adminCommand({addshard: replSet2.getURL()});

    bongosConn.getDB('admin').printShardingStatus();
    printjson(replSet2.getPrimary().getDBs());

    jsTest.log("Moving test db from replSet1 to replSet2");
    assert.commandWorked(
        bongosConn.getDB('admin').runCommand({moveprimary: testDBName, to: replSet2.getURL()}));
    bongosConn.getDB('admin').printShardingStatus();
    printjson(replSet2.getPrimary().getDBs());
