...
if (!_isWindows()) {
    ...
    sock = new Bongo('/tmp/bongodb-' + port + '.sock');
    sockdb = sock.getDB(db.getName());
    assert(sockdb.runCommand('ping').ok);

    // Test unix socket path
    var path = BongoRunner.dataDir + "/sockpath";
    mkdir(path);
    var dataPath = BongoRunner.dataDir + "/sockpath_data";

    var conn = BongoRunner.runBongod({dbpath: dataPath, unixSocketPrefix: path});

    var sock2 = new Bongo(path + "/bongodb-" + conn.port + ".sock");
