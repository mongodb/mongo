...
var dbdir = BongoRunner.dataPath + name + "/";  // this will work under windows as well as linux
var basedir = BongoRunner.dataPath + name + "files" + "/";
...
print("------ Start bongod with logpath set to new file");
var m = BongoRunner.runBongod({port: port[0], dbpath: dbdir, logpath: logdir + logs[0]});

// log should now exist (and no rotations should exist)
assert.eq(logCount(logs[0], true), 1);
BongoRunner.stopBongod(port[0]);

print("------ Start bongod with logpath set to existing file");
m = BongoRunner.runBongod({port: port[1], dbpath: dbdir, logpath: logdir + logs[0]});
...
BongoRunner.stopBongod(port[1]);

// Blocking on SERVER-5117:
// BongoRunner currently hangs if bongod fails to start so these tests don't work
if (false) {
    ...
    if (_isWindows()) {
        ...
        print("------ Start bongod with logpath set to new file, fork");
        var m = BongoRunner.runBongod(
            {port: port[2], dbpath: dbdir, logpath: logdir + logs[1], fork: true});

        // log should now exist (and no rotations should exist)
        assert.eq(logCount(logs[1], true), 1);
        BongoRunner.stopBongod(port[2]);

        print("------ Start bongod with logpath set to existing file, fork");
        ...
        BongoRunner.stopBongod(port[3]);
    }

    // the following tests depend on undefined behavior; assume that BongoRunner raises exception on
    // error
    print("------ Confirm that launch fails with directory");
    assert.throws(function() {
        BongoRunner.runBongod({port: port[4], dbpath: dbdir, logpath: testdir});
    });

    print("------ Confirm that launch fails with special file");
    assert.throws(function() {
        BongoRunner.runBongod({port: port[5], dbpath: dbdir, logpath: sfile});
    });
}
