...
(function() {
    ...
    if ("undefined" == typeof inner_mode) {
        ...
        const args = [
            ...
            "jstests/noPassthroughWithBongod/replset_host_connection_validation.js"
        ];
        const exitCode = _runBongoProgram(...args);
    ...
    const testHost = function(host) {
        const exitCode = runBongoProgram('bongo', '--eval', ';', '--host', host);
        if (exitCode !== 0) {
    ...
    const connStrings = [
        ...
        `bongodb://localhost:${port}/admin?replicaSet=${replSetName}`,
        `bongodb://localhost:${port}`,
    ];
