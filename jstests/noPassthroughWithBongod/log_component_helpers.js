...
(function(db) {
    ...
    // getLogComponents
    var components1 = bongo.getLogComponents();
    ...
    bongo.setLogLevel(2);
    assert.eq(bongo.getLogComponents().verbosity, 2);

    db.setLogLevel(0);
    assert.eq(bongo.getLogComponents().verbosity, 0);

    // setLogLevel - valid log component
    bongo.setLogLevel(2, "storage.journal");
    assert.eq(bongo.getLogComponents().storage.journal.verbosity, 2);

    db.setLogLevel(1, "storage.journal");
    assert.eq(bongo.getLogComponents().storage.journal.verbosity, 1);

    // setLogLevel - invalid argument
    assert.throws(function() {
        bongo.setLogLevel(2, 24);
    });
    assert.throws(function() {
