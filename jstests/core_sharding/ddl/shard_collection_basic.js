...
function testHashedShardKeyIndexOptionalAndCleanup({keyDoc, docsToInsert = []}) {
    assert(
        isHashedShardKey(keyDoc),
        "The shard key must contain at least one hashed field: " + tojson(keyDoc),
    );
...
if (
    ...
    testHashedShardKeyIndexOptionalAndCleanup({
        keyDoc: {a: "hashed"},
        docsToInsert: [{a: 1, b: 1}],
    });
}
