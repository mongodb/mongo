...
import {createChunks, getShardNames, getShards} from "jstests/sharding/libs/sharding_util.js";
...
function main() {
    const shardCount = getShardNames(db).length;
    if (shardCount < 2) {
        jsTestLog(
            `This test requires 2 or more shards and is running on a version that does not support the requires_2_or_more_shards tag. The configured cluster contains ${
                shardCount} shard(s), skipping test.`);
        return;
    }
