...
assert.commandFailedWithCode(
    db.adminCommand({reshardCollection: ns, numInitialChunks: 1}),
...
const featureFlagReshardingNumSamplesPerChunkEnabled = FeatureFlagUtil.isPresentAndEnabled(
    db,
    "ReshardingNumSamplesPerChunk",
);
if (featureFlagReshardingNumSamplesPerChunkEnabled) {
    ...
    reshardCmdTest.assertReshardCollOk(
        {reshardCollection: ns, key: {newKey: 1}, numInitialChunks: 1, numSamplesPerChunk: 10},
        1,
    );
}
