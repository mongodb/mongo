...
const committedTimestamp = assert.commandWorked(
...
const nonCommittedTimestamp = assert.commandWorked(
    primaryDB.runCommand({insert: collName, documents: [{_id: 1}]}),
).operationTime;
...
for (let db of [primaryDB, secondaryDB]) {
    ...
    for (let readConcern of [
        {level: "snapshot", atClusterTime: nonCommittedTimestamp},
