 ...
 *
 * This test can be slow to execute. Exclude from any build variants that are known to be slow.
 *
 * @tags: [
 *     incompatible_aubsan,
 *     resource_intensive,
...
function runTest(db) {
    ...
    const maxPayloadLengthForIdLength = (idLength) => {
        ...
        //
        // This formula is accurate for all id lengths <= 10000.
    ...
    [1, 10, 100, 1000, 10000].forEach((idLength) => {
        ...
        assert.commandFailedWithCode(runInsert(idLength, maxPayloadLength + 1),
                                     ErrorCodes.BSONObjectTooLarge);
        assert.commandFailedWithCode(runInsert(idLength + 1, maxPayloadLength),
                                     ErrorCodes.BSONObjectTooLarge);
        assert.commandFailedWithCode(runInsert(idLength + 1, maxPayloadLength + 1),
                                     ErrorCodes.BSONObjectTooLarge);
    });
    ...
    // actually inserted document, the size of the _id value and a few other factors (e.g. the size
    ...
    // duplicated in the oplog entry, and because there are other fields that are stored in an oplog
    // entry, the maximum _id length usable together with a large document payload is limited to
    // slightly less than 16KiB. The value of 135 has been determined for this particular test,
    // using the namespace string used by this test. It can be different for other namespace values.
    ...
    assert.commandFailedWithCode(runInsertForIdLength(kMaxIdLengthUsableInOplogEntry + 1),
                                 ErrorCodes.BSONObjectTooLarge);

    // Vastly too large for oplog.
    assert.commandFailedWithCode(runInsertForIdLength(kMaxIdLengthUsableInOplogEntry * 2),
                                 ErrorCodes.BSONObjectTooLarge);
