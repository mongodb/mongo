...
// The lastCommitted opTime should not advance on the secondary.
const opTimesAfterWMaj =
    assert.commandWorked(secondary.adminCommand({replSetGetStatus: 1})).optimes;
...
const secondaryStatusRes = assert.commandWorked(secondary.adminCommand({replSetGetStatus: 1}));
const secondaryOpTimes = secondaryStatusRes.optimes;
assert.gte(
...
assert(secondaryStatusRes.initialSyncStatus,
       () => "Response should have an 'initialSyncStatus' field: " + tojson(secondaryStatusRes));
// We should have applied at least 6 documents, not 5, as fetching and applying are inclusive of the
// sync source's lastApplied.
assert.gte(secondaryStatusRes.initialSyncStatus.appliedOps, 6);
