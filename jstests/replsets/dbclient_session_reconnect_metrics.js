...
function getMetrics(conn) {
    ...
    return Object.fromEntries(
        ["dbClientSessionReconnectAttempts", "dbClientSessionWithoutAutoReconnectFailures"].map(
            (name) => [name, status.metrics.network[name]],
        ),
    );
}
...
assert.soon(() => {
    ...
    return (
        after_disconnect.dbClientSessionReconnectAttempts >
