/**
 * Tests that the fetcher timeout when selecting a sync source candidate is configurable. This
 * enables customers to be able to quickly retry selecting a sync source candidate if one sync
 * source is timing out in its response.
 *
...
const rst = new ReplSetTest(
    {name: jsTestName(), nodes: 3, settings: {chainingAllowed: false}, useBridge: true});
rst.startSet();
...
for (let node of rst.nodes) {
    assert.commandWorked(
        node.adminCommand(
            {setParameter: 1, syncSourceResolverFindFetcherTimeoutMillis: kFetcherTimeoutMillis}),
    );
}
