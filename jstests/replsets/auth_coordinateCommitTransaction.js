...
function testCoordinateCommitTransactionFails(userConnection, directConnection) {
    ...
    authutil.asCluster(userConnection, keyFile, () => {
        userTestDB.getSiblingDB("admin").createUser(
            {user: 'root', pwd: 'root', roles: ['root']},
        );
        userTestDB.getSiblingDB("admin").createUser(
            {user: 'ro', pwd: 'ro', roles: ['read']},
        );
    });
    ...
    if (userConnection != directConnection) {
        ...
        authutil.asCluster(directConnection, keyFile, () => {
            testDB.getSiblingDB("admin").createUser(
                {user: 'ro', pwd: 'ro', roles: ['read']},
            );
            testDB.getSiblingDB("admin").createUser(
                {user: 'root', pwd: 'root', roles: ['root']},
...
const rs = new ReplSetTest(
    {nodes: 1, setParameter: {logComponentVerbosity: {transaction: 4}}, keyFile: keyFile});
rs.startSet();
rs.initiate();
