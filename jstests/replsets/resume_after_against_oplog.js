...
jsTestLog("Running tailable query on the oplog with no results");
{
    ...
    const latestOpTime = assert.commandWorked(
        node.getDB(dbName).runCommand({insert: collName + "_other", documents: [{dummy: 1}]}),
    ).opTime;
    ...
    const resumeToken3 = assertExpectedResumeTokenFormat(resGetMore2);
    assert.eq(
        timestampCmp(resumeToken3.ts, resumeToken2.ts),
        1,
        tojson({
