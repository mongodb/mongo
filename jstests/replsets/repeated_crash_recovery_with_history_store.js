...
function workload() {
    ...
    for (let t = 0; t < nthreads; t++) {
        const thread = new Thread(function(t) {
            while (true) {
                const id = db.c.insertMany([{entries: []}]).insertedIds[0];
                for (var i = 0; i < 200; i++) {
                    const entry = {bigField: new Array(120).fill(i)};
                    db.c.update({_id: id}, {$push: {entries: entry}});
                }
            }
        }, t);
        threads.push(thread);
...
for (let i = 0; i < 20; i++) {
    ...
    const workloadThread = startParallelShell(workload,
                                              rst.getPrimary().port,
                                              false, /* noConnect */
