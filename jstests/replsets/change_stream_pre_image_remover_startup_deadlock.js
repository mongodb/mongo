...
// The test targets the change stream pre-images remover on a secondary node, but when replicated
// truncates are enabled, the secondary does not execute the pre-image removal. Thus we can only
// execute the test if replicated truncates are disabled.
if (systemUsesReplicatedTruncates(primary)) {
