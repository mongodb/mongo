...
let rstWithBadStartupOptions = new ReplSetTest({
    name: testName,
    nodes: 1,
    nodeOptions: {setParameter: "rollbackTimeLimitSecs=-50"},
});
...
assert(
    rawMongoProgramOutput("Bad value for parameter").match('"rollbackTimeLimitSecs"'),
...
let valueSetOnStartup = assert.commandWorked(
    primary.adminCommand({getParameter: 1, rollbackTimeLimitSecs: 1}),
...
let valueSetAtRuntime = assert.commandWorked(
    primary.adminCommand({getParameter: 1, rollbackTimeLimitSecs: 1}),
).rollbackTimeLimitSecs;
