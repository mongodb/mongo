...
// Test commands using a security token for one tenant.
{
    ...
    assert.commandWorked(tokenDB.runCommand({insert: kCollName, documents: [{_id: 0, a: 1, b: 1}]}));

    // Find the document and call getMore on the cursor
    {
        // Add additional document in order to have two batches for getMore
        assert.commandWorked(tokenDB.runCommand({insert: kCollName, documents: [{_id: 100, x: 1}]}));

        const findRes = assert.commandWorked(tokenDB.runCommand({find: kCollName, filter: {a: 1}, batchSize: 1}));
    ...
    // Find and modify the document.
    {
        ...
        const fad2 = assert.commandWorked(
            tokenDB.runCommand({findAndModify: kCollName, query: {a: 11}, update: {$set: {a: 1, b: 1}}}),
        );
    ...
    // the collection and views and validate the results.
    {
        ...
        assert.commandWorked(tokenDB.runCommand({"create": kViewName, "viewOn": kCollName, pipeline: []}));
        ...
        let result = tokenAdminDB.runCommand({aggregate: 1, pipeline: [{$listCatalog: {}}], cursor: {}});
        ...
        assert(
            resultArray.some((entry) => entry.db === targetDb && entry.name === kViewName),
            tojson(resultArray),
        );

        // Get catalog when specifying our target collection, which should only return one
        // result.
        result = tokenDB.runCommand({aggregate: kCollName, pipeline: [{$listCatalog: {}}], cursor: {}});
    ...
    // Test count and distinct command.
    {
        ...
        const resCount = assert.commandWorked(tokenDB.runCommand({count: kCollName, query: {c: 1}}));
        assert.eq(2, resCount.n, tojson(resCount));

        const resDistinct = assert.commandWorked(tokenDB.runCommand({distinct: kCollName, key: "d", query: {}}));
        assert.eq([1, 2], resDistinct.values.sort(), tojson(resDistinct));
    }

    // Test count on view collection.
    {
        const resCount = assert.commandWorked(tokenDB.runCommand({count: kViewName, query: {c: 1}}));
        assert.eq(2, resCount.n, tojson(resCount));
    }
    ...
    // Rename the collection.
    {
        ...
        const tokenAdminDB = tokenConn.getDB("admin");
        assert.commandWorked(tokenAdminDB.runCommand({renameCollection: fromName, to: toName, dropTarget: true}));

        // Verify the the renamed collection by findAndModify existing documents.
        const fad1 = assert.commandWorked(
            tokenDB.runCommand({findAndModify: kCollName + "_renamed", query: {a: 1}, update: {$set: {a: 11, b: 1}}}),
        );
        assert.eq({_id: 0, a: 1, b: 1}, fad1.value, tojson(fad1));

        // Reset the collection name and document data.
        assert.commandWorked(tokenAdminDB.runCommand({renameCollection: toName, to: fromName, dropTarget: true}));
        assert.commandWorked(
            tokenDB.runCommand({findAndModify: kCollName, query: {a: 11}, update: {$set: {a: 1, b: 1}}}),
        );
    }
    ...
    // ListDatabases only returns databases associated with kTenant.
    {
        ...
        const tokenAdminDB = tokenConn.getDB("admin");
        const dbs = assert.commandWorked(tokenAdminDB.runCommand({listDatabases: 1, nameOnly: true}));
...

    {
        ...
        assert.commandWorked(tokenDB.createCollection(tsColl, {timeseries: {timeField: timeFieldName}}));
    ...
    // the 3 collections.
    {
        ...
        assert.eq(0, collsAfterDropColl.cursor.firstBatch.length, tojson(collsAfterDropColl.cursor.firstBatch));

        assert.commandWorked(tokenDB.runCommand({dropDatabase: 1}));
        const collsAfterDropDb = assert.commandWorked(tokenDB.runCommand({listCollections: 1, nameOnly: true}));
    ...
    // Test that transactions can be run successfully.
    {
        ...
        assert.commandWorked(sessionDb.runCommand({delete: kCollName, deletes: [{q: {_id: 0, a: 1, b: 1}, limit: 1}]}));
    ...
    // Test createIndexes, listIndexes and dropIndexes command.
    {
        ...
        res = assert.commandWorked(tokenDB.runCommand({dropIndexes: kCollName, index: ["indexA", "indexB"]}));
    ...
    // Test aggregation stage commands
    {
        ...
        {
            const graphLookupRes = assert.commandWorked(
                tokenDB.runCommand({
                    aggregate: kCollA,
            ...
            assert(
                arrayEq(graphLookupTarget, graphLookupRes.cursor.firstBatch),
        ...
        {
            const outStrRes = assert.commandWorked(
                tokenDB.runCommand({aggregate: kCollA, pipeline: [graphLookupStage, outStageStr], cursor: {}}),
            );
            ...
            let projectRes = assert.commandWorked(
                tokenDB.runCommand({aggregate: kCollC, pipeline: [{$project: {_id: 1, connections: 1}}], cursor: {}}),
            );
            assert(arrayEq(graphLookupTarget, projectRes.cursor.firstBatch), tojson(projectRes.cursor.firstBatch));
        ...
        {
            const outObjRes = assert.commandWorked(
                tokenDB.runCommand({aggregate: kCollA, pipeline: [graphLookupStage, outStageObj], cursor: {}}),
            );
            ...
            let projectRes = assert.commandWorked(
                tokenDB.runCommand({aggregate: kCollD, pipeline: [{$project: {_id: 1, connections: 1}}], cursor: {}}),
            );
            assert(arrayEq(graphLookupTarget, projectRes.cursor.firstBatch), tojson(projectRes.cursor.firstBatch));
        ...
        const lookupPipelineStage = {
            $lookup: {
                from: kCollB,
                let: {kCollA_start: "$start"},
                pipeline: [{$match: {$expr: {$eq: ["$$kCollA_start", "$ref"]}}}],
                as: "refs",
            },
        };
        {
            const lookupPipelineRes = assert.commandWorked(
                tokenDB.runCommand({
                    aggregate: kCollA,
            ...
            assert(
                arrayEq(lookupTarget, lookupPipelineRes.cursor.firstBatch),
        ...
        const mergeStage = {$merge: {into: kCollD}};
        {
            ...
            assert(arrayEq(collADocs, findRes.cursor.firstBatch), tojson(findRes.cursor.firstBatch));
        ...
        const unionWithStage = {$unionWith: kCollB};
        {
            ...
            assert(
                arrayEq(collADocs.concat(collBDocs), unionWithRes.cursor.firstBatch),
        ...
        const collStatsStage = {$collStats: {latencyStats: {}}};
        {
            ...
            assert.eq(1, collStatsRes.cursor.firstBatch.length, tojson(collStatsRes.cursor.firstBatch));
...
// access the other tenant's collection.
{
    ...
    const findOtherUser = assert.commandWorked(tokenDB2.runCommand({find: kCollName, filter: {a: 1}}));
    assert.eq(findOtherUser.cursor.firstBatch.length, 0, tojson(findOtherUser));

    const explainOtherUser = assert.commandWorked(tokenDB2.runCommand({explain: {find: kCollName, filter: {a: 1}}}));
    ...
    const countOtherUser = assert.commandWorked(tokenDB2.runCommand({count: kCollName, query: {c: 1}}));
    assert.eq(0, countOtherUser.n, tojson(countOtherUser));

    const distinctOtherUer = assert.commandWorked(tokenDB2.runCommand({distinct: kCollName, key: "d", query: {}}));
    assert.eq([], distinctOtherUer.values, tojson(distinctOtherUer));

    const fromName = kDbName + "." + kCollName;
    const toName = fromName + "_renamed";
    assert.commandFailedWithCode(
        tokenConn.getDB("admin").runCommand({renameCollection: fromName, to: toName, dropTarget: true}),
        ErrorCodes.NamespaceNotFound,
    );

    assert.commandFailedWithCode(tokenDB2.runCommand({listIndexes: kCollName}), ErrorCodes.NamespaceNotFound);
    assert.commandFailedWithCode(
        tokenDB2.runCommand({dropIndexes: kCollName, index: ["indexA", "indexB"]}),
        ErrorCodes.NamespaceNotFound,
    );

    assert.commandFailedWithCode(tokenDB2.runCommand({validate: kCollName}), ErrorCodes.NamespaceNotFound);
    ...
    assert(
        arrayEq(
            [expectedAdminDb],
            dbsWithDiffToken.databases.map((db) => db.name),
        ),
    ...
    // exists.
    tokenConn._setSecurityToken(securityToken);
    const collsAfterDropOtherTenant = assert.commandWorked(tokenDB.runCommand({listCollections: 1, nameOnly: true}));
    assert.eq(
        1,
...
// Test aggregation stage commands
{
    ...
    {
        const lookupPlannerRes = assert.commandWorked(
            tokenDB.runCommand({
                aggregate: kCollA,
        ...
        assert(arrayEq(lookupTarget, lookupPlannerRes.cursor.firstBatch), tojson(lookupPlannerRes.cursor.firstBatch));
...
// connection.
{
    ...
    // Find and modify the document
    {
        const fadCorrectConnection = assert.commandWorked(
            privelegedDB.runCommand({findAndModify: kCollName, query: {b: 1}, update: {$inc: {b: 10}}}),
        );
        assert.eq({_id: 0, a: 1, b: 1}, fadCorrectConnection.value, tojson(fadCorrectConnection));

        const fadOtherConnection = assert.commandWorked(
            otherPrivelegedDB.runCommand({findAndModify: kCollName, query: {b: 11}, update: {$inc: {b: 10}}}),
        );
        assert.eq(null, fadOtherConnection.value, tojson(fadOtherConnection));

        // Reset document data.
        assert.commandWorked(
            privelegedDB.runCommand({findAndModify: kCollName, query: {b: 11}, update: {$set: {a: 1, b: 1}}}),
        );
    }
    ...
    // Rename the collection
    {
        ...
        assert.commandWorked(privelegedAdminDB.runCommand({renameCollection: fromName, to: toName, dropTarget: true}));

        // Verify the the renamed collection by findAndModify existing documents.
        const fad1 = assert.commandWorked(
            privelegedDB.runCommand({
        ...
        // Reset the collection name and document data.
        assert.commandWorked(privelegedAdminDB.runCommand({renameCollection: toName, to: fromName, dropTarget: true}));
        assert.commandWorked(
            privelegedDB.runCommand({findAndModify: kCollName, query: {a: 11}, update: {$set: {a: 1, b: 1}}}),
        );
    }
}
...
// Test collMod
{
    ...
    res = assert.commandWorked(
        tokenDB.runCommand({"collMod": kCollName, "index": {"keyPattern": {c: 1}, expireAfterSeconds: 100}}),
    );
