...
function runFindOnPrefixedDb(conn, prefixedDb, collName, expectedDocsReturned) {
    ...
    const res = assert.commandWorked(conn.getDB(prefixedDb).runCommand({find: collName, filter: {}}));
 ...
 */
function runFindUsingSecurityTokenAndPrefix(conn, prefixedDb, collName, token, expectedDocsReturned) {
    conn._setSecurityToken(token);
    const res = assert.commandWorked(conn.getDB(prefixedDb).runCommand({find: collName, filter: {}}));
    assert(arrayEq(expectedDocsReturned, res.cursor.firstBatch), tojson(res));
    const prefixedNamespace = prefixedDb + "." + collName;
...
function assertTokenMustBeUsed(conn, tenant1DbPrefixed, tenant2DbPrefixed, kCollName) {
    conn._setSecurityToken(undefined);
    assert.commandFailedWithCode(conn.getDB(tenant1DbPrefixed).runCommand({find: kCollName, filter: {}}), 8233503);
    assert.commandFailedWithCode(conn.getDB(tenant2DbPrefixed).runCommand({find: kCollName, filter: {}}), 8233503);
}
...
assert.commandWorked(primary.getDB(tenant1DbPrefixed).runCommand({insert: kCollName, documents: tenant1Docs}));
...
assert.commandWorked(primary.getDB(tenant2DbPrefixed).runCommand({insert: kCollName, documents: tenant2Docs}));

assertFindBothTenantsPrefixedDb(primary, tenant1DbPrefixed, tenant2DbPrefixed, kCollName, tenant1Docs, tenant2Docs);
awaitReplication(rst);
assertFindBothTenantsPrefixedDb(secondary, tenant1DbPrefixed, tenant2DbPrefixed, kCollName, tenant1Docs, tenant2Docs);
...
secondary = rst.restart(secondary, {startClean: false, setParameter: {"multitenancySupport": true}});
...
assertFindBothTenantsUsingSecurityToken(secondary, kDbName, kCollName, kToken1, kToken2, tenant1Docs, tenant2Docs);

// Also assert both tenants find the new doc on the secondary using token and a prefixed db.
runFindUsingSecurityTokenAndPrefix(prefixedSecondary, tenant1DbPrefixed, kCollName, kExpectPrefixToken1, tenant1Docs);
runFindUsingSecurityTokenAndPrefix(prefixedSecondary, tenant2DbPrefixed, kCollName, kExpectPrefixToken2, tenant2Docs);
...
assert.commandWorked(primary.getDB(tenant1DbPrefixed).runCommand({insert: kCollName, documents: newTenant1Doc}));
assert.commandWorked(primary.getDB(tenant2DbPrefixed).runCommand({insert: kCollName, documents: newTenant2Doc}));
...
runFindAndModOnPrefixedDb(primary, tenant1DbPrefixed, kCollName, newTenant1Doc[0], {$set: {x: 4}}, newTenant1Doc[0]);
runFindAndModOnPrefixedDb(primary, tenant2DbPrefixed, kCollName, newTenant2Doc[0], {$set: {a: 40}}, newTenant2Doc[0]);
