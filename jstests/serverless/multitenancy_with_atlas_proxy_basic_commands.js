...
// Test create and listCollections commands, plus $listCatalog aggregate, on collection.
{
    ...
    assert.commandWorked(testDb.runCommand({"create": kViewName, "viewOn": kCollName, pipeline: []}));
    ...
    result = testDb.runCommand({aggregate: testColl.getName(), pipeline: [{$listCatalog: {}}], cursor: {}});
    ...
    const collsWithDiffTenant = assert.commandWorked(otherTestDb.runCommand({listCollections: 1, nameOnly: true}));
    assert.eq(0, collsWithDiffTenant.cursor.firstBatch.length, tojson(collsWithDiffTenant.cursor.firstBatch));
}
...
// Test listDatabases command.
{
    ...
    primary._setSecurityToken(otherSecurityToken);
    const dbsWithDiffTenant = assert.commandWorked(adminDb.runCommand({listDatabases: 1, nameOnly: true}));
    ...
    const expectedAllDbs = ["admin", "config", "local", kPrefixedDbName, kOtherDbName];

    assert.eq(expectedAllDbs.length, allDbs.databases.length);
    assert(
        arrayEq(
            expectedAllDbs,
            allDbs.databases.map((db) => db.name),
        ),
        tojson(allDbs),
    );

    // List all databases with tenant prefix filter. The tenant prefix is expected in response.
    const dbsWithTenantFilter = assert.commandWorked(
        adminDb.runCommand({listDatabases: 1, nameOnly: true, filter: {"name": new RegExp("(^" + kTenant + ")")}}),
    );
    const expectedDbsWithTenantFilter = [kPrefixedDbName, kOtherDbName];

    assert.eq(expectedDbsWithTenantFilter.length, dbsWithTenantFilter.databases.length, tojson(dbsWithTenantFilter));
    assert(
        arrayEq(
            expectedDbsWithTenantFilter,
            dbsWithTenantFilter.databases.map((db) => db.name),
        ),
...
// Test insert, agg, find, getMore, and explain commands.
{
    ...
    primary._setSecurityToken(securityToken);
    const findRes = assert.commandWorked(testDb.runCommand({find: kCollName, projection: {_id: 0}}));
    assert.eq(kTenantDocs.length, findRes.cursor.firstBatch.length, tojson(findRes.cursor.firstBatch));
    assert(arrayEq(kTenantDocs, findRes.cursor.firstBatch), tojson(findRes.cursor.firstBatch));

    primary._setSecurityToken(otherSecurityToken);
    const findRes2 = assert.commandWorked(otherTestDb.runCommand({find: kCollName, projection: {_id: 0}}));
    assert.eq(kOtherTenantDocs.length, findRes2.cursor.firstBatch.length, tojson(findRes2.cursor.firstBatch));
    assert(arrayEq(kOtherTenantDocs, findRes2.cursor.firstBatch), tojson(findRes2.cursor.firstBatch));

    // Test that getMore only works on a tenant's own cursor
    primary._setSecurityToken(securityToken);
    const cmdRes = assert.commandWorked(testDb.runCommand({find: kCollName, projection: {_id: 0}, batchSize: 1}));
    ...
    const cmdRes2 = assert.commandWorked(testDb.runCommand({find: kCollName, projection: {_id: 0}, batchSize: 1}));
    primary._setSecurityToken(otherSecurityToken);
    ...
    const aggRes = assert.commandWorked(
        testDb.runCommand({aggregate: kCollName, pipeline: [{$match: {w: 0}}, {$project: {_id: 0}}], cursor: {}}),
    );
    ...
    const aggRes2 = assert.commandWorked(
        otherTestDb.runCommand({aggregate: kCollName, pipeline: [{$match: {i: 1}}, {$project: {_id: 0}}], cursor: {}}),
    );
    ...
    assert.eq(kTenantDocs.length, kTenantExplainRes.executionStats.nReturned, tojson(kTenantExplainRes));
    primary._setSecurityToken(otherSecurityToken);
    const kOtherTenantExplainRes = assert.commandWorked(
        otherTestDb.runCommand({explain: {find: kCollName}, verbosity: "executionStats"}),
    );
    assert.eq(kOtherTenantDocs.length, kOtherTenantExplainRes.executionStats.nReturned, tojson(kOtherTenantExplainRes));
}
...
// Test insert and findAndModify command.
{
    ...
    const fad2 = assert.commandWorked(
        testDb.runCommand({findAndModify: kCollName, query: {a: 11}, update: {$set: {a: 1, b: 1}}}),
    );
...
// Test count and distinct command.
{
    ...
    primary._setSecurityToken(otherSecurityToken);
    const resCountOtherUser = assert.commandWorked(otherTestDb.runCommand({count: kCollName, query: {c: 1}}));
    assert.eq(0, resCountOtherUser.n, tojson(resCountOtherUser));

    // Test Distict command.
    primary._setSecurityToken(securityToken);
    const resDistinct = assert.commandWorked(testDb.runCommand({distinct: kCollName, key: "d", query: {}}));
    assert.eq([1, 2], resDistinct.values.sort(), tojson(resDistinct));
    primary._setSecurityToken(otherSecurityToken);
    const resDistinctOtherUser = assert.commandWorked(
        otherTestDb.runCommand({distinct: kCollName, key: "d", query: {}}),
    );
...
// Test renameCollection command.
{
    ...
    assert.commandWorked(adminDb.runCommand({renameCollection: fromName, to: toName, dropTarget: true}));

    // Verify the the renamed collection by findAndModify existing documents.
    const fad1 = assert.commandWorked(
        testDb.runCommand({findAndModify: kCollName + "_renamed", query: {a: 1}, update: {$inc: {a: 10}}}),
    );
    assert.eq({_id: 0, a: 1, b: 1}, fad1.value, tojson(fad1));
    ...
    primary._setSecurityToken(securityToken);
    assert.commandWorked(adminDb.runCommand({renameCollection: toName, to: fromName, dropTarget: true}));
}
...
// Test the dropCollection and dropDatabase commands.
{
    ...
    assert.eq(0, collsAfterDropCollection.cursor.firstBatch.length, tojson(collsAfterDropCollection.cursor.firstBatch));
...
// Test that transactions can be run successfully.
{
    ...
    assert.commandWorked(
        testDb.adminCommand({commitTransaction: 1, lsid: lsid, txnNumber: NumberLong(0), autocommit: false}),
    );
...
// Test createIndexes, listIndexes and dropIndexes command.
{
    ...
    primary._setSecurityToken(otherSecurityToken);
    assert.commandFailedWithCode(otherTestDb.runCommand({listIndexes: kCollName}), ErrorCodes.NamespaceNotFound);
    assert.commandFailedWithCode(
    ...
    primary._setSecurityToken(securityToken);
    res = assert.commandWorked(testDb.runCommand({dropIndexes: kCollName, index: ["indexA", "indexB"]}));
...
// Test collMod
{
    ...
    res = assert.commandWorked(
        testDb.runCommand({"collMod": kCollName, "index": {"keyPattern": {c: 1}, expireAfterSeconds: 100}}),
    );
