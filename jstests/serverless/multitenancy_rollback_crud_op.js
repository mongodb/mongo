...
const checkFinalResults = function (db) {
    ...
    validateCounts(db, kColl, tokenTenantB, [
        {q: {q: 1}, n: 1},
        {q: {q: 40}, n: 0},
    ]);
...
function setUpRst() {
    ...
    assert.eq(replSet.nodes.length, 3, "Mismatch between number of data bearing nodes and test configuration.");
...
updateDocs(syncSourceDB, kColl, tokenTenantA, [{q: {q: 0}, u: {$inc: {y: 33}}}]);
