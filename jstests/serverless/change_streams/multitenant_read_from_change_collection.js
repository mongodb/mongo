...
const startAtOperationTime = primary.getDB("local").oplog.rs.find().sort({ts: -1}).limit(1).next().ts;
...
const firstTenantResumeTokens = verifyEventsAndGetResumeTokens(firstTenantCsCursor, [
...
verifyEventsAndGetResumeTokens(firstTenantTestDb.stockPrice.watch([], {resumeAfter: firstTenantResumeTokens[0]}), [
    ["insert", firstTenantDocs[1]],
    ["insert", firstTenantDocs[2]],
]);
verifyEventsAndGetResumeTokens(secondTenantTestDb.stockPrice.watch([], {resumeAfter: secondTenantResumeTokens[0]}), [
    ["insert", secondTenantDocs[1]],
    ["insert", secondTenantDocs[2]],
]);
...
assert.throwsWithCode(() => assert.soon(() => secondTenantCsCursor.hasNext()), ErrorCodes.QueryPlanKilled);
