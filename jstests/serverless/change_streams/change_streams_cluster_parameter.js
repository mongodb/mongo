...
function assertGetResponse(db, expectedChangeStreamParam) {
    ...
    assert.eq(response.clusterParameters[0].expireAfterSeconds, expectedChangeStreamParam.expireAfterSeconds, response);
}
...
function testWithAdminDB(conn) {
    ...
    assert.commandFailedWithCode(
        adminDB.runCommand({setClusterParameter: {changeStreams: {expireAfterSeconds: NumberLong(-1)}}}),
        ErrorCodes.BadValue,
    );

    // A zero value of 'expireAfterSeconds' should fail.
    assert.commandFailedWithCode(
        adminDB.runCommand({setClusterParameter: {changeStreams: {expireAfterSeconds: NumberLong(0)}}}),
        ErrorCodes.BadValue,
    );

    // A positive value of 'expireAfterSeconds' should succeed.
    assert.commandWorked(
        adminDB.runCommand({setClusterParameter: {changeStreams: {expireAfterSeconds: NumberLong(36)}}}),
    );
    ...
    assert.commandWorked(
        adminDB.runCommand({setClusterParameter: {changeStreams: {expireAfterSeconds: NumberLong(100)}}}),
    );
...
function testWithoutAdminDB(conn) {
    ...
    assert.commandFailedWithCode(db.runCommand({getClusterParameter: "changeStreams"}), ErrorCodes.Unauthorized);
    assert.commandFailedWithCode(
        db.runCommand({setClusterParameter: {changeStreams: {expireAfterSeconds: NumberLong(3600)}}}),
...
// Tests the set and get change streams parameter on the sharded cluster.
{
    ...
    assert.commandFailedWithCode(
        adminDB.runCommand({setClusterParameter: {changeStreams: {expireAfterSeconds: NumberLong(40)}}}),
...
// the feature flag 'featureFlagServerlessChangeStreams' which has not been done in this scenario.
{
    ...
    assert.commandFailedWithCode(
        adminDB.runCommand({setClusterParameter: {changeStreams: {expireAfterSeconds: NumberLong(10)}}}),
