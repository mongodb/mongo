...
const restartNodeAsPrimary = function (originalConn) {
    const newPrimary = replSetTest.start(originalConn, {setParameter: setParameters, serverless: true}, true);
...
const runChangeCollectionStartupRecoveryTest = function (uncleanShutdown, testInStandalone) {
    jsTestLog(
        `Running startup recovery test. uncleanShutdown=${uncleanShutdown}, testInStandalone=${testInStandalone}`,
    );
    ...
    assert.commandWorked(
        firstTenantConn
            .getDB("admin")
            .runCommand({setClusterParameter: {changeStreams: {expireAfterSeconds: kExpireAfterSeconds}}}),
    );
    replSetTest.awaitReplication();
    ...
    if (uncleanShutdown) {
        ...
        replSetTest.stop(primary, 9, {allowedExitCode: MongoRunner.EXIT_SIGKILL}, {forRestart: true});
    } else {
    ...
    if (testInStandalone) {
        ...
        jsTest.log(
            `Confirming change collection entries of node originally at ${
                primaryHost
            } as standalone post shutdown (uncleanShutdown = ${uncleanShutdown})`,
        );
        ...
        MongoRunner.stopMongod(standaloneConn, null, {noCleanData: true, skipValidation: true, wait: true});
    }
...
runChangeCollectionStartupRecoveryTest(true /** uncleanShutdown **/, false /** testInStandalone **/);
runChangeCollectionStartupRecoveryTest(false /** uncleanShutdown **/, false /** testInStandalone **/);

runChangeCollectionStartupRecoveryTest(true /** uncleanShutdown **/, true /** testInStandalone **/);
runChangeCollectionStartupRecoveryTest(false /** uncleanShutdown **/, true /** testInStandalone **/);
