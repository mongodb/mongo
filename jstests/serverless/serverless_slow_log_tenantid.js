...
assert.commandWorked(primary.getDB("test").runCommand({insert: "foo", documents: [{_id: 0, a: 1, b: 1}]}));
...
const predicate = new RegExp(
    `Slow query.*"${kTenant.str}_test.foo.*"appName":"MongoDB Shell".*"command":` + `{"insert":"foo",`,
);
...
assert(
    ...
    "'Slow query' log line missing in mongod log file!\n" +
        "Log file contents: " +
        rst.getPrimary().fullOptions.logFile,
);
