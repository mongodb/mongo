...
assert.commandWorked(tenantConn.getDB("admin").runCommand({setChangeStreamState: 1, enabled: true}));
...
assert.soon(() => {
    ...
    return (
        serverStatusDiagnostics.hasOwnProperty("changeCollections") &&
        serverStatusDiagnostics.changeCollections.hasOwnProperty("purgingJob")
    );
});

const diagnosticsBeforeTestCollInsertions = verifyGetDiagnosticData(adminDb).serverStatus.changeCollections.purgingJob;
...
const wallTimeOfTheFirstOplogEntry = new NumberLong(
    changeCollection.find().sort({wall: 1}).limit(1).next().wall.getTime(),
);
const estimatedToBeRemovedDocsSize = changeCollection
    .find()
    .sort({wall: -1})
    .skip(1)
    .toArray()
...
assert.commandWorked(
    tenantConn.adminCommand({setClusterParameter: {changeStreams: {expireAfterSeconds: kExpireAfterSeconds}}}),
);
...
assert.soon(() => {
    ...
    return (
        diagnosticsAfterTestCollInsertions.totalPass > diagnosticsBeforeTestCollInsertions.totalPass &&
        ...
        diagnosticsAfterTestCollInsertions.maxStartWallTimeMillis.tojson() >= wallTimeOfTheFirstOplogEntry.tojson() &&
        diagnosticsAfterTestCollInsertions.timeElapsedMillis >= diagnosticsBeforeTestCollInsertions.timeElapsedMillis
    );
});
