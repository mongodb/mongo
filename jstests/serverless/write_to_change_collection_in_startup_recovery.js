...
const replSetTest = new ReplSetTest({nodes: 1, name: "ChangeStreamMultitenantReplicaSetTest", serverless: true});
...
const startTimestamp = assert.commandWorked(
    primary.getDB("test").runCommand({insert: "seedCollection", documents: [{_id: "beginTs"}]}),
).operationTime;
...
assert.commandWorked(primary.getDB("test").runCommand({insert: "stockPrice", documents: [{_id: "mdb", price: 250}]}));

// Verify that the inserted document can be queried from the 'stockPrice', the 'oplog.rs', and
// the 'system.change_collection'.
assert.eq(
    assert.commandWorked(primary.getDB("test").runCommand({find: "stockPrice", filter: {_id: "mdb", price: 250}}))
...
assert.eq(
    assert.commandWorked(
        primary
            .getDB("local")
            .runCommand({find: "oplog.rs", filter: {ns: "test.stockPrice", o: {_id: "mdb", price: 250}}}),
    ).cursor.firstBatch.length,
    1,
);
primary._setSecurityToken(token);
...
assert.eq(
    assert.commandWorked(standalone.getDB("test").runCommand({find: "stockPrice", filter: {_id: "mdb", price: 250}}))
        .cursor.firstBatch.length,
    0,
);
...
assert.eq(
    assert.commandWorked(
        standalone.getDB("config").runCommand({
            find: "system.change_collection",
            filter: {ns: "test.stockPrice", o: {_id: "mdb", price: 250}},
        }),
    ).cursor.firstBatch.length,
