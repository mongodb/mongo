...
function runTestWithSecurityTokenFlag() {
    ...
    checkDbStatsCommand({dbStats: 1}, tokenDb, true /*targetPass*/);
    ...
    // find with security token and prefixed DB using expectPrefix.
    {
        ...
        // TODO SERVER-78486: keep the false case after breaking the reliance on the feature flag
        if (featureFlagRequireTenantId) {
            assert(arrayEq([], findRes.cursor.firstBatch), tojson(findRes));
            checkNsSerializedCorrectly(
                kTenant + "_" + kTestDb, kCollName, findRes.cursor.ns, kTenant, true);
        } else {
...
function runTestWithoutSecurityToken() {
    ...
    // Run a sanity check to locate the collection
    checkDbStatsCommand({dbStats: 1}, prefixedDb, true /*targetPass*/);
    ...
    // find prefixed DB only (expectPrefix is ignored in parsing).
    {
        ...
        if (featureFlagRequireTenantId) {
            // Baseline sanity check.
            assert.commandFailedWithCode(prefixedDb.runCommand(request), 6972100);
