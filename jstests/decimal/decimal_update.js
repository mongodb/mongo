...
assert.commandWorked(
    col.update({}, {$inc: {"a": NumberDecimal("0")}}, {multi: true}),
    "update $inc 0 failed",
);
...
assert.commandWorked(
    col.update({}, {$mul: {"a": NumberDecimal("1")}}, {multi: true}),
    "update $mul failed",
);
assert.eq(col.find({a: 1}).count(), 4, "count after $mul incorrect");
assert.commandWorked(
    col.update({}, {$mul: {"a": NumberDecimal("2")}}, {multi: true}),
    "update $mul 2 failed",
);
assert.eq(col.find({a: 2}).count(), 4, "count after $mul incorrect");
assert.commandWorked(
    col.update({}, {$mul: {"a": NumberDecimal("0")}}, {multi: true}),
    "update $mul 0 failed",
);
