// Test that the explain will use backup plan if the original winning plan ran out of memory in the "executionStats" mode
...
(function() {
  ...
  // This query will not use the backup plan, hence it generates only two stages: winningPlan and rejectedPlans
  assert.commandWorked(db.foo.find({x: {$gte: 90}}).sort({_id: 1}).explain(true))

  // This query will use backup plan, the exaplin output for this query will generate three stages: winningPlan, rejectedPlans and originalWinningPlan
