...
function runTest(ascending) {
    ...
    assert.commandWorked(collIndexed.createIndex({t: -1}, {name: reverseIdxName}));

    if (ascending) {
        // A hint can only influence which index we use, not the direction.
        // TODO SERVER-64994 will fix this, by telling the query planner which
        // direction we prefer, conditional on it choosing a particular index.
        checkAgainstReferenceBoundedSortUnexpected(collIndexed,
                                                   referenceIndexed,
                                                   [
                                                       {$sort: {t: 1}},
    ...
    assert.commandWorked(collIndexed.dropIndex(reverseIdxName));
}
