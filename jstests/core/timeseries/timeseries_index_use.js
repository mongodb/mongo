...
/**
 * Creates the index specified by the spec and options, then explains the query to ensure that the
 * created index is used. Runs the query and verifies that the expected number of documents are
 * matched. Finally, deletes the created index.
 */
const testQueryUsesIndex = function(filter, numMatches, indexSpec, indexOpts = {}, queryOpts = {}) {
    ...
    // TODO (SERVER-56238): Remove conditional once queries always use the 'testIndexName' index.
...
assert.commandWorked(coll.insert([
    ...
    {_id: 1, [timeFieldName]: ISODate('2000-01-01 00:00:00.000Z'), [metaFieldName]: {a: 4, b: 5}},
    {_id: 2, [timeFieldName]: ISODate('2010-01-01 00:00:00.000Z'), [metaFieldName]: {a: "1", c: 0}}
]));
...
testQueryUsesIndex({'meta.a': {$lte: 4}, 'time': {$lte: timeDate}}, 2, {'meta.a': 1, 'time': 1});
testQueryUsesIndex({'meta.a': {$lte: 4}, 'time': {$lte: timeDate}}, 2, {'meta.a': 1, 'time': -1});
testQueryUsesIndex({'meta.a': {$eq: "1"}, 'time': {$gt: timeDate}}, 1, {'time': -1, 'meta.a': 1});
...
testQueryUsesIndex({'meta.a': {$eq: 1}, 'meta.b': {$gt: 0}}, 0, {'meta.a': "hashed", 'meta.b': -1});
testQueryUsesIndex({'meta.a': {$eq: 1}, 'meta.b': {$gt: 0}}, 0, {'meta.b': -1, 'meta.a': "hashed"});
...
testQueryUsesIndex({'meta.a': {$gte: 1}, 'meta.b': {$exists: 1}}, 1, {'meta.a': -1, 'meta.b': 1});
...
assert.commandWorked(coll.insert([
    {_id: 0, [timeFieldName]: ISODate('1990-01-01 00:00:00.000Z'), [metaFieldName]: "hello hello"},
    {_id: 1, [timeFieldName]: ISODate('2000-01-01 00:00:00.000Z'), [metaFieldName]: "hello world"},
