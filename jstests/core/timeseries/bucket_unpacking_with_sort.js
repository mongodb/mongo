...
/**
 * On 5.0, timeseries does not support hinting by index key, at least when
 * this test was backported. We support $natural, and index names.
 *
 * So this helper translates an index spec to the expected index name.
 */
function fixHint(spec) {
    if (spec == null || spec.$natural)
        return spec;
    return Object.keys(spec).map(field => `${field}_${spec[field]}`).join('_');
}
assert.eq(fixHint({m: 1, t: -1}), 'm_1_t_-1');
assert.eq(fixHint({$natural: -1}), {$natural: -1});
assert.eq(fixHint(null), null);
...
try {
    ...
    if (e.code == ErrorCodes.InvalidOptions &&
        e.message.match(/setWindowFields cannot run with readConcern/)) {
...
for (let m = -1; m < 2; m++) {
    for (let t = -1; t < 2; t++) {
        for (let k = -1; k < 2; k++) {
            ...
            let usesMeta = null;
            if (k != 0) {
                // This is the case where we add an intermediary incompatible field: it's not
                // metadata, and not time. $_internalBoundedSort can't satisfy this kind of
                // sort.
                if (m == 0) {
                    if (t == 0) {
                        sort = {k: k};
