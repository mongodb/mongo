...
const tsColl = assertDropAndRecreateCollection(db, jsTestName() + "_timeseries", {
    timeseries: {timeField: "time", metaField: "mt"},
});
...
function checkAllBucketings(predicate, documents) {
    ...
    for (let bucketing = 0; bucketing < numBucketings; ++bucketing) {
        ...
        const labeledDocs = documents.map((doc, i) =>
            Object.merge(doc, {meta: {bucket: bucketing & (1 << i)}}, true /*deep*/),
        );
...
// Test $in...
{
    ...
    checkAllBucketings({x: {$in: [2, 3, 4]}}, [{x: 1}, {x: 2}, {x: 3}, {x: 42}]);
...
// $exists: true
checkAllBucketings({x: {$exists: true}}, [{}, {x: 2}, {x: ISODate()}, {x: null}, {x: undefined}]);
...
// Test $or...
{
    ...
    checkAllBucketings(
        {
            $or: [{x: {$lt: 0}}, {y: {$exists: false}}],
        },
        [{x: -1}, {x: +1}, {x: -1, y: "asdf"}, {x: +1, y: "asdf"}],
    );

    // ... when neither argument can be pushed down.
    checkAllBucketings(
        {
            $or: [{x: {$exists: false}}, {y: {$exists: false}}],
        },
        [{}, {x: "qwer"}, {y: "asdf"}, {x: "qwer", y: "asdf"}],
    );
}
...
// Test $and...
{
    ...
    checkAllBucketings(
        {
            $and: [{x: {$exists: false}}, {y: {$exists: false}}],
        },
        [{}, {x: "qwer"}, {y: "asdf"}, {x: "qwer", y: "asdf"}],
    );
}
...
checkAllBucketings(
    {
        ...
        $or: [
            {
                $and: [{"mt.a": {$gt: 0}}, {"x": {$lt: 0}}],
            },
            {
                $and: [{"mt.b": {$gte: 0}}, {time: {$gt: ISODate("2020-01-01")}}],
            },
        ],
    },
    [
...
checkAllBucketings(
    {
        $or: [{"mt.a": {$elemMatch: {b: 2, c: 3}}}, {"x": {$gt: 3}}],
    },
    [
        {x: 1, mt: {a: []}},
        {x: 2, mt: {a: [{b: 2, c: 3}]}},
