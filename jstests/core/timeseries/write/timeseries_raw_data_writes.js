...
crudTest(() => {
    assert.eq(
        getTimeseriesCollForRawOps(coll).deleteOne({"control.count": 2}, kRawOperationSpec)
            .deletedCount,
        1,
    );
});
...
crudTest(() => {
    assert.eq(
        getTimeseriesCollForRawOps(coll).deleteMany({"control.version": 2}, kRawOperationSpec)
            .deletedCount,
        2,
    );
});
...
// findOneAndDelete()
crudTest(() => {
    const deletedBucket = getTimeseriesCollForRawOps(coll).findOneAndDelete(
        {"control.count": 2},
        kRawOperationSpec,
    );
...
crudTest(() => {
    ...
    assert.eq(
        getTimeseriesCollForRawOps(coll).findOne(
            {"control.count": 2},
            null,
            null,
            null,
...
crudTest(() => {
    assert.eq(
        getTimeseriesCollForRawOps(coll).update(
            {"control.count": 2},
            {$set: {meta: "3"}},
            kRawOperationSpec,
        ).nModified,
...
crudTest(() => {
    assert.eq(
        getTimeseriesCollForRawOps(coll).updateOne(
            {"control.count": 2},
            {$set: {meta: "3"}},
            kRawOperationSpec,
        ).modifiedCount,
...
crudTest(() => {
    assert.eq(
        getTimeseriesCollForRawOps(coll).updateMany(
            {"control.version": 2},
            {$set: {meta: "3"}},
            kRawOperationSpec,
        ).modifiedCount,
...
crudTest(() => {
    assert.eq(
        getTimeseriesCollForRawOps(coll).replaceOne(
            {"control.count": 2},
            replaceOneBucket,
            kRawOperationSpec,
        ).modifiedCount,
