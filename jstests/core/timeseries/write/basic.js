...
function validateBuckets(bucketsDocs, expectedLogicalDocNum) {
    ...
    assert.eq(
        expectedLogicalDocNum,
        logicalDocNum,
        `Unexpected numnber of logical documents found by looking at buckets metadata ${bucketsDocs}`,
    );
}
...
function testRawDataQueries() {
    let bucketsDocs = getTimeseriesCollForRawOps(coll)
    ...
    assert.eq(
        numBucketDocs,
        getTimeseriesCollForRawOps(coll).distinct("_id", bucketPredicate, kRawOperationSpec).length,
    );
