...
assert.commandWorked(
    tc.insert({
        ...
        text: "Linn-Kristin Riegelhuth Koren (born 1 August 1984, in Ski) is a Norwegian handballer playing for Larvik HK and the Norwegian national team. She is commonly known as Linka. Outside handball she is a qualified nurse.",
    }),
);
assert.commandWorked(
    tc.insert({
...
assert.commandWorked(tc.createIndex({"$**": "text"}, {weights: {"title": 10}}));

// -------------------------------------------- STEMMING -------------------------------------------

// tests stemming for basic plural case
res = tc.find({"$text": {"$search": "member"}});
let res2 = tc.find({"$text": {"$search": "members"}});
