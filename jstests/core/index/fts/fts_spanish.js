...
assert.commandWorked(coll.insert({_id: 2, title: "mi segundo post", text: "Este es un blog de prueba"}));
assert.commandWorked(coll.insert({_id: 3, title: "cuchillos son divertidos", text: "este es mi tercer blog stemmed"}));
assert.commandWorked(
    coll.insert({_id: 4, language: "en", title: "My fourth blog", text: "This stemmed blog is in english"}),
);

// Create a text index, giving more weight to the "title" field.
assert.commandWorked(coll.createIndex({title: "text", text: "text"}, {weights: {title: 10}, default_language: "es"}));
