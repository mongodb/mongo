...
assert.eq(
    {_id: 4, numbers: "four", addedField: 2},
    coll.findAndModify({
...
assert.eq(
    {_id: 3, numbers: "one two", score: 1.5},
    coll.findAndModify({
...
// new version of the document.
assert.eq(
    {_id: 3, numbers: "one two", addedField: 2},
    coll.findAndModify({
        query: {$text: {$search: "one two"}},
        update: [{$set: {addedField: {$add: ["$addedField", 1]}}}],
        sort: {score: {$meta: "textScore"}},
...
assert.eq(
    {_id: 3, numbers: "one two", addedField: 2, score: 1.5},
    coll.findAndModify({
        query: {$text: {$search: "one two"}},
        update: [{$set: {addedField: {$add: ["$addedField", 1]}}}],
        fields: {score: {$meta: "textScore"}},
...
assert.eq(
    {_id: 3, numbers: "one two", addedField: 4, score: 1.5},
    coll.findAndModify({
        query: {$text: {$search: "one two"}},
        update: [{$set: {addedField: {$add: ["$addedField", 1]}}}],
        fields: {score: {$meta: "textScore"}},
