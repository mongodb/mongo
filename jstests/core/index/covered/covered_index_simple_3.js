...
assert(isIndexOnly(db, plan.queryPlanner.winningPlan), "simple.3.1 - indexOnly should be true on covered query");
...
assert(isIndexOnly(db, plan.queryPlanner.winningPlan), "simple.3.2 - indexOnly should be true on covered query");
...
assert(isIndexOnly(db, plan.queryPlanner.winningPlan), "simple.3.3 - indexOnly should be true on covered query");
...
if (!TestData.isHintsToQuerySettingsSuite) {
    ...
    assert(isIndexOnly(db, plan.queryPlanner.winningPlan), "simple.3.4 - indexOnly should be true on covered query");
...
var plan = coll
    ...
    .hint({foo: 1})
    .explain("executionStats");
assert(isIndexOnly(db, plan.queryPlanner.winningPlan), "simple.3.5 - indexOnly should be true on covered query");
...
var plan = coll
    ...
    .hint({foo: 1})
    .explain("executionStats");
assert(isIndexOnly(db, plan.queryPlanner.winningPlan), "simple.3.6 - indexOnly should be true on covered query");
assert.eq(0, plan.executionStats.totalDocsExamined, "simple.3.6 - docs examined should be 0 for covered query");

// Test $exists true
var plan = coll
    .find({foo: {$exists: true}}, {foo: 1, _id: 0})
    .hint({foo: 1})
    .explain("executionStats");
assert(isIndexOnly(db, plan.queryPlanner.winningPlan), "simple.3.7 - indexOnly should be true on covered query");
...
var plan = coll
    .find({bar: {$nin: [5, 8]}}, {bar: 1, _id: 0})
    .hint({bar: 1})
    .explain("executionStats");
assert(isIndexOnly(db, plan.queryPlanner.winningPlan), "simple.3.8 - indexOnly should be true on covered query");
