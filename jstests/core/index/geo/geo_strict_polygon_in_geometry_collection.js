...
const strictGeoCollection = {
    ...
    geometries:
        [{type: "Polygon", coordinates: [[[0, 0], [5, 0], [5, 5], [0, 5], [0, 0]]], crs: strictCRS}]
};
...
// collection is matched correctly, confirming the query works and not just silently skips all docs.
{
    ...
    assert(!indexes.some(idx => idx.key && idx.key.geo === "2dsphere"),
    ...
    const intersectResults =
        coll.find({geo: {$geoIntersects: {$geometry: queryPolygon}}}).toArray();
...
// fails and no partial index is left behind.
{
    ...
    assert.soon(() => !coll.getIndexes().some(idx => idx.key && idx.key.geo === "2dsphere"),
