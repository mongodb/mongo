...
function testGeoWithinCenterSphereLinePolygon(coll) {
    ...
    function testGeoWithinCenterSphere(centerSphere, expected) {
        let result = coll
            .find({geoField: {$geoWithin: {$centerSphere: centerSphere}}}, {"name": 1, "_id": 0})
            .sort({"name": 1})
            .toArray();
    ...
    testGeoWithinCenterSphere(
        [[174.75211152791763, -36.88962755605813], 0.000550933650273084],
        [{name: "LineString3"}],
    );
    ...
    geoDoc = {
        ...
        "geoField": {
            "type": "LineString",
            "coordinates": [
                [96.328125, 5.61598581915534],
                [153.984375, -6.315298538330033],
            ],
        },
    };
    assert.commandWorked(coll.insert(geoDoc));
