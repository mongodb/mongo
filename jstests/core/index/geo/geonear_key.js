...
function assertGeoNearSucceedsAndReturnsIds(nearParams, expectedIds) {
    ...
    let errfn = () =>
        `expected ids ${tojson(expectedIds)}, but these documents were ` +
        `returned: ${tojson(res)}`;
...
assertGeoNearSucceedsAndReturnsIds(
    {near: {type: "Point", coordinates: [0, 0]}, key: "b.d"},
    [4, 5],
);
...
assertGeoNearSucceedsAndReturnsIds(
    {near: {type: "Point", coordinates: [0, 0]}, spherical: true, key: "a"},
    [0, 1],
);
...
assertGeoNearFails(
    {near: {type: "Point", coordinates: [0, 0]}, key: "b.c"},
