...
for (var i = 0; i < 3; i++) {
    ...
    // $or with duplicate predicates on the same path optimized to a single {_id: 1} update
    assert.eq(1, upsertedId({$or: [{_id: 1}, {_id: 1}]}, expr));
...
for (var i = 0; i < 3; i++) {
    ...
    // $or with duplicate predicates on the same path optimized to a single {_id: 1} update
    assert.eq(value, upsertedXVal({$or: [{x: 1}, {x: 1}]}, expr));
