...
const coll1 = db.getCollection(jsTestName() + "_1");
const coll2 = db.getCollection(jsTestName() + "_2");
...
db.setProfilingLevel(1, {filter: {ns: {$in: [coll1.getFullName(), coll2.getFullName()]}}});
...
const profileEntries = getNLatestProfilerEntries(db, 4).reverse();
...
assert.eq(profileInsert.keysInserted, 3 * 2, tojson(profileInsert));
...
assert.eq(profileInsert.keysInserted, 1, tojson(profileInsert));
assert(profileInsert.hasOwnProperty("numYield"), tojson(profileInsert));
assert(profileInsert.hasOwnProperty("locks"), tojson(profileInsert));
assert(profileInsert.hasOwnProperty("millis"), tojson(profileInsert));
assert(profileInsert.hasOwnProperty("ts"), tojson(profileInsert));
...
assert.eq(profileDelete.keysDeleted, 5 * 2, tojson(profileDelete));
