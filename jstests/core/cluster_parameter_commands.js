...
for (const [name, data] of Object.entries(kAllClusterParameters)) {
    jsTestLog("Running test for cluster parameter " + name + " with data " + tojson(data) +
              " and current FCV " + currentFCVInMultiversion);

    if (data.hasOwnProperty("featureFlag")) {
        if (data.featureFlag[0] !== '!' &&
            !FeatureFlagUtil.isPresentAndEnabled(conn, data.featureFlag)) {
