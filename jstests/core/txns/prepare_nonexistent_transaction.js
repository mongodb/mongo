...
jsTestLog(
    "Test that if there is a transaction running on the current session and the " +
...
assert.commandWorked(session.abortTransaction_forTesting());
jsTestLog(
    "Test that if there is no transaction active on the current session, the " +
...
jsTestLog(
    "Test that if there is no transaction active on the current session and the " +
        "'txnNumber' given is less than the current transaction, errors with " +
        "'TransactionTooOld'.",
);
assert.commandFailedWithCode(
...
jsTestLog(
    "Test the error precedence when calling prepare on a nonexistent transaction but " +
...
jsTestLog(
    "Test the error precedence when calling prepare on a nonexistent transaction and " +
