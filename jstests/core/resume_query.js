...
assert.commandWorked(db.runCommand({killCursors: "test", cursors: [res.cursor.id]}));
...
res = assert.commandWorked(db.runCommand({getMore: cursorId, collection: "test", batchSize: 1}));
