...
function runTest(documentList, arrayFieldName, cachedQuery, notCachedQuery, queryCollation) {
    ...
    const queryCollationForLookup =
        queryCollation.hasOwnProperty("collation") ? queryCollation.collation : {};
    const key1 = getPlanCacheKeyFromShape(
        {query: cachedQuery, collection: coll, db: db, collation: queryCollationForLookup});
    const key2 = getPlanCacheKeyFromShape(
        {query: notCachedQuery, collection: coll, db: db, collation: queryCollationForLookup});
