 ...
 * original state.
 * It's important that each run of the property is independent from one another, so in case the
 * function throws an exception, we'll catch it, then reset the knobs, and then re-throw the
 * exception. This way, before and after the property, we are in the same state for the query knobs.
 */
function runWithKnobs(knobToVal, fn) {
    ...
    // Set the requested knobs. Catch any exception from running the function, so we don't exit this
    // function without setting the knobs back.
    ...
    // TODO SERVER-104420 consider using `runCommandOnAllShards` to send the command to every shard.
    assert.commandWorked(db.adminCommand({setParameter: 1, ...priorSettings}));

    // If we threw an exception, it's now safe to re-throw it so the property fails.
    if (thrownException) {
