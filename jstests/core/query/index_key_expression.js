...
testScenarios.forEach((testScenario) => {
    ...
    if (testScenario.expectedIndexKeys) {
        assert.eq(
            internalCollection.aggregate(pipeline, {readConcern: {}, writeConcern: {w: "majority"}}).toArray()[0],
            {_id: testScenario.expectedIndexKeys},
            testScenario,
        );
    } else {
        assert.throwsWithCode(
            () => internalCollection.aggregate(pipeline, {readConcern: {}, writeConcern: {w: "majority"}}).toArray(),
