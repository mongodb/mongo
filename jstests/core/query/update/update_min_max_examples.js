...
assert.commandWorked(coll.insert({_id: 3, a: date}));
...
// $max for Date
assert.commandWorked(coll.insert({_id: 4, a: date}));
// setMilliseconds() will roll over to change seconds if necessary.
date.setMilliseconds(date.getMilliseconds() + 2);
// Test that we have advanced the date and it's no longer the same as the one we inserted.
...
assert.commandWorked(coll.insert({_id: 5, a: 1e-15}));
...
assert.commandWorked(coll.insert({_id: 6, a: biggerval}));
