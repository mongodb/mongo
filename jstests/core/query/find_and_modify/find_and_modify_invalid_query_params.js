...
function assertFailedWithCode(cmd, errorCode) {
    assert.commandFailedWithCode(
        coll.runCommand(Object.merge({findAndModify: coll.getName()}, cmd)),
        errorCode,
    );
}
...
assertFailedWithCode(
    {update: {}, arrayFilters: [{"x.a": {$gt: 85}}], remove: true},
...
assertWorked(
    {query: {_id: 1}, update: {value: "newerVal"}, new: 1, remove: false, upsert: 1},
    "newerVal",
);
assertWorked(
...
assertFailedWithCode(
    {query: {_id: 0}, update: {value: "usedHint"}, hint: 1},
    ErrorCodes.FailedToParse,
);
...
assertWorked(
    {query: {_id: 0}, update: {value: "usedHint"}, new: 1.0, hint: "value", remove: 0.0},
    "usedHint",
);
assertWorked(
    {
