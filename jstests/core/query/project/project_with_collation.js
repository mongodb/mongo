...
function runQueryWithCollation(testColl, collationToUse) {
    ...
    assert.docEq(findAndUpdateWithSortArrayOutput, {
        _id: 0,
        str: "a",
        sortedArray: [{str: "A"}, {str: "a"}, {str: "b"}, {str: "B"}],
    });
...
function queryWithSimpleCollation(testColl) {
    const elemMatchOutput = testColl
        .find({str: "A", elemMatch: {$elemMatch: {str: "a"}}}, {_id: 1, "elemMatch.$": 1})
        .collation({locale: "simple"})
        .toArray();
    assert.sameMembers(elemMatchOutput, [{_id: 2, elemMatch: [{str: "a"}]}]);

    const sortArrayOutput = testColl
        .find({str: "a"}, {sortedArray: {$sortArray: {input: "$array", sortBy: {str: 1}}}})
        .collation({locale: "simple"})
        .toArray();
    assert.sameMembers(sortArrayOutput, [
    ...
    assert.docEq(findAndUpdateWithSortArrayOutput, {
        _id: 0,
        str: "a",
        sortedArray: [{str: "A"}, {str: "B"}, {str: "a"}, {str: "b"}],
    });
