...
assert(db[regularColl].drop());
assert(db[timeseriesColl].drop());
assert.commandWorked(db.createCollection(regularColl));
assert.commandWorked(db.createCollection(timeseriesColl, {timeseries: {timeField: "t", metaField: "m"}}));
...
assert.commandWorked(db.runCommand({insert: regularColl, documents: [measurement(2)], stmtIds: [NumberInt(-1)]}));
