...
function insertDocsAndGetRecordIds(coll, testData) {
    ...
    const recordIdMap = {};
    const res = assert.commandWorked(db.runCommand({find: coll.getName(), hint: {$natural: 1}, showRecordId: true}));
...
const testFindCmd = function () {
    ...
    jsTestLog("[Find] resumeAfter should successfully resume with a resume token pointing to a valid recordId.");
    ...
    jsTestLog("[Find] startAt should successfully resume with a resume token pointing to a valid recordId.");
    res = assert.commandWorked(
        db.runCommand({
            find: collName,
            hint: {$natural: 1},
    ...
    jsTestLog("[Find] startAt should successfully resume with a resume token pointing to a deleted recordId.");
...
const testAggregateCmd = function () {
    ...
    jsTestLog("[Aggregate] startAt should successfully resume with a resume token pointing to a deleted recordId.");
    // Try to resume the query using $_startAt from the same token and check that it tolerates
    // resumeToken pointing to a deleted recordId.
    res = assert.commandWorked(
        db.runCommand({
    ...
    jsTestLog("[Aggregate] resumeAfter should fail with a resume token pointing to a deleted recordId.");
