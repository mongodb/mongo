...
// For Array.shuffle() we need to seed the RNG.
...
// Test that plan selection avoids bias toward the first $or branch.
{
    ...
    // Shuffle the documents to avoid any bias in the storage layer toward one index or the other.
    // This is needed to pass the burn-in tests.
    assert.commandWorked(coll.insertMany(Array.shuffle(docs)));
    ...
    // See: internalQueryPlanEvaluationMaxResults and internalQueryEnumerationMaxOrSolutions
    // for more details on how subplanning is handled.
    const q = {
        $or: [
            {a: {"$lte": 102}, b: {"$gte": 0}},
            {a: {"$gte": 0}, b: {"$lte": 102}},
