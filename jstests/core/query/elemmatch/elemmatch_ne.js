...
{
    // Test equality inside $elemmatch.
    const res = coll.explain("executionStats")
                    .find({arr: {$elemMatch: {a: {$eq: 1}}}, $or: [{c: 4, d: 5}, {c: 6, d: 7}]})
                    .finish();
    assert.eq(1, res.executionStats.nReturned);

    // Verify we get an indexed plan.
    assert.eq("IXSCAN", res.queryPlanner.winningPlan.queryPlan.inputStage.stage);
}

{
    ...
    // Verify we get a collection scan.
    assert.eq("COLLSCAN", res.queryPlanner.winningPlan.queryPlan.stage);
}
}());
