...
db.c.drop();
assert.commandWorked(db.c.insert({_id: 0, array: []}));
// When unwinding an empty array with preserveNullAndEmptyArrays and includeArrayIndex,
// the array index field must be null.
const pipeline = [
    {
        "$unwind": {
            path: "$array",
...
assert.commandWorked(db.c.deleteMany({}));
...
// When includeArrayIndex uses the same name as the unwind path, $addFields
...
assert.commandWorked(db.c.deleteMany({}));
assert.commandWorked(db.c.insert({"_id": 105, "m": {"m1": 0}, "a": 0}));
// Unwinding a missing nested field with preserveNullAndEmptyArrays and
// includeArrayIndex allows the overwritten field to be projected out.
const pipeline3 = [
    {
        "$project": {
            "_id": 0,
...
// Referencing a projected-out field after unwinding an empty array leaves the
// field absent.
const pipeline4 = [
    {
        "$project": {
