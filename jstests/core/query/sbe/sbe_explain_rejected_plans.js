...
for (let executionStage of executionStages) {
    ...
    assert.neq(
        ixscans.length, 0, {reason: "Expected ixseek to be present", explain, executionStages});
    for (let ixscan of ixscans) {
...
for (let rejectedPlan of getRejectedPlans(explain)) {
    ...
    assert.neq(
        ixscans.length, 0, {reason: "Expected rejected plans to use a1 or b1 index", explain});
    for (let ixscan of ixscans) {
