...
jsTestLog(
    "Check that tailing the capped collection with awaitData eventually ends up returning " +
...
if (FixtureHelpers.isReplSet(db)) {
    ...
    jsTestLog(
        "Check that tailing the oplog with awaitData eventually ends up returning " +
            "an empty batch after hitting the timeout.",
    );
