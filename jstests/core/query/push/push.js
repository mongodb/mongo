...
assert.sameMembers([1, 2], t.findOne().a, "Array should contain 2 elements after removing the last element.");
assert.commandWorked(t.update({_id: 2}, {$pop: {a: -1}}));
assert.sameMembers([2], t.findOne().a, "Array should contain 1 element after removing the first element.");

assert.commandWorked(t.update({_id: 2}, {$pop: {a: -1}}));
assert.sameMembers([], t.findOne().a, "Array should be empty after removing the first element");

assert.commandWorked(t.update({_id: 2}, {$pop: {a: 1}}));
assert.sameMembers([], t.findOne().a, "Empty array should remain empty after trying to remove the last element.");
