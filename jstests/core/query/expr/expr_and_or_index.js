...
function assertIndexBounds(explain, stage, expectedBounds, testId) {
    ...
    if (stage == "IXSCAN") {
        assert.eq(
            ...
            expectedBounds,
            `Expected bounds ${tojson(expectedBounds)} but got ${tojson(ixscans[0].indexBounds)} in test ${testId}` +
                tojson(explain),
        );
    } else {
        const clusteredIxscanBounds = `[${ixscans[0].minRecord.toFixed(1)}, ${ixscans[0].maxRecord.toFixed(1)}]`;
        assert.eq(
            {"_id": [clusteredIxscanBounds]},
            expectedBounds,
            `Expected bounds ${tojson(expectedBounds)} but got ${tojson({
                "_id": [clusteredIxscanBounds],
            })} in test ${testId}` + tojson(explain),
        );
    }
}
...
function test({testId, query, expectedStage, expectedBounds = {}}) {
    ...
    if (expectedStage == "IXSCAN") {
        ...
        assert(
            winningPlanHasStage(explain, expectedStage),
