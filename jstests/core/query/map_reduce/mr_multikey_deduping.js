...
(function testSingleKeyIndex() {
    ...
    res = assert.commandWorked(
        coll.mapReduce(mapFn, reduceFn, {out: {merge: outColl.getName()}, query: {arr: {$gte: 0}}}),
    );
    assert(outColl.drop());
...
(function testCompoundIndex() {
    ...
    const resultsNoIndexNoQuery = assert.commandWorked(
        db.runCommand({mapreduce: coll.getName(), map: mapFn, reduce: reduceFn, out: {inline: 1}}),
    ).results;
    const resultsNoIndexEqualityOnName = assert.commandWorked(
        db.runCommand({
    ...
    const resultsNoIndexRangeOnName = assert.commandWorked(
        db.runCommand({
            mapreduce: coll.getName(),
            map: mapFn,
            reduce: reduceFn,
    ...
    const resultsIndexedNoQuery = assert.commandWorked(
        db.runCommand({mapreduce: coll.getName(), map: mapFn, reduce: reduceFn, out: {inline: 1}}),
    ).results;
    const resultsIndexedEqualityOnName = assert.commandWorked(
        db.runCommand({
            mapreduce: coll.getName(),
            map: mapFn,
            reduce: reduceFn,
    ...
    const resultsIndexedRangeOnName = assert.commandWorked(
        db.runCommand({
            mapreduce: coll.getName(),
            map: mapFn,
            reduce: reduceFn,
