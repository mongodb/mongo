...
for (const [name, data] of Object.entries(kAllClusterParameters)) {
    ...
    if (currentFCVInMultiversion && data.hasOwnProperty("minFCV") &&
    ...
    if (data.hasOwnProperty("isSetInternally") && data.isSetInternally) {
        // Skip the parameters that are set internally to avoid altering the cluster services that
        // use them.
        continue;
    }
    ...
    if (typeof data.default === "object") {
        assert.commandFailedWithCode(
            conn.getDB("admin").runCommand(
                {setClusterParameter: {[name]: {"unknownField": "unknownValue", ...data.default}}}),
