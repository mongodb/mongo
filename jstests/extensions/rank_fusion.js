/**
 * Tests $rankFusion with extension stages on a sharded cluster.
 *
 * Exercises: $rankFusion inside $lookup targeting a view; $matchTopN in $rankFusion inside
...
describe("$rankFusion with extension stages on a sharded cluster", function () {
    ...
    it("$rankFusion with $matchTopN is allowed inside $lookup targeting a view on a sharded cluster", function () {
                        ...
                        pipeline: [
                            {
                                $rankFusion: {
                                    input: {
                                        pipelines: {a: [matchTopNStage], b: [{$sort: {y: -1}}]},
                                    },
                                },
                            },
                        ],
        ...
        assert.gt(res[0].joined.length, 0, "$rankFusion inside $lookup should return results", {
            res,
        });
    });

    it("$matchTopN in $rankFusion inside $unionWith against a view is allowed on a sharded cluster", function () {
    ...
    it("$addFieldsMatch in a $rankFusion input pipeline is rejected at parse time at the router", function () {
    ...
    it("$nativeVectorSearch in a $rankFusion input pipeline is rejected at parse time at the router", function () {
        assertRejectedAtParseTime(
            [
                {
                    $rankFusion: {
