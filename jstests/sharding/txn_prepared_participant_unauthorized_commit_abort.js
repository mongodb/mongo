/**
 * Tests that a user cannot commit or abort a prepared transaction on an individual txn participant
 * (shard).
 *
 * Tagged does_not_support_stepdowns because the test freezes the coordinator at a failpoint and
 * drives a precise 2PC race that config-server stepdowns would disrupt.
 *
 * @tags: [
 *   uses_transactions,
...
load("jstests/sharding/libs/sharded_transactions_helpers.js");  // getOplogEntriesForTxnOnNode
...
// ShardingTest is a global on this branch (jstests/libs/shardingtest.js does not exist).
const st = new ShardingTest({name: jsTestName(), shards: 2, rs: {nodes: 1}, keyFile: kKeyFile});

try {
    // rs0's primary is the 2PC coordinator (every transaction below writes to shard0 first).
    // rs1's primary is a pure participant -- the shard the user will try to commit/abort on
    // directly.
    const coordPrimary = st.rs0.getPrimary();
    const participantPrimary = st.rs1.getPrimary();

    // Create the low-privilege user and set up sharding as the cluster (__system).
    const userRoles = [{role: "readWrite", db: kDbName}];
    authutil.asCluster(st.s, kKeyFile, () => {
        // The user authenticates against mongos (config-server user) to run the transaction.
        st.s.getDB(kDbName).createUser({user: kUser, pwd: kUserPwd, roles: userRoles});
        ...
        assert.commandWorked(st.s.adminCommand({shardCollection: kNs, key: {_id: 1}}));
        assert.commandWorked(st.s.adminCommand({split: kNs, middle: {_id: 0}}));
        assert.commandWorked(
            st.s.adminCommand({moveChunk: kNs, find: {_id: 0}, to: st.shard1.shardName}),
        );
    });

    // A direct connection to a shard resolves users from that shard's *local* store, so create the
    // same user there too. The logical-session uid is SHA256("rwuser@<db>"), i.e. purely
    // name-based, so this shard-local user yields the identical uid the coordinator used when it
    // prepared the participant -- the user's direct commit/abort targets that exact prepared
    // session.
    authutil.asCluster(participantPrimary, kKeyFile, () => {
        participantPrimary.getDB(kDbName).createUser(
            {user: kUser, pwd: kUserPwd, roles: userRoles});
    });
    ...
    const startCrossShardTxn = (lsid, txnNumber, idOnShard0, idOnShard1) => {
        const user = new Mongo(st.s.host);
    ...
    const prepareAllParticipantsAndFreezeCoordinator = (lsid, txnNumber) => {
        ...
        const fp = authutil.asCluster(
            coordPrimary,
            kKeyFile,
            () => configureFailPoint(coordPrimary, kCoordinatorFp),
        );
        ...
        // prepared.
        authutil.asCluster(coordPrimary, kKeyFile, () => fp.wait());
        return {commitThread, fp};
    };

    jsTestLog("rejects a non-coordinator client committing a prepared participant");
    (() => {
        const lsid = {id: UUID()};
        ...
        try {
            ...
            const userDirect = new Mongo(participantPrimary.host);
            ...
            authutil.asCluster(coordPrimary, kKeyFile, () => fp.off());
            commitThread.join();
        }

        assert.commandWorked(
            commitThread.returnData(),
            "the client's commit through mongos should succeed",
        );

        // Atomicity holds: the coordinator drove both shards to commit at its single decision
        // timestamp, and both writes are durable.
        const coordCommit = getCommitOplogEntryOnNode(coordPrimary, lsid, txnNumber);
        const partCommit = getCommitOplogEntryOnNode(participantPrimary, lsid, txnNumber);
    ...
    jsTestLog("rejects a non-coordinator client aborting a prepared participant");
    (() => {
        const lsid = {id: UUID()};
        ...
        // Atomicity holds: the whole transaction committed; no write was lost to a unilateral
        // abort.
        const user = new Mongo(st.s.host);
        assert(user.getDB(kDbName).auth(kUser, kUserPwd));
        assert.eq(
        ...
        assert(
            getCommitOplogEntryOnNode(participantPrimary, lsid, txnNumber),
