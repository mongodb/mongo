/**
 * Run a query on a sharded cluster where one of the shards hangs. Running killCursors on the merizos
...
(function() {
    ...
    let coll = merizosDB.jstest_kill_pinned_cursor;
    coll.drop();
    ...
    //
    // Sets up a sharded cursor, opens a merizos cursor, and uses failpoints to cause the merizos
    // cursor to hang due to getMore commands hanging on each of the shards. Then invokes
    // 'killFunc', and verifies the cursors on the shards and the merizos cursor get cleaned up.
    //
    ...
    function testShardedKillPinned(
        ...
        try {
            // Set up the merizods to hang on a getMore request. ONLY set the failpoint on the
            // merizods. Setting the failpoint on the merizos will only cause it to spin, and not
            ...
            // Run a find against merizos. This should open cursors on both of the shards.
            let findCmd = {find: coll.getName(), batchSize: 2};

            if (useSession) {
                // Manually start a session so it can be continued from inside a parallel shell.
                sessionId = assert.commandWorked(merizosDB.adminCommand({startSession: 1})).id;
                findCmd.lsid = sessionId;
            }

            let cmdRes = merizosDB.runCommand(findCmd);
            ...
            // Sleep until we know the merizod cursors are pinned.
            ...
            // By now, the getMore run against the merizos has returned with an indication that the
            ...
            let killRes = merizosDB.runCommand({killCursors: coll.getName(), cursors: [cursorId]});
            ...
            // Eventually the cursors on the merizods should also be cleaned up. They should be
            // killed by merizos when the merizos cursor gets killed.
            assert.soon(() => shard0DB.serverStatus().metrics.cursor.open.pinned == 0);
            assert.soon(() => shard1DB.serverStatus().metrics.cursor.open.pinned == 0);
    ...
    for (let useSession of[true, false]) {
        // Test that running 'killCursors' against a pinned merizos cursor (with pinned merizod
        // cursors) correctly cleans up all of the involved cursors.
        testShardedKillPinned({
            killFunc: function(merizosCursorId) {
                // Run killCursors against the merizos cursor. Verify that the cursor is reported as
                // killed successfully, and does not hang or return a "CursorInUse" error.
                let cmdRes =
                    merizosDB.runCommand({killCursors: coll.getName(), cursors: [merizosCursorId]});
                assert.commandWorked(cmdRes);
                assert.eq(cmdRes.cursorsKilled, [merizosCursorId]);
        ...
        // Test that running killOp against one of the cursors pinned on merizod causes all involved
        // cursors to be killed.
        testShardedKillPinned({
            // This function ignores the merizos cursor id, since it instead uses currentOp to
        ...
        // Test that running killCursors against one of the cursors pinned on merizod causes all
    ...
    // cursor to be killed.
    testShardedKillPinned({
        // This function ignores the merizos cursor id, since it instead uses listLocalSessions
        ...
        killFunc: function() {
            ...
            assert.commandWorked(merizosDB.runCommand({killSessions: [{id: sessionUUID}]}));
        },
        // Killing a session on merizos kills all matching remote cursors (through KillCursors) then
