// Tests that commands that should not be runnable on sharded collections cannot be run on sharded
// collections.
(function() {

    const st = new ShardingTest({shards: 2, merizos: 2});

    const dbName = 'test';
    const coll = 'foo';
    const ns = dbName + '.' + coll;

    const freshMerizos = st.s0.getDB(dbName);
    const staleMerizos = st.s1.getDB(dbName);

    assert.commandWorked(staleMerizos.adminCommand({enableSharding: dbName}));
    assert.commandWorked(freshMerizos.adminCommand({shardCollection: ns, key: {_id: 1}}));

    // Test that commands that should not be runnable on sharded collection do not work on sharded
    // collections, using both fresh merizos and stale merizos instances.
    assert.commandFailedWithCode(freshMerizos.runCommand({convertToCapped: coll, size: 64 * 1024}),
                                 ErrorCodes.IllegalOperation);
    assert.commandFailedWithCode(staleMerizos.runCommand({convertToCapped: coll, size: 32 * 1024}),
                                 ErrorCodes.IllegalOperation);

    st.stop();

})();
