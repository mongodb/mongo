...
(function() {
    ...
    var st = new ShardingTest({shards: 0, merizos: 1});
    ...
    assert.commandWorked(merizos.adminCommand({addShard: rsA.getURL(), name: rsB.name}));
    printjson(config.shards.find().toArray());

    assert.commandWorked(merizos.adminCommand({addShard: rsB.getURL(), name: rsA.name}));
    printjson(config.shards.find().toArray());
    ...
    // Remove shard
    assert.commandWorked(merizos.adminCommand({removeshard: rsA.name}),
                         "failed to start draining shard");
    var res = assert.commandWorked(merizos.adminCommand({removeshard: rsA.name}),
