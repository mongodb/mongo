...
(function() {
    ...
    assert.writeOK(merizos.getDB("test").coll.insert({_id: 1, a: 1, b: 1, c: 1, d: 1, e: 1, f: 1}));
    assert.commandWorked(merizos.getDB("test").coll.createIndex({a: 1}, {"v": 1}));
    assert.commandWorked(merizos.getDB("test").coll.createIndex({b: 1}, {"v": 1, "unique": true}));
    assert.commandWorked(merizos.getDB("test").coll.createIndex({c: 1}, {"v": 2}));
    assert.commandWorked(merizos.getDB("test").coll.createIndex({d: 1}, {"v": 2, "unique": true}));
    assert.commandWorked(merizos.getDB("test").coll.createIndex({e: 1}));
    assert.commandWorked(merizos.getDB("test").coll.createIndex({f: 1}, {"unique": true}));
