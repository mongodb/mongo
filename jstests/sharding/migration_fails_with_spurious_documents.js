...

{
    ...
    assert.eq(
        0,
        shard0Coll.find({_id: 25}).count(),
        "Spurious document should not exist on shard0",
    );
...

{
    ...
    assert.commandWorked(
        admin.runCommand({
            shardCollection: compoundColl.getFullName(),
            key: {category: 1, item: 1},
        }),
    );
    ...
    jsTest.log.info(
        "Compound Chunk 1 (category: MinKey -> electronics) is on shard: " + compoundChunk1.shard,
    );
    jsTest.log.info(
        "Compound Chunk 2 (category: electronics -> MaxKey) is on shard: " + compoundChunk2.shard,
    );
    ...
    assert.commandWorked(
        shard1CompoundColl.insert({category: "books", item: "textbook", data: "spurious_document"}),
    );
...

{
    ...
    jsTest.log.info(
        "Time series collection successfully sharded with key: " +
            tojson(shardedCollections[0].key),
    );
    ...
    // Insert a document directly on shard1 that should belong to the first chunk (owned by shard0).
    jsTest.log.info(
        "Creating spurious time series document by inserting directly on shard1 " +
            tsCollDDLOps.getName(),
    );
    ...
    assert.eq(
        1,
        getTimeseriesCollForRawOps(shard1DB, shard1TsColl)
            .find({meta: "sensor_025"})
            .rawData()
            .count(),
    ...
    assert.commandFailed(
        tsMoveResult,
        "Time series migration should fail due to spurious documents",
    );
    ...
    assert.commandWorked(
        getTimeseriesCollForRawOps(shard1DB, shard1TsColl).remove(
            {meta: "sensor_025"},
            getRawOperationSpec(shard1DB),
        ),
    );
    ...
    assert.commandWorked(
        tsMoveResult,
        "Time series migration should succeed after removing spurious document",
    );
...

{
    ...
    assert.gte(
        hashedChunks.length,
        2,
        "Expected at least 2 chunks for hashed shard key collection",
    );
    ...
    jsTest.log.info(
        "Creating spurious hashed document by inserting " +
            testUserId +
...

{
    ...
    assert.eq(
        compoundHashedChunks.length,
        2,
        "Expected 2 chunks for compound hashed shard key collection",
    );

    jsTest.log.info(
        "Found " + compoundHashedChunks.length + " chunks for compound hashed shard key collection",
    );
    ...
    // Find chunks on different shards.
    let shard0CompoundHashedChunks = compoundHashedChunks.filter(
        (c) => c.shard === st.shard0.shardName,
    );
    let shard1CompoundHashedChunks = compoundHashedChunks.filter(
    ...
    assert.commandWorked(
        compoundHashedColl.insert({
            region: "us-east",
            userId: "user_001",
            data: "legitimate_compound_hashed_data",
        }),
    );
    assert.commandWorked(
        compoundHashedColl.insert({
