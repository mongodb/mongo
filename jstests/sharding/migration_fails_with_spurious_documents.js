...

{
    ...
    jsTest.log.info("Compound Chunk 1 (category: MinKey -> electronics) is on shard: " +
                    compoundChunk1.shard);
    jsTest.log.info("Compound Chunk 2 (category: electronics -> MaxKey) is on shard: " +
...

{
    ...
    jsTest.log.info("Time series collection successfully sharded with key: " +
    ...
    // Insert a document directly on shard1 that should belong to the first chunk (owned by shard0).
    jsTest.log.info("Creating spurious time series document by inserting directly on shard1 " +
    ...
    let tsMoveResult = admin.runCommand({
        moveChunk: tsCollDDLOpsFullName,
        find:
            {meta: "sensor_050", "control.min.timestamp": baseTime},  // This finds the first chunk
        to: st.shard1.shardName,
        _waitForDelete: true,
    });
    ...
    assert.commandWorked(tsMoveResult,
...

{
    ...
    jsTest.log.info("Shard0 has " + shard0Chunks.length + " chunks, Shard1 has " +
    ...
    jsTest.log.info("Creating spurious hashed document by inserting " + testUserId +
...

{
    ...
    jsTest.log.info("Found " + compoundHashedChunks.length +
    ...
    jsTest.log.info("Spurious document has: {region: " + testRegion +
                    ", userId: " + testCompoundHashedUserId + "}");
