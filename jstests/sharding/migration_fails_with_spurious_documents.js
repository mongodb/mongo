...

{
    ...
    let chunks =
        config.chunks.find({uuid: getUUIDFromConfigCollections(mongos, coll.getFullName())})
            .toArray();
    ...
    chunks = config.chunks.find({uuid: getUUIDFromConfigCollections(mongos, coll.getFullName())})
                 .toArray();
...

{
    ...
    let compoundChunks =
        config.chunks.find({uuid: getUUIDFromConfigCollections(mongos, compoundColl.getFullName())})
            .toArray();
...

{
    ...
    let hashedChunks =
        config.chunks.find({uuid: getUUIDFromConfigCollections(mongos, hashedColl.getFullName())})
            .toArray();
    assert.gte(
    ...
    jsTest.log("Shard0 has " + shard0Chunks.length + " chunks, Shard1 has " + shard1Chunks.length +
               " chunks");
    ...
    // Find the chunk on targetShard that contains the hashed value for testUserId.
...

{
    ...
    let compoundHashedChunks =
        config.chunks
            .find({uuid: getUUIDFromConfigCollections(mongos, compoundHashedColl.getFullName())})
            .toArray();
