...
const testCompoundShardKey =
    (mongos) => {
        ...
        assert.commandFailedWithCode(mongos.adminCommand(missSecondKeyCmd),
            ...
            shardDistribution: [
                {
                    ...
                    min: {num: MinKey, str: MinKey},
                    max: {num: 1, str: '1'}
                },
                {
                    shard: st.shard1.shardName,
                    min: {num: 2, str: '1'},
                    max: {num: MaxKey, str: MaxKey}
                }
            ]
        };
        assert.commandFailedWithCode(mongos.adminCommand(notContinuousCmd),
                                     ErrorCodes.InvalidOptions);
...
const testMoreShardsAndZones =
    (mongos) => {
        ...
        reshardCmdTest.assertReshardCollOk(
            ...
            [
                {
                    recipientShardId: st.shard0.shardName,
                    min: {oldKey: MinKey},
                    max: {oldKey: -1000}
                },
