//
// Tests that bongos validating writes when stale does not DOS config servers
//
...
(function() {
    ...
    var st = new ShardingTest({shards: 2, bongos: 3, other: {shardOptions: {verbose: 2}}});

    var bongos = st.s0;
    var staleBongosA = st.s1;
    var staleBongosB = st.s2;

    var admin = bongos.getDB("admin");
    var config = bongos.getDB("config");
    var coll = bongos.getCollection("foo.bar");
    var staleCollA = staleBongosA.getCollection(coll + "");
    var staleCollB = staleBongosB.getCollection(coll + "");
    ...
    // Let the stale bongos see the collection state
    staleCollA.findOne();
    staleCollB.findOne();
