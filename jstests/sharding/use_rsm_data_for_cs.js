(function() {
    ...
    var st = new ShardingTest({shards: 1, rs: {nodes: 1}, merizos: 1});
    var merizos = st.s;
    var rs = st.rs0;
    ...
    merizos.adminCommand({configureFailPoint: "failAsyncConfigChangeHook", mode: "alwaysOn"});
    ...
    merizos.getDB("admin").runCommand({flushRouterConfig: 1});

    // issue a read from merizos with secondaryOnly read preference to force it use just added node
