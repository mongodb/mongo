...
let spec = IndexCatalogHelpers.findByName(
    st.rs0.getPrimary().getDB("test").migration_id_index.getIndexes(),
    "_id_",
);
assert.neq(spec, null, "_id index spec not found");
assert.eq(spec.v, 1, tojson(spec));
spec = IndexCatalogHelpers.findByName(
    st.rs0.getSecondary().getDB("test").migration_id_index.getIndexes(),
    "_id_",
);
assert.neq(spec, null, "_id index spec not found");
assert.eq(spec.v, 1, tojson(spec));
...
spec = IndexCatalogHelpers.findByName(
    st.rs1.getPrimary().getDB("test").migration_id_index.getIndexes(),
    "_id_",
);
assert.neq(spec, null, "_id index spec not found");
assert.eq(spec.v, 1, tojson(spec));
spec = IndexCatalogHelpers.findByName(
    st.rs1.getSecondary().getDB("test").migration_id_index.getIndexes(),
    "_id_",
);
assert.neq(spec, null, "_id index spec not found");
assert.eq(spec.v, 1, tojson(spec));
