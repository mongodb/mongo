(function() {
    ...
    var s1 = new ShardingTest({name: "count2", shards: 2, bongos: 2});
    var s2 = s1._bongos[1];
    ...
    assert.eq(3, db1.count({name: {$gte: "aaa", $lt: "ddd"}}), "initial count bongos1");
    assert.eq(3, db2.count({name: {$gte: "aaa", $lt: "ddd"}}), "initial count bongos2");
    ...
    assert.eq(3, db1.count({name: {$gte: "aaa", $lt: "ddd"}}), "post count bongos1");

    // The second bongos still thinks its shard mapping is valid and accepts a cound
    print("before sleep: " + Date());
    sleep(2000);
    print("after  sleep: " + Date());
    s1.printChunks("test.foo");
    assert.eq(3, db2.find({name: {$gte: "aaa", $lt: "ddd"}}).count(), "post count bongos2");
