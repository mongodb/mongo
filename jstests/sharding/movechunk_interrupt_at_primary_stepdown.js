...
(function() {
    ...
    assert.commandWorked(bongos.adminCommand({shardCollection: 'TestDB.TestColl', key: {Key: 1}}));

    var coll = bongos.getDB('TestDB').TestColl;
    ...
    var joinMoveChunk = moveChunkParallel(
        staticBongod, bongos.host, {Key: 0}, null, 'TestDB.TestColl', st.shard1.shardName);
    ...
    // both step down operations, because bongos will retry to find the CSRS primary for up to 20
    ...
    assert.eq(1, bongos.getDB('config').chunks.find({shard: st.shard0.shardName}).itcount());
    assert.eq(0, bongos.getDB('config').chunks.find({shard: st.shard1.shardName}).itcount());
    ...
    assert.eq(0, bongos.getDB('config').chunks.find({shard: st.shard0.shardName}).itcount());
    assert.eq(1, bongos.getDB('config').chunks.find({shard: st.shard1.shardName}).itcount());
