...
(function() {
    ...
    const st =
        new ShardingTest({shards: 2, merizos: 1, rs: {nodes: 1}, other: {rsOptions: rsNodeOptions}});

    const merizos = st.s;
    const merizosColl = merizos.getCollection('test.foo');
    const merizosDB = merizos.getDB("test");

    // Enable sharding to inform merizos of the database, allowing us to open a cursor.
    assert.commandWorked(merizos.adminCommand({enableSharding: merizosDB.getName()}));
    ...
    const changeStream = merizosColl.aggregate([{$changeStream: {}}]);
    ...
    assert.commandWorked(
        merizos.adminCommand({shardCollection: merizosColl.getFullName(), key: {_id: 1}}));

    // Insert two documents.
    assert.writeOK(merizosColl.insert({_id: 0}, {writeConcern: {w: "majority"}}));
    assert.writeOK(merizosColl.insert({_id: 20}, {writeConcern: {w: "majority"}}));

    // Split the collection into two chunks: [MinKey, 10) and [10, MaxKey].
    assert.commandWorked(merizos.adminCommand({split: merizosColl.getFullName(), middle: {_id: 10}}));
    ...
    assert.writeOK(merizosColl.insert({_id: 21}, {writeConcern: {w: "majority"}}));
    ...
    assert.commandWorked(merizos.adminCommand({split: merizosColl.getFullName(), middle: {_id: 0}}));
    assert.commandWorked(merizos.adminCommand({
        moveChunk: merizosColl.getFullName(),
        find: {_id: 5},
        to: st.shard1.shardName,
    ...
    assert.writeOK(merizosColl.insert({_id: 2}, {writeConcern: {w: "majority"}}));
    assert.writeOK(merizosColl.insert({_id: 22}, {writeConcern: {w: "majority"}}));
    ...
    assert.writeOK(merizosColl.insert({_id: -3}, {writeConcern: {w: "majority"}}));
    assert.writeOK(merizosColl.insert({_id: 3}, {writeConcern: {w: "majority"}}));
    assert.writeOK(merizosColl.insert({_id: 23}, {writeConcern: {w: "majority"}}));
    ...
    // Insert again, into all three chunks.
    assert.writeOK(merizosColl.insert({_id: -4}, {writeConcern: {w: "majority"}}));
    assert.writeOK(merizosColl.insert({_id: 4}, {writeConcern: {w: "majority"}}));
    assert.writeOK(merizosColl.insert({_id: 24}, {writeConcern: {w: "majority"}}));
    ...
    newShard.initiate();
    assert.commandWorked(merizos.adminCommand({addShard: newShard.getURL(), name: "newShard"}));

    // At this point, there haven't been any migrations to that shard; check that the changeStream
    // works normally.
    assert.writeOK(merizosColl.insert({_id: -5}, {writeConcern: {w: "majority"}}));
    assert.writeOK(merizosColl.insert({_id: 5}, {writeConcern: {w: "majority"}}));
    assert.writeOK(merizosColl.insert({_id: 25}, {writeConcern: {w: "majority"}}));
    ...
    assert.writeOK(merizosColl.insert({_id: 16}, {writeConcern: {w: "majority"}}));
    ...
    assert.writeOK(merizosColl.insert({_id: -6}, {writeConcern: {w: "majority"}}));
    assert.writeOK(merizosColl.insert({_id: 6}, {writeConcern: {w: "majority"}}));
    assert.writeOK(merizosColl.insert({_id: 26}, {writeConcern: {w: "majority"}}));
