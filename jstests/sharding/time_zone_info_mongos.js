...
(function() {
    ...
    function testWithGoodTimeZoneDir(tzGoodInfoDir) {
        ...
        assert.commandWorked(
            mongosColl.insert({_id: 2, date: ISODate("2020-10-20T19:49:47.634Z")}));
        ...
        function buildTimeZonePipeline(tz) {
            // We use $const here so that the input pipeline matches the format of the explain
            // output.
        ...
        function testForTimezone(tz) {
            ...
            const tzExplain =
                assert.commandWorked(mongosColl.explain().aggregate(timeZonePipeline));
            ...
            // Confirm that both documents are output by the pipeline, demonstrating that the date
            // has been correctly disassembled on mongoD and reassembled on mongoS.
