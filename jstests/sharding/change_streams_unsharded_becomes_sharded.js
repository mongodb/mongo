...
(function() {
    ...
    function testUnshardedBecomesSharded(collToWatch) {
        merizosColl.drop();
        merizosDB.createCollection(testName);
        merizosColl.createIndex({x: 1});

        st.ensurePrimaryShard(merizosDB.getName(), st.rs0.getURL());

        // Establish a change stream cursor on the unsharded collection.
        const cst = new ChangeStreamTest(merizosDB);

        // Create a different collection in the same database, and verify that it doesn't affect the
        // results of the change stream.
        const merizosCollOther = merizosDB[testName + "other"];
        merizosCollOther.drop();
        merizosDB.createCollection(testName + "other");
        merizosCollOther.createIndex({y: 1});

        let cursor = cst.startWatchingChanges({
            pipeline: [{$changeStream: {}}, {$match: {"ns.coll": merizosColl.getName()}}],
        ...
        assert.writeOK(merizosColl.insert({_id: 0, x: 0}));
        assert.writeOK(merizosCollOther.insert({_id: 0, y: 0}));
        ...
        st.shardColl(merizosColl.getName(), {x: 1}, {x: 0}, false, merizosDB.getName());

        // Shard the other collection with shard key {y: 1} and split into 2 chunks.
        st.shardColl(merizosCollOther.getName(), {y: 1}, {y: 0}, false, merizosDB.getName());
        ...
        // shard.
        assert.writeOK(merizosColl.insert({_id: 1, x: 1}));
        assert.writeOK(merizosCollOther.insert({_id: 1, y: 1}));
        ...
        assert.commandWorked(merizosDB.adminCommand({
            moveChunk: merizosCollOther.getFullName(),
            find: {y: -1},
            to: st.rs1.getURL(),
            _waitForDelete: true
        }));
        ...
        assert.writeOK(merizosColl.insert({_id: -2, x: -2}));
        assert.writeOK(merizosColl.insert({_id: -3, x: -3}));

        // Drop and recreate the collection.
        merizosColl.drop();
        merizosDB.createCollection(merizosColl.getName());
        merizosColl.createIndex({z: 1});

        // Shard the collection on a different shard key and ensure that each shard has a chunk.
        st.shardColl(merizosColl.getName(), {z: 1}, {z: 0}, {z: -1}, merizosDB.getName());

        assert.writeOK(merizosColl.insert({_id: -1, z: -1}));
        assert.writeOK(merizosColl.insert({_id: 1, z: 1}));
    ...
    testUnshardedBecomesSharded(merizosColl.getName());
