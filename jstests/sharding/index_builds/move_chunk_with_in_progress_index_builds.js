...
(function testSucceedFirstMigrationWithInProgressIndexBuild() {
    ...
    const hangMigrationRecipientBeforeWaitingNoIndexBuildInProgress = configureFailPoint(
        st.shard1,
        "hangMigrationRecipientBeforeWaitingNoIndexBuildInProgress",
    );
    const shardName = st.shard1.shardName;
    const awaitMoveChunkShell = startParallelShell(
        funWithArgs(
            function (shardName, ns) {
...
(function testFailedFirstMigrationWithInProgressIndexBuild() {
    ...
    assert.includes(
        res.errmsg,
        "Non-trivial index creation should be scheduled manually",
        tojson(res),
    );
