...
(function() {
    ...
    let testCases = {
        _hashBSONElement: {skip: "executes locally on merizos (not sent to any remote node)"},
        _isSelf: {skip: "executes locally on merizos (not sent to any remote node)"},
        ...
        availableQueryOptions: {skip: "executes locally on merizos (not sent to any remote node)"},
        ...
        buildInfo: {skip: "executes locally on merizos (not sent to any remote node)"},
        clearLog: {skip: "executes locally on merizos (not sent to any remote node)"},
        collMod: {
            sendsDbVersion: true,
            sendsShardVersion: true,
            setUp: function(merizosConn) {
                // Expects the collection to exist, and doesn't implicitly create it.
                assert.commandWorked(merizosConn.getDB(dbName).runCommand({create: collName}));
            },
            command: {collMod: collName, noPadding: false},
            cleanUp: function(merizosConn) {
                assert(merizosConn.getDB(dbName).getCollection(collName).drop());
            }
        },
        collStats: {
            sendsDbVersion: true,
        ...
        compact: {skip: "not allowed through merizos"},
        configureFailPoint: {skip: "executes locally on merizos (not sent to any remote node)"},
        connPoolStats: {skip: "executes locally on merizos (not sent to any remote node)"},
        connPoolSync: {skip: "executes locally on merizos (not sent to any remote node)"},
        connectionStatus: {skip: "executes locally on merizos (not sent to any remote node)"},
        convertToCapped: {
            sendsDbVersion: true,
            sendsShardVersion: true,
            setUp: function(merizosConn) {
        ...
        features: {skip: "executes locally on merizos (not sent to any remote node)"},
        filemd5: {
            sendsDbVersion: true,
            sendsShardVersion: true,
        ...
        flushRouterConfig: {skip: "executes locally on merizos (not sent to any remote node)"},
        fsync: {skip: "broadcast to all shards"},
        getCmdLineOpts: {skip: "executes locally on merizos (not sent to any remote node)"},
        getDiagnosticData: {skip: "executes locally on merizos (not sent to any remote node)"},
        getLastError: {skip: "does not forward command to primary shard"},
        getLog: {skip: "executes locally on merizos (not sent to any remote node)"},
        getMore: {skip: "requires a previously established cursor"},
        getParameter: {skip: "executes locally on merizos (not sent to any remote node)"},
        getShardMap: {skip: "executes locally on merizos (not sent to any remote node)"},
        getShardVersion: {skip: "executes locally on merizos (not sent to any remote node)"},
        ...
        hostInfo: {skip: "executes locally on merizos (not sent to any remote node)"},
        insert: {
            sendsDbVersion: false,
            sendsShardVersion: true,
        ...
        invalidateUserCache: {skip: "executes locally on merizos (not sent to any remote node)"},
        isdbgrid: {skip: "executes locally on merizos (not sent to any remote node)"},
        isMaster: {skip: "executes locally on merizos (not sent to any remote node)"},
        ...
        listCommands: {skip: "executes locally on merizos (not sent to any remote node)"},
        ...
        logRotate: {skip: "executes locally on merizos (not sent to any remote node)"},
        ...
        mapReduce: {
            ...
            cleanUp: function(merizosConn) {
                assert(merizosConn.getDB(dbName).getCollection(collName).drop());
                assert(merizosConn.getDB(dbName).getCollection(collName + "_renamed").drop());
            }
        },
        ...
        netstat: {skip: "executes locally on merizos (not sent to any remote node)"},
        ping: {skip: "executes locally on merizos (not sent to any remote node)"},
        planCacheClear: {
            sendsDbVersion: false,
        ...
        profile: {skip: "not supported in merizos"},
        reapLogicalSessionCacheNow: {skip: "is a no-op on merizos"},
        refreshLogicalSessionCacheNow: {skip: "goes through the cluster write path"},
        refreshSessions: {skip: "executes locally on merizos (not sent to any remote node)"},
        refreshSessionsInternal: {skip: "executes locally on merizos (not sent to any remote node)"},
        ...
        replSetGetStatus: {skip: "not supported in merizos"},
        ...
        serverStatus: {skip: "executes locally on merizos (not sent to any remote node)"},
        ...
        setFreeMonitoring:
            {skip: "explicitly fails for merizos, primary merizod only", conditional: true},
        setParameter: {skip: "executes locally on merizos (not sent to any remote node)"},
        ...
        startRecordingTraffic: {skip: "executes locally on merizos (not sent to any remote node)"},
        startSession: {skip: "executes locally on merizos (not sent to any remote node)"},
        stopRecordingTraffic: {skip: "executes locally on merizos (not sent to any remote node)"},
        update: {
            skipProfilerCheck: true,
            sendsDbVersion: false,
        ...
        whatsmyuri: {skip: "executes locally on merizos (not sent to any remote node)"},
    };
    ...
    class AllCommandsTestRunner {
        ...
        runCommands() {
            ...
            // test cases that did not correspond to any merizos command.
            for (let key of Object.keys(testCases)) {
                ...
                // on the merizos), but these test cases won't be run in regular suites, so we skip
                // processing them below as well.
                if (commandsAddedToMerizosIn42.includes(key) ||
                    commandsRemovedFromMerizosIn42.includes(key)) {
                    continue;
                }
                assert(testCases[key].validated || testCases[key].conditional,
                       "you defined a test case for a command '" + key +
                           "' that does not exist on merizos: " + tojson(testCases[key]));
            }
        }
    }
