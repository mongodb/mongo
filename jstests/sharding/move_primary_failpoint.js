...
function testUserCollections({trackUnshardedCollections}) {
    ...
    const movePrimaryFp0 = configureFailPoint(
        shard0Primary,
        "movePrimaryFailIfNeedToCloneMovableCollections",
        {
            comment,
        },
    );
    ...
    const movePrimaryRes1 = st.s.adminCommand({
        movePrimary: dbName1,
        to: st.shard1.shardName,
        comment,
    });
    ...
    const transitionRes0 = assert.commandWorked(
        st.s.adminCommand({transitionToDedicatedConfigServer: 1}),
    );
    ...
    const transitionRes1 = assert.commandWorked(
        st.s.adminCommand({transitionToDedicatedConfigServer: 1}),
    );
    ...
    const chunkDocs = st.s
        .getCollection("config.chunks")
        .find({uuid: collUuid, shard: "config"})
        .toArray();
    ...
    assert.soon(
        () => {
            const transitionRes2 = assert.commandWorked(
                st.s.adminCommand({transitionToDedicatedConfigServer: 1}),
            );
            jsTest.log(
                "The third transitionToDedicatedConfigServer response: " + tojson(transitionRes2),
            );
    ...
    const movePrimaryFp1 = configureFailPoint(
        shard1Primary,
        "movePrimaryFailIfNeedToCloneMovableCollections",
    );
...
function testInternalCollections({featureFlagReshardingForTimeseries}) {
    ...
    const movePrimaryFp = configureFailPoint(
        shard0Primary,
        "movePrimaryFailIfNeedToCloneMovableCollections",
    );
    ...
    assert.commandFailedWithCode(
        st.s.adminCommand({movePrimary: dbName0, to: st.shard1.shardName}),
        9046501,
    );
    assert.commandWorked(
        st.s.adminCommand({moveCollection: dbName0 + "." + collName, toShard: st.shard1.shardName}),
    );
    assert.commandFailedWithCode(
        st.s.adminCommand({
            moveCollection: dbName0 + ".system.views",
    ...
    if (buildInfo().modules.includes("enterprise")) {
        ...
        assert.commandFailedWithCode(
            st.s.adminCommand({movePrimary: dbName1, to: st.shard1.shardName}),
            9046501,
        );
        assert.commandWorked(
            st.s.adminCommand({
                moveCollection: dbName1 + "." + collName,
                toShard: st.shard1.shardName,
            }),
        );
        ...
        assert.commandWorked(
            st.s.adminCommand({
                moveCollection: dbName1 + ".keystore",
                toShard: st.shard1.shardName,
            }),
        );
    ...
    if (featureFlagReshardingForTimeseries) {
        ...
        assert.commandFailedWithCode(
            st.s.adminCommand({movePrimary: dbName2, to: st.shard1.shardName}),
            9046501,
        );
        assert.commandWorked(
            st.s.adminCommand({
                moveCollection: dbName2 + "." + collName,
                toShard: st.shard1.shardName,
            }),
        );
    ...
    assert.commandFailedWithCode(
        st.s.adminCommand({
            moveCollection: dbName4 + ".system.resharding.foo",
            toShard: st.shard1.shardName,
        }),
    ...
    assert.commandFailedWithCode(
        st.s.adminCommand({movePrimary: dbName4, to: st.shard1.shardName}),
        9046501,
    );
