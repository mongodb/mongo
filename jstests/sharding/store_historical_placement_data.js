...

{
    ...
    // TODO SERVER-77915 Remove checkUnsplittableMetadata once 8.0 becomes last LTS. Update the test
    // as this variable is now always "true"
    ...
    function testShardCollection(dbName, collName) {
        ...
        const entriesInConfigShards =
            configDB.shards.find({}, {_id: 1}).toArray().map((s) => s._id);
    ...
    function testMoveChunk(dbName, collName) {
        ...
        assert(timestampCmp(placementAfterMigration.timestamp,
                            migratedChunk.history[0].validAfter) === 0);
        assert(timestampCmp(placementAfterMigration.timestamp,
                            migratedChunk.onCurrentShardSince) === 0);
    ...
    function testMoveRange(dbName, collName) {
        ...
        placementAfterMigration =
            getValidatedPlacementInfoForCollection(dbName, collName, [shard1]);
        migratedChunk = configDB.chunks.findOne({uuid: collUUID, min: {x: 0}});
        assert(timestampCmp(placementAfterMigration.timestamp,
                            migratedChunk.history[0].validAfter) === 0);
        assert(timestampCmp(placementAfterMigration.timestamp,
    ...
    function testRenameCollection() {
        ...
        const [targetCollPlacementInfoWhenRenamed, targetCollPlacementInfoWhenDropped] =
            (function() {
        ...
        if (checkUnsplittableMetadata) {
            ...
            assert.eq(2,
                      configDB.placementHistory.count({nss: dbName + '.' + unshardedOldCollName}));
            assert.eq(
                3, configDB.placementHistory.count({nss: dbName + '.' + unshardedTargetCollName}));
        } else {
            jsTest.log(
            ...
            assert.eq(0,
                      configDB.placementHistory.count({nss: dbName + '.' + unshardedOldCollName}));
            assert.eq(
                0, configDB.placementHistory.count({nss: dbName + '.' + unshardedTargetCollName}));
        }
    }
    ...
    function testReshardCollection() {
        ...
        const finalCollPlacementInfo = getValidatedPlacementInfoForCollection(
            dbName, collName, [shard0, shard1, shard2], false);
    ...
    jsTest.log(
        'Testing placement entries added by shardCollection() over a non-existing db (& coll)');
...
(() => {
    ...
    // as this variable is now always "true"
    const checkUnsplittableMetadata = FeatureFlagUtil.isPresentAndEnabled(
        newReplicaSet.getPrimary().getDB('admin'), "TrackUnshardedCollectionsUponCreation");
