...
assert.doesNotThrow(() =>
    db.aggregate([{$documents: Array.from({length: 20}, () => ({}))}, {$out: "test_collection"}]),
);
assert.eq(db.test_collection.count(), 20);

assert.doesNotThrow(() =>
    db.aggregate([{$documents: Array.from({length: 20}, () => ({}))}, {$merge: "test_collection"}]),
);
...
const lookupResult1 = coll.aggregate({
    $lookup: {
        as: "t",
        localField: "x",
        foreignField: "x",
