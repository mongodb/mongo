...
let updateResult;
try {
    updateResult = testColl.update({}, {x: 110, y: 55, z: 3, a: 110}, false);
    assert.commandWorked(updateResult);
    assert.eq(1, updateResult.nMatched);
    assert.eq(1, updateResult.nModified);
...
const upsertRes = st.s
    .getDB(kDbName)
    .coll.update({x: 4, y: 0, z: 0, opStyle: true}, upsertDoc, {upsert: true});
...
[false, true].forEach(function (isUpsert) {
    ...
    assertUpdateWorked(
        {_id: 0, x: 4, z: 3, y: 3},
        [{$addFields: {pipelineUpdate: isUpsert}}],
        isUpsert,
        0,
    );
    assert.eq(
...
assertUpdateWorkedWithNoMatchingDoc(
    {x: 46, z: 4},
    [{$addFields: {y: 10, pipelineUpdateNoOp: false}}],
    false,
);
