/**
 * Smoke tests for v2 cluster-level change streams, ignoreRemovedShards mode in a sharded cluster.
 *
 * @tags: [
...
describe("$changeStream v2, ignoreRemovedShards mode", function () {
    ...
    function assertCollDataDistribution(coll, expectedCounts) {
        assert.soon(
            () => {
                const actual = getCollDataDistribution(coll);
            ...
            () => {
                return (
                    ...
                    tojsononeline(getCollDataDistribution(coll)) +
                    ", expected: " +
    ...
    function buildExpectedInsertEvent(coll, shard, doc, documentKey = undefined) {
        return {
    ...
    // Builds an expected change event for a dropDatabase operation.
    function buildExpectedDropDatabaseEvent() {
        return {
            ns: {db: db.getName()},
    ...
    function insertDocumentOnShard(coll, doc, shard) {
        let counts = getCollDataDistribution(coll);
        doc.shardId = shard.shardName;
        assert.commandWorked(coll.insert(doc));
        counts[shard.shardName] += 1;
        assertCollDataDistribution(coll, counts);
    }

    // Execute a series of operations on the collection, then decommission the specified shards.
    function executeCollectionOperations(shardsToDecommission, beforeDecommission, dropDatabase) {
        ...
        withBalancerEnabled(() => {
            ...
            insertDocumentOnShard(coll, {_id: 1, a: 1}, st.shard1);
            insertDocumentOnShard(coll, {_id: -3, a: -3}, st.shard0);
            ...
            insertDocumentOnShard(coll, {_id: 3, a: -1}, st.shard1);
            insertDocumentOnShard(coll, {_id: 2, a: 2}, st.shard2);
            insertDocumentOnShard(coll, {_id: 4, a: 1}, st.shard2);
    ...
    // Create two collections on separate shards and insert documents into them.
    function executeMultiCollectionOperations(beforeDecommission, shardsToDecommission) {
        ...
        withBalancerEnabled(() => {
            ...
            // Insert documents into the collections so the documents are distributed to the shards.
            insertDocumentOnShard(coll1, {_id: -1}, st.shard0);
            insertDocumentOnShard(coll1, {_id: 1}, st.shard1);
            assertCollDataDistribution(coll1, {
                [st.shard0.shardName]: 1,
                [st.shard1.shardName]: 1,
                [st.shard2.shardName]: 0,
            });

            insertDocumentOnShard(coll2, {_id: -1}, st.shard1);
            insertDocumentOnShard(coll2, {_id: 1}, st.shard2);
    ...
    it("returns events in ignoreRemovedShards mode after shard has been removed", () => {
        ...
        csTest.assertNextChangesEqual({
            cursor: csCursor,
            expectedChanges: [
                buildExpectedInsertEvent(coll, st.shard1, {_id: 1, a: 1}, {_id: 1}),
                buildExpectedInsertEvent(coll, st.shard1, {_id: 3, a: -1}),
                buildExpectedInsertEvent(coll, st.shard2, {_id: 2, a: 2}),
                buildExpectedInsertEvent(coll, st.shard2, {_id: 4, a: 1}),
                buildExpectedInsertEvent(coll, st.shard1, {_id: -2, a: -2}),
                buildExpectedEventForNamespace("drop"),
                buildExpectedDropDatabaseEvent(),
            ],
        });
    ...
    it("returns events in ignoreRemovedShards mode after shard has been removed, showExpandedEvents", () => {
        ...
        csTest.assertNextChangesEqual({
            ...
            expectedChanges: [
                ...
                buildExpectedInsertEvent(coll, st.shard1, {_id: 1, a: 1}, {_id: 1}),
                buildExpectedInsertEvent(coll, st.shard0, {_id: -3, a: -3}, {_id: -3}),
    ...
    it("returns only new events in ignoreRemovedShards mode after all original shards were decommissioned", () => {
        ...
        // Re-create the collection after it was originally dropped.
        assertCreateCollection(db, coll.getName());

        // Insert a few documents on the just-added shard.
        insertDocumentOnShard(coll, {_id: 6, a: 6}, newShard);
        insertDocumentOnShard(coll, {_id: 7, a: 7}, newShard);
        assertDropCollection(db, coll.getName());
        ...
        csTest.assertNextChangesEqual({
            ...
            expectedChanges: [
                buildExpectedEventForNamespace("drop"),
                buildExpectedEventForNamespace("create"),
                buildExpectedInsertEvent(coll, newShard, {_id: 6, a: 6}, {_id: 6}),
                buildExpectedInsertEvent(coll, newShard, {_id: 7, a: 7}, {_id: 7}),
                buildExpectedEventForNamespace("drop"),
            ],
        });
    ...
    it("returns events in ignoreRemovedShards mode for multiple collections when no shards are decommissioned", () => {
        ...
        executeMultiCollectionOperations(() => {}, []);

        // Open a change stream on a non-existing collection in ignoreRemovedShards mode from the
        // original start point, and assume that all events are returned.
        csTest = new ChangeStreamTest(adminDb);
        const csCursor = csTest.startWatchingAllChangesForCluster(
            {},
            {
        ...
        csTest.assertNextChangesEqual({
            cursor: csCursor,
            expectedChanges: [
                buildExpectedInsertEvent(db.getCollection("test1"), st.shard0, {_id: -1}),
                buildExpectedInsertEvent(db.getCollection("test1"), st.shard1, {_id: 1}),
                buildExpectedInsertEvent(db.getCollection("test2"), st.shard1, {_id: -1}),
                buildExpectedInsertEvent(db.getCollection("test2"), st.shard2, {_id: 1}),
            ],
        });
    ...
    it("returns events in ignoreRemovedShards mode for multiple collections when one shard is decommissioned", () => {
        // Record high-watermark time marking the start point of the test.
        const startAtOperationTime = getClusterTime(db);
    ...
    it("returns events in ignoreRemovedShards mode for multiple collections when multiple shards are decommissioned", () => {
        // Record high-watermark time marking the start point of the test.
        const startAtOperationTime = getClusterTime(db);
