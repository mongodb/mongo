...
describe("ChangeStreamHandleTopologyChangeV2Stage: IRS degraded-mode state transitions", () => {
    ...
    describe("Cluster-level change streams", () => {
        it("FetchingStartingChangeStreamSegment → FetchingDegradedGettingChangeEvent when a shard in the cluster placement is removed", () => {
            ...
            // Capture T_before: both shard0 and shard1 are active and in the cluster placement.
            const T_before = waitForClusterTime(db, st);

            // Drain shard1 by moving its user-data chunk ({_id: 1}) to shard2, then remove shard1.
            // After removal, the cluster's placement history at T_before still records
            // {shard0, shard1}.
            ...
            // Open a cluster-level IRS change stream at T_before.
            ...
            // After recovery, the stage enters FetchingNormalGettingChangeEvent with
            // {shard0, shard2}.
            assert.commandWorked(st.s.adminCommand({clearLog: "global"}));
            const logOffset = checkLog.getGlobalLog(st.s).length;
            const comment = "state_trans_cluster_irs_degraded";
            csTest = new ChangeStreamTest(st.s.getDB("admin"));
        ...
        it("FetchingDegradedGettingChangeEvent undo: multiple documents spanning the segment boundary are all delivered at cluster level", () => {
            ...
            // Insert two documents inside the degraded window and two after T_drain. The post-
            // T_drain documents exercise the undoGetNext() path.
            removeShardFromCluster(st, st.shard1.shardName, () => {
            ...
            assert.commandWorked(st.s.adminCommand({clearLog: "global"}));
            const logOffset = checkLog.getGlobalLog(st.s).length;
            const comment = "state_trans_irs_undo_docs_cluster";
            csTest = new ChangeStreamTest(st.s.getDB("admin"));
            const csCursor = csTest.startWatchingChanges({
                pipeline: [
                    {
        ...
        it("Multiple documents on surviving shards delivered through Normal→Degraded→Normal recovery with two chunk moves at cluster level", () => {
            ...
            // Documents on shard1 (never removed) inserted before chunk moves are returned in the
            ...
            // After full recovery these documents are delivered in the final normal segment.
            assert.commandWorked(coll.insert({_id: 300}));
            assert.commandWorked(coll.insert({_id: -300}));

            assert.commandWorked(st.s.adminCommand({clearLog: "global"}));
            const logOffset = checkLog.getGlobalLog(st.s).length;
            const comment = "state_trans_irs_multi_docs_normal_to_degraded_cluster";
            csTest = new ChangeStreamTest(st.s.getDB("admin"));
            const csCursor = csTest.startWatchingChanges({
                pipeline: [
                    {
            ...
            assertOpenCursors(
                st,
                [st.shard0.shardName, st.shard1.shardName, st.shard3.shardName],
                /*expectedConfigCursor=*/ true,
                cursorCommentFilter(comment),
            );
        });
