...
describe("ChangeStreamHandleTopologyChangeV2Stage: state transitions", () => {
    ...
    describe("Cluster-level change streams", () => {
        it("strict mode init: Uninitialized → FetchingInitialization → FetchingGettingChangeEvent", () => {
            jsTest.log.info(
                "Cluster-level change streams: strict mode init: Uninitialized → FetchingInitialization → FetchingGettingChangeEvent",
            );
            ...
            const comment = "state_trans_cluster_strict_init";
        ...
        it("IRS mode init: Uninitialized → FetchingInitialization → FetchingStartingChangeStreamSegment → FetchingNormalGettingChangeEvent", () => {
            jsTest.log.info(
                "Cluster-level change streams: IRS mode init: Uninitialized → FetchingInitialization → FetchingStartingChangeStreamSegment → FetchingNormalGettingChangeEvent",
            );
            ...
            const logOffset = checkLog.getGlobalLog(st.s).length;
            const comment = "state_trans_cluster_irs_init";
            csTest = new ChangeStreamTest(st.s.getDB("admin"));
            const csCursor = csTest.startWatchingChanges({
                pipeline: [
                    {
