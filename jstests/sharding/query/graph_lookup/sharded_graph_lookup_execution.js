...
function assertGraphLookupExecution(pipeline, opts, expectedResults, executionList) {
    ...
    for (let exec of executionList) {
        ...
        if (exec.randomlyDelegatedMerger) {
            let totalGraphLookupExecution = shardList.reduce(
                (numExecs, shard) =>
                    numExecs +
        ...
        for (let shard = 0; shard < shardList.length; shard++) {
            ...
            const remoteReadCount = shardList[shard].system.profile
                .find({
...
assertGraphLookupExecution(pipeline, {comment: "unsharded_to_sharded_targeted"}, expectedRes, [
    {
