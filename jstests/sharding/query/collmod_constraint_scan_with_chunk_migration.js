...
describe("constraint validation scan with concurrent chunk migrations", function () {
    ...
    before(function () {
        ...
        // hangBeforeDocumentSourceCursorLoadBatch only fires on the classic DocumentSourceCursor
        // path. With featureFlagSbeFull the scan's $match + $limit pipeline is fully lowered to
        // SBE and the failpoint never fires.
        // TODO SERVER-129253: replace with an SBE-compatible failpoint so this test runs under SBE full.
        if (checkSbeFullyEnabled(st.rs1.getPrimary().getDB("admin"))) {
            jsTest.log.info(
                `Skipping ${jsTestName()} as SBE full bypasses the classic cursor failpoint`,
            );
            st.stop();
            quit();
        }
