...
let expectedResults = [{_id: 0}, {_id: 1, result: [{_id: 0, foreignField: 1, data: "lookup_data"}]}];
...
const unionUnderLookupPipeline = [
    {
        $lookup: {from: lookupColl.getName(), pipeline: [{$unionWith: unionColl.getName()}], as: "result"},
    },
];
...
assert.commandWorked(mongosDB.adminCommand({shardCollection: baseColl.getFullName(), key: {_id: 1}}));
assert.commandWorked(mongosDB.adminCommand({shardCollection: unionColl.getFullName(), key: {_id: 1}}));
...
assert.commandWorked(mongosDB.adminCommand({shardCollection: lookupColl.getFullName(), key: {_id: 1}}));
...
expectedResults = [{_id: 0, result: [{_id: 0}, {_id: 1, result: [{_id: 0, foreignField: 1, data: "lookup_data"}]}]}];
