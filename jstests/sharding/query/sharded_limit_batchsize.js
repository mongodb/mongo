...
function testLimit(coll) {
    ...
    let openCursorsBefore = assert.commandWorked(coll.getDB().serverStatus()).metrics.cursor.open
        .total;
    cursor = coll.find().sort({x: 1}).limit(1);
    assert(cursor.hasNext());
    assert.eq(-10, cursor.next()["_id"]);
    let openCursorsAfter = assert.commandWorked(coll.getDB().serverStatus()).metrics.cursor.open
        .total;
...
let st = new ShardingTest({
    shards: 2,
    other: {rsOptions: {setParameter: {"enableTestCommands": 1}}},
});
...
assert.commandWorked(
    db.adminCommand({moveChunk: shardedCol.getFullName(), find: {_id: 0}, to: st.shard1.shardName}),
);
...
assert.commandWorked(
    st.shard0
        .getDB("test")
        .adminCommand({configureFailPoint: "failReceivedGetmore", mode: "alwaysOn"}),
);
