...
function shardCollectionWithSplitsAndMoves(ns, shardKeyPattern, collation, splitPoints, chunksToMove) {
    const collection = st.s.getCollection(ns);
    const db = collection.getDB();

    assert.commandWorked(st.s.adminCommand({enableSharding: db.getName(), primaryShard: st.shard0.name}));
    ...
    for (let splitPoint of splitPoints) {
        assert.commandWorked(st.s.adminCommand({split: collection.getFullName(), middle: splitPoint}));
    }
...
{
    jsTestLog("Test find command in an _id:hashed sharded collection with simple default collation.");
    ...
    assert.eq([], findQueryWithCollation(st.s.getCollection(collection.getFullName()), {_id: "a"}, undefined));
    assert.eq([], findQueryWithCollation(st.s.getCollection(collection.getFullName()), {_id: "a"}, {locale: "simple"}));
    assert.eq(
        docs,
        findQueryWithCollation(st.s.getCollection(collection.getFullName()), {_id: "a"}, {locale: "en", strength: 2}),
    );
}

{
    jsTestLog("Test find command in an _id:hashed sharded collection with non-simple default collation.");
    ...
    // Check default collation, simple collation, non-simple collation.
    assert.eq(docs, findQueryWithCollation(st.s.getCollection(collection.getFullName()), {_id: "a"}, undefined));
    assert.eq([], findQueryWithCollation(st.s.getCollection(collection.getFullName()), {_id: "a"}, {locale: "simple"}));
    assert.eq(
        docs,
        findQueryWithCollation(st.s.getCollection(collection.getFullName()), {_id: "a"}, {locale: "en", strength: 2}),
...

{
    ...
    const db = collection.getDB();
    assert.commandWorked(db.runCommand({create: collection.getName(), collation: {locale: "en", strength: 2}}));
    ...
    assert(
        /The _id index must have the same collation as the collection/.test(res.errmsg),
        `expected shardCollection command to fail due to required collation for _id index: ${tojson(res)}`,
    );
}

{
    ...
    const collection = shardCollectionWithSplitsAndMoves(
        ...
        {locale: "simple"},
        [{notUnderscoreId: convertShardKeyToHashed("A")}, {notUnderscoreId: convertShardKeyToHashed("a")}],
        [
    ...
    assert.eq(
        [],
        findQueryWithCollation(st.s.getCollection(collection.getFullName()), {notUnderscoreId: "a"}, undefined),
    );
    assert.eq(
        [],
        findQueryWithCollation(
            st.s.getCollection(collection.getFullName()),
            {notUnderscoreId: "a"},
            {locale: "simple"},
        ),
    );
...
{
    jsTestLog("Test find command in a hashed sharded collection with non-simple default collation.");
...
{
    jsTestLog("Test findAndModify command in an _id:hashed sharded collection with simple default collation.");
    ...
    assert.eq(null, mongosCollection.findAndModify({query: {_id: "a"}, update: {$inc: {count: 1}}}));
    assert.eq(
        null,
        mongosCollection.findAndModify({query: {_id: "a"}, update: {$inc: {count: 1}}, collation: {locale: "simple"}}),
    );
    assert.eq(
        null,
    ...
    assert.eq({_id: "A", count: 0}, mongosCollection.findAndModify({query: {_id: "A"}, update: {$inc: {count: 1}}}));
    assert.eq(
        {_id: "A", count: 1},
        mongosCollection.findAndModify({query: {_id: "A"}, update: {$inc: {count: 1}}, collation: {locale: "simple"}}),
    );
...
{
    jsTestLog("Test findAndModify command in an _id:hashed sharded collection with non-simple default collation.");
...
{
    jsTestLog("Test findAndModify command in a hashed sharded collection with simple default collation.");
    const collection = shardCollectionWithSplitsAndMoves(
    ...
    // chunk for "A".
    assert.lt(convertShardKeyToHashed("a"), convertShardKeyToHashed("A"));
    assert.eq(null, mongosCollection.findAndModify({query: {notUnderscoreId: "a"}, update: {$inc: {count: 1}}}));
    assert.eq(
        null,
        mongosCollection.findAndModify({
    ...
    assert.eq(
        {_id: 0, notUnderscoreId: "A", count: 0},
        mongosCollection.findAndModify({query: {notUnderscoreId: "A"}, update: {$inc: {count: 1}}}),
    );
    assert.eq(
        {_id: 0, notUnderscoreId: "A", count: 1},
        mongosCollection.findAndModify({
            query: {notUnderscoreId: "A"},
            update: {$inc: {count: 1}},
            collation: {locale: "simple"},
    ...
    assert.eq(
        {_id: 0, notUnderscoreId: "A", count: 2},
        mongosCollection.findAndModify({
            query: {notUnderscoreId: "A"},
            update: {$inc: {count: 1}},
            collation: {locale: "en", strength: 2},
...
{
    jsTestLog("Test findAndModify command in a hashed sharded collection with non-simple default collation.");
