...
(function() {
    ...
    // Get connection to merizos for the cluster
    var merizosConn = shardingTest.s;
    var testDB = merizosConn.getDB(testDBName);

    // Add replSet1 as only shard
    assert.commandWorked(merizosConn.adminCommand({addshard: replSet1.getURL()}));

    // Enable sharding on test db and its collection foo
    assert.commandWorked(merizosConn.getDB('admin').runCommand({enablesharding: testDBName}));
