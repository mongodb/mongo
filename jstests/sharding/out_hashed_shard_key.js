// Tests that the $out stage works correctly when the shard key is hashed. This includes the case
// when a uniqueKey is not explicitly specified and also when there is a unique, non-hashed index
// that matches the uniqueKey.
(function() {
    "use strict";
    ...
    function testHashedShardKey(shardKey, spec, prefixPipeline = []) {
        ...
        // Test that $out passes without specifying a uniqueKey.
        assertUniqueKeyIsValid({source: source, target: target, prevStages: prefixPipeline});

        // Test that $out fails even if the uniqueKey matches the shardKey, since it isn't unique.
        assertUniqueKeyIsInvalid(
            {source: source, target: target, uniqueKey: shardKey, prevStages: prefixPipeline});

        // Test that the $out passes if there exists a unique index prefixed on the hashed shard
        // key.
    ...
    // Test that $out passes without specifying a uniqueKey.
    assertUniqueKeyIsValid({source: source, target: target});

    // Test that $out passes when the uniqueKey matches the shard key. Note that the _id index is
    // always create with {unique: true} regardless of whether the shard key was marked as unique
    // when the collection was sharded.
    assertUniqueKeyIsValid({source: source, target: target, uniqueKey: {_id: 1}});
