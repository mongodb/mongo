...
function testMigrationFailsAndWritesAreNotBlocked() {
    ...
    assert.commandFailedWithCode(moveChunkResult,
                                 [ErrorCodes.LockTimeout, ErrorCodes.CommandFailed]);
}
...
assert.commandWorked(st.s.adminCommand(
    {moveChunk: ns, find: {x: 1}, to: st.shard1.shardName, _waitForDelete: true}));
...
// Test that the migration recipient correctly applies the migrationLockAcquisitionMaxWaitMS timeout
// for strong lock acquisition.
{
...
// Test that the migration donor correctly applies the migrationLockAcquisitionMaxWaitMS timeout for
