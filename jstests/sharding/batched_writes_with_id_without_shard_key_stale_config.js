...
const performOps = function (ordered, numRetryCount) {
    ...
    assert.eq(numRetryCount, mongosServerStatus.metrics.query.updateOneWithoutShardKeyWithIdRetryCount);
    assert.commandWorked(db.adminCommand({moveChunk: coll.getFullName(), find: {x: -1}, to: st.shard0.shardName}));

    res = sessionColl.bulkWrite(
        [{deleteOne: {"filter": {_id: -1}}}, {deleteOne: {"filter": {_id: 1}}}, {deleteOne: {"filter": {_id: 2}}}],
        {ordered: ordered},
    );
    assert.eq(res.deletedCount, 2);
    mongosServerStatus = assert.commandWorked(st.s1.getDB(jsTestName()).adminCommand({serverStatus: 1}));
