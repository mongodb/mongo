...
reshardingTest.withReshardingInBackground(
    ...
    () => {},
    {
        postCheckConsistencyFn: () => {
            ...
            res = sourceCollection.runCommand({
                collMod: sourceCollection.getName(),
                maxTimeMS: 5000,
            });
            assert(ErrorCodes.isExceededTimeLimitError(res.code));
...
assert.commandWorked(
    sourceCollection.runCommand({insert: collName, documents: [{_id: 1, oldKey: -10, newKey: 10}]}),
);

assert.commandWorked(
    sourceCollection.runCommand({
        update: collName,
        updates: [{q: {_id: 0, newKey: 20}, u: {$set: {oldKey: 15}}}],
    }),
);
