/**
 * This tests whether slaveOk reads are properly routed through bongos in
...
(function() {
    ...
    function doesRouteToSec(coll, query) {
        ...
        var conn = new Bongo(serverInfo.host + ":" + serverInfo.port.toString());
     ...
     * remote location. This is because bongod allows unautheticated
     ...
     */
    awaitRSClientHosts(bongos, replTest.getSecondaries(), {ok: true, secondary: true});
    ...
    /* Although bongos never caches query results, try to do a different query
    ...
    awaitRSClientHosts(bongos, replTest.getSecondaries(), {ok: true, secondary: true});
    //
    // We also need to wait for the primary, it's possible that the bongos may think a node is a
    // secondary but it actually changed to a primary before we send our final query.
    //
    awaitRSClientHosts(bongos, replTest.getPrimary(), {ok: true, ismaster: true});
