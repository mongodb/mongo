...
(function testToRewrites() {
    ...
    // Create some new collections to ensure that test cases has sufficient namespaces to verify
    // that the filtering on the 'to' field is working correctly.
    const coll1 = createShardedCollection("coll1", 5 /* splitAt */);
    const coll2 = createShardedCollection("coll2", 9 /* splitAt */);
    ...
    // field. These fields should not interfere with the rewritten predicate and only expected
    // documents should be returned at the oplog level.
    ...
    // This group of tests ensures that the '$match' on the 'to' object only sees its documents and
    // only required document(s) are returned at the oplog for each shard.
    verifyOnWholeCluster(resumeAfterToken,
    ...
    // Ensure that the '$match' on the 'to' object with only db component should not emit any
    // document and the oplog should not return any documents.
    verifyOnWholeCluster(resumeAfterToken,
                         {$match: {to: {db: dbName}}},
                         {},
    ...
    // Ensure that the 'to' object with 'unknown' collection does not exists and the oplog
    // cursor returns 0 document.
    verifyOnWholeCluster(resumeAfterToken,
    ...
    // Ensure the '$match' on the db field path should only return documents with rename op type for
    // all collections and oplog should also return same for each shard.
    verifyOnWholeCluster(
        resumeAfterToken,
        {$match: {"to.db": dbName}},
    ...
    // This group of tests ensures that the '$match' on the regex matching all collections should
    // return documents with rename op type and oplog should also return same for each shard.
    verifyOnWholeCluster(
        resumeAfterToken,
    ...
    // These group of tests ensure that '$nin' on matching db name should not return any documents
    // with rename op type and oplog should also return same each shard.
    verifyOnWholeCluster(resumeAfterToken,
    ...
    // Ensure that '$nin' on a collection should return the documents with only insert op type for
    // that collection and oplog should also return same for each shard.
    verifyOnWholeCluster(resumeAfterToken,
    ...
    // Ensure that '$nin' on regex of matching all collections should only return documents with op
    // type insert and oplog should also return same for each shard.
    verifyOnWholeCluster(resumeAfterToken,
    ...
    // Ensure that '$nin' with mix of string and regex can be rewritten and oplog should return
    // required documents for each shard.
    verifyOnWholeCluster(resumeAfterToken,
