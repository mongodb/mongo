 ...
 * - Unmarked jumbo chunks are marked as jumbo and NOT moved during draining, requiring manual move
...
// and ~20MB of docs with the same shard key (jumbo chunk).
    ...
    () => {
        try {
            const statusRes = assert.commandWorked(
                mongos.adminCommand({removeShard: st.shard1.shardName}));

            // Check if remaining chunks on shard1 are marked as jumbo
            const chunksOnShard1 =
                findChunksUtil.findChunksByNs(configDB, ns, {shard: st.shard1.shardName}).toArray();
            const allJumbo =
                chunksOnShard1.length > 0 && chunksOnShard1.every((chunk) => chunk.jumbo === true);

            jsTest.log(`Draining status: ${statusRes.state}, chunks remaining on shard1: ${
                chunksOnShard1}`);
            return allJumbo;
        } catch (e) {
...
const finalChunksOnShard0 =
    findChunksUtil.findChunksByNs(configDB, ns, {shard: st.shard0.shardName}).toArray();
const finalChunksOnShard1 =
    findChunksUtil.findChunksByNs(configDB, ns, {shard: st.shard1.shardName}).toArray();
...
// Verify the shard can be successfully removed now that all chunks are gone.
