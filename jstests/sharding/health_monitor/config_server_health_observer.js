...
assert.soon(
    () => {
        ...
        if (
            result.configServer.totalChecks >= kWaitForCompletedChecksCount &&
            result.configServer.totalChecks - result.configServer.totalChecksWithFailure >= kWaitForPassedChecksCount
        ) {
            return true;
        }
        return false;
    },
    "Config server health check did not reach necessary count of passed checks",
    60000,
    1000,
);
