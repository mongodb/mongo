...
// Create a thread which leaves the TransactionCoordinator in a state right before it tries to
// delete the coordinator doc. Set up a failpoint for after deleting the coordinator doc because
// commitTransaction will return before deleting the coordinator, and we want to ensure that we wait
// for the deletion to finish before we allow the test to proceed.
const hangBeforeDeletingCoordinatorDocFp =
    configureFailPoint(txnCoordinator, "hangBeforeDeletingCoordinatorDoc");
const hangAfterDeletingCoordinatorDocFp =
    configureFailPoint(txnCoordinator, "hangAfterDeletingCoordinatorDoc");
...
// Allow the deleteCoordinatorDocThread to proceed with deleting the doc. This tests that we skip
// write ticket acquisition for deleting the doc.
...
// The commitTransaction command does wait for the coordinator doc to be deleted in v7.0. Wait for
// the command to fully return.
