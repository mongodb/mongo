...
assert.commandWorked(mongos.adminCommand({enableSharding: "TestDB", primaryShard: st.shard1.shardName}));

function checkChunkCounts(collName, chunksOnShard0, chunksOnShard1) {
    let counts = st.chunkCounts(collName, "TestDB");
    assert.eq(chunksOnShard0, counts[st.shard0.shardName], "Count mismatch on shard0: " + tojson(counts));
    assert.eq(chunksOnShard1, counts[st.shard1.shardName], "Count mismatch on shard1: " + tojson(counts));
}
...
assert.commandWorked(mongos.adminCommand({shardCollection: "TestDB.HashedCollNotEmpty", key: {aKey: "hashed"}}));
assert.eq(
    1,
...
assert.commandWorked(mongos.adminCommand({shardCollection: "TestDB.HashedCollEmpty", key: {aKey: "hashed"}}));
checkChunkCounts("HashedCollEmpty", 1, 1);

// Hashed sharding + non-existent collection
assert.commandWorked(mongos.adminCommand({shardCollection: "TestDB.HashedCollNonExistent", key: {aKey: "hashed"}}));
