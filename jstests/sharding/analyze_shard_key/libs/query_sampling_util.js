...
var QuerySamplingUtil = (function() {
    ...
    /**
     * Runs the commands generated by 'makeCmdObjFunc' against 'db' at rate 'numPerSec' for
     * 'durationSecs'.
     */
    function runCmdsOnRepeat(db, makeCmdObjFunc, numPerSec, durationSecs) {
        const durationMs = 1000 * durationSecs;
        const periodMs = 1000.0 / numPerSec;
        ...
        jsTest.log("Finished running commands on repeat: " + tojsononeline({
                       durationMs,
                       periodMs,
                       actualNumPerSec: actualNum * 1000 / durationMs,
                       expectedNumPerSec: numPerSec
                   }));
    }
