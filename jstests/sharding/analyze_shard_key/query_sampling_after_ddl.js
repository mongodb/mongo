...
function assertNumSampledQueries(conn, ns, expectedNum) {
    ...
    assert.soon(() => {
        const aggRes = assert.commandWorked(
            conn.adminCommand({
                aggregate: 1,
                pipeline: [{$listSampledQueries: {namespace: ns}}],
                cursor: {},
            }),
        );
...
function testDropDatabase(conn, {recreateCollection, isShardedColl, rst, st}) {
    ...
    const db = conn.getDB(dbName);
    jsTest.log(
        `Testing testDropDatabase ${tojson({dbName, collName, isShardedColl, recreateCollection})}`,
    );
...
function testRenameCollection(conn, {sameDatabase, isShardedColl, rst, st}) {
    ...
    if (!sameDatabase && st) {
        ...
        assert.commandWorked(
            st.s.adminCommand({
                movePrimary: dstDbName,
                to: st.getPrimaryShardIdForDatabase(srcDbName),
            }),
        );
    }
