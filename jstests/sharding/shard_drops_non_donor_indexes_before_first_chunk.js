...
assert.commandWorked(
    st.s.adminCommand({moveChunk: coll.getFullName(), find: {_id: 0}, to: st.shard1.shardName, waitForDelete: true}),
);

assert.commandWorked(
    st.s.adminCommand({moveChunk: coll.getFullName(), find: {_id: 0}, to: st.shard0.shardName, waitForDelete: true}),
);
...
assert.eq(0, shard0Coll.aggregate([{$indexStats: {}}, {$match: {name: indexName}}]).toArray().length);
assert.eq(1, shard1Coll.aggregate([{$indexStats: {}}, {$match: {name: indexName}}]).toArray().length);
...
assert.eq(0, shard1Coll.aggregate([{$indexStats: {}}, {$match: {name: indexName}}]).toArray().length);
...
assert.commandWorked(shard0Coll.createIndex(commonIndex, {name: commonIndexName, expireAfterSeconds: 2000}));
assert.commandWorked(shard1Coll.createIndex(commonIndex, {name: commonIndexName, expireAfterSeconds: 2000}));

// Now modify the index on shard1 to differ by one option.
assert.commandWorked(
    shard1DB.runCommand({collMod: collName, index: {keyPattern: commonIndex, expireAfterSeconds: 5000}}),
);
...
const commonIndexPipeline = [{$indexStats: {}}, {$match: {name: commonIndexName}}, {$project: {name: 1, spec: 1}}];
