...
assert.eq(
    2,
    docs.length,
    `Unexpected content in unsharded collection with replicated record IDs ${coll.getFullName()}: ${tojson(docs)}`,
);
...
for (const [name, recordId] of Object.entries(collRecordIdsBeforeMoveCollection)) {
    assert(
        ...
        `Unexpected document (${name}, ${
            recordId
        }) in unsharded collection with replicated record IDs ${coll.getFullName()}: ${tojson(docs)}`,
    );
...
assert.neq(
    collRecordIdsBeforeMoveCollection["Bart"],
    getRidForDoc(shard1.getDB(dbName), collName, {name: "Bart"}),
    `Unexpected value for record ID: Bart: ${tojson(shard1.getCollection(collNS).find().showRecordId().toArray())}`,
);
assert.neq(
    collRecordIdsBeforeMoveCollection["Lisa"],
    getRidForDoc(shard1.getDB(dbName), collName, {name: "Lisa"}),
    `Unexpected value for record ID: Lisa: ${tojson(shard1.getCollection(collNS).find().showRecordId().toArray())}`,
);
