...
assert.commandWorked(st.s.adminCommand({refineCollectionShardKey: kNsName, key: {_id: 1, aKey: 1}}));
assert.eq({_id: 1, aKey: 1}, st.s.getCollection(kConfigCollections).findOne({_id: kNsName}).key);
assert.eq(
    1,
    st.s.getCollection(kConfigChangelog).find({what: "refineCollectionShardKey.start", ns: kNsName}).itcount(),
);
assert.eq(1, st.s.getCollection(kConfigChangelog).find({what: "refineCollectionShardKey.end", ns: kNsName}).itcount());
st.stop();
