 ...
 * the mongos) and a shard specific user on a shard server. On a catalog shard, since the config
...
// Test that we can't create a shard specific user on the catalog shard if we already created a
...
// on a catalog shard using the localhost exception.
var st = new ShardingTest({
    mongos: 1,
    shards: 1,
    config: 1,
...
// Test that the shard specific user created on the catalog shard is also a cluster wide user by
