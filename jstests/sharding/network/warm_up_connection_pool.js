...
let testWarmUpParams = {};
let testWarmUpConnPoolStatsCheck = function (connPoolStats, currentShard) {
    return (
        connPoolStats["hosts"][currentShard]["inUse"] +
...
let warmUpDisabledParams = {
    setParameter: {warmMinConnectionsInShardingTaskExecutorPoolOnStartup: false},
};
let warmUpDisabledConnPoolStatsCheck = function (connPoolStats, currentShard) {
...
let shutdownNodeParams = {};
let shutdownNodeConnPoolStatsCheck = function (connPoolStats, currentShard, primary) {
...
let shutdownNodeExtraOptions = function (test) {
    const nodeList = test.rs0.nodeList();
    const primary = test.rs0.getPrimary();
    let pId = test.rs0.getNodeId(primary);
