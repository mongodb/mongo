...
(function() {
    ...
    /**
     * @summary This function executes a count command with read preference "secondary" and returns
     * the
     * command response
     ...
     *     against
     * @param {Object} query the fields by which the read command should query by. For example {_id:
     * 1,
     *     x: 4}, etc
     ...
     *     query on property 'n'
     */
    const runReadCmdWithReadPrefSecondary =
        (db, collectionName, query, {readConcern, maxTimeMS}) => {
            return db.runCommand({
                count: collectionName,
                query,
                $readPreference: {mode: "secondary"}, readConcern, maxTimeMS
            });
        };
     ...
     * 3.) Splits the chunk on the sharded collection based of {_id} as the middle of the chunk
     * split.
     * @param {ShardingTest} shardingTest The instance of the initialized shardingTest
     * @param {String} dbName The string name of the database in which sharding will be enabled.
     * Also
     *     the DB that contains the collection that will be sharded
     * @param {String} collectionNamespace The namespace (DB Name + Collection Name) of the
     * collection
     *     that will be sharded. After sharding the namespace will be used for chunk splitting
     * @param {String} primaryShardName - Used to ensure the desired shard becomes the primary
     * shard.
     */
    const setupShardedCollection =
        (shardingTest, dbName, collectionNamespace, primaryShardName) => {
    ...
    // Sends a versioned read through mongos1, the second router, to cause a refresh on the
    // secondary of
    // the donor shard. Since it runs while the migration is paused on 'chunkDataCommitted' the
    // primary
    // is only blocking writes and if the secondary isn't checking for writes being blocked when
    // sending
    ...
    // Allow the migration to commit and pause it before the donor shard's primary refreshes from
    // the
    // config server.
    ...
    checkLog.contains(st.shard0, "hangBeforePostMigrationCommitRefresh fail point enabled");

    // Write to the migrated ns1 chunk. Run flushRouterConfig on the mongos to force it to refresh
    // from
    // the config server when it does the write since otherwise it will route the request shard0
    // which
    // will be blocked behind the critical section. This insert will be used to test causal
    // consistency
    // in the later read.
