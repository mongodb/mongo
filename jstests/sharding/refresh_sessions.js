(function() {
    ...
    // Test that refreshing on merizos flushes local records to the collection.
    {
        var merizos = cluster.s.getDB(sessionsDb);
        var sessionCount = merizos.system.sessions.count();

        // Start one session.
        assert.commandWorked(merizos.runCommand(startSession));
        assert.commandWorked(merizos.runCommand(refresh));

        // Test that it landed in the collection.
        assert.eq(merizos.system.sessions.count(),
                  sessionCount + 1,
                  "refresh on merizos did not flush session record");
    }

    // Test that refreshing on merizod flushes local records to the collection.
    {
        var merizos = cluster.s.getDB(sessionsDb);
        ...
        assert.eq(merizos.system.sessions.count(),
                  sessionCount + 1,
                  "refresh on merizod did not flush session record");
    }
    ...
    // Test that refreshing on all servers flushes all records.
    {
        ...
        assert.eq(merizos.system.sessions.count(),
                  sessionCount + 1,
                  "refresh on merizos did not flush session records to disk");
