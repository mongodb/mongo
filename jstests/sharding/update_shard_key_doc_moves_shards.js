/*
 * Tests that changing the shard key value of a document using update works correctly when the doc
 * will change shards.
...
(function() {
    ...
    assert.eq(1, sessionDB.foo.find({"x": 300}).itcount());
    ...
    session.abortTransaction();
    assert.eq(1, sessionDB.foo.find({"x": 4}).itcount());
    assert.eq(0, sessionDB.foo.find({"x": 600}).itcount());
    assert.commandWorked(st.rs1.getPrimary().getDB(kDbName).adminCommand({
        configureFailPoint: "failCommand",
        mode: "off",
    }));
