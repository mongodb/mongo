(function() {
    ...
    var st = new ShardingTest({name: 'rename_across_bongos', shards: 1, bongos: 2});
    var dbName = 'RenameDB';

    st.s0.getDB(dbName).dropDatabase();
    st.s1.getDB(dbName).dropDatabase();

    // Create collection on first bongos and insert a document
    ...
    if (st.configRS) {
        // Ensure that the second bongos will see the newly created database metadata when
        // it tries to do the collection rename.
        st.configRS.awaitLastOpCommitted();
    }

    // Rename collection on second bongos and ensure the document is found
