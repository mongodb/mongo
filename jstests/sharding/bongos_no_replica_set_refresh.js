// Tests that bongos and the shard discover changes to the shard's replica set membership.
...
(function() {
    ...
    var numBongosHosts = function() {
        var commandResult = assert.commandWorked(bongos.adminCommand("connPoolStats"));
    ...
    var checkNumHosts = function(expectedNumHosts) {
        ...
        jsTest.log("Waiting for the bongos to discover that the shard now has " + expectedNumHosts +
                   " hosts.");
        var numHostsSeenByBongos;

        // Use a high timeout (5 minutes) because replica set refreshes are only done every 30
        // seconds.
        assert.soon(
            function() {
                numHostsSeenByBongos = numBongosHosts();
                return numHostsSeenByBongos === expectedNumHosts;
            },
            function() {
                return ("Expected bongos to see " + expectedNumHosts +
                        " hosts on shard but found " + numHostsSeenByBongos);
            },
            five_minutes);
    };

    var st = new ShardingTest({
        name: 'bongos_no_replica_set_refresh',
        shards: 1,
        bongos: 1,
        other: {
            rs0: {
    ...
    printjson(bongos.getCollection("foo.bar").findOne());
