...
(function testBucketCollectionWithoutValidView() {
    // TODO SERVER-110221 do not ignore MalformedTimeseriesBucketsCollection in multiversion suites
    // (the error doesn't exist in v8.0 and needs to be backported)
    const isMultiVersion = Boolean(jsTest.options().useRandomBinVersionsWithinReplicaSet);
    if (isMultiVersion) {
        return;
    }
