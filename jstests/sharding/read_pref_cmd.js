...
var setUp = function(rst) {
    ...
    rst.nodes.forEach(function(node) {
        ...
        const getProfilingLockDeadlineRes =
            node.adminCommand({getParameter: 1, internalQueryGlobalProfilingLockDeadlineMs: 1});
        if (getProfilingLockDeadlineRes.ok) {
            assert.commandWorked(node.adminCommand(
                {setParameter: 1, internalQueryGlobalProfilingLockDeadlineMs: 1000}));
        } else {
            assert.eq(getProfilingLockDeadlineRes.code,
                      ErrorCodes.InvalidOptions,
                      tojson(getProfilingLockDeadlineRes));
            assert.eq(getProfilingLockDeadlineRes.errmsg,
