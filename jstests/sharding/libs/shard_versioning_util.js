...
export var ShardVersioningUtil = (function () {
    ...
    let getRouterStaleConfigErrorCount = function (conn) {
        ...
        return Number((stats.catalogCache || {}).countStaleConfigErrors || 0);
    };
    ...
    let getShardStaleConfigErrorCount = function (conn) {
        const stats =
            assert.commandWorked(conn.adminCommand({serverStatus: 1})).shardingStatistics || {};
        return Number(stats.countStaleConfigErrors || 0);
    };
     ...
     *      shard.
     *   3. Runs `staleOperation(staleRouter)`, which routes by the stale cache, receives a
     ...
     *
     * Returns whatever `staleOperation` returns, so callers can make further assertions on it.
     */
    let runOperationOnStaleRouterAfterMoveChunk = function ({
        ...
        const result = staleOperation(staleRouter);
