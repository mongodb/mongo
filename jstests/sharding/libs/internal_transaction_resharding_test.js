...
function InternalTransactionReshardingTest({reshardInPlace}) {
    ...
    this.runTestForInsertUpdateDeleteDuringResharding = function(txnType) {
        const testCases = [];
        for (let isPreparedTxn of [true, false]) {
            for (let isLargeTxn of [true, false]) {
                for (let abortOnInitialTry of [true, false]) {
    ...
    this.runTestForInsertUpdateDeleteBeforeResharding = function(txnType) {
        const testCases = [];
        for (let isPreparedTxn of [true, false]) {
            for (let isLargeTxn of [true, false]) {
                for (let abortOnInitialTry of [true, false]) {
