...
// NEW QUERY EXPLAIN
assert.eq(coll.find({}, {b:1, c:1, _id:0}).sort({a:1,b:-1,c:1}).hint({a:1, b:-1, c:1}).itcount(), 100);
/* NEW QUERY EXPLAIN
...
// NEW QUERY EXPLAIN
assert.eq(coll.find({a:{$gt:25,$lt:43}}, {a:1, c:1, _id:0}).sort({a:1, b:1, c:-1}).hint({a:1, b:-1, c:1}).itcount(), 17);
/* NEW QUERY EXPLAIN
...
// Test range query, sort on subset of fields that in order different from index
// NEW QUERY EXPLAIN
assert.eq(coll.find({a:{$gt:25,$lt:43}}, {a:1, c:1, _id:0}).sort({b:1, c:-1}).hint({a:1, b:-1, c:1}).itcount(), 17);
/* NEW QUERY EXPLAIN
