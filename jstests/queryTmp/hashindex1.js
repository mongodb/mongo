...
// NEW QUERY EXPLAIN
assert.eq( t.find({a : 1}).itcount(), 1);
// NEW QUERY EXPLAIN
assert.eq( t.find({a : {$gte : 3 , $lte : 3}}).itcount(), 1);
// NEW QUERY EXPLAIN
assert.eq( t.find({c : 1}).itcount(), 0);

// Hash index used with a $in set membership predicate.
// NEW QUERY EXPLAIN
assert.eq( t.find({a : {$in : [1,2]}}).itcount(), 2);

// Hash index used with a singleton $and predicate conjunction.
// NEW QUERY EXPLAIN
assert.eq( t.find({$and : [{a : 1}]}).itcount(), 1);
