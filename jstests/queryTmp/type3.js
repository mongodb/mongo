...
t.remove();
// NEW QUERY EXPLAIN
assert.eq( 0, t.find( {a:{$type:10}} ).hint( {a:1} ).itcount() );

// Type Undefined
t.remove();
// 'null' is the client friendly version of undefined.
// NEW QUERY EXPLAIN
assert.eq( 0, t.find( {a:{$type:6}} ).hint( {a:1} ).itcount() );
...
// Type MinKey
// NEW QUERY EXPLAIN
assert.eq( 0, t.find( {a:{$type:-1}} ).hint( {a:1} ).itcount() );
// Type MaxKey
// NEW QUERY EXPLAIN
assert.eq( 0, t.find( {a:{$type:127}} ).hint( {a:1} ).itcount() );
