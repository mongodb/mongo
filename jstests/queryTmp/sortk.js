...
// NEW QUERY EXPLAIN
assert.eq( 1, simpleQueryWithLimit( -1 ).itcount() );

// The limit is -2.
assert.eq( 0, simpleQueryWithLimit( -2 )[ 0 ].b );
assert.eq( 1, simpleQueryWithLimit( -2 )[ 1 ].b );
// NEW QUERY EXPLAIN
assert.eq( 2, simpleQueryWithLimit( -2 ).itcount() );
...
// NEW QUERY EXPLAIN
assert.eq( 1, simpleQueryWithLimit( -1 ).skip( 1 ).itcount() );
...
// NEW QUERY EXPLAIN
assert.eq( 1, inThreeIntervalQueryWithLimit( -1 ).itcount() );
assert.eq( 1, inThreeIntervalQueryWithLimit( -2 )[ 0 ].b );
assert.eq( 2, inThreeIntervalQueryWithLimit( -2 )[ 1 ].b );
// NEW QUERY EXPLAIN
assert.eq( 2, inThreeIntervalQueryWithLimit( -2 ).itcount() );
t.save( { a:3, b:0 } );
...
// NEW QUERY EXPLAIN
assert.eq( 1, simpleQuery( {}, { b:-1 }, { a:1, b:-1 } ).limit( -1 ).itcount() );
...
// NEW QUERY EXPLAIN
assert.eq( 2, inQueryWithLimit( -2, { b:1, c:1 } ).itcount() );
