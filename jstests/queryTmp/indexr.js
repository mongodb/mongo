...
// NEW QUERY EXPLAIN
assert.eq( 0, t.find( { 'a.b':{ $gt:2 }, 'a.c': { $lt:4 } } ).itcount() );
// NEW QUERY EXPLAIN
assert.eq( 0, t.find( { a:{ b:3, c:6 }, 'a.c': { $lt:4 } } ).itcount() );

t.save( { a: { b: 3, c: 3 } } );
assert.eq( 1, t.count( { 'a.b':{ $gt:2 }, 'a.c': { $lt:4 } } ) );
assert.eq( 1, t.count( { a:{ b:3, c:3 }, 'a.c': { $lt:4 } } ) );
// NEW QUERY EXPLAIN
assert.eq( 1, t.find( { 'a.b':{ $gt:2 }, 'a.c': { $lt:4 } } ).itcount() );
// NEW QUERY EXPLAIN
assert.eq( 1, t.find( { a:{ b:3, c:3 }, 'a.c': { $lt:4 } } ).itcount() );
...
assert.eq( 1, t.find( { 'a.b':{ $gt:2 }, 'a.c': { $lt:4 } } ).itcount() );
// NEW QUERY EXPLAIN
assert.eq( 1, t.find( { a:{ b:3, c:6 }, 'a.c': { $lt:4 } } ).itcount() );
