...
assert.commandWorked(
    admin.runCommand({createUser: "admin", pwd: "pass", roles: jsTest.adminUserRoles}));
...
assert.commandFailedWithCode(
    config.runCommand(
        {aggregate: "system.sessions", pipeline: [{$listSessions: {allUsers: true}}], cursor: {}}),
    ErrorCodes.Unauthorized,
);

// Sanity check: an unprivileged user can list their own sessions without specifying
// $_internalPredicate.
assert.commandWorked(
    config.runCommand({
...
assertBypassRejected(
    config, "system.sessions", [{$listSessions: {$_internalPredicate: predicate}}]);
...
assertBypassRejected(
    config, "system.sessions", [{$listSessions: {allUsers: true, $_internalPredicate: predicate}}]);
