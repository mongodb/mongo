...
function commandWorked(db, expectedDbs) {
    ...
    expectedDbs.forEach((dbName) => {
        assert(
            isPresent(stageResult, dbName),
            `The db ${dbName} is expected to be returned by $listClusterCatalog, but it was not found. Stage Result: ${tojson(stageResult)}`,
        );
    });
}
        ...
        assert.commandWorked(
            adminDb.runCommand({
                ...
                roles: [
                    ...
                    {role: kListCollectionsRole, db: kDb1},
                ],
            }),
        );
    });
