/**
 * This tests that updates to user and role definitions made on one bongos propagate properly
 * to other bongoses.
 */
 ...
 * different bongoses.  "db1", "db2", and "db3" are all auth'd as spencer@test and will be used
 * to verify that user and role data changes get propaged to their bongoses.
 * "db2" is connected to a bongos with a 10 second user cache invalidation interval,
 * while "db3" is connected to a bongos with a 10 minute cache invalidation interval.
 */
...
(function testGrantingPrivileges() {
    ...
    }, "Bongos did not update its user cache after 10 seconds", 10 * 1000);
...
(function testConcurrentUserModification() {
    jsTestLog("Testing having 2 bongoses modify the same user at the same time");  // SERVER-13850
...
(function testDroppingUser() {
    ...
    assert.soon(
        ...
        "Bongos did not update its user cache after 10 seconds",
...
print("SUCCESS Completed bongos_cache_invalidation.js");
