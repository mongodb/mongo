...
// In order to test BONGODB-CR we need to "reset" the authSchemaVersion to
// 26Final "3" or else the user won't get BONGODB-CR credentials.
...
var restartedConn = BongoRunner.runBongod({
    auth: "",
    restart: conn,
    setParameter: "authenticationMechanisms=SCRAM-SHA-1,BONGODB-CR,CRAM-MD5"
});
if (restartedConn != null) {
    mechanisms = ["SCRAM-SHA-1", "BONGODB-CR", "CRAM-MD5"];
    hasCR = true;
    hasCramMd5 = true;
    print("test info: Enabling non-default authentication mechanisms.");
} else {
    restartedConn = BongoRunner.runBongod({restart: conn});
    mechanisms = ["SCRAM-SHA-1", "BONGODB-CR"];
    hasCR = true;
    hasCramMd5 = false;
...
// Try all the ways to call db.auth that uses SCRAM-SHA-1 or BONGODB-CR.
if (hasCR) {
    ...
    admin.logout();
    assert(admin.auth({mechanism: 'BONGODB-CR', user: 'andy', pwd: 'a'}));
    admin.logout();
}
...
// Sanity check that we tested at least one of BONGODB-CR and CRAM-MD5.
