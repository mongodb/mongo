    ...
    describe(`killCursors namespace authorization (${topologyName})`, function () {
        ...
        function tryKill({
            ...
            const openRes = assert.commandWorked(
                openerDb.runCommand({find: cursorColl, batchSize: 2}),
            );
            const cursorId = openRes.cursor.id;
            assert.neq(cursorId, 0, "cursor.id should be non-zero; need more data in collection", {
                openRes,
            });
            ...
            if (expectKilled) {
                ...
                assert.commandWorked(
                    adminConn
                        .getDB(cursorDb)
                        .runCommand({killCursors: cursorColl, cursors: [cursorId]}),
                );
            }
        }
