...
function registerTests(topologyName, startFn) {
    describe(`killCursors namespace authorization (${topologyName})`, function() {
        ...
        before(function() {
            ...
            assert.commandWorked(
                bootstrapAdmin.runCommand({
                    ...
                    privileges:
                        [{resource: {db: "dbA", collection: ""}, actions: ["killAnyCursor"]}],
                    roles: [],
                }),
            );
            assert.commandWorked(
                bootstrapAdmin.runCommand({
                    ...
                    privileges:
                        [{resource: {db: "dbA", collection: "collA1"}, actions: ["killAnyCursor"]}],
                    roles: [],
                }),
            );
            assert.commandWorked(
        ...
        function tryKill(
            ...
            const openRes =
                assert.commandWorked(openerDb.runCommand({find: cursorColl, batchSize: 2}));
            const cursorId = openRes.cursor.id;
            assert.neq(cursorId,
                       0,
                       "cursor.id should be non-zero; need more data in collection",
                       {openRes});
            ...
            if (expectKilled) {
                ...
                assert.commandWorked(
                    adminConn.getDB(cursorDb).runCommand(
        ...
        it("user can kill their own cursor even when the request namespace is spoofed to another db",
           function() {
               // noPriv has no killAnyCursor privilege anywhere, but it still allowed to kill a
               // cursor that they created.
               tryKill({
                   openerConn: noPrivConn,
                   cursorDb: "dbA",
                   cursorColl: "collA1",
        ...
        it("db-level killAnyCursor on dbA kills alice's cursor on dbA.collA2 (grant covers all collections in the db)",
           function() {
               tryKill({
                   openerConn: aliceConn,
                   cursorDb: "dbA",
        ...
        it("db-level killAnyCursor on dbA cannot kill bob's cursor on dbB (honest request namespace)",
           function() {
               tryKill({
                   openerConn: bobConn,
                   cursorDb: "dbB",
        ...
        it("db-level killAnyCursor on dbA cannot kill bob's cursor on dbB via request namespace spoofing (SERVER-128198 cross-db repro)",
           function() {
               tryKill({
                   openerConn: bobConn,
                   cursorDb: "dbB",
        ...
        it("collection-level killAnyCursor on dbA.collA1 kills alice's cursor on dbA.collA1",
           function() {
               tryKill({
                   openerConn: aliceConn,
                   cursorDb: "dbA",
        ...
        it("collection-level killAnyCursor on dbA.collA1 cannot kill alice's cursor on dbA.collA2 (honest request namespace)",
           function() {
        ...
        it("collection-level killAnyCursor on dbA.collA1 cannot kill alice's cursor on dbA.collA2 via request namespace spoofing (SERVER-128198 cross-collection repro)",
           function() {
               tryKill({
                   openerConn: aliceConn,
                   cursorDb: "dbA",
        ...
        it("cluster-level killAnyCursor kills any cursor regardless of request namespace",
           function() {
               tryKill({
                   openerConn: bobConn,
                   cursorDb: "dbB",
        ...
        it("db-level killAnyCursor on dbA kills alice's cursor on dbA even when the request namespace points to dbB",
           function() {
               tryKill({
                   openerConn: aliceConn,
                   cursorDb: "dbA",
