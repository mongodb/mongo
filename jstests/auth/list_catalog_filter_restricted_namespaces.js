...
function setupUsers(conn) {
    ...
    assert.commandWorked(adminDb.runCommand({createUser: "root", pwd: "rootpwd", roles: ["__system"]}));
    assert(adminDb.auth("root", "rootpwd"));
    assert.commandWorked(adminDb.runCommand({createUser: "reader", pwd: "readerpwd", roles: ["readAnyDatabase"]}));
    adminDb.logout();
}
...
function runFilterTest(conn) {
    ...
    // below.
    assert.commandWorked(conn.getDB("testdb").createCollection("tscoll", {timeseries: {timeField: "ts"}}));
    ...
    const systemEntries = readerEntries.filter(
        (e) => e.name.startsWith("system.") && e.name !== "system.js" && !e.name.startsWith("system.buckets."),
    );
    assert.eq(
