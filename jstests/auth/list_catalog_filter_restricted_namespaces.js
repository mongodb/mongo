...
function runFilterTest(conn) {
    ...
    assert.eq(configEntries.length,
              0,
              "reader should not see config.* entries: " + tojson(configEntries));
    ...
    const systemEntries = readerEntries.filter(
        (e) => e.name.startsWith("system.") && e.name !== "system.js" &&
    ...
    const hasUserSystemFoo =
        readerEntries.some((e) => e.db === "testdb" && e.name === "system_foo");
    assert(hasUserSystemFoo, "reader should see testdb.system_foo: " + tojson(readerEntries));

    const hasUserSystemBuckets =
        readerEntries.some((e) => e.db === "testdb" && e.name === "system_buckets_foo");
    assert(hasUserSystemBuckets,
           "reader should see testdb.system_buckets_foo: " + tojson(readerEntries));
    ...
    assert(rootConfigEntries.length > 0,
           "root should see config.* entries: " + tojson(rootEntries));
    ...
    for (const ns of readerNs) {
        assert(rootNs.has(ns),
               "root should include all namespaces visible to reader, missing: " + ns);
    }
