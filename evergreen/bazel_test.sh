...
ALL_FLAGS="--verbose_failures ${LOCAL_ARG} ${bazel_args} ${bazel_compile_flags} ${task_compile_flags} --define=MONGO_VERSION=${version} ${patch_compile_flags} ${REMOTE_EXEC_DISABLE}"
echo ${ALL_FLAGS} > .bazel_build_flags

set +o errexit

for i in {1..3}; do
  eval ${TIMEOUT_CMD} ${BAZEL_BINARY} fetch ${ALL_FLAGS} ${targets} ${REMOTE_EXEC_DISABLE} && RET=0 && break || RET=$? && sleep 60
  if [ $RET -eq 124 ]; then
...
for i in {1..3}; do
  eval ${TIMEOUT_CMD} ${BAZEL_BINARY} test ${ALL_FLAGS} ${targets} ${REMOTE_EXEC_DISABLE} 2>&1 | tee bazel_stdout.log \
...
if [[ $RET != 0 ]]; then
  ...
  eval ${BAZEL_BINARY} run ${CONFIG_FLAGS} ${REMOTE_EXEC_DISABLE} //buildscripts:gather_failed_unittests
fi
