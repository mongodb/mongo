...
# task expansion. scons_compile.sh passes MONGO_VERSION=${version} to scons, and
# version_expansions_generate.sh has already folded the override into the
# regenerated version expansion.
#
# This script also runs on variants that do not configure a custom build (it is
# part of the shared "scons compile" function), in which case it writes a
