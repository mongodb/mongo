...
public:
     ...
     *
     * For example, the predicate {time: {$lt: new Date(...)}} will generate the
     * following predicate:
     * {$and: [
     *      {control.min.time: {$_internalExprLt: new Date(...)}}
     *      {control.max.time: {$_internalExprLt: new Date(...) + bucketMaxSpanSeconds}}
     * ]}
     *
    ...
    static BucketPredicate createPredicatesOnBucketLevelField(
        ...
        bool fixedBuckets,
        Get2dsphereIndexVersionFn get2dsphereIndexVersion = nullptr);

    /**
     * Takes predicates that were generated by createPredicatesOnBucketLevelField() and possibly
     * returns an augmented predicate that has a comparison to _id ANDed on. This can be since the
     * _id field will start with control.min.time for any given bucket, provided that there are no
     * extended range values. Predicates like this will aid in index utilization.
     *
     * It is only correct to generate these predicates when it is known that there is no extended
     * range data.
     *
     * For example, for predicate {time: {$lt: new Date(...)}} will have these predicates
     * generated by createPredicatesOnBucketLevelField():
     *
     * {$and: [
     *      {control.min.time: {$_internalExprLt: new Date(...)}}
     *      {control.max.time: {$_internalExprLt: new Date(...) + bucketMaxSpanSeconds}}
     * ]}
     *
     * This predicate will be further augmented with another comparison on the _id field:
     *
     * {$and: [
     *      {_id: {$lt: ObjectId(...)}},
     *      {control.min.time: {$_internalExprLt: new Date(...)}}
     ...
     * ]}
     *
     * In addition, if the operands of previously generated predicates are themselves extended range
     * values, we can make even more aggressive transformations:
     *
     * {$and: [
     *      {control.min.time: {$_internalExprLt: ISODate("1960-01-01")}}
     *      {control.max.time: {$_internalExprLt: ISODate("1960-01-01") + bucketMaxSpanSeconds}}
     * ]}
     *
     * This can become:
     *
     * {$alwaysFalse: 1}
     *
     * Or if the operator is $_internalExprGt:
     *
     * {$alwaysTrue: 1}
     *
     * Note that this rewrite can only be applied if we know that there are no extended range values
     * in this collection on this shard, i.e., NOT on mongos. This is why generating _id predicates
     * are performed separately from those generated in createPredicatesOnBucketLevelField().
     *
     * Returns true if match expression was modified in-place and false otherwise.
     */
    static bool generateBucketLevelIdPredicates(const ExpressionContext& pExpCtx,
                                                const BucketSpec& bucketSpec,
                                                int bucketMaxSpanSeconds,
