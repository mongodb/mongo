...
TEST(PlanCacheTest, ComputeKeyPartialIndexConjunction) {
    ...
    ASSERT_EQ(planCache.computeKey(*satisfySinglePredicate).getIndexabilityDiscriminators(), "(0)");

    unique_ptr<CanonicalQuery> satisfyBothPredicates(canonicalize("{f: {$eq: 5}}"));
    ASSERT_EQ(planCache.computeKey(*satisfyBothPredicates).getIndexabilityDiscriminators(), "(1)");

    unique_ptr<CanonicalQuery> conjSingleField(canonicalize("{f: {$gt: 2, $lt: 9}}"));
    ASSERT_EQ(planCache.computeKey(*conjSingleField).getIndexabilityDiscriminators(), "(1)");

    unique_ptr<CanonicalQuery> conjSingleFieldNoMatch(canonicalize("{f: {$gt: 2, $lt: 11}}"));
    ASSERT_EQ(planCache.computeKey(*conjSingleFieldNoMatch).getIndexabilityDiscriminators(), "(0)");
    ...
    ASSERT_EQ(planCache.computeKey(*disjSingleFieldBothSatisfy).getIndexabilityDiscriminators(),
              "(1)");

    unique_ptr<CanonicalQuery> disjSingleFieldNotSubset(
        canonicalize("{$or: [{f: {$eq: 2}}, {f: {$eq: 11}}]}"));
    ASSERT_EQ(planCache.computeKey(*disjSingleFieldNotSubset).getIndexabilityDiscriminators(),
              "(0)");
}
