...
namespace mongo {
    ...
    void PlanEnumerator::enumerateOneIndex(const IndexToPredMap& idxToFirst,
        ...
        for (IndexToPredMap::const_iterator it = idxToFirst.begin(); it != idxToFirst.end(); ++it) {
            ...
            if (thisIndex.multikey) {
                // Since the index is multikey, we can only use one of the predicates over the
                // leading field of the index. However, we do not know which of these predicates is
                // most selective. Therefore, we will generate a plan for each so that they can be
                // ranked against each other.
                vector<MatchExpression*>::const_iterator multikeyIt;
                for (multikeyIt = it->second.begin(); multikeyIt != it->second.end();
                        ++multikeyIt) {
                    MatchExpression* pred = *multikeyIt;
