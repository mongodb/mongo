...
TEST(SupplementalMetricsStats, JoinOptimizationMetrics) {
    ...
    m1.planEnumerationMetrics = OpDebug::JoinOptimizationMetrics::PlanEnumerationMetrics{
        ...
        .numPersistentNDVStatsUsed = 4,
        .numApproxLeafPagesUnavailable = 2,
    ...
    ASSERT_BSONOBJ_EQ_AUTO(
        R"({
            "JoinOptimization": {
                ...
                "numPersistentNDVStatsUsed": {"sum": 4, "max": 4, "min": 4, "sumOfSquares": {"$numberDecimal":"16"}},
                "numApproxLeafPagesUnavailable": {"sum": 2, "max": 2, "min": 2, "sumOfSquares": {"$numberDecimal":"4"}},
