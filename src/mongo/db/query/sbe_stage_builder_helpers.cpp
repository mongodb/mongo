...
public:
    ...
    std::unique_ptr<sbe::PlanStage> clone() const final {
        ...
        return std::make_unique<VirtualScanStage>(
            _commonStats.nodeId, _outField, tag, val, _yieldPolicy, _participateInTrialRunTracking);
    }
...
std::pair<sbe::value::SlotId, std::unique_ptr<sbe::PlanStage>> generateVirtualScan(
    ...
    auto virtualScan = sbe::makeS<VirtualScanStage>(kEmptyPlanNodeId, outputSlot, arrTag, arrVal);
