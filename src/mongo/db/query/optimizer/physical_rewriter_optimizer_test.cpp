...
TEST(PhysRewriter, FilterIndexing4) {
    ...
    // Assert the correct CEs for each node in group 31. Group 31 contains residual predicates.
    std::vector<std::pair<std::string, double>> pathAndCEs = {
        {"Memo.31.physicalNodes.1.nodeInfo.node.ce", 125.087},
        {"Memo.31.physicalNodes.1.nodeInfo.node.child.ce", 143.681},
        {"Memo.31.physicalNodes.1.nodeInfo.node.child.child.ce", 189.571},
        {"Memo.31.physicalNodes.1.nodeInfo.node.child.child.child.ce", 330.00000000000006}};
...
TEST(PhysRewriter, ObjectElemMatchResidual) {
    ...
    ASSERT_EXPLAIN_V2Compact_AUTO(
        ...
        "|   |   Variable [evalTemp_4]\n"
        "|   PathGet [b] PathTraverse [1] PathCompare [Eq] Const [1]\n"
        "IndexScan [{'<indexKey> 1': evalTemp_4, '<rid>': rid_0}, scanDefName: c1, indexDefName: "
...
TEST(PhysRewriter, ScanNodeRemoveOrphansImplementerSeekTargetBasic) {
    ...
    ASSERT_EXPLAIN_V2_AUTO(
        ...
        "|   |   Variable [shardKey_4]\n"
        "|   LimitSkip [limit: 1, skip: 0]\n"
        "|   Seek [ridProjection: rid_0, {'<root>': root, 'a': shardKey_4}, c1]\n"
        "IndexScan [{'<rid>': rid_0}, scanDefName: c1, indexDefName: index1, interval: {=Const "
        "[1]}]\n",
        optimized);
}
...
TEST(PhysRewriter, ScanNodeRemoveOrphansImplementerSeekTargetDottedSharedPrefix) {
    ...
    ASSERT_EXPLAIN_V2_AUTO(
        ...
        "|   Filter []\n"
        "|   |   FunctionCall [shardFilter]\n"
        "|   |   |   Variable [shardKey_10]\n"
        "|   |   Variable [shardKey_9]\n"
        "|   Evaluation [{shardKey_10}]\n"
        "|   |   EvalPath []\n"
        "|   |   |   Variable [shardKey_8]\n"
        "|   |   PathGet [b]\n"
        "|   |   PathGet [d]\n"
        "|   |   PathIdentity []\n"
        "|   Evaluation [{shardKey_9}]\n"
        "|   |   EvalPath []\n"
        "|   |   |   Variable [shardKey_8]\n"
        "|   |   PathGet [b]\n"
        "|   |   PathGet [c]\n"
        "|   |   PathIdentity []\n"
        "|   LimitSkip [limit: 1, skip: 0]\n"
        "|   Seek [ridProjection: rid_0, {'<root>': root, 'a': shardKey_8}, c1]\n"
        "IndexScan [{'<rid>': rid_0}, scanDefName: c1, indexDefName: index1, interval: {=Const "
        "[3]}]\n",
        optimized);
}
