...
public:
    ...
    // If no more multikey elements are found, return eoo.
    ...
    std::pair<BSONElement, bool> nextElement();
...
private:
    ...
    std::pair<BSONElement, bool> _getFirstElementRooted(size_t rootIdx, BSONElement elem);
    // Given a BSON object, extracts the first multikey element from it.
    // If the given element is an array, unwinds it.
    std::pair<BSONElement, bool> _getFirstElementRooted(size_t rootIdx, const BSONObj& obj);
