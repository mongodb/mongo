...
void CardinalityEstimator::propagateLimit(const QuerySolutionNode* node, size_t limit) {
    ...
    const auto propagateToChildren = [this, &node](size_t childLimit) {
        for (const auto& child : node->children) {
            propagateLimit(&*child, childLimit);
        }
    };

    const auto applyLimitToSelf = [&limitCE, &outCE] {
        outCE = limitCE;
    };

    const StageType nodeType = node->getType();
    switch (nodeType) {
        case STAGE_COLLSCAN:
        case STAGE_VIRTUAL_SCAN:
        case STAGE_IXSCAN: {
            ...
            const auto selectivity = outCE / inCE;
            applyLimitToSelf();
            inCE = limitCE / selectivity;
            break;
        }
        case STAGE_FETCH: {
            ...
            const auto selectivity = outCE / inCE;
            // scale by selectivity.
            applyLimitToSelf();
            propagateToChildren(size_t((limitCE / selectivity).toDouble()));
            break;
        }
        case STAGE_AND_HASH:
        case STAGE_AND_SORTED: {
            // Propagate to children, but scale by relative cardinality estimates.
            // E.g.,
            //       * Limit 100
            //          * AND CE = 200
            //              * Child 1 CE = 500
            ...
            //      ...
            //      * _inputCard = 2000
            // (correlated events means the overall selectivity is not just the product).
            // Currently partial indexes aren't supported for CE, so all predicates here have the
            // same input cardinality _inputCard.
            // Limit 100 cannot be directly propagated to the children, it would significantly
            // underestimate the cardinality of both children.
            //
            // Instead, propagate a scaled limit for each child:
            //
            //       * Limit 100
            //          * AND CE = 100 (L 100 / CE 200 = 0.5)
            //              * Child 1 CE = (500 * 0.5 =) 250
            //              * Child 2 CE = (800 * 0.5 =) 400
            //      ...
            //      * _inputCard = 2000
            //
            // Equivalently,
            ...
            //       * Limit 100
            //          * AND CE = 100 (Orig 200)
            //              * Child 1 CE = 100 * (500 / 200) = 250
            //              * Child 2 CE = 100 * (800 / 200) = 400
            //      ...
            //      * _inputCard = 2000
            for (auto& child : node->children) {
                ...
                const auto selectivity = outCE / childCE;
                propagateLimit(child.get(), size_t((limitCE / selectivity).toDouble()));
            }
            applyLimitToSelf();
            break;
        }
        ...
        case STAGE_SORT_MERGE:
            applyLimitToSelf();
            // Passthrough to children; simple upper bound - no child of a disjunction will need to
            // product more values than the disjunction itself.
            propagateToChildren(limit);
            break;
        case STAGE_SORT_DEFAULT:
        case STAGE_SORT_SIMPLE: {
            // A later limit does not reduce the number of values a blocking sort needs to process
            // (and where possible the limit would be integrated into the sort stage anyway).
            break;
        }
        case STAGE_PROJECTION_DEFAULT:
        case STAGE_PROJECTION_COVERED:
        ...
        case STAGE_LIMIT:
            // Min of limits.
            if (limit < size_t(static_cast<const LimitNode*>(node)->limit)) {
                applyLimitToSelf();
                propagateToChildren(limit);
            }
            break;
        case STAGE_SKIP:
            // Add to limit.
            applyLimitToSelf();
            propagateToChildren(limit + size_t(static_cast<const SkipNode*>(node)->skip));
            break;
        case STAGE_SHARDING_FILTER:  // TODO SERVER-99073: Implement shard filter
        case STAGE_DISTINCT_SCAN:    // TODO SERVER-99075: Implement distinct scan
        case STAGE_TEXT_OR:
        ...
        case STAGE_UNPACK_TS_BUCKET:
            // These stages should never reach the cardinality estimator.
            tasserted(10064700,
                      str::stream{} << "Encountered " << nodeType
                                    << " stage in CardinalityEstimator limit propagation which "
