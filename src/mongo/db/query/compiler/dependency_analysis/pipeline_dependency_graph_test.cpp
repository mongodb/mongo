...
TEST_F(PipelineDependencyGraphTest, SubPipelineUsesSecondaryCollPathArrayness) {
    ...
    subExpCtx->setPathArraynessForNss(collBNss, secondaryPathArrayness);

    // Rebuild the graph so that the sub-pipeline graph is re-created with the updated PA.
    graph = std::make_unique<DependencyGraph>(pipeline->getSources(), canPathBeArray);
...
TEST_F(PipelineDependencyGraphTest, SubPipelineEmptyCanPathBeArray) {
    ...
    // Rebuild the graph so the sub-pipeline graph picks up the updated PA.
    graph = std::make_unique<DependencyGraph>(pipeline->getSources(), canPathBeArray);
...
TEST_F(PipelineDependencyGraphTest, SubPipelineNestedCanPathBeArray) {
    ...
    innerSubExpCtx->setPathArraynessForNss(collCNss, innerPathArrayness);

    // Rebuild the graph so nested sub-pipeline graphs pick up the updated PA.
    graph = std::make_unique<DependencyGraph>(pipeline->getSources(), canPathBeArray);
