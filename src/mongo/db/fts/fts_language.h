...
class FTSLanguage;
using StatusWithFTSLanguage = StatusWith<const FTSLanguage*>;
...
public:
     ...
     * version.  Returns an error Status if an invalid langName is passed.
     *
     ...
     */
    static StatusWithFTSLanguage make(StringData langName, TextIndexVersion textIndexVersion);

private:
    const std::string _canonicalName;
    const std::unique_ptr<FTSPhraseMatcher> _phraseMatcher;
};
