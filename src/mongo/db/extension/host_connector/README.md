...
- **Adapters** (in `host_connector/adapter/`): Implement C API structs (e.g. `MongoExtensionHostPortal`)
  by holding C++ implementations and forwarding C callbacks into them. They bridge the C API boundary
  for host-provided services.
...
`HostServicesAdapter`). Most C API handles live under `shared/handle/` because **both** the host and the C++ SDK need
the same `VTableAPI` wrappers when calling across the boundary, so `host_connector/handle` is knowingly small.
