 ...
 * Counts the total number of documents received.
 */
...
public:
     ...
     * Returns format: {$otherMetrics: {documentCount: N}}.
     */
    BSONObj serialize() const override {
        return BSON("documentCount" << _documentCount);
    }

    /**
     * Update metrics from binary data.
     * Expected format: one int64 value representing the document count delta.
     */
    void update(MongoExtensionByteView arguments) override {
        const int64_t* delta = reinterpret_cast<const int64_t*>(arguments.data);
        _documentCount += *delta;
    }
...
public:
    ...
    mongo::extension::ExtensionGetNextResult getNext(
        ...
        int64_t documentDelta = 1;
        metrics.update(MongoExtensionByteView{reinterpret_cast<const uint8_t*>(&documentDelta),
                                              sizeof(int64_t)});
