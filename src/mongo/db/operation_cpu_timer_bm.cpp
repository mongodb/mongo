...
public:
    Fixture() {
        ...
        _svcCtx->registerClientObserver(std::make_unique<LockerNoopClientObserver>());
        Client::initThread("test", _svcCtx.get(), nullptr);
    }
    ...
    auto makeOpCtx() {
        return _svcCtx->makeOperationContext(Client::getCurrent());
    }
