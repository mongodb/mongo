...
Status insertAuthzDocument(OperationContext* opCtx,
    ...
    // The sc is used to control serialization behavior executed on the request in
    // client.insert, and tenantIds should not be prefixed on the $db field.  Indicating that
    // the request received a tenantId from something other than a prefix, in this case the nss,
    // will prevent prefixing.
...
StatusWith<std::int64_t> updateAuthzDocuments(OperationContext* opCtx,
    ...
    // The sc is used to control serialization behavior executed on the request in
    // client.update, and tenantIds should not be prefixed on the $db field.  Indicating that
    // the request received a tenantId from something other than a prefix, in this case the nss,
    // will prevent prefixing.
    const auto vts = auth::ValidatedTenancyScope::get(opCtx);
    auto sc = SerializationContext::stateCommandRequest(
...
StatusWith<std::int64_t> removeAuthzDocuments(OperationContext* opCtx,
    ...
    // The sc is used to control serialization behavior executed on the request in
    // client.remove, and tenantIds should not be prefixed on the $db field.  Indicating that
    // the request received a tenantId from something other than a prefix, in this case the nss,
    // will prevent prefixing.
    const auto vts = auth::ValidatedTenancyScope::get(opCtx);
    auto sc = SerializationContext::stateCommandRequest(
 ...
 *   "asUserFragment" Render results as a partial user document as-if a user existed which
 * possessed these roles. This format may change over time with changes to the auth schema.
 */
...
Status queryAuthzDocument(OperationContext* opCtx,
    ...
    // The sc is used to control serialization behavior executed on the request in client.find,
    // and tenantIds should not be prefixed on the $db field.  Indicating that the request
    // received a tenantId from something other than a prefix, in this case the nss, will
    // prevent prefixing.
    const auto vts = auth::ValidatedTenancyScope::get(opCtx);
    auto sc = SerializationContext::stateCommandRequest(
        nss.tenantId() != boost::none, vts != boost::none && vts->isFromAtlasProxy());
