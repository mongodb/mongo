...
private:
    std::string _hashCollection(OperationContext* opCtx, const CollectionPtr& collection) {
        ...
        // scan order is preserved across nodes.
        bool replicatedRids = collection->areRecordIdsReplicated();
        ...
        } else if (collection->isCapped() || collection->isClustered() || replicatedRids) {
        ...
        try {
            ...
            const bool explicitlyHashRecordIds = replicatedRids && !collection->isClustered();
