...
ShardType::ShardType(std::string name, std::string host, std::vector<std::string> tags)
    : _handle(ShardHandle(name)), _host(std::move(host)), _tags(std::move(tags)) {}

StatusWith<ShardType> ShardType::fromBSON(const BSONObj& source) {
    ShardType shard;

    {
        const auto swShardHandle = [&]() -> StatusWith<ShardHandle> {
            std::string shardName;
            Status status = bsonExtractStringField(source, name.name(), &shardName);
            if (!status.isOK())
                return status;

            BSONElement shardRefElem = source[kShardRefFieldName];
            if (shardRefElem.eoo()) {
                return ShardHandle(shardName);
            } else if (shardRefElem.type() == BSONType::string) {
                return ShardHandle(shardName,
                                   ShardRef(std::string(shardRefElem.valueStringData())));
            } else if (shardRefElem.type() == BSONType::binData &&
                       shardRefElem.binDataType() == BinDataType::newUUID) {
                auto uuid = UUID::parse(shardRefElem);
                if (!uuid.isOK())
                    return uuid.getStatus();
                return ShardHandle(shardName, ShardRef(uuid.getValue()));
            } else {
                return Status(ErrorCodes::TypeMismatch,
                              str::stream()
                                  << "\"" << kShardRefFieldName << "\" must be a string or UUID");
            }
        }();
        if (!swShardHandle.isOK())
            return swShardHandle.getStatus();
        shard._handle = swShardHandle.getValue();
    }
...
BSONObj ShardType::toBSON() const {
    ...
    if (_handle)
        builder.append(name(), _handle->name());
    if (_host)
    ...
    // TODO (SERVER-126210): Persist the shard ref once generated by addShard
...
void ShardType::setName(const std::string& name) {
    if (_handle) {
        _handle = ShardHandle(name, _handle->ref());
    } else {
        _handle = ShardHandle(name);
    }
}
...
const ShardRef& ShardType::getShardRef() const {
    invariant(_handle);
    return _handle->ref();
}
