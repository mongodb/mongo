...
StatusWith<ShardIdentityType> ShardIdentityType::fromShardIdentityDocument(const BSONObj& source) {
    ...
    try {
        ...
        if (!shardIdentity.getShardRef()) {
            shardIdentity.setShardRef(ShardRef(std::string(shardIdentity.getShardName())));
        }
...
Status ShardIdentityType::validate(bool fassert) const {
    ...
    if (gFeatureFlagAllMongodsAreSharded.isEnabledAndIgnoreFCVUnsafe() && fassert) {
        ...
        const bool isShardRefConfigServer = getShardRef() && getShardRef()->isString() &&
            getShardRef()->getString() == ShardId::kConfigServerId.toString();
        if (isConfigServer) {
            if (getShardName() != ShardId::kConfigServerId) {
                return {ErrorCodes::UnsupportedFormat,
                        str::stream()
                            << "Invalid shard identity document: the shard name for a config "
                               "server cannot be \""
                            << getShardName() << "\""};
            }
            if (getShardRef() && !isShardRefConfigServer) {
                return {ErrorCodes::UnsupportedFormat,
                        str::stream()
                            << "Invalid shard identity document: the shard ref for a config "
                               "server must be \""
            ...
            if (getShardName() == ShardId::kConfigServerId) {
                return {ErrorCodes::UnsupportedFormat,
                        str::stream()
                            << "Invalid shard identity document: the shard name for a shard "
                               "server cannot be \""
                            << ShardId::kConfigServerId.toString() << "\""};
            }
            if (getShardRef() && isShardRefConfigServer) {
                return {ErrorCodes::UnsupportedFormat,
                        str::stream()
                            << "Invalid shard identity document: the shard ref for a shard "
                               "server cannot be \""
                            << ShardId::kConfigServerId.toString() << "\""};
            }
        }
...
BSONObj ShardIdentityType::toShardIdentityDocument() const {
    ...
    builder.appendElements(ShardIdentity::toBSON().removeField(ShardIdentity::kShardRefFieldName));
    return builder.obj();
}
