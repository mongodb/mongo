...
/**
 * Config-server command that commits a chunk merge to the global catalog and returns
 * 'changedChunks': the single chunk produced by merging the requested contiguous chunks.
 * Participants use this list to update their durable shard catalog and in-memory routing metadata.
 *
 * The commit is idempotent: a retry that finds the merge already applied rebuilds and returns the
 ...
 */
class ConfigSvrCommitMergeChunksCommand : public TypedCommand<ConfigSvrCommitMergeChunksCommand> {
public:
    using Request = ConfigSvrCommitMergeChunksRequest;
    using Response = ConfigSvrCommitMergeChunksResponse;
    ...
    public:
        ...
        Response typedRun(OperationContext* opCtx) {
            uassert(ErrorCodes::IllegalOperation,
                    "_configsvrCommitMergeChunks can only be run on config servers",
                    serverGlobalParams.clusterRole.has(ClusterRole::ConfigServer));

            // The command is always issued as a retryable write, so a session id must be present.
            uassert(ErrorCodes::IllegalOperation,
                    "_configsvrCommitMergeChunks must be run with a session id",
            ...
            std::vector<ChunkType> changedChunks;
            {
                ...
                auto newClient =
                    opCtx->getServiceContext()->getService()->makeClient("CommitMergeChunks");
                ...
                changedChunks =
                    uassertStatusOK(ShardingCatalogManager::get(newOpCtxPtr.get())
                                        ->commitMerge(newOpCtxPtr.get(),
                                                      nss,
                                                      request().getShardVersionPreMerge(),
                                                      request().getChunkRange(),
                                                      ShardId(request().getShard().toString())));
            }
            ...
            client.update(NamespaceString::kServerConfigurationNamespace,
                          BSON("_id" << "commitMergeChunksStats"),
...
MONGO_REGISTER_COMMAND(ConfigSvrCommitMergeChunksCommand).forShard();
