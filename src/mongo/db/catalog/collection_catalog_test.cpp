...
public:
    ...
    void setUp() override {
        ...
        catalog.registerCollection(opCtx.get(), collection);
    }
...
public:
    ...
    void checkCollections(const TenantDatabaseName& tenantDbName) {
        ...
        auto orderedIt = collsIterator(tenantDbName.dbName());
        auto catalogRange = catalog.range(tenantDbName);
        auto catalogIt = catalogRange.begin();
        for (; catalogIt != catalogRange.end() &&
...
public:
    void setUp() {
        ...
        int numEntries = 0;
        for (auto&& coll : catalog.range(TenantDatabaseName(boost::none, "resourceDb"))) {
    ...
    void tearDown() {
        ...
        for ([[maybe_unused]] auto&& coll :
             catalog.range(TenantDatabaseName(boost::none, "resourceDb"))) {
            numEntries++;
        }
...
TEST_F(CollectionCatalogIterationTest, GetUUIDWontRepositionEvenIfEntryIsDropped) {
    auto range = catalog.range(TenantDatabaseName(boost::none, "bar"));
    auto it = range.begin();
