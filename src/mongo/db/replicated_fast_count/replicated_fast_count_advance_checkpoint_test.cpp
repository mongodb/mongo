...
TEST_F(ReplicatedFastCountAdvanceCheckpointTest, TrackTwoUserCollections) {
    ...
    const auto entryCollA = readSizeCount(collA.uuid);
    ...
    const auto entryCollB = readSizeCount(collB.uuid);
...
TEST_F(ReplicatedFastCountAdvanceCheckpointTest,
    ...
    const SizeCountStore::Entry expectedCollAEntry{
        .timestamp = Timestamp(1, 1), .size = 100, .count = 3};
    const auto collAEntry = readSizeCount(collA.uuid);
    ...
    const auto collBEntry = readSizeCount(collB.uuid);
...
TEST_F(ReplicatedFastCountAdvanceCheckpointTest, DropWithoutExistingEntryCountsOnlyOplog) {
    // No pre-existing entry for collA in the size count store.
    EXPECT_FALSE(readSizeCount(collA.uuid).has_value());
