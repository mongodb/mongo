...
Milliseconds doubleToMillis(double t) {
    return duration_cast<Milliseconds>(duration_cast<stdx::chrono::milliseconds>(
        stdx::chrono::duration<double, std::ratio<1>>(t)));
}
}  // namespace
...
struct RateLimiter::RateLimiterPrivate {
    RateLimiterPrivate(double r, double b, int64_t m, std::string n)
        ...
        // `stdx::chrono::steady_clock`, whose epoch is unspecified but is usually the boot time
    ...
    AtomicWord<int64_t> maxQueueDepth;
    AtomicWord<int64_t> queued;
...
Status RateLimiter::acquireToken(OperationContext* opCtx) {
    _impl->stats.attemptedAdmissions.increment();
    ...
    if (auto napTime = doubleToMillis(waitForTokenSecs); napTime > Milliseconds{0}) {
        ...
        if (auto status = _impl->enqueue(); !status.isOK()) {
            ...
            _impl->stats.rejectedAdmissions.increment();
            return status;
        }
        _impl->stats.addedToQueue.increment();
        ON_BLOCK_EXIT([&] {
            _impl->stats.removedFromQueue.increment();
        ...
        try {
            ...
            _impl->stats.interruptedInQueue.increment();
    ...
    _impl->stats.successfulAdmissions.increment();
...
void RateLimiter::recordExemption() {
    _impl->stats.exemptedAdmissions.increment();
}
