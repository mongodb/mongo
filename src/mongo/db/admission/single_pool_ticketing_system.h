...
class SinglePoolTicketingSystem : public TicketingSystem {
public:
    SinglePoolTicketingSystem(std::unique_ptr<TicketHolder> readTicketHolder,
    ...
    void setMaxQueueDepth(AdmissionContext::Priority p, Operation o, int32_t depth) override;

    void setConcurrentTransactions(OperationContext* opCtx,
                                   AdmissionContext::Priority p,
                                   Operation o,
                                   int32_t transactions) override;

    void appendStats(BSONObjBuilder& b) const override;

    int32_t numOfTicketsUsed() const override;

    void incrementDelinquencyStats(OperationContext* opCtx) override;

    boost::optional<Ticket> waitForTicketUntil(OperationContext* opCtx,
                                               Operation o,
                                               Date_t until) const override;
