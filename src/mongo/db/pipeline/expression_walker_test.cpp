...
TEST_F(ExpressionWalkerTest, NullTreeWalkSucceeds) {
    struct {
        void postVisit(boost::intrusive_ptr<Expression>& expression) {}
    } nothingWalker;
...
TEST_F(ExpressionWalkerTest, PrintWalkReflectsMutation) {
    struct {
        void preVisit(boost::intrusive_ptr<Expression>& expression) {
        ...
        void inVisit(boost::intrusive_ptr<Expression>& expression) {
            string += ", ";
        }
        void postVisit(boost::intrusive_ptr<Expression>& expression) {
            if (typeid(*expression) == typeid(ExpressionConcat))
