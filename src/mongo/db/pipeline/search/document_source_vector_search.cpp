...
DocumentSource::GetNextResult DocumentSourceVectorSearch::doGetNext() {
    ...
    if (!_cursor) {
        _cursor = search_helpers::establishVectorSearchCursor(pExpCtx, _stageSpec, _taskExecutor);
    }
...
std::list<intrusive_ptr<DocumentSource>> DocumentSourceVectorSearch::createFromBson(
    ...
    if (view) {
        ...
        // must not be supplied by external clients. The isHybridSearch() and
        // isExpectedToExecuteQueries() guards mirror validateInternalSearchFieldsNotSetByUser:
        // $rankFusion/$scoreFusion desugar legitimately re-injects the view field when reparsing
        // serialized inner pipelines, and those reparses must not false-positive; same with
        // query-stats reparse.
