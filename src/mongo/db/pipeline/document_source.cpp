...
/**
 * Returns 'true' if the given stage is an internal change stream stage that can appear in a
 * router (mongoS) pipeline, or 'false' otherwise.
 */
...
bool DocumentSource::pushMatchBefore(Pipeline::SourceContainer::iterator itr,
    ...
    if (!nextStageAsMatch || nextStageAsMatch->isTextQuery()) {
        ...
        // predicate because, in that scenario, $match is already required to be the first stage
        // in the pipeline.
        return false;
    }
    ...
    // TODO SERVER-55492: Remove the following workaround when there are rename checks for
    ...
    // At this point we know that at least part of the $match expression can be moved
    // ahead of 'this'. So, we erase the original $match and move that renameable part
    // ahead of 'this' stage.
    container->erase(std::next(itr));
    container->insert(itr, std::move(renameableMatchPart));

    // If 'nonRenameableMatchPart' is not null, the 'renameableMatchPart' of the $match
    // expression was only one component of the original $match. So, we need to create a
    // new $match stage for the remaining 'nonRenameableMatchPart' and insert it after
    // 'this' - effectively keeping it in its original position in the pipeline.
