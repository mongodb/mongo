...
intrusive_ptr<DocumentSource> DocumentSourceSample::createFromBson(
    ...
    uassert(28739, "the $sample stage specification must be an object", specElem.type() == Object);
    ...
    for (auto&& elem : specElem.embeddedObject()) {
        ...
        if (fieldName == "size") {
            uassert(28740, "size argument to $sample must be a number", elem.isNumber());
            uassert(28741, "size argument to $sample must not be negative", elem.numberLong() >= 0);
            sample->_size = elem.numberLong();
            sizeSpecified = true;
        } else {
            uasserted(28742, str::stream() << "unrecognized option to $sample: " << fieldName);
        }
    }
    uassert(28743, "$sample stage must specify a size", sizeSpecified);
