...
std::vector<ScopedSetShardRole> createScopedShardRoles(
    ...
    for (const auto& nss : nssList) {
        ...
        // For untracked UNSHARDED collections, the collection will only be potentially considered
        // local if this shard is the dbPrimary shard.
        //
        ...
        const bool isDbPrimaryShard = nssCri->second.cm.dbPrimary() == myShardId;
        auto dbVersion = !isTracked && isDbPrimaryShard
            ? boost::optional<DatabaseVersion>(nssCri->second.cm.dbVersion())
            : boost::none;
        ...
        try {
            ...
            scopedDss->assertMatchingDbVersion(opCtx, nss.dbName());
