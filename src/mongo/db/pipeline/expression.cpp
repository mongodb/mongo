...
boost::optional<TimeZone> makeTimeZone(const TimeZoneDatabase* tzdb,
    ...
    if (!timeZone) {
        return merizo::TimeZoneDatabase::utcZone();
    }
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        if (val.numeric()) {
            ...
            if (productType == NumberDecimal) {
                ...
                if (merizoSignedMultiplyOverflow64(longProduct, val.coerceToLong(), &longProduct)) {
    ...
    ConversionTable() {
        ...
        table[BSONType::String][BSONType::Date] = [](
            ...
            return Value(expCtx->timeZoneDatabase->fromString(inputValue.getStringData(),
                                                              merizo::TimeZoneDatabase::utcZone()));
        };
...
Value ExpressionRegexFindAll::evaluate(const Document& root) const {
    ...
    do {
        ...
        uassert(51151,
                ...
                totalDocSize <= merizo::BufferMaxSize);
