...
public:
    ...
    void run() {
        ...
        createLegacyUnwind();
        assertResultsMatch(expectedResultSet(false));
        ...
        createUnwind(true);
        assertResultsMatch(expectedResultSet(true));
    }
...
private:
     ...
     * created with.
     */
    void checkBsonRepresentation(bool preserveNullAndEmptyArrays) {
        vector<Value> arr;
        _unwind->serializeToArray(arr);
        BSONObj generatedSpec = Value(arr[0]).getDocument().toBson();
        ASSERT_EQUALS(expectedSerialization(preserveNullAndEmptyArrays), generatedSpec);
    }

    BSONObj expectedSerialization(bool preserveNullAndEmptyArrays) const {
        if (preserveNullAndEmptyArrays) {
            return BSON("$unwind" << BSON("path" << unwindFieldPath()
    ...
    BSONObj expectedResultSet(bool preserveNullAndEmptyArrays) const {
        auto expectedResultsString = preserveNullAndEmptyArrays ? expectedPreservedResultSetString()
                                                                : expectedResultSetString();
        // fromjson() cannot parse an array, so place the array within an object.
...
class SeveralDocuments : public CheckResultsBase {
    ...
    string expectedPreservedResultSetString() const {
        return "[{_id: 0, a: 1}, {_id: 0, a: 2}, {_id: 0, a: 3},"
               " {_id: 1},"
               " {_id: 2},"
               " {_id: 3, a: 10}, {_id: 3, a: 20},"
...
public:
    void run() {
        auto unwind = DocumentSourceUnwind::create(ctx(), "x.y.z", false);
        DepsTracker dependencies;
