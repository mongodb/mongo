...
TEST_F(MapReduceFixture, GlobalThisIsSharedBetweenExpressionFunctionAndInternalJsEmit) {
...
TEST_F(MapReduceFixture, ExpressionInternalJsEmitStaleEmitCalledFromExpressionFunctionThrows) {
    auto expCtx = getExpCtxRaw();
    auto emitBson =
        BSON("expr" << BSON("this" << "$$ROOT"
                                   << "eval"
