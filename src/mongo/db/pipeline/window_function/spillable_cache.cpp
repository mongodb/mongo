...
void SpillableCache::writeBatchToDisk(std::vector<Record>& records) {
    // A single document can exceed 'kMaxWriteSize' on its own -- a ~16MB user document plus
    // metadata such as the '$sortKey' added by 'sortBy' may reach BSONObjMaxInternalSize -- so the
    // batching in spillToDisk() can reach a flush with nothing accumulated. Passing an empty record
    // vector to RecordStore::insertRecords trips a fatal invariant, so drop the write here, at the
    // single point where every batch reaches the record store.
    if (records.empty()) {
        return;
    }
