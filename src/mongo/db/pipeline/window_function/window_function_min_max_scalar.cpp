...
Value WindowFunctionMinMaxScalar::computeValue(const Value& currentValue,
    ...
    Value minMaxUnscaled = uassertStatusOK(exec::expression::evaluateDivide(
        uassertStatusOK(exec::expression::evaluateSubtract(currentValue, windowMin)),
        uassertStatusOK(exec::expression::evaluateSubtract(windowMax, windowMin))));

    // Scale the return value between the configured bounds.
    return uassertStatusOK(exec::expression::evaluateAdd(
        uassertStatusOK(exec::expression::evaluateMultiply(
            minMaxUnscaled, uassertStatusOK(exec::expression::evaluateSubtract(sMax, sMin)))),
        sMin));
}
