...
Status ReplicationCoordinatorImpl::processReplSetInitiate(OperationContext* opCtx,
    ...
    // In the EMRC=true case, we need to advance the commit point to make sure that we can recover
    // if we happen to roll back our first entries after replSetInitiate.
    if (serverGlobalParams.enableMajorityReadConcern) {
        stdx::unique_lock<Latch> lk(_mutex);
        _shouldSetStableTimestamp = true;
