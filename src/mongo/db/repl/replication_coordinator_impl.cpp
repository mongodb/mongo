...
void ReplicationCoordinatorImpl::_updateSlaveInfoFromConfig_inlock() {
    ...
    for (int i = 0; i < _rsConfig.getNumMembers(); ++i) {
        ...
        const HostAndPort& memberHostAndPort = memberConfig.getInternalHostAndPort();
...
ReplicationCoordinatorImpl::_setCurrentRSConfig_inlock(const ReplicaSetConfig& newConfig,
    ...
    _selfIndex = myIndex;
    if (_selfIndex >= 0) {
        log() << "This node is " << _rsConfig.getMemberAt(_selfIndex).getInternalHostAndPort()
              << " in the config";
    } else {
...
std::vector<HostAndPort> ReplicationCoordinatorImpl::getOtherNodesInReplSet() const {
    ...
    for (int i = 0; i < _rsConfig.getNumMembers(); ++i) {
        ...
        nodes.push_back(_rsConfig.getMemberAt(i).getInternalHostAndPort());
    }
    return nodes;
}
