...
void TopologyCoordinator::fillHelloForReplSet(std::shared_ptr<HelloResponse> response,
 ...
 bool anyHealthySecondary = false;
 for (int i = 0; i < _rsConfig.getNumMembers(); i++) {
     ...
     if (memberCfg.isHidden()) {
         continue;  // Only consider non-hidden secondaries for health check
     }
     const auto& memberData = _memberData.at(i);
     if (memberData.getState().secondary()) {
         if (memberData.up()) {
 ...
 for (const auto& member : _rsConfig.members()) {
     ...
     if (member.isHidden() && anyHealthySecondary) {
 ...
 if (selfConfig.isHidden() && anyHealthySecondary) {
