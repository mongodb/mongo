...
Status InitialSyncer::startup(OperationContext* opCtx,
    ...
    _attemptExec = std::make_unique<executor::ScopedTaskExecutor>(_exec);
    _clonerAttemptExec = std::make_unique<executor::ScopedTaskExecutor>(_clonerExec);
...
void InitialSyncer::_fcvFetcherCallback(const StatusWith<Fetcher::QueryResponse>& result,
    ...
    _oplogFetcher =
        _createOplogFetcherFn(*_attemptExec,
                              beginFetchingOpTime,
                              _syncSource,
                              config,
