                ...
                do {
                    if (operations.type() != Array) {
                        std::string message = str::stream()
                            ...
                            << redact(operations);
                        LOGV2_FATAL_CONTINUE(21734, "{message}", "message"_attr = message);
                        return Status(ErrorCodes::UnrecoverableRollbackError, message);
                    }
                    for (const auto& subopElement : operations.Array()) {
                        if (subopElement.type() != Object) {
                            std::string message = str::stream()
                                ...
                                << redact(subopElement);
                            LOGV2_FATAL_CONTINUE(21735, "{message}", "message"_attr = message);
                            return Status(ErrorCodes::UnrecoverableRollbackError, message);
                        }
...
void dropIndex(OperationContext* opCtx,
    ...
    if (!indexDescriptor) {
        LOGV2_WARNING(21725,
                      "Rollback failed to drop index {indexName} in {ns}: index not found.",
                      "indexName"_attr = indexName,
                      "ns"_attr = nss.toString());
        return;
    }
    ...
    if (entry->isReady(opCtx)) {
        ...
        if (!status.isOK()) {
            LOGV2_ERROR(21738,
                        "Rollback failed to drop index {indexName} in {ns}: {status}",
                        "indexName"_attr = indexName,
                        "ns"_attr = nss.toString(),
        ...
        if (!status.isOK()) {
            LOGV2_ERROR(21739,
                        "Rollback failed to drop unfinished index {indexName} in {ns}: {status}",
                        "indexName"_attr = indexName,
                        "ns"_attr = nss.toString(),
                        "status"_attr = redact(status));
        }
...
void rollbackCreateIndexes(OperationContext* opCtx, UUID uuid, std::set<std::string> indexNames) {
    ...
    for (auto itIndex = indexNames.begin(); itIndex != indexNames.end(); itIndex++) {
        ...
        LOGV2(21672,
              "Dropping index in rollback for collection: {ns}, UUID: {uuid}, index: {indexName}",
              "ns"_attr = *nss,
              "uuid"_attr = uuid,
              "indexName"_attr = indexName);
        ...
        LOGV2_DEBUG(
            21673,
            1,
            "Dropped index in rollback for collection: {ns}, UUID: {uuid}, index: {indexName}",
            "ns"_attr = *nss,
            "uuid"_attr = uuid,
            "indexName"_attr = indexName);
    }
...
void rollbackDropIndexes(OperationContext* opCtx,
    ...
    for (auto itIndex = indexNames.begin(); itIndex != indexNames.end(); itIndex++) {
        ...
        LOGV2(21675,
              "Creating index in rollback for collection: {ns}, UUID: {uuid}, index: {indexName}",
              "ns"_attr = *nss,
              "uuid"_attr = uuid,
              "indexName"_attr = indexName);
        ...
        LOGV2_DEBUG(
            21676,
            1,
            "Created index in rollback for collection: {ns}, UUID: {uuid}, index: {indexName}",
            "ns"_attr = *nss,
            "uuid"_attr = uuid,
            "indexName"_attr = indexName);
    }
...
void renameOutOfTheWay(OperationContext* opCtx, RenameCollectionInfo info, Database* db) {
    ...
    LOGV2_DEBUG(21678,
                ...
                "with UUID {collection_uuid} out of the way to {tempNs}",
                "renameFrom"_attr = info.renameFrom,
                "renameTo"_attr = info.renameTo,
                "collection_uuid"_attr = collection->uuid(),
                "tempNs"_attr = tempNss);
    ...
    if (!renameStatus.isOK()) {
        LOGV2_FATAL_CONTINUE(21744,
                             "Unable to rename collection {renameTo} out of the way to {tempNs}",
                             "renameTo"_attr = info.renameTo,
                             "tempNs"_attr = tempNss);
...
void rollback_internal::syncFixUp(OperationContext* opCtx,
    ...
    LOGV2(21690,
          "Finished refetching documents. Total size of documents refetched: {size}",
          "size"_attr = goodVersions.size());
    ...
    for (auto uuid : fixUpInfo.collectionsToDrop) {
        ...
        if (!nss) {
            ...
            LOGV2(21697,
                  "Dropping collection: {ns}, UUID: {uuid}",
                  "ns"_attr = *nss,
                  "uuid"_attr = uuid);
            ...
            if (db) {
                ...
                LOGV2_DEBUG(21698,
                            1,
                            "Dropped collection: {ns}, UUID: {uuid}",
                            "ns"_attr = *nss,
                            "uuid"_attr = uuid);
            }
        }
    ...
    if (!fixUpInfo.collectionsToResyncMetadata.empty()) {
        ...
        for (auto uuid : fixUpInfo.collectionsToResyncMetadata) {
            ...
            LOGV2(21702,
                  "Resyncing collection metadata for collection: {ns}, UUID: {uuid}",
                  "ns"_attr = *nss,
                  "uuid"_attr = uuid);
            ...
            if (!infoResult.isOK()) {
                ...
                LOGV2(21703,
                      "{ns} not found on remote host, so we do not roll back collmod "
            ...
            LOGV2_DEBUG(
                21704,
                1,
                "Resynced collection metadata for collection: {ns}, UUID: {uuid}, with: {info}, "
                "to: {catalogId}",
                "ns"_attr = *nss,
                "uuid"_attr = uuid,
    ...
    for (const auto& nsAndGoodVersionsByDocID : goodVersions) {
        ...
        for (const auto& idAndDoc : goodVersionsByDocID) {
            ...
            try {
                        ...
                        if (collection->isCapped()) {
                            ...
                            try {
                                ...
                                if (clock->now() - findOneStart > Milliseconds(200))
                                    LOGV2_WARNING(21726,
                                                  "Roll back slow no _id index for {ns} perhaps?",
                                                  "ns"_attr = nss->ns());
                ...
                LOGV2(21713,
                      "Exception in rollback ns:{ns} {pattern} {e} ndeletes:{deletes}",
                      "ns"_attr = nss->ns(),
