...
public:
    ...
    void clearOplogApplierProgress(NamespaceString progressNss) {
        auto opCtx = cc().makeOperationContext();
    ...
    std::shared_ptr<TenantOplogApplier> makeTenantMigrationOplogApplier(
        ...
        auto tenantOplogApplier =
            std::make_shared<TenantOplogApplier>(_migrationUuid,
                                                 ...
                                                 OpTime(Timestamp(1, 0), 0));
        if (isResuming) {
            tenantOplogApplier->recoverState();
        }
        return tenantOplogApplier;
    };

    std::shared_ptr<TenantOplogApplier> makeShardMergeOplogApplier(ThreadPool* writerPool) {
...
TEST_F(TenantOplogApplierTest, StoresOplogApplierProgress) {
    ...
    auto opCtx = cc().makeOperationContext();
    auto applier = makeTenantMigrationOplogApplier(writerPool.get(), progressNss);
    ASSERT_OK(applier->startup());
...
TEST_F(TenantOplogApplierTest, ResumesOplogApplierProgress) {
    ...
    applier = makeTenantMigrationOplogApplier(writerPool.get(), progressNss, true);
    ASSERT_OK(applier->startup());
...
TEST_F(TenantOplogApplierTest,
       ResumeOplogApplierDoesNotReApplyPreviouslyAppliedRetryableWriteStatements) {
    {
        auto opCtx = cc().makeOperationContext();
        createCollectionWithUuid(opCtx.get(), NamespaceString::kSessionTransactionsTableNamespace);
        DBDirectClient client(opCtx.get());
...
TEST_F(TenantOplogApplierTest, ResumeOplogApplierDoesNotReApplyPreviouslyAppliedRetryableWrites) {
    ...
    sessionInfo.setSessionId(sessionId);
    sessionInfo.setTxnNumber(0);
    auto retryableWrite1 = makeOplogEntry({Timestamp(2, 0), 1},
                                          OpTypeEnum::kInsert,
                                          nss,
                                          BSON("_id" << 1),
                                          boost::none,
    ...
    auto retryableWrite2 = makeOplogEntry({Timestamp(3, 0), 1},
                                          OpTypeEnum::kInsert,
                                          nss,
                                          BSON("_id" << 2),
                                          boost::none,
    ...
    pushOps({retryableWrite1, retryableWrite2});
    auto opAppliedFuture = applier->getNotificationForOpTime(retryableWrite2.getOpTime());
    ASSERT_OK(opAppliedFuture.getNoThrow().getStatus());

    ASSERT_EQ(applier->getNumOpsApplied(), 2);

    // The retryable write noop entries should have been logged.
    ASSERT_EQ(getNoopOplogEntryCount(), 2);
    ...
    // Create a new TenantOplogApplier with resume enabled. The retryable writes will be re-applied.
    applier = makeTenantMigrationOplogApplier(writerPool.get(), progressNss, true);
    ASSERT_OK(applier->startup());

    pushOps({retryableWrite1, retryableWrite2});
    opAppliedFuture = applier->getNotificationForOpTime(retryableWrite2.getOpTime());
    ASSERT_OK(opAppliedFuture.getNoThrow().getStatus());

    ASSERT_EQ(applier->getNumOpsApplied(), 2);

    // The active transaction state should have been refreshed from storage, the retryable writes
    // will have transaction numbers smaller than the active transactionn number, so the noop
    // entries are skipped and the count should be the same.
    ASSERT_EQ(getNoopOplogEntryCount(), 2);
...
TEST_F(TenantOplogApplierTest, ResumeOplogApplierDoesNotReApplyPreviouslyAppliedTransactions) {
    {
        auto opCtx = cc().makeOperationContext();
        createCollectionWithUuid(opCtx.get(), NamespaceString::kSessionTransactionsTableNamespace);
        DBDirectClient client(opCtx.get());
    ...
    auto lsid = [&]() {
        auto opCtx = cc().makeOperationContext();
        return makeLogicalSessionId(opCtx.get());
    }();

    int opCounter = 0;
    auto makeCommitOp = [&]() {
        ON_BLOCK_EXIT([&] { opCounter++; });
        auto opCtx = cc().makeOperationContext();
        TxnNumber txnNum(opCounter);

        const BSONObj doc = BSON("_id" << opCounter << "data" << opCounter);

        return makeCommandOplogEntryWithSessionInfoAndStmtIds(
            OpTime(Timestamp(opCounter, opCounter), 1LL),
            nss,
            BSON("applyOps" << BSON_ARRAY(makeInsertApplyOpsEntry(nss, UUID::gen(), doc))),
            lsid,
            txnNum,
            {StmtId(0)},
            OpTime());
    ...
    auto commitOp1 = makeCommitOp();
    auto commitOp2 = makeCommitOp();
    ...
    pushOps({commitOp1, commitOp2});
    auto opAppliedFuture = applier->getNotificationForOpTime(commitOp2.getOpTime());
    ASSERT_OK(opAppliedFuture.getNoThrow().getStatus());

    ASSERT_EQ(applier->getNumOpsApplied(), 2);

    // The transaction noop entries should have been logged.
    ASSERT_EQ(getNoopOplogEntryCount(), 2);
    ...
    // Create a new TenantOplogApplier with resume enabled. The transactions will be re-applied.
    applier = makeTenantMigrationOplogApplier(writerPool.get(), progressNss, true);
    ASSERT_OK(applier->startup());

    pushOps({commitOp1, commitOp2});
    opAppliedFuture = applier->getNotificationForOpTime(commitOp2.getOpTime());
    ASSERT_OK(opAppliedFuture.getNoThrow().getStatus());

    ASSERT_EQ(applier->getNumOpsApplied(), 2);

    // The active transaction state should have been refreshed from storage, the transactions will
    // have transaction numbers smaller than or equal to the active transactionn number, so the noop
    // entries are skipped and the count should be the same.
    ASSERT_EQ(getNoopOplogEntryCount(), 2);
