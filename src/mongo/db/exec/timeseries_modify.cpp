...
void TimeseriesModifyStage::_checkRestrictionsOnUpdatingShardKeyAreNotViolated(
    ...
    if (!updateOneWithoutShardKey) {
        ...
        // If the query is a simple equality match on _id, then '_originalPredicate' will be null.
        // But if we are here, we already know that the shard key is not _id, since we have an
        // assertion earlier for requests that try to modify the immutable _id field. So it is safe
        // to uassert if '_originalPredicate' is null OR if the query does not include equality
        // matches on all shard key fields.
        ...
        uassert(7717803,
                ...
                "query: pred = {}, shardKeyPaths = {}"_format(
                    _originalPredicate ? _originalPredicate->serialize().toString() : std::string{},
