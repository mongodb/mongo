...
class SectionNamesWithDurationSuffix {
private:
    static constexpr auto _suffix = Dur::mongoUnitSuffix();

    template <size_t idx>
    static constexpr auto _buf = [] {
        auto&& name = sectionNames[idx];
        std::array<char, name.size() + _suffix.size()> buf;
        auto pos = buf.begin();
        pos = std::copy(name.begin(), name.end(), pos);
        pos = std::copy(_suffix.begin(), _suffix.end(), pos);
        return buf;
    }();

public:
    static constexpr auto value = []<size_t... Is>(std::index_sequence<Is...>) {
        return std::array{StringData{_buf<Is>.data(), _buf<Is>.size()}...};
