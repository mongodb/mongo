...
#include "mongo/db/exec/bucket_level_id_predicate_generator.h"
        ...
        switch (cmpExpr->matchType()) {
            case MatchExpression::MatchType::INTERNAL_EXPR_GT:
                if (path == params.controlMaxTimePath) {
                    childVector->push_back(
                        makePredicate(MatchExprPredicate<GTMatchExpression, Value>(
                            timeseries::kBucketIdFieldName,
                            constructObjectIdValue<GTMatchExpression>(
                                data, params.bucketMaxSpanSeconds))));
                    changesMade = true;
                }
                break;
            case MatchExpression::MatchType::INTERNAL_EXPR_GTE:
                if (path == params.controlMaxTimePath) {
                    childVector->push_back(
                        makePredicate(MatchExprPredicate<GTEMatchExpression, Value>(
                            timeseries::kBucketIdFieldName,
            ...
            case MatchExpression::MatchType::INTERNAL_EXPR_LT:
                if (path == params.controlMinTimePath) {
                    childVector->push_back(
                        makePredicate(MatchExprPredicate<LTMatchExpression, Value>(
                            timeseries::kBucketIdFieldName,
            ...
            case MatchExpression::MatchType::INTERNAL_EXPR_LTE:
                if (path == params.controlMinTimePath) {
                    childVector->push_back(
                        makePredicate(MatchExprPredicate<LTEMatchExpression, Value>(
                            timeseries::kBucketIdFieldName,
