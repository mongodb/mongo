    ...
    for (;;) {
        ...
        for (auto& elem : *_response) {
            ...
            if (size_t pos = _metadataNames.findPos(elemName); pos != StringListSet::npos) {
                ...
                _metadataAccessors[pos].reset(value::TagValueView{tag, val});
            }
            if (_idSlot && elemName == "_id") {
                auto [tag, val] = bson::convertToView(elem);
                _idAccessor.reset(value::TagValueView{tag, val});
            }
        }
            ...
            if (_isStoredSource) {
                ...
                for (auto& elem : *_resultObj) {
                    ...
                    if (size_t pos = _fieldNames.findPos(elemName); pos != StringListSet::npos) {
                        auto [tag, val] = bson::convertToView(elem);
                        _fieldAccessors[pos].reset(value::TagValueView{tag, val});
                    }
                }
            }
            if (_resultSlot) {
                _resultAccessor.reset(
