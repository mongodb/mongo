...
protected:
    std::unique_ptr<EExpression> makeNewObjOfLargeValues(size_t numFields, size_t valueSize) {
        ...
        for (size_t i = 0; i < numFields; ++i) {
            args.emplace_back(stage_builder::makeStrConstant("f" + std::to_string(i)));
            args.emplace_back(stage_builder::makeStrConstant(largeStr));
        }
        return stage_builder::makeFunction("newObj", std::move(args));
    }
};
...
TEST_F(SBEBuiltinNewObjTest, NewObjLongFieldNamesCountTowardMemoryLimit) {
    ...
    for (int i = 0; i < 10; ++i) {
        args.emplace_back(stage_builder::makeStrConstant(longKey + std::to_string(i)));
        args.emplace_back(stage_builder::makeInt32Constant(1));
    }
    auto newObjExpr = stage_builder::makeFunction("newObj", std::move(args));
    auto compiledExpr = compileExpression(*newObjExpr);
