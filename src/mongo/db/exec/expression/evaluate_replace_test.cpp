...
auto eval(const std::string& expressionName,
    ...
    auto expCtx = std::make_unique<ExpressionContextForTest>();
    VariablesParseState vps = expCtx->variablesParseState;
    Value operandValue =
        ImplicitValue(Document{{"input", input}, {"find", find}, {"replacement", replacement}});
    const BSONObj obj = BSON(expressionName << operandValue);
    auto expression = Expression::parseExpression(expCtx.get(), obj, vps);
