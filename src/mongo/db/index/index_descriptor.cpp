...
IndexDescriptor::Comparison IndexDescriptor::compareIndexOptions(
    ...
    if (isFcvAtLeast50) {
        ...
        // If the candidate has a wildcardProjection, we must compare the normalized versions, not
        // the versions from the catalog which are kept as the user gave them and thus may be
        // semantically identical to but syntactically different from the normalized form. There are
        // no other types of index projections. Thus, if there is no projection, both the original
        // and normalized projections will be empty BSON objects, so we can still do the comparison
        // based on the normalized projection.
