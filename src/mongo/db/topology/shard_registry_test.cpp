...
TEST_F(ShardRegistryTest, GetAllShardRefsFallsBackToShardIdWhenNoUuid) {
    addShard({"0"}, kAdvanceTopologyTime);
    addShard({"1"}, kAdvanceTopologyTime);

    auto future = launchAsync([this] {
        auto shardRefs = getData()->getAllShardRefs();
        ASSERT_EQ(shardRefs.size(), 2u);

        std::set<std::string> names;
        for (const auto& ref : shardRefs) {
            ASSERT_TRUE(ref.isString()) << "expected string-variant ref, got " << ref;
            names.insert(ref.getString());
        }
        ASSERT_TRUE((names == std::set<std::string>{"0", "1"}));
    });
    expectCSRSLookup();
    future.default_timed_get();
}

TEST_F(ShardRegistryTest, GetAllShardRefsPrefersUuidWhenPresent) {
    const auto uuid0 = UUID::gen();
    const auto uuid1 = UUID::gen();
    addShard(buildShardTypeFrom(ShardId("0"), uuid0), kAdvanceTopologyTime);
    addShard(buildShardTypeFrom(ShardId("1"), uuid1), kAdvanceTopologyTime);

    auto future = launchAsync([this, uuid0, uuid1] {
        auto shardRefs = getData()->getAllShardRefs();
        ASSERT_EQ(shardRefs.size(), 2u);

        std::set<UUID> uuids;
        for (const auto& ref : shardRefs) {
            ASSERT_TRUE(ref.isUUID()) << "expected UUID-variant ref, got " << ref;
            uuids.insert(ref.getUUID());
        }
        ASSERT_TRUE((uuids == std::set<UUID>{uuid0, uuid1}));
    });
    expectCSRSLookup();
    future.default_timed_get();
}

// The variant is chosen independently per shard, based only on whether the registry holds a UUID
// for that shard.
TEST_F(ShardRegistryTest, GetAllShardRefsResolvesEachShardIndependently) {
    const auto uuid1 = UUID::gen();
    addShard({"0"}, kAdvanceTopologyTime);                                    // no UUID
    addShard(buildShardTypeFrom(ShardId("1"), uuid1), kAdvanceTopologyTime);  // has UUID

    auto future = launchAsync([this, uuid1] {
        auto shardRefs = getData()->getAllShardRefs();
        ASSERT_EQ(shardRefs.size(), 2u);

        bool sawUuidForShard1 = false;
        for (const auto& ref : shardRefs) {
            if (ref.isString()) {
                ASSERT_EQ(ref.getString(), "0");
            } else {
                ASSERT_TRUE(ref.isUUID());
                ASSERT_EQ(ref.getUUID(), uuid1);
                sawUuidForShard1 = true;
            }
        }
        ASSERT_TRUE(sawUuidForShard1);
    });
    expectCSRSLookup();
    future.default_timed_get();
}

// Exercises the public accessor end-to-end (it shares the _getData path with getAllShardIds).
TEST_F(ShardRegistryTest, GetAllShardRefsPublicAccessor) {
    addShard({"0"}, kAdvanceTopologyTime);
    addShard({"1"}, kAdvanceTopologyTime);

    auto future = launchAsync([this] {
        auto shardRefs = shardRegistry()->getAllShardRefs(operationContext());
        ASSERT_EQ(shardRefs.size(), 2u);
    });
    expectCSRSLookup();
    future.default_timed_get();
...
TEST_F(ShardRegistryTest, GetAllShardHandlesWithShardIdOnly) {
    addShard({"0"}, kAdvanceTopologyTime);
    addShard({"1"}, kAdvanceTopologyTime);

    auto future = launchAsync([this] {
        auto handles = getData()->getAllShardHandles();
        ASSERT_EQ(handles.size(), 2u);

        std::set<ShardId> names;
        for (const auto& handle : handles) {
            ASSERT_FALSE(handle.uuid().has_value())
                << "expected no UUID for shard " << handle.name();
            names.insert(handle.name());
        }
        ASSERT_TRUE((names == std::set<ShardId>{ShardId("0"), ShardId("1")}));
    });
    expectCSRSLookup();
    future.default_timed_get();
}

TEST_F(ShardRegistryTest, GetAllShardHandlesWithShardIdAndUuid) {
    const auto uuid0 = UUID::gen();
    const auto uuid1 = UUID::gen();
    addShard(buildShardTypeFrom(ShardId("0"), uuid0), kAdvanceTopologyTime);
    addShard(buildShardTypeFrom(ShardId("1"), uuid1), kAdvanceTopologyTime);
    ...
    auto future = launchAsync([this, uuid0, uuid1] {
        ...
        // Each handle must carry both the name and the UUID — unlike getAllShardRefs, which
        // collapses to one variant.
        std::map<ShardId, UUID> nameToUuid;
        for (const auto& handle : handles) {
            ASSERT_TRUE(handle.uuid().has_value()) << "expected UUID for shard " << handle.name();
            nameToUuid.emplace(handle.name(), *handle.uuid());
        }
        ASSERT_EQ(nameToUuid.at(ShardId("0")), uuid0);
        ASSERT_EQ(nameToUuid.at(ShardId("1")), uuid1);
    });
    expectCSRSLookup();
    future.default_timed_get();
}

// The variant is chosen independently per shard: a shard without UUID has no UUID in its handle,
// while a shard with UUID carries both name and UUID.
TEST_F(ShardRegistryTest, GetAllShardHandlesResolvesEachShardIndependently) {
    const auto uuid1 = UUID::gen();
    addShard({"0"}, kAdvanceTopologyTime);                                    // no UUID
    addShard(buildShardTypeFrom(ShardId("1"), uuid1), kAdvanceTopologyTime);  // has UUID
    ...
    auto future = launchAsync([this, uuid1] {
        ...
        bool sawShard1 = false;
        for (const auto& handle : handles) {
            if (handle.name() == ShardId("0")) {
                ASSERT_FALSE(handle.uuid().has_value()) << "shard 0 should have no UUID";
            } else {
                ASSERT_EQ(handle.name(), ShardId("1"));
                ASSERT_TRUE(handle.uuid().has_value()) << "shard 1 should have a UUID";
                ASSERT_EQ(*handle.uuid(), uuid1);
                sawShard1 = true;
            }
        }
        ASSERT_TRUE(sawShard1);
    });
    expectCSRSLookup();
    future.default_timed_get();
}
...
// Exercises the public accessor end-to-end (it shares the _getData path with getAllShardIds).
TEST_F(ShardRegistryTest, GetAllShardHandlesPublicAccessor) {
    addShard({"0"}, kAdvanceTopologyTime);
    addShard({"1"}, kAdvanceTopologyTime);

    auto future = launchAsync([this] {
        auto handles = shardRegistry()->getAllShardHandles(operationContext());
        ASSERT_EQ(handles.size(), 2u);
    });
    expectCSRSLookup();
    future.default_timed_get();
...
// When the config server is not itself a shard, getAllShardRefsIncludingConfigServer appends it by
// its ShardId.
TEST_F(ShardRegistryTest, GetAllShardRefsIncludingConfigServerAppendsDedicatedConfigServer) {
    addShard({"0"}, kAdvanceTopologyTime);
    addShard({"1"}, kAdvanceTopologyTime);

    auto future = launchAsync([this] {
        auto refs = shardRegistry()->getAllShardRefsIncludingConfigServer(operationContext());
        ASSERT_EQ(refs.size(), 3u);
        ASSERT_EQ(std::count(refs.begin(), refs.end(), ShardRef(ShardId::kConfigServerId)), 1);
    });
    expectCSRSLookup();
    future.default_timed_get();
}

// When the config server is itself a (UUID-less) shard, it must not be appended a second time.
TEST_F(ShardRegistryTest, GetAllShardRefsIncludingConfigServerDoesNotDuplicateConfigShard) {
    addShard({"0"}, kAdvanceTopologyTime);
    addShard({ShardId::kConfigServerId}, kAdvanceTopologyTime);
...
// The critical UUID case: when the config server is a shard carrying a UUID, it is present once as
// its UUID and must NOT be appended again as a kConfigServerId string ref.
TEST_F(ShardRegistryTest, GetAllShardRefsIncludingConfigServerDoesNotDuplicateConfigShardWithUuid) {
    const auto configUuid = UUID::gen();
    addShard({"0"}, kAdvanceTopologyTime);
    addShard(buildShardTypeFrom(ShardId::kConfigServerId, configUuid), kAdvanceTopologyTime);

    auto future = launchAsync([this, configUuid] {
        auto refs = shardRegistry()->getAllShardRefsIncludingConfigServer(operationContext());
        ASSERT_EQ(refs.size(), 2u);
        ASSERT_EQ(std::count(refs.begin(), refs.end(), ShardRef(configUuid)), 1);
        ASSERT_EQ(std::count(refs.begin(), refs.end(), ShardRef(ShardId::kConfigServerId)), 0);
    });
    expectCSRSLookup();
    future.default_timed_get();
}
