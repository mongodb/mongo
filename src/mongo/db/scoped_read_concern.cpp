...
ScopedReadConcern::ScopedReadConcern(OperationContext* opCtx,
    ...
    _originalReadSource = _opCtx->recoveryUnit()->getTimestampReadSource();
    if (_originalReadSource == RecoveryUnit::ReadSource::kProvided)
        _originalReadTimestamp = _opCtx->recoveryUnit()->getPointInTimeReadTimestamp(opCtx);

    stdx::lock_guard<Client> lk(*_opCtx->getClient());
    repl::ReadConcernArgs::get(_opCtx) = requestReadConcernArgs;
}
