...
template <typename C>
requires std::is_same_v<typename C::value_type, BSONElement> && requires(C c) {
    { std::begin(c) } -> std::convertible_to<typename C::iterator>;
    { std::end(c) } -> std::convertible_to<typename C::iterator>;
}
