...
DEATH_TEST_REGEX(EqOpDeathTest, InvalidEooOperand, "failure.*eoo") {
    try {
        BSONObj operand;
        EqualityMatchExpression eq(""sv, operand.firstElement());
    } catch (...) {
...
TEST(EqOp, Equality1) {
    ...
    EqualityMatchExpression eq1("a"sv, operand["a"]);
    EqualityMatchExpression eq2("a"sv, operand["b"]);
    EqualityMatchExpression eq3("c"sv, operand["c"]);
...
DEATH_TEST_REGEX(LtOpDeathTest, InvalidEooOperand, "failure.*eoo") {
    try {
        BSONObj operand;
        LTMatchExpression lt(""sv, operand.firstElement());
    } catch (...) {
        invariant(false, "Threw when trying to construct obj from eoo element");
    }
}
...
DEATH_TEST_REGEX(LteOpDeathTest, InvalidEooOperand, "failure.*eoo") {
    try {
        BSONObj operand;
        LTEMatchExpression lte(""sv, operand.firstElement());
    } catch (...) {
        invariant(false, "Threw when trying to construct obj from eoo element");
    }
}
...
DEATH_TEST_REGEX(GtOpDeathTest, InvalidEooOperand, "failure.*eoo") {
    try {
        BSONObj operand;
        GTMatchExpression gt(""sv, operand.firstElement());
    } catch (...) {
        invariant(false, "Threw when trying to construct obj from eoo element");
    }
}
...
DEATH_TEST_REGEX(GteOpDeathTest, InvalidEooOperand, "failure.*eoo") {
    try {
        BSONObj operand;
        GTEMatchExpression gte(""sv, operand.firstElement());
    } catch (...) {
        invariant(false, "Threw when trying to construct obj from eoo element");
    }
}
...
TEST(RegexMatchExpression, TooLargePattern) {
    ...
    ASSERT_THROWS_CODE(RegexMatchExpression("a"sv, tooLargePattern, ""), AssertionException, 51091);
}

TEST(RegexMatchExpression, Equality1) {
    RegexMatchExpression r1("a"sv, "b", "");
    RegexMatchExpression r2("a"sv, "b", "x");
    RegexMatchExpression r3("a"sv, "c", "");
    RegexMatchExpression r4("b"sv, "b", "");
...
TEST(RegexMatchExpression, RegexCannotContainEmbeddedNullByte) {
    {
        const auto embeddedNull = "a\0b"sv;
        ASSERT_THROWS_CODE(RegexMatchExpression("path"sv, embeddedNull, ""),
                           AssertionException,
                           ErrorCodes::BadValue);
    }

    {
        const auto singleNullByte = "\0"sv;
        ASSERT_THROWS_CODE(RegexMatchExpression("path"sv, singleNullByte, ""),
                           AssertionException,
                           ErrorCodes::BadValue);
    }

    {
        const auto leadingNullByte = "\0bbbb"sv;
        ASSERT_THROWS_CODE(RegexMatchExpression("path"sv, leadingNullByte, ""),
                           AssertionException,
                           ErrorCodes::BadValue);
    }

    {
        const auto trailingNullByte = "bbbb\0"sv;
        ASSERT_THROWS_CODE(RegexMatchExpression("path"sv, trailingNullByte, ""),
                           AssertionException,
                           ErrorCodes::BadValue);
    }
}
...
TEST(RegexMatchExpression, RegexOptionsStringCannotContainEmbeddedNullByte) {
    {
        const auto embeddedNull = "a\0b"sv;
        ASSERT_THROWS_CODE(
            RegexMatchExpression("path"sv, "pattern", embeddedNull), AssertionException, 51108);
    }

    {
        const auto singleNullByte = "\0"sv;
        ASSERT_THROWS_CODE(
            RegexMatchExpression("path"sv, "pattern", singleNullByte), AssertionException, 51108);
    }

    {
        const auto leadingNullByte = "\0bbbb"sv;
        ASSERT_THROWS_CODE(
            RegexMatchExpression("path"sv, "pattern", leadingNullByte), AssertionException, 51108);
    }

    {
        const auto trailingNullByte = "bbbb\0"sv;
        ASSERT_THROWS_CODE(
            RegexMatchExpression("path"sv, "pattern", trailingNullByte), AssertionException, 51108);
    }
}

TEST(RegexMatchExpression, MalformedRegexNotAccepted) {
    ASSERT_THROWS_CODE(RegexMatchExpression("a"sv,  // path
                                            "[",    // regex
                                            ""      // options
                                            ),
                       AssertionException,
...
TEST(RegexMatchExpression, MalformedRegexWithStartOptionNotAccepted) {
    ASSERT_THROWS_CODE(RegexMatchExpression("a"sv, "[(*ACCEPT)", ""), AssertionException, 51091);
}

TEST(ModMatchExpression, ZeroDivisor) {
    ASSERT_THROWS_CODE(ModMatchExpression(""sv, 0, 1), AssertionException, ErrorCodes::BadValue);
}

TEST(ModMatchExpression, Equality1) {
    ModMatchExpression m1("a"sv, 1, 2);
    ModMatchExpression m2("a"sv, 2, 2);
    ModMatchExpression m3("a"sv, 1, 1);
    ModMatchExpression m4("b"sv, 1, 2);
...
TEST(ExistsMatchExpression, Equivalent) {
    ExistsMatchExpression e1("a"sv);
    ExistsMatchExpression e2("b"sv);
...
TEST(InMatchExpression, InMatchExpressionsWithDifferentNumbersOfElementsAreUnequal) {
    ...
    InMatchExpression eq1(""sv);
    InMatchExpression eq2(""sv);
...
TEST(InMatchExpression, SerializeToQueryShapeEmptyList) {
    InMatchExpression matchExpression("a"sv);
...
TEST(BitTestMatchExpressionEquivalentTest, ArrayOrderDoesNotMatterForEquivalence) {
    BitsAllSetMatchExpression e1("a"sv, std::vector<uint32_t>{4, 1, 5, 2});
    BitsAllSetMatchExpression e2("a"sv, std::vector<uint32_t>{1, 2, 4, 5});
...
TEST(BitTestMatchExpressionEquivalentTest, MultiByteBinDataAndArrayAreEquivalent) {
    ...
    BitsAllSetMatchExpression fromBinData("a"sv, buf.data(), buf.size());
    BitsAllSetMatchExpression fromArray("a"sv, positions);
    BitsAllSetMatchExpression fromInt64("a"sv, uint64_t{0x3600});  // bits 9,10,12,13 = 0x3600
...
TEST(BitTestMatchExpressionEquivalentTest, IntegerMaskAndBinDataAreEquivalent) {
    ...
    auto buf = bitPositionsToBinData(positions);
    BitsAllSetMatchExpression fromMask("a"sv, uint64_t{0x36});
    BitsAllSetMatchExpression fromBinData("a"sv, buf.data(), buf.size());
...
TEST(BitTestMatchExpressionEquivalentTest, NegativeInt64Bit63EquivalentToBinDataAndArray) {
    ...
    BitsAllSetMatchExpression fromNeg("a"sv, std::numeric_limits<long long>::min());
    BitsAllSetMatchExpression fromBinData("a"sv, buf.data(), buf.size());
    BitsAllSetMatchExpression fromArray("a"sv, std::vector<uint32_t>{63});
...
TEST(BitTestMatchExpressionEquivalentTest, AllFourOperatorsWorkWithBinDataAndArray) {
    ...
    BitsAllSetMatchExpression allSet_bin("a"sv, data, len);
    BitsAllSetMatchExpression allSet_arr("a"sv, positions);
    ASSERT_TRUE(allSet_bin.equivalent(&allSet_arr));

    BitsAllClearMatchExpression allClear_bin("a"sv, data, len);
    BitsAllClearMatchExpression allClear_arr("a"sv, positions);
    ASSERT_TRUE(allClear_bin.equivalent(&allClear_arr));

    BitsAnySetMatchExpression anySet_bin("a"sv, data, len);
    BitsAnySetMatchExpression anySet_arr("a"sv, positions);
    ASSERT_TRUE(anySet_bin.equivalent(&anySet_arr));

    BitsAnyClearMatchExpression anyClear_bin("a"sv, data, len);
    BitsAnyClearMatchExpression anyClear_arr("a"sv, positions);
...
TEST(BitTestMatchExpressionEquivalentTest, DifferentOperatorsAreNotEquivalent) {
    ...
    const char* data = buf.data();
    const uint32_t len = buf.size();
    BitsAllSetMatchExpression allSet("a"sv, data, len);
    BitsAllClearMatchExpression allClear("a"sv, data, len);
    BitsAnySetMatchExpression anySet("a"sv, data, len);
    BitsAnyClearMatchExpression anyClear("a"sv, data, len);
...
TEST(BitTestMatchExpressionEquivalentTest, DifferentBitsAreNotEquivalent) {
    BitsAllSetMatchExpression e1("a"sv, std::vector<uint32_t>{1, 2, 4, 5});
    BitsAllSetMatchExpression e2("a"sv, std::vector<uint32_t>{1, 2, 4});
...
TEST(BitTestMatchExpressionEquivalentTest, DifferentPathsAreNotEquivalent) {
    ...
    const char* data = buf.data();
    const uint32_t len = buf.size();
    BitsAllSetMatchExpression onA("a"sv, data, len);
    BitsAllSetMatchExpression onB("b"sv, data, len);
...
TEST(BitTestMatchExpressionEquivalentTest, TrailingZeroBytesDoNotAffectEquivalence) {
    ...
    BitsAllSetMatchExpression e1("a"sv, buf1.data(), buf1.size());
    BitsAllSetMatchExpression e2("a"sv, buf2.data(), buf2.size());
    BitsAllSetMatchExpression e3("a"sv, uint64_t{0x36});  // bits 1,2,4,5 = 0x36
