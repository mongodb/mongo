...
const Collection* CollectionCatalog::_instantiateCollectionIfTesting(
    ...
    if (!TestingProctor::instance().isEnabled()) {
        // This is not a testing build, so don't try to enable the test-only collection
        // instantiation.
        return nullptr;
    }

    // We allow tests to only trigger the recovery with a given probability if needed to reduce the
    // rate of false positive errors.
    auto& randomGenerator = opCtx->getClient()->getPrng();
    if (!randomGenerator.trueWithProbability(
            gChanceForDurableCollectionMetadataRecovery.loadRelaxed())) {
        return nullptr;
    }
