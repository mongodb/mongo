...
TEST_F(IndexBuildsCoordinatorTest, ResumeFailsWhenTearableSideWriteAbortSentinelPresent) {
    ...
    auto nss = NamespaceString::createNamespaceString_forTest(
        "IndexBuildsCoordinatorTest.ResumeFailsWhenTearableSideWriteAbortSentinelPresent");
    ...
    ASSERT_OK(index_builds::primary_driven::start(
        opCtx, nss.dbName(), collectionUUID, buildUUID, indexes, indexBuildIdent));

    // Persist a normal, valid multi-record index build state (IndexBuildMetadata at key 1 followed
    // by per-index IndexStateInfo records).
    persistPrimaryDrivenResumeState(opCtx,
                                    indexes,
                                    buildUUID,
                                    collectionUUID,
                                    indexBuildIdent,
                                    IndexBuildPhaseEnum::kInitialized);
    ASSERT_DOES_NOT_THROW(index_builds::primary_driven::resumeInfo(
        opCtx, collectionUUID, buildUUID, indexes, indexBuildIdent));

    // Now write the abort sentinel. The resume reader parses every record, so it fails to parse the
    // sentinel.
    {
        WriteUnitOfWork wuow{opCtx};
        auto& ru = *shard_role_details::getRecoveryUnit(opCtx);
        ...
        auto sentinel = BSON("tearableSideWriteAbort" << true);
        ASSERT_OK(rs->insertRecord(opCtx,
                                   ru,
                                   RecordId(int64_t{1} << 62),
                                   sentinel.objdata(),
                                   sentinel.objsize(),
                                   Timestamp()));
        wuow.commit();
    ...
    ASSERT_THROWS_CODE(index_builds::primary_driven::resumeInfo(
                           opCtx, collectionUUID, buildUUID, indexes, indexBuildIdent),
                       DBException,
