...
namespace {
    ...
    public:
        MyOperationContext() {
            _recoveryUnit.reset( new WiredTigerRecoveryUnit() );
        }
    ...
    private:
        boost::scoped_ptr<WiredTigerRecoveryUnit> _recoveryUnit;

    };

    TEST(WiredTigerSimple, CreateDestroy) {
        WiredTigerDatabaseCatalogEntry db( "foo" );
    }

    TEST(WiredTigerSimple, ListCollections) {
        WiredTigerDatabaseCatalogEntry db( "foo" );
    ...
    TEST(WiredTigerSimple, RecordStore1) {
        WiredTigerDatabaseCatalogEntry db( "foo" );
