...
public:
    ...
    bool _deleteExcessDocuments() {
        ...
        try {
            ...
            // interruptions. PBWM, database lock or collection lock is not needed. This improves
            // concurrency if oplog truncation takes long time.
            opCtx->lockState()->setShouldConflictWithSecondaryBatchApplication(false);
            Lock::GlobalLock lk(opCtx.get(), MODE_IX, UINT_MAX);

            WiredTigerRecordStore* rs = nullptr;
            {
                // Release the database lock right away because we don't want to block other
                // operations on the local database and given the fact that oplog collection is so
                // special, Global IX lock can make sure the collection exists.
                Lock::DBLock dbLock(opCtx.get(), _ns.db(), MODE_IX);
                Database* db = dbHolder().get(opCtx.get(), _ns.db());
                if (!db) {
