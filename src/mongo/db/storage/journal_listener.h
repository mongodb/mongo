 ...
 *
 * An unlocked mutex is passed into getToken() so that the mutex can be locked after taking any
 * other locks that the function may need. The mutex must be held by the caller until after the
 * subsequent call to onDurable().
 *
 * All callers of these functions must be serialized by a common mutex, to make sure that the
 * listener is never told to go backwards in time due to races in calling onDurable() after
 * getToken().
 */
...
public:
    ...
    virtual Token getToken(OperationContext* opCtx, stdx::unique_lock<Latch>& lk) = 0;
