...
class TestQueryAnalysisOpObserver : public QueryAnalysisOpObserver {
public:
    TestQueryAnalysisOpObserver(std::unique_ptr<QueryAnalysisCoordinatorFactory> coordinatorFactory)
        : QueryAnalysisOpObserver(std::move(coordinatorFactory)) {}
...
class QueryAnalysisOpObserverTest : public ConfigServerTestFixture {
public:
    QueryAnalysisOpObserverTest() : ConfigServerTestFixture() {}
};

TEST_F(QueryAnalysisOpObserverTest, InsertSamplerHandlesException) {
    auto coordinatorFactory = std::make_unique<TestQueryAnalysisCoordinatorFactory>();
    TestQueryAnalysisOpObserver queryAnalysisOpObserver(std::move(coordinatorFactory));

    auto doc = MongosType();

    ASSERT_DOES_NOT_THROW(queryAnalysisOpObserver.testInsert(operationContext(), doc));
}

TEST_F(QueryAnalysisOpObserverTest, UpdateSamplerHandlesException) {
    auto coordinatorFactory = std::make_unique<TestQueryAnalysisCoordinatorFactory>();
    TestQueryAnalysisOpObserver queryAnalysisOpObserver(std::move(coordinatorFactory));

    auto doc = MongosType();

    ASSERT_DOES_NOT_THROW(queryAnalysisOpObserver.testUpdate(operationContext(), doc));
}

TEST_F(QueryAnalysisOpObserverTest, DeleteSamplerHandlesException) {
    auto coordinatorFactory = std::make_unique<TestQueryAnalysisCoordinatorFactory>();
    TestQueryAnalysisOpObserver queryAnalysisOpObserver(std::move(coordinatorFactory));

    auto doc = MongosType();

    ASSERT_DOES_NOT_THROW(queryAnalysisOpObserver.testDelete(operationContext(), doc));
}
