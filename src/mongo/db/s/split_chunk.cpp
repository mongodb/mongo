...
StatusWith<boost::optional<ChunkRange>> splitChunk(OperationContext* opCtx,
    ...
    if (KeyPattern::isHashedKeyPattern(keyPatternObj)) {
        for (BSONObj splitPoint : splitPoints) {
            BSONObjIterator it(splitPoint);
            while (it.more()) {
                BSONElement splitPointElement = it.next();
                if (splitPointElement.type() != NumberLong) {
                    return {ErrorCodes::CannotSplit,
                            str::stream()
                                << "splitChunk cannot split chunk " << chunkRange.toString()
                                << ", split point " << splitPointElement.toString()
                                << " must be of type "
