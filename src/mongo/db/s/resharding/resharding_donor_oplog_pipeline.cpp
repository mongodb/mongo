...
ReshardingDonorOplogPipeline::ScopedPipeline ReshardingDonorOplogPipeline::initWithOperationContext(
    ...
    if (_pipeline) {
        ...
        _pipeline =
            pipeline->getContext()->mongoProcessInterface->attachCursorSourceToPipelineForLocalRead(
                pipeline.release());
...
std::unique_ptr<Pipeline, PipelineDeleter> ReshardingDonorOplogPipeline::_makePipeline(
...
std::vector<repl::OplogEntry> ReshardingDonorOplogPipeline::_getNextBatch(size_t batchLimit) {
    ...
    invariant(_pipeline->getContext()->opCtx);

    std::vector<repl::OplogEntry> batch;

    int numBytes = 0;
    do {
        auto doc = _pipeline->getNext();
        if (!doc) {
            break;
        }
        ...
        if (resharding::isFinalOplog(entry)) {
            ...
            if (auto nextDoc = _pipeline->getNext()) {
                tasserted(6077499,
