...
protected:
    ...
    void assertStateAndCatalogEntriesMatchExpected(
        ...
        boost::optional<OID> collectionEpoch = boost::none) {
    ...
    void persistInitialStateAndCatalogUpdatesExpectSuccess(
        ...
        // correctly
        assertStateAndCatalogEntriesMatchExpected(opCtx, expectedCoordinatorDoc);
...
TEST_F(ReshardingCoordinatorPersistenceTest, PersistBasicStateTransitionSucceeds) {
    auto coordinatorDoc = insertStateAndCatalogEntries(CoordinatorStateEnum::kInitialized);
...
TEST_F(ReshardingCoordinatorPersistenceTest, PersistFetchTimestampStateTransitionSucceeds) {
    auto coordinatorDoc = insertStateAndCatalogEntries(CoordinatorStateEnum::kPreparingToDonate);
...
TEST_F(ReshardingCoordinatorPersistenceTest, PersistCommitSucceeds) {
    Timestamp fetchTimestamp = Timestamp(1, 1);
    auto coordinatorDoc =
        insertStateAndCatalogEntries(CoordinatorStateEnum::kMirroring, fetchTimestamp);
...
TEST_F(ReshardingCoordinatorPersistenceTest, PersistTransitionToDoneSucceeds) {
    auto coordinatorDoc = insertStateAndCatalogEntries(CoordinatorStateEnum::kDropping);
