...
private:
    ...
    // This iterator is a pointer into the _cloneRecordIds set.  It allows concurrent access to
    // the _cloneRecordIds set by allowing threads servicing _migrateClone requests to do the
    // following:
    ...
    //
    // If (_cloneRecordIdsIter == _cloneRecordIds.end() && _overflowDocs.empty() && _inProgressReads
    // == 0) then all documents have been returned to the destination.
    decltype(_cloneRecordIds.size()) _inProgressReads = 0;
    ...
    decltype(_cloneRecordIds.size()) _numRecordsCloned{0};
    decltype(_cloneRecordIds.size()) _numRecordsPassedOver{0};
