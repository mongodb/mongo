...
TEST(IsRawTimeseriesShardKeyTest, HashedMetaFieldKeyIsNotTranslated) {
    auto tsOptions = makeTimeseriesOptions("time", StringData("hostId"));
    ASSERT_FALSE(shardkeyutil::isRawTimeseriesShardKey(tsOptions,
                                                       BSON("hostId.x"
                                                            << "hashed")));
}
...
TEST(IsRawTimeseriesShardKeyTest, TranslatedHashedMetaFieldKeyIsTranslated) {
    auto tsOptions = makeTimeseriesOptions("time", StringData("hostId"));
    ASSERT_TRUE(shardkeyutil::isRawTimeseriesShardKey(tsOptions,
                                                      BSON("meta.x"
                                                           << "hashed")));
}
...
TEST(ValidateAndTranslateTimeseriesShardKeyTest, HashedMetaFieldKeyTranslatesCorrectly) {
    auto tsOptions = makeTimeseriesOptions("time", StringData("hostId"));
    auto result = shardkeyutil::validateAndTranslateTimeseriesShardKey(tsOptions,
                                                                       BSON("hostId.x"
                                                                            << "hashed"));
    ASSERT_BSONOBJ_EQ(result,
                      BSON("meta.x"
                           << "hashed"));
}
...
TEST(ValidateAndTranslateTimeseriesShardKeyTest, TimeFieldWithHashedThrows880031) {
    auto tsOptions = makeTimeseriesOptions("time", StringData("hostId"));
    ASSERT_THROWS_CODE(shardkeyutil::validateAndTranslateTimeseriesShardKey(tsOptions,
                                                                            BSON("time"
                                                                                 << "hashed")),
                       AssertionException,
                       880031);
