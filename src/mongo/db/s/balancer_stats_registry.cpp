        ...
        pipeline.push_back(
            BSON("$group" << BSON(
                     "_id" << "numOrphans"
                           << "count"
                           << BSON("$sum" << "$" +
                                       std::string{RangeDeletionTask::kNumOrphanDocsFieldName}))));
...
void BalancerStatsRegistry::_loadOrphansCount(OperationContext* opCtx) {
    ...
    static const BSONObj groupStage{BSON(
        "$group" << BSON(
            "_id" << "$" + std::string{RangeDeletionTask::kCollectionUuidFieldName}
                  << kNumOrphanDocsLabel
                  << BSON("$sum" << "$" + std::string{RangeDeletionTask::kNumOrphanDocsFieldName})
