    ...
    PeriodicRunner::PeriodicJob job(
        ...
        [this](Client* client) {
            ...
            if (serverGlobalParams.featureCompatibility.getVersion() !=
                ...
                // Short circuit when not fully FCV upgraded because 4.0 binary shards do not
                // produce the necessary $indexStats output for the pipeline to work correctly.  If
                // the config server is fully upgraded to FCV 4.2, then all shards must be running a
                // 4.2 binary, which avoids this problem at the expense of not detecting
                // inconsistent indexes when FCV < 4.2.
                log() << "Skipping sharded index consistency check because feature compatibility "
            ...
            const auto aggRequestBSON = fromjson(
                ...
                // Omit shards that don't include a spec field because only shards in 4.2.4 or later
                // will include it. This avoids incorrectly considering an index missing in a
                // cluster with at least one 4.2.3 shard.
                "{$match:"
                "{spec: {$exists: true}}},"
