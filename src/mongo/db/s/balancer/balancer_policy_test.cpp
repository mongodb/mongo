...
TEST(BalancerPolicy, ParallelBalancingDoesNotScheduleMigrationsOnShardsAboveIdealDataSize) {
    ...
    auto [cluster, cm] =
        generateCluster({{100, 100 * kDefaultMaxChunkSizeBytes},
                         {90, 90 * ChunkSizeSettingsType::kDefaultMaxChunkSizeBytes},
                         {90, 90 * ChunkSizeSettingsType::kDefaultMaxChunkSizeBytes},
                         {80, 80 * ChunkSizeSettingsType::kDefaultMaxChunkSizeBytes},
                         {0, 0 * ChunkSizeSettingsType::kDefaultMaxChunkSizeBytes},
                         {0, 0 * ChunkSizeSettingsType::kDefaultMaxChunkSizeBytes}});
...
TEST(BalancerPolicy, OverloadedShardCannotDonateDueToTags) {
    // There is a large imbalance between shard0 and the other shard, but shard0 can't donate
    // chunks because it would violate tags.
