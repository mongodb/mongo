...
void TransactionCoordinatorCatalog::join() {
    ...
    while (!_noActiveCoordinatorsCV.wait_for(
        ul, std::chrono::seconds{5}, [this] { return _coordinatorsBySession.empty(); })) {
        LOGV2(22442,
...
void TransactionCoordinatorCatalog::_waitForStepUpToComplete(std::unique_lock<std::mutex>& lk,
                                                             OperationContext* opCtx) {
