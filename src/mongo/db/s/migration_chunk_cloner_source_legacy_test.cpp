...
public:
    ...
    Status checkValidation(OperationContext* opCtx, const BSONObj& document) const override {
    ...
    Status updateCappedSize(OperationContext* opCtx, long long newCappedSize) override {
        MONGO_UNREACHABLE;
    }

    Status checkMetaDataForIndex(const std::string& indexName, const BSONObj& spec) const override {
    ...
    bool isIndexMultikey(OperationContext* opCtx,
                         ...
                         MultikeyPaths* multikeyPaths) const override {
        return _coll->isIndexMultikey(opCtx, indexName, multikeyPaths);
    }

    bool setIndexIsMultikey(OperationContext* opCtx,
                            StringData indexName,
                            const MultikeyPaths& multikeyPaths) const override {
        return _coll->setIndexIsMultikey(opCtx, indexName, multikeyPaths);
    }
    ...
    void deleteDocument(OperationContext* opCtx,
                        ...
                        bool noWarn = false,
                        StoreDeletedDoc storeDeletedDoc = StoreDeletedDoc::Off) const override {
        MONGO_UNREACHABLE;
    }
    ...
    void establishOplogCollectionForLogging(OperationContext* opCtx) override {
        MONGO_UNREACHABLE;
    }
...
protected:
    ...
    void insertDocsInShardedCollection(const std::vector<BSONObj>& docs) {
        ...
        while (!docsToInsert.empty()) {
            ...
            client()->insert(kNss.ns(), batchToInsert);
...
TEST_F(MigrationChunkClonerSourceLegacyTest, CloneFetchThatOverflows) {
    ...
    auto req =
        createMoveChunkRequest(ChunkRange(BSON("X" << 100), BSON("X" << 200)), 64 * 1024 * 1024);
    MigrationChunkClonerSourceLegacy cloner(
...
TEST_F(MigrationChunkClonerSourceLegacyTest, CloneShouldNotCrashWhenNextCloneBatchThrows) {
    ...
    auto req = createMoveChunkRequest(ChunkRange(BSON("X" << 100), BSON("X" << 200)));
    MigrationChunkClonerSourceLegacy cloner(
        req, kShardKeyPattern, kDonorConnStr, kRecipientConnStr.getServers()[0]);
