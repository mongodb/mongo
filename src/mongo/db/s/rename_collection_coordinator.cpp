...
SemiFuture<BatchedCommandResponse> insertToPlacementHistoryStatement(
    ...
    // Skip the insertion of the placement entry if the previous statement
    // didn't change any document - we can deduce that the whole transaction
    // was already committed in a previous attempt.
...
void renameCollectionMetadataInTransaction(OperationContext* opCtx,
    ...
    if (optFromCollType) {
        ...
        // we forcely add a tick to have 2 valid timestamp that we can use
        // to differentiate the 2 operations.
    ...
    return ExecutorFuture<void>(**executor)
        ...
        .then(_buildPhaseHandler(
            ...
            [this, executor = executor, anchor = shared_from_this()] {
                ...
                if (!_isPre70Compatible() &&
                    ...
                    /*
                    ShardsvrRenameIndexMetadata does not require gGlobalIndexesShardingCatalog to be
                    enabled since it's guaranteed to be present in case of FCV >= 7.0
                    */
        ...
        .then(_buildPhaseHandler(
            ...
            [this, executor = executor, anchor = shared_from_this()] {
                ...
                // This is performed after releasing the critical section in order to
                // reduce stalls and performed outside of a transaction to prevent timeout
