...
TEST(OperationLatencyHistogram, CheckBucketCountsAndTotalLatency) {
    ...
    BSONObj out = outBuilder.done();
    EXPECT_EQ(static_cast<uint64_t>(out["reads"]["latency"].Long()), expectedSum);
    EXPECT_EQ(static_cast<uint64_t>(out["reads"]["queryableEncryptionLatencyMicros"].Long()), 0);

    // Each bucket has three counts with the exception of the last bucket, which has two.
    EXPECT_EQ(out["reads"]["ops"].Long(), 3 * kMaxBuckets - 1);
    std::vector<BSONElement> readBuckets = out["reads"]["histogram"].Array();
    ASSERT_EQUALS(readBuckets.size(), static_cast<size_t>(kMaxBuckets));
    size_t bucketIndex = 0;
    for (const BSONElement& elem : readBuckets) {
        EXPECT_EQ(elem["micros"].Long(), static_cast<long long>(kLowerBounds[bucketIndex]));
        EXPECT_EQ(elem["count"].Long(), (bucketIndex < kMaxBuckets - 1) ? 3 : 2);
        ++bucketIndex;
    }
}
...
TEST(OperationLatencyHistogram, CheckBucketCountsAndTotalLatencyExcludeEmptyBuckets) {
    ...
    // bucket.
    std::vector<BSONElement> readBuckets = out["reads"]["histogram"].Array();
    ASSERT_EQUALS(readBuckets.size(), static_cast<size_t>((kMaxBuckets + 1) / 2));
    size_t arrayIndex = 0;
    for (const BSONElement& elem : readBuckets) {
        size_t bucketIndex = arrayIndex * 2;
        EXPECT_EQ(elem["micros"].Long(), static_cast<long long>(kLowerBounds[bucketIndex]));
        EXPECT_EQ(elem["count"].Long(), 1);
        ++arrayIndex;
    }
}
...
TEST(OperationLatencyHistogram, CheckBucketCountsAndTotalLatencyIncludeEmptyBuckets) {
    ...
    // Every other bucket has one count. Add one to kMaxBuckets since we start with the first
    // bucket.
    EXPECT_EQ(out["reads"]["ops"].Long(), (kMaxBuckets + 1) / 2);
    std::vector<BSONElement> readBuckets = out["reads"]["histogram"].Array();
    ASSERT_EQUALS(readBuckets.size(), static_cast<size_t>(kMaxBuckets));
    size_t bucketIndex = 0;
    for (const BSONElement& elem : readBuckets) {
        EXPECT_EQ(elem["micros"].Long(), static_cast<long long>(kLowerBounds[bucketIndex]));
        EXPECT_EQ(elem["count"].Long(), (bucketIndex % 2 == 0) ? 1 : 0);
        ++bucketIndex;
    }
}
...
TEST(OperationLatencyHistogram, CheckBucketCountsAndTotalLatencySlowBuckets) {
    ...
    EXPECT_EQ(out["reads"]["ops"].Long(), 3 * kMaxBuckets - 1);
    std::vector<BSONElement> readBuckets = out["reads"]["histogram"].Array();
    ASSERT_EQUALS(readBuckets.size(), static_cast<size_t>(kMaxUnFilteredBuckets + 1));
    size_t bucketIndex = 0;
    for (const BSONElement& elem : readBuckets) {
        ASSERT_LESS_THAN_OR_EQUALS(bucketIndex, kMaxUnFilteredBuckets);
        EXPECT_EQ(elem["micros"].Long(), static_cast<long long>(kLowerBounds[bucketIndex]));
        EXPECT_EQ(elem["count"].Long(), (bucketIndex < kMaxUnFilteredBuckets) ? 3 : 83);
        ++bucketIndex;
    }
}
...
TEST(OperationLatencyHistogram, CheckBucketCountsAndTotalLatencySlowBucketsExcludeEmptyBuckets) {
    ...
    EXPECT_EQ(out["reads"]["ops"].Long(), (kMaxBuckets + 1) / 2);
    std::vector<BSONElement> readBuckets = out["reads"]["histogram"].Array();
    ASSERT_EQUALS(readBuckets.size(), static_cast<size_t>((kMaxUnFilteredBuckets + 1) / 2 + 1));
    size_t bucketIndex = 0;
    size_t arrayIndex = 0;
    for (const BSONElement& elem : readBuckets) {
        ...
        EXPECT_EQ(elem["micros"].Long(), static_cast<long long>(kLowerBounds[bucketIndex]));
        EXPECT_EQ(elem["count"].Long(), bucketIndex < kMaxUnFilteredBuckets ? 1 : 14);
        ++arrayIndex;
    }
}
...
TEST(OperationLatencyHistogram,
    ...
    EXPECT_EQ(out["reads"]["ops"].Long(), kMaxBuckets / 3 + 1);
    std::vector<BSONElement> readBuckets = out["reads"]["histogram"].Array();
    ASSERT_EQUALS(readBuckets.size(), static_cast<size_t>(kMaxUnFilteredBuckets + 2) / 6 + 1);
    size_t arrayIndex = 0;
    for (const BSONElement& elem : readBuckets) {
        ...
        if (bucketIndex < static_cast<int>(kMaxUnFilteredBuckets)) {
            ...
            EXPECT_EQ(elem["count"].Long(), 2);
        } else {
            // Aggregate bucket
            EXPECT_EQ(elem["count"].Long(), 10);
        }
        ++arrayIndex;
    }
}
...
TEST(OperationLatencyHistogram,
    ...
    EXPECT_EQ(out["reads"]["ops"].Long(), kMaxBuckets / 3 + 1);
    std::vector<BSONElement> readBuckets = out["reads"]["histogram"].Array();
    ASSERT_EQUALS(readBuckets.size(), static_cast<size_t>(kMaxUnFilteredBuckets + 2) / 3 + 1);
    size_t arrayIndex = 0;
    for (const BSONElement& elem : readBuckets) {
        ...
        if (bucketIndex < static_cast<int>(kMaxUnFilteredBuckets)) {
            ...
            EXPECT_EQ(elem["count"].Long(), bucketIndex % 6 == 0 ? 2 : 0);
        } else {
            // Aggregate bucket
            EXPECT_EQ(elem["count"].Long(), 10);
        }
...
BSONElement getLastElement(std::vector<BSONElement>& arr) {
...
TEST(OperationLatencyHistogram, WhichBucketIsSlowLogBucketScalingFactorExcludeEmptyBuckets) {
    ...
    EXPECT_EQ(lastElement["micros"].Long(), static_cast<long long>(kLowerBounds[kSlowBucket3Ms]));
    ASSERT_EQ(lastElement["count"].Long(), 3);
    ...
    EXPECT_EQ(lastElement["micros"].Long(), static_cast<long long>(kLowerBounds[kSlowBucket4Ms]));
    ASSERT_EQ(lastElement["count"].Long(), 2);
    ...
    ASSERT_EQUALS(readBuckets.size(), 2);
    lastElement = getLastElement(readBuckets);
    EXPECT_EQ(lastElement["micros"].Long(), static_cast<long long>(kLowerBounds[kSlowBucket6Ms]));
    ASSERT_EQ(lastElement["count"].Long(), 1);
}
...
TEST(OperationLatencyHistogram, WhichBucketIsSlowLogBucketScalingFactorIncludeEmptyBuckets) {
    ...
    BSONObj out = outBuilder1.done();
    std::vector<BSONElement> readBuckets = out["reads"]["histogram"].Array();
    ASSERT_EQUALS(readBuckets.size(), 5);
    EXPECT_EQ(getLastElement(readBuckets)["micros"].Long(),
              static_cast<long long>(kLowerBounds[kSlowBucket3Ms]));
    ...
    out = outBuilder2.done();
    readBuckets = out["reads"]["histogram"].Array();
    ASSERT_EQUALS(readBuckets.size(), 6);
    EXPECT_EQ(getLastElement(readBuckets)["micros"].Long(),
              static_cast<long long>(kLowerBounds[kSlowBucket4Ms]));
    ...
    ASSERT_EQUALS(readBuckets.size(), 6);
    EXPECT_EQ(getLastElement(readBuckets)["micros"].Long(),
              static_cast<long long>(kLowerBounds[kSlowBucket6Ms]));
}
...
TEST(OperationLatencyHistogram, CheckLastBucketValueSlowBucketsExcludeEmptyBuckets) {
    ...
    EXPECT_EQ(static_cast<uint64_t>(out["reads"]["latency"].Long()), expectedSum);
    EXPECT_EQ(out["reads"]["ops"].Long(), 1);
    // Only one entry, so just one bucket.
    std::vector<BSONElement> readBuckets = out["reads"]["histogram"].Array();
    ASSERT_EQUALS(readBuckets.size(), 1);
    BSONElement field = readBuckets[0];
    EXPECT_EQ(field["micros"].Long(), static_cast<long long>(kLowerBounds[kUnfilteredBucket]));
    EXPECT_EQ(field["count"].Long(), 1);
    ...
    EXPECT_EQ(static_cast<uint64_t>(out["reads"]["latency"].Long()), expectedSum);
    EXPECT_EQ(out["reads"]["ops"].Long(), 2);
    ...
    for (const BSONElement& field : readBuckets) {
        if (bucketIndex == 0) {
            EXPECT_EQ(field["micros"].Long(),
                      static_cast<long long>(kLowerBounds[kUnfilteredBucket]));
        } else {
            EXPECT_EQ(field["micros"].Long(),
                      static_cast<long long>(kLowerBounds[kMaxUnFilteredBuckets + 2]));
        }
        EXPECT_EQ(field["count"].Long(), 1);
        ++bucketIndex;
    }
...
TEST(OperationLatencyHistogram, CheckLastBucketValueSlowBucketsIncludeEmptyBuckets) {
    ...
    size_t bucketIndex = 0;
    for (const BSONElement& field : readBuckets) {
        EXPECT_EQ(field["micros"].Long(), static_cast<long long>(kLowerBounds[bucketIndex]));
        EXPECT_EQ(field["count"].Long(),
                  bucketIndex == static_cast<int>(kUnfilteredBucket) ? 1 : 0);
        ++bucketIndex;
    }
    ...
    for (const BSONElement& field : readBuckets) {
        ...
        EXPECT_EQ(field["count"].Long(), isPopulatedBucket ? 1 : 0);
        ++bucketIndex;
    }
}
...
TEST(OperationLatencyHistogram, CheckLastBucketValuePresentLogScalingFactor) {
    ...
    for (int logScalingFactor : kLogScalingFactors) {
        ...
        ASSERT_EQUALS(readBuckets.size(), static_cast<size_t>(1));
        int64_t expectedBucketValue =
            kLowerBounds[kMaxBuckets - 1 - ((kMaxBuckets - 1) % logScalingFactor)];
        EXPECT_EQ(readBuckets[0]["micros"].Long(), expectedBucketValue);
        EXPECT_EQ(readBuckets[0]["count"].Long(), 1);
    }
}
...
TEST(OperationLatencyHistogram, CheckBucketCountsAndTotalLatencyQueryableEncryption) {
    ...
    BSONObj out = outBuilder.done();
    EXPECT_EQ(static_cast<uint64_t>(out["reads"]["latency"].Long()), expectedSum);
    EXPECT_EQ(static_cast<uint64_t>(out["reads"]["queryableEncryptionLatencyMicros"].Long()),
              expectedSum);
    ...
    for (const BSONElement& field : readBuckets) {
        EXPECT_EQ(field["micros"].Long(), static_cast<long long>(kLowerBounds[bucketIndex]));
        EXPECT_EQ(field["count"].Long(), (bucketIndex < kMaxBuckets - 1) ? 3 : 2);
        ++bucketIndex;
    }
}
