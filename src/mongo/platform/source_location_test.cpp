 ...
 */
#if defined(_MSC_VER)
constexpr bool isBadMsvc = true;
#else
constexpr bool isBadMsvc = false;
#endif
...
TEST(SourceLocation, DefaultStructMember) {
    ...
    Obj o{};
    // MSVC incorrectly chooses site of the data member definition.
    ASSERT_EQ(o.loc.line(), isBadMsvc ? o.memberLine : o.ctorLine);
}
...
TEST(SourceLocation, FunctionReportsCaller) {
    ...
    // MSVC incorrectly chooses the function source line.
    ASSERT_EQ(reported, isBadMsvc ? someFunctionSourceLine : callSiteLine);
}
...
TEST(SourceLocation, ConstructorReportsCaller) {
    ...
    auto callSiteLine = __LINE__ - 1;
    // MSVC incorrectly chooses the ctor source line.
    ASSERT_EQ(reported, isBadMsvc ? SomeClass::ctorSiteLine : callSiteLine);
}
