...
TEST_F(WriteRarelyRWMutexTest, WriterWaitsForReaders) {
    AtomicWord<bool> didWrite(false);
...
TEST_F(WriteRarelyRWMutexTest, MultiWriter) {
    ...
    size_t counter = 0;
    AtomicWord<int> activeWriters(0);
...
TEST(RWMutex, MultipleReadersAndWriters) {
    ...
    RWMutex mutex;
    AtomicWord<size_t> counter{0};
    AtomicWord<int> readers, writers;
