...
TEST_F(ConfigOnlyServerSelectorTest, ShouldReturnNoneWhenAllNodesAreInjectors) {
    AllInjectorsTopology topo(sdamConfiguration);

    for (auto pref : {ReadPreference::PrimaryOnly,
                      ReadPreference::SecondaryOnly,
                      ReadPreference::PrimaryPreferred,
                      ReadPreference::SecondaryPreferred,
                      ReadPreference::Nearest}) {
        auto result =
            selector.selectServers(topo.topologyDescription, ReadPreferenceSetting(pref), {});
        ASSERT_EQ(adaptForAssert(boost::none), adaptForAssert(result));
    }
}
