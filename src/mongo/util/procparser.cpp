...
Status parseProcSelfStatus(const std::vector<StringData>& keys,
    ...
    // Split the file by lines.
    // token_compress_on means the iterator skips over consecutive '\n'. This should not be a
    // problem in normal /proc/self/status output.
    for (string_split_iterator lineIt = string_split_iterator(
             data.begin(),
             data.end(),
             boost::token_finder([](char c) { return c == '\n'; }, boost::token_compress_on));
        ...
        // Split the line by colons since these are the delimiters for status files.
        string_split_iterator partIt = string_split_iterator(
        ...
        // Check if the key is in the list. /proc/self/status will have extra keys, and may not have
        // the keys we want.
        if (keys.empty() || std::find(keys.begin(), keys.end(), key) != keys.end()) {
            foundKeys = true;

            // Remove leading whitespace
            auto it = partIt->begin();
            while (*it == ' ' || *it == '\t') {
                ++it;
            }

            StringData stringValue(it, partIt->end());
    ...
    return foundKeys ? Status::OK()
                     : Status(ErrorCodes::NoSuchKey, "Failed to find any keys in status string");
}
...
Status parseProcSelfStatusFile(StringData filename,
    ...
    return parseProcSelfStatus(keys, swString.getValue(), builder);
}
