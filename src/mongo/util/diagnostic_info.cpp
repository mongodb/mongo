...
auto DiagnosticInfo::maybeMakeBlockedOpForTest(Client* client) -> std::unique_ptr<BlockedOpGuard> {
    ...
    MONGO_FAIL_POINT_BLOCK_IF(currentOpSpawnsThreadWaitingForLatch, data, [&](const BSONObj& data) {
        return data.hasField("clientName") && (data.getStringField("clientName") == client->desc());
    }) {
