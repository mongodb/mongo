...
private:
    ...
    MutexType _mutex;  // NOLINT(mongo-mutex-check)
    LockerType _locker;
};

BENCHMARK_TEMPLATE_DEFINE_F(BM_Mutex, MutexLockThenUnlock, std::mutex, ExclusiveLocker)  // NOLINT
(benchmark::State& s) {
    runLockThenUnlock(s);
}
...
BENCHMARK_TEMPLATE_DEFINE_F(BM_Mutex,
                            SharedMutexLockThenUnlock,
                            std::shared_mutex,  // NOLINT
                            SharedLocker)
(benchmark::State& s) {
    runLockThenUnlock(s);
}
...
BENCHMARK_TEMPLATE_DEFINE_F(BM_Mutex, MutexLockSleepUnlock, std::mutex, ExclusiveLocker)  // NOLINT
(benchmark::State& s) {
