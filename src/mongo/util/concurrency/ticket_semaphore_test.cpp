 ...
 */
TEST_P(TicketSemaphoreTest, ComplexPermitForwardingWithMixedOutcomes) {
    auto sem = makeSemaphore(1);
    auto* rawSem = sem.get();
    ...
    auto client1 = getServiceContext()->getService()->makeClient("waiter1");
    auto opCtx1 = client1->makeOperationContext();
    auto* rawOpCtx1 = opCtx1.get();

    auto client2 = getServiceContext()->getService()->makeClient("waiter2");
    auto opCtx2 = client2->makeOperationContext();
    auto* rawOpCtx2 = opCtx2.get();

    auto client3 = getServiceContext()->getService()->makeClient("waiter3");
    auto opCtx3 = client3->makeOperationContext();
    auto* rawOpCtx3 = opCtx3.get();

    // Waiter 1: will timeout quickly.
    auto deltaTime = Milliseconds{100};
    auto shortDeadline = Date_t::now() + deltaTime;
    Future<bool> waiter1 = spawn([&, rawSem, rawOpCtx1, shortDeadline]() {
        MockAdmissionContext admCtx;
        return rawSem->acquire(rawOpCtx1, &admCtx, shortDeadline, true /* interruptible */);
    });

    waitUntilBlocked(rawSem, 1);

    // Waiter 2: will be interrupted.
    Future<void> waiter2 = spawn([&, rawSem, rawOpCtx2]() {
        MockAdmissionContext admCtx;
        rawSem->acquire(rawOpCtx2, &admCtx, Date_t::max(), true /* interruptible */);
    });

    waitUntilBlocked(rawSem, 2);

    // Waiter 3: will succeed eventually.
    Future<bool> waiter3 = spawn([&, rawSem, rawOpCtx3]() {
        MockAdmissionContext admCtx;
        return rawSem->acquire(rawOpCtx3, &admCtx, getDeadline(), true /* interruptible */);
    });

    waitUntilBlocked(rawSem, 3);
    sleepFor(deltaTime);

    opCtx2->markKilled(ErrorCodes::Interrupted);

    // Release the permit - it should be forwarded to one of the waiters.
    rawSem->release();

    bool result1 = true;
    _opCtx->runWithDeadline(getDeadline(), ErrorCodes::ExceededTimeLimit, [&] {
        result1 = std::move(waiter1).get(_opCtx.get());
    });
    ASSERT_FALSE(result1);

    ASSERT_THROWS_CODE(([&] {
                           _opCtx->runWithDeadline(getDeadline(),
                                                   ErrorCodes::ExceededTimeLimit,
                                                   [&] { std::move(waiter2).get(_opCtx.get()); });
                       })(),
                       DBException,
                       ErrorCodes::Interrupted);

    // The permit should eventually reach waiter 3 (may need another release if
    // the forwarded permit was consumed by one that then failed).
    // Release to ensure waiter3 can proceed.
    rawSem->release();

    bool result3 = false;
    _opCtx->runWithDeadline(getDeadline(), ErrorCodes::ExceededTimeLimit, [&] {
        result3 = std::move(waiter3).get(_opCtx.get());
    });
    ASSERT_TRUE(result3);

    ASSERT_EQ(rawSem->waiters(), 0);
    // We started with 1 permit, acquired 1, released 2, waiter3 acquired 1.
    // Expected available: 1 - 1 + 2 - 1 = 1
