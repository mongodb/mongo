...
TEST_F(ThreadPoolTest, MinPoolSize0) {
    ...
    ASSERT_EQ(options.minThreads, stats.numThreads) << "Failed to reap all threads after " <<
        durationCount<Milliseconds>(reapTime) << "ms";
    lk.lock();
    flag2 = false;
    count1 = 0;
    ASSERT_OK(pool.schedule([this] { blockingWork(); }));
...
TEST_F(ThreadPoolTest, MaxPoolSize20MinPoolSize15) {
    ...
    const Microseconds reapTime(reapTimer.micros());
    ASSERT_EQ(options.minThreads, stats.numThreads) << "Failed to reap excess threads after " <<
        durationCount<Milliseconds>(reapTime) << "ms";
}
...
DEATH_TEST(ThreadPoolTest,
           DieWhenExceptionBubblesUp,
           "Exception escaped task in thread pool") {
    ThreadPool pool((ThreadPool::Options()));
    pool.startup();
...
DEATH_TEST(ThreadPoolTest,
    ...
    stdx::thread t([&pool] { pool->shutdown(); pool->join(); });
