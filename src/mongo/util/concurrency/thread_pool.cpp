...
Status ThreadPool::schedule(Task task) {
    ...
    case joining:
    case shutdownComplete:
        return Status(ErrorCodes::ShutdownInProgress, str::stream() <<
                      "Shutdown of thread pool " << _options.poolName << " in progress");
    case preStart:
    case running:
        break;
    }
...
void ThreadPool::_consumeTasks() {
    ...
    while (_state == running) {
        if (_pendingTasks.empty()) {
            if (_threads.size() > _options.minThreads) {
                ...
                _workAvailable.wait_until(lk, nextThreadRetirementDate);
            }
            else {
    ...
    severe().stream() << "Could not find this thread, with id " <<
        stdx::this_thread::get_id() << " in pool " << _options.poolName;
    fassertFailedNoTrace(28697);
}
...
void ThreadPool::_startWorkerThread_inlock() {
    ...
    case joining:
    case shutdownComplete:
        LOG(1) << "Not starting new thread in pool " << _options.poolName << " while shutting down";
        return;
    case running:
        break;
    }
