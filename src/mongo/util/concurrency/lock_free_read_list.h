...
public:
    ...
    ~LockFreeReadList() {
        for (auto current = _head.load(); current;) {
            invariant(current->readers() == 0);
            _allocator.delete_object(std::exchange(current, current->next()));
        }
    }
    ...
    Entry* add(T data) noexcept {
        ...
        auto entry = _allocator.template new_object<EntryImpl>(_head.load(), std::move(data));
        _head.store(entry);
...
private:
    ...
    std::pmr::polymorphic_allocator<Entry> _allocator{&_mbr};
    std::pmr::list<EntryImpl*> _freeList;
