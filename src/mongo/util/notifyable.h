 ...
 * NotifyableParkingLot::notify(One|All).
 *
...
private:
    ...
    // waiting on one.  It's important that we pre-allocate this entry on construction.
    //
...
public:
    ...
    bool notifyOne() noexcept {
        if (MONGO_unlikely(_notifyableCount.load())) {
    ...
    /**
     * Runs the callback with the Notifyable registered in this parking lot. This ensures that for
     * the duration of the callback execution, a notification on the parking lot will trigger a
     * notify() on the Notifyable.
     *
     * The scheme here is that list entries are spliced back to their Notifyable from the
     * notification list when notified (so that they don't eat multiple notify_one's).  We
     * detect that condition by noting that our list isn't empty (in which case we should avoid
     * a double splice back).
     */
    ...
    void runWithNotifyable(Notifyable& notifyable, Callback&& cb) noexcept {
        ...
        // The supplied callback should perform a wait which returns when the Notifyable is
...
private:
    ...
    bool _notifyNextNotifyable(WithLock wl) noexcept {
        ...
        if (iter == _notifyables.end()) [[likely]] {
            return false;
        }
    ...
    Atomic<unsigned long long> _notifyableCount;
};
