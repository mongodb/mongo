...
namespace merizo {
/**
 * A simple class for mocking merizo::DBClientConnection.
 *
 * Also check out sample usage in dbtests/mock_dbclient_conn_test.cpp
 */
class MockDBClientConnection : public merizo::DBClientConnection {
public:
    /**
    ...
    std::unique_ptr<merizo::DBClientCursor> query(const NamespaceStringOrUUID& nsOrUuid,
    ...
    merizo::ConnectionString::ConnectionType type() const override;
    ...
    bool call(merizo::Message& toSend,
              merizo::Message& response,
              bool assertOk,
              std::string* actualServer) override;
    void say(merizo::Message& toSend, bool isRetry = false, std::string* actualServer = 0) override;
