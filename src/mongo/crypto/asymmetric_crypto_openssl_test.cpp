...
public:
    ...
    void evaluate(RSAKeySignatureVerificationVector test) {
        ...
        StringData key = StringData(test.keyID.data(), test.keyID.length());
        StringData sdE = StringData(test.e.data(), test.e.length());
        StringData sdN = StringData(test.n.data(), test.n.length());

        RsaPublicKey rsaKey(key, sdE, sdN);
        ...
        if (test.shouldPass == false) {
            ASSERT_NOT_OK(asymmetricKey->verifySignature(
            ...
            ASSERT_OK(asymmetricKey->verifySignature(
                ConstDataRange(test.msg.data(), test.msg.length()),
                ConstDataRange(test.signature.data(), test.signature.length())));
        }
    }
