...
private:
    class CaptureSink {
    public:
        using VecType = synchronized_value<std::vector<std::string>>;
        using UpdateGuardType = VecType::UpdateGuard<VecType>;
        using ConstUpdateGuardType = VecType::UpdateGuard<const VecType>;

        explicit CaptureSink(auto formatter) {
            _sink = logv2::LogCaptureBackend::create(std::make_unique<Listener>(this), true);
        ...
        void attach() {
            _sink->locked_backend()->setEnabled(true);
        ...
        void detach() {
            ...
            _sink->locked_backend()->setEnabled(false);
            boost::log::core::get()->remove_sink(_sink);
        }

        UpdateGuardType sync() {
            return *_vec;
        }
        ConstUpdateGuardType sync() const {
            return *_vec;
        }
    ...
    private:
        ...
        public:
            explicit Listener(CaptureSink* owner) : _owner(owner) {}

            void accept(const std::string& line) override {
                _owner->sync()->push_back(line);
            }
        ...
        VecType _vec;
        boost::shared_ptr<boost::log::sinks::unlocked_sink<logv2::LogCaptureBackend>> _sink;
    };
    ...
    bool _active = false;
    CaptureSink _text{logv2::PlainFormatter()};
    CaptureSink _bson{logv2::BSONFormatter()};
};
