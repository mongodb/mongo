...
// Appends string content to an existing file with path and type validation. The filepath is
// supplied in the type of the first element in `args` and the string content is supplied in the
// value of the first element in `args`.
BSONObj appendFile(const BSONObj& args, void* data) {
    ...
    uassert(9380004,
            "appendFile() can only append to a file in a directory that already exists",
            boost::filesystem::exists(absoluteFilePath.parent_path()));
    uassert(9380005,
