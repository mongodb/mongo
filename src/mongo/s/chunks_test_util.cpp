...
void assertEqualChunkInfo(const ChunkInfo& x, const ChunkInfo& y) {
    ...
    ASSERT_EQ(x.getShardIdAt(boost::none), y.getShardIdAt(boost::none));
    ASSERT_EQ(x.getLastmod(), y.getLastmod());
    ASSERT_EQ(x.isJumbo(), y.isJumbo());
    ASSERT_EQ(x.getWritesTracker()->getBytesWritten(), y.getWritesTracker()->getBytesWritten());
}
...
std::vector<ChunkVersion> genRandomVersions(size_t num, const ChunkVersion& initialVersion) {
    ...
    for (size_t i = 0; i < num; ++i) {
        ...
        versions.emplace_back(major, minor, initialVersion.epoch());
    }
...
std::vector<ChunkType> genChunkVector(const NamespaceString& nss,
    ...
    return genChunkVector(
        nss, splitPoints, genRandomVersions(splitPoints.size() - 1, initialVersion), numShards);
}
...
std::vector<ChunkType> genRandomChunkVector(const NamespaceString& nss,
    ...
    const ChunkVersion initialVersion{1, 0, epoch};

    log() << "Generating random chunk vector. {numChunks: " << numChunks
          << ", numShards: " << numShards << "}";

    return genChunkVector(nss, genRandomSplitPoints(numChunks), initialVersion, numShards);
}
