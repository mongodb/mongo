...
BSONElement getFirstFieldWithIncompatibleCollation(const BSONObj& shardKey,
    ...
    for (BSONElement elt : shardKey) {
        ...
        // used as the shard key, set permitHashedFields. This assumes that a request with
...
bool isEqualityOnShardKeyTargetable(const BSONObj& shardKey,
                                    const ShardKeyPattern& shardKeyPattern,
                                    bool queryHasSimpleCollation) {
    return getFirstFieldWithIncompatibleCollation(
               shardKey, shardKeyPattern, queryHasSimpleCollation, /* permitHashedFields */ false)
        .eoo();
}
}  // namespace mongo
