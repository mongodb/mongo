...
protected:
    void setUp() {
        ...
        Status status = loader.makeCollectionMetadata("test.foo", "shard0000", NULL, &_metadata);
        ASSERT(status.isOK());
...
TEST_F(SingleChunkFixture, LastChunkMinusCantHaveNonZeroVersion) {
    ...
    scoped_ptr<CollectionMetadata> cloned(getCollMetadata().cloneMigrate(chunk, version, &errMsg));
    ...
    void setUp() {
        ...
        _dummyConfig->insert(
            ...
            BSON(CollectionType::ns("x.y")
                 << CollectionType::dropped(false) << CollectionType::keyPattern(BSON("a" << 1))
                 << CollectionType::unique(false) << CollectionType::updatedAt(1ULL)
