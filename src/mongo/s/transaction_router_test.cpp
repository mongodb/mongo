...
TEST_F(TransactionRouterTestWithDefaultSession, FirstParticipantIsCoordinator) {
    ...
    {
        txnRouter.attachTxnFieldsIfNeeded(operationContext(), shard1, {}, "db");
    ...
    {
        txnRouter.attachTxnFieldsIfNeeded(operationContext(), shard2, {}, "db");
...
TEST_F(TransactionRouterTestWithDefaultSession, SnapshotErrorsClearsAllParticipants) {
    ...
    {
        auto newCmd = txnRouter.attachTxnFieldsIfNeeded(operationContext(), shard2, {}, "db");
        ASSERT_TRUE(newCmd["startTransaction"].trueValue());

        newCmd = txnRouter.attachTxnFieldsIfNeeded(operationContext(), shard2, {}, "db");
...

    {
        ...
        auto newCmd = txnRouter.attachTxnFieldsIfNeeded(operationContext(), shard1, {}, "db");
        ASSERT_TRUE(newCmd["startTransaction"].trueValue());

        newCmd = txnRouter.attachTxnFieldsIfNeeded(operationContext(), shard1, {}, "db");
        ASSERT_FALSE(newCmd["startTransaction"].trueValue());
    }
}
...
TEST_F(TransactionRouterTestWithDefaultSession, ParticipantsRememberStmtIdCreatedAt) {
    ...
    txnRouter.attachTxnFieldsIfNeeded(operationContext(), shard3, {}, "db");
...
TEST_F(TransactionRouterTestWithDefaultSession, OnlyNewlyCreatedParticipantsClearedOnStaleError) {
    ...
    ASSERT_FALSE(
        txnRouter.attachTxnFieldsIfNeeded(operationContext(), shard1, {}, "db")["startTransaction"]
            .trueValue());
    ASSERT_TRUE(
        txnRouter.attachTxnFieldsIfNeeded(operationContext(), shard2, {}, "db")["startTransaction"]
            .trueValue());
    ASSERT_TRUE(
        txnRouter.attachTxnFieldsIfNeeded(operationContext(), shard3, {}, "db")["startTransaction"]
            .trueValue());
}
...
TEST_F(TransactionRouterTestWithDefaultSession, OnViewResolutionErrorClearsAllNewParticipants) {
    ...
    auto firstShardCmd = txnRouter.attachTxnFieldsIfNeeded(operationContext(), shard1, {}, "db");
    ...
    firstShardCmd = txnRouter.attachTxnFieldsIfNeeded(operationContext(), shard1, {}, "db");
    ASSERT_TRUE(firstShardCmd["startTransaction"].trueValue());
    ...
    auto secondShardCmd = txnRouter.attachTxnFieldsIfNeeded(operationContext(), shard2, {}, "db");
    ...
    secondShardCmd = txnRouter.attachTxnFieldsIfNeeded(operationContext(), shard2, {}, "db");
    ASSERT_TRUE(secondShardCmd["startTransaction"].trueValue());
}
...
TEST_F(TransactionRouterTestWithDefaultSession, AbortPropagatesWriteConcern) {
    ...
    txnRouter.attachTxnFieldsIfNeeded(opCtx, shard1, {}, "db");
...
TEST_F(TransactionRouterTestWithDefaultSession, DetectsConflictsDueToMovePrimary) {
    ...
    enum class TestCase { NO_TIMESTAMP, PRE_50_LAST_MOVED_TIMESTAMP, DATABASE_VERSION_TIMESTAMP };

    const auto runTest = [&](TestCase testCase) {
        const auto setCatalogCache = [&](boost::optional<Timestamp> databaseTimestamp,
            ...
            expectFindSendBSONObjVector(kTestConfigShardHost, [&]() {
                DatabaseType db(kDbName,
                                shard1,
                                false,
                                DatabaseVersion(UUID::gen(), databaseTimestamp),
                                lastMovedTimestampPre50);
        ...
        // Note the fixture sets VectorClock::clusterTime to Timestamp(3, 1). This will get used by
        // the transaction as 'atClusterTime'/'placementConflictTime'.

        // Database timestamp is valid for the transaction.
        switch (testCase) {
            case TestCase::DATABASE_VERSION_TIMESTAMP:
                setCatalogCache(Timestamp(2, 0), boost::none);
                break;
            case TestCase::PRE_50_LAST_MOVED_TIMESTAMP:
                setCatalogCache(boost::none, Timestamp(2, 0));
                break;
            case TestCase::NO_TIMESTAMP:
                setCatalogCache(boost::none, boost::none);
                break;
        }
        ...
        switch (testCase) {
            case TestCase::DATABASE_VERSION_TIMESTAMP:
                setCatalogCache(Timestamp(4, 0), boost::none);
                break;
            case TestCase::PRE_50_LAST_MOVED_TIMESTAMP:
                setCatalogCache(boost::none, Timestamp(4, 0));
                break;
            case TestCase::NO_TIMESTAMP:
                setCatalogCache(boost::none, boost::none);
                break;
    ...
    runTest(TestCase::PRE_50_LAST_MOVED_TIMESTAMP);
    runTest(TestCase::DATABASE_VERSION_TIMESTAMP);
