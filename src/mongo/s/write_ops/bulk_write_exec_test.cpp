...
TEST_F(BulkWriteOpTest, TargetErrorsInTxn) {
    ...
    auto [replies, summaryFields, _, __] = bulkWriteOp.generateReplyInfo();
...
TEST_F(BulkWriteOpTest, SuccessfulShardRepliesAreSavedAfterRetargeting) {
    ...
    auto [replies, summaryFields, _, __] = op.generateReplyInfo();
    ASSERT_EQ(replies.size(), 1);
...
TEST_F(BulkWriteOpWithoutShardKeyWithIdTest, NoShardFindsMatch) {
    ...
    auto& childOps = updateOp.getChildWriteOps_forTest();
    // Since we only got one reply so far and it was n=0, that child op should be deferred.
    ASSERT_EQ(childOps[0].state, WriteOpState_Deferred);
    ASSERT_EQ(childOps[1].state, WriteOpState_Pending);
    ASSERT_EQ(childOps[2].state, WriteOpState_Pending);
    ...
    ASSERT_EQ(childOps[0].state, WriteOpState_Deferred);
    ASSERT_EQ(childOps[1].state, WriteOpState_Deferred);
    ASSERT_EQ(childOps[2].state, WriteOpState_Pending);
    ...
    // All of the child ops should have been transitioned to deferred now.
    ASSERT_EQ(childOps[0].state, WriteOpState_Deferred);
    ASSERT_EQ(childOps[1].state, WriteOpState_Deferred);
    ASSERT_EQ(childOps[2].state, WriteOpState_Deferred);
    ASSERT(op.isFinished());
...
TEST_F(BulkWriteOpWithoutShardKeyWithIdTest, SecondShardFindMatch) {
    ...
    ASSERT(op.isFinished());
    // The first op should be marked deferred, the second complete, and the third should
    // have been marked no-op as we decided not to wait for a reply.
    ASSERT_EQ(childOps[0].state, WriteOpState_Deferred);
    ASSERT_EQ(childOps[1].state, WriteOpState_Completed);
    ASSERT_EQ(childOps[2].state, WriteOpState_NoOp);
...
TEST_F(BulkWriteOpWithoutShardKeyWithIdTest, FirstShardFindMatch) {
    ...
    ASSERT_EQ(updateOp.getWriteState(), WriteOpState_Completed);
    auto& childOps = updateOp.getChildWriteOps_forTest();
    // The write we got a reply for should be completed and the others marked no-ops.
    ASSERT_EQ(childOps[0].state, WriteOpState_Completed);
    ASSERT_EQ(childOps[1].state, WriteOpState_NoOp);
    ASSERT_EQ(childOps[2].state, WriteOpState_NoOp);
...
TEST_F(BulkWriteOpWithoutShardKeyWithIdTest, MatchAndRetryableError) {
    ...
    ASSERT_EQ(childOps[0].state, WriteOpState_Deferred);
    ASSERT_EQ(childOps[1].state, WriteOpState_Pending);
    ASSERT_EQ(childOps[2].state, WriteOpState_Error);
    ASSERT_EQ(childOps[2].error->getStatus().code(), ErrorCodes::StaleConfig);
