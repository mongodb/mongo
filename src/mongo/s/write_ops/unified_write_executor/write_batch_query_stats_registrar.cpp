...
void parseAndRegisterUpdateOp(OperationContext* opCtx,
    ...
    updateRequest.setUpdateModification(updateOp.getUpdateMods());
    updateRequest.setQuery(updateOp.getFilter());
    updateRequest.setUpdateConstants(updateOp.getConstants());
    if (updateOp.getArrayFilters()) {
        updateRequest.setArrayFilters(*updateOp.getArrayFilters());
    }
    updateRequest.setMulti(updateOp.getMulti());
    updateRequest.setUpsert(updateOp.getUpsert());
    if (updateOp.getCollation()) {
        updateRequest.setCollation(*updateOp.getCollation());
    }
