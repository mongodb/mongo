...
public:
    ...
    /**
     * Returns the StaleDbVersion receivedVersion observed during the most recent call to
     * onWriteBatchResponse(), keyed by database name. Only the first stale error per database is
     * recorded; a single entry is sufficient to detect productive database-version refreshes.
     */
    const stdx::unordered_map<DatabaseName, DatabaseVersion>& getLastRoundStaleDbVersions() const {
        return _lastRoundStaleDbVersions;
    }
...
private:
    ...
    // noteRetryableError(). Only the first stale error per database is recorded.
    stdx::unordered_map<DatabaseName, DatabaseVersion> _lastRoundStaleDbVersions;
};
