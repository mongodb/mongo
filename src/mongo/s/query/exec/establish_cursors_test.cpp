...
// Reproduces SERVER-123598: when one shard returns a non-retriable cursor error, stopRetrying()
// cancels any pending retry backoffs on other shards, generating CallbackCanceled. Before the fix,
// CallbackCanceled would incorrectly clobber the original non-retargeting error.
TEST_F(EstablishCursorsTest, CallbackCanceledDoesNotClobberNonRetargetingError) {
    ...
    auto future = launchAsync([&] {
        // The original non-retriable error must be thrown, not a CallbackCanceled produced by
        // stopRetrying() cancelling the first shard's pending backoff.
        ASSERT_THROWS(establishCursors(operationContext(),
                                       executor(),
                                       _nss,
                                       ReadPreferenceSetting{ReadPreference::PrimaryOnly},
    ...
    // First remote responds with a system-overload error. The SystemOverloadedError label triggers
    // incrementAttemptCount() in DefaultRetryStrategy so that the subsequent getNextRetryDelay()
    // returns the non-zero value supplied by setBackoffDelayForTesting. The ARS then registers a
    // waitUntil() alarm on the baton with the cancellation token.
    onCommand([this](const RemoteCommandRequest& request) {
        ASSERT_EQ(_nss.coll(), request.cmdObj.firstElement().valueStringData());
        return createErrorSystemOverloaded(ErrorCodes::HostUnreachable);
    });

    // Second remote responds with a non-retriable cursor error. _handleFailure() stores
    // FailedToParse as _maybeFailure (isInterruption=false path, since the error comes from cursor
    // response parsing) and calls stopRetrying(). stopRetrying() cancels the cancellation token,
    // which resolves the first remote's pending waitUntil() with CallbackCanceled. That
    // CallbackCanceled response is then processed by _handleFailure() with isInterruption=true,
    // calling _prioritizeFailures(). Before the fix, _prioritizeFailures() would overwrite
    // _maybeFailure with CallbackCanceled; after the fix it correctly keeps FailedToParse.
    onCommand([this](const RemoteCommandRequest& request) {
        ASSERT_EQ(_nss.coll(), request.cmdObj.firstElement().valueStringData());
        return createErrorCursorResponse(Status(ErrorCodes::FailedToParse, "failed to parse"));
    });
