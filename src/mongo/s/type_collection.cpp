...
bool CollectionType::isValid(std::string* errMsg) const {
    ...
    int numSet = 0;
    if (_isPrimarySet && !_primary.empty())
        numSet++;
    if (_isKeyPatternSet && !(_keyPattern.nFields() == 0))
        numSet++;
    if (_isDroppedSet && _dropped)
        numSet++;
    if (numSet != 1) {
        *errMsg = stream() << "one of " << primary.name() << " or " << keyPattern.name() << " or "
                           << dropped.name() << " should be filled";
        return false;
    }

    // Sharding related fields may only be set if the sharding key pattern is present, unless
    // we're dropped.
    if ((_unique || _noBalance) && (!_isDroppedSet || !_dropped) && (_keyPattern.nFields() == 0)) {
...
BSONObj CollectionType::toBSON() const {
    ...
    if (_isKeyPatternSet)
        builder.append(keyPattern(), _keyPattern);
    if (_isUniqueSet)
        builder.append(unique(), _unique);
    if (_isUpdatedAtSet)
        builder.append(updatedAt(), _updatedAt);
    if (_isNoBalanceSet)
        builder.append(noBalance(), _noBalance);
