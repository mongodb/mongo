...
Status checkClusterMongoVersions(const ConnectionString& configLoc, const string& minMongoVersion) {
    ...
    try {
        ...
        scoped_ptr<DBClientCursor> cursor(_safeCursor(conn->query(MongosType::ConfigNS, Query())));
    ...
    try {
        ScopedDbConnection& conn = *connPtr;
        scoped_ptr<DBClientCursor> cursor(_safeCursor(conn->query(ShardType::ConfigNS, Query())));
...
Status _findAllCollections(const ConnectionString& configLoc,
    ...
    try {
        ...
        scoped_ptr<DBClientCursor> cursor(
            _safeCursor(conn->query(CollectionType::ConfigNS, Query())));
...
Status findAllChunks(const ConnectionString& configLoc,
    ...
    try {
        ...
        ScopedDbConnection& conn = *connPtr;
        scoped_ptr<DBClientCursor> cursor(
            _safeCursor(conn->query(ChunkType::ConfigNS, BSON(ChunkType::ns(ns)))));
...
Status logConfigChange(const ConnectionString& configLoc,
    ...
    Status result = clusterInsert(
        ChangelogType::ConfigNS, changelog.toBSON(), WriteConcernOptions::AllConfigs, NULL);

    if (!result.isOK()) {
        return Status(result.code(),
                      str::stream() << "failed to write to changelog: " << result.reason());
    }
