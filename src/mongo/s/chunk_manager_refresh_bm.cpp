...
void BM_IncrementalSpacedRefreshMoveChunks(benchmark::State& state) {
    ...
    auto lastVersion = metadata.getChunkManager()->getVersion();
...
template <typename CollectionMetadataBuilderFn>
void BM_replaceTimestampPessimalBalancedDistribution(
    benchmark::State& state, CollectionMetadataBuilderFn makeCollectionMetadata) {
    const int nShards = state.range(0);
    const int nChunks = state.range(1);
    const auto metadata = makeCollectionMetadata(nShards, nChunks);

    const auto makeUpdatedReplacingTimestamp = [&](const CollectionMetadata& cm) {
        auto rt =
            cm.getChunkManager()->getRoutingTableHistory_ForTest().makeUpdatedReplacingTimestamp(
                Timestamp(123));
        return CollectionMetadata(ChunkManager(getShardId(0),
                                               DatabaseVersion(UUID::gen()),
                                               makeStandaloneRoutingTableHistory(std::move(rt)),
    ...
    for (auto keepRunning : state) {
        benchmark::DoNotOptimize(makeUpdatedReplacingTimestamp(metadata));
    }
}
...
MONGO_INITIALIZER(RegisterBenchmarks)(InitializerContext* context) {
    std::initializer_list<benchmark::internal::Benchmark*> bmCases{
        ...
        REGISTER_BENCHMARK_CAPTURE(BM_replaceTimestampPessimalBalancedDistribution,
                                   Optimal,
                                   makeChunkManagerWithOptimalBalancedDistribution),
        REGISTER_BENCHMARK_CAPTURE(BM_replaceTimestampPessimalBalancedDistribution,
                                   Pessimal,
