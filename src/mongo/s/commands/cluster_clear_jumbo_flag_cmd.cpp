    ...
    private:
        ...
        void run(OperationContext* opCtx, CommandReplyBuilder* result) override {
            ...
            if (request().getFind()) {
                ...
                uassert(51260,
                        str::stream() << "no shard key found in chunk query "
                                      << *request().getFind(),
                        !shardKey.isEmpty());
            ...
            ConfigsvrClearJumboFlag configCmd(ns());
            configCmd.setEpoch(cm->getVersion().epoch());
            configCmd.setMinKey(chunk->getMin());
            configCmd.setMaxKey(chunk->getMax());
