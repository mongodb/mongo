...
public:
    ...
    // On merizoS, the 'profile' command is only used to change the global 'slowms' and 'sampleRate'
    // parameters. Since it does not apply to any specific database but rather the merizoS as a
...
protected:
    int _applyProfilingLevel(OperationContext* opCtx,
        ...
        // Because merizoS does not allow profiling, but only uses the 'profile' command to change
        ...
        uassert(ErrorCodes::BadValue,
                "Profiling is not permitted on merizoS: the 'profile' field should be 0 to change "
