...
BSONObj TransactionRouter::Participant::attachTxnFieldsIfNeeded(
    BSONObj cmd, bool isFirstStatementInThisParticipant, bool hasTxnCreatedAnyDatabase) const {
...
BSONObj TransactionRouter::Router::attachTxnFieldsIfNeeded(OperationContext* opCtx,
    ...
    if (auto txnPart = getParticipant(shardId)) {
        ...
        return txnPart->attachTxnFieldsIfNeeded(cmdObj, false, hasTxnCreatedAnyDatabase);
    }
    ...
    return txnPart.attachTxnFieldsIfNeeded(cmdObj, true, hasTxnCreatedAnyDatabase);
}
