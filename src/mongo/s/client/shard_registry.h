...
public:
    ...
    /**
     * Returns true if the config server is in the shard registry, and false otherwise. This
     * function should only be used for serverStatus metric reporting where a result that is stale
     * by a few seconds is okay. This function will not refresh the shard registry or otherwise
     * perform any network traffic.
     */
