...
void ChunkMap::_updateShardVersionFromDiscardedChunk(const ChunkInfo& chunk) {
    ...
    if (shardVersionIt != _shardVersions.end() && shardVersionIt->second == chunk.getLastmod()) {
...
void ChunkMap::_updateShardVersionFromUpdateChunk(const ChunkInfo& chunk) {
    ...
    auto [shardVersionIt, created] =
        _shardVersions.try_emplace(chunk.getShardIdAt(boost::none), chunkVersion);

    if (created || shardVersionIt->second < chunkVersion) {
        // Update shard version with the most recent one from new chunk
        shardVersionIt->second = chunkVersion;
        if (_collectionVersion < chunkVersion) {
