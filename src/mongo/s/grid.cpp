            ...
            if (create) {
                ...
                try {
...

                    {
                        ...
                        if (!dbObj.isEmpty()) {
                            uasserted(DatabaseDifferCaseCode,
                                      str::stream()
...
bool Grid::addShard(string* name,
    ...
    try {
        ...
        if (newShardConn->type() == ConnectionString::SYNC) {
            newShardConn.done();
            errMsg =
                "can't use sync cluster as a shard.  for replica set, have to use "
        ...
        if (commandSetName.empty() && !setName.empty()) {
            ostringstream ss;
            ss << "host is part of set " << setName
        ...
        if (!commandSetName.empty() && setName.empty()) {
            ostringstream ss;
            ss << "host did not return a set name, is the replica set still initializing? "
               << resIsMaster;
            errMsg = ss.str();
            newShardConn.done();
            return false;
        ...
        if (setName.empty()) {
            ...
            if (!ok && res["info"].type() == String && res["info"].String() == "configsvr") {
                errMsg =
                    "the specified mongod is a --configsvr and should thus not be a shard server";
                newShardConn.done();
                return false;
            }
        }
    ...
    b.append(ShardType::host(), rsMonitor ? rsMonitor->getServerAddress() : servers.toString());
    if (maxSize > 0) {
...

    {
        ...
        BSONObj old = conn->findOne(ShardType::ConfigNS, BSON(ShardType::host(servers.toString())));
    ...
    Status result =
        clusterInsert(ShardType::ConfigNS, shardDoc, WriteConcernOptions::AllConfigs, NULL);
    ...
    for (vector<string>::const_iterator it = dbNames.begin(); it != dbNames.end(); ++it) {
        ...
        if (!config) {
            log() << "adding shard " << servers << " even though could not add database " << *it
                  << endl;
        }
    }
...
bool Grid::shouldBalance(const SettingsType& balancerSettings) const {
    ...
    if (MONGO_FAIL_POINT(neverBalance))
        return false;
...
bool Grid::getBalancerSettings(SettingsType* settings, string* errMsg) const {
    ...
    try {
        balancerDoc = conn->findOne(SettingsType::ConfigNS, BSON(SettingsType::key("balancer")));
        conn.done();
...
BSONObj Grid::getConfigSetting(const std::string& name) const {
    ScopedDbConnection conn(configServer.getPrimary().getConnString(), 30);
    BSONObj result = conn->findOne(SettingsType::ConfigNS, BSON(SettingsType::key(name)));
    conn.done();
