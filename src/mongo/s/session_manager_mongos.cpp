...
void SessionManagerMongos::onClientConnect(Client* client) {
    if (load_balancer_support::isFromLoadBalancer(client)) {
        _loadBalancedConnections.increment();
    }
}

void SessionManagerMongos::onClientDisconnect(Client* client) {
    if (load_balancer_support::isFromLoadBalancer(client)) {
...
void SessionManagerMongos::appendStats(BSONObjBuilder* bob) const {
