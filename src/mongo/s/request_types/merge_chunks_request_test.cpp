...
TEST(MergeChunksRequest, BasicValidConfigCommand) {
    ...
    auto request =
        assertGet(MergeChunksRequest::parseFromConfigCommand(BSON("_configsvrCommitChunksMerge"
                                                                  << "TestDB.TestColl"
                                                                  << "collUUID"
                                                                  << collUUID.toBSON()
                                                                  << "chunkRange"
...
TEST(MergeChunksRequest, WrongNamespaceTypeErrors) {
    auto collUUID = UUID::gen();
    auto request = MergeChunksRequest::parseFromConfigCommand(BSON(
        "_configsvrCommitChunksMerge" << 1234 << "collUUID" << collUUID.toBSON() << "chunkRange"
                                      << chunkRange.toBSON()
                                      << "shard"
                                      << "shard0000"));
