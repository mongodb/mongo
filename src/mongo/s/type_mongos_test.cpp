...
TEST(Validity, MissingName) {
    MongosType mongos;
    string errMsg;
    BSONObj obj = BSON(MongosType::ping(1ULL) << MongosType::up(100) << MongosType::waiting(false)
...
TEST(Validity, MissingUp) {
    ...
    string errMsg;
    BSONObj obj = BSON(MongosType::name("localhost:27017")
                       << MongosType::ping(1ULL) << MongosType::waiting(false)
                       << MongosType::mongoVersion("x.x.x") << MongosType::configVersion(0));
    ASSERT(mongos.parseBSON(obj, &errMsg));
    ASSERT_EQUALS(errMsg, "");
    ASSERT_TRUE(mongos.isNameSet());
...
TEST(Validity, MissingWaiting) {
    ...
    string errMsg;
    BSONObj obj = BSON(MongosType::name("localhost:27017")
                       << MongosType::ping(1ULL) << MongosType::up(100)
                       << MongosType::mongoVersion("x.x.x") << MongosType::configVersion(0));
    ASSERT(mongos.parseBSON(obj, &errMsg));
    ASSERT_EQUALS(errMsg, "");
    ASSERT_TRUE(mongos.isNameSet());
...
TEST(Validity, Valid) {
    ...
    BSONObj obj =
        BSON(MongosType::name("localhost:27017")
             << MongosType::ping(1ULL) << MongosType::up(100) << MongosType::waiting(false)
             << MongosType::mongoVersion("x.x.x") << MongosType::configVersion(0));
    string errMsg;
    ASSERT(mongos.parseBSON(obj, &errMsg));
    ASSERT_EQUALS(errMsg, "");
