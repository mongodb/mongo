...
std::map<ShardId, ChunkVersion> calculateShardVersions(
    ...
    for (const auto& chunk : chunkVector) {
        auto mapIt = svMap.find(chunk->getShardIdAt(boost::none));
        if (mapIt == svMap.end()) {
            svMap.emplace(chunk->getShardIdAt(boost::none), chunk->getLastmod());
            continue;
        }
...
void validateChunkMap(const ChunkMap& chunkMap,
    ...
    for (const auto& mapIt : shardVersions) {
        ASSERT_EQ(expectedShardVersions.at(mapIt.first), mapIt.second);
    }
...
public:
    ...
    ChunkMap makeChunkMap(const std::vector<std::shared_ptr<ChunkInfo>>& chunks) const {
        ...
        log() << "Creating new chunk map. {chunkBucketSize: " << chunkBucketSize << "}";
        return ChunkMap{collEpoch(), chunkBucketSize}.createMerged(chunks);
    }
...
TEST_F(ChunkMapTest, TestAddChunk) {
    ...
    ASSERT_EQ(newChunkMap.size(), 1ull);
...
TEST_F(ChunkMapTest, ConstructChunkMapMismatchingEpochs) {
    ...
    const ChunkVersion wrongVersion{
        oldVersion.majorVersion(), oldVersion.minorVersion(), wrongEpoch};
    chunkVector[wrongChunkIdx] = std::make_shared<ChunkInfo>(
        ChunkType{kNss, oldChunk->getRange(), wrongVersion, oldChunk->getShardIdAt(boost::none)});
...
TEST_F(ChunkMapTest, UpdateMapNotLeaveSmallVectors) {
    const ChunkVersion initialVersion{1, 0, collEpoch()};
    ...
    log() << "Creating new chunk map. {chunkBucketSize: " << chunkBucketSize << "}";
    const auto initialChunkMap = ChunkMap(collEpoch(), chunkBucketSize).createMerged(chunkVector);
...
TEST_F(ChunkMapTest, UpdateChunkMapMismatchingEpochs) {
    ...
    const auto oldChunk = chunkVector.at(wrongChunkIdx);
    const ChunkVersion wrongVersion{
        collVersion.majorVersion(), collVersion.minorVersion(), wrongEpoch};
...
TEST_F(ChunkMapTest, UpdateChunkMapRandom) {
    ...
    auto initialChunkMap = makeChunkMap(initialChunksInfo);
    ...
    for (auto& chunkPtr : chunksInfo) {
        ...
        if (initialCollVersion.isOlderThan(chunkPtr->getLastmod())) {
            updatedChunksInfo.push_back(
                std::make_shared<ChunkInfo>(ChunkType{kNss,
                                                      chunkPtr->getRange(),
                                                      chunkPtr->getLastmod(),
                                                      chunkPtr->getShardIdAt(boost::none)}));
        }
    }
