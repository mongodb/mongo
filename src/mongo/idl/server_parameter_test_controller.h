...
public:
    template <typename T>
    requires idl_server_parameter_detail::IsBSONAppendable<T> ||
        idl_server_parameter_detail::HasToBSON<T>
    ServerParameterControllerForTest(StringData name, T value) {
        ...
        if constexpr (idl_server_parameter_detail::IsBSONAppendable<T>) {
        ...
        } else if constexpr (idl_server_parameter_detail::HasToBSON<T>) {
    ...
    ServerParameterControllerForTest(ServerParameterControllerForTest&& other) noexcept
        : _serverParam(std::exchange(other._serverParam, nullptr)),
          _oldValue(std::move(other._oldValue)) {}

    ServerParameterControllerForTest& operator=(ServerParameterControllerForTest&& other) noexcept {
        if (&other == this) {
            return *this;
        }
