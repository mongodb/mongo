...
namespace mongo {
    ...
    MONGO_INITIALIZER_GENERAL(ParseStartupConfiguration,
        ...
        Status retStatus = addBSONDumpOptions(&toolsOptions);
        if (!retStatus.isOK()) {
            return retStatus;
        }
        ...
        if (!retStatus.isOK()) {
            ...
            printBSONDumpHelp(toolsOptions, &oss);
            return Status(ErrorCodes::FailedToParse, oss.str());
        }

        retStatus = handlePreValidationBSONDumpOptions(toolsParsedOptions);
        if (!retStatus.isOK()) {
            return retStatus;
        }
        ...
        retStatus = storeBSONDumpOptions(toolsParsedOptions, context->args());
        if (!retStatus.isOK()) {
            return retStatus;
        }
