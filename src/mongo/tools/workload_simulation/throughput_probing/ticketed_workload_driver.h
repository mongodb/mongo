...
protected:
    void _read(const std::stop_token& token, int32_t i);
    void _write(const std::stop_token& token, int32_t i);
...
protected:
    ...
    std::vector<std::jthread> _readWorkers;
    std::vector<std::jthread> _writeWorkers;
};
