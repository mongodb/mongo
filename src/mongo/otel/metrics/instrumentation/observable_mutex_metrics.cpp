...
private:
    ...
    void _addDeltas(TagState& state, const MutexStats& currStats) {
        const auto delta = [](uint64_t prev, uint64_t curr) -> int64_t {
            // If a mutex is destroyed and recreated, the counters will reset to 0.
            if (curr < prev) {
                return 0;
            }
            int64_t diff = static_cast<int64_t>(curr - prev);
            // If there is overflow on conversion, we will get a negative value. Return 0.
            if (diff < 0) {
                return 0;
            }
            return diff;
