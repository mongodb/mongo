...
SessionEstablishmentRateLimiter* SessionEstablishmentRateLimiter::get(ServiceContext& svcCtx) {
    SessionEstablishmentRateLimiter* rateLimiter = nullptr;
    bool asioSeen = false;

    if (auto tlm = svcCtx.getTransportLayerManager()) {
        tlm->forEach([&](TransportLayer* tl) {
            // On the master branch, we identify the TL according to its TransportProtocol.
            // As this does not exist on 8.0, we compare the TL's name instead.
            if (tl->getNameForLogging() == "asio") {
                if (auto sm = tl->getSessionManager()) {
                    massert(10640600, "Multiple AsioSessionManagers", !asioSeen);
                    asioSeen = true;
                    rateLimiter = &sm->getSessionEstablishmentRateLimiter();
                }
            }
        });
    }
