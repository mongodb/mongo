...
public:
    explicit StubFactoryImpl(GRPCClient::Options options) {
        if (options.tlsCertificateKeyFile) {
            auto certKeyPair = util::parsePEMKeyFile(*options.tlsCertificateKeyFile);
            _sslOps.pem_cert_chain = std::move(certKeyPair.cert_chain);
            _sslOps.pem_private_key = std::move(certKeyPair.private_key);
        }
        if (options.tlsCAFile) {
            _sslOps.pem_root_certs = ssl_util::readPEMFile(options.tlsCAFile.get()).getValue();
        }
    }
...
private:
    ::grpc::SslCredentialsOptions _sslOps;
