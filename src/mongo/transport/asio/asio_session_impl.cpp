...
CounterMetric totalMessageSizeErrorsPreAuth("network.totalMessageSizeErrorPreAuth");
CounterMetric totalMessageSizeErrorsPostAuth("network.totalMessageSizeErrorPostAuth");
}  // namespace
        ...
        .then([headerBuffer = std::move(headerBuffer), this, baton]() mutable {
            ...
            if (msgLen < kHeaderSize || msgLen > maxMessageSize) {
                ...
                sb << "recv(): message msgLen " << msgLen << " is invalid. "
                   << "Min " << kHeaderSize << " Max: " << maxMessageSize;
