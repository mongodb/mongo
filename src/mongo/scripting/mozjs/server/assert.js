...
// Wrap the body in an IIFE so the private helpers (e.g. _assertCommandFailed, _doassert) stay
// encapsulated rather than leaking onto the JS global scope. This is a classic (non-module) script,
// where top-level `function` declarations would otherwise become global properties. Only the public
// API is exposed on globalThis at the end, mirroring assert_global.js (and the original ES module).
(function () {
    /**
    ...
    assert.soonNoExcept = function (
        ...
        const safeFunc = _convertExceptionToReturnStatus(
            func,
            "assert.soonNoExcept caught exception",
        );
        const getFunc = () => {
    ...
    assert.retryNoExcept = function (
        ...
        const safeFunc = _convertExceptionToReturnStatus(
            func,
            "assert.retryNoExcept caught exception",
        );
     ...
     */
    assert.commandWorkedIgnoringWriteConcernErrorsOrFailedWithCode = function (
        res,
        errorCodeSet,
        msg,
    ) {
        ...
        for (let currField of Object.keys(docA)) {
            if (docB.hasOwnProperty(currField)) {
                if (fuzzyFields.includes(currField)) {
                    if (
                        !_isClose(parseFloat(docA[currField]), parseFloat(docB[currField]), places)
                    ) {
                        return false;
                    }
                } else {
    ...
    // Expose only the public API on the global scope (mirrors assert_global.js); the private helpers
    // declared above stay encapsulated inside this IIFE, matching the ES-module original's scoping.
