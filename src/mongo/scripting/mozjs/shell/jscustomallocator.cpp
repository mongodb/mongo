...
void* wrap_alloc(T&& func, void* ptr, size_t bytes) {
...
#if __has_feature(address_sanitizer)
    {
        ...
        if (bytes) {
            // Only track successful allocations (p != nullptr)
            if (p) {
                if (ptr) {
                    // realloc succeeded
                    if (ptr != p) {
                    ...
                    // else we didn't need to move, don't have to update tracking
                } else {
                    // malloc/calloc succeeded
                    handles.addPointer(p);
                }
            }
            // If p is nullptr, allocation failed - don't modify tracking
            // (for realloc failure, old pointer is still valid and already tracked)
        } else {
            // free
            handles.removePointer(ptr);
        }
