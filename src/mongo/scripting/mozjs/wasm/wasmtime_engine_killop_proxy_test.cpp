...
/**
 * A ScriptEngine whose interrupt() bumps a caller-owned counter that outlives the engine, so the
 * tally survives engine swaps/deletions.
 */
...
// Drives the proxy's interrupt path on one thread while another thread continuously swaps in fresh
// engines (deleting the previously installed one). Without serialization between the swap and the
...
TEST_F(ScriptEngineKillOpProxyTest, ConcurrentSwapAndInterruptIsSafe) {
    ...
    // The proxy delegated interrupts safely despite concurrent swaps/deletions.
    ASSERT_GT(interruptCount.load(), 0);
}
...
TEST_F(ScriptEngineSetupTest, KillOpProxyWorksAfterSetupThenEngineSwap) {
    // Call the real setup(), which creates a WasmtimeScriptEngine and registers the proxy.
    ScriptEngine::setup(ExecutionEnvironment::TestRunner);
    ASSERT_NE(getGlobalScriptEngine(), nullptr);

    auto client = makeClient();

    // Verify the proxy works with the Wasmtime engine (should not crash).
    {
        auto opCtx = client->makeOperationContext();
        ClientLock lk(client.get());
        getServiceContext()->killOperation(lk, opCtx.get(), ErrorCodes::Interrupted);
    ...
    // Swap the engine to a MockScriptEngine. This deletes the Wasmtime engine.
...
TEST_F(ScriptEngineSetupTest, ReSetupAfterClearRegistersProxyOnce) {
    // First setup registers the proxy and installs an engine.
    ScriptEngine::setup(ExecutionEnvironment::TestRunner);
    ASSERT_NE(getGlobalScriptEngine(), nullptr);

    // Clear the engine, then re-enter setup(). The presence guard in setup() is keyed on the
    // engine, so this re-runs the registration path; the proxy must still register only once.
    setGlobalScriptEngine(nullptr);
    ScriptEngine::setup(ExecutionEnvironment::TestRunner);
    ASSERT_NE(getGlobalScriptEngine(), nullptr);
    ...
    ASSERT_EQ(interruptCount.load(), 1);
}
