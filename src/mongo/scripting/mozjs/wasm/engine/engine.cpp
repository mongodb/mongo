...
static const char* const kFreezeScript =
    "(function() {"
...
err_code_t MozJSScriptEngine::_setupNewGlobal(ExecutionCheck& chk, wasm_mozjs_error_t* err) {
...

    {
        ...
        // kFreezeScript is shared with _setupChildRealm.
        ...
        if (!chk.ok(freezeSrc.init(
                        _cx, kFreezeScript, strlen(kFreezeScript), JS::SourceOwnership::Borrowed),
                    SM_E_INTERNAL) ||
...
err_code_t MozJSScriptEngine::_setupChildRealm(ExecutionCheck& chk, wasm_mozjs_error_t* err) {
...

    {
        ...
        // Copy MongoDB global helpers (hex_md5, etc.) to the child realm's globalThis.
        {
            static const char* kGlobalHelpers[] = {"hex_md5"};
...
err_code_t MozJSScriptEngine::shutdown(wasm_mozjs_error_t* err) {
    ...
    // JS_ShutDown() is once-per-process; handled by the destructor. Only tear down the
    // context so a subsequent init() can create a fresh one.
...
bool MozJSScriptEngine::isJavaScriptProtectionEnabled() const {
    // Not applicable: WASM engine runs in a sandboxed component.
    return false;
}
