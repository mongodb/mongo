    ...
    BenchRunner::BenchRunner( BenchRunConfig *config )
             ...
             if (_config->username != "") {
				 ...
				 if (!conn->auth(_config->db, _config->username, _config->password, errmsg)) {
                     uasserted(16704, 
                               ...
                               << " could not authenticate to " << _config->db 
							   << " db; " << _config->db << " db access is "
                               "required to use benchRun with auth enabled");
                 }
             }
             // Get initial stats
             conn->simpleCommand( _config->db , &before , "serverStatus" );
     ...
     void BenchRunner::stop() {
...

         {
             ...
             // Get final stats
             conn->simpleCommand( _config->db , &after , "serverStatus" );
