...
public:
    LogDuringInitShutdownTester() {
        auto sink = LogCaptureBackend::create(std::make_unique<Listener>(&syncedLines));
...
public:
    ...
    public:
        LineCapture()
            : _syncedLines{synchronized_value<std::vector<std::string>>()},
              _sink{LogCaptureBackend::create(std::make_unique<Listener>(&_syncedLines))} {}
        auto lines() {
            return **_syncedLines;
        }
        auto& sink() {
    ...
    std::unique_ptr<LineCapture> makeLineCapture(Fmt&& formatter) {
        auto ret = std::make_unique<LineCapture>();
        auto& s = ret->sink();
...
TEST_F(LogV2Test, MultipleDomains) {
    ...
    auto other_sink = LogCaptureBackend::create(std::make_unique<Listener>(&other_lines));
...
TEST_F(LogV2Test, UserAssert) {
    ...
    auto sink = wrapInSynchronousSink(wrapInCompositeBackend(
        boost::make_shared<LogCaptureBackend>(std::make_unique<Listener>(&syncedLines)),
