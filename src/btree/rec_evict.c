...
__wt_rec_evict(WT_SESSION_IMPL *session, WT_PAGE *page, int exclusive)
{
	...
	if (inmem_split) {
		...
		WT_RUNSTAT_CONN_INCR(session, cache_inmem_split);
		WT_RUNSTAT_DATA_INCR(session, cache_inmem_split);
		goto done;
	}
	...
	if (!exclusive && !merge &&
		...
		WT_RUNSTAT_CONN_INCR(session, cache_eviction_internal);
		WT_RUNSTAT_DATA_INCR(session, cache_eviction_internal);
	}
	...
	if (mod == NULL || !F_ISSET(mod, WT_PM_REC_MASK)) {
		...
		WT_RUNSTAT_CONN_INCR(session, cache_eviction_clean);
		WT_RUNSTAT_DATA_INCR(session, cache_eviction_clean);
	} else {
		if (WT_PAGE_IS_ROOT(page))
			__rec_root_update(session);
		else
		...
		WT_RUNSTAT_CONN_INCR(session, cache_eviction_dirty);
		WT_RUNSTAT_DATA_INCR(session, cache_eviction_dirty);
	}
...
err:		/*
		...
		WT_RUNSTAT_CONN_INCR(session, cache_eviction_fail);
		WT_RUNSTAT_DATA_INCR(session, cache_eviction_fail);
	}
...
ckpt:		WT_RUNSTAT_CONN_INCR(session, cache_eviction_checkpoint);
		WT_RUNSTAT_DATA_INCR(session, cache_eviction_checkpoint);
		return (EBUSY);
	}
...
__hazard_exclusive(WT_SESSION_IMPL *session, WT_REF *ref, int top)
{
	...
	WT_RUNSTAT_DATA_INCR(session, cache_eviction_hazard);
	WT_RUNSTAT_CONN_INCR(session, cache_eviction_hazard);
