    ...
    WT_PAGE *page, WT_SALVAGE_COOKIE *salvage, uint32_t flags)
{
	...
	WT_RUNSTAT_CONN_INCR(session, rec_pages);
	WT_RUNSTAT_DATA_INCR(session, rec_pages);
	if (LF_ISSET(WT_EVICTION_SERVER_LOCKED)) {
		WT_RUNSTAT_CONN_INCR(session, rec_pages_eviction);
		WT_RUNSTAT_DATA_INCR(session, rec_pages_eviction);
	}
...
__rec_txn_skip_chk(WT_SESSION_IMPL *session, WT_RECONCILE *r)
{
	...
	case WT_SKIP_UPDATE_QUIT:
		WT_RUNSTAT_CONN_INCR(session, rec_skipped_update);
		WT_RUNSTAT_DATA_INCR(session, rec_skipped_update);
		return (EBUSY);
	case 0:
	default:
		break;
...
__rec_split_raw_worker(WT_SESSION_IMPL *session, WT_RECONCILE *r, int final)
{
	...
	if (result_slots != 0) {
		WT_RUNSTAT_DATA_INCR(session, compress_raw_ok);
		...
		WT_RUNSTAT_DATA_INCR(session, compress_raw_fail);
...
too_small:	/*
		...
		WT_RUNSTAT_DATA_INCR(session, compress_raw_fail_temporary);
...
__rec_col_merge(WT_SESSION_IMPL *session, WT_RECONCILE *r, WT_PAGE *page)
{
	...
	WT_RUNSTAT_DATA_INCR(session, rec_page_merge);
...
__rec_write_wrapup(WT_SESSION_IMPL *session, WT_RECONCILE *r, WT_PAGE *page)
{
	...
	case 0:						/* Page delete */
		...
		WT_RUNSTAT_DATA_INCR(session, rec_page_delete);
	...
	default:					/* Page split */
		...
		case WT_PAGE_COL_INT:
		case WT_PAGE_ROW_INT:
			WT_RUNSTAT_DATA_INCR(session, rec_split_internal);
			break;
		case WT_PAGE_COL_FIX:
		case WT_PAGE_COL_VAR:
		case WT_PAGE_ROW_LEAF:
			WT_RUNSTAT_DATA_INCR(session, rec_split_leaf);
			break;
		WT_ILLEGAL_VALUE(session);
		}
    ...
    WT_RECONCILE *r, const void *data, uint32_t size, int *is_ovflp)
{
	...
	*is_ovflp = 0;
	...
	if (size > btree->maxintlitem) {
		WT_RUNSTAT_DATA_INCR(session, rec_overflow_key_internal);
    ...
    WT_RECONCILE *r, const void *data, uint32_t size, int *is_ovflp)
{
	...
	*is_ovflp = 0;
	...
	if (key->buf.size > btree->maxleafitem) {
		 ...
		 */
		if (pfx == 0) {
			WT_RUNSTAT_DATA_INCR(session, rec_overflow_key_leaf);
    ...
    WT_RECONCILE *r, const void *data, uint32_t size, uint64_t rle)
{
	...
	if (size != 0) {
		...
		if (val->buf.size > btree->maxleafitem) {
			WT_RUNSTAT_DATA_INCR(session, rec_overflow_value);
    ...
    WT_SESSION_IMPL *session, WT_RECONCILE *r, WT_KV *val, WT_DICTIONARY **dpp)
{
	...
	*dpp = NULL;
	...
	for (dp = __rec_dictionary_skip_search(r->dictionary_head, hash);
		...
		if (match) {
			WT_RUNSTAT_DATA_INCR(session, rec_dictionary);
			*dpp = dp;
			return (0);
		}
	}
