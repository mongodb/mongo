...
retry:
	 ...
	 */
	if (!__wt_atomic_cas4(&op->state, WT_ASYNCOP_FREE, WT_ASYNCOP_READY)) {
	...
	op->unique_id = __wt_atomic_add8(&async->op_id, 1);
	op->optype = WT_AOP_NONE;
	(void)__wt_atomic_store4(&async->ops_index, (i + 1) % conn->async_size);
	*opp = op;
	return (0);
}
...
retry:
	...
	if (!__wt_atomic_cas4(&async->flush_state, WT_ASYNC_FLUSH_NONE,
	 ...
	 */
	async->flush_count = 0;
	(void)__wt_atomic_add8(&async->flush_gen, 1);
