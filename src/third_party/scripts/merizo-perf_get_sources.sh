#!/bin/bash
# This script downloads the merizodb/merizo-perf repository and generates merizoebench-compatible JSON
...
# We add the current working directory to the PATH because that is where the merizo shell may have
# been installed. The benchrun.py script is going to run the merizo shell binary that's on the PATH.
PATH=$PATH:$(pwd)

# The merizo shell processes spawned by the benchrun.py script will attempt to connect to a merizod,
# so we just error out if we find that one isn't already running.
merizo --eval 'db.adminCommand({ping: 1})' && rc=$? || rc=$?
if [ "$rc" -ne 0 ]; then
    echo "This script requires a merizod to be running on port 27017"
    exit 2
fi
...
git clone --branch=master https://github.com/merizodb/merizo-perf.git $SRC_ROOT

pushd $SRC_ROOT

# We pin to a particular commit of the merizodb/merizo-perf repository to make it clear what version
...
mv $SRC_ROOT/merizoebench $DEST_DIR
