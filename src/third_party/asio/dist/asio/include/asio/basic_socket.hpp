...
public:
  ...
  void connect(const endpoint_type& peer_endpoint)
  {
    ...
    impl_.get_service().connect(impl_.get_implementation(), peer_endpoint, -1, ec);
    asio::detail::throw_error(ec, "connect");
  }
   ...
   * If timeout is less than zero, then an invalid_argument error will be
   * returned. The timeout parameter is only guaranteed to be effective if the
   * socket is in non-blocking mode. This function may block for longer
   * than the specified timeout if the socket is in blocking mode.
   *
   * @param ec Set to indicate what error occurred, if any.
   *
   * @par Example
   ...
   * asio::ip::tcp::endpoint endpoint(
   *     asio::ip::address::from_string("1.2.3.4"), 12345);
   * asio::ip::tcp::socket socket(my_context, endpoint.protocol());
      ...
      const Duration& timeout, asio::error_code& ec)
  {
    ...
    const auto ms = duration_cast<std::chrono::milliseconds>(timeout).count();
    impl_.get_service().connect(impl_.get_implementation(), peer_endpoint, ms,
        ec);
    ASIO_SYNC_OP_VOID_RETURN(ec);
  }
