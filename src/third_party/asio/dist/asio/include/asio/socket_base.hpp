...
public:
  ...
  /// Socket option for setting a timeout on recv operations.
  /**
   * Implements the SOL_SOCKET/SO_RCVTIMEO socket option.
   * A timeout of zero, which is the default, means "no timeout."
   *
   * @par Examples
   * Setting the option:
   * @code
   * asio::ip::tcp::socket socket(my_context);
   * ...
   * asio::socket_base::recv_timeout option(std::chrono::milliseconds(100));
   * socket.set_option(option);
   * @endcode
   *
   * @par
   ...
   * asio::ip::tcp::socket socket(my_context);
   * ...
   * asio::socket_base::recv_timeout option;
   * socket.get_option(option);
   * std::chrono::milliseconds value = option.value();
   * @endcode
   *
   * @par Concepts:
   * Socket_Option, Timeout_Socket_Option.
   */
#if defined(GENERATING_DOCUMENTATION)
  typedef implementation_defined recv_timeout;
#else
  typedef asio::detail::socket_option::timeout<
    ASIO_OS_DEF(SOL_SOCKET), ASIO_OS_DEF(SO_RCVTIMEO)>
      recv_timeout;
#endif

  /// Socket option for setting a timeout on send operations.
  /**
   * Implements the SOL_SOCKET/SO_RCVTIMEO socket option.
   * A timeout of zero, which is the default, means "no timeout."
   *
   ...
   * asio::ip::tcp::socket socket(my_context);
   * ...
   * asio::socket_base::send_timeout option(std::chrono::milliseconds(100));
   * socket.set_option(option);
   * @endcode
   *
   * @par
   ...
   * asio::ip::tcp::socket socket(my_context);
   * ...
   * asio::socket_base::send_timeout option;
   * socket.get_option(option);
   * std::chrono::milliseconds value = option.value();
   * @endcode
   *
   ...
   */
#if defined(GENERATING_DOCUMENTATION)
  typedef implementation_defined send_timeout;
#else
  typedef asio::detail::socket_option::timeout<
    ASIO_OS_DEF(SOL_SOCKET), ASIO_OS_DEF(SO_SNDTIMEO)>
      send_timeout;
#endif
