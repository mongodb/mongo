...
/**
 * Notice: this code has been modified from the folly source code to be compatible with MongoDB
 * utilities.
 */
...
public:
     ...
     */
    double balance(double rate, double burstSize, double nowInSeconds) const noexcept {
        invariant(rate > 0);
    ...
    double consume(double rate, double burstSize, double nowInSeconds, const Callback& callback) {
        invariant(rate > 0);
        invariant(burstSize > 0);
        ...
        do {
            ...
            MONGO_unlikely(!compare_exchange_weak_relaxed(zeroTime_, zeroTimeOld, zeroTimeNew)));
...
private:
    ...
    static constexpr size_t AlignZeroTime = constexpr_max(Align::value, alignof(Atom<double>));
...
public:
     ...
     * however.)
     */
    BasicDynamicTokenBucket(const BasicDynamicTokenBucket& other) noexcept = default;
    BasicDynamicTokenBucket& operator=(const BasicDynamicTokenBucket& other) noexcept = default;
