...
__desc_read(WT_SESSION_IMPL *session, WT_BLOCK *block)
{
    ...
    WT_ERR(__wt_read(session, block->fh, (wt_off_t)0, (size_t)block->allocsize, buf->mem));
...
#endif
    desc->checksum = 0;
    checksum_matched = __wt_checksum_match(desc, block->allocsize, checksum_tmp);
    ...
    __wt_verbose(session, WT_VERB_BLOCK,
      ...
      block->name, desc->magic, desc->majorv, desc->minorv, desc->checksum);
