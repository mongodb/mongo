...
int
__wti_block_disagg_addr_pack(
  uint8_t **pp, uint64_t page_id, uint64_t lsn, uint32_t size, uint32_t checksum)
{
    uint64_t c, l, p, s;
  ...
  WT_BM *bm, WT_SESSION_IMPL *session, WT_ITEM *buf, const uint8_t *addr, size_t addr_size)
{
    ...
    WT_RET(__wt_buf_fmt(session, buf, "[%" PRIuMAX ", %" PRIuMAX ", %" PRIu32 ", %" PRIu32 "]",
      (uintmax_t)page_id, (uintmax_t)lsn, size, checksum));
...
__wti_block_disagg_ckpt_pack(WT_BLOCK_DISAGG *block_disagg, uint8_t **buf, uint64_t root_id,
  uint64_t lsn, uint32_t root_sz, uint32_t root_checksum)
{
    WT_UNUSED(block_disagg);

    WT_RET(__wti_block_disagg_addr_pack(buf, root_id, lsn, root_sz, root_checksum));
  ...
  uint64_t *root_id, uint64_t *lsn, uint32_t *root_sz, uint32_t *root_checksum)
{
    ...
    WT_RET(__wti_block_disagg_addr_unpack(&buf, buf_size, root_id, lsn, root_sz, root_checksum));
