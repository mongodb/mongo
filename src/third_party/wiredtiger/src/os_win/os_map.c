  ...
  size_t *lenp, void *mapped_cookiep)
{
    ...
    if (mapped_cookie == NULL) {
        ...
        __wt_errx(session, "%s: memory-map: CreateFileMappingW: %s", file_handle->name,
    ...
    if ((map = MapViewOfFile(mapped_cookie, FILE_MAP_READ, 0, 0, len)) == NULL) {
        ...
        __wt_errx(session, "%s: memory-map: MapViewOfFile: %s", file_handle->name,
          __wt_formatmessage(session, windows_error));
        return (__wt_map_windows_error(windows_error));
    }
  ...
  size_t length, void *mapped_cookie)
{
    ...
    if (UnmapViewOfFile(mapped_region) == 0) {
        windows_error = __wt_getlasterror();
        __wt_errx(session, "%s: memory-unmap: UnmapViewOfFile: %s", file_handle->name,
          __wt_formatmessage(session, windows_error));
    ...
    if (CloseHandle(*(void **)mapped_cookie) == 0) {
        windows_error = __wt_getlasterror();
        __wt_errx(session, "%s: memory-unmap: CloseHandle: %s", file_handle->name,
          __wt_formatmessage(session, windows_error));
        ret = __wt_map_windows_error(windows_error);
    }
