...
__wt_salvage(WT_SESSION_IMPL *session, const char *cfg[])
{
    ...
    /*
     * If no checkpoint was created, well, it's probably bad news, but there is nothing to do but
     * clear any recorded checkpoints for the file. If a checkpoint was created, life is good,
     * replace any existing list of checkpoints with the single new one.
     */
