...
__wt_bulk_insert_row(WT_SESSION_IMPL *session, WT_CURSOR_BULK *cbulk)
{
    ...
    if (cursor->value.size == 0)
        ...
        WT_ERR(__wt_rec_cell_build_val(session, r, cursor->value.data, /* Build value cell */
    ...
    if (WT_CROSSING_SPLIT_BND(r, key->len + val->len)) {
        ...
        ret = __wt_rec_split_crossing_bnd(session, r, key->len + val->len);
        if (ret != 0)
    ...
    for (; ins != NULL; ins = WT_SKIP_NEXT(ins)) {
        ...
        case WT_UPDATE_STANDARD:
            if (upd->size == 0 && WT_TIME_WINDOW_IS_EMPTY(&tw))
                ...
                WT_ERR(__wt_rec_cell_build_val(session, r, upd->data, upd->size, &tw, 0, NULL));
            break;
  ...
  WT_CELL_UNPACK_KV *vpack, WT_TIME_WINDOW *tw)
{
    ...
    WT_ERR(__wt_rec_cell_build_val(session, r, p, size, tw, 0, NULL));
  ...
  WT_SESSION_IMPL *session, WT_RECONCILE *r, WT_REF *pageref, WT_SALVAGE_COOKIE *salvage)
{
    ...
    WT_ROW_FOREACH (page, rip, i) {
        ...
        if (upd == NULL) {
            ...
            case WT_UPDATE_STANDARD:
                /* Take the value from the update. */
                WT_ERR(__wt_rec_cell_build_val(session, r, upd->data, upd->size, twp, 0, NULL));
                dictionary = true;
                break;
            case WT_UPDATE_TOMBSTONE:
                /*
