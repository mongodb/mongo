  ...
  WT_MULTI *multi, WT_PAGE_HEADER *dsk)
{
    ...
    /*
     * There are page header fields which need to be cleared for consistent checksums: specifically,
     * the write generation and the memory owned by the block manager.
     */
...
copy_image:
    ...
    WT_ASSERT(session, verify_image == false ||
        __wt_verify_dsk_image(session, "[reconcile-image]", chunk->image.data, 0, true) == 0);
#endif
...
int
__wt_rec_cell_build_ovfl(
  WT_SESSION_IMPL *session, WT_RECONCILE *r, WT_REC_KV *kv, uint8_t type, uint64_t rle)
{
    WT_BM *bm;
    WT_BTREE *btree;
    WT_DECL_ITEM(tmp);
