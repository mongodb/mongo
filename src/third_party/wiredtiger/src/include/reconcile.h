...
typedef struct {
     ...
     * into the disk image buffer, where we are in the buffer, and how much memory remains. Those
     * values are packaged here rather than passing pointers to stack locations around the code.
     */
