...
__col_insert_search_gt(WT_INSERT_HEAD *ins_head, uint64_t recno)
{
    ...
    /*
     * If we didn't find any records smaller than the target, we never set the return value, set it
     * to the first record in the list. Otherwise, it references a record less-than-or-equal to the
     * target, move to a later record, that is, a subsequent record greater than the target. Because
     * inserts happen concurrently, additional records might be inserted after the searched-for
     * record that are still smaller than the target, continue to move forward until reaching a
     * record larger than the target. There isn't any safety testing because we confirmed such a
     * record exists before searching.
     */
    if (ins == NULL)
