...
__checksum_with_seed_hw(uint32_t seed, const void *chunk, size_t len)
{
    ...
    p = (const uint8_t *)chunk;
    /*
     * Checksum in 16B chunks. Do two loads at a time as the looping logic generated by the compiler
     * is faster.
     */
    for (; len >= 2 * sizeof(uint64_t); p += 2 * sizeof(uint64_t), len -= 2 * sizeof(uint64_t)) {
        memcpy(&uint64_v, p, sizeof(uint64_t));
        CRC32CX(crc, uint64_v);
        memcpy(&uint64_v, p + sizeof(uint64_t), sizeof(uint64_t));
        CRC32CX(crc, uint64_v);
    }
