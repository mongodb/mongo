...
)
{
    ...
    *busyp = false;
    ...
    if (session->hazards.num_active >= session->hazards.size) {
        WT_ASSERT(session,
          session->hazards.num_active == session->hazards.size &&
            __wt_atomic_load_uint32_relaxed(&session->hazards.inuse) == session->hazards.size);
     ...
     */
    if (session->hazards.num_active >= __wt_atomic_load_uint32_relaxed(&session->hazards.inuse)) {
        WT_ASSERT(session,
          session->hazards.num_active == __wt_atomic_load_uint32_relaxed(&session->hazards.inuse) &&
            __wt_atomic_load_uint32_relaxed(&session->hazards.inuse) < session->hazards.size);
        /*
         ...
         * logic.
         */
        hp = &session->hazards.arr[__wt_atomic_fetch_add_uint32(&session->hazards.inuse, 1)];
    } else {
        WT_ASSERT(session,
          session->hazards.num_active < __wt_atomic_load_uint32_relaxed(&session->hazards.inuse) &&
            __wt_atomic_load_uint32_relaxed(&session->hazards.inuse) <= session->hazards.size);
...
__wt_hazard_clear(WT_SESSION_IMPL *session, WT_REF *ref)
{
    ...
    for (hp = session->hazards.arr + __wt_atomic_load_uint32_relaxed(&session->hazards.inuse) - 1;
        ...
        if (hp->ref == ref) {
             ...
             */
            __wt_atomic_store_ptr_release(&hp->ref, NULL);
