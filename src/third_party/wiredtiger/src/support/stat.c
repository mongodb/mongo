...
static const char *const __stats_dsrc_desc[] = {
  ...
  "btree: pages rewritten by compaction", "btree: row-store internal pages",
  "btree: row-store leaf pages", "cache: bytes currently in the cache",
  "cache: bytes dirty in the cache cumulative", "cache: bytes read into cache",
  ...
  "cache_walk: Total number of pages currently in cache", "compression: compressed pages read",
  "compression: compressed pages written", "compression: page written failed to compress",
  ...
  "cursor: cursor operation restarted", "cursor: cursor-insert key and value bytes inserted",
  "cursor: cursor-remove key bytes removed", "cursor: cursor-update value bytes updated",
  "cursor: cursors reused from cache", "cursor: insert calls", "cursor: modify calls",
  "cursor: next calls", "cursor: open cursor count", "cursor: prev calls", "cursor: remove calls",
...
static const char *const __stats_connection_desc[] = {
  ...
  "capacity: threshold to call fsync", "capacity: throttled bytes read",
  "capacity: throttled bytes written for checkpoint",
  "capacity: throttled bytes written for eviction", "capacity: throttled bytes written for log",
  "capacity: throttled bytes written total", "capacity: time waiting due to total capacity (usecs)",
  ...
  "cursor: cursor insert calls", "cursor: cursor modify calls", "cursor: cursor next calls",
  ...
  "cursor: cursor search near calls", "cursor: cursor sweep buckets",
  "cursor: cursor sweep cursors closed", "cursor: cursor sweep cursors examined",
  "cursor: cursor sweeps", "cursor: cursor update calls", "cursor: cursors reused from cache",
  "cursor: open cursor count", "cursor: truncate calls",
  ...
  "lock: dhandle write lock acquisitions", "lock: metadata lock acquisitions",
  ...
  "transaction: commit timestamp queue inserts total", "transaction: commit timestamp queue length",
  ...
  "transaction: set timestamp calls", "transaction: set timestamp commit calls",
  "transaction: set timestamp commit updates", "transaction: set timestamp oldest calls",
...
__wt_stat_connection_aggregate(WT_CONNECTION_STATS **from, WT_CONNECTION_STATS *to)
{
    ...
    to->lock_commit_timestamp_read_count += WT_STAT_READ(from, lock_commit_timestamp_read_count);
    to->lock_commit_timestamp_write_count += WT_STAT_READ(from, lock_commit_timestamp_write_count);
    ...
    to->txn_commit_queue_walked += WT_STAT_READ(from, txn_commit_queue_walked);
    to->txn_commit_queue_empty += WT_STAT_READ(from, txn_commit_queue_empty);
    to->txn_commit_queue_head += WT_STAT_READ(from, txn_commit_queue_head);
    to->txn_commit_queue_inserts += WT_STAT_READ(from, txn_commit_queue_inserts);
    ...
    to->txn_set_ts_commit_upd += WT_STAT_READ(from, txn_set_ts_commit_upd);
