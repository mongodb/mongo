...
__rebalance_leaf_append(WT_SESSION_IMPL *session, const uint8_t *key, size_t key_len,
  uint64_t recno, const uint8_t *addr, size_t addr_len, u_int addr_type, WT_REBALANCE_STUFF *rs)
{
    WT_ADDR *copy_addr;
    WT_REF *copy;
...
__rebalance_col_walk(WT_SESSION_IMPL *session, const WT_PAGE_HEADER *dsk, WT_REBALANCE_STUFF *rs)
{
    ...
    WT_CELL_FOREACH(btree, dsk, cell, &unpack, i)
    {
        ...
        case WT_CELL_ADDR_LEAF_NO:
            WT_ERR(__rebalance_leaf_append(session, NULL, 0, unpack.v, unpack.data, unpack.size,
              unpack.type == WT_CELL_ADDR_LEAF ? WT_ADDR_LEAF : WT_ADDR_LEAF_NO, rs));
            break;
        default:
...
__rebalance_row_walk(WT_SESSION_IMPL *session, const WT_PAGE_HEADER *dsk, WT_REBALANCE_STUFF *rs)
{
    ...
    WT_CELL_FOREACH(btree, dsk, cell, &unpack, i)
    {
        ...
        case WT_CELL_ADDR_LEAF_NO:
            ...
            WT_ERR(__rebalance_leaf_append(session, p, len, WT_RECNO_OOB, unpack.data, unpack.size,
              unpack.type == WT_CELL_ADDR_LEAF ? WT_ADDR_LEAF : WT_ADDR_LEAF_NO, rs));
...
__wt_bt_rebalance(WT_SESSION_IMPL *session, const char *cfg[])
{
    ...
    case WT_PAGE_ROW_INT:
        WT_ERR(__rebalance_row_walk(session, btree->root.page->dsk, rs));
        break;
    case WT_PAGE_COL_INT:
        WT_ERR(__rebalance_col_walk(session, btree->root.page->dsk, rs));
        break;
    default:
