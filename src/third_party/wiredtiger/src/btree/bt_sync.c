...
__sync_file(WT_SESSION_IMPL *session, WT_CACHE_OP syncop)
{
     ...
     * the checkpoint as part of reconciling internal pages (specifically in __wt_rec_child_modify).
     */
    ...
    case WT_SYNC_CHECKPOINT:
        ...
        for (;;) {
            ...
            if (!WT_PAGE_IS_INTERNAL(page) && page->read_gen == WT_READGEN_WONT_NEED &&
              F_ISSET(&session->txn, WT_TXN_HAS_SNAPSHOT) && !tried_eviction) {
...
__wt_cache_op(WT_SESSION_IMPL *session, WT_CACHE_OP op)
{
    ...
    case WT_SYNC_CLOSE:
        /*
         * Make sure the checkpoint reference is set for reconciliation; it's ugly, but drilling a
         * function parameter path from our callers to the reconciliation of the tree's root page is
         * going to be worse.
         */
