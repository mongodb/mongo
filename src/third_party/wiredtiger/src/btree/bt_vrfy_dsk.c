...
__wt_verify_dsk_image(WT_SESSION_IMPL *session, const char *tag, const WT_PAGE_HEADER *dsk,
  size_t size, bool empty_page_ok)
{
    uint8_t flags;
    const uint8_t *p, *end;
    u_int i;
...
{
    return (__wt_verify_dsk_image(session, tag, buf->data, buf->size, false));
}
 ...
 */
static int
__verify_dsk_row(WT_SESSION_IMPL *session, const char *tag, const WT_PAGE_HEADER *dsk)
{
    WT_BM *bm;
    WT_BTREE *btree;
    WT_CELL *cell;
    ...
    WT_CELL_FOREACH(btree, dsk, cell, unpack, i)
    {
        ...
        case WT_CELL_VALUE_OVFL:
            if ((ret = bm->addr_invalid(bm, session, unpack->data, unpack->size)) == EINVAL)
                ret = __err_cell_corrupt_or_eof(session, cell_num, tag);
            WT_ERR(ret);
            break;
        }
 ...
 */
static int
__verify_dsk_col_int(WT_SESSION_IMPL *session, const char *tag, const WT_PAGE_HEADER *dsk)
{
    WT_BM *bm;
    WT_BTREE *btree;
    WT_CELL *cell;
    ...
    WT_CELL_FOREACH(btree, dsk, cell, unpack, i)
    {
        ...
        if (ret == EINVAL)
            return (__err_cell_corrupt_or_eof(session, cell_num, tag));
    }
 ...
 */
static int
__verify_dsk_col_var(WT_SESSION_IMPL *session, const char *tag, const WT_PAGE_HEADER *dsk)
{
    WT_BM *bm;
    WT_BTREE *btree;
    WT_CELL *cell;
 ...
 */
static int
__err_cell_corrupt(WT_SESSION_IMPL *session, uint32_t entry_num, const char *tag)
{
    F_SET(S2C(session), WT_CONN_DATA_CORRUPTION);
    WT_RET_VRFY(session, "item %" PRIu32 " on page at %s is a corrupted cell", entry_num, tag);
}
 ...
 */
static int
__err_cell_corrupt_or_eof(WT_SESSION_IMPL *session, uint32_t entry_num, const char *tag)
{
    WT_RET_VRFY(session, "item %" PRIu32
