...
static int __verify_overflow_cell(WT_SESSION_IMPL *, WT_REF *, bool *, WT_VSTUFF *);
...
recno_chk:
    ...
    if (!__wt_ref_is_root(ref)) {
        ...
        case WT_PAGE_COL_FIX:
            ...
            break;
        case WT_PAGE_COL_VAR:
            if (unpack->raw != WT_CELL_ADDR_LEAF && unpack->raw != WT_CELL_ADDR_LEAF_NO)
                goto celltype_err;
            break;
        case WT_PAGE_ROW_LEAF:
            if (unpack->raw != WT_CELL_ADDR_DEL && unpack->raw != WT_CELL_ADDR_LEAF &&
...
celltype_err:
    ...
    /*
     * Check overflow pages. We check overflow cells separately from other tests that walk the page
     * as it's simpler, and I don't care much how fast table verify runs.
     */
    switch (page->type) {
    case WT_PAGE_COL_VAR:
    case WT_PAGE_ROW_INT:
    case WT_PAGE_ROW_LEAF:
         ...
         * object if a leaf address cell references a page without overflow keys. Reconciliation
         * doesn't guarantee every leaf page without overflow items will be a leaf-no-overflow type.
         */
 ...
 */
static int
__verify_overflow_cell(WT_SESSION_IMPL *session, WT_REF *ref, bool *found, WT_VSTUFF *vs)
{
    WT_BTREE *btree;
    WT_CELL *cell;
    WT_CELL_UNPACK *unpack, _unpack;
...
err:
    WT_RET_MSG(session, ret, "cell %" PRIu32
                             " on page at %s references an overflow item at %s "
