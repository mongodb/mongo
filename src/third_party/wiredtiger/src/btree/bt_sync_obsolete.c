...
__sync_obsolete_deleted_cleanup(WT_SESSION_IMPL *session, WT_REF *ref)
{
    ...
    if (page_del == NULL ||
      __wt_txn_visible_all(session, page_del->txnid, page_del->durable_timestamp)) {
        /*
         * When called inline during a checkpoint tree walk, only the parent page needs to be marked
         * dirty the current checkpoint will write it. When called from the dedicated cleanup
         * thread, the tree must also be marked dirty so the next checkpoint picks up the change.
         */
...
__sync_obsolete_disk_cleanup(WT_SESSION_IMPL *session, WT_REF *ref, bool *ref_deleted)
{
    ...
    *ref_deleted = false;
    ...
    if (obsolete &&
      ((ret = __wt_page_parent_modify_set(session, ref, WT_SESSION_BTREE_SYNC(session))) == 0)) {
        __wt_verbose_debug2(session, WT_VERB_CHECKPOINT_CLEANUP,
