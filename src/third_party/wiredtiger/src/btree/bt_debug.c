...
__debug_item_key(WT_DBG *ds, const char *tag, const void *data_arg, size_t size)
{
    ...
    return (ds->f(ds, "\t%s%s{%s}\n", tag == NULL ? "" : tag, tag == NULL ? "" : " ",
      __wt_buf_set_printable_format(ds->session, data_arg, size, ds->key_format, ds->t1)));
}
...
__debug_item_value(WT_DBG *ds, const char *tag, const void *data_arg, size_t size)
{
    ...
    return (ds->f(ds, "\t%s%s{%s}\n", tag == NULL ? "" : tag, tag == NULL ? "" : " ",
      __wt_buf_set_printable_format(ds->session, data_arg, size, ds->value_format, ds->t1)));
}
...
__debug_tree_shape_info(WT_REF *ref, char *buf, size_t len)
{
    ...
    (void)__wt_snprintf(buf, len,
      "(%p, %" PRIu64 "%s, evict gen %" PRIu64 ", create gen %" PRIu64 ")", (void *)ref, v, unit,
...
addr:
        WT_RET(__wt_scr_alloc(session, 128, &buf));
        ret =
          ds->f(ds, ", %s %s", type, __wt_addr_string(session, unpack->data, unpack->size, buf));
        __wt_scr_free(session, &buf);
        WT_RET(ret);
        break;
    }
