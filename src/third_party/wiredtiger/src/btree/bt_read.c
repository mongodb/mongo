  ...
  )
{
    ...
    for (did_read = wont_need = stalled = false, force_attempts = 0, sleep_usecs = yield_cnt = 0;
         ;) {
 ...
 */
             ...
             */
            if (did_read || LF_ISSET(WT_READ_NO_SPLIT) || btree->evict_disabled > 0 ||
              btree->lsm_primary)
                goto skip_evict;
...
skip_evict:
        ...
        /*
         * If stalling and this thread is allowed to do eviction work, check if the cache needs
         * help. If we do work for the cache, substitute that for a sleep.
         */
        if (!LF_ISSET(WT_READ_IGNORE_CACHE_SIZE)) {
            WT_RET(__wt_cache_eviction_check(
              session, true, !F_ISSET(&session->txn, WT_TXN_HAS_ID), &cache_work));
            if (cache_work)
                continue;
        }
