...
/*
 * move_data_ahead --
 *     Update the tables with new data and take a checkpoint twice. WiredTiger keeps the previous
 *     checkpoint so we do it twice so that the old checkpoint address no longer exists.
 */
static void
 ...
 *     Make copies of the database so that we can test various mix and match of turtle files and
 *     metadata files. We take some checkpoints and update the data too.
 */
static void
make_database_copies(TABLE_INFO *table_data)
{
    /*
     * If we're running an out-of-sync test, then we want to make copies of the turtle and metadata
     * file, then checkpoint and again save a copy of the turtle file and the metadata file. Then we
     * add more data and checkpoint again at least twice. Using the original and current files we
     * can test various out of sync scenarios.
     */
    /*
...
setup_database(const char *src, const char *turtle_dir, const char *meta_dir)
{
    ...
    /*
     * Remove the test home directory and copy the source to it. Then copy the saved turtle and/or
     * metadata file from the given args.
     */
    testutil_check(__wt_snprintf(buf, sizeof(buf),
      "rm -rf ./%s.%s; mkdir ./%s.%s; "
      "cp -p %s.%s/* ./%s.%s",
    ...
    if (turtle_dir != NULL) {
        testutil_check(__wt_snprintf(buf, sizeof(buf), "cp -p %s.%s/%s.%s %s.%s/%s", home,
          turtle_dir, WT_METADATA_TURTLE, SAVE, home, TEST, WT_METADATA_TURTLE));
        printf("copy: %s\n", buf);
        if ((ret = system(buf)) < 0)
            testutil_die(ret, "system: %s", buf);
    }
    ...
    if (meta_dir != NULL) {
        testutil_check(__wt_snprintf(buf, sizeof(buf), "cp -p %s.%s/%s.%s %s.%s/%s", home, meta_dir,
          WT_METAFILE, SAVE, home, TEST, WT_METAFILE));
        printf("copy: %s\n", buf);
        if ((ret = system(buf)) < 0)
            testutil_die(ret, "system: %s", buf);
    }
