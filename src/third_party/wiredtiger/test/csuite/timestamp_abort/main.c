...
thread_ts_run(void *arg)
{
    ...
    for (;;) {
        ...
        ret = td->conn->query_timestamp(td->conn, ts_buf, "get=all_committed");
        ...
        if (ret == 0) {
            ...
            testutil_check(__wt_snprintf(
              tscfg, sizeof(tscfg), "oldest_timestamp=%s,stable_timestamp=%s", ts_buf, ts_buf));
...
thread_ckpt_run(void *arg)
{
    ...
    for (i = 0;; ++i) {
        ...
        testutil_check(td->conn->query_timestamp(td->conn, buf, "get=last_checkpoint"));
...
thread_run(void *arg)
{
    ...
    testutil_check(td->conn->open_session(td->conn, NULL, NULL, &session));
    if (use_prep)
        testutil_check(td->conn->open_session(td->conn, NULL, NULL, &prepared_session));
    /*
    ...
    for (i = td->start;; ++i) {
        ...
        testutil_check(__wt_snprintf(cbuf, sizeof(cbuf),
          "COLL: thread:%" PRIu64 " ts:%" PRIu64 " key: %" PRIu64, td->info, active_ts, i));
        testutil_check(__wt_snprintf(lbuf, sizeof(lbuf),
          "LOCAL: thread:%" PRIu64 " ts:%" PRIu64 " key: %" PRIu64, td->info, active_ts, i));
        testutil_check(__wt_snprintf(obuf, sizeof(obuf),
          "OPLOG: thread:%" PRIu64 " ts:%" PRIu64 " key: %" PRIu64, td->info, active_ts, i));
...
run_workload(uint32_t nth)
{
    ...
    if (inmem)
        (void)__wt_snprintf(envconf, sizeof(envconf), ENV_CONFIG_DEF, SESSION_MAX);
    else
        (void)__wt_snprintf(envconf, sizeof(envconf), ENV_CONFIG_TXNSYNC, SESSION_MAX);
    if (compat)
