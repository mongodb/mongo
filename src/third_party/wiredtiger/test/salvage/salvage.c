...
build(int ikey, int ivalue, int cnt)
{
     ...
     */
    CHECK(wiredtiger_open(NULL, NULL, "create,log=(enabled=false)", &conn) == 0);
    ...
    CHECK(session->open_cursor(session, "file:" LOAD, NULL, "bulk,append", &cursor) == 0);
...
process(void)
{
    ...
    CHECK(wiredtiger_open(NULL, NULL, config, &conn) == 0);
    CHECK(conn->open_session(conn, NULL, NULL, &session) == 0);
    CHECK(session->open_cursor(session, "file:" SLVG, NULL, "dump=print", &cursor) == 0);
