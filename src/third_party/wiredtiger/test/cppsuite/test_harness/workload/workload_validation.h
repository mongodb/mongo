...
class workload_validation {
      ...
      const std::vector<uint64_t> &known_collection_ids)
    {
        ...
        log_msg(LOG_INFO, "Beginning validation.");
      ...
      std::vector<uint64_t> &created_collections, std::vector<uint64_t> &deleted_collections)
    {
        ...
        while (cursor->next(cursor.get()) == 0) {
            ...
            log_msg(LOG_TRACE, "Collection id is " + std::to_string(key_collection_id));
            log_msg(LOG_TRACE, "Timestamp is " + std::to_string(key_timestamp));
            log_msg(LOG_TRACE, "Operation type is " + std::to_string(value_operation_type));
    ...
    void
    update_data_model(const tracking_operation &operation, validation_collection &collection,
    ...
    void
    verify_key_value(scoped_session &session, const uint64_t collection_id, const std::string &key,
