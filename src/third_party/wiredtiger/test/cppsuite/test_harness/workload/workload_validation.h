...
class workload_validation {
      ...
      const std::vector<uint64_t> &known_collection_ids)
    {
        ...
        debug_print("Beginning validation.", DEBUG_INFO);
        ...
        while ((ret = cursor->next(cursor.get())) == 0) {
            ...
            debug_print("Retrieved tracked values. \n Collection id: " +
        ...
        if (ret != WT_NOTFOUND)
            testutil_die(
              DEBUG_ERROR, "Validation failed: cursor->next() return an unexpected error %d.", ret);
    }
