...
#define ESET(a)                                                                               \
    do {                                                                                      \
        ...
        if ((__v = (a)) != 0) {                                                               \
             ...
             * with the error.                                                                \
             */                                                                               \
            if (__v == WT_PANIC || ret == 0 || ret == WT_DUPLICATE_KEY || ret == WT_NOTFOUND) \
                ret = __v;                                                                    \
            /*                                                                                \
...
#define ERET(wt_api, session, v, ...)                                      \
    do {                                                                   \
        (void)wt_api->err_printf(wt_api, session, "helium: " __VA_ARGS__); \
        ESET(v);                                                           \
        return (ret);                                                      \
    } while (0)
#undef EMSG
#define EMSG(wt_api, session, v, ...)                                      \
    do {                                                                   \
        (void)wt_api->err_printf(wt_api, session, "helium: " __VA_ARGS__); \
        ESET(v);                                                           \
    } while (0)
#undef EMSG_ERR
#define EMSG_ERR(wt_api, session, v, ...)                                  \
    do {                                                                   \
        (void)wt_api->err_printf(wt_api, session, "helium: " __VA_ARGS__); \
        ESET(v);                                                           \
        goto err;                                                          \
...
#define VERBOSE_L2 2
#undef VMSG
#define VMSG(wt_api, session, v, ...)                                          \
    do {                                                                       \
        if (verbose >= v)                                                      \
            (void)wt_api->msg_printf(wt_api, session, "helium: " __VA_ARGS__); \
    } while (0)
...
/*
 * WiredTiger name space on the Helium store: all objects are named with the WiredTiger prefix (we
 * don't require the Helium store be exclusive to our files). Primary objects are named
 * "WiredTiger.[name]", associated cache objects are "WiredTiger.[name].cache". The per-connection
 * transaction object is "WiredTiger.WiredTigerTxn". When we first open a Helium volume, we
 * open/close a file in order to apply flags for the first open of the volume, that's
 * "WiredTiger.WiredTigerInit".
 */
...
typedef struct __wt_source {
    ...
    /*
     * Each WiredTiger object has a "primary" namespace in a Helium store plus a "cache" namespace,
     * which has not-yet-resolved updates. There is a dirty flag so read-only data sets can ignore
     * the cache.
     */
...
typedef struct __he_source {
    ...
    /*
     * Maintain a handle for each underlying Helium source so checkpoint is faster, we can "commit"
     * a single handle per source, regardless of the number of objects.
     */
    he_t he_volume;
    ...
    /*
     * Each Helium source has a cleaner thread to migrate WiredTiger source updates from the cache
     * namespace to the primary namespace, based on the number of bytes or the number of operations.
     * (There's a cleaner thread per Helium store so migration operations can overlap.) We read
     * these fields without a lock, but serialize writes to minimize races (and because it costs us
     * nothing).
     */
...
/*
 * Each WiredTiger connection has a transaction namespace which lists resolved transactions with
 * their committed or aborted state as a value. That namespace appears in a single Helium store (the
 * first one created, if it doesn't already exist), and then it's referenced from other Helium
 * stores.
 */
...
lock_init(WT_EXTENSION_API *wt_api, WT_SESSION *session, pthread_rwlock_t *lockp)
{
    ...
    if ((ret = pthread_rwlock_init(lockp, NULL)) != 0)
        ERET(wt_api, session, WT_PANIC, "pthread_rwlock_init: %s", strerror(ret));
    return (0);
}
...
lock_destroy(WT_EXTENSION_API *wt_api, WT_SESSION *session, pthread_rwlock_t *lockp)
{
    ...
    if ((ret = pthread_rwlock_destroy(lockp)) != 0)
        ERET(wt_api, session, WT_PANIC, "pthread_rwlock_destroy: %s", strerror(ret));
    return (0);
}
...
writelock(WT_EXTENSION_API *wt_api, WT_SESSION *session, pthread_rwlock_t *lockp)
{
    ...
    if ((ret = pthread_rwlock_wrlock(lockp)) != 0)
        ERET(wt_api, session, WT_PANIC, "pthread_rwlock_wrlock: %s", strerror(ret));
    return (0);
}
...
unlock(WT_EXTENSION_API *wt_api, WT_SESSION *session, pthread_rwlock_t *lockp)
{
    ...
    if ((ret = pthread_rwlock_unlock(lockp)) != 0)
        ERET(wt_api, session, WT_PANIC, "pthread_rwlock_unlock: %s", strerror(ret));
    return (0);
}
...
static inline int
helium_call(
  WT_CURSOR *wtcursor, const char *fname, he_t he, int (*f)(he_t, HE_ITEM *, size_t, size_t))
{
    CURSOR *cursor;
    HE_ITEM *r;
...
restart:
    ...
    for (;;) {
        ...
        if ((ret = he_lookup(he, r, (size_t)0, cursor->mem_len)) != 0) {
            if (ret == HE_ERR_ITEM_NOT_FOUND)
                goto restart;
            ERET(wt_api, session, ret, "he_lookup: %s", he_strerror(ret));
        }
    }
    /* NOTREACHED */
}
...
static int
txn_state_set(
  WT_EXTENSION_API *wt_api, WT_SESSION *session, HELIUM_SOURCE *hs, uint64_t txnid, int commit)
{
    HE_ITEM txn;
    uint8_t val;
    int ret = 0;
 ...
 */
static int
txn_notify(WT_TXN_NOTIFY *handler, WT_SESSION *session, uint64_t txnid, int committed)
{
    HELIUM_SOURCE *hs;
...
cache_value_unmarshall(WT_CURSOR *wtcursor)
{
    ...
    if (entries > cursor->cache_slots) {
        if ((p = realloc(cursor->cache, (entries + 20) * sizeof(cursor->cache[0]))) == NULL)
            return (os_errno());
 ...
 *     Return if an update can proceed based on the previous updates made to the cache entry.
 */
static int
cache_value_update_check(WT_CURSOR *wtcursor)
{
    ...
    if (wt_api->transaction_isolation_level(wt_api, session) != WT_TXN_ISO_SNAPSHOT)
        return (0);

    /*
     * If there's an entry that's not visible and hasn't been aborted, return a deadlock.
     */
...
/*
 * cache_value_visible --
 *     Return the most recent cache entry update visible to the running transaction.
 */
static int
cache_value_visible(WT_CURSOR *wtcursor, CACHE_RECORD **cpp)
{
    ...
    *cpp = NULL;
    ...
    /*
     * We want the most recent cache entry update; the cache entries are in update order, walk from
...
copyin_key(WT_CURSOR *wtcursor, int allocate_key)
{
    ...
    if (ws->config_recno) {
        ...
        WT_RET(wt_api->struct_size(wt_api, session, &size, "r", wtcursor->recno));
        WT_RET(wt_api->struct_pack(wt_api, session, r->key, HE_MAX_KEY_LEN, "r", wtcursor->recno));
        r->key_len = size;
    } else {
        /* I'm not sure this test is necessary, but it's cheap. */
        if (wtcursor->key.size > HE_MAX_KEY_LEN)
            return (key_max_err(wt_api, session, wtcursor->key.size));

        /*
         * A set cursor key might reference application memory, which is only OK until the cursor
         * operation has been called (in other words, we can only reference application memory from
         * the WT_CURSOR.set_key call until the WT_CURSOR.op call). For this reason, do a full copy,
         * don't just reference the WT_CURSOR key's data.
         */
...
copyout_key(WT_CURSOR *wtcursor)
{
    ...
    r = &cursor->record;
    if (ws->config_recno)
        WT_RET(wt_api->struct_unpack(wt_api, session, r->key, r->key_len, "r", &wtcursor->recno));
    else {
 ...
 */
static int
nextprev(WT_CURSOR *wtcursor, const char *fname, int (*f)(he_t, HE_ITEM *, size_t, size_t))
{
    CACHE_RECORD *cp;
    CURSOR *cursor;
    HE_ITEM *r;
    ...
    /*
     * If the cache isn't yet in use, it's a simpler problem, just check the store. We don't care if
     * we race, we're not guaranteeing any special behavior with respect to phantoms.
     */
...
skip_deleted:
    ...
    /*
     * Move through the cache until we either find a record with a visible entry, or we reach the
     * end/beginning.
     */
    for (cache_rm = 0;;) {
        ...
        /*
         * If the entry has been deleted, remember that and continue. We can't just skip the entry
         * because it might be a delete of an entry in the primary store, which means the cache
        ...
        /*
         * Copy the cache key. If the cache's entry wasn't a delete, copy the value as well, we may
...
cache_clean:
    ...
    /*
     * If both the cache and the primary had entries, decide which is a better choice and pretend we
    ...
    /*
     * If the cache is the key we'd choose, but it's a delete, skip past it by moving from the
     * deleted key to the next/prev item in either the primary or the cache.
     */
...
helium_cursor_reset(WT_CURSOR *wtcursor)
{
    ...
    /*
     * Reset the cursor by setting the key length to 0, causing subsequent next/prev operations to
     * return the first/last record of the object.
     */
    r->key_len = 0;
    return (0);
}
...
helium_cursor_search(WT_CURSOR *wtcursor)
{
    ...
    /*
     * Check for an entry in the cache. If we find one, unmarshall it and check for a visible entry
     * we can return.
     */
    if ((ret = helium_call(wtcursor, "he_lookup", ws->he_cache, he_lookup)) == 0) {
        WT_RET(cache_value_unmarshall(wtcursor));
        if (cache_value_visible(wtcursor, &cp))
            return (cp->remove ? WT_NOTFOUND : copyout_val(wtcursor, cp));
...
helium_cursor_search_near(WT_CURSOR *wtcursor, int *exact)
{
    ...
    /*
     * XXX I'm not confident this is sufficient: if there are multiple threads of control, it's
     * possible for the search for an exact match to fail, another thread of control to insert (and
     * commit) an exact match, and then it's possible we'll return the wrong value. This needs to be
...
helium_cursor_insert(WT_CURSOR *wtcursor)
{
    ...
    VMSG(
      wt_api, session, VERBOSE_L2, "I %.*s.%.*s", (int)r->key_len, r->key, (int)r->val_len, r->val);
    ...
    switch (ret = helium_call(wtcursor, "he_lookup", ws->he_cache, he_lookup)) {
    case 0:
        /* Crack the record. */
        ...
        /*
         * If overwrite is false, a visible entry (that's not a removed entry), is an error. We're
         * done checking if there is a visible entry in the cache, otherwise repeat the check on the
         * primary store.
         */
        if (cache_value_visible(wtcursor, &cp)) {
            if (cp->remove)
    ...
    case WT_NOTFOUND:
        ...
        if ((ret = helium_call(wtcursor, "he_lookup", ws->he, he_lookup)) != WT_NOTFOUND) {
            if (ret == 0)
                ret = WT_DUPLICATE_KEY;
            goto err;
        }
    ...
    /*
     * Create a new value using the current cache record plus the WiredTiger cursor's value, and
     * update the cache.
     */
...
err:
    ...
    if (ret == 0)
        ESET(wt_api->transaction_notify(wt_api, session, &hs->txn_notify));
...
update(WT_CURSOR *wtcursor, int remove_op)
{
    ...
    VMSG(wt_api, session, VERBOSE_L2, "%c %.*s.%.*s", remove_op ? 'R' : 'U', (int)r->key_len,
      r->key, (int)r->val_len, r->val);
    ...
    case 0:
        ...
        /*
         * If overwrite is false, no entry (or a removed entry), is an error. We're done checking if
         * there is a visible entry in the cache, otherwise repeat the check on the primary store.
         */
        if (cache_value_visible(wtcursor, &cp)) {
            if (!cp->remove)
                break;
    ...
    case WT_NOTFOUND:
        ...
        /*
         * All we care about is the cache entry, which didn't exist; clear the returned value, we're
    ...
    /*
     * Create a new cache value based on the current cache record plus the WiredTiger cursor's
     * value.
     */
...
helium_cursor_reserve(WT_CURSOR *wtcursor)
{
    ...
    /*
     * XXX We don't currently support reserve, this will require some work. The test programs don't
     * currently detect it, so return success.
     */
    return (0);
}
...
helium_cursor_remove(WT_CURSOR *wtcursor)
{
    ...
    /*
     * WiredTiger's "remove" of a bitfield is really an update with a value of zero.
     */
...
static int
ws_source_name(
  WT_DATA_SOURCE *wtds, WT_SESSION *session, const char *uri, const char *suffix, char **pp)
{
    DATA_SOURCE *ds;
    WT_EXTENSION_API *wt_api;
    size_t len;
    ...
    /*
     * Create the store's name. Application URIs are "helium:device/name"; we want the names on the
     * Helium device to be obviously WiredTiger's, and the device name isn't interesting. Convert to
     * "WiredTiger:name", and add an optional suffix.
     */
    if (!prefix_match(uri, "helium:") || (p = strchr(uri, '/')) == NULL)
        ERET(wt_api, session, EINVAL, "%s: illegal Helium URI", uri);
    ++p;

    len = strlen(WT_NAME_PREFIX) + strlen(p) + (suffix == NULL ? 0 : strlen(suffix)) + 5;
    if ((*pp = malloc(len)) == NULL)
        return (os_errno());
    (void)snprintf(*pp, len, "%s%s%s", WT_NAME_PREFIX, p, suffix == NULL ? "" : suffix);
    return (0);
}
...
ws_source_close(WT_EXTENSION_API *wt_api, WT_SESSION *session, WT_SOURCE *ws)
{
    ...
    /*
     * Warn if open cursors: it shouldn't happen because the upper layers of WiredTiger prevent it,
     * so we don't do anything more than warn.
     */
    if (ws->ref != 0)
        EMSG(wt_api, session, WT_ERROR, "%s: open object with %u open cursors being closed",
          ws->uri, ws->ref);

    if (ws->he != NULL) {
        if ((tret = he_commit(ws->he)) != 0)
            EMSG(wt_api, session, tret, "he_commit: %s: %s", ws->uri, he_strerror(tret));
        if ((tret = he_close(ws->he)) != 0)
            EMSG(wt_api, session, tret, "he_close: %s: %s", ws->uri, he_strerror(tret));
        ws->he = NULL;
    }
    if (ws->he_cache != NULL) {
        if ((tret = he_close(ws->he_cache)) != 0)
            EMSG(wt_api, session, tret, "he_close: %s(cache): %s", ws->uri, he_strerror(tret));
        ws->he_cache = NULL;
    }
 ...
 */
static int
ws_source_open_object(WT_DATA_SOURCE *wtds, WT_SESSION *session, HELIUM_SOURCE *hs, const char *uri,
  const char *suffix, int flags, he_t *hep)
{
    DATA_SOURCE *ds;
    WT_EXTENSION_API *wt_api;
    he_t he;
    ...
    *hep = NULL;
    ...
    if ((he = he_open(hs->device, p, flags, NULL)) == NULL) {
        ret = os_errno();
        EMSG(wt_api, session, ret, "he_open: %s/%s: %s", hs->name, p, he_strerror(ret));
    }
    *hep = he;
...
/*
 * ws_source_open --
 *     Return a locked WiredTiger source, allocating and opening if it doesn't already exist.
 */
static int
ws_source_open(WT_DATA_SOURCE *wtds, WT_SESSION *session, const char *uri, WT_CONFIG_ARG *config,
  u_int flags, WT_SOURCE **refp)
{
    DATA_SOURCE *ds;
    HELIUM_SOURCE *hs;
    WT_CONFIG_ITEM a;
    ...
    *refp = NULL;
    ...
    /*
     * The URI will be "helium:" followed by a Helium name and object name pair separated by a
     * slash, for example, "helium:volume/object".
     */
    ...
    for (hs = ds->hs_head; hs != NULL; hs = hs->next)
        if (string_match(hs->name, p, len))
            break;
    if (hs == NULL)
        ERET(wt_api, NULL, EINVAL, "%s: no matching Helium store found", uri);

    /*
     * We're about to walk the Helium device's list of files, acquire the global lock.
     */
    WT_RET(writelock(wt_api, session, &ds->global_lock));

    /*
     * Check for a match: if we find one, optionally trade the global lock for the object's lock,
     * optionally check if the object is busy, and return.
     */
    ...
    if ((ws = calloc(1, sizeof(*ws))) == NULL || (ws->uri = strdup(uri)) == NULL) {
        ret = os_errno();
        goto err;
    }
     ...
     */
    oflags = HE_O_CREATE;
    if ((ret = wt_api->config_get(wt_api, session, config, "helium_o_compress", &a)) == 0 &&
      a.val != 0)
        oflags |= HE_O_COMPRESS;
    if (ret != 0 && ret != WT_NOTFOUND)
        EMSG_ERR(wt_api, session, ret, "helium_o_compress configuration: %s",
          wt_api->strerror(wt_api, session, ret));
    if ((ret = wt_api->config_get(wt_api, session, config, "helium_o_truncate", &a)) == 0 &&
      a.val != 0)
        oflags |= HE_O_TRUNCATE;
    if (ret != 0 && ret != WT_NOTFOUND)
        EMSG_ERR(wt_api, session, ret, "helium_o_truncate configuration: %s",
          wt_api->strerror(wt_api, session, ret));

    WT_ERR(ws_source_open_object(wtds, session, hs, uri, NULL, oflags, &ws->he));
    WT_ERR(
      ws_source_open_object(wtds, session, hs, uri, WT_NAME_CACHE, HE_O_CREATE, &ws->he_cache));
    if ((ret = he_commit(ws->he)) != 0)
        EMSG_ERR(wt_api, session, ret, "he_commit: %s", he_strerror(ret));
 ...
 */
static int
master_uri_get(WT_DATA_SOURCE *wtds, WT_SESSION *session, const char *uri, char **valuep)
{
    DATA_SOURCE *ds;
    WT_EXTENSION_API *wt_api;
 ...
 */
static int
master_uri_rename(WT_DATA_SOURCE *wtds, WT_SESSION *session, const char *uri, const char *newuri)
{
    DATA_SOURCE *ds;
    WT_EXTENSION_API *wt_api;
    int ret = 0;
    ...
    /*
     * Remove the original record, and if that fails, attempt to remove the new record.
     */
 ...
 */
static int
master_uri_set(WT_DATA_SOURCE *wtds, WT_SESSION *session, const char *uri, WT_CONFIG_ARG *config)
{
    DATA_SOURCE *ds;
    ...
    exclusive = 0;
    if ((ret = wt_api->config_get(wt_api, session, config, "exclusive", &a)) == 0)
        exclusive = a.val != 0;
    else if (ret != WT_NOTFOUND)
        ERET(wt_api, session, ret, "exclusive configuration: %s",
          wt_api->strerror(wt_api, session, ret));

    /* Get the key/value format strings. */
    if ((ret = wt_api->config_get(wt_api, session, config, "key_format", &a)) != 0) {
        if (ret == WT_NOTFOUND) {
            a.str = "u";
            a.len = 1;
        } else
    ...
    if ((ret = wt_api->config_get(wt_api, session, config, "value_format", &b)) != 0) {
        if (ret == WT_NOTFOUND) {
            b.str = "u";
            b.len = 1;
        } else
            ERET(wt_api, session, ret, "value_format configuration: %s",
              wt_api->strerror(wt_api, session, ret));
    }

    /* Get the compression configuration. */
    if ((ret = wt_api->config_get(wt_api, session, config, "helium_o_compress", &c)) != 0) {
        if (ret == WT_NOTFOUND)
            c.val = 0;
        else
            ERET(wt_api, session, ret, "helium_o_compress configuration: %s",
              wt_api->strerror(wt_api, session, ret));
    }

    /*
     * Create a new reference using insert (which fails if the record already exists).
     */
    (void)snprintf(value, sizeof(value),
      "wiredtiger_helium_version=(major=%d,minor=%d),"
      "key_format=%.*s,value_format=%.*s,"
      "helium_o_compress=%d",
      WIREDTIGER_HELIUM_MAJOR, WIREDTIGER_HELIUM_MINOR, (int)a.len, a.str, (int)b.len, b.str,
      c.val ? 1 : 0);
    if ((ret = wt_api->metadata_insert(wt_api, session, uri, value)) == 0)
        return (0);
    if (ret == WT_DUPLICATE_KEY)
        return (exclusive ? EEXIST : 0);
    ERET(wt_api, session, ret, "%s: %s", uri, wt_api->strerror(wt_api, session, ret));
}
 ...
 */
static int
helium_session_open_cursor(WT_DATA_SOURCE *wtds, WT_SESSION *session, const char *uri,
  WT_CONFIG_ARG *config, WT_CURSOR **new_cursor)
{
    ...
    *new_cursor = NULL;
    ...
    if ((ret = wt_api->config_get(wt_api, session, config, "overwrite", &v)) != 0)
        EMSG_ERR(wt_api, session, ret, "overwrite configuration: %s",
          wt_api->strerror(wt_api, session, ret));
    cursor->config_overwrite = v.val != 0;

    if ((ret = wt_api->collator_config(wt_api, session, uri, config, NULL, &own)) != 0)
        EMSG_ERR(wt_api, session, ret, "collator configuration: %s",
          wt_api->strerror(wt_api, session, ret));
    ...
    /*
     * If this is the first access to the URI, we have to configure it using information stored in
     * the master record.
     */
    if (!ws->configured) {
        WT_ERR(master_uri_get(wtds, session, uri, &value));
        ...
        if ((ret = wt_api->config_parser_open(
               wt_api, session, value, strlen(value), &config_parser)) != 0)
            EMSG_ERR(wt_api, session, ret, "Configuration string parser: %s",
              wt_api->strerror(wt_api, session, ret));
        if ((ret = config_parser->get(config_parser, "key_format", &v)) != 0)
            EMSG_ERR(wt_api, session, ret, "key_format configuration: %s",
              wt_api->strerror(wt_api, session, ret));
        ws->config_recno = v.len == 1 && v.str[0] == 'r';

        if ((ret = config_parser->get(config_parser, "value_format", &v)) != 0)
            EMSG_ERR(wt_api, session, ret, "value_format configuration: %s",
              wt_api->strerror(wt_api, session, ret));
        ws->config_bitfield = v.len == 2 && isdigit((u_char)v.str[0]) && v.str[1] == 't';

        /*
         * If it's a record-number key, read the last record from the object and set the allocation
         * record value.
         */
        if (ws->config_recno) {
...
err:
    ...
    if (config_parser != NULL && (tret = config_parser->close(config_parser)) != 0)
        EMSG(wt_api, session, tret, "WT_CONFIG_PARSER.close: %s",
  ...
  WT_DATA_SOURCE *wtds, WT_SESSION *session, const char *uri, WT_CONFIG_ARG *config)
{
    ...
    /*
     * Get a locked reference to the WiredTiger source, then immediately unlock it, we aren't doing
     * anything else.
     */
  ...
  WT_DATA_SOURCE *wtds, WT_SESSION *session, const char *uri, WT_CONFIG_ARG *config)
{
     ...
     */
    WT_RET(
      ws_source_open(wtds, session, uri, config, WS_SOURCE_OPEN_BUSY | WS_SOURCE_OPEN_GLOBAL, &ws));
    hs = ws->hs;
    ...
    /*
     * If we have an error at this point, panic -- there's an inconsistency in what WiredTiger knows
     * about and the underlying store.
     */
    if (ret != 0)
        ret = WT_PANIC;
 ...
 */
static int
helium_session_rename(WT_DATA_SOURCE *wtds, WT_SESSION *session, const char *uri,
  const char *newuri, WT_CONFIG_ARG *config)
{
    DATA_SOURCE *ds;
    WT_EXTENSION_API *wt_api;
    WT_SOURCE *ws;
    ...
    /*
     * Get a locked reference to the data source; hold the global lock, we are going to change the
     * object's name, and we can't allow other threads walking the list and comparing against the
     * name.
     */
    WT_RET(
      ws_source_open(wtds, session, uri, config, WS_SOURCE_OPEN_BUSY | WS_SOURCE_OPEN_GLOBAL, &ws));
  ...
  WT_DATA_SOURCE *wtds, WT_SESSION *session, const char *uri, WT_CONFIG_ARG *config)
{
    ...
    /*
     * XXX Fail URI truncation for now. (Truncation based on a cursor range is handled by the
     * upper-levels of WiredTiger, this is just support for URI truncation.) The problem is there's
     * no way to truncate an open object in Helium without closing handles, and we can't
     * close/re-open handles because we don't have the configuration information from the open.
     */
 ...
 */
static int
helium_session_checkpoint(WT_DATA_SOURCE *wtds, WT_SESSION *session, WT_CONFIG_ARG *config)
{
    DATA_SOURCE *ds;
    HELIUM_SOURCE *hs;
    WT_EXTENSION_API *wt_api;
    ...
    /* Flush all volumes. */
    if ((hs = ds->hs_head) != NULL && (ret = he_commit(hs->he_volume)) != 0)
        ERET(wt_api, session, ret, "he_commit: %s: %s", hs->device, he_strerror(ret));
 ...
 */
static int
helium_source_close(WT_EXTENSION_API *wt_api, WT_SESSION *session, HELIUM_SOURCE *hs)
{
    WT_SOURCE *ws;
    int ret = 0, tret;
    ...
    if (hs->cleaner_id != 0) {
        ...
        if ((tret = pthread_join(hs->cleaner_id, NULL)) != 0)
            EMSG(wt_api, session, tret, "pthread_join: %s", strerror(tret));
        hs->cleaner_id = 0;
    }
    ...
    if (hs->he_txn != NULL && hs->he_owner) {
        if ((tret = he_close(hs->he_txn)) != 0)
            EMSG(wt_api, session, tret, "he_close: %s: %s: %s", hs->name, WT_NAME_TXN,
              he_strerror(tret));
        hs->he_txn = NULL;
        hs->he_owner = false;
    }
    ...
    if (hs->he_volume != NULL) {
        if ((tret = he_commit(hs->he_volume)) != 0)
            EMSG(wt_api, session, tret, "he_commit: %s: %s", hs->device, he_strerror(tret));

        if ((tret = he_close(hs->he_volume)) != 0)
            EMSG(wt_api, session, tret, "he_close: %s: %s: %s", hs->name, WT_NAME_INIT,
              he_strerror(tret));
        hs->he_volume = NULL;
    }
 ...
 */
static int
cache_cleaner(WT_EXTENSION_API *wt_api, WT_CURSOR *wtcursor, uint64_t oldest, uint64_t *txnminp)
{
    CACHE_RECORD *cp;
    CURSOR *cursor;
    HE_ITEM *r;
    ...
    /*
     * Called in two ways: in normal processing mode where we're supplied a value for the oldest
     * transaction ID not yet visible to a running transaction, and we're tracking the smallest
     * transaction ID referenced by any cache entry, and in recovery mode where neither of those are
     * true.
     */
    if (txnminp == NULL)
        recovery = 1;
     ...
     */
    for (r->key_len = 0; (ret = helium_call(wtcursor, "he_next", ws->he_cache, he_next)) == 0;) {
        /*
         * Unmarshall the value, and if all of the updates are globally visible, update the primary
         * with the last committed update. In normal processing, the last committed update test is
         * for a globally visible update that's not explicitly aborted. In recovery processing, the
         * last committed update test is for an explicitly committed update. See the underlying
        ...
        if (cp->remove) {
            ...
            /*
             * Updates confined to the cache may not appear in the primary at all, that is, an
             * insert and remove pair may be confined to the cache.
             */
            if (ret == HE_ERR_ITEM_NOT_FOUND) {
                ret = 0;
                continue;
            }
            ERET(wt_api, NULL, ret, "he_delete: %s", he_strerror(ret));
        } else {
            r->val = cp->v;
            r->val_len = cp->len;
            ...
            ERET(wt_api, NULL, ret, "he_update: %s", he_strerror(ret));
        }
    }
    ...
    /*
     * If we didn't move any keys from the cache to the primary, quit. It's possible we could still
     * remove values from the cache, but not likely, and another pass would probably be wasted
     * effort (especially locked).
     */
    if (!pushed)
        return (0);

    /*
     * Push the store to stable storage for correctness. (It doesn't matter what Helium handle we
     * commit, so we just commit one of them.)
     */
    if ((ret = he_commit(ws->he)) != 0)
        ERET(wt_api, NULL, ret, "he_commit: %s", he_strerror(ret));

    /*
     * If we're performing recovery, that's all we need to do, we're going to simply discard the
     * cache, there's no reason to remove entries one at a time.
     */
    if (recovery)
        return (0);
    ...
    for (r->key_len = 0; (ret = helium_call(wtcursor, "he_next", ws->he_cache, he_next)) == 0;) {
        /*
         * Unmarshall the value, and if all of the updates are globally visible, remove the cache
         * entry.
         */
        WT_ERR(cache_value_unmarshall(wtcursor));
        if (cache_value_visible_all(wtcursor, oldest)) {
            if ((ret = he_delete(ws->he_cache, r)) != 0)
                EMSG_ERR(wt_api, NULL, ret, "he_delete: %s", he_strerror(ret));
            continue;
        }

        /*
         * If the entry will remain in the cache, figure out the oldest transaction for which it
         * contains an update (which might be different from the oldest transaction in the system).
         * We need the oldest transaction ID that appears anywhere in any cache, it limits the
...
txn_cleaner(WT_CURSOR *wtcursor, he_t he_txn, uint64_t txnmin)
{
    ...
    /*
     * Remove all entries from the transaction store that are before the oldest transaction ID that
     * appears anywhere in any cache.
     */
    for (r->key_len = 0; (ret = helium_call(wtcursor, "he_next", he_txn, he_next)) == 0;) {
...
cache_cleaner_worker(void *arg)
{
    ...
    for (cleaner_stop = delay = 0; !cleaner_stop;) {
        /*
         * Check if this will be the final run; cleaner_stop is declared volatile, and so the read
         * will happen. We don't much care if there's extra loops, it's enough if a read eventually
         * happens and finds the variable set. Store the read locally, reading the variable twice
         * might race.
         */
        cleaner_stop = hs->cleaner_stop;

        /*
         * Delay if this isn't the final run and the last pass didn't find any work to do.
         */
...
/*
 * Clean the datastore caches. It's both more and less expensive to return values from the cache:
 * more because we have to marshall/unmarshall the values, less because there's only a single lookup
 * to the cache store rather than a lookup into the cache and then the primary. I have no tuning
 * information, for now, just clean if there have been 1K operations.
 */
...
#define CACHE_SIZE_TRIGGER (1000)
        ...
        /*
         * Get the oldest transaction ID not yet visible to a running transaction. Do this before
         * doing anything else, avoiding any race with creating new WT_SOURCE handles.
         */
        ...
        for (ws = hs->ws_head; ws != NULL; ws = ws->next) {
            ...
            cursor->ws = ws;
            WT_ERR(cache_cleaner(wt_api, wtcursor, oldest, &txntmp));
            if (txntmp < txnmin)
                txnmin = txntmp;
        }
 ...
 */
static int
helium_config_read(WT_EXTENSION_API *wt_api, WT_CONFIG_ITEM *config, char **devicep, HE_ENV *envp,
  int *env_setp, int *flagsp)
{
    WT_CONFIG_ITEM k, v;
    WT_CONFIG_PARSER *config_parser;
    int ret = 0, tret;
    ...
    *flagsp = 0;
    ...
    if ((ret = wt_api->config_parser_open(
        ...
        ERET(wt_api, NULL, ret, "WT_EXTENSION_API.config_parser_open: %s",
    ...
    while ((ret = config_parser->next(config_parser, &k, &v)) == 0) {
        ...
        EMSG_ERR(wt_api, NULL, EINVAL, "unknown configuration key value pair %.*s=%.*s", (int)k.len,
    ...
    if (ret != 0)
        EMSG_ERR(
          wt_api, NULL, ret, "WT_CONFIG_PARSER.next: %s", wt_api->strerror(wt_api, NULL, ret));

err:
    if ((tret = config_parser->close(config_parser)) != 0)
        EMSG(
          wt_api, NULL, tret, "WT_CONFIG_PARSER.close: %s", wt_api->strerror(wt_api, NULL, tret));
...
helium_source_open(DATA_SOURCE *ds, WT_CONFIG_ITEM *k, WT_CONFIG_ITEM *v)
{
    ...
    VMSG(wt_api, NULL, VERBOSE_L1, "volume %.*s=%.*s", (int)k->len, k->str, (int)v->len, v->str);

    /*
     * Check for a Helium source we've already opened: we don't check the value (which implies you
     * can open the same underlying stores using more than one name, but I don't know of any
     * problems that causes), we only check the key, that is, the top-level WiredTiger name.
     */
    for (hs = ds->hs_head; hs != NULL; hs = hs->next)
        if (string_match(hs->name, k->str, k->len))
            ERET(wt_api, NULL, EINVAL, "%s: device already open", hs->name);

    /* Allocate and initialize a new underlying Helium source object. */
    if ((hs = calloc(1, sizeof(*hs))) == NULL || (hs->name = calloc(1, k->len + 1)) == NULL) {
        free(hs);
        return (os_errno());
    }
    ...
    WT_ERR(helium_config_read(wt_api, v, &hs->device, &env, &env_set, &flags));
    if (hs->device == NULL)
        EMSG_ERR(wt_api, NULL, EINVAL, "%s: no Helium volumes specified", hs->name);

    /*
     * Open the Helium volume, creating it if necessary. We have to open an object at the same time,
     * that's why we have object flags as well as volume flags.
     */
    flags |= HE_O_CREATE | HE_O_TRUNCATE | HE_O_CLEAN | HE_O_VOLUME_CREATE;
    if ((hs->he_volume = he_open(hs->device, WT_NAME_INIT, flags, env_set ? &env : NULL)) == NULL) {
        ret = os_errno();
        EMSG_ERR(
          wt_api, NULL, ret, "he_open: %s: %s: %s", hs->name, WT_NAME_INIT, he_strerror(ret));
    }
...
helium_source_txn_open(DATA_SOURCE *ds)
{
    ...
    /*
     * If we didn't find a transaction store, open a transaction store in the first Helium source we
     * loaded. (It could just as easily be the last one we loaded, we're just picking one, but
     * picking the first seems slightly less likely to make people wonder.)
     */
    if ((hs = hs_txn) == NULL) {
        for (hs = ds->hs_head; hs->next != NULL; hs = hs->next)
            ;
        if ((he_txn = he_open(hs->device, WT_NAME_TXN, HE_O_CREATE, NULL)) == NULL) {
            ret = os_errno();
            ERET(wt_api, NULL, ret, "he_open: %s: %s: %s", hs->name, WT_NAME_TXN, he_strerror(ret));
        }
    ...
    VMSG(wt_api, NULL, VERBOSE_L1,
      "%s"
      "transactional store on %s",
...
helium_source_txn_truncate(DATA_SOURCE *ds)
{
    ...
    /*
     * We want to truncate the transaction store after recovery, but there isn't a Helium truncate
     * operation. Remove/re-open the store instead.
     */
    hs = ds->hs_head;
    if (hs->he_txn != NULL && (ret = he_remove(hs->he_txn)) != 0)
        ERET(wt_api, NULL, ret, "he_remove: %s: %s: %s", hs->name, WT_NAME_TXN, he_strerror(ret));
...
static int
helium_source_recover_namespace(
  WT_DATA_SOURCE *wtds, HELIUM_SOURCE *hs, const char *name, WT_CONFIG_ARG *config)
{
    CURSOR *cursor;
    DATA_SOURCE *ds;
    WT_CURSOR *wtcursor;
    ...
    /*
     * The name we store on the Helium device is a translation of the WiredTiger name: do the
     * reverse process here so we can use the standard source-open function.
     */
    ...
    /*
     * Open the cache/primary pair by going through the full open process, instantiating the
     * underlying WT_SOURCE object.
     */
    ...
    if ((ret = he_remove(ws->he_cache)) != 0)
        EMSG(wt_api, NULL, ret, "he_remove: %s(cache): %s", ws->uri, he_strerror(ret));
    ws->he_cache = NULL; /* The handle is dead. */
...
helium_namespace_list(void *cookie, const char *name)
{
    ...
    /*
     * Ignore any files without a WiredTiger prefix. Ignore the metadata and cache files.
     */
    ...
    if (strcmp(name, WT_NAME_TXN) == 0)
        return (0);
    if (string_match(strrchr(name, '.'), WT_NAME_CACHE, strlen(WT_NAME_CACHE)))
        return (0);

    if (names->list_cnt + 1 >= names->list_max) {
        if ((allocp = realloc(names->list, (names->list_max + 20) * sizeof(names->list[0]))) ==
          NULL)
            return (os_errno());
        names->list = allocp;
        names->list_max += 20;
 ...
 */
static int
helium_source_recover(WT_DATA_SOURCE *wtds, HELIUM_SOURCE *hs, WT_CONFIG_ARG *config)
{
    struct helium_namespace_cookie names;
    DATA_SOURCE *ds;
    WT_EXTENSION_API *wt_api;
    ...
    if ((ret = he_enumerate(hs->device, helium_namespace_list, &names)) != 0)
        ERET(wt_api, NULL, ret, "he_enumerate: %s: %s", hs->name, he_strerror(ret));

    /* Recover the objects. */
    for (i = 0; i < names.list_cnt; ++i)
        WT_ERR(helium_source_recover_namespace(wtds, hs, names.list[i], config));
...
helium_terminate(WT_DATA_SOURCE *wtds, WT_SESSION *session)
{
    ...
    /*
     * Close the Helium sources, close the Helium source that "owns" the database transaction store
     * last.
     */
    last = NULL;
...
wiredtiger_extension_init(WT_CONNECTION *connection, WT_CONFIG_ARG *config)
{
    ...
    static const char *session_create_opts[] = {"helium_o_compress=0", /* HE_O_COMPRESS */
...
/* Check the library version */
    ...
    ERET(wt_api, NULL, EINVAL, "unsupported Helium header file %d.%d, expected version 2.12",
    ...
    if ((ret = wt_api->config_parser_open_arg(wt_api, NULL, config, &config_parser)) != 0)
        EMSG_ERR(wt_api, NULL, ret, "WT_EXTENSION_API.config_parser_open: config: %s",
          wt_api->strerror(wt_api, NULL, ret));
    while ((ret = config_parser->next(config_parser, &k, &v)) == 0) {
    ...
    if (ret != WT_NOTFOUND)
        EMSG_ERR(wt_api, NULL, ret, "WT_CONFIG_PARSER.next: config: %s",
          wt_api->strerror(wt_api, NULL, ret));
    if ((ret = config_parser->close(config_parser)) != 0)
        EMSG_ERR(wt_api, NULL, ret, "WT_CONFIG_PARSER.close: config: %s",
          wt_api->strerror(wt_api, NULL, ret));
    config_parser = NULL;

    /*
     * Find and open the database transaction store, recover each Helium source, then discard the
     * transaction store's contents.
     */
    ...
    for (hs = ds->hs_head; hs != NULL; hs = hs->next)
        if ((ret = pthread_create(&hs->cleaner_id, NULL, cache_cleaner_worker, hs)) != 0)
            EMSG_ERR(
              wt_api, NULL, ret, "%s: pthread_create: cleaner thread: %s", hs->name, strerror(ret));

    /* Add Helium-specific WT_SESSION.create configuration options.  */
    for (p = session_create_opts; *p != NULL; ++p)
        if ((ret = connection->configure_method(
               connection, "WT_SESSION.create", "helium:", *p, "boolean", NULL)) != 0)
    ...
    /* Add the data source */
    if ((ret = connection->add_data_source(connection, "helium:", (WT_DATA_SOURCE *)ds, NULL)) != 0)
        EMSG_ERR(wt_api, NULL, ret, "WT_CONNECTION.add_data_source: %s",
          wt_api->strerror(wt_api, NULL, ret));
    return (0);
