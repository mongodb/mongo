...
#
# Update Heavy workload- ~90% updates, ~5% inserts, ~5% reads + checkpoints,
...
# Updates in snapshot transactions (~90% of total ops)
...
# ------------------------
# Lagged readers near stable_timestamp (~5% of total ops)
# ------------------------
# stable_timestamp_lag = 50 below; these lags are just behind it.
lags = [60, 75, 90, 105, 120, 135, 150, 165, 180, 195]
reader_threads = []

# Reads: 10 lags * 1 thread/lag * 500 ops => 5,000 read ops total
for lag in lags:
...
# ------------------------
# Writes: 90 * tupdate (90k updates) + 5 * tinsert (5k inserts) => 95k write ops
threads = 90 * tupdate + 5 * tinsert

# Reads: 10 lags * 1 thread each * 500 ops => 5k read ops
for tread in reader_threads:
    threads = threads + tread
...
print("disagg workload: update-heavy with checkpoints:")
start_time = time.time()
