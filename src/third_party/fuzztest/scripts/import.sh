...
GTEST_GIT_URL="https://github.com/mongodb-forks/fuzztest"
VERSION=2025-07-28
GTEST_GIT_BRANCH=v${VERSION}-mongo

GIT_ROOT=$(git rev-parse --show-toplevel)
DEST_DIR=$GIT_ROOT/src/third_party/fuzztest
if [[ -d $DEST_DIR/dist ]]; then
    echo "You must remove '$DEST_DIR/dist' before running $0" >&2
    exit 1
fi

# Import files from our fork of fuzztest.
import() {
    LIB_GIT_DIR=$(mktemp -d /tmp/import-fuzztest.XXXXXX)
    pushd $LIB_GIT_DIR
