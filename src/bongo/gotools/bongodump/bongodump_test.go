...
var (
	...
	testDB = "bongodump_test_db"
	// temp database used for restoring a DB
	testRestoreDB       = "temp_bongodump_restore_test_db"
...
func simpleBongoDumpInstance() *BongoDump {
...
func setUpBongoDumpTestData() error {
	session, err := getBareSession()
	if err != nil {
		return err
	}
...
func tearDownBongoDumpTestData() error {
	session, err := getBareSession()
	if err != nil {
		return err
	}
...
func testQuery(md *BongoDump, session *mgo.Session) string {
...
func TestBongoDumpValidateOptions(t *testing.T) {
	testutil.VerifyTestType(t, testutil.UnitTestType)

	Convey("With a BongoDump instance", t, func() {
		md := simpleBongoDumpInstance()
...
func TestBongoDumpKerberos(t *testing.T) {
	testutil.VerifyTestType(t, testutil.KerberosTestType)

	Convey("Should be able to run bongodump with Kerberos auth", t, func() {
		...
		bongoDump.OutputOptions.Out = KerberosDumpDirectory
...
func TestBongoDumpBSON(t *testing.T) {
	testutil.VerifyTestType(t, testutil.IntegrationTestType)
	log.SetWriter(ioutil.Discard)

	Convey("With a BongoDump instance", t, func() {
		err := setUpBongoDumpTestData()
		So(err, ShouldBeNil)

		Convey("testing that using BongoDump WITHOUT giving a query dumps everything in the database and/or collection", func() {
			md := simpleBongoDumpInstance()
		...
		Convey("testing that using BongoDump WITH a query dumps a subset of documents in a database and/or collection", func() {
			session, err := getBareSession()
			So(err, ShouldBeNil)
			md := simpleBongoDumpInstance()
		...
		Reset(func() {
			So(tearDownBongoDumpTestData(), ShouldBeNil)
		})
	})
}

func TestBongoDumpMetaData(t *testing.T) {
	testutil.VerifyTestType(t, testutil.IntegrationTestType)
	log.SetWriter(ioutil.Discard)
