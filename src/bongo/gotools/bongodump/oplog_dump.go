...
func (dump *BongoDump) determineOplogCollectionName() error {
	masterDoc := bson.M{}
	...
	if isMaster := masterDoc["ismaster"]; util.IsFalsy(isMaster) {
		log.Logvf(log.Info, "bongodump is not connected to a master")
...
func (dump *BongoDump) getOplogStartTime() (bson.BongoTimestamp, error) {
...
// since bongodump started. It does this by checking the oldest oplog entry
// still in the database and making sure it happened at or before the timestamp
// captured at the start of the dump.
func (dump *BongoDump) checkOplogTimestampExists(ts bson.BongoTimestamp) (bool, error) {
...
func (dump *BongoDump) DumpOplogAfterTimestamp(ts bson.BongoTimestamp) error {
