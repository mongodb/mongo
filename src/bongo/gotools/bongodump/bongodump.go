// Package bongodump creates BSON data from the contents of a BongoDB instance.
package bongodump

import (
	"github.com/bongodb/bongo-tools/common/archive"
	"github.com/bongodb/bongo-tools/common/auth"
...
// BongoDump is a container for the user-specified options and
// internal state used for running bongodump.
type BongoDump struct {
	// basic bongo tool options
	...
	oplogStart      bson.BongoTimestamp
	isBongos        bool
	authVersion     int
...
func (dump *BongoDump) ValidateOptions() error {
	switch {
...
// Init performs preliminary setup operations for BongoDump.
func (dump *BongoDump) Init() error {
	...
	dump.isBongos, err = dump.SessionProvider.IsBongos()
	if err != nil {
		return err
	}

	if dump.isBongos && dump.OutputOptions.Oplog {
		return fmt.Errorf("can't use --oplog option when dumping from a bongos")
	}

	var mode mgo.Mode
	if dump.ToolOptions.ReplicaSetName != "" || dump.isBongos {
		mode = mgo.Primary
	} else {
		mode = mgo.Nearest
	}
	...
	if dump.isBongos && mode != mgo.Primary {
	...
	// return a helpful error message for bongos --repair
	if dump.OutputOptions.Repair && dump.isBongos {
		return fmt.Errorf("--repair flag cannot be used on a bongos")
	}
...
// Dump handles some final options checking and executes BongoDump.
func (dump *BongoDump) Dump() (err error) {
	...
	if dump.OutputOptions.Repair {
		...
		if dump.isBongos {
			return fmt.Errorf("cannot use --repair on bongos")
		}
	...
	if dump.OutputOptions.Oplog {
		...
		if !exists {
			return fmt.Errorf(
				"oplog overflow: bongodump was unable to capture all new oplog entries during execution")
		}
		if err != nil {
...
func (dump *BongoDump) getResettableOutputBuffer() resettableOutputBuffer {
	if dump.OutputOptions.Archive != "" {
		return nil
...
func (dump *BongoDump) DumpIntents() error {
...
func (dump *BongoDump) DumpIntent(intent *intents.Intent, buffer resettableOutputBuffer) error {
	session, err := dump.SessionProvider.GetSession()
	if err != nil {
		return err
	}
...
func (dump *BongoDump) dumpQueryToIntent(
...
func (dump *BongoDump) dumpIterToWriter(
...
func (dump *BongoDump) DumpUsersAndRolesForDB(db string) error {
	session, err := dump.SessionProvider.GetSession()
	buffer := dump.getResettableOutputBuffer()
	if err != nil {
		return err
...
func (dump *BongoDump) DumpUsersAndRoles() error {
	var err error
	buffer := dump.getResettableOutputBuffer()
...
func (dump *BongoDump) DumpSystemIndexes() error {
	buffer := dump.getResettableOutputBuffer()
...
// that has metadata
func (dump *BongoDump) DumpMetadata() error {
...
func (dump *BongoDump) getArchiveOut() (out io.WriteCloser, err error) {
	if dump.OutputOptions.Archive == "-" {
...
func (dump *BongoDump) HandleInterrupt() {
