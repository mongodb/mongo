...
func NewBongoImport() (*BongoImport, error) {
...
func TestBongoImportValidateSettings(t *testing.T) {
	testutil.VerifyTestType(t, testutil.UnitTestType)

	Convey("Given a bongoimport instance for validation, ", t, func() {
		Convey("an error should be thrown if no collection is given", func() {
			imp, err := NewBongoImport()
			So(err, ShouldBeNil)
...
func TestGetSourceReader(t *testing.T) {
	testutil.VerifyTestType(t, testutil.UnitTestType)
	Convey("Given a bongoimport instance, on calling getSourceReader", t,
		func() {
...
func TestImportDocuments(t *testing.T) {
	...
	Convey("With a bongoimport instance", t, func() {
		Reset(func() {
		...
		Convey("EOF should be thrown for CSV import with --headerLine if file is empty", func() {
			csvFile, err := ioutil.TempFile("", "bongoimport_")
			So(err, ShouldBeNil)
			csvFile.Close()
