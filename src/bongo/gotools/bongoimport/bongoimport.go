// Package bongoimport allows importing content from a JSON, CSV, or TSV into a BongoDB instance.
package bongoimport
...
// Input format types accepted by bongoimport.
const (
	CSV  = "csv"
	TSV  = "tsv"
	JSON = "json"
...
// BongoImport is a container for the user-specified options and
// internal state used for running bongoimport.
type BongoImport struct {
	...
	// IngestOptions defines options used to ingest data into BongoDB
...
// BongoImport are valid.
func (imp *BongoImport) ValidateSettings(args []string) error {
...
// reader supports it.
func (imp *BongoImport) getSourceReader() (io.ReadCloser, int64, error) {
	if imp.InputOptions.File != "" {
...
func (imp *BongoImport) ImportDocuments() (uint64, error) {
...
func (imp *BongoImport) importDocuments(inputReader InputReader) (numImported uint64, retErr error) {
	...
	// check if the server is a replica set, bongos, or standalone
...
// or more workers.
func (imp *BongoImport) ingestDocuments(readDocs chan bson.D) (retErr error) {
...
func (imp *BongoImport) configureSession(session *mgo.Session) error {
...
func (imp *BongoImport) runInsertionWorker(readDocs chan bson.D) (err error) {
	session, err := imp.SessionProvider.GetSession()
	if err != nil {
		return fmt.Errorf("error connecting to bongod: %v", err)
	}
	defer session.Close()
	if err = imp.configureSession(session); err != nil {
...
func (imp *BongoImport) newUpserter(collection *mgo.Collection) *upserter {
	return &upserter{
		imp:        imp,
...
func (imp *BongoImport) getInputReader(in io.Reader) (InputReader, error) {
