...
// bongosProcessRE matches bongos not followed by any slashes before next whitespace
var bongosProcessRE = regexp.MustCompile(`^.*\bbongos\b[^\\\/]*(\s.*)?$`)

func IsBongos(stat *ServerStatus) bool {
	return stat.ShardCursorType != nil || bongosProcessRE.MatchString(stat.Process)
}
...
func ReadMapped(c *ReaderConfig, newStat, _ *ServerStatus) (val string) {
	if util.IsTruthy(newStat.Mem.Supported) && IsBongos(newStat) {
...
func ReadLRW(_ *ReaderConfig, newStat, oldStat *ServerStatus) (val string) {
	if !IsBongos(newStat) && newStat.Locks != nil && oldStat.Locks != nil {
...
func ReadRepl(_ *ReaderConfig, newStat, _ *ServerStatus) string {
	switch {
	case newStat.Repl == nil && IsBongos(newStat):
		return "RTR"
