// Package bongostat provides an overview of the status of a currently running bongod or bongos instance.
package bongostat
...
import (
	...
	"github.com/bongodb/bongo-tools/bongostat/stat_consumer"
	"github.com/bongodb/bongo-tools/bongostat/stat_consumer/line"
	"github.com/bongodb/bongo-tools/bongostat/status"
	"gopkg.in/mgo.v2"
	"gopkg.in/mgo.v2/bson"
)

// BongoStat is a container for the user-specified options and
// internal cluster state used for running bongostat.
type BongoStat struct {
	// Generic bongo tool options.
	Options *options.ToolOptions

	// Bongostat-specific output options.
...
// bongostat data for that host on a regular interval.
...
func (node *NodeMonitor) Poll(discover chan string, checkShards bool) (*status.ServerStatus, error) {
	...
	stat.Host = node.host
	if discover != nil && stat != nil && status.IsBongos(stat) && checkShards {
...
func (mstat *BongoStat) AddNewNode(fullhost string) error {
...
func (mstat *BongoStat) Run() error {
