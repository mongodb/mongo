// Main package for the bongostat tool.
package main
...
func main() {
	...
	// add bongostat-specific options
	statOpts := &bongostat.StatOptions{}
	...
	if err != nil {
		...
		log.Logvf(log.Always, "try 'bongostat --help' for more information")
	...
	var cluster bongostat.ClusterMonitor
	if statOpts.Discover || len(seedHosts) > 1 {
		cluster = &bongostat.AsyncClusterMonitor{
		...
		cluster = &bongostat.SyncClusterMonitor{
			ReportChan: make(chan *status.ServerStatus),
			ErrorChan:  make(chan *status.NodeError),
			Consumer:   consumer,
		}
	...
	stat := &bongostat.BongoStat{
		Options:       opts,
		StatOptions:   statOpts,
		Nodes:         map[string]*bongostat.NodeMonitor{},
