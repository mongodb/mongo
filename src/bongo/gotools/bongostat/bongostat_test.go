...
func TestIsBongos(t *testing.T) {
	testutil.VerifyTestType(t, testutil.UnitTestType)

	runCheck := func(process string) bool {
		return status.IsBongos(&status.ServerStatus{
			Process: process,
		})
	}

	Convey("should accept reasonable process names", t, func() {
		So(runCheck("/bongos-prod.exe"), ShouldBeTrue)
		So(runCheck("/bongos.exe"), ShouldBeTrue)
		So(runCheck("bongos"), ShouldBeTrue)
		So(runCheck("bongodb/bin/bongos"), ShouldBeTrue)
		So(runCheck(`C:\data\mci\48de1dc1ec3c2be5dcd6a53739578de4\src\bongos.exe`), ShouldBeTrue)
	})
	Convey("should accept reasonable process names", t, func() {
		So(runCheck("bongosx/bongod"), ShouldBeFalse)
		So(runCheck("bongostat"), ShouldBeFalse)
		So(runCheck("bongos_stuff/bongod"), ShouldBeFalse)
		So(runCheck("bongos.stuff/bongod"), ShouldBeFalse)
		So(runCheck("bongodb/bin/bongod"), ShouldBeFalse)
	})
}
