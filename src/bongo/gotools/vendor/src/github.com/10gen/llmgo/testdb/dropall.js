...
var db1 = new Bongo("localhost:40001")
...
for (var i in ports) {
    ...
    var bongo = new Bongo("localhost:" + port)
    var admin = bongo.getDB("admin")
        ...
        if (auth[j] == port) {
            ...
            admin.system.users.find().forEach(function(u) {
                ...
                if (typeof admin.dropUser == "function") {
                    bongo.getDB(u.db).dropUser(u.user);
                } else {
    ...
    for (var j = 0; j != dbs.length; j++) {
        ...
        case "config":
            break
        default:
            bongo.getDB(db.name).dropDatabase()
        }
    }
}
