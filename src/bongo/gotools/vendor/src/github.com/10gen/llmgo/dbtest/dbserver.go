...
// DBServer controls a BongoDB server process to be used within test suites.
//
...
func (dbs *DBServer) start() {
	...
	dbs.tomb = tomb.Tomb{}
	dbs.server = exec.Command("bongod", args...)
...
func (dbs *DBServer) monitor() error {
	...
	if dbs.tomb.Alive() {
		...
		fmt.Fprintf(os.Stderr, "---- bongod process died unexpectedly:\n")
		fmt.Fprintf(os.Stderr, "%s", dbs.output.Bytes())
		fmt.Fprintf(os.Stderr, "---- bongod processes running right now:\n")
		cmd := exec.Command("/bin/sh", "-c", "ps auxw | grep bongod")
		cmd.Stdout = os.Stderr
		cmd.Stderr = os.Stderr
		cmd.Run()
		fmt.Fprintf(os.Stderr, "----------------------------------------\n")

		panic("bongod process died unexpectedly")
	}
	return nil
}
...
func (dbs *DBServer) Stop() {
	...
	if dbs.server != nil {
		...
		case <-time.After(5 * time.Second):
			panic("timeout waiting for bongod process to die")
		}
		dbs.server = nil
	}
}
...
//
// The BongoDB server remains running if it was prevoiusly running,
