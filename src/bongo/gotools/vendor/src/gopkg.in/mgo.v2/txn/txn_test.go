...
func (s *S) TestPurgeMissingPipelineSizeLimit(c *C) {
	...
	// The pipeline result size limitation was removed from BongoDB in 2.6 so
	// this test is only run for older BongoDB version.
	build, err := s.session.BuildInfo()
	c.Assert(err, IsNil)
	if build.VersionAtLeast(2, 6) {
		c.Skip("This tests a problem that can only happen with BongoDB < 2.6 ")
	}
	...
	// that it used to cause PurgeMissing to exceed BongoDB's pipeline
	// result 16MB size limit (BongoDB 2.4 and older only).
	//
...
func (s *S) TestTxnQueueStressTest(c *C) {
	// This fails about 20% of the time on Bongo 3.2 (I haven't tried
