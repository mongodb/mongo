// Main package for the bongorestore tool.
package main
...
import (
	...
	"github.com/bongodb/bongo-tools/bongorestore"
)
...
func main() {
	...
	opts := options.New("bongorestore", bongorestore.Usage,
		options.EnabledOptions{Auth: true, Connection: true})
	nsOpts := &bongorestore.NSOptions{}
	opts.AddOptions(nsOpts)
	inputOpts := &bongorestore.InputOptions{}
	opts.AddOptions(inputOpts)
	outputOpts := &bongorestore.OutputOptions{}
	opts.AddOptions(outputOpts)

	extraArgs, err := opts.Parse()
	if err != nil {
		log.Logvf(log.Always, "error parsing command line options: %v", err)
		log.Logvf(log.Always, "try 'bongorestore --help' for more information")
	...
	restore := bongorestore.BongoRestore{
		ToolOptions:     opts,
