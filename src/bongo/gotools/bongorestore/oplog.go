...
// RestoreOplog attempts to restore a BongoDB oplog.
func (restore *BongoRestore) RestoreOplog() error {
...
func (restore *BongoRestore) ApplyOps(session *mgo.Session, entries []interface{}) error {
	res := bson.M{}
...
// applied to bongorestore's target database.
func (restore *BongoRestore) TimestampBeforeLimit(ts bson.BongoTimestamp) bool {
...
// It parses this timestamp string and returns a bson.BongoTimestamp type.
func ParseTimestampFlag(ts string) (bson.BongoTimestamp, error) {
	...
	return bson.BongoTimestamp(timestamp), nil
}
