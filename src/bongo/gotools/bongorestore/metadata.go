...
type authVersionPair struct {
	...
	Dump int
	// Server is the auth version of the connected BongoDB server
	Server int
}
...
func (restore *BongoRestore) MetadataFromJSON(jsonBytes []byte) (bson.D, []IndexDocument, error) {
	if len(jsonBytes) == 0 {
...
// caches them in the BongoRestore object.
func (restore *BongoRestore) LoadIndexesFromBSON() error {
...
func (restore *BongoRestore) CollectionExists(intent *intents.Intent) (bool, error) {
...
func (restore *BongoRestore) CreateIndexes(intent *intents.Intent, indexes []IndexDocument) error {
...
func (restore *BongoRestore) LegacyInsertIndex(intent *intents.Intent, index IndexDocument) error {
	session, err := restore.SessionProvider.GetSession()
	if err != nil {
		return fmt.Errorf("error establishing connection: %v", err)
	}
...
// given options.
func (restore *BongoRestore) CreateCollection(intent *intents.Intent, options bson.D) error {
...
// nothing is done.
func (restore *BongoRestore) RestoreUsersOrRoles(users, roles *intents.Intent) error {
	...
	for _, arg := range args {
		...
		if arg.intent.Size == 0 {
			// BongoDB complains if we try and remove a non-existent collection, so we should
...
func (restore *BongoRestore) GetDumpAuthVersion() (int, error) {
	...
	if intent == nil {
		if restore.InputOptions.RestoreDBUsersAndRoles {
			...
			log.Logv(log.Always, "if users are from an earlier version of BongoDB, they may not restore properly")
			return 3, nil
		}
...
// are incompatible.
func (restore *BongoRestore) ValidateAuthVersions() error {
...
// ShouldRestoreUsersAndRoles returns true if bongorestore should go through
// through the process of restoring collections pertaining to authentication.
func (restore *BongoRestore) ShouldRestoreUsersAndRoles() bool {
...
func (restore *BongoRestore) DropCollection(intent *intents.Intent) error {
	session, err := restore.SessionProvider.GetSession()
	if err != nil {
		return fmt.Errorf("error establishing connection: %v", err)
	}
