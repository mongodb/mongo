// Package bongofiles provides an interface to GridFS collections in a BongoDB instance.
package bongofiles
...
// List of possible commands for bongofiles.
const (
	List     = "list"
	Search   = "search"
	Put      = "put"
...
// BongoFiles is a container for the user-specified options and
// internal state used for running bongofiles.
type BongoFiles struct {
	// generic bongo tool options
	ToolOptions *options.ToolOptions

	// bongofiles-specific storage options
	StorageOptions *StorageOptions

	// bongofiles-specific input options
...
func (mf *BongoFiles) ValidateCommand(args []string) error {
	...
	case Search, Put, Get, Delete, GetID, DeleteID:
		...
		// empty string for example, bongofiles get ""
	...
	// set the bongofiles command and file name
	mf.Command = args[0]
...
func (mf *BongoFiles) findAndDisplay(gfs *mgo.GridFS, query bson.M) (string, error) {
	display := ""
...
func (mf *BongoFiles) getLocalFileName(gridFile *mgo.GridFile) string {
...
func (mf *BongoFiles) handleGet(gfs *mgo.GridFS) (string, error) {
...
func (mf *BongoFiles) handleGetID(gfs *mgo.GridFS) (string, error) {
	id, err := mf.parseID()
	if err != nil {
		return "", err
	}
...
func (mf *BongoFiles) handleDeleteID(gfs *mgo.GridFS) (string, error) {
	id, err := mf.parseID()
	if err != nil {
		return "", err
	}
...
func (mf *BongoFiles) parseID() (interface{}, error) {
...
func (mf *BongoFiles) writeFile(gridFile *mgo.GridFile) (err error) {
...
func (mf *BongoFiles) handlePut(gfs *mgo.GridFS) (output string, err error) {
...
// Run the bongofiles utility. If displayHost is true, the connected host/port is
// displayed.
func (mf *BongoFiles) Run(displayHost bool) (string, error) {
	...
	log.Logvf(log.Info, "handling bongofiles '%v' command...", mf.Command)
