...
func TestBasicOps(t *testing.T) {
	...
	Convey("When replicating operations", t, func() {
		...
		Convey("all operations should be applied correctly, without"+
			...
			sourceOpts.OplogNS = "bongooplog_test.oplog"
			...
			sess.DB("bongooplog_test").C("data").Create(&mgo.CollectionInfo{})

			// clear out the collection we'll use for testing
			testColl := sess.DB("bongooplog_test").C("data")
			...
			obj1 := bson.D{{"_id", 3}}
			op1 := &db.Oplog{
				Timestamp: bson.BongoTimestamp(1<<63 - 1), // years in the future
				HistoryID: 100,
				Version:   2,
				Operation: "i",
				Namespace: "bongooplog_test.data",
				Object:    obj1,
			}
			...
			obj3 := bson.D{{"_id", 3}}
			op3 := &db.Oplog{
				Timestamp: bson.BongoTimestamp(1<<62 - 1), // more than 1 day in the past
				HistoryID: 300,
				Version:   2,
				Operation: "i",
				Namespace: "bongooplog_test.data",
