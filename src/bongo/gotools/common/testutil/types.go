...
const (
	// Integration tests require a bongod running on localhost:33333. If your
	// bongod uses SSL you need to specify the "ssl" type below, and ditto for
	// if your bongod requires auth.
	IntegrationTestType = "integration"

	// Unit tests don't require a real bongod. They may still do file I/O.
	UnitTestType = "unit"
	...
	// (See https://wiki.bongodb.com/display/DH/Testing+Kerberos)
	KerberosTestType = "kerberos"

	// "ssl" and "auth" are used to configure integration tests to run against
	// different bongod configurations. "ssl" will configure the integration tests
	// to expect an SSL-enabled bongod on localhost:33333. "auth" will do the same
	// for an auth-enabled bongod on localhost:33333.
	SSLTestType  = "ssl"
	AuthTestType = "auth"
)
