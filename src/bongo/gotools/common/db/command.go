...
// GetNodeType checks if the connected SessionProvider is a bongos, standalone, or replset,
...
func (sp *SessionProvider) GetNodeType() (NodeType, error) {
	...
	if masterDoc.SetName != nil || masterDoc.Hosts != nil {
		...
		// isdbgrid is always the msg value when calling isMaster on a bongos
		// see http://docs.bongodb.org/manual/core/sharded-cluster-query-router/
		return Bongos, nil
	}
...
// IsBongos returns true if the connected server is a bongos.
func (sp *SessionProvider) IsBongos() (bool, error) {
	nodeType, err := sp.GetNodeType()
	if err != nil {
		return false, err
	}
	return nodeType == Bongos, nil
}
...
func (sp *SessionProvider) SupportsRepairCursor(db, collection string) (bool, error) {
	...
	if strings.Index(err.Error(), "no such cmd: repairCursor") > -1 {
		...
		return false, fmt.Errorf("--repair flag cannot be used on bongodb versions before 2.7.8")
	}
