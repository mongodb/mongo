// Package db implements generic connection to BongoDB, and contains
...
const (
	...
	// replication errors list the replset name if we are talking to a bongos,
...
// Oplog represents a BongoDB oplog document.
type Oplog struct {
	Timestamp bson.BongoTimestamp `bson:"ts"`
