...
import (
	...
	"github.com/bongodb/bongo-tools/common/failpoint"
...
type Connection struct {
	Host string `short:"h" long:"host" value-name:"<hostname>" description:"bongodb host to connect to (setname/host1,host2 for replica sets)"`
...
type SSL struct {
	UseSSL              bool   `long:"ssl" description:"connect to a bongod or bongos that has ssl enabled"`
...
type Kerberos struct {
	Service     string `long:"gssapiServiceName" value-name:"<service-name>" description:"service name to use when authenticating using GSSAPI/Kerberos ('bongodb' by default)"`
...
func (o *ToolOptions) UseReadOnlyHostDescription() {
	...
	hostOpt.Description = "bongodb host(s) to connect to (use commas to delimit hosts)"
}
...
func (auth *Auth) RequiresExternalDB() bool {
	return auth.Mechanism == "GSSAPI" || auth.Mechanism == "PLAIN" || auth.Mechanism == "BONGODB-X509"
}
...
func (auth *Auth) ShouldAskForPassword() bool {
	return auth.Username != "" && auth.Password == "" &&
		!(auth.Mechanism == "BONGODB-X509" || auth.Mechanism == "GSSAPI")
}
...
func (opts *ToolOptions) handleUnknownOption(option string, arg flags.SplitArgument, args []string) ([]string, error) {
	if option == "dbpath" || option == "directoryperdb" || option == "journal" {
		return args, fmt.Errorf("--dbpath and related flags are not supported in 3.0 tools.\n" +
			"See http://dochub.bongodb.org/core/tools-dbpath-deprecated for more information")
	}
