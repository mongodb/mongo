...
// BongoOpStream->streamOps goroutine for which turns them in to protocol
// messages.
...
type bidi struct {
	...
	opStream         *BongoOpStream
	responseStream   bool
	sawStart         bool
	connectionNumber int64
}

func newBidi(netFlow, tcpFlow gopacket.Flow, opStream *BongoOpStream, num int64) *bidi {
...
// BongoOpStream is the opstream which yields RecordedOps
type BongoOpStream struct {
	Ops chan *RecordedOp
...
// NewBongoOpStream initializes a new BongoOpStream
func NewBongoOpStream(heapBufSize int) *BongoOpStream {
...
func (os *BongoOpStream) New(netFlow, tcpFlow gopacket.Flow) tcpassembly.Stream {
...
// have been processed.
func (os *BongoOpStream) Close() error {
	close(os.unorderedOps)
	os.unorderedOps = nil
	return nil
}

// SetFirstSeen sets the time for the first message on the BongoOpStream.
...
func (bidi *bidi) handleStreamStateBeforeMessage(stream *stream) {
	...
	if !stream.op.Header.LooksReal() {
		...
		// know that we're actually not looking at bongodb traffic and that this
