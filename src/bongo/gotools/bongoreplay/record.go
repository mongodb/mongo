...
// RecordCommand stores settings for the bongoreplay 'record' subcommand
...
func getOpstream(cfg OpStreamSettings) (*packetHandlerContext, error) {
	...
	m := NewBongoOpStream(cfg.PacketBufSize)
...
func Record(ctx *packetHandlerContext,
	...
	go func() {
		defer close(ch)
		var fail error
		for op := range ctx.bongoOpStream.Ops {
			// since we don't currently have a way to shutdown packetHandler.Handle()
			// continue to read from ctx.bongoOpStream.Ops even after a faltal error
			if fail != nil {
	...
	if err := ctx.packetHandler.Handle(ctx.bongoOpStream, -1); err != nil {
