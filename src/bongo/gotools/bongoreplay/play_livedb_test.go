...
const (
	...
	nonAuthTestServerURL = "bongodb://localhost"
	authTestServerURL    = "bongodb://authorizedUser:authorizedPwd@localhost"
	testDB               = "bongoreplay"
	testCollection       = "test"
...
func getPrimaryPort(session *mgo.Session) (string, error) {
	...
	isBongosTestServer = (res.Msg == "isdbgrid")
	if isBongosTestServer {
		return "", nil
	}
...
// TestOpInsertLiveDB tests the functionality of bongoreplay replaying inserts
// against a live database Generates 20 recorded inserts and passes them to the
// main execution of bongoreplay and queries the database to verify they were
...
func TestOpInsertLiveDB(t *testing.T) {
	...
	// run bongoreplay's Play loop with the stubbed objects
	t.Logf("Beginning bongoreplay playback of generated traffic against host: %v\n", currentTestURL)
	err := Play(context, generator.opChan, testSpeed, currentTestURL, 1, 10)
	if err != nil {
		t.Errorf("Error Playing traffic: %v\n", err)
	}
	t.Log("Completed bongoreplay playback of generated traffic")
	...
	for i := 0; i < numInserts; i++ {
		...
		// All commands should be inserts into bongoreplay.test
		if stat.OpType != "insert" ||
			stat.Ns != "bongoreplay.test" {
			t.Errorf("Expected to see an insert into bongoreplay.test, but instead saw %v, %v\n", stat.OpType, stat.Command)
		}
	}
...
// TestUpdateOpLiveDB tests the functionality of bongoreplay replaying an update
// against a live database Generates 20 recorded inserts and an update and
// passes them to the main execution of bongoreplay and queries the database to
...
func TestUpdateOpLiveDB(t *testing.T) {
	...
	if stat.OpType != "update" ||
		stat.Ns != "bongoreplay.test" {
		t.Errorf("Expected to see an update to bongoreplay.test, but instead saw %v, %v\n", stat.OpType, stat.Ns)
	}
	if err := teardownDB(); err != nil {
		t.Error(err)
	}
...
// TestQueryOpLiveDB tests the functionality of some basic queries through bongoreplay.
// It generates inserts and queries and sends them to the main execution of bongoreplay.
...
func TestQueryOpLiveDB(t *testing.T) {
	...
	// created to ensure that they match what we expected bongoreplay to have
	// executed
	for i := 0; i < numQueries; i++ {
...
// through bongoreplay. It generates inserts, a query, and a series of getmores
...
func TestOpGetMoreLiveDB(t *testing.T) {
	...
	go func() {
		...
		// generate a query with a known requestID to be played in bongoreplay
		...
		// information can be correlated by bongoreplay
...
func TestOpGetMoreMultiCursorLiveDB(t *testing.T) {
	...
	go func() {
		...
		// be played in bongoreplay's main Play function
...
func TestOpKillCursorsLiveDB(t *testing.T) {
	...
	go func() {
		...
		// generate numInserts RecordedOp inserts and send them to a channel to be played in bongoreplay's main Play function
		t.Logf("Generating %v inserts\n", numInserts)
		err := generator.generateInsertHelper(insertName, 0, numInserts)
		if err != nil {
			t.Error(err)
	...
	for i := 0; i < 2; i++ {
		...
		if stat.OpType != "getmore" ||
			...
			t.Errorf("Getmore Not matched: Expected OpType: %s NumReturned: %d NameSpace: %s --- Found OpType: %s NumReturned: %d NameSpace: %s",
				"getmore", 2, "bongoreplay.test", stat.OpType, stat.NumReturned, stat.Ns)
		}
	}
	...
	for i := 0; i < 2; i++ {
		...
		if stat.OpType != "getmore" ||
			...
			stat.Ns != "bongoreplay.test" {
			t.Errorf("Getmore Not matched: Expected OpType: %s NumReturned: %d NameSpace: %s --- Found OpType: %s NumReturned: %d NameSpace: %s",
				"getmore", 0, "bongoreplay.test", stat.OpType, stat.NumReturned, stat.Ns)
		}
	}
...
func TestCommandOpInsertLiveDB(t *testing.T) {
	...
	if isBongosTestServer {
		t.Skipf("Skipping OpCommand test against bongos")
	}
	...
	for i := 0; i < numInserts; i++ {
		...
		if stat.OpType != "op_command" ||
			stat.Command != "insert" ||
			stat.Ns != "bongoreplay" {
			t.Errorf("Expected to see an insert into bongoreplay, but instead saw %v, %v into %v\n", stat.OpType, stat.Command, stat.Ns)
		}
	}
...
func TestCommandOpFindLiveDB(t *testing.T) {
	...
	// created, and ensure that they match what we expected bongoreplay to have
	// executed
	for i := 0; i < numFinds; i++ {
		stat := statRec.Buffer[numInserts+i]
		t.Logf("Stat result: %#v\n", stat)
	...
	for i := 0; i < numGetMores; i++ {
		...
		if stat.OpType != "op_command" ||
			...
			t.Errorf("CommandOpGetmore Not matched.Expected OpType:%s NumReturned:%d Ns:%s --- Found OpType:%s NumReturned: %d Ns: %s\n",
				"op_command", 5, "bongoreplay", stat.OpType, stat.NumReturned, stat.Ns)
		}
	}
	if err := teardownDB(); err != nil {
		t.Error(err)
