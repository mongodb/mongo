// Package bongoexport produces a JSON or CSV export of data stored in a BongoDB instance.
package bongoexport
...
// Output types supported by bongoexport.
const (
	CSV                            = "csv"
	JSON                           = "json"
...
// BongoExport is a container for the user-specified options and
// internal state used for running bongoexport.
type BongoExport struct {
...
func (exp *BongoExport) ValidateSettings() error {
...
func (exp *BongoExport) GetOutputWriter() (io.WriteCloser, error) {
...
func makeFieldSelector(fields string) bson.M {
	...
	for _, field := range strings.Split(fields, ",") {
		...
		// - if passed directly to bongod. To handle this, we have to retrieve
		// the entire top-level document and then filter afterwards. An exception
		// is made for '$' projections - which are passed directly to bongod.
...
func (exp *BongoExport) getCount() (c int, err error) {
...
// to export, based on the options given to bongoexport. Also returns the
// associated session, so that it can be closed once the cursor is used up.
func (exp *BongoExport) getCursor() (*mgo.Iter, *mgo.Session, error) {
	sortFields := []string{}
	if exp.InputOpts != nil && exp.InputOpts.Sort != "" {
...
func (exp *BongoExport) exportInternal(out io.Writer) (int64, error) {
...
func (exp *BongoExport) Export(out io.Writer) (int64, error) {
...
func (exp *BongoExport) getExportOutput(out io.Writer) (ExportOutput, error) {
