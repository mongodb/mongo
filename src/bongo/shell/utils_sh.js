...
sh._checkBongos = function() {
    var x = db.runCommand("ismaster");
    if (x.msg != "isdbgrid")
        throw Error("not connected to a bongos");
};
...
sh.help = function() {
    ...
    print(
        "\tsh.isBalancerRunning()                    return true if the balancer has work in progress on any bongos");
    print(
...
sh.waitForPingChange = function(activePings, timeout, interval) {
    var isPingChanged = function(activePing) {
        var newPing = sh._getConfigDB().bongos.findOne({_id: activePing._id});
...
sh._lastMigration = function(ns) {
    ...
    if (!ns) {
        ...
        if (ns.indexOf(".") > 0) {
            config = db.getSisterDB("config");
            coll = db.getBongo().getCollection(ns);
        } else {
            config = db.getSisterDB("config");
...
function printShardingStatus(configDB, verbose) {
    ...
    if (version == null) {
        print(
            "printShardingStatus: this db does not have sharding enabled. be sure you are connecting to a bongos from the shell and not to a bongod.");
        return;
    }
    ...
    // (most recently) active bongoses
    var bongosActiveThresholdMs = 60000;
    var mostRecentBongos = configDB.bongos.find().sort({ping: -1}).limit(1);
    var mostRecentBongosTime = null;
    var bongosAdjective = "most recently active";
    if (mostRecentBongos.hasNext()) {
        mostRecentBongosTime = mostRecentBongos.next().ping;
        // Bongoses older than the threshold are the most recent, but cannot be
        // considered "active" bongoses. (This is more likely to be an old(er)
        // configdb dump, or all the bongoses have been stopped.)
        if (mostRecentBongosTime.getTime() >= Date.now() - bongosActiveThresholdMs) {
            bongosAdjective = "active";
        }
    }

    output("  " + bongosAdjective + " bongoses:");
    if (mostRecentBongosTime === null) {
        output("\tnone");
    } else {
        var recentBongosQuery = {
            ping: {
                $gt: (function() {
                    var d = mostRecentBongosTime;
                    d.setTime(d.getTime() - bongosActiveThresholdMs);
                    return d;
                })()
            }
        };
        ...
        if (verbose) {
            configDB.bongos.find(recentBongosQuery).sort({ping: -1}).forEach(function(z) {
                output("\t" + tojsononeline(z));
            });
        } else {
            configDB.bongos
                .aggregate([
                    {$match: recentBongosQuery},
                    {$group: {_id: "$bongoVersion", num: {$sum: 1}}},
                    {$sort: {num: -1}}
                ])
                .forEach(function(z) {
