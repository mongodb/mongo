var BongoRunner, _startBongod, startBongoProgram, runBongoProgram, startBongoProgramNoConnect,
    myPort;
...
(function() {
    ...
    var createBongoArgs = function(binaryName, args) {
        if (!Array.isArray(args)) {
            throw new Error("The second argument to createBongoArgs must be an array");
        }
    ...
    BongoRunner = function() {};

    BongoRunner.dataDir = "/data/db";
    BongoRunner.dataPath = "/data/db/";

    BongoRunner.bongodPath = "bongod";
    BongoRunner.bongosPath = "bongos";
    BongoRunner.bongoShellPath = "bongo";

    BongoRunner.VersionSub = function(pattern, version) {
    ...
    var convertVersionStringToArray = function(versionString) {
        ...
        assert.gt(versionArray.length,
                  1,
                  "BongoDB versions must have at least two components to compare, but \"" +
    ...
    // the
    // dev/stable BongoDB release cycle.
    //
    ...
    BongoRunner.binVersionSubs = [
        new BongoRunner.VersionSub("latest", shellVersion()),
        new BongoRunner.VersionSub(extractMajorVersionFromVersionString(shellVersion()),
                                   shellVersion()),
        // To-be-updated when we branch for the next release.
        new BongoRunner.VersionSub("last-stable", "3.4")
    ];

    BongoRunner.getBinVersionFor = function(version) {
        if (version instanceof BongoRunner.versionIterator.iterator) {
        ...
        for (var i = 0; i < BongoRunner.binVersionSubs.length; i++) {
            var sub = BongoRunner.binVersionSubs[i];
     ...
     */
    BongoRunner.areBinVersionsTheSame = function(versionA, versionB) {

        versionA = convertVersionStringToArray(BongoRunner.getBinVersionFor(versionA));
        versionB = convertVersionStringToArray(BongoRunner.getBinVersionFor(versionB));
    ...
    BongoRunner.toRealPath = function(path, pathOpts) {

        // Replace all $pathOptions with actual values
        pathOpts = pathOpts || {};
        path = path.replace(/\$dataPath/g, BongoRunner.dataPath);
        path = path.replace(/\$dataDir/g, BongoRunner.dataDir);
        ...
        if (!path.match(/^(\/|\\|[A-Za-z]:)/)) {
            ...
            path = BongoRunner.dataPath + path;
        }
    ...
    BongoRunner.toRealDir = function(path, pathOpts) {

        path = BongoRunner.toRealPath(path, pathOpts);
    ...
    BongoRunner.toRealFile = BongoRunner.toRealDir;
     ...
     */
    BongoRunner.versionIterator = function(arr, isRandom) {
        ...
        return new BongoRunner.versionIterator.iterator(i, arr);
    };

    BongoRunner.versionIterator.iterator = function(i, arr) {
     ...
     * are defined in BongoRunner.logicalOptions, of which they will be ignored.
     *
     ...
     *   to the binary.
     */
    BongoRunner.arrOptions = function(binaryName, args) {
        ...
        if (isObject(args) || (args.length == 1 && isObject(args[0]))) {
            ...
            for (var k in o) {
                ...
                if (!o.hasOwnProperty(k) || k in BongoRunner.logicalOptions ||
    ...
    BongoRunner.arrToOpts = function(arr) {
    ...
    BongoRunner.bongoOptions = function(opts) {
        ...
        if (opts.restart && opts.remember) {
            opts = Object.merge(BongoRunner.savedOptions[opts.runId], opts);
        }
        ...
        if (opts.hasOwnProperty('binVersion')) {
            if (opts.binVersion instanceof BongoRunner.versionIterator.iterator) {
                // Advance the version iterator so that subsequent calls to
                // BongoRunner.bongoOptions() use the next version in the list.
                const iterator = opts.binVersion;
                opts.binVersion = iterator.current();
                iterator.advance();
            }
            opts.binVersion = BongoRunner.getBinVersionFor(opts.binVersion);
        }
        ...
        if (shouldRemember) {
            BongoRunner.savedOptions[opts.runId] = Object.merge(opts, {});
        }
    ...
    var _isBongodVersionEqualOrAfter = function(version1, version2) {
        if (version2 === "latest") {
            return true;
        }
    ...
    // Removes a setParameter parameter from bongods running a version that won't recognize them.
    var _removeSetParameterIfBeforeVersion = function(opts, parameterName, requiredVersion) {
        var versionCompatible = (opts.binVersion === "" || opts.binVersion === undefined ||
                                 _isBongodVersionEqualOrAfter(requiredVersion, opts.binVersion));
        if (!versionCompatible && opts.setParameter &&
            ...
            print("Removing '" + parameterName + "' setParameter with value " +
                  ...
                  " because it isn't compatibile with bongod running version " + opts.binVersion);
     ...
     *   }
     */
    BongoRunner.bongodOptions = function(opts) {

        opts = BongoRunner.bongoOptions(opts);

        opts.dbpath = BongoRunner.toRealDir(opts.dbpath || "$dataDir/bongod-$port", opts.pathOpts);
        ...
        if (!opts.logFile && opts.useLogFiles) {
            opts.logFile = opts.dbpath + "/bongod.log";
        } else if (opts.logFile) {
            opts.logFile = BongoRunner.toRealFile(opts.logFile, opts.pathOpts);
        }
    ...
    BongoRunner.bongosOptions = function(opts) {
        opts = BongoRunner.bongoOptions(opts);
        ...
        if (!opts.logFile && opts.useLogFiles) {
            opts.logFile =
                BongoRunner.toRealFile("$dataDir/bongos-$configdb-$port.log", opts.pathOpts);
        } else if (opts.logFile) {
            opts.logFile = BongoRunner.toRealFile(opts.logFile, opts.pathOpts);
        }
        ...
        if (!opts.hasOwnProperty('binVersion') && testOptions.bongosBinVersion) {
            opts.binVersion = BongoRunner.getBinVersionFor(testOptions.bongosBinVersion);
        }

        // If the bongos is being restarted with a newer version, make sure we remove any options
        // that no longer exist in the newer version.
        if (opts.restart && BongoRunner.areBinVersionsTheSame('latest', opts.binVersion)) {
     ...
     *     binVersion {string}: version for binary (also see BongoRunner.binVersionSubs).
     *
     *     @see BongoRunner.bongodOptions for other options
     *   }
     *
     * @return {Bongo} connection object to the started bongod instance.
     *
     * @see BongoRunner.arrOptions
     */
    BongoRunner.runBongod = function(opts) {
        ...
        if (isObject(opts)) {
            opts = BongoRunner.bongodOptions(opts);
            fullOptions = opts;
            ...
            if (opts.forceLock)
                removeFile(opts.dbpath + "/bongod.lock");
            ...
            var bongodProgram = BongoRunner.bongodPath;
            opts = BongoRunner.arrOptions(bongodProgram, opts);
        }

        var bongod = BongoRunner._startWithArgs(opts, env, waitForConnect);
        if (!bongod) {
            return null;
        }

        bongod.commandLine = BongoRunner.arrToOpts(opts);
        bongod.name = (useHostName ? getHostName() : "localhost") + ":" + bongod.commandLine.port;
        bongod.host = bongod.name;
        bongod.port = parseInt(bongod.commandLine.port);
        bongod.runId = runId || ObjectId();
        bongod.dbpath = fullOptions.dbpath;
        bongod.savedOptions = BongoRunner.savedOptions[bongod.runId];
        bongod.fullOptions = fullOptions;

        return bongod;
    };

    BongoRunner.runBongos = function(opts) {
        opts = opts || {};
        ...
        if (isObject(opts)) {
            opts = BongoRunner.bongosOptions(opts);
            fullOptions = opts;
            ...
            waitForConnect = opts.waitForConnect;
            env = opts.env;
            var bongosProgram = BongoRunner.bongosPath;
            opts = BongoRunner.arrOptions(bongosProgram, opts);
        }

        var bongos = BongoRunner._startWithArgs(opts, env, waitForConnect);
        if (!bongos) {
            return null;
        }

        bongos.commandLine = BongoRunner.arrToOpts(opts);
        bongos.name = (useHostName ? getHostName() : "localhost") + ":" + bongos.commandLine.port;
        bongos.host = bongos.name;
        bongos.port = parseInt(bongos.commandLine.port);
        bongos.runId = runId || ObjectId();
        bongos.savedOptions = BongoRunner.savedOptions[bongos.runId];
        bongos.fullOptions = fullOptions;

        return bongos;
    };

    BongoRunner.StopError = function(message, returnCode) {
        this.name = "StopError";
        this.returnCode = returnCode || "non-zero";
        this.message = message || "BongoDB process stopped with exit code: " + this.returnCode;
        this.stack = this.toString() + "\n" + (new Error()).stack;
    };

    BongoRunner.StopError.prototype = Object.create(Error.prototype);
    BongoRunner.StopError.prototype.constructor = BongoRunner.StopError;

    // Constants for exit codes of BongoDB processes
    BongoRunner.EXIT_ABORT = -6;
    BongoRunner.EXIT_CLEAN = 0;
    BongoRunner.EXIT_BADOPTIONS = 2;
    BongoRunner.EXIT_REPLICATION_ERROR = 3;
    BongoRunner.EXIT_NEED_UPGRADE = 4;
    BongoRunner.EXIT_SHARDING_ERROR = 5;
    BongoRunner.EXIT_KILL = 12;
    BongoRunner.EXIT_ABRUPT = 14;
    BongoRunner.EXIT_NTSERVICE_ERROR = 20;
    BongoRunner.EXIT_JAVA = 21;
    BongoRunner.EXIT_OOM_MALLOC = 42;
    BongoRunner.EXIT_OOM_REALLOC = 43;
    BongoRunner.EXIT_FS = 45;
    BongoRunner.EXIT_CLOCK_SKEW = 47;  // OpTime clock skew; deprecated
    BongoRunner.EXIT_NET_ERROR = 48;
    BongoRunner.EXIT_WINDOWS_SERVICE_STOP = 49;
    BongoRunner.EXIT_POSSIBLE_CORRUPTION = 60;
    BongoRunner.EXIT_UNCAUGHT = 100;  // top level exception that wasn't caught
    BongoRunner.EXIT_TEST = 101;

    /**
     * Kills a bongod process.
     *
     ...
     *    }
     *
     * Note: The auth option is required in a authenticated bongod running in Windows since
     *  it uses the shutdown command, which requires admin credentials.
     */
    BongoRunner.stopBongod = function(port, signal, opts) {

        if (!port) {
            print("Cannot stop bongo process " + port);
            return null;
        }
        ...
        var allowedExitCodes = [BongoRunner.EXIT_CLEAN];
        ...
        if (port instanceof ObjectId) {
            var opts = BongoRunner.savedOptions(port);
            if (opts)
                port = parseInt(opts.port);
        }

        var returnCode = _stopBongoProgram(parseInt(port), signal, opts);

        if (!Array.contains(allowedExitCodes, returnCode)) {
            throw new BongoRunner.StopError(
                `BongoDB process on port ${port} exited with error code ${returnCode}`, returnCode);
        }

        return returnCode;
    };

    BongoRunner.stopBongos = BongoRunner.stopBongod;

    /**
     * Starts an instance of the specified bongo tool
     *
     ...
     * BongoRunner.runBongoTool("executable", {key: value}, arg1, arg2) would invoke
     * ./executable --key value arg1 arg2.
     *
     * @see BongoRunner.arrOptions
     */
    BongoRunner.runBongoTool = function(binaryName, opts, ...positionalArgs) {

        var opts = opts || {};

        // Normalize and get the binary version to use
        if (opts.binVersion instanceof BongoRunner.versionIterator.iterator) {
            // Advance the version iterator so that subsequent calls to BongoRunner.runBongoTool()
        ...
        // Recent versions of the bongo tools support a --dialTimeout flag to set for how
        // long they retry connecting to a bongod or bongos process. We have them retry
        ...
        var argsArray = BongoRunner.arrOptions(binaryName, opts);

        // Append any positional arguments that were specified.
        argsArray.push(...positionalArgs);

        return runBongoProgram.apply(null, argsArray);
    ...
    BongoRunner.getAndPrepareDumpDirectory = function(testName) {
        var dir = BongoRunner.dataPath + testName + "_external/";
        resetDbpath(dir);
        return dir;
    };

    // Start a bongod instance and return a 'Bongo' object connected to it.
    // This function's arguments are passed as command line arguments to bongod.
    // The specified 'dbpath' is cleared if it exists, created if not.
    // var conn = _startBongodEmpty("--port", 30000, "--dbpath", "asdf");
    var _startBongodEmpty = function() {
        var args = createBongoArgs("bongod", Array.from(arguments));

        var dbpath = _parsePath.apply(null, args);
        resetDbpath(dbpath);

        return startBongoProgram.apply(null, args);
    };

    _startBongod = function() {
        print("startBongod WARNING DELETES DATA DIRECTORY THIS IS FOR TESTING ONLY");
        return _startBongodEmpty.apply(null, arguments);
    };
    ...
    function appendSetParameterArgs(argArray) {
        // programName includes the version, e.g., bongod-3.2.
        // baseProgramName is the program name without any version information, e.g., bongod.
        var programName = argArray[0];
        var [baseProgramName, programVersion] = programName.split("-");
        if (baseProgramName === 'bongod' || baseProgramName === 'bongos') {
            ...
            // Since options may not be backward compatible, bongos options are not
            // set on older versions, e.g., bongos-3.0.
            if (programName.endsWith('bongos')) {
                // apply setParameters for bongos
                if (jsTest.options().setParametersBongos) {
                    var params = jsTest.options().setParametersBongos.split(",");
            ...
            } else if (baseProgramName === 'bongod') {
                // Set storageEngine for bongod. There was no storageEngine parameter before 3.0.
                ...
                // Since options may not be backward compatible, bongod options are not
                // set on older versions, e.g., bongod-3.0.
                if (programName.endsWith('bongod')) {
                    ...
                    // apply setParameters for bongod
    ...
    /**
     * Start a bongo process with a particular argument array.
     ...
     */
    BongoRunner._startWithArgs = function(argArray, env, waitForConnect) {
        // TODO: Make there only be one codepath for starting bongo processes

        argArray = appendSetParameterArgs(argArray);
        var port = _parsePort.apply(null, argArray);
        var pid = -1;
        if (env === undefined) {
            pid = _startBongoProgram.apply(null, argArray);
        } else {
            pid = _startBongoProgram({args: argArray, env: env});
        }
        ...
        assert.soon(function() {
            try {
                conn = new Bongo("127.0.0.1:" + port);
                conn.pid = pid;
                return true;
            } catch (e) {
                if (!checkProgram(pid)) {
                    print("Could not start bongo program at " + port + ", process ended");
            ...
            return false;
        }, "unable to connect to bongo program on port " + port, 600 * 1000);
     ...
     * DEPRECATED
     *
     * Start bongod or bongos and return a Bongo() object connected to there.
     * This function's first argument is "bongod" or "bongos" program name, \
     * and subsequent arguments to this function are passed as
     * command line arguments to the program.
     */
    startBongoProgram = function() {
    ...
    runBongoProgram = function() {
        ...
        if (progName == 'bongo' && !_useWriteCommandsDefault()) {
            progName = args[0];
            args = args.slice(1);
            args.unshift(progName, '--useLegacyWriteOps');
        }

        return _runBongoProgram.apply(null, args);
    };

    // Start a bongo program instance.  This function's first argument is the
    // program name, and subsequent arguments to this function are passed as
    // command line arguments to the program.  Returns pid of the spawned program.
    startBongoProgramNoConnect = function() {
        var args = Array.from(arguments);
        args = appendSetParameterArgs(args);
        var progName = args[0];
        ...
        return _startBongoProgram.apply(null, args);
    };
