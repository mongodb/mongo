// bongo/shell/shell_utils_launcher.cpp
/*
...
#include "bongo/util/signal_win32.h"
...
void ProgramOutputMultiplexer::appendLine(int port,
    ...
    stdx::lock_guard<stdx::mutex> lk(bongoProgramOutputMutex);
...
ProgramRunner::ProgramRunner(const BSONObj& args, const BSONObj& env) {
    ...
    string prefix("bongod-");
    bool isBongodProgram = string("bongod") == programName ||
        programName.string().compare(0, prefix.size(), prefix) == 0;
    prefix = "bongos-";
    bool isBongosProgram = string("bongos") == programName ||
        programName.string().compare(0, prefix.size(), prefix) == 0;

    if (isBongodProgram) {
        _name = "d";
    } else if (isBongosProgram) {
        _name = "s";
    } else if (programName == "bongobridge") {
        _name = "b";
    } else {
        _name = "sh";
    }
    ...
    while (j.more()) {
        ...
        } else if (isBongodProgram && str == "--configsvr") {
            _name = "c";
        }
        _argv.push_back(str);
    }
...
#endif
    bool needsPort = isBongodProgram || isBongosProgram || (programName == "bongobridge");
    if (!needsPort) {
        _port = -1;
    }
    uassert(ErrorCodes::FailedToParse,
    ...
    while (std::getline(fdStream, line)) {
        if (line.find('\0') != std::string::npos) {
            programOutputLogger.appendLine(
                _port, _pid, _name, "WARNING: bongod wrote null bytes to output");
        }
...
boost::filesystem::path ProgramRunner::findProgram(const string& prog) {
    ...
    // (e.g., bongorestore-2.4) or just <utility>. For windows, the appropriate extension
...
BSONObj RawBongoProgramOutput(const BSONObj& args, void* data) {
    return BSON("" << programOutputLogger.str());
}

BSONObj ClearRawBongoProgramOutput(const BSONObj& args, void* data) {
...
BSONObj StartBongoProgram(const BSONObj& a, void* data) {
    _nokillop = true;
    BSONObj args = a;
    BSONObj env{};
    ...
    if (firstElement.ok() && firstElement.isABSONObj()) {
        ...
        uassert(40098,
                "If StartBongoProgram is called with a BSONObj, "
...
BSONObj RunBongoProgram(const BSONObj& a, void* data) {
    BSONObj env{};
...
void copyDir(const boost::filesystem::path& from, const boost::filesystem::path& to) {
    ...
    while (i != end) {
        ...
        } else if (p.leaf() != "bongod.lock" && p.leaf() != "WiredTiger.lock") {
...
BSONObj getStopBongodOpts(const BSONObj& a) {
    if (a.nFields() == 3) {
        BSONObjIterator i(a);
        i.next();
        i.next();
...
/** stopBongoProgram(port[, signal]) */
BSONObj StopBongoProgram(const BSONObj& a, void* data) {
    int nFields = a.nFields();
    verify(nFields >= 1 && nFields <= 3);
    uassert(15853, "stopBongo needs a number", a.firstElement().isNumber());
    int port = int(a.firstElement().number());
    int code = killDb(port, ProcessId::fromNative(0), getSignal(a), getStopBongodOpts(a));
    log() << "shell: stopped bongo program on port " << port;
    return BSON("" << (double)code);
}

BSONObj StopBongoProgramByPid(const BSONObj& a, void* data) {
    verify(a.nFields() == 1 || a.nFields() == 2);
    uassert(15852, "stopBongoByPid needs a number", a.firstElement().isNumber());
    ProcessId pid = ProcessId::fromNative(int(a.firstElement().number()));
    int code = killDb(0, pid, getSignal(a));
    log() << "shell: stopped bongo program on pid " << pid;
    return BSON("" << (double)code);
}

int KillBongoProgramInstances() {
...
BongoProgramScope::~BongoProgramScope() {
    DESTRUCTOR_GUARD(KillBongoProgramInstances(); ClearRawBongoProgramOutput(BSONObj(), 0);)
}

void installShellUtilsLauncher(Scope& scope) {
    scope.injectNative("_startBongoProgram", StartBongoProgram);
    scope.injectNative("runProgram", RunBongoProgram);
    scope.injectNative("run", RunBongoProgram);
    scope.injectNative("_runBongoProgram", RunBongoProgram);
    scope.injectNative("_stopBongoProgram", StopBongoProgram);
    scope.injectNative("stopBongoProgramByPid", StopBongoProgramByPid);
    scope.injectNative("rawBongoProgramOutput", RawBongoProgramOutput);
    scope.injectNative("clearRawBongoProgramOutput", ClearRawBongoProgramOutput);
