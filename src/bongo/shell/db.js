...
(function() {
    ...
    if (DB === undefined) {
        DB = function(bongo, name) {
            this._bongo = bongo;
            this._name = name;
        };
    }

    DB.prototype.getBongo = function() {
        assert(this._bongo, "why no bongo!");
        return this._bongo;
    };

    DB.prototype.getSiblingDB = function(name) {
        return this.getBongo().getDB(name);
    };
    ...
    DB.prototype.getCollection = function(name) {
        return new DBCollection(this._bongo, this, name, this._name + "." + name);
    };
    ...
    DB.prototype.runReadCommand = function(obj, extra, queryOptions) {
        ...
        var cmdObjWithReadPref =
            this._attachReadPreferenceToCommand(mergedObj, this.getBongo().getReadPref());

        var options =
            (typeof(queryOptions) !== "undefined") ? queryOptions : this.getQueryOptions();
        var readPrefMode = this.getBongo().getReadPrefMode();
    ...
    DB.prototype._runCommandImpl = function(name, obj, options) {
        return this.getBongo().runCommand(name, obj, options);
    };
    ...
    DB.prototype.runCommandWithMetadata = function(commandName, commandArgs, metadata) {
        return this.getBongo().runCommandWithMetadata(
    ...
    */
     ...
     * @param {String} from bongod instance from which to clnoe (dbhostname:port).  May
     not be this bongod instance, as clone from self is not allowed.
    ...
    */
    DB.prototype.copyDatabase = function(
        ...
        assert(mechanism == "SCRAM-SHA-1" || mechanism == "BONGODB-CR");
        ...
        if (mechanism == "SCRAM-SHA-1") {
            return this.getBongo().copyDatabaseWithSCRAM(
                fromdb, todb, fromhost, username, password, slaveOk);
        }

        // Fall back to BONGODB-CR
    ...
    */
    ...
    DB.prototype.help = function() {
        ...
        print("\tdb.getBongo() get the server connection object");
        print("\tdb.getBongo().setSlaveOk() allow queries on a replication slave server");
        print("\tdb.getName()");
    ...
    DB.prototype._getCollectionInfosCommand = function(filter) {
        ...
        if (res.code == 59) {
            // command doesn't exist, old bongod
            return null;
        }
        ...
        return new DBCommandCursor(res._bongo, res).toArray().sort(compareOn("name"));
    };
    ...
    DB.prototype.currentOp = function(arg) {
        ...
        if (commandUnsupported(res)) {
            ...
            var _readPref = this.getBongo().getReadPrefMode();
            try {
                this.getBongo().setReadPref(null);
                res = this.getSiblingDB("admin").$cmd.sys.inprog.findOne(q);
            } finally {
                this.getBongo().setReadPref(_readPref);
            }
        }
        return res;
    };
    ...
    */
    ...
    DB.prototype.createUser = function(userObj, writeConcern) {
        ...
        if (res.errmsg == "no such cmd: createUser") {
            throw Error("'createUser' command not found.  This is most likely because you are " +
                        "talking to an old (pre v2.6) BongoDB server");
        }
    ...
    DB.prototype.logout = function() {
        return this.getBongo().logout(this.getName());
    };
    ...
    DB.prototype._getDefaultAuthenticationMechanism = function() {
        ...
        // Use BONGODB-CR for v2.6 and earlier.
    ...
    DB.prototype._authOrThrow = function() {
        ...
        if (params.db !== undefined) {
            throw Error("Do not override db field on db.auth(). Use getBongo().auth(), instead.");
        }
    ...
    DB.prototype.getWriteConcern = function() {
        ...
        if (this._bongo.getWriteConcern())
            return this._bongo.getWriteConcern();
    ...
    DB.prototype.getLogComponents = function() {
        return this.getBongo().getLogComponents();
    };

    DB.prototype.setLogLevel = function(logLevel, component) {
        return this.getBongo().setLogLevel(logLevel, component);
    };
