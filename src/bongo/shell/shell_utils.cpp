...
#include "bongo/db/index/external_key_generator.h"
#include "bongo/platform/random.h"
#include "bongo/scripting/engine.h"
#include "bongo/shell/bench.h"
#include "bongo/shell/shell_options.h"
#include "bongo/shell/shell_utils_extended.h"
#include "bongo/shell/shell_utils_launcher.h"
#include "bongo/util/concurrency/threadlocal.h"
...
void initScope(Scope& scope) {
    ...
    scope.externalSetup();
    bongo::shell_utils::installShellUtils(scope);
...
void ConnectionRegistry::killOperationsOnAllConnections(bool withPrompt) const {
    ...
    for (map<string, set<string>>::const_iterator i = _connectionUris.begin();
        ...
        string errmsg;
        std::unique_ptr<DBClientWithCommands> conn(cs.connect("BongoDB Shell", errmsg));
        if (!conn) {
            continue;
        }
...
stdx::mutex& bongoProgramOutputMutex(*(new stdx::mutex()));
}
}
