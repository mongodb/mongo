...
#include "bongo/scripting/bson_template_evaluator.h"
...
BenchRunOp opFromBson(const BSONObj& op) {
    ...
    for (auto arg : myOp.myBsonOp) {
        ...
        if (name == "batchSize") {
            ...
            // Bongo-perf wants to pass the write concern into all calls. It is only used for
...
void BenchRunState::waitForState(State awaitedState) {
    ...
    switch (awaitedState) {
        ...
        default:
            msgasserted(16152,
                        bongoutils::str::stream() << "Cannot wait for state " << awaitedState);
    }
}
...
class BenchRunWorkerStateGuard {
    BONGO_DISALLOW_COPYING(BenchRunWorkerStateGuard);
