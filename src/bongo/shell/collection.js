// @file collection.js - DBCollection support in the bongo shell
// db.colName is a DBCollection object
// or db["colName"]

if ((typeof DBCollection) == "undefined") {
    DBCollection = function(bongo, db, shortName, fullName) {
        this._bongo = bongo;
        this._db = db;
...
DBCollection.prototype.verify = function() {
    ...
    assert(this._bongo, "no bongo in DBCollection");
    assert(this.getBongo(), "no bongo from getBongo()");
};
...
DBCollection.prototype.getBongo = function() {
    return this._db.getBongo();
};
...
DBCollection.prototype._validateObject = function(o) {
    ...
    if (this.getBongo()._skipValidation)
        return;
...
DBCollection.prototype.find = function(query, fields, limit, skip, batchSize, options) {
    var cursor = new DBQuery(this._bongo,
                             this._db,
                             this,
                             this._fullName,
...
DBCollection.prototype.insert = function(obj, options, _allow_dot) {
    ...
    if (this.getBongo().writeMode() != "legacy") {
        ...
        this.getBongo().insert(this._fullName, obj, flags);
...
DBCollection.prototype.remove = function(t, justOne) {
    ...
    if (this.getBongo().writeMode() != "legacy") {
        ...
        this.getBongo().remove(this._fullName, query, justOne);
...
DBCollection.prototype.update = function(query, obj, upsert, multi) {
    ...
    if (this.getBongo().writeMode() != "legacy") {
        ...
        this.getBongo().update(this._fullName, query, obj, upsert, multi);
...
DBCollection.prototype.createIndexes = function(keys, options) {
    ...
    if (this.getBongo().writeMode() == "commands") {
        for (var i = 0; i < indexSpecs.length; i++) {
    ...
    } else if (this.getBongo().writeMode() == "compatibility") {
...
DBCollection.prototype.diskStorageStats = function(opt) {
    ...
    if (!res.ok && res.errmsg.match(/no such cmd/)) {
        print("this command requires starting bongod with --enableExperimentalStorageDetailsCmd");
    }
    return res;
};
...
DBCollection.prototype._getIndexesCommand = function(filter) {
    ...
    return new DBCommandCursor(res._bongo, res).toArray();
};
...
DBCollection.prototype.exists = function() {
    ...
    if (res.ok) {
        var cursor = new DBCommandCursor(res._bongo, res);
        if (!cursor.hasNext())
            return null;
        return cursor.next();
    }
...
DBCollection.prototype.aggregate = function(pipeline, aggregateOptions) {
    ...
    if ("cursor" in res) {
        ...
        return new DBCommandCursor(res._bongo, res, batchSizeValue);
    }
...
> var collection = db.getBongo().getCollection("foo.bar")
...
DBCollection.prototype.getShardDistribution = function() {
    ...
    var config = this.getBongo().getDB("config");
...
DBCollection.prototype.getSplitKeysForChunks = function(chunkSize) {
    ...
    var admin = this.getBongo().getDB("admin");
    var coll = this;
