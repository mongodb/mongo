ToolTest = function(name, extraOptions) {
    ...
    this.root = BongoRunner.dataPath + this.baseName;
...
ToolTest.prototype.startDB = function(coll) {
    ...
    this.m = startBongoProgram.apply(null, BongoRunner.arrOptions("bongod", options));
...
ToolTest.prototype.runTool = function() {
    ...
    return runBongoProgram.apply(null, a);
};
...
ReplTest.prototype.getPath = function(master) {
    var p = BongoRunner.dataPath + this.name + "-";
    if (master)
        p += "master";
    else
        p += "slave";
...
ReplTest.prototype.getOptions = function(master, extra, putBinaryFirst, norepl) {
    ...
    for (var k in extra) {
        var v = extra[k];
        if (k in BongoRunner.logicalOptions)
            continue;
        a.push("--" + k);
        if (v != null && v !== "")
            a.push(v);
...
ReplTest.prototype.start = function(master, options, restart, norepl) {
    var lockFile = this.getPath(master) + "/bongod.lock";
    removeFile(lockFile);
    var o = this.getOptions(master, options, restart, norepl);

    if (restart) {
        var conn = startBongoProgram.apply(null, o);
        if (!master) {
            conn.setSlaveOk();
        }
        return conn;
    } else {
        var conn = _startBongod.apply(null, o);
...
ReplTest.prototype.stop = function(master, signal) {
    ...
    print('*** ' + this.name + " completed successfully ***");
    return _stopBongoProgram(this.getPort(master), signal || 15);
};

/**
 * Returns a port number that has not been given out to any other caller from the same bongo shell.
 */
...
function startParallelShell(jsCode, port, noConnect) {
    var shellPath = BongoRunner.bongoShellPath;
    var args = [shellPath];

    if (typeof db == "object") {
        var hostAndPort = db.getBongo().host.split(':');
    ...
    var pid = startBongoProgramNoConnect.apply(null, args);
