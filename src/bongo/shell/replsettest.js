 ...
 *      a BongoD host to be used for recreating a ReplSetTest from. Otherwise, if it is an object,
 ...
 *        Format for Object:
 *          {
 *            <any string>: replica member option Object. @see BongoRunner.runBongod
 ...
 *
 *        Format for Array:
 *           An array of replica member option Object. @see BongoRunner.runBongod
 *
 ...
 *           formats to configure the options for the bongobridge corresponding to that node. These
 ...
 *            param-with-arg : arg }
 *        This turns into "bongod --cmdline-param-with-no-arg --param-with-arg arg"
 *
 ...
 *
 *     useBridge {boolean}: If true, then a bongobridge process is started for each node in the
 ...
 *
 *     bridgeOptions {Object}: Options to apply to all bongobridge processes. Defaults to {}.
 *
 ...
 *
 * Member variables:
 *  nodes {Array.<Bongo>} - connection to replica set members
 */
var ReplSetTest = function(opts) {
    ...
    // bongobridge related variables. Only available if the bridge option is selected.
    var _useBridge;
    var _bridgeOptions;
    var _unbridgedPorts;
    var _unbridgedNodes;
     ...
     *
     * @param options - The options passed to {@link BongoRunner.runBongod}
     */
        ...
        assert.soonNoExcept(function() {
            try {
                ...
                for (var i = 0; i < self.liveNodes.slaves.length; i++) {
                    ...
                    slave.getDB("admin").getBongo().setSlaveOk();
    ...
    this.getHashes = function(db) {
        ...
        Object.defineProperty(res.master, "_bongo", {value: this.liveNodes.master});
        res.slaves = [];
        this.liveNodes.slaves.forEach(function(node) {
            ...
            if (!isArbiter) {
                ...
                Object.defineProperty(slaveRes, "_bongo", {value: node});
        ...
        function checkDBHashesForReplSet(rst, dbBlacklist = [], msgPrefix) {
            ...
            for (var dbName of combinedDBs) {
                ...
                dbHashes.slaves.forEach(secondaryDBHash => {
                    ...
                    var secondary = secondaryDBHash._bongo;
    ...
    function checkOplogs(rst, msgPrefix = 'checkOplogs') {
        var OplogReader = function(bongo) {
            this.next = function() {
                if (!this.cursor)
            ...
            this.getOplogColl = function() {
                return this.bongo.getDB("local")[oplogName];
            };
                    ...
                    if (!bsonBinaryEqual(oplogEntry, otherOplogEntry)) {
                        ...
                        assert(false,
                               msgPrefix + ", non-matching oplog entry for nodes: " +
                                   firstReader.bongo.host + " " + readers[i].bongo.host);
                    }
                }
    ...
    this.start = function(n, options, restart, wait) {
        ...
        if (options && options.binVersion) {
            options.binVersion = BongoRunner.versionIterator(options.binVersion);
        }
        ...
        if (_useBridge) {
            ...
            bridgeOptions = Object.merge(bridgeOptions, {
                hostName: this.host,
                port: this.ports[n],
                // The bongod processes identify themselves to bongobridge as host:port, where the
            ...
            this.nodes[n] = new BongoBridge(bridgeOptions);
        }
     ...
     *
     * @param {number|Bongo} n the index or connection object of the replica set member to stop.
     * @param {number} signal the signal number to use for killing
     * @param {Object} opts @see BongoRunner.stopBongod
     */
    this.stop = function(n, signal, opts) {
        ...
        print('ReplSetTest stop *** Shutting down bongod in port ' + port + ' ***');
        var ret = BongoRunner.stopBongod(port, signal, opts);

        print('ReplSetTest stop *** Bongod in port ' + port + ' shutdown with code (' + ret +
              ') ***');
    ...
    function _constructFromExistingSeedNode(seedNode) {
        var conf = _replSetGetConfig(new Bongo(seedNode));
        ...
        self.nodes = existingNodes.map(node => new Bongo(node));
    }
