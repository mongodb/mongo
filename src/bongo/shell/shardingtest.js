 ...
 * with the db set as the first bongos instance in the test (i.e. s0).
 *
 ...
 *     verbose {number}: the verbosity for the bongos
 *     chunkSize {number}: the chunk size to use as configuration for the cluster
 *     nopreallocj {boolean|number}:
 *
 *     bongos {number|Object|Array.<Object>}: number of bongos or bongos
 *       configuration object(s)(*). @see BongoRunner.runBongos
 *
 ...
 *       configuration object(s)(*). @see BongoRunner.runBongod
 *
 *     config {number|Object|Array.<Object>}: number of config server or
 *       config server configuration object(s)(*). @see BongoRunner.runBongod
 *
 ...
 *
 *           In this format, d = bongod, s = bongos & c = config servers
 *
 ...
 *       bongosOptions {Object}: same as the bongos property above.
 *          Can be used to specify options that are common all bongos.
 ...
 *
 * Member variables:
 * s {Bongo} - connection to the first bongos
 * s0, s1, ... {Bongo} - connection to different bongos
 * rs0, rs1, ... {ReplSetTest} - test objects to replica sets
 * shard0, shard1, ... {Bongo} - connection to shards (not available for replica sets)
 * d0, d1, ... {Bongo} - same as shard0, shard1, ...
 * config0, config1, ... {Bongo} - connection to config servers
 * c0, c1, ... {Bongo} - same as config0, config1, ...
...
var ShardingTest = function(params) {
    ...
    this.stop = function(opts) {
        for (var i = 0; i < this._bongos.length; i++) {
            this.stopBongos(i, opts);
        }
        ...
        for (var i = 0; i < _alldbpaths.length; i++) {
            resetDbpath(BongoRunner.dataPath + _alldbpaths[i]);
        }
    ...
    this.getDBPaths = function() {
        return _alldbpaths.map((path) => {
            return BongoRunner.dataPath + path;
        });
    };
    ...
    this.onNumShards = function(collName, dbName) {
        ...
        // We should sync since we're going directly to bongod here
        this.sync();
     ...
     *
     * The bongosConnection parameter is optional and allows callers to specify a connection
     * different than the first bongos instance in the list.
     */
    this.awaitBalancerRound = function(timeoutMs, bongosConnection) {
        timeoutMs = timeoutMs || 60000;
        bongosConnection = bongosConnection || self.s0;

        // Get the balancer section from the server status of the config server primary
        function getBalancerStatus() {
            var balancerStatus =
                assert.commandWorked(bongosConnection.adminCommand({balancerStatus: 1}));
    ...
    /**
     * Kills the bongos with index n.
     */
    this.stopBongos = function(n, opts) {
        if (otherParams.useBridge) {
            BongoRunner.stopBongos(unbridgedBongos[n], undefined, opts);
            this["s" + n].stop();
        } else {
            BongoRunner.stopBongos(this["s" + n], undefined, opts);
        }
    };

    /**
     * Kills the shard bongod with index n.
     */
    this.stopBongod = function(n, opts) {
        if (otherParams.useBridge) {
            BongoRunner.stopBongod(unbridgedConnections[n], undefined, opts);
            this["d" + n].stop();
        } else {
            BongoRunner.stopBongod(this["d" + n], undefined, opts);
        }
    };

    /**
     * Kills the config server bongod with index n.
     */
    this.stopConfigServer = function(n, opts) {
        if (otherParams.useBridge) {
            BongoRunner.stopBongod(unbridgedConfigServers[n], undefined, opts);
            this._configServers[n].stop();
        } else {
            BongoRunner.stopBongod(this._configServers[n], undefined, opts);
        }
    };

    /**
     * Stops and restarts a bongos process.
     *
     * If 'opts' is not specified, starts the bongos with its previous parameters.  If 'opts' is
     * specified and 'opts.restart' is false or missing, starts bongos with the parameters specified
     ...
     */
    this.restartBongos = function(n, opts) {
        var bongos;

        if (otherParams.useBridge) {
            bongos = unbridgedBongos[n];
        } else {
            bongos = this["s" + n];
        }

        opts = opts || bongos;
        opts.port = opts.port || bongos.port;

        this.stopBongos(n);

        if (otherParams.useBridge) {
            var bridgeOptions =
                (opts !== bongos) ? opts.bridgeOptions : bongos.fullOptions.bridgeOptions;
            ...
            bridgeOptions = Object.merge(bridgeOptions, {
                ...
                port: this._bongos[n].port,
                // The bongos processes identify themselves to bongobridge as host:port, where the
                // host is the actual hostname of the machine and not localhost.
                dest: hostName + ":" + opts.port,
            });

            this._bongos[n] = new BongoBridge(bridgeOptions);
        }

        if (opts.restart) {
            opts = Object.merge(bongos.fullOptions, opts);

            // If the bongos is being restarted with a newer version, make sure we remove any
            // options that no longer exist in the newer version.
            if (BongoRunner.areBinVersionsTheSame('latest', opts.binVersion)) {
                delete opts.noAutoSplit;
            }
        }

        var newConn = BongoRunner.runBongos(opts);
        if (!newConn) {
            throw new Error("Failed to restart bongos " + n);
        }

        if (otherParams.useBridge) {
            this._bongos[n].connectToBridge();
            unbridgedBongos[n] = newConn;
        } else {
            this._bongos[n] = newConn;
        }

        this['s' + n] = this._bongos[n];
        if (n == 0) {
            this.s = this._bongos[n];
            this.admin = this._bongos[n].getDB('admin');
            this.config = this._bongos[n].getDB('config');
        }
    };

    /**
     * Stops and restarts a shard bongod process.
     *
     * If opts is specified, the new bongod is started using those options. Otherwise, it is started
     * with its previous parameters. The 'beforeRestartCallback' parameter is an optional function
     * that will be run after the BongoD is stopped, but before it is restarted. The intended uses
     * of the callback are modifications to the dbpath of the bongod that must be made while it is
     * stopped.
     *
     * Warning: Overwrites the old dn/shardn member variables.
     */
    this.restartBongod = function(n, opts, beforeRestartCallback) {
        var bongod;

        if (otherParams.useBridge) {
            bongod = unbridgedConnections[n];
        } else {
            bongod = this["d" + n];
        }

        opts = opts || bongod;
        opts.port = opts.port || bongod.port;

        this.stopBongod(n);

        if (otherParams.useBridge) {
            var bridgeOptions =
                (opts !== bongod) ? opts.bridgeOptions : bongod.fullOptions.bridgeOptions;
            bridgeOptions = Object.merge(otherParams.bridgeOptions, bridgeOptions || {});
            bridgeOptions = Object.merge(bridgeOptions, {
                hostName: otherParams.useHostname ? hostName : "localhost",
            ...
            this._connections[n] = new BongoBridge(bridgeOptions);
        }
        ...
        var newConn = BongoRunner.runBongod(opts);
        if (!newConn) {
    ...
    /**
     * Stops and restarts a config server bongod process.
     *
     * If opts is specified, the new bongod is started using those options. Otherwise, it is started
    ...
    this.restartConfigServer = function(n) {
        ...
        if (otherParams.useBridge) {
            bongod = unbridgedConfigServers[n];
        } else {
            bongod = this["c" + n];
        }
        ...
        if (otherParams.useBridge) {
            var bridgeOptions =
                Object.merge(otherParams.bridgeOptions, bongod.fullOptions.bridgeOptions || {});
            bridgeOptions = Object.merge(bridgeOptions, {
                hostName: otherParams.useHostname ? hostName : "localhost",
                port: this._configServers[n].port,
                // The bongod processes identify themselves to bongobridge as host:port, where the
                // host is the actual hostname of the machine and not localhost.
                dest: hostName + ":" + bongod.port,
            });

            this._configServers[n] = new BongoBridge(bridgeOptions);
        }

        bongod.restart = true;
        var newConn = BongoRunner.runBongod(bongod);
        if (!newConn) {
     ...
     * Note: first bongos needs to be up.
     */
     ...
     *     jsTestOptions().shardMixedBinVersions, jsTestOptions().bongosBinVersion,
     *     otherParams.configOptions.binVersion, otherParams.shardOptions.binVersion,
     *     otherParams.bongosOptions.binVersion
     */
    function _isMixedVersionCluster() {
        var lastStableBinVersion = BongoRunner.getBinVersionFor('last-stable');

        // Must check shardMixedBinVersion because it causes shardOptions.binVersion to be an object
        // (versionIterator) rather than a version string. Must check bongosBinVersion, as well,
        // because it does not update bongosOptions.binVersion.
        if (jsTestOptions().shardMixedBinVersions ||
            (jsTestOptions().bongosBinVersion &&
             BongoRunner.areBinVersionsTheSame(lastStableBinVersion,
                                               jsTestOptions().bongosBinVersion))) {
            return true;
        }
        ...
        if (otherParams.configOptions && otherParams.configOptions.binVersion &&
            BongoRunner.areBinVersionsTheSame(
                lastStableBinVersion,
                BongoRunner.getBinVersionFor(otherParams.configOptions.binVersion))) {
            return true;
        }
        for (var i = 0; i < numConfigs; ++i) {
            if (otherParams['c' + i] && otherParams['c' + i].binVersion &&
                BongoRunner.areBinVersionsTheSame(
                    lastStableBinVersion,
                    BongoRunner.getBinVersionFor(otherParams['c' + i].binVersion))) {
                return true;
            }
        }

        // Check for 'last-stable' bongod servers.
        if (otherParams.shardOptions && otherParams.shardOptions.binVersion &&
            BongoRunner.areBinVersionsTheSame(
                lastStableBinVersion,
                BongoRunner.getBinVersionFor(otherParams.shardOptions.binVersion))) {
            return true;
        }
        for (var i = 0; i < numShards; ++i) {
            if (otherParams['d' + i] && otherParams['d' + i].binVersion &&
                BongoRunner.areBinVersionsTheSame(
                    lastStableBinVersion,
                    BongoRunner.getBinVersionFor(otherParams['d' + i].binVersion))) {
                return true;
            }
        }

        // Check for 'last-stable' bongos servers.
        if (otherParams.bongosOptions && otherParams.bongosOptions.binVersion &&
            BongoRunner.areBinVersionsTheSame(
                lastStableBinVersion,
                BongoRunner.getBinVersionFor(otherParams.bongosOptions.binVersion))) {
            return true;
        }
        for (var i = 0; i < numBongos; ++i) {
            if (otherParams['s' + i] && otherParams['s' + i].binVersion &&
                BongoRunner.areBinVersionsTheSame(
                    lastStableBinVersion,
                    BongoRunner.getBinVersionFor(otherParams['s' + i].binVersion))) {
                return true;
            }
        }
    ...
    var bongosVerboseLevel = otherParams.hasOwnProperty('verbose') ? otherParams.verbose : 1;
    var numBongos = otherParams.hasOwnProperty('bongos') ? otherParams.bongos : 1;
    ...
    if (Array.isArray(numBongos)) {
        for (var i = 0; i < numBongos.length; i++) {
            otherParams["s" + i] = numBongos[i];
        }

        numBongos = numBongos.length;
    } else if (isObject(numBongos)) {
        var tempCount = 0;
        for (var i in numBongos) {
            otherParams[i] = numBongos[i];
            tempCount++;
        }
    ...
    // Start the BongoD servers (shards)
    for (var i = 0; i < numShards; i++) {
        if (otherParams.rs || otherParams["rs" + i]) {
            ...
            if (jsTestOptions().shardMixedBinVersions) {
                ...
                if (!otherParams.shardOptions.binVersion) {
                    ...
                    otherParams.shardOptions.binVersion =
                        BongoRunner.versionIterator(["latest", "last-stable"], true);
                }
            }

            if (otherParams.shardOptions && otherParams.shardOptions.binVersion) {
                otherParams.shardOptions.binVersion =
                    BongoRunner.versionIterator(otherParams.shardOptions.binVersion);
            }
            ...
            if (otherParams.useBridge) {
                ...
                bridgeOptions = Object.merge(bridgeOptions, {
                    hostName: otherParams.useHostname ? hostName : "localhost",
                    // The bongod processes identify themselves to bongobridge as host:port, where
                    // the host is the actual hostname of the machine and not localhost.
                    dest: hostName + ":" + options.port,
                });

                var bridge = new BongoBridge(bridgeOptions);
            }
    ...
    for (var i = 0; i < numShards; i++) {
        ...
        var rsConn = new Bongo(rs.getURL());
    ...
    if (otherParams.configOptions && otherParams.configOptions.binVersion) {
        otherParams.configOptions.binVersion =
            BongoRunner.versionIterator(otherParams.configOptions.binVersion);
    }
    ...
    // Wait for master to be elected before starting bongos
    var csrsPrimary = this.configRS.getPrimary();

    // If 'otherParams.bongosOptions.binVersion' is an array value, then we'll end up constructing a
    // version iterator. We initialize the options for the bongos processes before checking whether
    // we need to run {setFeatureCompatibilityVersion: "3.2"} on the CSRS primary so we know
    // definitively what binVersions will be used for the bongos processes.
    const bongosOptions = [];
    for (var i = 0; i < numBongos; ++i) {
        let options = {
            useHostname: otherParams.useHostname,
            pathOpts: Object.merge(pathOpts, {bongos: i}),
            verbose: bongosVerboseLevel,
            keyFile: keyFile,
        };

        if (otherParams.bongosOptions && otherParams.bongosOptions.binVersion) {
            otherParams.bongosOptions.binVersion =
                BongoRunner.versionIterator(otherParams.bongosOptions.binVersion);
        }

        options = Object.merge(options, otherParams.bongosOptions);
        options = Object.merge(options, otherParams["s" + i]);

        options.port = options.port || allocatePort();

        // TODO(esha): remove after v3.4 ships.
        // Legacy bongoses use a command line option to disable autosplit instead of reading the
        // config.settings collection.
        if (options.binVersion && BongoRunner.areBinVersionsTheSame('3.2', options.binVersion) &&
            !otherParams.enableAutoSplit) {
            options.noAutoSplit = "";
        }

        bongosOptions.push(options);
    }
    ...
    if (_hasNewFeatureCompatibilityVersion && _isMixedVersionCluster()) {
        function setFeatureCompatibilityVersion() {
            ...
            // to ensure that older versions of bongos can successfully connect.
    ...
    // Start the BongoS servers
    for (var i = 0; i < numBongos; i++) {
        const options = bongosOptions[i];
        ...
        var conn = BongoRunner.runBongos(options);
        if (!conn) {
            throw new Error("Failed to start bongos " + i);
        }

        if (otherParams.useBridge) {
            bridge.connectToBridge();
            this._bongos.push(bridge);
            unbridgedBongos.push(conn);
        } else {
            this._bongos.push(conn);
        }

        if (i === 0) {
            this.s = this._bongos[i];
            this.admin = this._bongos[i].getDB('admin');
            this.config = this._bongos[i].getDB('config');
        }

        this["s" + i] = this._bongos[i];
    }

    _extendWithShMethods();

    // If auth is enabled for the test, login the bongos connections as system in order to configure
    // the instances and then log them out again.
    if (keyFile) {
        authutil.asCluster(this._bongos, keyFile, _configureCluster);
    } else {
        _configureCluster();
    }
    ...
    // multiple bongoses. In those cases, the first bongos initializes the contents of the 'config'
    // database, but without waiting for those writes to replicate to all the config servers then
    // the secondary bongoses risk reading from a stale config server and seeing an empty config
    // database.
    ...
    if (jsTestOptions().keyFile) {
        ...
        jsTest.authenticateNodes(this._bongos);
    }
};
