...
// NOTE 'Bongo' may be defined here or in BongoJS.cpp.  Add code to init, not to this constructor.
if (typeof Bongo == "undefined") {
    Bongo = function(host) {
        this.init(host);
    };
}
...
if (!Bongo.prototype) {
    throw Error("Bongo.prototype not defined");
}

if (!Bongo.prototype.find)
    Bongo.prototype.find = function(ns, query, fields, limit, skip, batchSize, options) {
        throw Error("find not implemented");
    };
if (!Bongo.prototype.insert)
    Bongo.prototype.insert = function(ns, obj) {
        throw Error("insert not implemented");
    };
if (!Bongo.prototype.remove)
    Bongo.prototype.remove = function(ns, pattern) {
        throw Error("remove not implemented");
    };
if (!Bongo.prototype.update)
    Bongo.prototype.update = function(ns, query, obj, upsert) {
        throw Error("update not implemented");
    };

if (typeof bongoInject == "function") {
    bongoInject(Bongo.prototype);
}

Bongo.prototype.setSlaveOk = function(value) {
    if (value == undefined)
        value = true;
    this.slaveOk = value;
};

Bongo.prototype.getSlaveOk = function() {
    return this.slaveOk || false;
};

Bongo.prototype.getDB = function(name) {
...
Bongo.prototype.getDBs = function() {
...
Bongo.prototype.adminCommand = function(cmd) {
 ...
 */
Bongo.prototype.getLogComponents = function() {
 ...
 */
Bongo.prototype.setLogLevel = function(logLevel, component) {
    componentNames = [];
...
Bongo.prototype.getDBNames = function() {
    return this.getDBs().databases.map(function(z) {
        return z.name;
    });
};

Bongo.prototype.getCollection = function(ns) {
    var idx = ns.indexOf(".");
    if (idx < 0)
...
Bongo.prototype.toString = function() {
    return "connection to " + this.host;
};
Bongo.prototype.tojson = Bongo.prototype.toString;
 ...
 */
Bongo.prototype.setReadPref = function(mode, tagSet) {
...
Bongo.prototype._setReadPrefUnsafe = function(mode, tagSet) {
    this._readPrefMode = mode;
    this._readPrefTagSet = tagSet;
};

Bongo.prototype.getReadPrefMode = function() {
    return this._readPrefMode;
};

Bongo.prototype.getReadPrefTagSet = function() {
    return this._readPrefTagSet;
};

// Returns a readPreference object of the type expected by bongos.
Bongo.prototype.getReadPref = function() {
    var obj = {}, mode, tagSet;
 ...
 */
Bongo.prototype.setReadConcern = function(level) {
    if (!level) {
 ...
 */
Bongo.prototype.getReadConcern = function() {
    return this._readConcernLevel;
};

connect = function(url, user, pass) {
    if (url instanceof BongoURI) {
        user = url.user;
        pass = url.password;
        url = url.uri;
    }
    ...
    if (!url.startsWith("bongodb://")) {
        const colon = url.lastIndexOf(":");
        const slash = url.lastIndexOf("/");
        if (slash == 0) {
            throw Error("Failed to parse bongodb:// URL: " + url);
        }
        if (slash == -1 && colon == -1) {
            url = "bongodb://127.0.0.1:27017/" + url;
        } else if (slash != -1) {
            url = "bongodb://" + url;
        }
    }
    ...
    chatty("BongoDB server version: " + serverVersion);
 ...
 *
 */
Bongo.prototype.useWriteCommands = function() {
    return (this.writeMode() != "legacy");
};

Bongo.prototype.forceWriteMode = function(mode) {
    this._writeMode = mode;
};

Bongo.prototype.hasWriteCommands = function() {
    var hasWriteCommands = (this.getMinWireVersion() <= 2 && 2 <= this.getMaxWireVersion());
    return hasWriteCommands;
};

Bongo.prototype.hasExplainCommand = function() {
...
Bongo.prototype.writeMode = function() {
 ...
 */
Bongo.prototype.useReadCommands = function() {
 ...
 */
Bongo.prototype.forceReadMode = function(mode) {
 ...
 */
Bongo.prototype.readMode = function() {
...
Bongo.prototype.setWriteConcern = function(wc) {
...
Bongo.prototype.getWriteConcern = function() {
    return this._writeConcern;
};

Bongo.prototype.unsetWriteConcern = function() {
