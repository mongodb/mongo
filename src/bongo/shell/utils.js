...
jsTestOptions = function() {
    if (TestData) {
        return Object.merge(_jsTestOptions, {
            ...
            setParametersBongos: TestData.setParametersBongos,
            ...
            bongosBinVersion: TestData.bongosBinVersion || "",
...
jsTest.isBongos = function(conn) {
    return conn.getDB('admin').isMaster().msg == 'isdbgrid';
};

defaultPrompt = function() {
    var status = db.getBongo().authStatus;
    var prefix = db.getBongo().promptPrefix;
    ...
    if (typeof prefix == 'undefined') {
        ...
        db.getBongo().promptPrefix = prefix;
    }
    ...
    try {
        ...
        if (!status || !status.authRequired) {
            try {
                ...
                db.getBongo().authStatus = {replSetGetStatus: true, isMaster: true};
...
replSetMemberStatePrompt = function() {
    ...
    if (stateInfo.ok) {
        ...
        // Otherwise fall back to reporting the numeric myState field (bongodb 1.6).
        if (!state) {
...
if (typeof(_useWriteCommandsDefault) == 'undefined') {
    // This is for cases when the v8 engine is used other than the bongo shell, like map reduce.
...
shellPrintHelper = function(x) {
    if (typeof(x) == "undefined") {
        ...
        if (__callLastError && typeof(db) != "undefined" && db.getBongo &&
            db.getBongo().writeMode() == "legacy") {
            __callLastError = false;
 ...
 *
 * It should be used instead of awaitRSClientHost when there is no BongoS with a connection to the
 * replica set.
 */
...
rs.slaveOk = function(value) {
    return db.getBongo().setSlaveOk(value);
};
rs.status = function() {
...
help = shellHelper.help = function(x) {
    if (x == "mr") {
        print("\nSee also http://dochub.bongodb.org/core/mapreduce");
        print("\nfunction mapf() {");
        ...
        print(
            "\nNormally one specifies the server on the bongo shell command line.  Run bongo --help to see those options.");
        print("Additional connections may be opened:\n");
        print("    var x = new Bongo('host[:port]');");
        ...
        print("\tBongoRunner.runBongod(args)   DELETES DATA DIR and then starts bongod");
        ...
        print("\t" + "exit                         quit the bongo shell");
    } else
