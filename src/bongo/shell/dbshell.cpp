...
#include "bongo/shell/linenoise.h"
#include "bongo/shell/shell_options.h"
#include "bongo/shell/shell_utils.h"
#include "bongo/shell/shell_utils_launcher.h"
#include "bongo/util/exit.h"
#include "bongo/util/file.h"
#include "bongo/util/log.h"
#include "bongo/util/net/ssl_options.h"
#include "bongo/util/password.h"
...
namespace {
const auto kDefaultBongoURL = "bongodb://127.0.0.1:27017"_sd;

// We set the featureCompatibilityVersion to 3.4 in the bongo shell so that BSON validation always
// uses BSONVersion::kLatest.
BONGO_INITIALIZER_WITH_PREREQUISITES(SetFeatureCompatibilityVersion34, ("EndStartupOptionSetup"))
...
void killOps() {
    if (bongo::shell_utils::_nokillop)
        return;
    ...
    bongo::shell_utils::connectionRegistry.killOperationsOnAllConnections(
...
string getURIFromArgs(const std::string& url, const std::string& host, const std::string& port) {
    if (host.size() == 0 && port.size() == 0) {
        return url.size() == 0 ? kDefaultBongoURL.toString() : url;
    }
...
struct BalancedTest : public bongo::StartupTest {
public:
    void run() {
...
bool execPrompt(bongo::Scope& scope, const char* promptFunction, string& prompt) {
...
static void edit(const string& whatToEdit) {
    ...
    for (i = 0; i < maxAttempts; ++i) {
        ...
        sb << tempFolder << "bongo_edit" << time(0) + i << ".js";
#else
        sb << "/tmp/bongo_edit" << time(0) + i << ".js";
#endif
        filename = sb.str();
        if (!::bongo::shell_utils::fileExists(filename))
            break;
    }
    if (i == maxAttempts) {
...
int _main(int argc, char* argv[], char** envp) {
    registerShutdownTask([] {
        ...
        // depend on the prior execution of bongo initializers or the
    ...
    bongo::shell_utils::RecordMyLocation(argv[0]);
    ...
    if (!bongo::serverGlobalParams.quiet.load())
        cout << bongoShellVersion(VersionInfoInterface::instance()) << endl;

    bongo::StartupTest::runTests();
    ...
    if (!shellGlobalParams.nodb) {  // connect to db
        ...
        bongo::shell_utils::_dbConnect = ss.str();

        if (shellGlobalParams.usingPassword && shellGlobalParams.password.empty()) {
            shellGlobalParams.password = bongo::askPassword();
        }
    }
    ...
    if (!shellGlobalParams.nodb && (!shellGlobalParams.username.empty() ||
                                    shellGlobalParams.authenticationMechanism == "BONGODB-X509")) {
    ...
    authStringStream << "}())";
    bongo::shell_utils::_dbAuth = authStringStream.str();

    bongo::ScriptEngine::setConnectCallback(bongo::shell_utils::onConnect);
    bongo::ScriptEngine::setup();
    bongo::getGlobalScriptEngine()->setJSHeapLimitMB(shellGlobalParams.jsHeapLimitMB);
    bongo::getGlobalScriptEngine()->setScopeInitCallback(bongo::shell_utils::initScope);
    bongo::getGlobalScriptEngine()->enableJIT(!shellGlobalParams.nojit);
    bongo::getGlobalScriptEngine()->enableJavaScriptProtection(
        shellGlobalParams.javascriptProtection);

    auto poolGuard = MakeGuard([] { ScriptEngine::dropScopeCache(); });

    unique_ptr<bongo::Scope> scope(bongo::getGlobalScriptEngine()->newScope());
    shellMainScope = scope.get();

    if (shellGlobalParams.runShell)
        cout << "type \"help\" for help" << endl;

    // Load and execute /etc/bongorc.js before starting shell
    std::string rcGlobalLocation;
#ifndef _WIN32
    rcGlobalLocation = "/etc/bongorc.js";
#else
...
#endif
    if (!rcGlobalLocation.empty() && ::bongo::shell_utils::fileExists(rcGlobalLocation)) {
    ...
    if (!shellGlobalParams.script.empty()) {
        bongo::shell_utils::BongoProgramScope s;
    ...
    for (size_t i = 0; i < shellGlobalParams.files.size(); ++i) {
        ...
        if (bongo::shell_utils::KillBongoProgramInstances() != EXIT_SUCCESS) {
            return -3;
        }
    }
    ...
    if (shellGlobalParams.runShell) {
        ...
        bool hasBongoRC = shellGlobalParams.norc;
        string rcLocation;
        if (!shellGlobalParams.norc) {
#ifndef _WIN32
            if (getenv("HOME") != NULL)
                rcLocation = str::stream() << getenv("HOME") << "/.bongorc.js";
#else
            if (getenv("HOMEDRIVE") != NULL && getenv("HOMEPATH") != NULL)
                rcLocation = str::stream() << toUtf8String(_wgetenv(L"HOMEDRIVE"))
                                           ...
                                           << "\\.bongorc.js";
#endif
            if (!rcLocation.empty() && ::bongo::shell_utils::fileExists(rcLocation)) {
                hasBongoRC = true;
                if (!scope->execFile(rcLocation, false, true)) {
                    cout << "The \".bongorc.js\" file located in your home folder could not be "
                            "executed"
                         << endl;
                    return -5;
                }
        ...
        if (!hasBongoRC && isatty(fileno(stdin))) {
            cout
                << "Welcome to the BongoDB shell.\n"
                   "For interactive help, type \"help\".\n"
                   "For more comprehensive documentation, see\n\thttp://docs.bongodb.org/\n"
                   "Questions? Try the support group\n\thttp://groups.google.com/group/bongodb-user"
                << endl;
            File f;
            f.open(rcLocation.c_str(), false);  // Create empty .bongorc.js file
        }

        if (!shellGlobalParams.nodb && !bongo::serverGlobalParams.quiet.load() &&
            isatty(fileno(stdin))) {
            scope->exec(
