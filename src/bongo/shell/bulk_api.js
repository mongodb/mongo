...
var _bulk_api_module = (function() {
    ...
    var Bulk = function(collection, ordered) {
        ...
        var executeBatch = function(batch) {
            ...
            var cursor = new DBQuery(collection.getBongo(),
                                     collection._db,
                                     cmdColl,
                                     cmdColl.getFullName(),
                                     cmd,
                                     ...
                                     0 /* batchSize */,
                                     0 /* flags */);
            var rc = collection.getBongo().getReadConcern();
            if (rc) {
        ...
        var extractGLEErrors = function(gleResponse) {
            ...
            // Handling of writeback not needed for bongo shell.
            return extractedErr;
        };
        ...
        this.execute = function(_writeConcern) {
            ...
            var useWriteCommands = collection.getBongo().useWriteCommands();

            // Execute all the batches
            for (var i = 0; i < batches.length; i++) {
                // Execute the batch
                if (collection.getBongo().hasWriteCommands() &&
                    collection.getBongo().writeMode() == "commands") {
