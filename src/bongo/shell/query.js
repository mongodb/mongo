...
DBQuery.prototype._exec = function() {
    if (!this._cursor) {
        ...
        if (this._bongo.useReadCommands() && this._canUseFindCommand()) {
            ...
            this._cursor = new DBCommandCursor(cmdRes._bongo, cmdRes, this._batchSize);
        } else {
            ...
            this._cursor = this._bongo.find(this._ns,
                                            this._query,
                                            this._fields,
                                            this._limit,
                                            this._skip,
...
/**
* Get partial results from a bongos if some shards are down (instead of throwing an error).
*
* @method
* @see http://docs.bongodb.org/meta-driver/latest/legacy/bongodb-wire-protocol/#op-query
* @return {DBQuery}
*/
...
*
* @method
* @see http://docs.bongodb.org/manual/tutorial/project-fields-from-query-results/
...
*
* @method
* @see http://docs.bongodb.org/manual/tutorial/create-tailable-cursor/
...
*
* @method
* @see http://docs.bongodb.org/manual/reference/operator/query-modifier/
 ...
 * @see http://dochub.bongodb.org/core/bongowireprotocol#BongoWireProtocol-OPQUERY
 */
DBQuery.Option = {
    tailable: 0x2,
    slaveOk: 0x4,
...
function DBCommandCursor(bongo, cmdResult, batchSize) {
    if (cmdResult.ok != 1) {
    ...
    if (bongo.useReadCommands()) {
        ...
        if (cmdResult.cursor.id) {
            ...
            this._cursorHandle = bongo.cursorHandleFromId(cmdResult.cursor.id);
        }
    } else {
        this._cursor = bongo.cursorFromId(cmdResult.cursor.ns, cmdResult.cursor.id, batchSize);
    }
}
