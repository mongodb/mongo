...
Status addBongoShellOptions(moe::OptionSection* options) {
    ...
    options->addOptionChaining("nodb",
                               "nodb",
                               moe::Switch,
                               "don't connect to bongod on startup - no 'db address' arg expected");

    options->addOptionChaining(
        "norc", "norc", moe::Switch, "will not run the \".bongorc.js\" file on start up");
    ...
    // for testing, kill op will also be disabled automatically if the tests starts a bongo
    // program
...
bool handlePreValidationBongoShellOptions(const moe::Environment& params,
    ...
    if (params.count("version") || params.count("help")) {
        ...
        log() << bongoShellVersion(vii);
        if (params.count("help")) {
            log() << getBongoShellHelp(args[0], moe::startupOptions);
        } else {
            vii.logBuildInfo();
        }
        return false;
...
Status storeBongoShellOptions(const moe::Environment& params,
                              const std::vector<std::string>& args) {
    Status ret = Status::OK();
    if (params.count("quiet")) {
        bongo::serverGlobalParams.quiet.store(true);
    }
#ifdef BONGO_CONFIG_SSL
    ...
    if (params.count("nokillop")) {
        bongo::shell_utils::_nokillop = true;
    }
    ...
    if (params.count("writeMode")) {
        ...
        if (mode != "commands" && mode != "legacy" && mode != "compatibility") {
            throw MsgAssertionException(
                17396, bongoutils::str::stream() << "Unknown writeMode option: " << mode);
        }
        ...
        if (shellGlobalParams.nodb) {
            ...
            if (basename.find_first_of('.') == string::npos ||
                (basename.find(".js", basename.size() - 3) == string::npos &&
                 !::bongo::shell_utils::fileExists(dbaddress))) {
    ...
    if (shellGlobalParams.url == "*") {
        ...
        sb << getBongoShellHelp(args[0], moe::startupOptions);
        return Status(ErrorCodes::BadValue, sb.str());
    }

    if (shellGlobalParams.url.find("bongodb://") == 0) {
        auto cs_status = BongoURI::parse(shellGlobalParams.url);
        if (!cs_status.isOK()) {
