...
namespace {
void validateInputString(bongo::StringData input, std::uint32_t* signalingFlags) {
    ...
    bongo::StringData noSign = input.substr(start);
    ...
    if (!std::isdigit(noSign[0])) {
        if (noSign[0] != '.') {
            *signalingFlags = bongo::Decimal128::SignalingFlag::kInvalid;
            return;
    ...
    if (isZero) {
        ...
        *signalingFlags = bongo::Decimal128::SignalingFlag::kNoFlag;
    }
    ...
    // Check exponent
    bongo::StringData exponent = noSign.substr(i);
...
BONGO_STATIC_ASSERT(t34hi64 == 0x1ed09bead87c0);
BONGO_STATIC_ASSERT(t34lo64 == 0x378d8e63ffffffff);
}  // namespace
