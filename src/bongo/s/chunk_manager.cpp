...
/**
 * This is an adapter so we can use config diffs - bongos and bongod do them slightly differently.
 *
 * The bongos adapter here tracks all shards, and stores ranges by (max, Chunk) in the map.
 */
...
IndexBounds ChunkManager::getIndexBoundsForQuery(const BSONObj& key,
    ...
    // $text is not allowed in planning since we don't have text index on bongos.
    // TODO: Treat $text query as a no-op in planning on bongos. So with shard key {a: 1},
