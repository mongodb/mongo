 ...
 * bongos-specific behavior on bongod (auditing and replication information in commands)
 */
 ...
 * is sent to the remote shard host to initialize the connection as coming from bongos.
...
bool VersionManager::isVersionableCB(DBClientBase* conn) {
    // We do not version shard connections when issued from bongod
    if (!isBongos()) {
        return false;
    }
