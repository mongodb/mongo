 ...
 */
class ShardConnFixture : public bongo::unittest::Test {
public:
    void setUp() {
        if (!haveClient()) {
            Client::initThread("ShardConnFixture", getGlobalServiceContext(), NULL);
        }
        _maxPoolSizePerHost = bongo::shardConnectionPool.getMaxPoolSize();

        bongo::ConnectionString::setConnectionHook(
            bongo::MockConnRegistry::get()->getConnStrHook());
        _dummyServer = new MockRemoteDBServer(TARGET_HOST);
        bongo::MockConnRegistry::get()->addServer(_dummyServer);
    }

    void tearDown() {
        ShardConnection::clearPool();

        bongo::MockConnRegistry::get()->removeServer(_dummyServer->getServerAddress());
        delete _dummyServer;

        bongo::shardConnectionPool.setMaxPoolSize(_maxPoolSizePerHost);
    }
...
protected:
    ...
    void checkNewConns(void (*checkFunc)(uint64_t, uint64_t),
        ...
        const uint64_t oldCreationTime = bongo::curTimeMicros64();
...
TEST_F(ShardConnFixture, InvalidateBadConnInPool) {
    ...
    const uint64_t badCreationTime = bongo::curTimeMicros64();
    killServer();

    try {
        conn2.get()->query("test.user", bongo::Query());
...
TEST_F(ShardConnFixture, DontReturnKnownBadConnToPool) {
    ...
    try {
        conn3.get()->query("test.user", bongo::Query());
    } catch (const bongo::SocketException&) {
    }
...
TEST_F(ShardConnFixture, InvalidateBadConnEvenWhenPoolIsFull) {
    bongo::shardConnectionPool.setMaxPoolSize(2);
