...
#include "bongo/util/concurrency/spin_lock.h"
...
/**
 * Command to allow access to the sharded conn pool information in bongos.
 */
...
public:
    ...
    virtual bool run(OperationContext* txn,
                     const string& dbname,
                     bongo::BSONObj& cmdObj,
                     int options,
                     std::string& errmsg,
                     bongo::BSONObjBuilder& result) {
 ...
 */
class ClientConnections {
    BONGO_DISALLOW_COPYING(ClientConnections);
...
ShardConnection::ShardConnection(const ConnectionString& connectionString,
    ...
    if (isBongos()) {
        // In bongos, we record this connection as having been used for useful work to provide
