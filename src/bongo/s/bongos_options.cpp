...
BongosGlobalParams bongosGlobalParams;

Status addBongosOptions(moe::OptionSection* options) {
...
void printBongosHelp(const moe::OptionSection& options) {
    std::cout << options.helpString() << std::endl;
};

bool handlePreValidationBongosOptions(const moe::Environment& params,
                                      const std::vector<std::string>& args) {
    if (params.count("help") && params["help"].as<bool>() == true) {
        printBongosHelp(moe::startupOptions);
        return false;
    }
    ...
    if (params.count("test") && params["test"].as<bool>() == true) {
        ::bongo::logger::globalLogDomain()->setMinimumLoggedSeverity(
            ::bongo::logger::LogSeverity::Debug(5));
...
Status validateBongosOptions(const moe::Environment& params) {
...
Status canonicalizeBongosOptions(moe::Environment* params) {
...
Status storeBongosOptions(const moe::Environment& params) {
    ...
    if (params.count("noscripting")) {
        // This option currently has no effect for bongos
    }
    ...
    if (bongosGlobalParams.configdbs.getServers().size() < 3) {
