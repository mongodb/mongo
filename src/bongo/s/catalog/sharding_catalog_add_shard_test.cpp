...
#include "bongo/s/write_ops/batched_insert_request.h"
...
protected:
     ...
     * kind of host it is -- bongos, regular bongod, config bongod -- and whether the replica set
...
// Cannot add bongos as a shard.
TEST_F(AddShardTest, AddBongosAsShard) {
    std::unique_ptr<RemoteCommandTargeterMock> targeter(
        stdx::make_unique<RemoteCommandTargeterMock>());
    HostAndPort shardTarget("StandaloneHost:12345");
    targeter->setConnectionStringReturnValue(ConnectionString(shardTarget));
...
// Attempt to add a pre-v3.4 bongod.
