 ...
 * config.bongos collection. All manipulation of documents coming from that
 * collection should be done with this class.
 */
class BongosType {
public:
    // Name of the bongos collection in the config server.
    static const std::string ConfigNS;

    // Field names and types in the bongos collection type.
    static const BSONField<std::string> name;
    static const BSONField<Date_t> ping;
    static const BSONField<long long> uptime;
    static const BSONField<bool> waiting;
    static const BSONField<std::string> bongoVersion;
    ...
    /**
     * Constructs a new BongosType object from BSON.
     * Also does validation of the contents.
     */
    static StatusWith<BongosType> fromBSON(const BSONObj& source);
    ...
    const std::string& getBongoVersion() const {
        return _bongoVersion.get();
    }
    bool isBongoVersionSet() const {
        return _bongoVersion.is_initialized();
    }
    void setBongoVersion(const std::string& bongoVersion);
...
private:
    ...
    // (O) the bongodb version of the pinging bongos
    boost::optional<std::string> _bongoVersion;
    // (O) the config version of the pinging bongos
