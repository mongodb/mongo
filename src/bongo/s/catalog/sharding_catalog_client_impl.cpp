...
BONGO_FP_DECLARE(failApplyChunkOps);
BONGO_FP_DECLARE(setDropCollDistLockWait);
...
StatusWith<ChunkVersion> createFirstChunks(OperationContext* txn,
    ...
    if (initPoints.empty()) {
        ...
        if (initShardIds.empty()) {
            // If not specified, only use the primary shard (note that it's not safe for bongos to
            // put initial chunks on other shards without the primary bongod knowing)
            shardIds.push_back(primaryShardId);
        } else {
...
Status ShardingCatalogClientImpl::shardCollection(OperationContext* txn,
    ...
    // Lock the collection globally so that no other bongos can try to shard or drop the collection
    // at the same time.
    auto scopedDistLock = getDistLockManager()->lock(
    ...
    // Tell the primary bongod to refresh its data
    // TODO:  Think the real fix here is for bongos to just
...
Status ShardingCatalogClientImpl::dropCollection(OperationContext* txn, const NamespaceString& ns) {
    ...
    BONGO_FAIL_POINT_BLOCK(setDropCollDistLockWait, customWait) {
...
Status ShardingCatalogClientImpl::applyChunkOpsDeprecated(OperationContext* txn,
    ...
    // SERVER-22659.
    if (BONGO_FAIL_POINT(failApplyChunkOps)) {
