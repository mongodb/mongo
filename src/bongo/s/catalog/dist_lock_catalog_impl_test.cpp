...
#include "bongo/s/write_ops/batched_update_request.h"
 ...
 */
class DistLockCatalogFixture : public ShardingBongodTestFixture {
public:
...
protected:
    void setUp() override {
        ...
        uassertStatusOK(initializeGlobalShardingStateForBongodForTest(ConnectionString(dummyHost)));
...
TEST_F(DistLockCatalogFixture, GrabLockNoOp) {
    auto future = launchAsync([this] {
        ...
        auto resultStatus =
            distLockCatalog()
                ->grabLock(operationContext(), "test", myID, "me", "bongos", now, "because")
                .getStatus();
...
TEST_F(DistLockCatalogFixture, GrabLockWithNewDoc) {
    auto future = launchAsync([this] {
        ...
        auto resultStatus = distLockCatalog()->grabLock(
            operationContext(), "test", myID, "me", "bongos", now, "because");
        ...
        ASSERT_EQUALS("bongos", lockDoc.getProcess());
...
TEST_F(DistLockCatalogFixture, OvertakeLockNoOp) {
    auto future = launchAsync([this] {
        ...
        auto resultStatus =
            distLockCatalog()
                ->overtakeLock(
                    operationContext(), "test", myID, currentOwner, "me", "bongos", now, "because")
                .getStatus();
