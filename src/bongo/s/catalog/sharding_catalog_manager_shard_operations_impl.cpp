...
BONGO_FP_DECLARE(dontUpsertShardIdentityOnNewShards);
...
StatusWith<ShardType> ShardingCatalogManagerImpl::_validateHostAsShard(
    ...
    // Check if the node being added is a bongos or a version of bongod too old to speak the current
    ...
    if (!swCommandResponse.isOK()) {
        if (swCommandResponse.getStatus() == ErrorCodes::RPCProtocolNegotiationFailed) {
            // Bongos to bongos commands are no longer supported in the wire protocol
            // (because bongos does not support OP_COMMAND), similarly for a new bongos
            // and an old bongod. So the call will fail in such cases.
            // TODO: If/When bongos ever supports opCommands, this logic will break because
            ...
            return {ErrorCodes::RPCProtocolNegotiationFailed,
                    str::stream() << targeter->connectionString().toString()
                                  ...
                                  << " likely because it contains a node that is a bongos or an old"
                                  << " version of bongod."};
        } else {
    ...
    // If we're running this code, that means the bongos that the addShard request originated from
    // must be at least version 3.4 (since 3.2 bongoses don't know about the _configsvrAddShard
    // command).  Since it is illegal to have v3.4 bongoses with v3.2 shards, we should reject
    ...
    // TODO(SERVER-25623): This approach won't work to prevent v3.6 bongoses from adding v3.4
    ...
    if (maxWireVersion < WireVersion::COMMANDS_ACCEPT_WRITE_CONCERN) {
        return Status(ErrorCodes::IncompatibleServerVersion,
                      str::stream() << "Cannot add " << connectionString.toString()
                                    << " as a shard because we detected a bongod with server "
                                       "version older than 3.4.0.  It is invalid to add v3.2 and "
                                       "older shards through a v3.4 bongos.");
    }
...
StatusWith<std::string> ShardingCatalogManagerImpl::addShard(
    ...
    // Note: This is necessary because as of 3.4, removeShard is performed by bongos (unlike
    ...
    if (!BONGO_FAIL_POINT(dontUpsertShardIdentityOnNewShards)) {
