 ...
 * and the versioning protocol between clients (i.e. the set of calls between bongos and
 * bongod).
 *
 * Friendly notice:
 *
 ...
 *   new bongos/d can co-exist in a cluster with the old ones.
...
enum UpgradeHistory {
    ...
    /**
     * The unreported version older bongoses used before config.version collection existed
     *
    ...
    /**
     * Base version used by pre-2.4 bongoses with no collection epochs.
     */
     ...
     * + Bongos pings include a "bongoVersion" field indicating the bongos version
     * + Bongos pings include a "configVersion" field indicating the current config version
     * + Bongos explicitly ignores any collection with a "primary" field
     */
     ...
     *
     * + Dropping a collection from bongos now waits for the chunks to be removed from the
     *   config server before contacting each shard. Because of this, bongos should be
     *   upgraded first before bongod or never drop collections during upgrade.
     */
