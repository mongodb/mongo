...
#include "bongo/s/query/cluster_query_knobs.h"
...
Status ClusterAggregate::runAggregate(OperationContext* txn,
    ...
    // We won't try to execute anything on a bongos, but we still have to populate this map so that
    // any $lookups, etc. will be able to have a resolved view definition. It's okay that this is
    // incorrect, we will repopulate the real resolved namespace map on the bongod. Note that we
    ...
    // Split the pipeline into pieces for bongod(s) and this bongos. If needSplit is true,
    ...
    // ShardConnection so that the merging bongod is sent the config servers on connection init.
...
Status ClusterAggregate::aggPassthrough(OperationContext* txn,
    ...
    if (ResolvedView::isResolvedViewErrorResponse(responseObj)) {
        ...
        // underlying collection namespace is used to execute the aggregation on bongoD. Any cursor
