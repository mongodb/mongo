...
// BongoS commands implementation
        ...
        for (const ShardId& shardId : shardIds) {
            ...
            while (j.more()) {
                ...
                // flags broken out in 2.4+
                    ...
                    warning() << "bongos collstats doesn't know about: " << e.fieldName();
                }
            }
...
public:
    ...
    Status explain(OperationContext* txn,
        ...
        try {
            ...
            // TODO: this can throw a stale config when bongos is not up-to-date -- fix.
...
public:
    ...
    virtual bool run(OperationContext* txn,
        ...
        if (!str::startsWith(x, dbName)) {
            errmsg = str::stream() << "doing a splitVector across dbs isn't supported via bongos";
            return false;
        }
...
public:
    ...
    Status explain(OperationContext* txn,
        ...
        const char* bongosStageName = ClusterExplain::getStageNameForReadOp(shardResults, cmdObj);
...
public:
    ...
    virtual bool run(OperationContext* txn,
                     ...
                     string& errmsg,
                     BSONObjBuilder& result) {
        errmsg = "compact not allowed through bongos";
        return false;
    }
} compactCmd;
