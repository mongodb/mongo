...
#include "bongo/db/commands/user_management_commands.h"
...
#include "bongo/s/commands/sharded_command_processing.h"
...
/**
 * This command is used only by bongorestore to handle restoring users/roles.  We do this so
 * that bongorestore doesn't do direct inserts into the admin.system.users and
 * admin.system.roles, which would bypass the authzUpdateLock and allow multiple concurrent
 * modifications to users/roles.  What bongorestore now does instead is it inserts all user/role
...
public:
    ...
    virtual void help(stringstream& ss) const {
        ss << "Internal command used by bongorestore for updating user/role data";
    }
