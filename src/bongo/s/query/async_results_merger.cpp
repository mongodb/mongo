...
Status AsyncResultsMerger::askForNextBatch_inlock(OperationContext* txn, size_t remoteIndex) {
    ...
    // If bongod returned less docs than the requested batchSize then modify the next getMore
...
bool AsyncResultsMerger::MergingComparator::operator()(const size_t& lhs, const size_t& rhs) {
    ...
    // This does not need to sort with a collator, since bongod has already mapped strings to their
