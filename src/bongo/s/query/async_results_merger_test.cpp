...
#include "bongo/s/query/async_results_merger.h"
...
protected:
    ...
    /**
     * Schedules a "CommandOnShardedViewNotSupportedOnBongod" error response w/ view definition.
     */
    void scheduleNetworkViewResponse(const std::string& ns, const std::string& pipelineJsonArr) {
        ...
        bob.append("ok", 0.0);
        bob.append("errmsg", "Command on view must be executed by bongos");
        bob.append("code", 169);
...
TEST_F(AsyncResultsMergerTest, ReceivedViewDefinitionFromShard) {
    ...
    ASSERT_EQ(statusWithNext.getStatus().code(),
              ErrorCodes::CommandOnShardedViewNotSupportedOnBongod);
