...
/**
 * This is the lightweight bongoS analogue of the PlanStage abstraction used to execute queries on
 * bongoD (see bongo/db/plan_stage.h).
 *
 * Each subclass is a query execution stage which executes on the merging node. In general, the
 * execution plan on bongos could have a tree of execution stages, but currently each node has at
