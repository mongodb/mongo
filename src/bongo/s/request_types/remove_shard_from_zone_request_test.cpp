...
TEST(RemoveShardFromZoneRequest, BasicValidBongosCommand) {
    auto requestStatus =
        RemoveShardFromZoneRequest::parseFromBongosCommand(BSON("removeShardFromZone"
                                                                << "a"
                                                                << "zone"
                                                                << "z"));
...
TEST(RemoveShardFromZoneRequest, MissingZoneErrors) {
    auto request = RemoveShardFromZoneRequest::parseFromBongosCommand(BSON("removeShardFromZone"
                                                                           << "a"));
    ASSERT_EQ(ErrorCodes::NoSuchKey, request.getStatus());
}

TEST(RemoveShardFromZoneRequest, MissingShardNameErrors) {
    auto request = RemoveShardFromZoneRequest::parseFromBongosCommand(BSON("zone"
                                                                           << "z"));
    ASSERT_EQ(ErrorCodes::NoSuchKey, request.getStatus());
}

TEST(RemoveShardFromZoneRequest, WrongShardNameTypeErrors) {
    auto request = RemoveShardFromZoneRequest::parseFromBongosCommand(
...
TEST(RemoveShardFromZoneRequest, CannotUseBongosToParseConfigCommand) {
    auto request =
        RemoveShardFromZoneRequest::parseFromBongosCommand(BSON("_configsvrRemoveShardFromZone"
                                                                << "a"
                                                                << "zone"
                                                                << "z"));
    ASSERT_EQ(ErrorCodes::NoSuchKey, request.getStatus());
...
TEST(CfgRemoveShardFromZoneRequest, CannotUseConfigToParseBongosCommand) {
