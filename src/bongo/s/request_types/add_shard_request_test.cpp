...
// top-level command name) are identical between the external bongos version and internal config
// version.

TEST(AddShardRequest, ParseInternalFieldsInvalidConnectionString) {
    {
        BSONObj obj = BSON(AddShardRequest::bongosAddShard << ",,,");

        auto swAddShardRequest = AddShardRequest::parseFromBongosCommand(obj);
...
TEST(AddShardRequest, ParseInternalFieldsMissingMaxSize) {
    {
        BSONObj obj =
            BSON(AddShardRequest::bongosAddShard << kConnString << AddShardRequest::shardName
                                                 << kShardName);
...
TEST(AddShardRequest, ParseInternalFieldsMissingName) {
    {
        BSONObj obj =
            BSON(AddShardRequest::bongosAddShard << kConnString << AddShardRequest::maxSizeMB
                                                 << kMaxSizeMB);
...
TEST(AddShardRequest, ToCommandForConfig) {
    BSONObj bongosCmdObj = BSON(
        AddShardRequest::bongosAddShard << kConnString << AddShardRequest::shardName << kShardName
                                        << AddShardRequest::maxSizeMB
                                        << kMaxSizeMB);

    auto swAddShardRequest = AddShardRequest::parseFromBongosCommand(bongosCmdObj);
    ASSERT_OK(swAddShardRequest.getStatus());
    auto req = swAddShardRequest.getValue();
...
TEST(AddShardRequest, ToCommandForConfigMissingName) {
    BSONObj bongosCmdObj = BSON(
        AddShardRequest::bongosAddShard << kConnString << AddShardRequest::maxSizeMB << kMaxSizeMB);
...
TEST(AddShardRequest, ValidateLocalHostAllowed) {
    ...
    {
        BSONObj bongosCmdObj = BSON(AddShardRequest::bongosAddShard << kConnString);
    ...
    {
        BSONObj bongosCmdObj = BSON(AddShardRequest::bongosAddShard << kConnStringNonLocalHost);
