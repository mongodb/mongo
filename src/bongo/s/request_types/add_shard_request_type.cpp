...
const BSONField<std::string> AddShardRequest::bongosAddShard("addShard");
const BSONField<std::string> AddShardRequest::bongosAddShardDeprecated("addshard");
...
StatusWith<AddShardRequest> AddShardRequest::parseFromBongosCommand(const BSONObj& obj) {
    invariant(obj.nFields() >= 1);
    invariant(bongosAddShard.name() == obj.firstElement().fieldNameStringData() ||
              bongosAddShardDeprecated.name() == obj.firstElement().fieldNameStringData());
