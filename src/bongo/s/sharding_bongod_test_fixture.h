...
/**
 * Sets up this fixture as a bongod with a storage engine, OpObserver, and as a member of a replica
 * set.
 *
 ...
 */
class ShardingBongodTestFixture : public ServiceContextBongoDTest {
public:
    ShardingBongodTestFixture();
    ~ShardingBongodTestFixture();
     ...
     * initialization done by an actual config or shard bongod server.
     *
     * It is illegal to call this if serverGlobalParams.clusterRole is not ClusterRole::ShardServer
     * or ClusterRole::ConfigServer.
     */
    Status initializeGlobalShardingStateForBongodForTest(const ConnectionString& configConnStr);
