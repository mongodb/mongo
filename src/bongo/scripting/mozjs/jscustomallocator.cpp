...
#include <malloc.h>
#else
#define BONGO_NO_MALLOC_USABLE_SIZE
#endif

/**
 * This shim interface (which controls dynamic allocation within SpiderMonkey),
 * consciously uses std::malloc and friends over bongoMalloc. It does this
 ...
 */
BONGO_TRIVIALLY_CONSTRUCTIBLE_THREAD_LOCAL size_t total_bytes;
BONGO_TRIVIALLY_CONSTRUCTIBLE_THREAD_LOCAL size_t max_bytes;
...
void* wrap_alloc(T&& func, void* ptr, size_t bytes) {
    ...
    if (mb && (tb + bytes > mb)) {
        auto scope = bongo::mozjs::MozJSImplScope::getThreadScope();
        if (scope)
            scope->setOOM();
...
#ifdef BONGO_NO_MALLOC_USABLE_SIZE
...
void js_free(void* p) {
    ...
    size_t current = bongo::sm::get_current(p);
    size_t tb = bongo::sm::get_total_bytes();

    if (tb >= current) {
        bongo::sm::total_bytes = tb - current;
    }
...
void* js_realloc(void* p, size_t bytes) {
    ...
    size_t tb = bongo::sm::total_bytes;
