...
#include "bongo/scripting/mozjs/bongohelpers.h"
...
namespace bongo {
namespace JSFiles {
extern const JSFile bongohelpers;
}  // namespace JSFiles

namespace mozjs {

const char* const BongoHelpersInfo::className = "BongoHelpers";

namespace {
const char kExportsObjectName[] = "exportToBongoHelpers";
...
std::string parseJSFunctionOrExpression(JSContext* cx, const StringData input) {
    ...
    ObjectWrapper helpersWrapper(cx, getScope(cx)->getProto<BongoHelpersInfo>().getProto());
...
void BongoHelpersInfo::postInstall(JSContext* cx, JS::HandleObject global, JS::HandleObject proto) {
    ObjectWrapper protoWrapper(cx, proto);
    ObjectWrapper globalWrapper(cx, global);

    // Initialize the reflection API and move it under the BongoHelpers object
    ...
    getScope(cx)->execSetup(JSFiles::bongohelpers);
