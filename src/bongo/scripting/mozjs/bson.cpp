...
const JSFunctionSpec BSONInfo::freeFunctions[3] = {
    BONGO_ATTACH_JS_FUNCTION(bsonWoCompare), BONGO_ATTACH_JS_FUNCTION(bsonBinaryEqual), JS_FS_END,
};
...
void BSONInfo::resolve(JSContext* cx, JS::HandleObject obj, JS::HandleId id, bool* resolvedp) {
    ...
    *resolvedp = false;
    ...
    if (holder->_obj.hasField(sname)) {
        ...
        if (!holder->_readOnly && (elem.type() == bongo::Object || elem.type() == bongo::Array)) {
