 ...
 * js/src/vm/PosixNSPR.cpp and modified to use the BongoDB threading
 * primitives.
 *
...
public:
    ...
    bongo::stdx::thread& thread() {
        return thread_;
    }
};

namespace {
BONGO_TRIVIALLY_CONSTRUCTIBLE_THREAD_LOCAL nspr::Thread* kCurrentThread;
}  // namespace
...
PRThread* PR_CreateThread(PRThreadType type,
    ...
    try {
        ...
        t->thread() = bongo::stdx::thread(&nspr::Thread::ThreadRoutine, t.get());
...
namespace {
BONGO_TRIVIALLY_CONSTRUCTIBLE_THREAD_LOCAL std::array<void*, MaxTLSKeyCount> gTLSArray;
...
class nspr::Lock {
    bongo::stdx::mutex mutex_;

public:
    Lock() {}
    bongo::stdx::mutex& mutex() {
        return mutex_;
    }
};
...
class nspr::CondVar {
    bongo::stdx::condition_variable cond_;
    nspr::Lock* lock_;

public:
    CondVar(nspr::Lock* lock) : lock_(lock) {}
    bongo::stdx::condition_variable& cond() {
        return cond_;
    }
    nspr::Lock* lock() {
        return lock_;
...
PRStatus PR_WaitCondVar(PRCondVar* cvar, uint32_t timeout) {
    if (timeout == PR_INTERVAL_NO_TIMEOUT) {
        try {
            bongo::stdx::unique_lock<bongo::stdx::mutex> lk(cvar->lock()->mutex(),
                                                            bongo::stdx::adopt_lock_t());
        ...
        try {
            ...
            cvar->cond().wait_for(lk, bongo::Microseconds(timeout).toSystemDuration());
            lk.release();
