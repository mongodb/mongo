...
const JSFunctionSpec BongoBase::methods[] = {
    BONGO_ATTACH_JS_CONSTRAINED_METHOD_NO_PROTO(auth, BongoExternalInfo),
    BONGO_ATTACH_JS_CONSTRAINED_METHOD_NO_PROTO(close, BongoExternalInfo),
    BONGO_ATTACH_JS_CONSTRAINED_METHOD_NO_PROTO(
        copyDatabaseWithSCRAM, BongoLocalInfo, BongoExternalInfo),
    BONGO_ATTACH_JS_CONSTRAINED_METHOD_NO_PROTO(cursorFromId, BongoLocalInfo, BongoExternalInfo),
    BONGO_ATTACH_JS_CONSTRAINED_METHOD_NO_PROTO(
        cursorHandleFromId, BongoLocalInfo, BongoExternalInfo),
    BONGO_ATTACH_JS_CONSTRAINED_METHOD_NO_PROTO(find, BongoLocalInfo, BongoExternalInfo),
    BONGO_ATTACH_JS_CONSTRAINED_METHOD_NO_PROTO(
        getClientRPCProtocols, BongoLocalInfo, BongoExternalInfo),
    BONGO_ATTACH_JS_CONSTRAINED_METHOD_NO_PROTO(
        getServerRPCProtocols, BongoLocalInfo, BongoExternalInfo),
    BONGO_ATTACH_JS_CONSTRAINED_METHOD_NO_PROTO(insert, BongoLocalInfo, BongoExternalInfo),
    BONGO_ATTACH_JS_CONSTRAINED_METHOD_NO_PROTO(
        isReplicaSetConnection, BongoLocalInfo, BongoExternalInfo),
    BONGO_ATTACH_JS_CONSTRAINED_METHOD_NO_PROTO(logout, BongoLocalInfo, BongoExternalInfo),
    BONGO_ATTACH_JS_CONSTRAINED_METHOD_NO_PROTO(remove, BongoLocalInfo, BongoExternalInfo),
    BONGO_ATTACH_JS_CONSTRAINED_METHOD_NO_PROTO(runCommand, BongoLocalInfo, BongoExternalInfo),
    BONGO_ATTACH_JS_CONSTRAINED_METHOD_NO_PROTO(
        runCommandWithMetadata, BongoLocalInfo, BongoExternalInfo),
    BONGO_ATTACH_JS_CONSTRAINED_METHOD_NO_PROTO(
        setClientRPCProtocols, BongoLocalInfo, BongoExternalInfo),
    BONGO_ATTACH_JS_CONSTRAINED_METHOD_NO_PROTO(update, BongoLocalInfo, BongoExternalInfo),
    BONGO_ATTACH_JS_CONSTRAINED_METHOD_NO_PROTO(
        getMinWireVersion, BongoLocalInfo, BongoExternalInfo),
    BONGO_ATTACH_JS_CONSTRAINED_METHOD_NO_PROTO(
        getMaxWireVersion, BongoLocalInfo, BongoExternalInfo),
    JS_FS_END,
};

const char* const BongoBase::className = "Bongo";

const JSFunctionSpec BongoExternalInfo::freeFunctions[4] = {
    BONGO_ATTACH_JS_FUNCTION(_forgetReplSet),
    BONGO_ATTACH_JS_FUNCTION(load),
    BONGO_ATTACH_JS_FUNCTION(quit),
    JS_FS_END,
};
...
DBClientBase* getConnection(JS::CallArgs& args) {
    auto ret =
        static_cast<std::shared_ptr<DBClientBase>*>(JS_GetPrivate(args.thisv().toObjectOrNull()))
            ->get();
    uassert(
        ErrorCodes::BadValue, "Trying to get connection for closed Bongo object", ret != nullptr);
    return ret;
}
...
void setHiddenBongo(JSContext* cx,
                    DBClientWithCommands* resPtr,
                    DBClientWithCommands* origConn,
                    JS::CallArgs& args) {
    ObjectWrapper o(cx, args.rval());
    // If the connection that ran the command is the same as conn, then we set a hidden "_bongo"
    // property on the returned object that is just "this" Bongo object.
    if (resPtr == origConn) {
        o.defineProperty(InternedString::_bongo, args.thisv(), JSPROP_READONLY | JSPROP_PERMANENT);
    } else {
        // Otherwise, we construct a new Bongo object that is a copy of "this", but has a different
        ...
        JS::RootedObject newBongo(cx);

        auto scope = getScope(cx);
        auto isLocalInfo = scope->getProto<BongoLocalInfo>().instanceOf(args.thisv());
        if (isLocalInfo) {
            scope->getProto<BongoLocalInfo>().newObject(&newBongo);
        } else {
            scope->getProto<BongoExternalInfo>().newObject(&newBongo);
        }
        JS_SetPrivate(
            newBongo,
        ...
        value.setObjectOrNull(newBongo);

        o.defineProperty(InternedString::_bongo, value, JSPROP_READONLY | JSPROP_PERMANENT);
    }
}
}  // namespace

void BongoBase::finalize(JSFreeOp* fop, JSObject* obj) {
...
void BongoBase::Functions::close::call(JSContext* cx, JS::CallArgs args) {
    getConnection(args);
...
void BongoBase::Functions::runCommand::call(JSContext* cx, JS::CallArgs args) {
    if (args.length() != 3)
    ...
    setHiddenBongo(cx, std::get<1>(resTuple), conn, args);
}

void BongoBase::Functions::runCommandWithMetadata::call(JSContext* cx, JS::CallArgs args) {
    if (args.length() != 4)
...
void BongoBase::Functions::find::call(JSContext* cx, JS::CallArgs args) {
    auto scope = getScope(cx);
...
void BongoBase::Functions::insert::call(JSContext* cx, JS::CallArgs args) {
    auto scope = getScope(cx);
...
void BongoBase::Functions::remove::call(JSContext* cx, JS::CallArgs args) {
...
void BongoBase::Functions::update::call(JSContext* cx, JS::CallArgs args) {
    if (args.length() < 3)
...
void BongoBase::Functions::auth::call(JSContext* cx, JS::CallArgs args) {
    auto conn = getConnection(args);
    if (!conn)
    ...
    switch (args.length()) {
        ...
        case 3:
            params =
                BSON(saslCommandMechanismFieldName << "BONGODB-CR" << saslCommandUserDBFieldName
            ...
            break;
        default:
            uasserted(ErrorCodes::BadValue, "bongoAuth takes 1 object or 3 string arguments");
    }
...
void BongoBase::Functions::logout::call(JSContext* cx, JS::CallArgs args) {
    if (args.length() != 1)
...
void BongoBase::Functions::cursorFromId::call(JSContext* cx, JS::CallArgs args) {
    auto scope = getScope(cx);
...
void BongoBase::Functions::cursorHandleFromId::call(JSContext* cx, JS::CallArgs args) {
    auto scope = getScope(cx);
...
void BongoBase::Functions::copyDatabaseWithSCRAM::call(JSContext* cx, JS::CallArgs args) {
    auto conn = getConnection(args);
...
void BongoBase::Functions::getClientRPCProtocols::call(JSContext* cx, JS::CallArgs args) {
    auto conn = getConnection(args);
...
void BongoBase::Functions::setClientRPCProtocols::call(JSContext* cx, JS::CallArgs args) {
    auto conn = getConnection(args);
...
void BongoBase::Functions::getServerRPCProtocols::call(JSContext* cx, JS::CallArgs args) {
    auto conn = getConnection(args);
...
void BongoBase::Functions::isReplicaSetConnection::call(JSContext* cx, JS::CallArgs args) {
    auto conn = getConnection(args);
...
void BongoLocalInfo::construct(JSContext* cx, JS::CallArgs args) {
    auto scope = getScope(cx);

    if (args.length() != 0)
        uasserted(ErrorCodes::BadValue, "local Bongo constructor takes no args");

    auto txn = scope->getOpContext();
    auto conn = DBDirectClientFactory::get(txn).create(txn);

    JS::RootedObject thisv(cx);
    scope->getProto<BongoLocalInfo>().newObject(&thisv);
...
void BongoExternalInfo::construct(JSContext* cx, JS::CallArgs args) {
    auto scope = getScope(cx);
    ...
    auto statusWithHost = BongoURI::parse(host);
    auto cs = uassertStatusOK(statusWithHost);

    std::string errmsg;
    std::unique_ptr<DBClientBase> conn(cs.connect("BongoDB Shell", errmsg));
    ...
    JS::RootedObject thisv(cx);
    scope->getProto<BongoExternalInfo>().newObject(&thisv);
    ObjectWrapper o(cx, thisv);
...
void BongoBase::Functions::getMinWireVersion::call(JSContext* cx, JS::CallArgs args) {
    auto conn = getConnection(args);

    args.rval().setInt32(conn->getMinWireVersion());
}

void BongoBase::Functions::getMaxWireVersion::call(JSContext* cx, JS::CallArgs args) {
    auto conn = getConnection(args);

    args.rval().setInt32(conn->getMaxWireVersion());
}

void BongoExternalInfo::Functions::load::call(JSContext* cx, JS::CallArgs args) {
    auto scope = getScope(cx);
...
void BongoExternalInfo::Functions::quit::call(JSContext* cx, JS::CallArgs args) {
    auto scope = getScope(cx);
...
void BongoExternalInfo::Functions::_forgetReplSet::call(JSContext* cx, JS::CallArgs args) {
    if (args.length() != 1) {
        uasserted(ErrorCodes::BadValue,
