...
#include "bongo/scripting/mozjs/valuewriter.h"

#include <js/Conversions.h>

#include "bongo/base/error_codes.h"
#include "bongo/platform/decimal128.h"
#include "bongo/scripting/mozjs/exception.h"
...
void ValueWriter::writeThis(BSONObjBuilder* b,
    ...
    if (_value.isString()) {
        ...
        uasserted(16662,
                  str::stream() << "unable to convert JavaScript property to bongo element " << sd);
    }
}
        ...
        if (jsclass) {
            ...
            if (scope->getProto<CodeInfo>().getJSClass() == jsclass) {
                if (o.hasOwnField(InternedString::scope)  // CodeWScope
                    &&
                    o.type(InternedString::scope) == bongo::Object) {
                    if (o.type(InternedString::code) != bongo::String) {
