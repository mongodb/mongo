...
bool closeToMaxMemory() {
    return bongo::sm::get_total_bytes() > (kInterruptGCThreshold * bongo::sm::get_max_bytes());
}
}  // namespace

BONGO_TRIVIALLY_CONSTRUCTIBLE_THREAD_LOCAL MozJSImplScope* kCurrentScope;
...
void MozJSImplScope::_gcCallback(JSRuntime* rt, JSGCStatus status, void* data) {
    ...
    log() << "MozJS GC " << (status == JSGC_BEGIN ? "prologue" : "epilogue") << " heap stats - "
          << " total: " << bongo::sm::get_total_bytes() << " limit: " << bongo::sm::get_max_bytes();
}
...
MozJSImplScope::MozRuntime::MozRuntime(const MozJSScriptEngine* engine) {
    ...
    bongo::sm::reset(mallocMemoryLimit);

    // If this runtime isn't running on an NSPR thread, then it is
    // running on a bongo thread. In that case, we need to insert a
...
#if defined(__powerpc64__) && defined(BONGO_CONFIG_DEBUG_BUILD)
    ...
    uassert(ErrorCodes::ExceededMemoryLimit,
            ...
            mallocMemoryLimit == 0 || bongo::sm::get_total_bytes() < mallocMemoryLimit);
}
...
MozJSImplScope::MozJSImplScope(MozJSScriptEngine* engine)
    : _engine(engine),
      ...
      _bongoExternalProto(_context),
      _bongoHelpersProto(_context),
      _bongoLocalProto(_context),
    ...
    _bongoHelpersProto.install(_global);
...
void MozJSImplScope::newFunction(StringData raw, JS::MutableHandleValue out) {
    ...
    std::string code = str::stream() << "____BongoToSM_newFunction_temp = " << raw;
...
void MozJSImplScope::localConnectForDbEval(OperationContext* txn, const char* dbName) {
    ...
    // install the Bongo function object and instantiate the 'db' global
    _bongoLocalProto.install(_global);
    execCoreFiles();

    const char* const makeBongo = "_bongo = new Bongo()";
    exec(makeBongo, "local connect 2", false, true, true, 0);

    std::string makeDB = str::stream() << "db = _bongo.getDB(\"" << dbName << "\");";
...
void MozJSImplScope::externalSetup() {
    ...
    // install the Bongo function object
    _bongoExternalProto.install(_global);
    execCoreFiles();
