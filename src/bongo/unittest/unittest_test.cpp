...
#define ASSERT_TEST_FAILS(TEST_STMT) \
    ASSERT_THROWS(TEST_STMT, bongo::unittest::TestAssertionFailureException)

#define ASSERT_TEST_FAILS_MATCH(TEST_STMT, PATTERN)                                        \
    ASSERT_THROWS_PRED(                                                                    \
        TEST_STMT,                                                                         \
        bongo::unittest::TestAssertionFailureException,                                    \
        stdx::bind(containsPattern,                                                        \
                   PATTERN,                                                                \
                   stdx::bind(&bongo::unittest::TestAssertionFailureException::getMessage, \
...
TEST(UnitTestSelfTest, BSONElementEQ) {
    bongo::BSONObj obj1 = BSON("foo"
                               << "bar");
    bongo::BSONObj obj2 = BSON("foo"
                               << "bar");
...
class DeathTestSelfTestFixture : public ::bongo::unittest::Test {
public:
    void setUp() override {}
    void tearDown() override {
        bongo::unittest::log() << "Died in tear-down";
        invariant(false);
    }
};
