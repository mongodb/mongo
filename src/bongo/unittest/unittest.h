 ...
 *
 * For examples of basic usage, see bongo/unittest/unittest_test.cpp.
 */
...
#include "bongo/unittest/bson_test_util.h"
#include "bongo/unittest/unittest_helpers.h"
 ...
 */
#define FAIL(MESSAGE) ::bongo::unittest::TestAssertionFailure(__FILE__, __LINE__, MESSAGE).stream()
 ...
 */
#define ASSERT_OK(EXPRESSION) ASSERT_EQUALS(::bongo::Status::OK(), (EXPRESSION))

/**
 * Asserts that a status code is anything but OK.
 */
#define ASSERT_NOT_OK(EXPRESSION) ASSERT_NOT_EQUALS(::bongo::Status::OK(), (EXPRESSION))
...
#define _ASSERT_COMPARISON(COMPARISON, a, b)                                                       \
    if (::bongo::unittest::ComparisonAssertion_##COMPARISON ca =                                   \
            ::bongo::unittest::ComparisonAssertion_##COMPARISON(__FILE__, __LINE__, #a, #b, a, b)) \
    ca.failure().stream()
...
#define ASSERT_THROWS(STATEMENT, EXCEPTION_TYPE) \
    ASSERT_THROWS_PRED(                          \
        STATEMENT, EXCEPTION_TYPE, ::bongo::stdx::bind(::bongo::unittest::alwaysTrue))
...
#define ASSERT_THROWS_WHAT(STATEMENT, EXCEPTION_TYPE, EXPECTED_WHAT)               \
    ASSERT_THROWS_PRED(STATEMENT, EXCEPTION_TYPE, ([&](const EXCEPTION_TYPE& ex) { \
                           return ::bongo::StringData(ex.what()) ==                \
                               ::bongo::StringData(EXPECTED_WHAT);                 \
                       }))
    ...
    do {                                                                                        \
        ...
        if (myString.find(myContains) == std::string::npos) {                                   \
            ...
            err << "Expected to find " #CONTAINS " (" << myContains << ") in " #BIG_STRING " (" \
                << myString << ")";                                                             \
            ::bongo::unittest::TestAssertionFailure(__FILE__, __LINE__, err).stream();          \
        }                                                                                       \
    } while (false)
...
#define TEST(CASE_NAME, TEST_NAME)                                                          \
    class _TEST_TYPE_NAME(CASE_NAME, TEST_NAME) : public ::bongo::unittest::Test {          \
    private:                                                                                \
        virtual void _doTest();                                                             \
                                                                                            \
        static const RegistrationAgent<_TEST_TYPE_NAME(CASE_NAME, TEST_NAME)> _agent;       \
    };                                                                                      \
    const ::bongo::unittest::Test::RegistrationAgent<_TEST_TYPE_NAME(CASE_NAME, TEST_NAME)> \
 ...
 * Usage:
 *
 * class FixtureClass : public bongo::unittest::Test {
 * protected:
 *   int myVar;
 *   void setUp() { myVar = 10; }
 * };
...
#define TEST_F(FIXTURE_NAME, TEST_NAME)                                                        \
    ...
    const ::bongo::unittest::Test::RegistrationAgent<_TEST_TYPE_NAME(FIXTURE_NAME, TEST_NAME)> \
 ...
 */
bongo::logger::LogstreamBuilder log();
 ...
 */
class TestHolder {
    BONGO_DISALLOW_COPYING(TestHolder);
...
protected:
    ...
    class RegistrationAgent {
        BONGO_DISALLOW_COPYING(RegistrationAgent);
...
#include "bongo/unittest/unittest-inl.h"
