...
void logCommonStartupWarnings(const ServerGlobalParams& serverParams) {
    ...
    bool warned = false;
    {
        ...
        if ((vii.minorVersion() % 2) != 0) {
            ...
            log() << "** NOTE: This is a development version (" << vii.version() << ") of BongoDB."
                  << startupWarningsLog;
    ...
    if (is32bit) {
        log() << startupWarningsLog;
        log() << "** WARNING: This 32-bit BongoDB binary is deprecated" << startupWarningsLog;
        warned = true;
    }
    ...
    */
    ...
    if (retWow64 && wow64Process) {
        log() << "** NOTE: This is a 32-bit BongoDB binary running on a 64-bit operating"
              << startupWarningsLog;
        log() << "**      system. Switch to a 64-bit build of BongoDB to" << startupWarningsLog;
