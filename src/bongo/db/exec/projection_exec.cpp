...
#include "bongo/db/exec/projection_exec.h"

#include "bongo/bson/mutable/document.h"
#include "bongo/db/exec/working_set_computed_data.h"
#include "bongo/db/matcher/expression.h"
#include "bongo/db/matcher/expression_parser.h"
#include "bongo/db/ops/path_support.h"
#include "bongo/db/query/collation/collator_interface.h"
...
ProjectionExec::ProjectionExec(const BSONObj& spec,
    ...
    while (it.more()) {
        ...
        if (Object == e.type()) {
            ...
            if (bongoutils::str::equals(e2.fieldName(), "$slice")) {
                if (e2.isNumber()) {
                    int i = e2.numberInt();
                    if (i < 0) {
            ...
            } else if (bongoutils::str::equals(e2.fieldName(), "$elemMatch")) {
                ...
                _matchers[bongoutils::str::before(e.fieldName(), '.').c_str()] =
                    statusWithMatcher.getValue().release();

                add(e.fieldName(), true);
            } else if (bongoutils::str::equals(e2.fieldName(), "$meta")) {
        ...
        } else if (bongoutils::str::equals(e.fieldName(), "_id") && !e.trueValue()) {
            _includeID = false;
        } else {
        ...
        if (bongoutils::str::contains(e.fieldName(), ".$")) {
...
Status ProjectionExec::transform(WorkingSetMember* member) const {
    ...
    if (member->hasObj()) {
        ...
        for (auto&& specElt : _source) {
            if (bongoutils::str::equals("_id", specElt.fieldName())) {
                continue;
            }
...
Status ProjectionExec::transform(const BSONObj& in,
    ...
    while (it.more()) {
        ...
        // Case 1: _id
        if (bongoutils::str::equals("_id", elt.fieldName())) {
            if (_includeID) {
                bob->append(elt);
            }
            continue;
