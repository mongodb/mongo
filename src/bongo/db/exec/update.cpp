...
Status storageValid(const mb::ConstElement& elem, const bool deep) {
    ...
    const mb::ConstElement& parent = elem.parent();
    const bool childOfArray = parent.ok() ? (parent.getType() == bongo::Array) : false;
            ...
            if (original.isEmpty()) {
                ...
                if (!(current.dottedField() == idFieldName))
                    return Status(ErrorCodes::NoSuchKey,
                                  bongoutils::str::stream() << "After applying the update, the new"
...
void UpdateStage::doInsert() {
    ...
    BONGO_WRITE_CONFLICT_RETRY_LOOP_END(getOpCtx(), "upsert", _collection->ns().ns());
}
