...
BONGO_FP_DECLARE(maxTimeAlwaysTimeOut);
...
BONGO_FP_DECLARE(maxTimeNeverTimeOut);
...
BONGO_FP_DECLARE(checkForInterruptFail);
...
bool OperationContext::hasDeadlineExpired() const {
    ...
    if (BONGO_FAIL_POINT(maxTimeNeverTimeOut)) {
        return false;
    }
    if (BONGO_FAIL_POINT(maxTimeAlwaysTimeOut)) {
        return true;
    }

    // TODO: Remove once all OperationContexts are properly connected to Clients and ServiceContexts
    // in tests.
    if (BONGO_unlikely(!getClient() || !getServiceContext())) {
        return false;
    }
...
Status OperationContext::checkForInterruptNoAssert() {
    // TODO: Remove the BONGO_likely(getClient()) once all operation contexts are constructed with
    // clients.
    if (BONGO_likely(getClient() && getServiceContext()) &&
    ...
    BONGO_FAIL_POINT_BLOCK(checkForInterruptFail, scopedFailPoint) {
