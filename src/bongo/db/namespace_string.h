...
/* in the bongo source code, "client" means "database". */
...
public:
    ...
    /**
     * Returns true for DBs with special meaning to bongodb.
     */
...
#include "bongo/db/namespace_string-inl.h"

BONGO_HASH_NAMESPACE_START
template <>
struct hash<bongo::NamespaceString> {
    size_t operator()(const bongo::NamespaceString& nss) const {
        bongo::NamespaceString::Hasher hasher;
        return hasher(nss);
    }
};
