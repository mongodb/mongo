    ...
    if (helpStr != "no help defined") {
        ...
        while (*p) {
            ...
            if (strncmp(p, "http:", 5) == 0) {
                ...
                ss << "\">";
                q = p;
                if (str::startsWith(q, "http://www.bongodb.org/display/"))
                    q += 31;
                while (*q && *q != ' ' && *q != '\n') {
                    ss << (*q == '+' ? ' ' : *q);
                    q++;
...
public:
    ...
    virtual void handle(OperationContext* txn,
        ...
        ss << p(
            "<b>BongoDB List of "
            "<a href=\"http://dochub.bongodb.org/core/commands\">Commands</a>"
            "</b>\n");
...
BONGO_INITIALIZER(WebStatusLogPlugin)(InitializerContext*) {
