...
#if defined(BONGO_CONFIG_HAVE_HEADER_UNISTD_H)
#include <unistd.h>
#endif
#ifndef _WIN32
#include <sys/types.h>
...
#include "bongo/db/server_options_helpers.h"
...
TEST(Verbosity, Default) {
    ...
    ::bongo::logger::globalLogDomain()->setMinimumLoggedSeverity(
        ::bongo::logger::LogSeverity::Info());

    ASSERT_OK(::bongo::addGeneralServerOptions(&options));
    ...
    ASSERT_OK(::bongo::validateServerOptions(environment));
    ASSERT_OK(::bongo::canonicalizeServerOptions(&environment));
    ASSERT_OK(::bongo::setupServerOptions(argv));
    ASSERT_OK(::bongo::storeServerOptions(environment));

    // Make sure the log level didn't change since we didn't specify any verbose options
    ASSERT_EQUALS(::bongo::logger::globalLogDomain()->getMinimumLogSeverity(),
                  ::bongo::logger::LogSeverity::Info());
}
...
TEST(Verbosity, CommandLineImplicit) {
    ...
    int verbosity = 1;
    ASSERT_EQUALS(::bongo::logger::globalLogDomain()->getMinimumLogSeverity(),
                  ::bongo::logger::LogSeverity::Debug(verbosity));
}
...
TEST(Verbosity, CommandLineBadString) {
    ...
    ASSERT_NOT_OK(::bongo::validateServerOptions(environment));
}
...
TEST(Verbosity, YAMLConfigStringLogComponent) {
    ...
    ::bongo::logger::globalLogDomain()->setMinimumLoggedSeverity(
        ::bongo::logger::LogComponent::kStorage, ::bongo::logger::LogSeverity::Debug(1));
    ...
    ASSERT_EQUALS(::bongo::logger::globalLogDomain()->getMinimumLogSeverity(),
                  ::bongo::logger::LogSeverity::Debug(verbosity));
    ASSERT_EQUALS(::bongo::logger::globalLogDomain()->getMinimumLogSeverity(
                      ::bongo::logger::LogComponent::kDefault),
                  ::bongo::logger::LogSeverity::Debug(verbosity));

    // AccessControl
    ASSERT_TRUE(::bongo::logger::globalLogDomain()->hasMinimumLogSeverity(
        ::bongo::logger::LogComponent::kAccessControl));
    ASSERT_EQUALS(::bongo::logger::globalLogDomain()->getMinimumLogSeverity(
                      ::bongo::logger::LogComponent::kAccessControl),
                  ::bongo::logger::LogSeverity::Log());

    // Query - not mentioned in configuration. should match default.
    ASSERT_FALSE(::bongo::logger::globalLogDomain()->hasMinimumLogSeverity(
        ::bongo::logger::LogComponent::kStorage));
    ASSERT_EQUALS(::bongo::logger::globalLogDomain()->getMinimumLogSeverity(
                      ::bongo::logger::LogComponent::kStorage),
                  ::bongo::logger::LogSeverity::Debug(verbosity));
    ...
    ASSERT_TRUE(::bongo::logger::globalLogDomain()->hasMinimumLogSeverity(
        ::bongo::logger::LogComponent::kJournal));
    ASSERT_EQUALS(::bongo::logger::globalLogDomain()->getMinimumLogSeverity(
                      ::bongo::logger::LogComponent::kJournal),
                  ::bongo::logger::LogSeverity::Debug(2));
}
...
TEST(SetupOptions, Default) {
    ...
    ASSERT_EQUALS(::bongo::serverGlobalParams.binaryName, "binaryname");
    ASSERT_FALSE(::bongo::serverGlobalParams.cwd.empty());
    ASSERT_EQUALS(::bongo::serverGlobalParams.argvArray.nFields(), static_cast<int>(argv.size()));
}
...
TEST(SetupOptions, EmptyBinaryName) {
    ...
    ASSERT_EQUALS(::bongo::serverGlobalParams.binaryName, "");
    ASSERT_FALSE(::bongo::serverGlobalParams.cwd.empty());
    ASSERT_EQUALS(::bongo::serverGlobalParams.argvArray.nFields(), static_cast<int>(argv.size()));
}
...
TEST(SetupOptions, MissingBinaryName) {
    ...
    ASSERT_NOT_OK(::bongo::setupServerOptions(argv));
}
...
TEST(SetupOptions, DeepCwd) {
    ...
    // Run the actual test.
    Status res = ::bongo::setupServerOptions(argv);

    ASSERT_OK(res);
    ASSERT_FALSE(::bongo::serverGlobalParams.cwd.empty());
    ASSERT_EQUALS(::bongo::serverGlobalParams.cwd, deepCwd.native())
        << "serverGlobalParams.cwd is " << ::bongo::serverGlobalParams.cwd.size()
...
TEST(SetupOptions, UnlinkedCwd) {
    ...
    pid_t pid = fork();
    ASSERT_NOT_EQUALS(pid, -1) << "unable to fork: " << ::bongo::errnoWithDescription();
    if (pid == 0) {
        // Subprocess
