...
BongodGlobalParams bongodGlobalParams;
...
Status addBongodOptions(moe::OptionSection* options) {
    ...
    sharding_options
        .addOptionChaining(
            ...
            "",
            moe::String,
            "Choose what role this bongod has in a sharded cluster.  Possible values are:\n"
    ...
    sharding_options
        .addOptionChaining(
            ...
            "a BongoDB Extended JSON document in string format")
        .setSources(moe::SourceYAMLConfig)
        .incompatibleWith("configsvr")
    ...
    // If it is 'dbpath', bongod prints the dbpath and exits.  Any extra values are ignored.
    // If it is 'run', bongod runs normally.  Providing extra values is an error.
...
Status canonicalizeBongodOptions(moe::Environment* params) {
    ...
    // httpinterface and nohttpinterface are shared between bongos and bongod, but bongod has
...
Status storeBongodOptions(const moe::Environment& params) {
    ...
    if (params.count("security.javascriptEnabled")) {
        bongodGlobalParams.scriptingEnabled = params["security.javascriptEnabled"].as<bool>();
    }
    ...
    if (params.count("pairwith") || params.count("arbiter") || params.count("opIdMem")) {
        return Status(ErrorCodes::BadValue,
                      ...
                      "<http://dochub.bongodb.org/core/replicapairs>\n"
                      "****");
    }
