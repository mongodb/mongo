...
#include "bongo/db/repl/rollback_source_impl.h"
...
BONGO_FP_DECLARE(rollbackHangBeforeStart);
...
void BackgroundSync::_produce(OperationContext* txn) {
    if (BONGO_FAIL_POINT(stopReplProducer)) {
        ...
        //        while (BONGO_FAIL_POINT(stopReplProducer) && !inShutdown()) {
        //            bongo::sleepsecs(1);
        //        }
        bongo::sleepsecs(1);
        return;
    ...
    } catch (const bongo::DBException& ex) {
...
void BackgroundSync::_rollback(OperationContext* txn,
    ...
    if (BONGO_FAIL_POINT(rollbackHangBeforeStart)) {
        // This log output is used in js tests so please leave it.
        log() << "rollback - rollbackHangBeforeStart fail point "
                 "enabled. Blocking until fail point is disabled.";
        while (BONGO_FAIL_POINT(rollbackHangBeforeStart) && !inShutdown()) {
            bongo::sleepsecs(1);
        }
    }
...
OpTimeWithHash BackgroundSync::_readLastAppliedOpTimeWithHash(OperationContext* txn) {
    ...
    try {
        ...
        BONGO_WRITE_CONFLICT_RETRY_LOOP_END(txn, "readLastAppliedHash", rsOplogName);
    } catch (const DBException& ex) {
