...
#include "bongo/db/repl/database_cloner.h"

#include <algorithm>
#include <iterator>
#include <set>

#include "bongo/client/remote_command_retry_scheduler.h"
#include "bongo/db/catalog/collection_options.h"
#include "bongo/db/commands/list_collections_filter.h"
...
BONGO_EXPORT_SERVER_PARAMETER(numInitialSyncListCollectionsAttempts, int, 3);
...
void DatabaseCloner::_listCollectionsCallback(const StatusWith<Fetcher::QueryResponse>& result,
    ...
    for (auto&& info : _collectionInfos) {
        ...
        if (nameElement.type() != bongo::String) {
            _finishCallback_inlock(
                lk,
