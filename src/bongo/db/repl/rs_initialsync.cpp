...
BONGO_EXPORT_SERVER_PARAMETER(num3Dot2InitialSyncAttempts, int, 10);
...
void truncateAndResetOplog(OperationContext* txn,
    ...
    BONGO_WRITE_CONFLICT_RETRY_LOOP_END(txn, "truncate", collection->ns().ns());
}
...
bool _initialSyncApplyOplog(OperationContext* txn,
    ...
    if (BONGO_FAIL_POINT(failInitSyncWithBufferedEntriesLeft)) {
...
Status _initialSync(OperationContext* txn, BackgroundSync* bgsync) {
    ...
    if (BONGO_FAIL_POINT(initialSyncHangBeforeCopyingDatabases)) {
        ...
        while (BONGO_FAIL_POINT(initialSyncHangBeforeCopyingDatabases)) {
            bongo::sleepsecs(1);
        }
    }
...
Status checkAdminDatabase(OperationContext* txn, Database* adminDb) {
    ...
    if (!adminVersionCollection ||
        ...
        std::string msg = str::stream()
            ...
            << ", which is the minimum supported schema version in this version of BongoDB";
    ...
    if ((foundSchemaVersion != AuthorizationManager::schemaVersion26Final) &&
        ...
        std::string msg = str::stream()
            ...
            << ", but this version of BongoDB only supports schema versions "
