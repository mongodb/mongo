...
BONGO_EXPORT_SERVER_PARAMETER(numInitialSyncCollectionCountAttempts, int, 3);
// The number of attempts for the listIndexes commands.
BONGO_EXPORT_SERVER_PARAMETER(numInitialSyncListIndexesAttempts, int, 3);
// The number of attempts for the find command, which gets the data.
BONGO_EXPORT_SERVER_PARAMETER(numInitialSyncCollectionFindAttempts, int, 3);
}  // namespace
...
void CollectionCloner::_insertDocumentsCallback(
    ...
    BONGO_FAIL_POINT_BLOCK(initialSyncHangDuringCollectionClone, options) {
        ...
        if (data["namespace"].String() == _destNss.ns() &&
            ...
            while (BONGO_FAIL_POINT(initialSyncHangDuringCollectionClone) && !_isShuttingDown()) {
                bongo::sleepsecs(1);
            }
            lk.lock();
        }
