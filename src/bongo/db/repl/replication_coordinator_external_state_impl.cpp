...
#include "bongo/db/repl/isself.h"
#include "bongo/db/repl/last_vote.h"
#include "bongo/db/repl/master_slave.h"
#include "bongo/db/repl/member_state.h"
#include "bongo/db/repl/noop_writer.h"
#include "bongo/db/repl/oplog.h"
#include "bongo/db/repl/oplog_buffer_blocking_queue.h"
#include "bongo/db/repl/oplog_buffer_collection.h"
#include "bongo/db/repl/oplog_buffer_proxy.h"
#include "bongo/db/repl/repl_settings.h"
#include "bongo/db/repl/replication_coordinator_global.h"
#include "bongo/db/repl/rs_initialsync.h"
#include "bongo/db/repl/rs_sync.h"
#include "bongo/db/repl/snapshot_thread.h"
#include "bongo/db/repl/storage_interface.h"
...
#include "bongo/executor/network_interface.h"
#include "bongo/executor/network_interface_factory.h"
#include "bongo/executor/thread_pool_task_executor.h"
...
#include "bongo/stdx/functional.h"
...
#include "bongo/util/concurrency/thread_pool.h"
...
BONGO_EXPORT_STARTUP_SERVER_PARAMETER(enableReplSnapshotThread, bool, false);

BONGO_EXPORT_STARTUP_SERVER_PARAMETER(use3dot2InitialSync, bool, false);
...
BONGO_EXPORT_STARTUP_SERVER_PARAMETER(initialSyncOplogBuffer,
                                      std::string,
                                      kCollectionOplogBufferName);

// Set this to specify size of read ahead buffer in the OplogBufferCollection.
BONGO_EXPORT_STARTUP_SERVER_PARAMETER(initialSyncOplogBufferPeekCacheSize, int, 10000);
...
BONGO_INITIALIZER(initialSyncOplogBuffer)(InitializerContext*) {
...
Status ReplicationCoordinatorExternalStateImpl::initializeReplSetStorage(OperationContext* txn,
    ...
    try {
        ...
        BONGO_WRITE_CONFLICT_RETRY_LOOP_END(txn, "initiate oplog entry", "local.oplog.rs");
...
OpTime ReplicationCoordinatorExternalStateImpl::onTransitionToPrimary(OperationContext* txn,
    ...
    if (isV1ElectionProtocol) {
        ...
        BONGO_WRITE_CONFLICT_RETRY_LOOP_END(
...
StatusWith<BSONObj> ReplicationCoordinatorExternalStateImpl::loadLocalConfigDocument(
    ...
    try {
        ...
        BONGO_WRITE_CONFLICT_RETRY_LOOP_END(txn, "load replica set config", configCollectionName);
    } catch (const DBException& ex) {
...
Status ReplicationCoordinatorExternalStateImpl::storeLocalConfigDocument(OperationContext* txn,
    ...
    try {
        ...
        BONGO_WRITE_CONFLICT_RETRY_LOOP_END(txn, "save replica set config", configCollectionName);
    } catch (const DBException& ex) {
        return ex.toStatus();
    }
}
