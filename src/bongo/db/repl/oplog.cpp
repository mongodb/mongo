...
#include "bongo/db/catalog/apply_ops.h"
#include "bongo/db/catalog/capped_utils.h"
#include "bongo/db/catalog/coll_mod.h"
...
#include "bongo/db/repl/sync_tail.h"
...
#include "bongo/util/elapsed_tracker.h"
...
BONGO_FP_DECLARE(disableSnapshotting);
...
void createOplog(OperationContext* txn, const std::string& oplogCollectionName, bool isReplSet) {
        ...
        if (replSettings.getOplogSizeBytes() != 0) {
            ...
            if (n != o) {
                ...
                ss << "cmdline oplogsize (" << n << ") different than existing (" << o
                   << ") see: http://dochub.bongodb.org/core/increase-oplog";
                log() << ss.str() << endl;
    ...
    BONGO_WRITE_CONFLICT_RETRY_LOOP_END(txn, "createCollection", oplogCollectionName);
...
NamespaceString parseNs(const string& ns, const BSONObj& cmdObj) {
    ...
    uassert(40073,
            ...
            first.canonicalType() == canonicalizeBSONType(bongo::String));
    ...
    if (*opType == 'i') {
        if (nsToCollectionSubstring(ns) == "system.indexes") {
            ...
            if (!indexSpec["v"]) {
                ...
                // v=1 index from an older version of BongoDB. This is because
    ...
    while (!done) {
        ...
        if (op == opsMap.end()) {
            return Status(ErrorCodes::BadValue,
                          bongoutils::str::stream() << "Invalid key '" << o.firstElementFieldName()
                                                    << "' found in field 'o'");
        }
...
BONGO_EXPORT_STARTUP_SERVER_PARAMETER(replSnapshotThreadThrottleMicros, int, 1000);
...
void SnapshotThread::run() {
    ...
    while (true) {
        ...
        while (BONGO_FAIL_POINT(disableSnapshotting)) {
            sleepsecs(1);
            if (_inShutdown.load()) {
                return;
            }
