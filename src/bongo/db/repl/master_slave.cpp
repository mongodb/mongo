    ...
    if (!replSettings.getSource().empty()) {
        ...
        while (PlanExecutor::ADVANCED == (state = exec->getNext(&obj, NULL))) {
            ...
            if (tmp.hostName != replSettings.getSource()) {
                ...
                log() << "http://dochub.bongodb.org/core/masterslave" << endl;
                log() << "terminating bongod after 30 seconds" << endl;
                sleepsecs(30);
...
void ReplSource::_sync_pullOpLog_applyOperation(OperationContext* txn,
    ...
    // bongos will not send requests there. That's why the last argument is false (do not do
    // version checking).
