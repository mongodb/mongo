 ...
 */
namespace {
#if defined(BONGO_PLATFORM_64)
int replWriterThreadCount = 16;
#elif defined(BONGO_PLATFORM_32)
int replWriterThreadCount = 2;
#else
#error need to include something that defines BONGO_PLATFORM_XX
#endif
...
Status SyncTail::syncApply(OperationContext* txn,
    ...
    if (isCommand) {
        ...
        BONGO_WRITE_CONFLICT_RETRY_LOOP_END(txn, "syncApply_command", ns);
    }
    ...
    if (isNoOp || (opType[0] == 'i' && nsToCollectionSubstring(ns) == "system.indexes")) {
        ...
        BONGO_WRITE_CONFLICT_RETRY_LOOP_END(txn, opStr, ns);
    }
    ...
    if (isCrudOpType(opType)) {
        ...
        BONGO_WRITE_CONFLICT_RETRY_LOOP_END(txn, "syncApply_CRUD", ns);
    }
...
class SyncTail::OpQueueBatcher {
    BONGO_DISALLOW_COPYING(OpQueueBatcher);
...
void SyncTail::oplogApplication(ReplicationCoordinator* replCoord) {
    ...
    while (true) {  // Exits on message from OpQueueBatcher.
        ...
        while (BONGO_FAIL_POINT(rsSyncApplyStop)) {
...
BSONObj SyncTail::getMissingDoc(OperationContext* txn, Database* db, const BSONObj& o) {
    ...
    if (BONGO_FAIL_POINT(initialSyncHangBeforeGettingMissingDocument)) {
        log() << "initial sync - initialSyncHangBeforeGettingMissingDocument fail point enabled. "
                 "Blocking until fail point is disabled.";
        while (BONGO_FAIL_POINT(initialSyncHangBeforeGettingMissingDocument)) {
            bongo::sleepsecs(1);
        }
    }
...
bool SyncTail::shouldRetry(OperationContext* txn, const BSONObj& o) {
    ...
    BONGO_WRITE_CONFLICT_RETRY_LOOP_END(txn, "InsertRetry", nss.ns());
