...
#include "bongo/db/repl/rollback_checker.h"
...
BONGO_FP_FORWARD_DECLARE(failInitSyncWithBufferedEntriesLeft);

// Failpoint which causes the initial sync function to hang before copying databases.
BONGO_FP_FORWARD_DECLARE(initialSyncHangBeforeCopyingDatabases);

// Failpoint which causes the initial sync function to hang before calling shouldRetry on a failed
// operation.
BONGO_FP_FORWARD_DECLARE(initialSyncHangBeforeGettingMissingDocument);

// Failpoint which stops the applier.
BONGO_FP_FORWARD_DECLARE(rsSyncApplyStop);
...
class DataReplicator {
    BONGO_DISALLOW_COPYING(DataReplicator);
