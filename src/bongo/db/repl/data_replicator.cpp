...
#include "bongo/db/repl/initial_sync_state.h"
...
BONGO_EXPORT_SERVER_PARAMETER(numInitialSyncConnectAttempts, int, 10);

// The number of attempts to call find on the remote oplog.
BONGO_EXPORT_SERVER_PARAMETER(numInitialSyncOplogFindAttempts, int, 3);
...
void DataReplicator::_chooseSyncSourceCallback(
    ...
    if (BONGO_FAIL_POINT(failInitialSyncWithBadHost)) {
...
void DataReplicator::_lastOplogEntryFetcherCallbackForBeginTimestamp(
    ...
    auto listDatabasesFilter = [](BSONObj dbInfo) {
        std::string name;
        auto status = bongo::bsonExtractStringField(dbInfo, "name", &name);
        if (!status.isOK()) {
    ...
    if (BONGO_FAIL_POINT(initialSyncHangBeforeCopyingDatabases)) {
        lock.unlock();
        ...
        while (BONGO_FAIL_POINT(initialSyncHangBeforeCopyingDatabases) && !_isShuttingDown()) {
            bongo::sleepsecs(1);
        }
        lock.lock();
    }
...
void DataReplicator::_finishCallback(StatusWith<OpTimeWithHash> lastApplied) {
    ...
    if (BONGO_FAIL_POINT(initialSyncHangBeforeFinish)) {
        // This log output is used in js tests so please leave it.
        log() << "initial sync - initialSyncHangBeforeFinish fail point "
                 "enabled. Blocking until fail point is disabled.";
        while (BONGO_FAIL_POINT(initialSyncHangBeforeFinish) && !_isShuttingDown()) {
            bongo::sleepsecs(1);
        }
    }
...
StatusWith<Operations> DataReplicator::_getNextApplierBatch_inlock() {
    ...
    if (BONGO_FAIL_POINT(rsSyncApplyStop)) {
        return Operations();
    }
