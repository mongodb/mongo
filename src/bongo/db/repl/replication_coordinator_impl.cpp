...
#include "bongo/db/repl/data_replicator_external_state_initial_sync.h"
#include "bongo/db/repl/elect_cmd_runner.h"
#include "bongo/db/repl/freshness_checker.h"
#include "bongo/db/repl/freshness_scanner.h"
#include "bongo/db/repl/handshake_args.h"
#include "bongo/db/repl/is_master_response.h"
...
BONGO_FP_DECLARE(transitionToPrimaryHangBeforeTakingGlobalExclusiveLock);
...
BONGO_EXPORT_SERVER_PARAMETER(numInitialSyncAttempts, int, 10);
...
bool ReplicationCoordinatorImpl::_startLoadLocalConfig(OperationContext* txn) {
    ...
    if (!status.isOK()) {
        error() << "Locally stored replica set configuration does not parse; See "
                   "http://www.bongodb.org/dochub/core/recover-replica-set-from-invalid-config "
...
void ReplicationCoordinatorImpl::signalDrainComplete(OperationContext* txn,
    ...
    if (BONGO_FAIL_POINT(transitionToPrimaryHangBeforeTakingGlobalExclusiveLock)) {
        log() << "transition to primary - "
                 "transitionToPrimaryHangBeforeTakingGlobalExclusiveLock fail point enabled. "
                 "Blocking until fail point is disabled.";
        while (BONGO_FAIL_POINT(transitionToPrimaryHangBeforeTakingGlobalExclusiveLock)) {
            bongo::sleepsecs(1);
            {
                stdx::lock_guard<stdx::mutex> lock(_mutex);
                if (_inShutdown) {
