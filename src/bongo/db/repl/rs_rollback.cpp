...
BONGO_FP_DECLARE(rollbackHangBeforeFinish);
BONGO_FP_DECLARE(rollbackHangThenFailAfterWritingMinValid);
...
void checkRbidAndUpdateMinValid(OperationContext* txn,
    ...
    if (BONGO_FAIL_POINT(rollbackHangThenFailAfterWritingMinValid)) {
        // This log output is used in js tests so please leave it.
        log() << "rollback - rollbackHangThenFailAfterWritingMinValid fail point "
                 "enabled. Blocking until fail point is disabled.";
        while (BONGO_FAIL_POINT(rollbackHangThenFailAfterWritingMinValid)) {
...
Status _syncRollback(OperationContext* txn,
    ...
    if (BONGO_FAIL_POINT(rollbackHangBeforeFinish)) {
        // This log output is used in js tests so please leave it.
        log() << "rollback - rollbackHangBeforeFinish fail point "
                 "enabled. Blocking until fail point is disabled.";
        while (BONGO_FAIL_POINT(rollbackHangBeforeFinish)) {
            invariant(!globalInShutdownDeprecated());  // It is an error to shutdown while enabled.
            bongo::sleepsecs(1);
        }
    }
