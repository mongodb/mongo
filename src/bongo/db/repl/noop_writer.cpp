...
BONGO_EXPORT_SERVER_PARAMETER(writePeriodicNoops, bool, true);
...
class NoopWriter::PeriodicNoopRunner {
    BONGO_DISALLOW_COPYING(PeriodicNoopRunner);
...
void NoopWriter::_writeNoop(OperationContext* txn) {
    ...
    if (lastAppliedOpTime != _lastKnownOpTime) {
        ...
        if (writePeriodicNoops.load()) {
            ...
            BONGO_WRITE_CONFLICT_RETRY_LOOP_END(txn, "writeNoop", rsOplogName);
        }
    }
