...
#include "bongo/db/repl/heartbeat_response_action.h"
...
BONGO_EXPORT_STARTUP_SERVER_PARAMETER(priorityTakeoverFreshnessWindowSeconds, int, 2);
...
BONGO_FP_DECLARE(disableMaxSyncSourceLagSecs);
...
HeartbeatResponseAction TopologyCoordinatorImpl::processHeartbeatResponse(
    ...
    if (hbResponse.isOK() && hbResponse.getValue().hasConfig()) {
        ...
        if (newConfig.getConfigVersion() > currentConfigVersion) {
            ...
            if (logger::globalLogDomain()->shouldLog(BongoLogDefaultComponent_component,
                                                     ::bongo::LogstreamBuilder::severityCast(2))) {
...
bool TopologyCoordinatorImpl::shouldChangeSyncSource(
    ...
    if (BONGO_FAIL_POINT(disableMaxSyncSourceLagSecs)) {
