...
class SortIteratorInterface {
    BONGO_DISALLOW_COPYING(SortIteratorInterface);
...
class SortedFileWriter {
    BONGO_DISALLOW_COPYING(SortedFileWriter);
...
/**
 * #include "bongo/db/sorter/sorter.cpp" and call this in a single translation
 * unit once for each unique set of template parameters.
 */
#define BONGO_CREATE_SORTER(Key, Value, Comparator)                                      \
    /* public classes */                                                                 \
    template class ::bongo::Sorter<Key, Value>;                                          \
    template class ::bongo::SortIteratorInterface<Key, Value>;                           \
    template class ::bongo::SortedFileWriter<Key, Value>;                                \
    /* internal classes */                                                               \
    template class ::bongo::sorter::NoLimitSorter<Key, Value, Comparator>;               \
    template class ::bongo::sorter::LimitOneSorter<Key, Value, Comparator>;              \
    template class ::bongo::sorter::TopKSorter<Key, Value, Comparator>;                  \
    template class ::bongo::sorter::MergeIterator<Key, Value, Comparator>;               \
    template class ::bongo::sorter::InMemIterator<Key, Value>;                           \
    template class ::bongo::sorter::FileIterator<Key, Value>;                            \
    /* factory functions */                                                              \
    template ::bongo::SortIteratorInterface<Key, Value>* ::bongo::                       \
        SortIteratorInterface<Key, Value>::merge<Comparator>(                            \
            const std::vector<std::shared_ptr<SortIteratorInterface>>& iters,            \
            const SortOptions& opts,                                                     \
            const Comparator& comp);                                                     \
    template ::bongo::Sorter<Key, Value>* ::bongo::Sorter<Key, Value>::make<Comparator>( \
