...
void _assertIteratorsEquivalent(It1 it1, It2 it2, int line) {
    ...
    try {
        ...
        bongo::unittest::log() << "Failure from line " << line << " on iteration " << iteration
                               << std::endl;
        throw;
    }
}
...
public:
    ...
    SortOptions adjustSortOptions(SortOptions opts) {
        ...
        BONGO_STATIC_ASSERT((NUM_ITEMS * sizeof(IWPair)) / MEM_LIMIT > 50);
        BONGO_STATIC_ASSERT((NUM_ITEMS * sizeof(IWPair)) / MEM_LIMIT < 500);
...
class LotsOfDataWithLimit : public LotsOfDataLittleMemory<Random> {
    ...
    SortOptions adjustSortOptions(SortOptions opts) {
        ...
        BONGO_STATIC_ASSERT(MEM_LIMIT / 2 > (100 * sizeof(IWPair)));
        BONGO_STATIC_ASSERT(MEM_LIMIT < (5000 * sizeof(IWPair)));
        BONGO_STATIC_ASSERT(MEM_LIMIT * 2 > (5000 * sizeof(IWPair)));

        // Make sure we use a reasonable number of files when we spill
        BONGO_STATIC_ASSERT((Parent::NUM_ITEMS * sizeof(IWPair)) / MEM_LIMIT > 100);
        BONGO_STATIC_ASSERT((Parent::NUM_ITEMS * sizeof(IWPair)) / MEM_LIMIT < 500);
...
class SorterSuite : public bongo::unittest::Suite {
public:
...
bongo::unittest::SuiteInstance<SorterSuite> extSortTests;
}
