 ...
 * BONGO_CREATE_SORTER(MyKeyType, MyValueType, MyComparatorType);
 *
 * Do this once for each unique set of parameters to BONGO_CREATE_SORTER.
 */
...
#include "bongo/util/bufreader.h"
...
#if defined(BONGO_CONFIG_DEBUG_BUILD) && !defined(_MSC_VER)
...
SortedFileWriter<Key, Value>::SortedFileWriter(const SortOptions& opts, const Settings& settings)
    ...
    massert(
        16946, "Attempting to use external sort from bongos. This is not allowed.", !isBongos());
...
Sorter<Key, Value>* Sorter<Key, Value>::make(const SortOptions& opts,
    ...
    massert(16947,
            "Attempting to use external sort from bongos. This is not allowed.",
            !(isBongos() && opts.extSortAllowed));
