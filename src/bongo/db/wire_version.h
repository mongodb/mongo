 ...
 * mandatory.
 *
 * We use the wire version to determine if two agents (a driver, a bongos, or a bongod) can
...
enum WireVersion {
    ...
    // Support find and getMore commands, as well as OP_COMMAND in bongod (but not bongos).
    FIND_COMMAND = 4,
...
struct WireSpec {
    BONGO_DISALLOW_COPYING(WireSpec);
    ...
    // Set to true if the client is internal to the cluster---this is a bongod or bongos connecting
    // to another bongod.
