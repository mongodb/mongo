...
#include "bongo/db/auth/user_document_parser.h"
...
const std::string BONGODB_CR_CREDENTIAL_FIELD_NAME = "BONGODB-CR";
const std::string SCRAM_CREDENTIAL_FIELD_NAME = "SCRAM-SHA-1";
const std::string BONGODB_EXTERNAL_CREDENTIAL_FIELD_NAME = "external";
...
Status V2UserDocumentParser::checkValidUserDocument(const BSONObj& doc) const {
    ...
    if (!NamespaceString::validDBName(userDBStr, NamespaceString::DollarInDbNameBehavior::Allow) &&
        userDBStr != "$external") {
        return _badValue(bongoutils::str::stream() << "'" << userDBStr
    ...
    if (userDBStr == "$external") {
        BSONElement externalElement = credentialsObj[BONGODB_EXTERNAL_CREDENTIAL_FIELD_NAME];
        ...
        BSONElement bongoCRElement = credentialsObj[BONGODB_CR_CREDENTIAL_FIELD_NAME];

        if (!bongoCRElement.eoo()) {
            if (bongoCRElement.type() != String || bongoCRElement.valueStringData().empty()) {
                return _badValue(
                    "BONGODB-CR credential must to be a non-empty string"
                    ", if present",
                    0);
            }
    ...
    if (!credentialsElement.eoo()) {
        ...
        if (userDB == "$external") {
            BSONElement externalCredentialElement =
                credentialsElement.Obj()[BONGODB_EXTERNAL_CREDENTIAL_FIELD_NAME];
            ...
            BSONElement bongoCRCredentialElement =
                credentialsElement.Obj()[BONGODB_CR_CREDENTIAL_FIELD_NAME];

            if (scramElement.eoo() && bongoCRCredentialElement.eoo()) {
                return Status(ErrorCodes::UnsupportedFormat,
                              "User documents must provide credentials for SCRAM-SHA-1 "
                              "or BONGODB-CR authentication");
            }
            ...
            if (!bongoCRCredentialElement.eoo()) {
                if (bongoCRCredentialElement.type() != String ||
                    bongoCRCredentialElement.valueStringData().empty()) {
                    return Status(ErrorCodes::UnsupportedFormat,
                                  "BONGODB-CR credentials must be non-empty strings");
                } else {
                    credentials.password = bongoCRCredentialElement.String();
