...
SASLGlobalParams::SASLGlobalParams() {
    ...
    authenticationMechanisms.push_back("BONGODB-CR");
    authenticationMechanisms.push_back("BONGODB-X509");
...
Status addSASLOptions(moe::OptionSection* options) {
    ...
    saslOptions
        .addOptionChaining("security.authenticationMechanisms",
                           ...
                           "Default is BONGODB-CR, SCRAM-SHA-1 and BONGODB-X509.")
...
BONGO_MODULE_STARTUP_OPTIONS_REGISTER(SASLOptions)(InitializerContext* context) {
    return addSASLOptions(&moe::startupOptions);
}

BONGO_STARTUP_OPTIONS_STORE(SASLOptions)(InitializerContext* context) {
...
public:
    ...
    virtual Status validate(const int& newValue) {
        if (newValue < minimumScramIterationCount) {
            return Status(
                ErrorCodes::BadValue,
                bongoutils::str::stream() << "Invalid value for SCRAM iteration count: " << newValue
