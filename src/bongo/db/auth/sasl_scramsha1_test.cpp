...
#include "bongo/db/auth/native_sasl_authentication_session.h"
...
BSONObj generateBONGODBCRUserDocument(StringData username, StringData password) {
    auto database = "test"_sd;
...
class SCRAMSHA1Fixture : public bongo::unittest::Test {
protected:
    ...
    void setUp() {
        ...
        saslServerSession->start("test", "SCRAM-SHA-1", "bongodb", "MockServer.test", 1, false);
        ...
        saslClientSession->setParameter(NativeSaslClientSession::parameterServiceName, "bongodb");
...
TEST_F(SCRAMSHA1Fixture, testBONGODBCR) {
    authzManagerExternalState->insertPrivilegeDocument(
        txn.get(), generateBONGODBCRUserDocument("sajack", "sajack"), BSONObj());
