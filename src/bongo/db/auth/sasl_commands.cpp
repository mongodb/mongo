...
#include "bongo/db/auth/authz_manager_external_state_mock.h"
#include "bongo/db/auth/authz_session_external_state_mock.h"
#include "bongo/db/auth/bongo_authentication_session.h"
#include "bongo/db/auth/sasl_authentication_session.h"
#include "bongo/db/auth/sasl_options.h"
#include "bongo/db/client.h"
#include "bongo/db/commands.h"
#include "bongo/db/commands/authentication_commands.h"
#include "bongo/db/server_options.h"
#include "bongo/util/base64.h"
#include "bongo/util/log.h"
#include "bongo/util/bongoutils/str.h"
#include "bongo/util/sequence_util.h"
...
Status doSaslStart(const Client* client,
    ...
    if (!sequenceContains(saslGlobalParams.authenticationMechanisms, mechanism) &&
        ...
        return Status(ErrorCodes::BadValue,
                      bongoutils::str::stream() << "Unsupported mechanism " << mechanism);
    }
...
bool CmdSaslContinue::run(OperationContext* txn,
    ...
    // Authenticating the __system@local user to the admin database on bongos is required
...
BONGO_INITIALIZER_WITH_PREREQUISITES(PreSaslCommands, ("NativeSaslServerCore"))
(InitializerContext*) {
    if (!sequenceContains(saslGlobalParams.authenticationMechanisms, "BONGODB-CR"))
        CmdAuthenticate::disableAuthMechanism("BONGODB-CR");

    if (!sequenceContains(saslGlobalParams.authenticationMechanisms, "BONGODB-X509"))
        CmdAuthenticate::disableAuthMechanism("BONGODB-X509");

    // For backwards compatibility, in 3.0 we are letting BONGODB-CR imply general
    // challenge-response auth and hence SCRAM-SHA-1 is enabled by either specifying
    // SCRAM-SHA-1 or BONGODB-CR in the authenticationMechanism server parameter.
    if (!sequenceContains(saslGlobalParams.authenticationMechanisms, "SCRAM-SHA-1") &&
        sequenceContains(saslGlobalParams.authenticationMechanisms, "BONGODB-CR"))
