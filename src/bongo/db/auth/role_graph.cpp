...
Status RoleGraph::createRole(const RoleName& role) {
    if (roleExists(role)) {
        return Status(ErrorCodes::DuplicateKey,
                      bongoutils::str::stream() << "Role: " << role.getFullName()
                                                << " already exists",
                      0);
    }
...
Status RoleGraph::deleteRole(const RoleName& role) {
    ...
    if (isBuiltinRole(role)) {
        return Status(ErrorCodes::InvalidRoleModification,
                      bongoutils::str::stream() << "Cannot delete built-in role: "
                                                << role.getFullName(),
                      0);
    }
...
Status RoleGraph::addRoleToRole(const RoleName& recipient, const RoleName& role) {
    if (!roleExists(recipient)) {
        return Status(ErrorCodes::RoleNotFound,
                      bongoutils::str::stream() << "Role: " << recipient.getFullName()
                                                << " does not exist");
    }
    if (isBuiltinRole(recipient)) {
        return Status(ErrorCodes::InvalidRoleModification,
                      bongoutils::str::stream() << "Cannot grant roles to built-in role: "
                                                << role.getFullName());
    }
    if (!roleExists(role)) {
        return Status(ErrorCodes::RoleNotFound,
...
Status RoleGraph::removeRoleFromRole(const RoleName& recipient, const RoleName& role) {
    ...
    if (isBuiltinRole(recipient)) {
        return Status(ErrorCodes::InvalidRoleModification,
                      bongoutils::str::stream() << "Cannot remove roles from built-in role: "
                                                << role.getFullName(),
                      0);
    }
    if (!roleExists(role)) {
    ...
    if (itToRm != _roleToMembers[role].end()) {
        ...
        return Status(ErrorCodes::RolesNotRelated,
                      bongoutils::str::stream() << recipient.getFullName() << " is not a member"
                                                                              " of "
                                                << role.getFullName(),
                      0);
    }
...
Status RoleGraph::removeAllRolesFromRole(const RoleName& victim) {
    ...
    if (!roleExists(victim)) {
        return Status(ErrorCodes::RoleNotFound,
                      bongoutils::str::stream() << "Role: " << victim.getFullName()
                                                << " does not exist",
                      0);
    }
...
Status RoleGraph::addPrivilegeToRole(const RoleName& role, const Privilege& privilegeToAdd) {
    ...
    if (isBuiltinRole(role)) {
        return Status(ErrorCodes::InvalidRoleModification,
                      bongoutils::str::stream() << "Cannot grant privileges to built-in role: "
                                                << role.getFullName(),
                      0);
    }
...
Status RoleGraph::removePrivilegeFromRole(const RoleName& role,
    ...
    if (isBuiltinRole(role)) {
        return Status(ErrorCodes::InvalidRoleModification,
                      bongoutils::str::stream() << "Cannot remove privileges from built-in role: "
                                                << role.getFullName());
    }
...
Status RoleGraph::_recomputePrivilegeDataHelper(const RoleName& startingRole,
    ...
    while (inProgressRoles.size()) {
        ...
        if (!roleExists(currentRole)) {
            return Status(ErrorCodes::RoleNotFound,
                          bongoutils::str::stream() << "Role: " << currentRole.getFullName()
                                                    << " does not exist",
                          0);
        }
