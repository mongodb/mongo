...
#include "bongo/db/auth/sasl_scramsha1_server_conversation.h"
...
StatusWith<bool> SaslSCRAMSHA1ServerConversation::_firstStep(std::vector<string>& input,
    ...
    if (input.size() == 4) {
        ...
        if (!str::startsWith(input[1], "a=") || input[1].size() < 3) {
            return StatusWith<bool>(ErrorCodes::BadValue,
                                    bongoutils::str::stream() << "Incorrect SCRAM-SHA-1 authzid: "
                                                              << input[1]);
        }
    ...
    if (input.size() != 3) {
        ...
        return StatusWith<bool>(ErrorCodes::BadValue,
                                bongoutils::str::stream() << "Incorrect SCRAM-SHA-1 user name: "
                                                          << input[1]);
    } else if (!str::startsWith(input[2], "r=") || input[2].size() < 6) {
        return StatusWith<bool>(ErrorCodes::BadValue,
                                bongoutils::str::stream() << "Incorrect SCRAM-SHA-1 client nonce: "
                                                          << input[2]);
    }
    ...
    if (!authzId.empty() && _user != authzId) {
        return StatusWith<bool>(ErrorCodes::BadValue,
                                bongoutils::str::stream() << "SCRAM-SHA-1 user name " << _user
    ...
    // Generate SCRAM credentials on the fly for mixed BONGODB-CR/SCRAM mode.
...
StatusWith<bool> SaslSCRAMSHA1ServerConversation::_secondStep(const std::vector<string>& input,
    ...
    if (input.size() != 3) {
        ...
        return StatusWith<bool>(
            ErrorCodes::BadValue,
            bongoutils::str::stream() << "Incorrect SCRAM-SHA-1 channel binding: " << input[0]);
    } else if (!str::startsWith(input[1], "r=") || input[1].size() < 6) {
        return StatusWith<bool>(
            ErrorCodes::BadValue,
            bongoutils::str::stream() << "Incorrect SCRAM-SHA-1 client|server nonce: " << input[1]);
    } else if (!str::startsWith(input[2], "p=") || input[2].size() < 3) {
        return StatusWith<bool>(ErrorCodes::BadValue,
                                bongoutils::str::stream() << "Incorrect SCRAM-SHA-1 ClientProof: "
                                                          << input[2]);
    }
