...
/**
 * Standard method in bongodb for determining if "authenticatedUser" may act as "requestedUser."
 *
 * The standard rule in BongoDB is simple.  The authenticated user name must be the same as the
 * requested user name.
 */
...
StringData SaslAuthenticationSession::getAuthenticationDatabase() const {
    if (Command::testCommandsEnabled && _authenticationDatabase == "admin" &&
        ...
        // support the auth passthrough test framework on bongos (since you can't use the local
        // database on a bongos, so you can't auth as the internal user without this).
