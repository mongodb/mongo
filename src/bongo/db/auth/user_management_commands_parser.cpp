...
#include "bongo/db/auth/user_management_commands_parser.h"
    ...
    for (BSONObjIterator iter(cmdObj); iter.more(); iter.next()) {
        ...
        if (!validFieldNames.count(fieldName.toString())) {
            return Status(ErrorCodes::BadValue,
                          bongoutils::str::stream() << "\"" << fieldName << "\" is not "
                                                                            "a valid argument to "
                                                    << cmdName);
        }
    }
...
Status parseRolePossessionManipulationCommands(const BSONObj& cmdObj,
    ...
    if (!parsedRoleNames->size()) {
        return Status(ErrorCodes::BadValue,
                      bongoutils::str::stream() << cmdName << " command requires a non-empty "
                                                              "\"roles\" array");
    }
    return Status::OK();
}
...
Status parseCreateOrUpdateUserCommands(const BSONObj& cmdObj,
    ...
    if (cmdObj.hasField("pwd")) {
        ...
        if (digestPassword) {
            parsedArgs->hashedPassword = bongo::createPasswordDigest(userName, password);
        } else {
...
Status parseAuthSchemaUpgradeCommand(const BSONObj& cmdObj,
    ...
    if (steps < minUpgradeSteps || steps > maxUpgradeSteps) {
        return Status(ErrorCodes::BadValue,
                      bongoutils::str::stream() << "Legal values for \"maxSteps\" are at least "
                                                << minUpgradeSteps
                                                << " and no more than "
                                                << maxUpgradeSteps
                                                << "; found "
