...
BONGO_INITIALIZER_WITH_PREREQUISITES(SetupInternalSecurityUser, BONGO_NO_PREREQUISITES)
...
class AuthorizationManager::CacheGuard {
    BONGO_DISALLOW_COPYING(CacheGuard);
...
Status AuthorizationManager::getBSONForRole(RoleGraph* graph,
    ...
    if (!graph->roleExists(roleName)) {
        return Status(ErrorCodes::RoleNotFound,
                      bongoutils::str::stream() << roleName.getFullName()
                                                << "does not name an existing role");
    }
    std::string id = bongoutils::str::stream() << roleName.getDB() << "." << roleName.getRole();
...
Status AuthorizationManager::_initializeUserFromPrivilegeDocument(User* user,
    ...
    if (userName != user->getName().getUser()) {
        return Status(ErrorCodes::BadValue,
                      bongoutils::str::stream() << "User name from privilege document \""
                                                << userName
            ...
            case schemaVersion24:
                status = Status(ErrorCodes::AuthSchemaIncompatible,
                                bongoutils::str::stream()
                                    ...
                                    << schemaVersion24
                                    << " not supported after BongoDB version 2.6.");
                break;
        }
        if (status.isOK())
            break;
...
void AuthorizationManager::releaseUser(User* user) {
    ...
    if (user->getRefCount() == 0) {
        ...
        if (user->isValid()) {
            BONGO_COMPILER_VARIABLE_UNUSED bool erased = _userCache.erase(user->getName());
            dassert(erased);
        }
        delete user;
    }
