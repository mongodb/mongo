...
#include "bongo/db/auth/sasl_plain_server_conversation.h"
...
StatusWith<bool> SaslPLAINServerConversation::step(StringData inputData, std::string* outputData) {
    ...
    try {
        ...
        return Status(ErrorCodes::AuthenticationFailed,
                      bongoutils::str::stream() << "Incorrectly formatted PLAIN client message");
    }
    ...
    if (!creds.password.empty()) {
        // Handle schemaVersion26Final (BONGODB-CR/SCRAM mixed mode)
        if (authDigest != creds.password) {
            return StatusWith<bool>(ErrorCodes::AuthenticationFailed,
                                    bongoutils::str::stream() << "Incorrect user name or password");
        }
    } else {
