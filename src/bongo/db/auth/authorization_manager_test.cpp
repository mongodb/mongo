...
class AuthorizationManagerTest : public ::bongo::unittest::Test {
public:
...
TEST_F(AuthorizationManagerTest, testLocalX509Authorization) {
    ...
    setX509PeerInfo(
        session,
        SSLPeerInfo("CN=bongodb.com", {RoleName("read", "test"), RoleName("readWrite", "test")}));
    ...
    User* x509User;
    ASSERT_OK(
        authzManager->acquireUser(txn.get(), UserName("CN=bongodb.com", "$external"), &x509User));
...
TEST_F(AuthorizationManagerTest, testLocalX509AuthorizationInvalidUser) {
    ...
    setX509PeerInfo(
        session,
        SSLPeerInfo("CN=bongodb.com", {RoleName("read", "test"), RoleName("write", "test")}));
    ServiceContext::UniqueClient client = serviceContext.makeClient("testClient", session);
    ServiceContext::UniqueOperationContext txn = client->makeOperationContext();
    ...
    Status _getUserDocument(OperationContext* txn, const UserName& userName, BSONObj* userDoc) {
        ...
        if (status == ErrorCodes::NoMatchingDocument) {
            status = Status(ErrorCodes::UserNotFound,
                            bongoutils::str::stream() << "Could not find user "
...
class AuthorizationManagerWithExplicitUserPrivilegesTest : public ::bongo::unittest::Test {
public:
