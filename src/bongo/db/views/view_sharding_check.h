...
/**
 * When a read against a view is forwarded from bongoS, it is done so without any awareness as to
 * whether the underlying collection is sharded. If it is found that the underlying collection is
 * sharded(*) we return an error to bongos with the view definition requesting
...
public:
    ...
    /**
     * Appends the resolved view definition and CommandOnShardedViewNotSupportedOnBongod status to
     * 'result'.
     */
