...
/**
 * This file contains tests for bongo/db/geo/hash.cpp.
 */
...
static string makeRandomBitString(int length) {
    stringstream ss;
    bongo::PseudoRandom random(31337);
...
TEST(GeoHash, MakeTooLongHash) {
    ...
    ASSERT_THROWS(makeHash(a), bongo::UserException);
}
...
TEST(GeoHashConverter, GeoHashBox) {
    ...
    bongo::Point p(-7201198.6497758823, -0.1);
    bongo::GeoHash hash = converter.hash(p);
    bongo::Box box = converter.unhashToBoxCovering(hash);
    ASSERT(box.inside(p));
}
