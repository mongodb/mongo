...
// specification.
BONGO_FP_DECLARE(skipIndexCreateFieldNameValidation);
...
Status validateKeyPattern(const BSONObj& key, IndexDescriptor::IndexVersion indexVersion) {
    ...
    if (pluginName.size()) {
        if (!IndexNames::isKnownName(pluginName))
            return Status(
                code, bongoutils::str::stream() << "Unknown index plugin '" << pluginName << '\'');
    }
    ...
    while (it.more()) {
        ...
        if (bongoutils::str::equals(keyElement.fieldName(), "$**") &&
            keyElement.valuestrsafe() == IndexNames::TEXT)
            continue;

        if (bongoutils::str::equals(keyElement.fieldName(), "_fts") &&
            keyElement.valuestrsafe() != IndexNames::TEXT) {
...
Status validateIndexSpecFieldNames(const BSONObj& indexSpec) {
    if (BONGO_FAIL_POINT(skipIndexCreateFieldNameValidation)) {
        return Status::OK();
    }
