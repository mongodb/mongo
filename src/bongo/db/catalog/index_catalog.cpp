...
/**
*    Copyright (C) 2013-2014 BongoDB Inc.
*
...
#define BONGO_LOG_DEFAULT_COMPONENT ::bongo::logger::LogComponent::kIndex
...
#include "bongo/bson/simple_bsonelement_comparator.h"
...
#include "bongo/db/field_ref.h"
#include "bongo/db/index/index_access_method.h"
#include "bongo/db/index/index_descriptor.h"
#include "bongo/db/index_legacy.h"
...
#include "bongo/db/ops/delete.h"
#include "bongo/db/query/collation/collation_spec.h"
...
#include "bongo/util/represent_as.h"
...
Status IndexCatalog::_upgradeDatabaseMinorVersionIfNeeded(OperationContext* txn,
    ...
    while (auto record = cursor->next()) {
        ...
        const string errmsg = str::stream()
            ...
            << "http://dochub.bongodb.org/core/index-type-changes";
...
Status IndexCatalog::_isSpecOk(OperationContext* txn, const BSONObj& spec) const {
    ...
    if (!IndexDescriptor::isIndexVersionSupported(indexVersion)) {
        return Status(ErrorCodes::CannotCreateIndex,
                      str::stream() << "this version of bongod cannot build new indexes "
                                    << "of version number "
    ...
    if (storageEngineElement.type() != bongo::Object) {
        return Status(ErrorCodes::CannotCreateIndex,
