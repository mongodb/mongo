...
/**
*    Copyright (C) 2008-2014 BongoDB Inc.
*
...
#include "bongo/util/fail_point.h"
...
BONGO_FP_DECLARE(crashAfterStartingIndexBuild);
BONGO_FP_DECLARE(hangAfterStartingIndexBuild);
BONGO_FP_DECLARE(hangAfterStartingIndexBuildUnlocked);
...
StatusWith<std::vector<BSONObj>> MultiIndexBlock::init(const std::vector<BSONObj>& indexSpecs) {
    ...
    if (BONGO_FAIL_POINT(crashAfterStartingIndexBuild)) {
...
Status MultiIndexBlock::insertAllDocumentsInCollection(std::set<RecordId>* dupsOut) {
    ...
    if (BONGO_FAIL_POINT(hangAfterStartingIndexBuild)) {
        ...
        while (BONGO_FAIL_POINT(hangAfterStartingIndexBuild)) {
    ...
    if (BONGO_FAIL_POINT(hangAfterStartingIndexBuildUnlocked)) {
        // Unlock before hanging so replication recognizes we've completed.
        Locker::LockSnapshot lockInfo;
        _txn->lockState()->saveLockStateAndUnlock(&lockInfo);
        while (BONGO_FAIL_POINT(hangAfterStartingIndexBuildUnlocked)) {
