...
#include "bongo/db/stats/top.h"
#include "bongo/db/storage/recovery_unit.h"
...
Database::Database(OperationContext* txn, StringData name, DatabaseCatalogEntry* dbEntry)
    ...
    // caused by incorrect bongod versions or similar, they are found right away.
    _views.invalidate();
...
void dropAllDatabasesExceptLocal(OperationContext* txn) {
    ...
    for (vector<string>::iterator i = n.begin(); i != n.end(); i++) {
        if (*i != "local") {
            ...
            BONGO_WRITE_CONFLICT_RETRY_LOOP_END(txn, "dropAllDatabasesExceptLocal", *i);
        }
    }
}
...
void Database::dropDatabase(OperationContext* txn, Database* db) {
    ...
    BONGO_WRITE_CONFLICT_RETRY_LOOP_END(txn, "dropDatabase", name);
}
