...
#include "bongo/db/ops/modifier_push.h"
...
Status parseEachMode(ModifierPush::ModifierPushMode pushMode,
    ...
    *eachElem = modExpr.embeddedObject()[kEach];
    ...
    while (itMod.more()) {
        ...
        if (bongoutils::str::equals(modElem.fieldName(), kEach)) {
            if (seenEach) {
    ...
    if (doFullCopy) {
        ...
        if (_eachMode || _pushMode == PUSH_ALL) {
            ...
            while (itEach.more()) {
                ...
                const std::string positionalName = bongoutils::str::stream()
            ...
            const std::string positionalName = bongoutils::str::stream() << _fieldRef.dottedField()
                                                                         << "." << position++;
