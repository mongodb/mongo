...
BONGO_FP_DECLARE(failAllInserts);
BONGO_FP_DECLARE(failAllUpdates);
BONGO_FP_DECLARE(failAllRemoves);
...
void makeCollection(OperationContext* txn, const NamespaceString& ns) {
    ...
    BONGO_WRITE_CONFLICT_RETRY_LOOP_END(txn, "implicit collection creation", ns.ns());
}
...
static bool insertBatchAndHandleErrors(OperationContext* txn,
    ...
    auto acquireCollection = [&] {
        while (true) {
            ...
            if (BONGO_FAIL_POINT(failAllInserts)) {
    ...
    for (auto it = batch.begin(); it != batch.end(); ++it) {
        ...
        try {
            ...
            BONGO_WRITE_CONFLICT_RETRY_LOOP_END(txn, "insert", wholeOp.ns.ns());
        } catch (const DBException& ex) {
...
static WriteResult::SingleResult performSingleUpdateOp(OperationContext* txn,
    ...
    while (true) {
        txn->checkForInterrupt();
        if (BONGO_FAIL_POINT(failAllUpdates)) {
...
static WriteResult::SingleResult performSingleDeleteOp(OperationContext* txn,
    ...
    if (BONGO_FAIL_POINT(failAllRemoves)) {
