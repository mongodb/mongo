...
public:
    explicit Mod(BSONObj modObj)
        : _modObj(modObj),
          _mod(bongoutils::str::equals(modObj.firstElement().fieldName(), "$mul")
...
TEST(Upcasting, UpcastIntToLong) {
    ...
    // correctly.
    Document doc(BSON("a" << static_cast<int>(1)));
    ASSERT_EQUALS(bongo::NumberInt, doc.root()["a"].getType());
    ...
    ASSERT_FALSE(doc.isInPlaceModeEnabled());
    ASSERT_EQUALS(fromjson("{ a : 1 }"), doc);
    ASSERT_EQUALS(bongo::NumberLong, doc.root()["a"].getType());

    Document logDoc;
    LogBuilder logBuilder(logDoc.root());
    ASSERT_OK(incMod.log(&logBuilder));
    ASSERT_EQUALS(fromjson("{ $set : { a : 1 } }"), logDoc);
    ASSERT_EQUALS(bongo::NumberLong, logDoc.root()["$set"]["a"].getType());
}
...
TEST(Upcasting, UpcastIntToDouble) {
    ...
    ASSERT_EQUALS(bongo::NumberDouble, doc.root()["a"].getType());

    Document logDoc;
    LogBuilder logBuilder(logDoc.root());
    ASSERT_OK(incMod.log(&logBuilder));
    ASSERT_EQUALS(fromjson("{ $set : { a : 1.0 } }"), logDoc);
    ASSERT_EQUALS(bongo::NumberDouble, logDoc.root()["$set"]["a"].getType());
}
...
TEST(Upcasting, UpcastIntToDecimal) {
    ...
    ASSERT_EQUALS(bongo::NumberDecimal, doc.root()["a"].getType());

    Document logDoc;
    LogBuilder logBuilder(logDoc.root());
    ASSERT_OK(incMod.log(&logBuilder));
    ASSERT_EQUALS(fromjson("{ $set : { a : NumberDecimal(\"1.0\") }}"), logDoc);
    ASSERT_EQUALS(bongo::NumberDecimal, logDoc.root()["$set"]["a"].getType());
}
...
TEST(Upcasting, DecimalsStayDecimals) {
    ...
    // correctly.
    Document doc(BSON("a" << bongo::Decimal128("1.0")));
    ASSERT_EQUALS(bongo::NumberDecimal, doc.root()["a"].getType());
...
TEST(Multiplication, ApplyMissingElementInt) {
    ...
    ASSERT_EQUALS(bongo::NumberInt, doc.root().rightChild().getType());
}
...
TEST(Multiplication, ApplyMissingElementLongLong) {
    ...
    ASSERT_EQUALS(bongo::NumberLong, doc.root().rightChild().getType());
}
...
TEST(Multiplication, ApplyMissingElementDouble) {
    ...
    ASSERT_EQUALS(bongo::NumberDouble, doc.root().rightChild().getType());
}
