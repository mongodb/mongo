...
// static
StatusWith<std::string> StartupWarningsBongod::readTransparentHugePagesParameter(
...
void logBongodStartupWarnings(const StorageGlobalParams& storageParams,
    ...
    if (sizeof(int*) == 4) {
        ...
        log() << "** NOTE: This is a 32 bit BongoDB binary." << startupWarningsLog;
        ...
        log() << "**       See http://dochub.bongodb.org/core/32bit" << startupWarningsLog;
        warned = true;
    }
        ...
        if (f.is_open()) {
            ...
            if (f.fail()) {
                ...
                // issue the warning.
                    ...
                    log() << "**          We suggest launching bongod like this to avoid "
                          << "performance problems:" << startupWarningsLog;
                    log() << "**              numactl --interleave=all bongod [other options]"
                          << startupWarningsLog;
                    warned = true;
                }
            }
    ...
    StatusWith<std::string> transparentHugePagesEnabledResult =
        StartupWarningsBongod::readTransparentHugePagesParameter("enabled");
    ...
    StatusWith<std::string> transparentHugePagesDefragResult =
        StartupWarningsBongod::readTransparentHugePagesParameter("defrag");
