...
#include "bongo/util/net/listen.h"  // For DEFAULT_MAX_CONN
...
Status addGeneralServerOptions(moe::OptionSection* options) {
    ...
    options->addOptionChaining("systemLog.syslogFacility",
                               "syslogFacility",
                               moe::String,
                               "syslog facility used for bongodb syslog message");
...
Status addWindowsServerOptions(moe::OptionSection* options) {
    ...
    options->addOptionChaining("service", "service", moe::Switch, "start bongodb service")
        .hidden()
        .setSources(moe::SourceAllLegacy);
