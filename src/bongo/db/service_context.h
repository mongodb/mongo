...
class StorageFactoriesIterator {
    BONGO_DISALLOW_COPYING(StorageFactoriesIterator);
...
/**
 * Class representing the context of a service, such as a BongoD database service or
 * a BongoS routing service.
 *
...
class ServiceContext : public Decorable<ServiceContext> {
    BONGO_DISALLOW_COPYING(ServiceContext);
...
public:
    ...
    /**
     * Register a storage engine.  Called from a BONGO_INIT that depends on initializiation of
    ...
    /**
     * Shuts down storage engine cleanly and releases any locks on bongod.lock.
     */
