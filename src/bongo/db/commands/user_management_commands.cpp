    ...
    for (std::vector<RoleName>::const_iterator it = rolesToAdd.begin(); it != rolesToAdd.end();
        ...
        if (roleToAdd == role) {
            return Status(ErrorCodes::InvalidRoleModification,
                          bongoutils::str::stream() << "Cannot grant role " << role.getFullName()
                                                    << " to itself.");
        }
        ...
        if (sequenceContains(indirectRoles, role)) {
            return Status(
                ErrorCodes::InvalidRoleModification,
                bongoutils::str::stream() << "Granting " << roleToAdd.getFullName() << " to "
                                          << role.getFullName()
 ...
 * for the BongoDB 2.6 and 3.0 BongoDB-CR/SCRAM mixed auth mode.
...
/**
 * Returns Status::OK() if the current Auth schema version is at least the auth schema version
 * for BongoDB 2.6 during the upgrade process.
 * Returns an error otherwise.
 */
...
public:
    ...
    bool run(OperationContext* txn,
        ...
        if (!args.hasHashedPassword) {
            ...
            } else {  // Otherwise default to BONGODB-CR.
                credentialsBuilder.append("BONGODB-CR", args.hashedPassword);
            }
        }
...
public:
    ...
    virtual void help(stringstream& ss) const {
        ss << "Internal command used by bongorestore for updating user/role data" << endl;
    }
    ...
    static void auditCreateOrUpdateUser(const BSONObj& userObj, bool create) {
        ...
        if (create) {
            audit::logCreateUser(Client::getCurrent(),
                                 userName,
                                 userObj["credentials"].Obj().hasField("BONGODB-CR"),
    ...
    static void addUser(OperationContext* txn,
        ...
        if (update && usersToDrop->count(userName)) {
            ...
            if (!status.isOK()) {
                // Match the behavior of bongorestore to continue on failure
...
/**
 * Updates a single user document from BONGODB-CR to SCRAM credentials.
 *
...
void updateUserCredentials(OperationContext* txn,
    ...
    BSONElement bongoCRElement = credentialsObj["BONGODB-CR"];
    ...
    uassert(18744,
            bongoutils::str::stream()
                << "While preparing to upgrade user doc from "
                   "2.6/3.0 user data schema to the 3.0+ SCRAM only schema, found a user doc "
                   "missing BONGODB-CR credentials :"
                << userDoc.toString(),
            !bongoCRElement.eoo());

    std::string hashedPassword = bongoCRElement.String();
 ...
 *   1. Compute SCRAM credentials based on the BONGODB-CR hash
 *   2. Remove the BONGODB-CR hash
...
Status upgradeAuthSchema(OperationContext* txn, AuthorizationManager* authzManager, int maxSteps) {
    ...
    return Status(ErrorCodes::OperationIncomplete,
                  bongoutils::str::stream() << "Auth schema upgrade incomplete after " << maxSteps
                                            << " successful steps.");
}
