...
#include "bongo/logger/parse_log_component_settings.h"
...
public:
    ...
    virtual Status set(const BSONElement& newValueElement) {
        ...
        if (!newValueElement.coerce(&newValue) || newValue < 0)
            return Status(ErrorCodes::BadValue,
                          bongoutils::str::stream() << "Invalid value for logLevel: "
                                                    << newValueElement);
    ...
    virtual Status setFromString(const std::string& str) {
        ...
        if (newValue < 0)
            return Status(ErrorCodes::BadValue,
                          bongoutils::str::stream() << "Invalid value for logLevel: " << newValue);
        LogSeverity newSeverity =
            (newValue > 0) ? LogSeverity::Debug(newValue) : LogSeverity::Log();
        globalLogDomain()->setMinimumLoggedSeverity(newSeverity);
        return Status::OK();
...
class LogComponentVerbositySetting : public ServerParameter {
    BONGO_DISALLOW_COPYING(LogComponentVerbositySetting);
...
public:
    ...
    virtual Status setFromString(const std::string& str) {
        try {
            return _set(bongo::fromjson(str));
...
#ifndef BONGO_CONFIG_SSL
        return Status(ErrorCodes::IllegalOperation,
                      bongoutils::str::stream() << "Unable to set clusterAuthMode, "
...
public:
    ...
    virtual Status set(const BSONElement& newValueElement) override {
        if (newValueElement.type() != bongo::String)
            return {ErrorCodes::TypeMismatch,
                    bongoutils::str::stream() << "Value for parameter " << kName
