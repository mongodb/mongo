...
#include "bongo/db/range_preserver.h"
...
public:
    ...
    void help(stringstream& h) const {
        h << "http://dochub.bongodb.org/core/geo#GeospatialIndexing-geoNearCommand";
    }
    ...
    bool run(OperationContext* txn,
        ...
        while (PlanExecutor::ADVANCED == (state = exec->getNext(&currObj, NULL))) {
            ...
            while (resIt.more()) {
                ...
                if (!bongoutils::str::equals("$pt", elt.fieldName()) &&
                    !bongoutils::str::equals("$dis", elt.fieldName())) {
                    resBob.append(elt);
                }
            }
