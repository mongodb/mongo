...
class Finalizer {
    BONGO_DISALLOW_COPYING(Finalizer);
...
class Reducer {
    BONGO_DISALLOW_COPYING(Reducer);
 ...
 */
class JSFunction {
    BONGO_DISALLOW_COPYING(JSFunction);
...
public:
    ...
    // true when called from bongos to do phase-1 of M/R
