...
namespace {
BONGO_FP_DECLARE(rsStopGetMoreCmd);
}  // namespace
...
class GetMoreCmd : public Command {
    BONGO_DISALLOW_COPYING(GetMoreCmd);
...
public:
    ...
    bool runParsed(OperationContext* txn,
        ...
        if (request.nss.isOplog() && BONGO_FAIL_POINT(rsStopGetMoreCmd)) {
            return appendCommandStatus(
                result,
