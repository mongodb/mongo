...
void CmdAuthenticate::disableAuthMechanism(std::string authMechanism) {
    if (authMechanism == "BONGODB-CR") {
        _isCRAuthDisabled = true;
    }
    if (authMechanism == "BONGODB-X509") {
...
public:
    ...
    bool run(OperationContext* txn,
        ...
        AuthenticationSession::set(Client::getCurrent(),
                                   stdx::make_unique<BongoAuthenticationSession>(n));
        return true;
    }
...
bool CmdAuthenticate::run(OperationContext* txn,
    ...
    if (mechanism.empty()) {
        mechanism = "BONGODB-CR";
    }
    UserName user;
    auto& sslPeerInfo = SSLPeerInfo::forSession(txn->getClient()->session());
    if (mechanism == "BONGODB-X509" && !cmdObj.hasField("user")) {
...
Status CmdAuthenticate::_authenticateCR(OperationContext* txn,
    ...
    if (_isCRAuthDisabled) {
        // SERVER-8461, BONGODB-CR must be enabled for authenticating the internal user, so that
...

    {
        ...
        if (!session || session->getType() != AuthenticationSession::SESSION_TYPE_BONGO) {
            sleepmillis(30);
            return Status(ErrorCodes::ProtocolError, "No pending nonce");
        } else {
            nonce64 nonce = static_cast<BongoAuthenticationSession*>(session.get())->getNonce();
    ...
    if (pwd.empty()) {
        return Status(ErrorCodes::AuthenticationFailed,
                      "BONGODB-CR credentials missing in the user document");
    }
...
Status CmdAuthenticate::_authenticateX509(OperationContext* txn,
    ...
    if (!getSSLManager()) {
        return Status(ErrorCodes::ProtocolError,
                      "SSL support is required for the BONGODB-X509 mechanism.");
    }
...
public:
    ...
    bool run(OperationContext* txn,
        ...
        if (Command::testCommandsEnabled && dbname == "admin") {
            ...
            // support the auth passthrough test framework on bongos (since you can't use the
            // local database on a bongos, so you can't logout as the internal user
            // without this).
