...
#include "bongo/db/commands/copydb.h"
...
BONGO_EXPORT_SERVER_PARAMETER(skipCorruptDocumentsWhenCloning, bool, false);
...
struct Cloner::Fun {
    ...
    void operator()(DBClientCursorBatchIterator& i) {
        ...
        if (!collection) {
            ...
            BONGO_WRITE_CONFLICT_RETRY_LOOP_END(txn, "createCollection", to_collection.ns());
        }
        ...
        while (i.moreInCurrentBatch()) {
            ...
            BONGO_WRITE_CONFLICT_RETRY_LOOP_END(txn, "cloner insert", to_collection.ns());
            RARELY if (time(0) - saveLast > 60) {
                log() << numSeen << " objects cloned so far from collection " << from_collection;
                saveLast = time(0);
            }

            BONGO_FAIL_POINT_BLOCK(initialSyncHangDuringCollectionClone, options) {
                ...
                if (data["namespace"].String() == to_collection.ns() &&
                    ...
                    while (BONGO_FAIL_POINT(initialSyncHangDuringCollectionClone)) {
                        bongo::sleepsecs(1);
                    }
                }
            }
...
bool Cloner::copyCollection(OperationContext* txn,
    ...
    if (shouldCreateCollection) {
        ...
        BONGO_WRITE_CONFLICT_RETRY_LOOP_END(txn, "createCollection", ns);
    } else {
...
Status Cloner::createCollectionsForDb(
    ...
    for (auto&& params : createCollectionParams) {
        ...
        BONGO_WRITE_CONFLICT_RETRY_LOOP_END(txn, "createCollection", nss.ns());
    }
    return Status::OK();
}
