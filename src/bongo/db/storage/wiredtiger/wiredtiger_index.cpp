...
BONGO_FP_DECLARE(WTEmulateOutOfOrderNextIndexKey);
...
Status checkKeySize(const BSONObj& key) {
    if (key.objsize() >= TempKeyMaxSize) {
        string msg = bongoutils::str::stream()
...
WiredTigerIndex::WiredTigerIndex(OperationContext* ctx,
    ...
    if (!version.isOK()) {
        ...
        ss << versionStatus.reason() << " Index: {name: " << desc->indexName()
           << ", ns: " << desc->parentNS() << "} - version too new for this bongod."
           << " See http://dochub.bongodb.org/core/3.4-index-downgrade for detailed"
...
protected:
    ...
    void updatePosition(bool inNext = false) {
        ...
        if (isForwardNextCall) {
            ...
            if (BONGO_FAIL_POINT(WTEmulateOutOfOrderNextIndexKey)) {
...
extern "C" BONGO_COMPILER_API_EXPORT int index_collator_extension(WT_CONNECTION* conn,
                                                                  WT_CONFIG_ARG* cfg) {
    static WT_COLLATOR idx_static;

    idx_static.customize = index_collator_customize;
    return conn->add_collator(conn, "bongo_index", &idx_static, NULL);
}
