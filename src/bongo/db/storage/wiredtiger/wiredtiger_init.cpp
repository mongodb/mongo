...
#include "bongo/db/storage/wiredtiger/wiredtiger_global_options.h"
#include "bongo/db/storage/wiredtiger/wiredtiger_index.h"
#include "bongo/db/storage/wiredtiger/wiredtiger_kv_engine.h"
#include "bongo/db/storage/wiredtiger/wiredtiger_parameters.h"
#include "bongo/db/storage/wiredtiger/wiredtiger_record_store.h"
#include "bongo/db/storage/wiredtiger/wiredtiger_server_status.h"
#include "bongo/db/storage/wiredtiger/wiredtiger_util.h"
...
#define EXT4_SUPER_MAGIC 0xEF53
        {
            ...
            if (ret == 0 && fs_stats.f_type == EXT4_SUPER_MAGIC) {
                ...
                log() << "** WARNING: Using the XFS filesystem is strongly recommended with the "
                      ...
                      << startupWarningsLog;
                log() << "**          See "
                         "http://dochub.bongodb.org/core/prodnotes-filesystem"
                      << startupWarningsLog;
            }
        }
#endif
...
BONGO_INITIALIZER_WITH_PREREQUISITES(WiredTigerEngineInit, ("SetGlobalEnvironment"))
