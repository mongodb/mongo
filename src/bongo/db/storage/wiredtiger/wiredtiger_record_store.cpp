...
#include "bongo/db/storage/wiredtiger/wiredtiger_record_store_oplog_stones.h"
...
BONGO_STATIC_ASSERT(kCurrentRecordStoreVersion >= kMinimumRecordStoreVersion);
BONGO_STATIC_ASSERT(kCurrentRecordStoreVersion <= kMaximumRecordStoreVersion);
...
BONGO_FP_DECLARE(WTWriteConflictException);
BONGO_FP_DECLARE(WTPausePrimaryOplogDurabilityLoop);
...
void WiredTigerRecordStore::_oplogJournalThreadLoop(WiredTigerSessionCache* sessionCache) try {
    ...
    while (true) {
        ...
        while (!_shuttingDown && BONGO_FAIL_POINT(WTPausePrimaryOplogDurabilityLoop)) {
            lk.unlock();
            sleepmillis(10);
            lk.lock();
        }
