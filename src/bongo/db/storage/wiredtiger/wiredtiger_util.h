...
/**
 * converts wiredtiger return codes to bongodb statuses.
 */
inline Status wtRCToStatus(int retCode, const char* prefix = NULL) {
    if (BONGO_likely(retCode == 0))
        return Status::OK();
...
#define invariantWTOK(expression)                                                       \
    do {                                                                                \
        ...
        if (BONGO_unlikely(_invariantWTOK_retCode != 0)) {                              \
            invariantOKFailed(                                                          \
...
class WiredTigerUtil {
    BONGO_DISALLOW_COPYING(WiredTigerUtil);
...
public:
    ...
    /**
     * Returns a WT_EVENT_HANDER with BongoDB's default handlers.
...
class WiredTigerConfigParser {
    BONGO_DISALLOW_COPYING(WiredTigerConfigParser);
