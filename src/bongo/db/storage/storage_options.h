 ...
 * persistence.  Many executables that can access data files directly such as bongod and certain
...
struct StorageGlobalParams {
    // Default data directory for bongod when running in non-config server mode.
    static const char* kDefaultDbPath;

    // Default data directory for bongod when running as the config database of
    // a sharded cluster.
    static const char* kDefaultConfigDbPath;

    // --storageEngine
    // storage engine for this instance of bongod.
    ...
    // --repairpath
    // Specifies the root directory containing BongoDB data files to use for the --repair
    // operation.
    ...
    // --syncdelay
    // Controls how much time can pass before BongoDB flushes data to the data files
