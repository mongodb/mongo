...
class KeyStringTest : public bongo::unittest::Test {
public:
    void run() {
        auto base = static_cast<bongo::unittest::Test*>(this);
        try {
            ...
            log() << "exception while testing KeyString version "
                  << bongo::KeyString::versionToString(version);
            throw;
        }
    }
...
#if !defined(BONGO_CONFIG_OPTIMIZED_BUILD)
...
void perfTest(KeyString::Version version, const Numbers& numbers) {
    ...
    log() << 1E3 * micros / static_cast<double>(iters * numbers.size()) << " ns per "
          << bongo::KeyString::versionToString(version) << " roundtrip"
