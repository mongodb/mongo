...
TEST(DataFileVersionTest, MustUpgradeWhenMajorVersionIsUnsupported) {
    ...
    ASSERT_EQ(
        "The data files have major version 5, but this version of bongod only supports version 4",
        status.reason());
}
...
TEST(DataFileVersionTest, MustUpgradeWhenSingleMinorFeatureBitIsUnrecognized) {
    ...
    ASSERT_EQ(ErrorCodes::MustUpgrade, status.code());
    ASSERT_EQ(
        "The data files use features not recognized by this version of bongod; the feature bits in"
        " positions [ 10 ] aren't recognized by this version of bongod",
        status.reason());
}
...
TEST(DataFileVersionTest, MustUpgradeWhenMultipleMinorFeatureBitsAreUnrecognized) {
    ...
    ASSERT_EQ(
        "The data files use features not recognized by this version of bongod; the feature bits in"
        " positions [ 10, 14, 15 ] aren't recognized by this version of bongod",
        status.reason());
}
...
TEST(DataFileVersionTest, MustUpgradeWhenIndexPluginVersionIsUnsupported) {
    ...
    ASSERT_EQ(ErrorCodes::MustUpgrade, status.code());
    ASSERT_EQ(
        "The data files have index plugin version 7, but this version of bongod only supports"
        " versions 5 and 6",
        status.reason());
}
