...
class MemoryMappedCOWBitset {
    BONGO_DISALLOW_COPYING(MemoryMappedCOWBitset);
...
public:
    ...
    BONGO_STATIC_ASSERT_MSG(MaxChunkMemory == MaxWinMemory,
                            "Need a larger bitset to cover max process VM space");

public:
    MemoryMappedCOWBitset() {
        BONGO_STATIC_ASSERT_MSG(MemoryMappedCOWBitset::MaxChunkBytes == sizeof(bits),
...
class PointerToDurableMappedFile {
    BONGO_DISALLOW_COPYING(PointerToDurableMappedFile);
