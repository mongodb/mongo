/**
 *    Copyright (C) 2009-2014 BongoDB Inc.
 *
...
#define BONGO_LOG_DEFAULT_COMPONENT ::bongo::logger::LogComponent::kJournal

#include "bongo/platform/basic.h"

#include "bongo/db/storage/mmap_v1/dur.h"

#include <iomanip>
#include <utility>

#include "bongo/base/static_assert.h"
...
#include "bongo/db/storage/mmap_v1/aligned_builder.h"
#include "bongo/db/storage/mmap_v1/commit_notifier.h"
#include "bongo/db/storage/mmap_v1/dur_commitjob.h"
#include "bongo/db/storage/mmap_v1/dur_journal.h"
#include "bongo/db/storage/mmap_v1/dur_journal_writer.h"
#include "bongo/db/storage/mmap_v1/dur_recover.h"
#include "bongo/db/storage/mmap_v1/dur_stats.h"
#include "bongo/db/storage/mmap_v1/durable_mapped_file.h"
...
#include "bongo/util/clock_source.h"
...
// Size sanity checks
BONGO_STATIC_ASSERT(UncommittedBytesLimit > BSONObjMaxInternalSize * 3);
BONGO_STATIC_ASSERT(sizeof(void*) == 4 || UncommittedBytesLimit > BSONObjMaxInternalSize * 6);
...
// might get an access violation. That's why we acquire the bongo files mutex here in X
...
// See SERVER-8795 to see why this code is necessary on Solaris.
    ...
    std::set<BongoFile*>& files = BongoFile::getAllFiles();
    ...
    const set<BongoFile*>::iterator b = files.begin();
    const set<BongoFile*>::iterator e = files.end();
    set<BongoFile*>::iterator i = b;
...
bool DurableImpl::commitIfNeeded() {
    if (BONGO_likely(commitJob.bytes() < UncommittedBytesLimit)) {
        return false;
    }
