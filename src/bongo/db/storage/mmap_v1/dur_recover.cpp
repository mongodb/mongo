...
class JournalSectionIterator {
    BONGO_DISALLOW_COPYING(JournalSectionIterator);
...
RecoveryJob::Last::Last(OperationContext* txn) : _txn(txn), mmf(NULL), fileNo(-1) {
    ...
    LockBongoFilesShared::assertAtLeastReadLocked(txn);
}
...
DurableMappedFile* RecoveryJob::Last::newEntry(const dur::ParsedJournalEntry& entry,
    ...
    BongoFile* file;
    {
        BongoFileFinder finder(_txn);  // must release lock before creating new DurableMappedFile
...
void RecoveryJob::processSection(OperationContext* txn,
                                 ...
                                 unsigned len,
                                 const JSectFooter* f) {
    LockBongoFilesShared lkFiles(txn);  // for RecoveryJob::Last
    stdx::lock_guard<stdx::mutex> lk(_mx);
...

        {
            ...
            if (!h.versionOk()) {
                log() << "journal file version number mismatch got:" << hex << h._version
                      ...
                      << ". if you have just upgraded, recover with old version of bongod, "
...
bool RecoveryJob::processFile(OperationContext* txn, boost::filesystem::path journalfile) {
    ...
    MemoryMappedFile f{txn, BongoFile::Options::READONLY | BongoFile::Options::SEQUENTIAL};
    ON_BLOCK_EXIT([&f, &txn] {
        LockBongoFilesExclusive lock(txn);
        f.close(txn);
    });
...
void RecoveryJob::go(OperationContext* txn, vector<boost::filesystem::path>& files) {
    ...
    LockBongoFilesExclusive lkFiles(txn);  // for RecoveryJob::Last
    _recovering = true;
