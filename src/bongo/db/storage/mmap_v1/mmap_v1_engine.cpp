...
#include "bongo/db/storage/mmap_v1/data_file_sync.h"
...
#include "bongo/db/storage/mmap_v1/dur_recovery_unit.h"
...
void checkForUncleanShutdown(MMAPV1Engine* storageEngine,
    ...
    if (oldFile) {
        ...
        // if we can't, then its probably just another bongod running
        ...
        if (doingRepair && dur::haveJournalFiles()) {
            ...
            if (!dur::haveJournalFiles(/*anyFiles=*/true)) {
                ...
                if (dbnames.size() == 0) {
                    // this means that bongod crashed
                    ...
                    errmsg = str::stream()
                        ...
                        << "see: http://dochub.bongodb.org/core/repair for more information\n"
                        << "*************";
                }
            }
        } else {
            if (!dur::haveJournalFiles() && !doingRepair) {
                errmsg = str::stream() << "************** \n"
                                       ...
                                       << "Please visit http://dochub.bongodb.org/core/repair for "
...
void checkReadAhead(const string& dir) {
    ...
    try {
        ...
        if (boost::filesystem::exists(path)) {
            ...
            if (file.is_open()) {
                ...
                if (kb > 256) {
                    ...
                    log() << "**          http://dochub.bongodb.org/core/readahead"
                          << startupWarningsLog;
                }
            }
        }
        ...
        log() << "unable to validate readahead settings due to error: " << e.what()
              << startupWarningsLog;
        log() << "for more information, see http://dochub.bongodb.org/core/readahead"
              << startupWarningsLog;
    }
#endif  // __linux__
}
...
MMAPV1Engine::MMAPV1Engine(const StorageEngineLockFile* lockFile,
    ...
    if (!storageGlobalParams.readOnly) {
        ...
        BONGO_ASSERT_ON_EXCEPTION_WITH_MSG(clearTmpFiles(), "clear tmp files");
    }
}
...
int MMAPV1Engine::flushAllFiles(OperationContext* txn, bool sync) {
    return BongoFile::flushAll(txn, sync);
}
