...
#include "bongo/db/storage/mmap_v1/mmap_v1_extent_manager.h"
...
#include "bongo/db/storage/mmap_v1/data_file.h"
#include "bongo/db/storage/mmap_v1/dur.h"
#include "bongo/db/storage/mmap_v1/extent.h"
#include "bongo/db/storage/mmap_v1/extent_manager.h"
#include "bongo/db/storage/mmap_v1/mmap.h"
#include "bongo/db/storage/mmap_v1/mmap_v1_engine.h"
#include "bongo/db/storage/mmap_v1/mmap_v1_options.h"
#include "bongo/db/storage/mmap_v1/record.h"
#include "bongo/db/storage/record_fetcher.h"
...
BONGO_FP_DECLARE(recordNeedsFetchFail);
...
class MmapV1RecordFetcher : public RecordFetcher {
    BONGO_DISALLOW_COPYING(MmapV1RecordFetcher);
...
public:
    ...
    virtual void setup(OperationContext* txn) {
        ...
        _filesLock.reset(new LockBongoFilesShared(txn));
    }
    ...
    virtual void fetch() {
        ...
        // lock on bongo files. We do this here because we have to release the
        // lock on bongo files prior to reacquiring lock mgr locks.
        _filesLock.reset();
    }
...
private:
    ...
    // we dereference it.
    std::unique_ptr<LockBongoFilesShared> _filesLock;
};
...
std::unique_ptr<RecordFetcher> MmapV1ExtentManager::recordNeedsFetch(const DiskLoc& loc) const {
    ...
    if (BONGO_FAIL_POINT(recordNeedsFetchFail)) {
