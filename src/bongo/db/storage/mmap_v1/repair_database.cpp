...
#include "bongo/db/storage/mmap_v1/file_allocator.h"
#include "bongo/db/storage/mmap_v1/mmap.h"
#include "bongo/db/storage/mmap_v1/mmap_v1_database_catalog_entry.h"
...
void _deleteDataFiles(const std::string& database) {
    if (storageGlobalParams.directoryperdb) {
        ...
        BONGO_ASSERT_ON_EXCEPTION_WITH_MSG(
...
Path uniqueReservedPath(const char* prefix) {
    ...
    do {
        ...
        BONGO_ASSERT_ON_EXCEPTION(exists = boost::filesystem::exists(reservedPath));
    } while (exists);
    return reservedPath;
}
...
void _applyOpToDataFiles(const string& database,
    ...
    q = p / (c + "ns");
    bool ok = false;
    BONGO_ASSERT_ON_EXCEPTION(ok = fo.apply(q));
    if (ok) {
...
public:
    ...
    ~RepairFileDeleter() {
        ...
        try {
            ...
            BongoFile::flushAll(_txn, true);

            BONGO_ASSERT_ON_EXCEPTION(boost::filesystem::remove_all(_path));
        } catch (DBException& e) {
...
Status MMAPV1Engine::repairDatabase(OperationContext* txn,
    ...
    bool created = false;
    BONGO_ASSERT_ON_EXCEPTION(created = boost::filesystem::create_directory(reservedPath));
    invariant(created);
    ...
    if (backupOriginalFiles) {
        ...
        BONGO_ASSERT_ON_EXCEPTION(boost::filesystem::create_directory(newDir));
    ...
    if (!backupOriginalFiles) {
        BONGO_ASSERT_ON_EXCEPTION(boost::filesystem::remove_all(reservedPath));
    }
