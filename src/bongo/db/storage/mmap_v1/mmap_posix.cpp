...
void* MemoryMappedFile::remapPrivateView(OperationContext* txn, void* oldPrivateAddr) {
    ...
    LockBongoFilesExclusive lockBongoFiles(txn);
#endif
...
public:
    ...
    void flush(OperationContext* txn) {
        ...
        LockBongoFilesShared mmfilesLock(txn);
        std::set<BongoFile*> mmfs = BongoFile::getAllFiles();
        std::set<BongoFile*>::const_iterator it = mmfs.find(_theFile);
