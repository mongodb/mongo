...
public:
     ...
     * level. If the engine returns true, BongoDB will acquire intent locks down to the
     * collection level and will assume that the engine will ensure consistency at the level of
     * documents. If false, BongoDB will lock the entire collection in Shared/Exclusive mode
     ...
     *
     * If this function returns an OK status, BongoDB can call endBackup to signal the storage
