...
void flushMyDirectory(const boost::filesystem::path& file) {
    ...
    // if called without a fully qualified path it asserts; that makes bongoperf fail.
    ...
    if (fsync(fd) != 0) {
        ...
        if (e == EINVAL) {  // indicates filesystem does not support synchronization
            if (!_warnedAboutFilesystem) {
                ...
                log() << "\t\t\thttp://dochub.bongodb.org/core/unsupported-filesystems"
                      << startupWarningsLog;
                log() << "\t\tPlease notify BongoDB, Inc. if an unlisted filesystem generated "
