...
StatusWithMatchExpression MatchExpressionParser::_parseSubField(const BSONObj& context,
    ...
    if (bongoutils::str::equals("$eq", e.fieldName()))
        return _parseComparison(name, new EqualityMatchExpression(), e, collator);

    if (bongoutils::str::equals("$not", e.fieldName())) {
    ...
    switch (x) {
        case -1:
            ...
            if (bongoutils::str::equals("$where", e.fieldName())) {
                return {Status(ErrorCodes::BadValue, "$where cannot be applied to a field")};
            }

            return {Status(ErrorCodes::BadValue,
                           bongoutils::str::stream() << "unknown operator: " << e.fieldName())};
        case BSONObj::LT:
        ...
        case BSONObj::opNEAR:
            return {Status(ErrorCodes::BadValue,
                           bongoutils::str::stream() << "near must be first in: " << context)};
    ...
    return {Status(ErrorCodes::BadValue,
                   bongoutils::str::stream() << "not handled: " << e.fieldName())};
}
...
StatusWithMatchExpression MatchExpressionParser::_parse(const BSONObj& obj,
    ...
    while (i.more()) {
        ...
        if (e.fieldName()[0] == '$') {
            ...
            if (bongoutils::str::equals("or", rest)) {
                if (e.type() != Array)
            ...
            } else if (bongoutils::str::equals("and", rest)) {
                if (e.type() != Array)
                ...
                if (!s.isOK())
                    return s;
                root->add(temp.release());
            } else if (bongoutils::str::equals("nor", rest)) {
                if (e.type() != Array)
                ...
                if (!s.isOK())
                    return s;
                root->add(temp.release());
            } else if (bongoutils::str::equals("atomic", rest) ||
                       bongoutils::str::equals("isolated", rest)) {
                if (!topLevel)
                    return {Status(ErrorCodes::BadValue,
            ...
            } else if (bongoutils::str::equals("where", rest)) {
                StatusWithMatchExpression s = _extensionsCallback->parseWhere(e);
                if (!s.isOK())
                    return s;
                root->add(s.getValue().release());
            } else if (bongoutils::str::equals("text", rest)) {
                StatusWithMatchExpression s = _extensionsCallback->parseText(e);
                if (!s.isOK()) {
                    return s;
                }
                root->add(s.getValue().release());
            } else if (bongoutils::str::equals("comment", rest)) {
            } else if (bongoutils::str::equals("ref", rest) ||
                       bongoutils::str::equals("id", rest) || bongoutils::str::equals("db", rest)) {
                // DBRef fields.
                ...
                return {Status(ErrorCodes::BadValue,
                               bongoutils::str::stream() << "unknown top level operator: "
                                                         << e.fieldName())};
            }
...
Status MatchExpressionParser::_parseSub(const char* name,
    ...
    if (geoIt.more()) {
        ...
        if (firstElt.isABSONObj()) {
            ...
            if (bongoutils::str::equals(fieldName, "$near") ||
                bongoutils::str::equals(fieldName, "$nearSphere") ||
                bongoutils::str::equals(fieldName, "$geoNear")) {
                StatusWithMatchExpression s =
...
bool MatchExpressionParser::_isDBRefDocument(const BSONObj& obj, bool allowIncompleteDBRef) {
    ...
    while (i.more() && !(hasRef && hasID)) {
        ...
        // $ref
        if (!hasRef && bongoutils::str::equals("$ref", fieldName)) {
            hasRef = true;
        }
        // $id
        else if (!hasID && bongoutils::str::equals("$id", fieldName)) {
            hasID = true;
        }
        // $db
        else if (!hasDB && bongoutils::str::equals("$db", fieldName)) {
            hasDB = true;
        }
    }
...
StatusWithMatchExpression MatchExpressionParser::_parseElemMatch(const char* name,
    ...
    if (_isExpressionDocument(e, true)) {
        ...
        isElemMatchValue = !bongoutils::str::equals("$and", elt.fieldName()) &&
            !bongoutils::str::equals("$nor", elt.fieldName()) &&
            !bongoutils::str::equals("$or", elt.fieldName()) &&
            !bongoutils::str::equals("$where", elt.fieldName());
    }
...
StatusWithMatchExpression MatchExpressionParser::_parseAll(const char* name,
    ...
    if (arr.firstElement().type() == Object &&
        bongoutils::str::equals("$elemMatch",
        ...
        while (i.more()) {
            ...
            if (!bongoutils::str::equals("$elemMatch",
