...
TEST(MatchExpressionParserLeafTest, TypeDecimalOperator) {
    BSONObj query = BSON("x" << BSON("$type" << bongo::NumberDecimal));
    const CollatorInterface* collator = nullptr;
    StatusWithMatchExpression result =
        MatchExpressionParser::parse(query, ExtensionsCallbackDisallowExtensions(), collator);
    ASSERT_TRUE(result.isOK());
    ...
    ASSERT_TRUE(result.getValue()->matchesBSON(BSON("x" << bongo::Decimal128("1"))));
}
...
TEST(MatchExpressionParserLeafTest, TypeStringNameNumberDecimal) {
    ...
    ASSERT_TRUE(tmeNumberDecimal->matchesBSON(BSON("a" << bongo::Decimal128("1"))));
