...
#include "bongo/db/s/active_migrations_registry.h"
#include "bongo/db/s/collection_range_deleter.h"
#include "bongo/db/s/migration_destination_manager.h"
...
#include "bongo/stdx/unordered_map.h"
...
/**
 * Contains the global sharding state for a running bongod. There is one instance of this object per
 * service context and it is never destroyed for the lifetime of the context.
 */
class ShardingState {
    BONGO_DISALLOW_COPYING(ShardingState);
...
public:
     ...
     * context, otherwise it will fassert. In other words, it may only be called on BongoD and
