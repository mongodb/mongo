...
/**
 * A decoration on OperationContext representing per-operation shard version metadata sent to bongod
 * from bongos as a command parameter.
 *
...
class OperationShardingState {
    BONGO_DISALLOW_COPYING(OperationShardingState);
...
class OperationShardingState::IgnoreVersioningBlock {
    BONGO_DISALLOW_COPYING(IgnoreVersioningBlock);
