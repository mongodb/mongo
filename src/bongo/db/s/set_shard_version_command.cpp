    ...
    bool run(OperationContext* txn,
...

        {
            ...
            if (!status.isOK()) {
                ...
                // We reloaded a version that doesn't match the version bongos was trying to
                // set.
                ...
                // WARNING: the exact fields below are important for compatibility with bongos
                // version reload.

                result.append("ns", ns);
                currVersion.addToBSON(result, "globalVersion");

                // If this was a reset of a collection or the last chunk moved out, inform bongos to
                // do a full reload.
                if (currVersion.epoch() != requestedVersion.epoch() || !currVersion.isSet()) {
                    result.appendBool("reloadConfig", true);
                    // Zero-version also needed to trigger full bongos reload, sadly
