...
BONGO_FP_DECLARE(migrateThreadHangAtStep1);
BONGO_FP_DECLARE(migrateThreadHangAtStep2);
BONGO_FP_DECLARE(migrateThreadHangAtStep3);
BONGO_FP_DECLARE(migrateThreadHangAtStep4);
BONGO_FP_DECLARE(migrateThreadHangAtStep5);
BONGO_FP_DECLARE(migrateThreadHangAtStep6);

BONGO_FP_DECLARE(failMigrationReceivedOutOfRangeOperation);
...
void MigrationDestinationManager::_migrateDriver(OperationContext* txn,
...

    {
        ...
        timing.done(1);
        BONGO_FAIL_POINT_PAUSE_WHILE_SET(migrateThreadHangAtStep1);
    }

    {
        ...
        timing.done(2);
        BONGO_FAIL_POINT_PAUSE_WHILE_SET(migrateThreadHangAtStep2);
    }

    {
        ...
        timing.done(3);
        BONGO_FAIL_POINT_PAUSE_WHILE_SET(migrateThreadHangAtStep3);
    }
...

    {
        ...
        timing.done(4);
        BONGO_FAIL_POINT_PAUSE_WHILE_SET(migrateThreadHangAtStep4);
    }
...

    {
        ...
        timing.done(5);
        BONGO_FAIL_POINT_PAUSE_WHILE_SET(migrateThreadHangAtStep5);
    }

    setState(DONE);

    timing.done(6);
    BONGO_FAIL_POINT_PAUSE_WHILE_SET(migrateThreadHangAtStep6);
        ...
        while (i.more()) {
            ...
            if (Helpers::findById(txn, ctx.db(), ns.c_str(), id, fullObj)) {
                if (!isInRange(fullObj, min, max, shardKeyPattern)) {
                    if (BONGO_FAIL_POINT(failMigrationReceivedOutOfRangeOperation)) {
                        invariant(0);
                    }
                    continue;
                }
