...
#include "bongo/db/s/migration_chunk_cloner_source_legacy.h"
#include "bongo/db/s/migration_util.h"
...
BONGO_FP_DECLARE(migrationCommitNetworkError);
BONGO_FP_DECLARE(failMigrationCommit);
BONGO_FP_DECLARE(hangBeforeLeavingCriticalSection);
...
Status MigrationSourceManager::commitChunkOnRecipient(OperationContext* txn) {
    ...
    if (BONGO_FAIL_POINT(failMigrationCommit) && commitCloneStatus.isOK()) {
...
Status MigrationSourceManager::commitChunkMetadataOnConfig(OperationContext* txn) {
    ...
    if (BONGO_FAIL_POINT(migrationCommitNetworkError)) {
    ...
    BONGO_FAIL_POINT_PAUSE_WHILE_SET(hangBeforeLeavingCriticalSection);
