...
#include "bongo/db/s/metadata_manager.h"
 ...
 * to each sharded collection known on a bongod instance. A set of these objects is linked off the
...
class CollectionShardingState {
    BONGO_DISALLOW_COPYING(CollectionShardingState);
...
public:
    ...
    /**
     * Returns whether this collection is sharded. Valid only if bongoD is primary.
