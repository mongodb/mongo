...
#include "bongo/db/s/sharding_egress_metadata_hook_for_bongod.h"
...
void ShardedConnectionInfo::addHook() {
    ...
    globalConnPool.addHook(new ShardingConnectionHook(
        false, stdx::make_unique<rpc::ShardingEgressMetadataHookForBongod>()));

    shardConnectionPool.addHook(new ShardingConnectionHook(
        true, stdx::make_unique<rpc::ShardingEgressMetadataHookForBongod>()));
