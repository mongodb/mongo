...
class ServiceContextBongoD final : public ServiceContext {
public:
    typedef std::map<std::string, const StorageEngine::Factory*> FactoryMap;

    ServiceContextBongoD();
...
private:
    ...
    // All possible storage engines are registered here through BONGO_INIT.
    FactoryMap _storageFactories;
};

class StorageFactoriesIteratorBongoD : public StorageFactoriesIterator {
public:
    typedef ServiceContextBongoD::FactoryMap::const_iterator FactoryMapIterator;
    StorageFactoriesIteratorBongoD(const FactoryMapIterator& begin, const FactoryMapIterator& end);
