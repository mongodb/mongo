...
bongo::Document::FieldPair getNthField(bongo::Document doc, size_t index) {
    bongo::FieldIterator it(doc);
...
public:
    void run() {
        ...
        docBuilder["mega array"] = bongo::Value(values);
    ...
    void append(const char* name, const T& thing) {
        ...
        arrBuilder << thing;
        docBuilder[name] = bongo::Value(thing);
        values.push_back(bongo::Value(thing));
    }

    vector<bongo::Value> values;
...
public:
    void run() {
        ...
        ASSERT_EQUALS(bongo::String, value.getType());
        assertRoundTrips(value);
    }
};
...
public:
    void run() {
        ...
        ASSERT_EQUALS(bongo::Date, value.getType());
        assertRoundTrips(value);
    }
};
...
public:
    void run() {
        ...
        ASSERT_EQUALS(bongo::bsonTimestamp, value.getType());
        assertRoundTrips(value);
...
public:
    void run() {
        bongo::Document document = bongo::Document();
...
public:
    void run() {
        bongo::MutableDocument md;
        md.addField("a", Value(5));
        md.addField("apple", Value("rrr"_sd));
        md.addField("banana", Value(-.3));
        bongo::Document document = md.freeze();
...
public:
    void run() {
        ...
        ASSERT_EQUALS(bongo::Array, value.getType());
...
public:
    void run() {
        ...
        ASSERT_EQUALS(bongo::Bool, value.getType());
        assertRoundTrips(value);
    }
};
...
public:
    void run() {
        ...
        ASSERT_EQUALS(bongo::Symbol, value.getType());
        assertRoundTrips(value);
    }
};
...
public:
    void run() {
        ...
        ASSERT_EQUALS(bongo::Undefined, value.getType());
        assertRoundTrips(value);
    }
};
...
public:
    void run() {
        ASSERT_THROWS(Value(bongo::Document()).coerceToString(), UserException);
    }
};
...
public:
    void run() {
        ...
        assertWidest(Undefined, NumberInt, bongo::Bool);
        assertWidest(Undefined, bongo::String, NumberDouble);
    }
...
public:
    void run() {
        ...
        assertComparison(-1, Value(bongo::MINKEY), Value());
        ...
        assertComparison(-1, Value("string"_sd), Value(bongo::Document()));
        assertComparison(-1, Value(bongo::Document()), Value(vector<Value>()));
        assertComparison(-1, Value(vector<Value>()), Value(BSONBinData("", 0, MD5Type)));
        assertComparison(-1, Value(BSONBinData("", 0, MD5Type)), Value(bongo::OID()));
        assertComparison(-1, Value(bongo::OID()), Value(false));
        ...
        assertComparison(-1, Value(BSONRegEx("")), Value(BSONDBRef("", bongo::OID())));
        assertComparison(-1, Value(BSONDBRef("", bongo::OID())), Value(BSONCode("")));
        assertComparison(-1, Value(BSONCode("")), Value(BSONCodeWScope("", BSONObj())));
        assertComparison(-1, Value(BSONCodeWScope("", BSONObj())), Value(bongo::MAXKEY));
    }
...
private:
    ...
    void assertComparison(int expectedResult, const Value& a, const Value& b) {
        bongo::unittest::log() << "testing " << a.toString() << " and " << b.toString();
