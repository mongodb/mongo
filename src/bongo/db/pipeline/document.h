...
#include "bongo/db/pipeline/document_internal.h"
 ...
 */
class MutableDocument {
    BONGO_DISALLOW_COPYING(MutableDocument);
...
private:
    ...
    DocumentStorage& storage() {
        if (BONGO_unlikely(!_storage))
            return newStorage();

        if (BONGO_unlikely(_storage->isShared()))
...
inline void swap(bongo::Document& lhs, bongo::Document& rhs) {
    lhs.swap(rhs);
}
