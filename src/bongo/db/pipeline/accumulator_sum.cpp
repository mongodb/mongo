    ...
    switch (totalType) {
        ...
        case NumberLong:
            ...
            if (toBeMerged) {
                ...
                // adding up to the desired total. Older BongoDB versions used to ignore signed
                // integer overflow and cause undefined behavior, that in practice resulted in
                // values that would wrap around modulo 2**64. Now an older bongos with a newer
                // bongod will yield an error that $sum resulted in a non-numeric type, which is
