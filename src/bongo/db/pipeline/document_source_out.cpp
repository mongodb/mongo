...
#include "bongo/db/pipeline/document_source_out.h"
...
DocumentSourceOut::~DocumentSourceOut() {
    DESTRUCTOR_GUARD(
        ...
        if (_bongod && _tempNs.size()) _bongod->directClient()->dropCollection(_tempNs.ns());)
}
...
void DocumentSourceOut::initialize() {
    invariant(_bongod);
    DBClientBase* conn = _bongod->directClient();

    // Save the original collection options and index specs so we can check they didn't change
    // during computation.
    _originalOutOptions = _bongod->getCollectionOptions(_outputNs);
...
void DocumentSourceOut::spill(const vector<BSONObj>& toInsert) {
    BSONObj err = _bongod->insert(_tempNs, toInsert);
    uassert(16996,
...
DocumentSource::GetNextResult DocumentSourceOut::getNext() {
    ...
    switch (nextInput.getStatus()) {
        ...
        case GetNextResult::ReturnStatus::kEOF: {
            ...
            auto status = _bongod->renameIfOptionsAndIndexesHaveNotChanged(
