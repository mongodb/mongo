...
#include "bongo/base/compare_numbers.h"
...
Value::Value(const BSONElement& elem) : _storage(elem.type()) {
    switch (elem.type()) {
        ...
        case jstOID:
            BONGO_STATIC_ASSERT(sizeof(_storage.oid) == OID::kOIDSize);
...
void Value::hash_combine(size_t& seed,
    ...
    switch (type) {
        ...
        case bsonTimestamp:
        case Date:
            BONGO_STATIC_ASSERT(sizeof(_storage.dateValue) == sizeof(_storage.timestampValue));
