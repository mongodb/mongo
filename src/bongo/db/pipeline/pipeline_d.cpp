...
namespace {
class BongodImplementation final : public DocumentSourceNeedsBongod::BongodInterface {
public:
    BongodImplementation(const intrusive_ptr<ExpressionContext>& ctx)
    ...
    StatusWith<boost::intrusive_ptr<Pipeline>> makePipeline(
        ...
        // than the DocumentSource this BongodImplementation is injected into, but both
...
void PipelineD::prepareCursorSource(Collection* collection,
    ...
    // Inject a BongodImplementation to sources that need them.
    for (auto&& source : sources) {
        DocumentSourceNeedsBongod* needsBongod =
            dynamic_cast<DocumentSourceNeedsBongod*>(source.get());
        if (needsBongod) {
            needsBongod->injectBongodInterface(std::make_shared<BongodImplementation>(expCtx));
        }
    }
...
StatusWith<std::unique_ptr<PlanExecutor>> PipelineD::prepareExecutor(
    ...
    // If we are connecting directly to the shard rather than through a bongos, don't filter out
    // orphaned documents.
