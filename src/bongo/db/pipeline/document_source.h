...
#define REGISTER_DOCUMENT_SOURCE(key, liteParser, fullParser)                                \
    BONGO_INITIALIZER(addToDocSourceParserMap_##key)(InitializerContext*) {                  \
...
#define REGISTER_MULTI_STAGE_ALIAS(key, liteParser, fullParser)                  \
    BONGO_INITIALIZER(addAliasToDocSourceParserMap_##key)(InitializerContext*) { \
        LiteParsedDocumentSource::registerParser("$" #key, (liteParser));        \
...
/** This class marks DocumentSources which need bongod-specific functionality.
 *  It causes a BongodInterface to be injected when in a bongod and prevents bongos from
 *  merging pipelines containing this stage.
 */
class DocumentSourceNeedsBongod : public DocumentSource {
public:
    // Wraps bongod-specific functions to allow linking into bongos.
    class BongodInterface {
    public:
        virtual ~BongodInterface(){};
         ...
         * DBClientBase* because DBDirectClient isn't linked into bongos.
         */
    ...
    DocumentSourceNeedsBongod(const boost::intrusive_ptr<ExpressionContext>& expCtx)
        : DocumentSource(expCtx) {}

    void injectBongodInterface(std::shared_ptr<BongodInterface> bongod) {
        _bongod = bongod;
        doInjectBongodInterface(bongod);
    }

    /**
     * Derived classes may override this method to register custom inject functionality.
     */
    virtual void doInjectBongodInterface(std::shared_ptr<BongodInterface> bongod) {}

    void detachFromOperationContext() override {
        invariant(_bongod);
        _bongod->setOperationContext(nullptr);
    ...
    void reattachToOperationContext(OperationContext* opCtx) final {
        invariant(_bongod);
        _bongod->setOperationContext(opCtx);
...
protected:
    // It is invalid to delete through a DocumentSourceNeedsBongod-typed pointer.
    virtual ~DocumentSourceNeedsBongod() {}

    // Gives subclasses access to a BongodInterface implementation
    std::shared_ptr<BongodInterface> _bongod;
};
