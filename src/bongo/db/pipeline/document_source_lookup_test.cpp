...
#include "bongo/db/pipeline/stub_bongod_interface.h"
...
/**
 * A mock BongodInterface which allows mocking a foreign pipeline.
 */
class MockBongodInterface final : public StubBongodInterface {
public:
    MockBongodInterface(deque<DocumentSource::GetNextResult> mockResults)
...
TEST_F(DocumentSourceLookUpTest, ShouldPropagatePauses) {
    ...
    deque<DocumentSource::GetNextResult> mockForeignContents{Document{{"_id", 0}},
                                                             Document{{"_id", 1}}};
    lookup->injectBongodInterface(
        std::make_shared<MockBongodInterface>(std::move(mockForeignContents)));
