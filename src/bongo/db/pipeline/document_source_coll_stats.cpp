...
#include "bongo/db/pipeline/document_source_coll_stats.h"
...
DocumentSource::GetNextResult DocumentSourceCollStats::getNext() {
    ...
    if (_collStatsSpec.hasField("latencyStats")) {
        ...
        _bongod->appendLatencyStats(pExpCtx->ns, includeHistograms, &builder);
    }
    ...
    if (_collStatsSpec.hasField("storageStats")) {
        ...
        Status status = _bongod->appendStorageStats(
