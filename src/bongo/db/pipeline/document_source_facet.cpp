...
#include "bongo/db/pipeline/document_source_facet.h"
...
#include "bongo/db/pipeline/document_source_tee_consumer.h"
...
DocumentSourceFacet::DocumentSourceFacet(std::vector<FacetPipeline> facetPipelines,
    ...
    : DocumentSourceNeedsBongod(expCtx),
...
void DocumentSourceFacet::doInjectBongodInterface(std::shared_ptr<BongodInterface> bongod) {
    for (auto&& facet : _facets) {
        for (auto&& stage : facet.pipeline->getSources()) {
            if (auto stageNeedingBongod = dynamic_cast<DocumentSourceNeedsBongod*>(stage.get())) {
                stageNeedingBongod->injectBongodInterface(bongod);
            }
        }
    }
}
