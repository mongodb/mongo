...
#include "bongo/db/pipeline/document_source_graph_lookup.h"
...
/**
 * A BongodInterface use for testing that supports making pipelines with an initial
 * DocumentSourceMock source.
 */
class MockBongodImplementation final : public StubBongodInterface {
public:
    MockBongodImplementation(std::deque<DocumentSource::GetNextResult> results)
...
TEST_F(DocumentSourceGraphLookUpTest,
    ...
    graphLookupStage->injectBongodInterface(
        std::make_shared<MockBongodImplementation>(std::move(fromContents)));
