...
#include "bongo/db/pipeline/document_source_geo_near.h"

#include "bongo/db/pipeline/document.h"
#include "bongo/db/pipeline/document_source_limit.h"
#include "bongo/db/pipeline/document_source_sort.h"
...
void DocumentSourceGeoNear::runCommand() {
    ...
    bool ok = _bongod->directClient()->runCommand(
...
DocumentSourceGeoNear::DocumentSourceGeoNear(const intrusive_ptr<ExpressionContext>& pExpCtx)
    : DocumentSourceNeedsBongod(pExpCtx),
      coordsIsArray(false),
