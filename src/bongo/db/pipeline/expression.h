...
#define REGISTER_EXPRESSION(key, parser)                                     \
    BONGO_INITIALIZER(addToExpressionParserMap_##key)(InitializerContext*) { \
...
class Variables {
    BONGO_DISALLOW_COPYING(Variables);
...
public:
    ...
    void validateArguments(const Expression::ExpressionVector& args) const override {
        uassert(28667,
                bongoutils::str::stream() << "Expression " << this->getOpName()
                                          << " takes at least "
                                          << MinArgs
...
public:
    ...
    void validateArguments(const Expression::ExpressionVector& args) const override {
        uassert(16020,
                bongoutils::str::stream() << "Expression " << this->getOpName() << " takes exactly "
                                          << NArgs
                                          << " arguments. "
                                          << args.size()
                                          << " were passed in.",
