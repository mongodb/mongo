...
class ResourceId {
    ...
    BONGO_STATIC_ASSERT(ResourceTypesCount <= (1 << resourceTypeBits));
...
#ifndef BONGO_CONFIG_DEBUG_BUILD
...
BONGO_STATIC_ASSERT(sizeof(ResourceId) == sizeof(uint64_t));
#endif
...
template <>
struct hash<bongo::ResourceId> {
    size_t operator()(const bongo::ResourceId& resource) const {
        return resource;
    }
};
