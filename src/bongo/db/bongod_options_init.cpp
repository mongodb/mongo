...
BONGO_INITIALIZER_GENERAL(BongodOptions,
                          ("BeginStartupOptionValidation", "AllFailPointsRegistered"),
                          ("EndStartupOptionValidation"))
(InitializerContext* context) {
    if (!handlePreValidationBongodOptions(moe::startupOptionsParsed, context->args())) {
    ...
    ret = validateBongodOptions(moe::startupOptionsParsed);
    if (!ret.isOK()) {
        return ret;
    }
    ret = canonicalizeBongodOptions(&moe::startupOptionsParsed);
    if (!ret.isOK()) {
        return ret;
    }
...
BONGO_INITIALIZER_GENERAL(BongodOptions_Store,
                          ("BeginStartupOptionStorage"),
                          ("EndStartupOptionStorage"))
(InitializerContext* context) {
    Status ret = storeBongodOptions(moe::startupOptionsParsed);
    if (!ret.isOK()) {
