 ...
 */
bool isOplogTsPred(const bongo::MatchExpression* me) {
    if (bongo::MatchExpression::GT != me->matchType() &&
        bongo::MatchExpression::GTE != me->matchType()) {
        return false;
    }

    return bongoutils::str::equals(me->path().rawData(), "ts");
}

bongo::BSONElement extractOplogTsOptime(const bongo::MatchExpression* me) {
    invariant(isOplogTsPred(me));
    return static_cast<const bongo::ComparisonMatchExpression*>(me)->getData();
}
...
std::string getProjectedDottedField(const std::string& field, bool* isIDOut) {
    ...
    bongo::splitStringDelim(field, &res, '.');
    ...
    *isIDOut = ("_id" == res[0]);
    ...
    for (size_t i = 1; i < res.size(); ++i) {
        if (bongo::parseNumberFromStringWithBase(res[i], 10, &arrayIndex).isOK()) {
            ...
            if (arrayIndex >= 0) {
                ...
                bongo::joinStringDelim(prefixStrings, &projectedField, '.');
