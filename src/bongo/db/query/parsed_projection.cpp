...
Status ParsedProjection::make(const BSONObj& spec,
    ...
    for (auto&& elem : spec) {
        if (Object == elem.type()) {
            ...
            if (bongoutils::str::equals(e2.fieldName(), "$slice")) {
                ...
                if (bongoutils::str::contains(elem.fieldName(), '.')) {
                    return Status(ErrorCodes::BadValue,
        ...
        } else if (bongoutils::str::equals(elem.fieldName(), "_id") && !elem.trueValue()) {
            pp->_hasId = false;
        } else {
        ...
        if (_isPositionalOperator(elem.fieldName())) {
            ...
            std::string after = bongoutils::str::after(elem.fieldName(), ".$");
            if (bongoutils::str::contains(after, ".$")) {
            ...
            std::string matchfield = bongoutils::str::before(elem.fieldName(), '.');
    ...
    if (!pp->_requiresDocument) {
        ...
        while (srcIt.more()) {
            ...
            if (pp->_hasId && bongoutils::str::equals(elt.fieldName(), "_id")) {
                continue;
            }
...
bool ParsedProjection::_isPositionalOperator(const char* fieldName) {
    return bongoutils::str::contains(fieldName, ".$") &&
        !bongoutils::str::contains(fieldName, ".$ref") &&
        !bongoutils::str::contains(fieldName, ".$id") &&
        !bongoutils::str::contains(fieldName, ".$db");
}
...
bool ParsedProjection::_hasPositionalOperatorMatch(const MatchExpression* const query,
    ...
    if (query->isLogical()) {
        ...
        std::string pathPrefix = bongoutils::str::before(pathRawData, '.');
