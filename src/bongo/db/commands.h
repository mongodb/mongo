/**
 *    Copyright (C) 2009-2016 BongoDB Inc.
 *
...
#include "bongo/db/logical_time.h"
#include "bongo/db/query/explain.h"
#include "bongo/db/write_concern.h"
#include "bongo/rpc/reply_builder_interface.h"
#include "bongo/rpc/request_interface.h"
...
#include "bongo/db/commands_helpers.h"
...
class Command {
protected:
    // The type of the first field in 'cmdObj' must be bongo::String. The first field is
    // interpreted as a collection name.
    static std::string parseNsFullyQualified(const std::string& dbname, const BSONObj& cmdObj);

    // The type of the first field in 'cmdObj' must be bongo::String or Symbol.
...
public:
    ...
    // bongo::String, then that field is interpreted as the collection name, and is
    // appended to 'dbname' after a '.' character. If the first field is not of type
    // bongo::String, then 'dbname' is returned unmodified.
    ...
    */
     ...
     * ServerSelectionMetadata off 'txn'. Once OP_COMMAND is implemented in bongos, this metadata
     ...
     *
     * Note that this is never called on bongos. Sharded commands are responsible for forwarding
...
public:
     ...
     *
     * This is currently used by bongod and dbwebserver.
     */
     ...
     * a BONGO_INITIALIZER context and/or a single-threaded context.
     */
     ...
     *
     *     BONGO_INITIALIZER(RegisterMyTestCommand)(InitializerContext* context) {
    ...
    /**
     * Records the error on to the OperationContext. This hook is needed because bongos
     ...
     * bongos does not have CurOp linked in to it. This must be called from a BONGO_INITIALIZER
...
private:
    ...
    friend void bongo::execCommandClient(OperationContext* txn,
                                         Command* c,
                                         int queryOptions,
                                         const char* ns,
                                         BSONObj& cmdObj,
    ...
    friend void bongo::execCommandDatabase(OperationContext* txn,
                                           Command* command,
                                           const rpc::RequestInterface& request,
                                           rpc::ReplyBuilderInterface* replyBuilder);
};
