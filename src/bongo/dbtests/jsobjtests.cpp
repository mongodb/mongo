...
#include "bongo/util/embedded_builder.h"
...
public:
    void run() {
        ...
        ASSERT(written == bongo::BufferMaxSize / 2);
    }
};
...
struct AppendNumber {
    void run() {
        ...
        b.appendNumber("f", bongo::Decimal128("1"));
...
public:
    void run() {
        ...
        ASSERT_EQUALS(objTypeOf(bongo::Decimal128("1")), NumberDecimal);
        ASSERT_EQUALS(arrTypeOf(bongo::Decimal128("1")), NumberDecimal);
...
public:
    void run() {
        ...
        bongo::BSONArrayIteratorSorted sorted(arr);
        while (unsorted.more()) {
            ASSERT(sorted.more());
...
public:
    void run() {
        ...
        {
            char* crap = (char*)bongoMalloc(x.objsize());
