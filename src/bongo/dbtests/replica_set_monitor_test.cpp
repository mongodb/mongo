 ...
 */
class ReplicaSetMonitorTest : public bongo::unittest::Test {
protected:
    void setUp() {
...
TEST(ReplicaSetMonitorTest, PrimaryRemovedFromSetStress) {
    ...
    for (size_t idxToRemove = 0; idxToRemove < NODE_COUNT; idxToRemove++) {
        ...
        string hostToRemove;
        {
            ...
            BSONElement addrElem = hostsElem[bongo::str::stream() << idxToRemove]["addr"];
 ...
 */
class TwoNodeWithTags : public bongo::unittest::Test {
protected:
    void setUp() {
...
TEST_F(TwoNodeWithTags, SecDownRetryNoTag) {
    ...
    HostAndPort node = assertGet(monitor->getHostOrRefresh(
        ReadPreferenceSetting(bongo::ReadPreference::SecondaryOnly, TagSet()), Milliseconds(0)));
...
TEST_F(TwoNodeWithTags, SecDownRetryWithTag) {
    ...
    TagSet tags(BSON_ARRAY(BSON("dc"
                                << "ny")));
    HostAndPort node = assertGet(monitor->getHostOrRefresh(
        ReadPreferenceSetting(bongo::ReadPreference::SecondaryOnly, tags), Milliseconds(0)));
