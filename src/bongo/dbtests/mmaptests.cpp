...
public:
    ...
    void run() {
        ...
        for (int i = 0; i < N; i++) {
            ...
            DurableMappedFile f{
                txn.get(), i % 4 == 1 ? BongoFile::Options::SEQUENTIAL : BongoFile::Options::NONE};
            ON_BLOCK_EXIT([&f, &txn] {
                LockBongoFilesExclusive lock(txn.get());
                f.close(txn.get());
            });
        ...
        if (t.millis() > 10000) {
            bongo::unittest::log() << "warning: MMap LeakTest is unusually slow N:" << N << ' '
                                   << t.millis() << "ms" << endl;
        }
    }
};
