...
namespace bongo {
/**
 * A simple class for mocking bongo::DBClientConnection.
 *
 * Also check out sample usage in dbtests/mock_dbclient_conn_test.cpp
 */
class MockDBClientConnection : public bongo::DBClientConnection {
public:
    /**
    ...
    std::unique_ptr<bongo::DBClientCursor> query(const std::string& ns,
    ...
    //
    unsigned long long query(stdx::function<void(const bongo::BSONObj&)> f,
                             const std::string& ns,
                             bongo::Query query,
                             const bongo::BSONObj* fieldsToReturn = 0,
                             int queryOptions = 0);

    unsigned long long query(stdx::function<void(bongo::DBClientCursorBatchIterator&)> f,
                             const std::string& ns,
                             bongo::Query query,
                             const bongo::BSONObj* fieldsToReturn = 0,
                             int queryOptions = 0);
    ...
    bool call(bongo::Message& toSend,
              bongo::Message& response,
              bool assertOk,
              std::string* actualServer);
    void say(bongo::Message& toSend, bool isRetry = false, std::string* actualServer = 0);
