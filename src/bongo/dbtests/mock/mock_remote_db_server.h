...
public:
     ...
     * ConnectionString class by using bongo::MockConnRegistry as follows:
     *
     ...
     * less than or equal to the current one will throw a bongo::SocketException.
     ...
     */
    void setCommandReply(const std::string& cmdName, const bongo::BSONObj& replyObj);
    ...
    void setCommandReply(const std::string& cmdName,
                         const std::vector<bongo::BSONObj>& replySequence);
    ...
    bongo::BSONArray query(InstanceID id,
                           const std::string& ns,
                           bongo::Query query = bongo::Query(),
                           int nToReturn = 0,
                           int nToSkip = 0,
                           const bongo::BSONObj* fieldsToReturn = 0,
                           int queryOptions = 0,
                           int batchSize = 0);
    ...
    bongo::ConnectionString::ConnectionType type() const;
...
private:
    ...
    public:
         ...
         */
        CircularBSONIterator(const std::vector<bongo::BSONObj>& replyVector);
        bongo::BSONObj next();

    private:
        std::vector<bongo::BSONObj>::iterator _iter;
        std::vector<bongo::BSONObj> _replyObjs;
    };

    /**
     * Checks whether the instance of the server is still up.
     *
     * @throws bongo::SocketException if this server is down
     */
