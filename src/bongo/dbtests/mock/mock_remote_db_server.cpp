...
MockRemoteDBServer::CircularBSONIterator::CircularBSONIterator(const vector<BSONObj>& replyVector) {
    for (std::vector<bongo::BSONObj>::const_iterator iter = replyVector.begin();
...
void MockRemoteDBServer::setCommandReply(const string& cmdName, const bongo::BSONObj& replyObj) {
...
rpc::UniqueReply MockRemoteDBServer::runCommandWithMetadata(MockRemoteDBServer::InstanceID id,
    ...
    if (_delayMilliSec > 0) {
        bongo::sleepmillis(_delayMilliSec);
    }
...
bongo::BSONArray MockRemoteDBServer::query(MockRemoteDBServer::InstanceID id,
                                           const string& ns,
                                           bongo::Query query,
                                           int nToReturn,
                                           int nToSkip,
...
bongo::ConnectionString::ConnectionType MockRemoteDBServer::type() const {
...
void MockRemoteDBServer::checkIfUp(InstanceID id) const {
    ...
    if (!_isRunning || id < _instanceID) {
        throw bongo::SocketException(bongo::SocketException::CLOSED, _hostAndPort);
    }
}
}
