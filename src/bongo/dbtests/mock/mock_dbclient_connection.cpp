...
#include "bongo/dbtests/mock/mock_dbclient_cursor.h"
...
MockDBClientConnection::MockDBClientConnection(MockRemoteDBServer* remoteServer, bool autoReconnect)
    : _remoteServerInstanceID(remoteServer->getInstanceID()),
      ...
      _isFailed(false),
      _sockCreationTime(bongo::curTimeMicros64()),
...
bool MockDBClientConnection::runCommand(const string& dbname,
    ...
    } catch (const bongo::SocketException&) {
        _isFailed = true;
        throw;
    }
...
std::unique_ptr<bongo::DBClientCursor> MockDBClientConnection::query(const string& ns,
                                                                     bongo::Query query,
                                                                     int nToReturn,
                                                                     int nToSkip,
    ...
    try {
        bongo::BSONArray result(_remoteServer->query(_remoteServerInstanceID,
                                                     ns,
                                                     query,
                                                     nToReturn,
                                                     nToSkip,
        ...
        std::unique_ptr<bongo::DBClientCursor> cursor;
    ...
    std::unique_ptr<bongo::DBClientCursor> nullPtr;
    return nullPtr;
}

bongo::ConnectionString::ConnectionType MockDBClientConnection::type() const {
    return bongo::ConnectionString::CUSTOM;
}
...
unsigned long long MockDBClientConnection::query(
    stdx::function<void(bongo::DBClientCursorBatchIterator&)> f,
    const std::string& ns,
    bongo::Query query,
    const bongo::BSONObj* fieldsToReturn,
    int queryOptions) {
    verify(false);
    return 0;
}
...
bool MockDBClientConnection::call(bongo::Message& toSend,
                                  bongo::Message& response,
                                  bool assertOk,
                                  string* actualServer) {
    verify(false);  // unimplemented
    return false;
...
void MockDBClientConnection::say(bongo::Message& toSend, bool isRetry, string* actualServer) {
    verify(false);  // unimplemented
}
