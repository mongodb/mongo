...
#include "bongo/dbtests/mock/mock_dbclient_connection.h"
...
using bongo::BSONObj;
using bongo::ConnectionString;
using bongo::MockDBClientConnection;
using bongo::MockRemoteDBServer;
using bongo::Query;

using std::string;
using std::vector;

namespace bongo_test {
...
TEST(MockDBClientConnTest, InsertAndQuery) {
    ...
    {
        MockDBClientConnection conn(&server);
        std::unique_ptr<bongo::DBClientCursor> cursor = conn.query(ns);
        ASSERT(!cursor->more());
...
TEST(MockDBClientConnTest, QueryWithNoResults) {
    ...
    server.insert(ns, BSON("x" << 1));
    MockDBClientConnection conn(&server);
    std::unique_ptr<bongo::DBClientCursor> cursor = conn.query("other.ns");
...
TEST(MockDBClientConnTest, MultiNSInsertAndQuery) {
    ...
    {
        MockDBClientConnection conn(&server);
        std::unique_ptr<bongo::DBClientCursor> cursor = conn.query(ns1);
    ...
    {
        MockDBClientConnection conn(&server);
        std::unique_ptr<bongo::DBClientCursor> cursor = conn.query(ns2);
    ...
    {
        MockDBClientConnection conn(&server);
        std::unique_ptr<bongo::DBClientCursor> cursor = conn.query(ns3);
...
TEST(MockDBClientConnTest, Shutdown) {
...

    {
        ...
        ASSERT_THROWS(conn.query("test.user"), bongo::SocketException);
    }
...
TEST(MockDBClientConnTest, Restart) {
    ...
    server.shutdown();
    ASSERT_THROWS(conn1.query("test.user"), bongo::SocketException);

    // New connections shouldn't work either
    MockDBClientConnection conn2(&server);
    ASSERT_THROWS(conn2.query("test.user"), bongo::SocketException);
