...
public:
    void run() {
        BSONObjBuilder b;
        b.append("a", bongo::Decimal128("123456789.12345"));
...
private:
    void assertEquals(const BSONObj& expected, const BSONObj& actual, const char* msg) {
        ...
        if (bad) {
            ::bongo::log() << "want:" << expected.jsonString() << " size: " << expected.objsize()
                           << endl;
            ::bongo::log() << "got :" << actual.jsonString() << " size: " << actual.objsize()
                           << endl;
            ::bongo::log() << expected.hexDump() << endl;
            ::bongo::log() << actual.hexDump() << endl;
            ::bongo::log() << msg << endl;
            ::bongo::log() << "orig json:" << this->json();
        }
        ASSERT(!bad);
    }
};
...
// Added for consistency with the bongo shell
