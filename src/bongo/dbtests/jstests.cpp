...
public:
    LogRecordingScope()
        : _logged(false),
          _threadName(bongo::getThreadName()),
          _handle(bongo::logger::globalLogDomain()->attachAppender(
              bongo::logger::MessageLogDomain::AppenderAutoPtr(new Tee(this)))) {}
    ~LogRecordingScope() {
        bongo::logger::globalLogDomain()->detachAppender(_handle);
    }
...
private:
    class Tee : public bongo::logger::MessageLogDomain::EventAppender {
    public:
        ...
        virtual Status append(const logger::MessageEventEphemeral& event) {
            ...
            if (bongo::getThreadName() == _scope->_threadName) {
                _scope->_logged = true;
            }
            return Status::OK();
        }
    ...
    bongo::logger::MessageLogDomain::AppenderHandle _handle;
};
...
public:
    void run() {
        ...
        ASSERT_THROWS(s->invoke("blah.y = 'e'", 0, 0), bongo::UserException);
        ASSERT_THROWS(s->invoke("blah.a = 19;", 0, 0), bongo::UserException);
        ASSERT_THROWS(s->invoke("blah.zz.a = 19;", 0, 0), bongo::UserException);
        ASSERT_THROWS(s->invoke("blah.zz = { a : 19 };", 0, 0), bongo::UserException);
        ASSERT_THROWS(s->invoke("delete blah['x']", 0, 0), bongo::UserException);
...
public:
    void run() {
        ...
        ASSERT_EQUALS(bongo::NumberLong, out.firstElement().type());
...
public:
    void run() {
        ...
        BSONObj out = s->getObject("a");
        ASSERT_EQUALS(bongo::NumberDecimal, out.firstElement().type());
...
public:
    ...
    void run() {
        if (!getGlobalScriptEngine()->utf8Ok()) {
            bongo::unittest::log() << "warning: utf8 not supported" << endl;
            return;
        }
...
class TimestampMax : public TestRoundTrip {
    virtual BSONObj bson() const {
        BSONObjBuilder b;
        b.appendMaxForType("a", bongo::bsonTimestamp);
        BSONObj o = b.obj();
        return o;
    }
    virtual string json() const {
