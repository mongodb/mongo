...
TEST(SystemError, Category) {
    ASSERT(make_error_code(ErrorCodes::AuthenticationFailed).category() == bongoErrorCategory());
    ASSERT(std::error_code(ErrorCodes::AlreadyInitialized, bongoErrorCategory()).category() ==
           bongoErrorCategory());
...
TEST(SystemError, Conversions) {
    ...
    ASSERT(std::error_code(ErrorCodes::AlreadyInitialized, bongoErrorCategory()) ==
