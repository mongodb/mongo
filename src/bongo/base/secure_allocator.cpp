...
#define BONGO_MAP_ANONYMOUS MAP_ANONYMOUS
#else
#if defined(MAP_ANON)
#define BONGO_MAP_ANONYMOUS MAP_ANON
#endif
#endif

#if !defined(BONGO_MAP_ANONYMOUS)
...
void* systemAllocate(std::size_t bytes) {
    ...
    auto ptr =
        mmap(nullptr, bytes, PROT_READ | PROT_WRITE, MAP_PRIVATE | BONGO_MAP_ANONYMOUS, -1, 0);
 ...
 */
class Allocation {
    BONGO_DISALLOW_COPYING(Allocation);
...
BONGO_INITIALIZER_GENERAL(SecureAllocator, ("SystemInfo"), BONGO_NO_DEPENDENTS)
