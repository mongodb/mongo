...
template <typename T>
class StatusWith {
    BONGO_STATIC_ASSERT_MSG(!(std::is_same<T, bongo::Status>::value),
...
public:
     ...
     * for the error case
     */
    BONGO_COMPILER_COLD_FUNCTION StatusWith(ErrorCodes::Error code,
                                            std::string reason,
                                            int location = 0)
    ...
    BONGO_COMPILER_COLD_FUNCTION StatusWith(ErrorCodes::Error code,
                                            const bongoutils::str::stream& reason,
                                            int location = 0)
        : _status(code, reason, location) {}
     ...
     * for the error case
     */
    BONGO_COMPILER_COLD_FUNCTION StatusWith(Status status) : _status(std::move(status)) {
        dassert(!isOK());
    }
