 ...
 */
class BongoErrorCategoryImpl final : public std::error_category {
public:
    BongoErrorCategoryImpl() = default;
...
const std::error_category& bongoErrorCategory() {
    // TODO: Remove this static, and make a constexpr instance when we move to C++14.
    static const BongoErrorCategoryImpl instance{};
    return instance;
}

std::error_code make_error_code(ErrorCodes::Error code) {
    return std::error_code(ErrorCodes::fromInt(code), bongoErrorCategory());
}

std::error_condition make_error_condition(ErrorCodes::Error code) {
    return std::error_condition(ErrorCodes::fromInt(code), bongoErrorCategory());
}
