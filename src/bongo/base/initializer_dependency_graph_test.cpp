...
#include "bongo/base/initializer_dependency_graph.h"
...
#define ADD_INITIALIZER(GRAPH, NAME, FN, PREREQS, DEPS) \
    (GRAPH).addInitializer(                             \
        (NAME), (FN), BONGO_MAKE_STRING_VECTOR PREREQS, BONGO_MAKE_STRING_VECTOR DEPS)
...
TEST(InitializerDependencyGraphTest, InsertNullFunctionFails) {
    ...
    ASSERT_EQUALS(
        ErrorCodes::BadValue,
        ADD_INITIALIZER(
            graph, "A", InitializerFunction(), BONGO_NO_PREREQUISITES, BONGO_NO_DEPENDENTS));
}

TEST(InitializerDependencyGraphTest, InsertSameNameTwiceFails) {
    InitializerDependencyGraph graph;
    ASSERT_ADD_INITIALIZER(graph, "A", doNothing, BONGO_NO_PREREQUISITES, BONGO_NO_DEPENDENTS);
    ASSERT_EQUALS(
        ErrorCodes::DuplicateKey,
        ADD_INITIALIZER(graph, "A", doNothing, BONGO_NO_PREREQUISITES, BONGO_NO_DEPENDENTS));
}
...
TEST(InitializerDependencyGraphTest, TopSortGraphNoDeps) {
    ...
    std::vector<std::string> nodeNames;
    ASSERT_ADD_INITIALIZER(graph, "A", doNothing, BONGO_NO_PREREQUISITES, BONGO_NO_DEPENDENTS);
    ASSERT_ADD_INITIALIZER(graph, "B", doNothing, BONGO_NO_PREREQUISITES, BONGO_NO_DEPENDENTS);
    ASSERT_ADD_INITIALIZER(graph, "C", doNothing, BONGO_NO_PREREQUISITES, BONGO_NO_DEPENDENTS);
    ASSERT_EQUALS(Status::OK(), graph.topSort(&nodeNames));
...
TEST(InitializerDependencyGraphTest, TopSortWithDiamondPrerequisites) {
    ...
    std::vector<std::string> nodeNames;
    ASSERT_ADD_INITIALIZER(graph, "A", doNothing, BONGO_NO_PREREQUISITES, BONGO_NO_DEPENDENTS);
    ASSERT_ADD_INITIALIZER(graph, "D", doNothing, ("B", "C"), BONGO_NO_DEPENDENTS);
    ASSERT_ADD_INITIALIZER(graph, "B", doNothing, ("A"), BONGO_NO_DEPENDENTS);
    ASSERT_ADD_INITIALIZER(graph, "C", doNothing, ("A"), BONGO_NO_DEPENDENTS);
    ASSERT_EQUALS(Status::OK(), graph.topSort(&nodeNames));
...
TEST(InitializerDependencyGraphTest, TopSortWithDiamondDependents) {
    ...
    InitializerDependencyGraph graph;
    std::vector<std::string> nodeNames;
    ASSERT_ADD_INITIALIZER(graph, "A", doNothing, BONGO_NO_PREREQUISITES, ("B", "C"));
    ASSERT_ADD_INITIALIZER(graph, "D", doNothing, BONGO_NO_PREREQUISITES, BONGO_NO_DEPENDENTS);
    ASSERT_ADD_INITIALIZER(graph, "B", doNothing, BONGO_NO_PREREQUISITES, ("D"));
    ASSERT_ADD_INITIALIZER(graph, "C", doNothing, BONGO_NO_PREREQUISITES, ("D"));
    ASSERT_EQUALS(Status::OK(), graph.topSort(&nodeNames));
    ASSERT_EQUALS(4U, nodeNames.size());
    ASSERT_EXACTLY_ONE_IN_CONTAINER(nodeNames, "A");
    ASSERT_EXACTLY_ONE_IN_CONTAINER(nodeNames, "B");
...
TEST(InitializerDependencyGraphTest, TopSortWithDiamondGeneral2) {
    ...
    std::vector<std::string> nodeNames;
    ASSERT_ADD_INITIALIZER(graph, "A", doNothing, BONGO_NO_PREREQUISITES, ("B", "C"));
    ASSERT_ADD_INITIALIZER(graph, "D", doNothing, ("C", "B"), BONGO_NO_DEPENDENTS);
    ASSERT_ADD_INITIALIZER(graph, "B", doNothing, BONGO_NO_PREREQUISITES, BONGO_NO_DEPENDENTS);
    ASSERT_ADD_INITIALIZER(graph, "C", doNothing, BONGO_NO_PREREQUISITES, BONGO_NO_DEPENDENTS);
    ASSERT_EQUALS(Status::OK(), graph.topSort(&nodeNames));
    ASSERT_EQUALS(4U, nodeNames.size());
...
TEST(InitializerDependencyGraphTest, TopSortFailsWhenMissingDependent) {
    ...
    InitializerDependencyGraph graph;
    std::vector<std::string> nodeNames;
    ASSERT_ADD_INITIALIZER(graph, "A", doNothing, BONGO_NO_PREREQUISITES, ("B"));
    ASSERT_EQUALS(ErrorCodes::BadValue, graph.topSort(&nodeNames));
}
