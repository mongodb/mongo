...
#include "bongo/util/bongoutils/str.h"  // for str::stream()!
...
TEST(ParseNumber, Int8) {
    ...
    for (int32_t i = -128; i <= 127; ++i)
        ASSERT_PARSES(int8_t, std::string(bongoutils::str::stream() << i), i);
}
...
TEST(ParseNumber, UInt8) {
    ...
    for (uint32_t i = 0; i <= 255; ++i)
        ASSERT_PARSES(uint8_t, std::string(bongoutils::str::stream() << i), i);
}
