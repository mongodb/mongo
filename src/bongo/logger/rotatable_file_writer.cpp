...
class Win32FileStreambuf : public std::streambuf {
    BONGO_DISALLOW_COPYING(Win32FileStreambuf);
...
class Win32FileOStream : public std::ostream {
    BONGO_DISALLOW_COPYING(Win32FileOStream);
        ...
        if (renameOnRotate) {
            try {
                if (boost::filesystem::exists(renameTarget)) {
                    return Status(
                        ...
                        bongoutils::str::stream() << "Renaming file " << _writer->_fileName
                                                  << " to "
                                                  << renameTarget
                ...
                return Status(
                    ErrorCodes::FileRenameFailed,
                    bongoutils::str::stream() << "Renaming file " << _writer->_fileName << " to "
                                              << renameTarget
            ...
            if (ec) {
                return Status(ErrorCodes::FileRenameFailed,
                              bongoutils::str::stream() << "Failed  to rename \""
                                                        << _writer->_fileName
                                                        << "\" to \""
                                                        << renameTarget
                                                        << "\": "
...
Status RotatableFileWriter::Use::status() {
    if (!_writer->_stream) {
        return Status(ErrorCodes::FileNotOpen,
                      bongoutils::str::stream() << "File \"" << _writer->_fileName
                                                << "\" not open");
    }
