...
StatusWith<ProtocolSetAndWireVersionInfo> parseProtocolSetFromIsMasterReply(
    ...
    // BongoDB 2.4 and earlier do not have maxWireVersion/minWireVersion in their 'isMaster' replies
    ...
    return {{(!isBongos && supportsWireVersionForOpCommandInBongod(version))
                 ? supports::kAll
                 : supports::kOpQueryOnly,
             version}};
}

bool supportsWireVersionForOpCommandInBongod(const WireVersionInfo version) {
    // FIND_COMMAND versions support OP_COMMAND (in bongod but not bongos).
