...
StatusWith<std::tuple<bool, BSONObj>> unwrapCommand(const BSONObj& maybeWrapped) {
    ...
    BSONElement inner;
    auto extractStatus =
        bsonExtractTypedField(maybeWrapped, firstElFieldName, bongo::Object, &inner);
...
Status extractWrappedReadPreference(const BSONObj& wrappedCommand, BSONObjBuilder* metadataBob) {
    ...
    auto rpExtractStatus =
        bsonExtractTypedField(wrappedCommand, kReadPreferenceFieldName, bongo::Object, &readPrefEl);
...
Status extractUnwrappedReadPreference(const BSONObj& unwrappedCommand,
    ...
    auto queryOptionsExtractStatus = bsonExtractTypedField(
        unwrappedCommand, kQueryOptionsFieldName, bongo::Object, &queryOptionsEl);
    ...
    auto rpExtractStatus = bsonExtractTypedField(
        queryOptionsEl.embeddedObject(), kReadPreferenceFieldName, bongo::Object, &readPrefEl);
...
StatusWith<ServerSelectionMetadata> ServerSelectionMetadata::readFromMetadata(
    ...
    } else if (metadataElem.type() != bongo::Object) {
...
Status ServerSelectionMetadata::downconvert(const BSONObj& command,
    ...
    if (ssmElem.eoo()) {
        ...
        *legacyQueryFlags &= ~bongo::QueryOption_SlaveOk;
        legacyCommand->appendElements(command);
        return Status::OK();
    } else if (ssmElem.type() != bongo::Object) {
        return {
    ...
    if (!readPreferenceElem.eoo()) {
        ...
        // TODO: this may need to use the $queryOptions hack on bongos.
...
Status ServerSelectionMetadata::upconvert(const BSONObj& legacyCommand,
    ...
    if (wasWrapped) {
        ...
        // If the command was not wrapped, we need to check for a readPreference sent by bongos
