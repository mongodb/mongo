...
BONGO_FP_DECLARE(failAsyncConfigChangeHook);
...
ReplicaSetMonitor::ReplicaSetMonitor(const BongoURI& uri)
...
shared_ptr<ReplicaSetMonitor> ReplicaSetMonitor::createIfNeeded(const BongoURI& uri) {
...
Status Refresher::receivedIsMasterFromMaster(const HostAndPort& from, const IsMasterReply& reply) {
    ...
    if (reply.normalHosts != _set->seedNodes) {
        ...
        if (asyncConfigChangeHook && !BONGO_FAIL_POINT(failAsyncConfigChangeHook)) {
        ...
        switch (ns.step) {
            ...
            case NextStep::CONTACT_HOST: {
                ...
                if (_set->setUri.isValid()) {
                    ...
                    targetURI = BongoURI(ConnectionString(ns.host));
                }
...
SetState::SetState(const BongoURI& uri)
