...
/** Represents a Bongo query expression.  Typically one uses the QUERY(...) macro to construct a
 * Query object.
    Examples:
...
public:
    ...
    */
     ...
     * results.
     *  Normally it is easier to use the bongo shell to run db.find(...).explain().
     */
    Query& explain();
    ...
    */
    ...
    /** Queries to the Bongo database support a $where parameter option which contains
        ...
        Your query may also contain other traditional Bongo query terms.
