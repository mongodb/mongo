...
class DummyServiceEntryPoint : public ServiceEntryPoint {
    BONGO_DISALLOW_COPYING(DummyServiceEntryPoint);
...
private:
     ...
     * bongod or bongos) as opposed to an external one (e.g. the shell).
     */
...
TEST_F(DummyServerFixture, ScopedDbConnectionWithTimeout) {
    ...
    auto uri_sw = BongoURI::parse("bongodb://" + TARGET_HOST + "/?socketTimeoutMS=4000");
    ...
    log() << "Testing BongoURI with explicit timeout";
    start = Date_t::now();
    ...
    log() << "Testing BongoURI doesn't timeout";
    start = Date_t::now();
