...
const char kBongoDBURL[] =
    ...
    // database: matches anything but the chars that cannot be part of a BongoDB database name which
...
StatusWith<BongoURI> BongoURI::parse(const std::string& url) {
    if (!boost::algorithm::starts_with(url, "bongodb://")) {
        auto cs_status = ConnectionString::parse(url);
        if (!cs_status.isOK()) {
            return cs_status.getStatus();
        }

        return BongoURI(cs_status.getValue());
    }

    const boost::regex bongoUrlRe(kBongoDBURL);

    boost::smatch matches;
    if (!boost::regex_match(url, matches, bongoUrlRe)) {
        return Status(ErrorCodes::FailedToParse,
                      str::stream() << "Failed to parse bongodb:// URL: " << url);
    }
    ...
    if (matches[5].matched) {
        ...
        if (optionsTokens.size() % 2 != 0) {
            return Status(ErrorCodes::FailedToParse,
                          str::stream()
                              << "Missing a key or value in the options for bongodb:// URL: "
                              << url);
            ;
        }
