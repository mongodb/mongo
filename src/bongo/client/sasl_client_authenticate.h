 ...
 * client application must have successfully executed bongo::runGlobalInitializersOrDie() or its
 ...
 *         information is stored outside of the bongo cluster.
 *     "pwd": The password.
 *     "serviceName": The GSSAPI service name to use.  Defaults to "bongodb".
...
/// or "$external" if the credential information is stored outside of the bongo cluster.
extern const char* const saslCommandUserDBFieldName;

/// Field overriding the FQDN of the hostname hosting the bongodb srevice in
/// saslClientAuthenticate().
extern const char* const saslCommandServiceHostnameFieldName;

/// Field overriding the name of the bongodb service saslClientAuthenticate().
...
/// Default sasl service name, "bongodb".
