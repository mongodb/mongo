...
void HandleLastError(const sasl_utils_t* utils, DWORD errCode, const char* msg) {
    ...
    std::string buffer(bongoutils::str::stream() << "SSPI: " << msg << ": " << err);
 ...
 * created.
 */
BONGO_INITIALIZER_WITH_PREREQUISITES(SaslSspiClientPlugin,
...
(InitializerContext*) {
    ...
    if (SASL_OK != ret) {
        return Status(ErrorCodes::UnknownError,
                      bongoutils::str::stream() << "could not add SASL Client SSPI plugin "
                                                << sspiPluginName
                                                << ": "
...
BONGO_INITIALIZER_WITH_PREREQUISITES(SaslCramClientPlugin,
                                     ("CyrusSaslAllocatorsAndMutexes", "CyrusSaslClientContext"))
(InitializerContext*) {
    int ret = sasl_client_add_plugin("CRAMMD5", crammd5_client_plug_init);
    if (SASL_OK != ret) {
        return Status(ErrorCodes::UnknownError,
                      bongoutils::str::stream() << "Could not add SASL Client CRAM-MD5 plugin "
                                                << sspiPluginName
                                                << ": "
                                                << sasl_errstring(ret, NULL, NULL));
    }
...
BONGO_INITIALIZER_WITH_PREREQUISITES(SaslPlainClientPlugin,
                                     ("CyrusSaslAllocatorsAndMutexes", "CyrusSaslClientContext"))
(InitializerContext*) {
    int ret = sasl_client_add_plugin("PLAIN", plain_client_plug_init);
    if (SASL_OK != ret) {
        return Status(ErrorCodes::UnknownError,
                      bongoutils::str::stream() << "Could not add SASL Client PLAIN plugin "
                                                << sspiPluginName
                                                << ": "
                                                << sasl_errstring(ret, NULL, NULL));
    }
