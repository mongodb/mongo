...
#include "bongo/client/cyrus_sasl_client_session.h"
 ...
 */
BONGO_INITIALIZER(CyrusSaslAllocatorsAndMutexes)(InitializerContext*) {
 ...
 * initialization, it should implement a BONGO_INITIALIZER_GENERAL with
 * CyrusSaslAllocatorsAndMutexes as a prerequisite and CyrusSaslClientContext as a
 * dependent.  If it wishes to override both, it should implement a BONGO_INITIALIZER_GENERAL
 * with CyrusSaslAllocatorsAndMutexes and CyrusSaslClientContext as dependents, or
 * initialize the library before calling bongo::runGlobalInitializersOrDie().
 */
BONGO_INITIALIZER_WITH_PREREQUISITES(CyrusSaslClientContext,
...
(InitializerContext* context) {
    ...
    if (result != SASL_OK) {
        return Status(ErrorCodes::UnknownError,
                      bongoutils::str::stream() << "Could not initialize sasl client components ("
 ...
 *
 * Note that in Bongo, the authentication and authorization ids (authid and authzid) are always
...
Status CyrusSaslClientSession::initialize() {
    ...
    if (SASL_OK != result) {
        return Status(ErrorCodes::UnknownError,
                      bongoutils::str::stream() << sasl_errstring(result, NULL, NULL));
    }
