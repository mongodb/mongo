 ...
 */
class AuthClientTest : public bongo::unittest::Test {
public:
    AuthClientTest()
        : _mockHost(),
          _millis(100),
    ...
    BSONObj loadBongoCRConversation() {
...
TEST_F(AuthClientTest, BongoCR) {
    auto params = loadBongoCRConversation();
    auth::authenticateClient(std::move(params), HostAndPort(), "", _runCommandCallback);
}

TEST_F(AuthClientTest, asyncBongoCR) {
    auto params = loadBongoCRConversation();
