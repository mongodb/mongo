// dbclient.cpp - connect to a Bongo database as a database, from C++
...
#include "bongo/client/authenticate.h"
#include "bongo/client/constants.h"
...
#include "bongo/s/stale_exception.h"  // for RecvStaleConfigException
...
#include "bongo/util/net/asio_message_port.h"
#include "bongo/util/net/message_port.h"
#include "bongo/util/net/message_port_startup_param.h"
...
string DBClientWithCommands::createPasswordDigest(const string& username,
    ...
    return bongo::createPasswordDigest(username, clearTextPassword);
}

namespace {
class ScopedMetadataWriterRemover {
    BONGO_DISALLOW_COPYING(ScopedMetadataWriterRemover);
...
executor::RemoteCommandResponse initWireVersion(DBClientConnection* conn,
    ...
    try {
        ...
        // where we reconnect to an older version of BongoDB running at the same host/port.
        ...
        if (Command::testCommandsEnabled) {
            ...
            // commands are enabled. bongobridge uses this field to identify the process opening a
            // connection to it.
            StringBuilder sb;
        ...
        Status serializeStatus = ClientMetadata::serialize(
            "BongoDB Internal Client", versionString, applicationName, &bob);
