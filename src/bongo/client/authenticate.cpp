...
const char* const kMechanismBongoCR = "BONGODB-CR";
const char* const kMechanismBongoX509 = "BONGODB-X509";
...
AuthRequest createBongoCRGetNonceCmd(const BSONObj& params) {
...
AuthRequest createBongoCRAuthenticateCmd(const BSONObj& params, StringData nonce) {
    std::string username;
...
void authBongoCR(RunCommandHook runCommand, const BSONObj& params, AuthCompletionHandler handler) {
    invariant(runCommand);
    invariant(handler);

    // Step 1: send getnonce command, receive nonce
    auto nonceRequest = createBongoCRGetNonceCmd(params);
    ...
    runCommand(nonceRequest.getValue(), [runCommand, params, handler](AuthResponse response) {
        ...
        auto authRequest = createBongoCRAuthenticateCmd(params, nonce);
        if (!authRequest.isOK())
...
AuthRequest createX509AuthCmd(const BSONObj& params, StringData clientName) {
    if (clientName.empty()) {
        return {ErrorCodes::AuthenticationFailed,
                "Please enable SSL on the client-side to use the BONGODB-X509 authentication "
                "mechanism."};
    }
    auto db = extractDBField(params);
    if (!db.isOK())
...
// Use the BONGODB-X509 protocol to authenticate as "username." The certificate details
...
void auth(RunCommandHook runCommand,
    ...
    if (mechanism == kMechanismBongoCR)
        return authBongoCR(runCommand, params, authCompletionHandler);

#ifdef BONGO_CONFIG_SSL
    else if (mechanism == kMechanismBongoX509)
