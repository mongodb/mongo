 ...
 *
 * Format B (unofficial internal format from bongos):
...
ReadPreferenceSetting* _extractReadPref(const BSONObj& query, int queryOptions) {
    ...
    ReadPreference pref = queryOptions & QueryOption_SlaveOk
        ...
        : bongo::ReadPreference::PrimaryOnly;
...
DBClientConnection* DBClientReplicaSet::checkMaster() {
    ...
    BongoURI masterUri;
    if (!_uri.isValid())
        masterUri = BongoURI(ConnectionString(_masterHost));
    else
...
DBClientConnection* DBClientReplicaSet::selectNodeUsingTags(
    ...
    // versioned in bongos. Therefore, we have to make sure that this object
    ...
    if (_authPooledSecondaryConn) {
        ...
        // Bongos pooled connections are authenticated through
...
void DBClientReplicaSet::resetSlaveOkConn() {
    if (_lastSlaveOkConn.get() == _master.get()) {
        ...
        if (_authPooledSecondaryConn) {
            ...
            // Bongos pooled connections are all authenticated with the same credentials;
