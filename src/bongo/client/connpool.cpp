...
DBClientBase* DBConnectionPool::get(const BongoURI& uri, double socketTimeout) {
...
void DBConnectionPool::appendConnectionStats(executor::ConnectionPoolStats* stats) const {
    {
        ...
        for (PoolMap::const_iterator i = _pools.begin(); i != _pools.end(); ++i) {
            ...
            // Bongos may use either a replica set uri or a list of addresses as
...
ScopedDbConnection::ScopedDbConnection(const BongoURI& uri, double socketTimeout)
    : _host(uri.toString()),
