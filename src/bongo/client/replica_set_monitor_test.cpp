...
TEST(ReplicaSetMonitor, InitialStateBongoURI) {
    auto uri = BongoURI::parse("bongodb://a,b,c/?replicaSet=name");
...
TEST(ReplicaSetMonitor, IsMasterBadParse) {
    BSONObj ismaster = BSON("hosts" << BSON_ARRAY("bongo.example:badport"));
    IsMasterReply imr(HostAndPort("bongo.example:27017"), -1, ismaster);
...
TEST(ReplicaSetMonitor, IsMasterReplyRSPrimary) {
    ...
    IsMasterReply imr(HostAndPort("bongo.example:3000"), -1, ismaster);

    ASSERT_EQUALS(imr.ok, true);
    ASSERT_EQUALS(imr.host.toString(), HostAndPort("bongo.example:3000").toString());
    ...
    ASSERT_EQUALS(imr.primary.toString(), HostAndPort("bongo.example:3000").toString());
    ASSERT(imr.normalHosts.count(HostAndPort("bongo.example:3000")));
    ASSERT(imr.tags.isEmpty());
}
...
TEST(ReplicaSetMonitor, IsMasterReplyPassiveSecondary) {
    ...
    IsMasterReply imr(HostAndPort("bongo.example:3001"), -1, ismaster);

    ASSERT_EQUALS(imr.ok, true);
    ASSERT_EQUALS(imr.host.toString(), HostAndPort("bongo.example:3001").toString());
    ASSERT_EQUALS(imr.setName, "test");
    ...
    ASSERT_EQUALS(imr.primary.toString(), HostAndPort("bongo.example:3000").toString());
    ASSERT(imr.normalHosts.count(HostAndPort("bongo.example:3000")));
    ASSERT(imr.normalHosts.count(HostAndPort("bongo.example:3001")));
    ASSERT(imr.tags.isEmpty());
    ASSERT(!imr.electionId.isSet());
}
