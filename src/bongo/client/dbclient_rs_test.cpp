...
protected:
    void setUp() {
        ...
        ConnectionString::setConnectionHook(bongo::MockConnRegistry::get()->getConnStrHook());
    }
    ...
    void tearDown() {
        ...
        bongo::ScopedDbConnection::clearPool();
    }
...
TEST_F(BasicRS, QueryPrimary) {
    ...
    Query query;
    query.readPref(bongo::ReadPreference::PrimaryOnly, BSONArray());
...
TEST_F(BasicRS, QuerySecondaryOnly) {
    ...
    Query query;
    query.readPref(bongo::ReadPreference::SecondaryOnly, BSONArray());
...
TEST_F(BasicRS, QueryPrimaryPreferred) {
    ...
    Query query;
    query.readPref(bongo::ReadPreference::PrimaryPreferred, BSONArray());
...
TEST_F(BasicRS, QuerySecondaryPreferred) {
    ...
    Query query;
    query.readPref(bongo::ReadPreference::SecondaryPreferred, BSONArray());
...
TEST_F(AllNodesDown, QueryNearest) {
    ...
    Query query;
    query.readPref(bongo::ReadPreference::Nearest, BSONArray());
    ASSERT_THROWS(replConn.query(IdentityNS, query), AssertionException);
}
...
protected:
    void setUp() {
        ...
        {
            bongo::repl::ReplicaSetConfig oldConfig = _replSet->getReplConfig();

            bongo::BSONObjBuilder newConfigBuilder;
            newConfigBuilder.append("_id", oldConfig.getReplSetName());
            newConfigBuilder.append("version", oldConfig.getConfigVersion());

            bongo::BSONArrayBuilder membersBuilder(newConfigBuilder.subarrayStart("members"));
            {
                const string host(_replSet->getPrimary());
                const bongo::repl::MemberConfig* member =
            ...
            membersBuilder.done();
            bongo::repl::ReplicaSetConfig newConfig;
...
TEST_F(TaggedFiveMemberRS, ConnShouldNotPinIfDiffTag) {
    ...
    {
        Query query;
        query.readPref(bongo::ReadPreference::SecondaryPreferred,
                       BSON_ARRAY(BSON("dc"
                                       << "sf")));
...

    {
        ...
        query.readPref(bongo::ReadPreference::SecondaryPreferred, BSON_ARRAY(BSON("group" << 1)));
        unique_ptr<DBClientCursor> cursor = replConn.query(IdentityNS, query);
        BSONObj doc = cursor->next();
        const string newDest = doc[HostField.name()].str();
        ASSERT_NOT_EQUALS(dest, newDest);
...
TEST_F(TaggedFiveMemberRS, SlaveConnReturnsSecConn) {
    ...
    string dest;
    bongo::DBClientConnection& secConn = replConn.slaveConn();
