...
boost::optional<HostAndPort> extractHostInfo(const rpc::RequestInterface& request) {
    // The initial isMaster request made by bongod and bongos processes should contain a hostInfo
...
public:
    ...
    void operator()() {
        ...
        {
            HostAndPort destAddr{bongoBridgeGlobalParams.destUri};
            ...
            while (true) {
                ...
                if (elapsed >= kConnectTimeout) {
                    warning() << "Unable to establish connection to "
                              << bongoBridgeGlobalParams.destUri << " after " << elapsed
                              << " seconds: " << status;
        ...
        while (true) {
            try {
                ...
                // Handle a message intended to configure the bongobridge and return a response.
                // The 'request' is consumed by the bongobridge and does not get forwarded to
                // 'dest'.
...
public:
    BridgeListener()
        : Listener("bridge", "", bongoBridgeGlobalParams.port, getGlobalServiceContext(), false),
          _seedSource(bongoBridgeGlobalParams.seed) {
        log() << "Setting random seed: " << bongoBridgeGlobalParams.seed;
    }
...
std::unique_ptr<bongo::BridgeListener> listener;
...
int wmain(int argc, wchar_t* argvW[], wchar_t* envpW[]) {
    bongo::WindowsCommandLine wcl(argc, argvW, envpW);
    int exitCode = bongo::bridgeMain(argc, wcl.argv(), wcl.envp());
    bongo::quickExit(exitCode);
}
#else
int main(int argc, char* argv[], char** envp) {
    int exitCode = bongo::bridgeMain(argc, argv, envp);
    bongo::quickExit(exitCode);
}
#endif
