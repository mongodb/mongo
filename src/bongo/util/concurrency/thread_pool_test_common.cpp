...
using ThreadPoolTestCaseFactory =
    stdx::function<std::unique_ptr<::bongo::unittest::Test>(ThreadPoolFactory)>;
...
class TptRegistrationAgent {
    BONGO_DISALLOW_COPYING(TptRegistrationAgent);
...
class TptDeathRegistrationAgent {
    BONGO_DISALLOW_COPYING(TptDeathRegistrationAgent);
...
public:
    TptDeathRegistrationAgent(const std::string& name, ThreadPoolTestCaseFactory makeTest) {
        ...
        entry = [makeTest](ThreadPoolFactory makeThreadPool) {
            return stdx::make_unique<::bongo::unittest::DeathTest<T>>(std::move(makeThreadPool));
        };
    }
};
