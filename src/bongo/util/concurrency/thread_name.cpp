...
// the execution of bongo initializers (which only happens once we have entered main, and
// therefore after dynamic initialization is complete) to signal that it is safe to use
// 'threadName'.
bool bongoInitializersHaveRun{};
BONGO_INITIALIZER(ThreadNameInitializer)(InitializerContext*) {
    bongoInitializersHaveRun = true;
...
void setThreadName(StringData name) {
    invariant(bongoInitializersHaveRun);
...
#elif defined(__linux__) && defined(BONGO_CONFIG_HAVE_PTHREAD_SETNAME_NP)
...
const string& getThreadName() {
    if (BONGO_unlikely(!bongoInitializersHaveRun)) {
