...
#define BONGO_ISO_DATE_FMT_NO_TZ "%Y-%m-%dT%H:%M:%S"
...
void _dateToISOString(Date_t date, bool local, DateStringBuffer* result) {
    ...
    int pos = strftime(buf, bufSize, BONGO_ISO_DATE_FMT_NO_TZ, &t);
    dassert(0 < pos);
    char* cur = buf + pos;
...
#undef BONGO_ISO_DATE_FMT_NO_TZ
...
BONGO_INITIALIZER(Init32TimeSupport)(InitializerContext*) {
