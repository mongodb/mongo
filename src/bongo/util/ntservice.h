...
/**
 * The ntservice namespace provides minimal support for running bongo servers as NT services.
 *
...
/**
 * Construct an argv array that Windows should use to start bongod/bongos as a service
 * if bongo was started with "inputArgv", which is assumed to be an argument vector that
 * dictates that Windows should install bongo as a service.
 *
 * The result is suitable for passing to bongo::constructUtf8WindowsCommandLine() to construct
 ...
 */
BONGO_COMPILER_NORETURN void startService();
