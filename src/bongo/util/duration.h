...
ToDuration duration_cast(const Duration<FromPeriod>& from) {
    ...
    if (ToDuration::template isHigherPrecisionThan<Duration<FromPeriod>>()) {
        ...
        uassert(ErrorCodes::DurationOverflow,
                ...
                !bongoSignedMultiplyOverflow64(from.count(), FromOverTo::num, &toCount));
...
class Duration {
public:
    BONGO_STATIC_ASSERT_MSG(Period::num > 0, "Duration::period's numerator must be positive");
    BONGO_STATIC_ASSERT_MSG(Period::den > 0, "Duration::period's denominator must be positive");
    ...
    struct IsHigherPrecisionThan {
        ...
        BONGO_STATIC_ASSERT_MSG(
            ...
            "Bongo duration types are only compatible with each other when one's period "
    ...
    int compare(const Duration<OtherPeriod>& other) const {
        ...
        rep otherCount;
        if (bongoSignedMultiplyOverflow64(other.count(), OtherOverThis::num, &otherCount)) {
    ...
    Duration& operator+=(const Duration& other) {
        uassert(ErrorCodes::DurationOverflow,
                ...
                !bongoSignedAddOverflow64(count(), other.count(), &_count));
        return *this;
    }
    ...
    Duration& operator-=(const Duration& other) {
        uassert(ErrorCodes::DurationOverflow,
                ...
                !bongoSignedSubtractOverflow64(count(), other.count(), &_count));
        return *this;
    }
    ...
    Duration& operator*=(const Rep2& scale) {
        ...
        uassert(ErrorCodes::DurationOverflow,
                ...
                !bongoSignedMultiplyOverflow64(count(), scale, &_count));
        return *this;
    }

    template <typename Rep2>
    Duration& operator/=(const Rep2& scale) {
        BONGO_STATIC_ASSERT_MSG(std::is_integral<Rep2>::value && std::is_signed<Rep2>::value,
