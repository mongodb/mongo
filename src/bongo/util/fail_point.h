 ...
 * return false || BONGO_FAIL_POINT(makeBadThingsHappen);
 *
 * or
 *
 * // Somewhere in the code
 * BONGO_FAIL_POINT_BLOCK(makeBadThingsHappen, blockMakeBadThingsHappen) {
 ...
 */
class FailPoint {
    BONGO_DISALLOW_COPYING(FailPoint);
...
public:
    ...
    inline RetCode shouldFailOpenBlock() {
        if (BONGO_likely((_fpInfo.loadRelaxed() & ACTIVE_BIT) == 0)) {
            return fastOff;
        }
 ...
 * BONGO_FAIL_POINT_BLOCK macro.
 */
class ScopedFailPoint {
    BONGO_DISALLOW_COPYING(ScopedFailPoint);
...
#define BONGO_FAIL_POINT(symbol) BONGO_unlikely(symbol.shouldFail())

#define BONGO_FAIL_POINT_PAUSE_WHILE_SET(symbol) \
    do {                                         \
        while (BONGO_FAIL_POINT(symbol)) {       \
            sleepmillis(100);                    \
        }                                        \
    } while (false)
 ...
 */
#define BONGO_FAIL_POINT_BLOCK(symbol, blockSymbol) \
    for (bongo::ScopedFailPoint blockSymbol(&symbol); BONGO_unlikely(blockSymbol.isActive());)
}
