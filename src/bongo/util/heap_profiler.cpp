...
#if defined(_POSIX_VERSION) && defined(BONGO_CONFIG_HAVE_EXECINFO_BACKTRACE)
...
//     bongod --setParameter heapProfilingEnabled=true
//
...
//    }
//
// Each new stack encountered is also logged to bongod log with a message like
...
class HashTable {
    BONGO_DISALLOW_COPYING(HashTable);
...
private:
    ...
    struct Stack {
        ...
        Hash hash() {
            Hash hash;
            BONGO_STATIC_ASSERT_MSG(sizeof(frames) == sizeof(FrameInfo) * kMaxFramesPerStack,
...
BONGO_INITIALIZER_GENERAL(StartHeapProfiling, ("EndStartupOptionHandling"), ("default"))
...
#endif  // BONGO_HAVE_HEAP_PROFILER
