...
#if defined(BONGO_UTIL_LOG_H_)
#error \
    "bongo/util/log.h cannot be included multiple times. " \
       "This may occur when log.h is included in a header. " \
       "Please check your #include's."
#else  // BONGO_UTIL_LOG_H_
#define BONGO_UTIL_LOG_H_
...
// Provide log component in global scope so that BONGO_LOG will always have a valid component.
// Global log component will be kDefault unless overridden by BONGO_LOG_DEFAULT_COMPONENT.
#if defined(BONGO_LOG_DEFAULT_COMPONENT)
const ::bongo::logger::LogComponent BongoLogDefaultComponent_component =
    BONGO_LOG_DEFAULT_COMPONENT;
#else
#error \
    "bongo/util/log.h requires BONGO_LOG_DEFAULT_COMPONENT to be defined. " \
       "Please see http://www.bongodb.org/about/contributors/reference/server-logging-rules/ "
#endif  // BONGO_LOG_DEFAULT_COMPONENT
...
inline LogstreamBuilder severe() {
    return LogstreamBuilder(logger::globalLogDomain(),
                            ...
                            ::BongoLogDefaultComponent_component);
}
...
inline bool shouldLog(logger::LogSeverity severity) {
    return logger::globalLogDomain()->shouldLog(::BongoLogDefaultComponent_component, severity);
}

}  // namespace

// BONGO_LOG uses log component from BongoLogDefaultComponent from current or global namespace.
#define BONGO_LOG(DLEVEL)                                                              \
    if (!(::bongo::logger::globalLogDomain())                                          \
             ->shouldLog(BongoLogDefaultComponent_component,                           \
                         ::bongo::LogstreamBuilder::severityCast(DLEVEL))) {           \
    } else                                                                             \
...
#define LOG BONGO_LOG

#define BONGO_LOG_COMPONENT(DLEVEL, COMPONENT1)                                            \
    if (!(::bongo::logger::globalLogDomain())                                              \
             ->shouldLog((COMPONENT1), ::bongo::LogstreamBuilder::severityCast(DLEVEL))) { \
    } else                                                                                 \
    ::bongo::logger::LogstreamBuilder(::bongo::logger::globalLogDomain(),                  \
                                      ::bongo::getThreadName(),                            \
                                      ::bongo::LogstreamBuilder::severityCast(DLEVEL),     \
...
#define BONGO_LOG_COMPONENT2(DLEVEL, COMPONENT1, COMPONENT2)                                     \
    if (!(::bongo::logger::globalLogDomain())                                                    \
             ->shouldLog(                                                                        \
                 (COMPONENT1), (COMPONENT2), ::bongo::LogstreamBuilder::severityCast(DLEVEL))) { \
    } else                                                                                       \
    ::bongo::logger::LogstreamBuilder(::bongo::logger::globalLogDomain(),                        \
                                      ::bongo::getThreadName(),                                  \
                                      ::bongo::LogstreamBuilder::severityCast(DLEVEL),           \
...
#define BONGO_LOG_COMPONENT3(DLEVEL, COMPONENT1, COMPONENT2, COMPONENT3)               \
    if (!(::bongo::logger::globalLogDomain())                                          \
