...
#undef BONGO_PCH_WHITELISTED  // for malloc/realloc pulled from bson

#include "bongo/bson/bsonobj.h"
#include "bongo/bson/bsontypes.h"
...
using bongo::SafeNum;
using bongo::Decimal128;

TEST(Basics, Initialization) {
    const SafeNum numInt(0);
    ASSERT_EQUALS(numInt.type(), bongo::NumberInt);

    const SafeNum numLong(static_cast<int64_t>(0));
    ASSERT_EQUALS(numLong.type(), bongo::NumberLong);

    const SafeNum numDouble(0.0);
    ASSERT_EQUALS(numDouble.type(), bongo::NumberDouble);

    const SafeNum numDecimal(Decimal128("1.0"));
    ASSERT_EQUALS(numDecimal.type(), bongo::NumberDecimal);
}
...
TEST(Comparison, EOO) {
    ...
    ASSERT_EQUALS(safeNumA.type(), bongo::EOO);
    ASSERT_EQUALS(safeNumB.type(), bongo::EOO);
...
TEST(Addition, UpConvertion) {
    ...
    ASSERT_EQUALS((zeroInt32 + zeroInt64).type(), bongo::NumberLong);
    ASSERT_EQUALS((zeroInt64 + zeroInt32).type(), bongo::NumberLong);
    ASSERT_EQUALS((zeroInt32 + zeroDouble).type(), bongo::NumberDouble);
    ASSERT_EQUALS((zeroInt64 + zeroDouble).type(), bongo::NumberDouble);
    ...
    ASSERT_EQUALS(stillInt32.type(), bongo::NumberInt);
    ASSERT_EQUALS(stillInt64.type(), bongo::NumberLong);
    ASSERT_EQUALS(stillDouble.type(), bongo::NumberDouble);

    const SafeNum zeroDecimal(Decimal128(0));
    ASSERT_EQUALS((zeroInt64 + zeroDecimal).type(), bongo::NumberDecimal);
    ASSERT_EQUALS((zeroInt32 + zeroDecimal).type(), bongo::NumberDecimal);
    ASSERT_EQUALS((zeroDouble + zeroDecimal).type(), bongo::NumberDecimal);
    ASSERT_EQUALS((zeroDecimal + zeroInt32).type(), bongo::NumberDecimal);
    ASSERT_EQUALS((zeroDecimal + zeroInt64).type(), bongo::NumberDecimal);
    ASSERT_EQUALS((zeroDecimal + zeroDouble).type(), bongo::NumberDecimal);

    const SafeNum stillDecimal(zeroDecimal + zeroDecimal);
    ASSERT_EQUALS(stillDecimal.type(), bongo::NumberDecimal);
}

TEST(Addition, Overflow32to64) {
    const SafeNum maxInt32(std::numeric_limits<int32_t>::max());
    ASSERT_EQUALS(maxInt32.type(), bongo::NumberInt);

    const SafeNum int32PlusOne(maxInt32 + 1);
    ASSERT_EQUALS(int32PlusOne.type(), bongo::NumberLong);

    const SafeNum int32MinusOne(maxInt32 + -1);
    ASSERT_EQUALS(int32MinusOne.type(), bongo::NumberInt);
...
TEST(Addition, Negative32to64) {
    ...
    ASSERT_EQUALS(minInt32.type(), bongo::NumberInt);

    const SafeNum int32MinusOne(minInt32 + -1);
    ASSERT_EQUALS(int32MinusOne.type(), bongo::NumberLong);

    const SafeNum int32PlusOne(minInt32 + 1);
    ASSERT_EQUALS(int32PlusOne.type(), bongo::NumberInt);
...
TEST(Addition, Negative64toDouble) {
    ...
    ASSERT_EQUALS(minInt64.type(), bongo::NumberLong);

    // We don't overflow int64 to double.
    const SafeNum int64MinusOne(minInt64 + -1);
    ASSERT_EQUALS(int64MinusOne.type(), bongo::EOO);

    const SafeNum int64PlusOne(minInt64 + 1);
    ASSERT_EQUALS(int64PlusOne.type(), bongo::NumberLong);
...
TEST(BitAnd, 32and32) {
    ...
    ASSERT_EQUALS(bongo::NumberInt, result.type());
...
TEST(BitAnd, 64and64) {
    ...
    const SafeNum result = val1 & val2;
    ASSERT_EQUALS(bongo::NumberLong, result.type());
    ASSERT_TRUE(expected.isIdentical(result));
}
...
TEST(BitAnd, MixedSize) {
    ...
    ASSERT_EQUALS(bongo::NumberLong, result_s_b.type());
    ASSERT_TRUE(expected.isIdentical(result_s_b));

    ASSERT_EQUALS(bongo::NumberLong, result_b_s.type());
...
TEST(Multiplication, UpConvertion) {
    ...
    ASSERT_EQUALS((zeroInt32 * zeroDouble).type(), bongo::NumberDouble);
    ASSERT_EQUALS((zeroInt64 * zeroDouble).type(), bongo::NumberDouble);
    ASSERT_EQUALS((zeroDouble * zeroInt32).type(), bongo::NumberDouble);
    ASSERT_EQUALS((zeroDouble * zeroInt64).type(), bongo::NumberDouble);
...
TEST(Multiplication, Overflow32to64) {
    ...
    ASSERT_EQUALS(int32TimesOne.type(), bongo::NumberInt);

    const SafeNum int32TimesTwo(maxInt32 * 2);
    ASSERT_EQUALS(int32TimesTwo.type(), bongo::NumberLong);
}
...
TEST(Multiplication, Overflow64toDouble) {
    ...
    ASSERT_EQUALS(int64TimesTwo.type(), bongo::EOO);
...
TEST(Multiplication, Negative64toDouble) {
    ...
    ASSERT_EQUALS(int64TimesOne.type(), bongo::NumberLong);
...
TEST(Multiplication, 64OverflowsFourWays) {
    const SafeNum maxInt64(std::numeric_limits<int64_t>::max());
    const SafeNum minInt64(std::numeric_limits<int64_t>::min());
    ASSERT_EQUALS(bongo::EOO, (maxInt64 * maxInt64).type());
    ASSERT_EQUALS(bongo::EOO, (maxInt64 * minInt64).type());
    ASSERT_EQUALS(bongo::EOO, (minInt64 * maxInt64).type());
    ASSERT_EQUALS(bongo::EOO, (minInt64 * minInt64).type());
}
...
TEST(Multiplication, BoundsWithNegativeOne) {
    ...
    ASSERT_NOT_EQUALS(bongo::EOO, (maxInt64 * minusOneInt64).type());
    ASSERT_NOT_EQUALS(bongo::EOO, (minusOneInt64 * maxInt64).type());
    ASSERT_EQUALS(bongo::EOO, (minInt64 * minusOneInt64).type());
    ASSERT_EQUALS(bongo::EOO, (minusOneInt64 * minInt64).type());
}
