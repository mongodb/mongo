...
#include "bongo/util/concurrency/value.h"
...
#include "bongo/util/net/socket_poll.h"
...
BONGO_FP_DECLARE(throwSockExcep);
...
string makeUnixSockPath(int port) {
    return bongoutils::str::stream() << serverGlobalParams.socket << "/bongodb-" << port << ".sock";
}
...
string prettyHostName() {
    ...
    if (serverGlobalParams.port != ServerGlobalParams::DefaultDBPort)
        s << ':' << bongo::serverGlobalParams.port;
    return s.str();
}
...
void Socket::send(const char* data, int len, const char* context) {
    while (len > 0) {
        int ret = -1;
        if (BONGO_FAIL_POINT(throwSockExcep)) {
#if defined(_WIN32)
...
#if defined(_WIN32)
    const int bongo_errno = WSAGetLastError();
    if (bongo_errno == WSAETIMEDOUT && _timeout != 0) {
#else
    const int bongo_errno = errno;
    if ((bongo_errno == EAGAIN || bongo_errno == EWOULDBLOCK) && _timeout != 0) {
#endif
        LOG(_logLevel) << "Socket " << context << " send() timed out " << remoteString();
        throw SocketException(SocketException::SEND_TIMEOUT, remoteString());
    } else if (bongo_errno != EINTR) {
        LOG(_logLevel) << "Socket " << context << " send() " << errnoWithDescription(bongo_errno)
                       << ' ' << remoteString();
        throw SocketException(SocketException::SEND_ERROR, remoteString());
    }
}
