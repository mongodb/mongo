...
static void disableNonblockingMode(SOCKET socket) {
    ...
    if (status == SOCKET_ERROR) {
        ...
        error() << "Windows WSAIoctl returned " << errnoWithDescription(bongo_errno);
        fassertFailed(16726);
    }
}
...
public:
    EventHolder() {
        ...
        if (_socketEventHandle == WSA_INVALID_EVENT) {
            const int bongo_errno = WSAGetLastError();
            error() << "Windows WSACreateEvent returned " << errnoWithDescription(bongo_errno);
            fassertFailed(16728);
        }
    }
    ~EventHolder() {
        ...
        if (bstatus == FALSE) {
            const int bongo_errno = WSAGetLastError();
            error() << "Windows WSACloseEvent returned " << errnoWithDescription(bongo_errno);
            fassertFailed(16725);
        }
    }
    WSAEVENT get() {
...
void Listener::initAndListen() {
    ...
    while (!globalInShutdownDeprecated()) {
            ...
            if (status == SOCKET_ERROR) {
                ...
                if (!globalInShutdownDeprecated()) {
                    error() << "Windows WSAEventSelect returned "
                            << errnoWithDescription(bongo_errno);
                }
        ...
        if (status == SOCKET_ERROR) {
            const int bongo_errno = WSAGetLastError();
            error() << "Windows WSAEventSelect returned " << errnoWithDescription(bongo_errno);
            continue;
        }
