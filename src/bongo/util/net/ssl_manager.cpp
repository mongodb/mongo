...
#include "bongo/util/net/ssl_expiration.h"
...
// clang-format off
#ifndef BONGO_CONFIG_HAVE_ASN1_ANY_DEFINITIONS
...
#endif // BONGO_CONFIG_NEEDS_ASN1_ANY_DEFINITIONS
// clang-format on
...
private:
    const int _rolesNid = OBJ_create(bongodbRolesOID.identifier.c_str(),
                                     bongodbRolesOID.shortDescription.c_str(),
                                     bongodbRolesOID.longDescription.c_str());
     ...
     */
    BONGO_COMPILER_NORETURN void _handleSSLError(int code, int ret);
...
#if defined(BONGO_CONFIG_HAVE_FIPS_MODE_SET)
...
#else
    severe() << "this version of bongodb was not compiled with FIPS support";
...
BONGO_INITIALIZER(SetupOpenSSL)(InitializerContext*) {
    SSL_library_init();
...
BONGO_INITIALIZER_WITH_PREREQUISITES(SSLManager, ("SetupOpenSSL"))(InitializerContext*) {
...
void canonicalizeClusterDN(std::vector<std::string>* dn) {
    ...
    for (size_t i = 0; i < dn->size(); i++) {
        ...
        if (!bongoutils::str::startsWith(comp.c_str(), "DC=") &&
            !bongoutils::str::startsWith(comp.c_str(), "O=") &&
            !bongoutils::str::startsWith(comp.c_str(), "OU=")) {
            dn->erase(dn->begin() + i);
            i--;
        }
    }
...
StatusWith<boost::optional<SSLPeerInfo>> SSLManager::parseAndValidatePeerCertificate(
    ...
    // If this is an SSL client context (on a BongoDB server or client)
...
StatusWith<stdx::unordered_set<RoleName>> SSLManager::_parsePeerRoles(X509* peerCert) const {
    ...
    for (int i = 0; i < extCount; i++) {
        ...
        if (!OBJ_cmp(obj, rolesObj)) {
            ...
            /*
             * BongoDBAuthorizationGrant ::= CHOICE {
             *  BongoDBRole,
             *  ...!UTF8String:"Unrecognized entity in BongoDBAuthorizationGrant"
             * }
             * BongoDBAuthorizationGrants ::= SET OF BongoDBAuthorizationGrant
             */
            // Extract the set of roles from our extension, and load them into an OpenSSL stack.
            STACK_OF(ASN1_TYPE)* bongoDBAuthorizationGrants = nullptr;
            ...
            bongoDBAuthorizationGrants =
                d2i_ASN1_SET_ANY(&bongoDBAuthorizationGrants, &dataBytes, data->length);
            if (!bongoDBAuthorizationGrants) {
                return Status(ErrorCodes::FailedToParse,
                              "Failed to parse x509 authorization grants");
            }
            const auto grantGuard = MakeGuard([&bongoDBAuthorizationGrants]() {
                sk_ASN1_TYPE_pop_free(bongoDBAuthorizationGrants, ASN1_TYPE_free);
            });

            /*
             * BongoDBRole ::= SEQUENCE {
             *  role     UTF8String,
             *  database UTF8String
             * }
             */
            // Loop through every role in the stack.
            ASN1_TYPE* BongoDBRoleWrapped = nullptr;
            while ((BongoDBRoleWrapped = sk_ASN1_TYPE_pop(bongoDBAuthorizationGrants))) {
                const auto roleWrappedGuard =
                    MakeGuard([BongoDBRoleWrapped]() { ASN1_TYPE_free(BongoDBRoleWrapped); });

                if (BongoDBRoleWrapped->type == V_ASN1_SEQUENCE) {
                    // Unwrap the ASN1Type into a STACK_OF(ASN1_TYPE)
                    unsigned char* roleBytes = ASN1_STRING_data(BongoDBRoleWrapped->value.sequence);
                    int roleBytesLength = ASN1_STRING_length(BongoDBRoleWrapped->value.sequence);
                    ASN1_SEQUENCE_ANY* BongoDBRole = nullptr;
                    BongoDBRole = d2i_ASN1_SEQUENCE_ANY(
                        &BongoDBRole, (const unsigned char**)&roleBytes, roleBytesLength);
                    if (!BongoDBRole) {
                        return Status(ErrorCodes::FailedToParse,
                                      "Failed to parse role in x509 authorization grant");
                    }
                    const auto roleGuard = MakeGuard(
                        [&BongoDBRole]() { sk_ASN1_TYPE_pop_free(BongoDBRole, ASN1_TYPE_free); });

                    if (sk_ASN1_TYPE_num(BongoDBRole) != 2) {
                        return Status(ErrorCodes::FailedToParse,
                                      "Role entity in BongoDBAuthorizationGrant must have exactly "
                                      "2 sequence elements");
                    }
                    // Extract the subcomponents of the sequence, which are popped off the stack in
                    // reverse order. Here, parse the role's database.
                    ASN1_TYPE* roleComponent = sk_ASN1_TYPE_pop(BongoDBRole);
                    const auto roleDBGuard =
                        MakeGuard([roleComponent]() { ASN1_TYPE_free(roleComponent); });
                    if (roleComponent->type != V_ASN1_UTF8STRING) {
                        return Status(ErrorCodes::FailedToParse,
                                      "database in BongoDBRole must be a UTF8 string");
                    }
                    std::string roleDB(
                        reinterpret_cast<char*>(ASN1_STRING_data(roleComponent->value.utf8string)));

                    // Parse the role's name.
                    roleComponent = sk_ASN1_TYPE_pop(BongoDBRole);
                    const auto roleNameGuard =
                        MakeGuard([roleComponent]() { ASN1_TYPE_free(roleComponent); });
                    if (roleComponent->type != V_ASN1_UTF8STRING) {
                        return Status(ErrorCodes::FailedToParse,
                                      "role in BongoDBRole must be a UTF8 string");
                    }
                    std::string roleName(
                        reinterpret_cast<char*>(ASN1_STRING_data(roleComponent->value.utf8string)));
                    ...
                    return Status(ErrorCodes::FailedToParse,
                                  "Unrecognized entity in BongoDBAuthorizationGrant");
                }
            }
            LOG(1) << "BONGODB-X509 authorization parsed the following roles from peer "
                      "certificate: "
                   << [&roles]() {
                          StringBuilder sb;
