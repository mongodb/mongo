...
/**
 * This include exists so that bongo/base/status_with.h can use the invariant macro without causing
 * a circular include chain. It should never be included directly in any other file other than that
 * one (and assert_util.h).
 */

#if !defined(BONGO_INCLUDE_INVARIANT_H_WHITELISTED)
#error "Include assert_util.h instead of invariant.h."
#endif

BONGO_COMPILER_NORETURN void invariantFailed(const char* expr,
                                             const char* file,
                                             unsigned line) noexcept;

#define BONGO_invariant(_Expression)                                    \
    do {                                                                \
        if (BONGO_unlikely(!(_Expression))) {                           \
            ::bongo::invariantFailed(#_Expression, __FILE__, __LINE__); \
        }                                                               \
    } while (false)

#define invariant BONGO_invariant
...
#define BONGO_dassert(x) \
    if (kDebugBuild)     \
    invariant(x)

#define dassert BONGO_dassert
