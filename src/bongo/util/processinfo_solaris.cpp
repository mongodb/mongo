...
struct ProcPsinfo {
    ProcPsinfo() {
        ...
        massert(16846,
                bongoutils::str::stream() << "couldn't open \"/proc/self/psinfo\": "
        ...
        fclose(f);
        massert(16847,
                bongoutils::str::stream() << "couldn't read from \"/proc/self/psinfo\": "
...
struct ProcUsage {
    ProcUsage() {
        ...
        massert(16848,
                bongoutils::str::stream() << "couldn't open \"/proc/self/usage\": "
                                          << errnoWithDescription(),
                f);
        ...
        fclose(f);
        massert(16849,
                bongoutils::str::stream() << "couldn't read from \"/proc/self/usage\": "
                                          << errnoWithDescription(err),
                num == 1);
    }
    prusage_t prusage;
...
void ProcessInfo::SystemInfo::collectSystemInfo() {
    ...
    if (sysinfo(SI_ARCHITECTURE_64, buf_64, sizeof(buf_64)) != -1 &&
        ...
        addrSize = bongoutils::str::equals(buf_64, buf_native) ? 64 : 32;
    } else {
        log() << "Unable to determine system architecture: " << strerror(errno);
    }

    osType = unameData.sysname;
    osName = bongoutils::str::ltrim(readLineFromFile("/etc/release"));
    ...
    if (bongoutils::str::startsWith(osName, "Oracle Solaris")) {
