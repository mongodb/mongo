...
// This implementation assumes that inputArgv was a valid argv to bongod.  That is, it assumes
...
std::vector<std::string> constructServiceArgv(const std::vector<std::string>& inputArgv) {
    ...
    for (std::vector<std::string>::const_iterator iter = inputArgv.begin(), end = inputArgv.end();
        ...
        std::string name;
        std::string value;
        bool foundEqualSign = bongoutils::str::splitOn(*iter, '=', name, value);
        if (!foundEqualSign)
            name = *iter;
...
void installServiceOrDie(const wstring& serviceName,
    ...
    if (!serviceInstalled) {
#else
    // This code sets the bongod service to auto-restart, forever. This might be a fine thing to do
    // except that when bongod or Windows has a crash, the bongo.lock file is still around, so any
