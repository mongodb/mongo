...
using bongo::BSONObj;
using bongo::FailPoint;
namespace stdx = bongo::stdx;
...
TEST(FailPoint, AlwaysOn) {
    ...
    BONGO_FAIL_POINT_BLOCK(failPoint, scopedFp) {
...
class FailPointStress : public bongo::unittest::Test {
public:
    void setUp() {
...
private:
    void blockTask() {
        while (true) {
            BONGO_FAIL_POINT_BLOCK(_fp, scopedFp) {
                const bongo::BSONObj& data = scopedFp.getData();

                // Expanded ASSERT_EQUALS since the error is not being
                // printed out properly
                if (data["a"].numberInt() != 44) {
                    bongo::error() << "blockTask thread detected anomaly"
                                   << " - data: " << data << std::endl;
                    ASSERT(false);
                }
            }
        ...
        while (true) {
            try {
                BONGO_FAIL_POINT_BLOCK(_fp, scopedFp) {
                    ...
                    if (data["a"].numberInt() != 44) {
                        bongo::error() << "blockWithExceptionTask thread detected anomaly"
                                       << " - data: " << data << std::endl;
                        ASSERT(false);
                    }
    ...
    void simpleTask() {
        while (true) {
            static_cast<void>(BONGO_FAIL_POINT(_fp));
            stdx::lock_guard<stdx::mutex> lk(_mutex);
            if (_inShutdown)
                break;
        }
