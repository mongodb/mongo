...
class VersionInfoInterface {
    BONGO_DISALLOW_COPYING(VersionInfoInterface);
...
public:
    ...
    /**
     * Returns the major version as configured via BONGO_VERSION.
     */
    virtual int majorVersion() const noexcept = 0;

    /**
     * Returns the minor version as configured via BONGO_VERSION.
     */
    virtual int minorVersion() const noexcept = 0;

    /**
     * Returns the patch version as configured via BONGO_VERSION.
     */
    virtual int patchVersion() const noexcept = 0;

    /**
     * Returns the extra version as configured via BONGO_VERSION.
     */
    virtual int extraVersion() const noexcept = 0;

    /**
     * Returns a string representation of BONGO_VERSION.
     */
    virtual StringData version() const noexcept = 0;

    /**
     * Returns a string representation of BONGO_GIT_HASH.
     */
 ...
 */
std::string bongoShellVersion(const VersionInfoInterface& provider);

/**
 * Returns a pretty string describing the current bongos version.
 */
std::string bongosVersion(const VersionInfoInterface& provider);

/**
 * Returns a pretty string describing the current bongod version.
 */
std::string bongodVersion(const VersionInfoInterface& provider);
