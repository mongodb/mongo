...
#define BONGO_ASYNC_OP_INVARIANT(_Expression, _Error)  \
    do {                                               \
...
NetworkInterfaceASIO::AsyncConnection& NetworkInterfaceASIO::AsyncOp::connection() {
    BONGO_ASYNC_OP_INVARIANT(_connection.is_initialized(), "Connection not yet initialized");
    return *_connection;
}

void NetworkInterfaceASIO::AsyncOp::setConnection(AsyncConnection&& conn) {
    BONGO_ASYNC_OP_INVARIANT(!_connection.is_initialized(), "Connection already initialized");
...
Status NetworkInterfaceASIO::AsyncOp::beginCommand(Message&& newCommand,
    ...
    BONGO_ASYNC_OP_INVARIANT(_connection.is_initialized(),
...
NetworkInterfaceASIO::AsyncCommand* NetworkInterfaceASIO::AsyncOp::command() {
    BONGO_ASYNC_OP_INVARIANT(_command.is_initialized(), "Command is not yet initialized");
...
rpc::Protocol NetworkInterfaceASIO::AsyncOp::operationProtocol() const {
    BONGO_ASYNC_OP_INVARIANT(_operationProtocol.is_initialized(), "Protocol not yet set");
    return *_operationProtocol;
}

void NetworkInterfaceASIO::AsyncOp::setOperationProtocol(rpc::Protocol proto) {
    BONGO_ASYNC_OP_INVARIANT(!_operationProtocol.is_initialized(), "Protocol already set");
...
void NetworkInterfaceASIO::AsyncOp::_transitionToState_inlock(AsyncOp::State newState) {
    ...
    for (int i = 0; i < kMaxStateTransitions; i++) {
        ...
        BONGO_ASYNC_OP_INVARIANT(_states[i] != newState,
        ...
        if (_states[i] == State::kNoState) {
            ...
            switch (newState) {
                ...
                case State::kTimedOut:
                    ...
                    break;
                case State::kCanceled:
                    BONGO_ASYNC_OP_INVARIANT(
                        i > 1, _stateToString(newState) + " must come after kInProgress");
                    BONGO_ASYNC_OP_INVARIANT(_states[i - 1] != State::kUninitialized,
                                             _stateToString(newState) +
                                                 " cannot come after kUninitialized");
                    break;
                case State::kFinished:
                    BONGO_ASYNC_OP_INVARIANT(i > 0, "kFinished must come after kUninitialized");
                    break;
                default:
                    BONGO_UNREACHABLE;
            }
