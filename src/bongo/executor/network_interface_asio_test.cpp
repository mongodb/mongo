...
#include "bongo/executor/async_mock_stream_factory.h"
#include "bongo/executor/async_timer_mock.h"
#include "bongo/executor/network_interface_asio.h"
#include "bongo/executor/network_interface_asio_test_utils.h"
#include "bongo/executor/test_network_connection_hook.h"
...
RemoteCommandResponse simulateIsMaster(RemoteCommandRequest request) {
    ...
    response.data = BSON("minWireVersion" << bongo::WireSpec::instance().incoming.minWireVersion
                                          << "maxWireVersion"
                                          << bongo::WireSpec::instance().incoming.maxWireVersion);
    return response;
}

class NetworkInterfaceASIOTest : public bongo::unittest::Test {
public:
    void setUp() override {
        initWireSpecBongoD();
...
TEST_F(NetworkInterfaceASIOConnectionHookTest, ValidateHostInvalid) {
    ...
    stream->simulateServer(
        rpc::Protocol::kOpQuery, [](RemoteCommandRequest request) -> RemoteCommandResponse {
            RemoteCommandResponse response;
            response.data =
                BSON("minWireVersion" << bongo::WireSpec::instance().incoming.minWireVersion
                                      << "maxWireVersion"
                                      << bongo::WireSpec::instance().incoming.maxWireVersion
                                      << "TESTKEY"
                                      << "TESTVALUE");
