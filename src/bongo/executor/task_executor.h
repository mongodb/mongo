 ...
 */
class TaskExecutor {
    BONGO_DISALLOW_COPYING(TaskExecutor);
...
public:
    ...
    /**
     * Type of a callback from a request to run a command on a remote BongoDB node.
     *
...
template <>
struct hash<::bongo::executor::TaskExecutor::CallbackHandle> {
    size_t operator()(const ::bongo::executor::TaskExecutor::CallbackHandle& x) const {
        return x.hash();
    }
};
