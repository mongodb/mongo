...
BONGO_FP_DECLARE(NetworkInterfaceASIOasyncRunCommandFail);
...
void asyncRecvMessageBody(AsyncStreamInterface& stream,
    ...
    if (len == 542393671) {
        LOG(3) << "attempt to access BongoDB over HTTP on the native driver port.";
...
void NetworkInterfaceASIO::_networkErrorCallback(AsyncOp* op, const std::error_code& ec) {
    ErrorCodes::Error errorCode = (ec.category() == bongoErrorCategory())
...
void NetworkInterfaceASIO::_completeOperation(AsyncOp* op, ResponseStatus resp) {
    ...
    if (op->_inSetup) {
        ...
        BONGO_ASIO_INVARIANT(!resp.isOK(), "Failed to connect in setup", op);
    ...
    if (op->_inRefresh) {
        ...
        BONGO_ASIO_INVARIANT(!resp.isOK(), "In refresh, but did not fail to heartbeat", op);
    ...
    BONGO_ASIO_INVARIANT(static_cast<bool>(ownedOp), "Invalid AsyncOp", op);
...
void NetworkInterfaceASIO::_asyncRunCommand(AsyncOp* op, NetworkOpHandler handler) {
    ...
    if (BONGO_FAIL_POINT(NetworkInterfaceASIOasyncRunCommandFail)) {
