...
#define BONGO_LOG_DEFAULT_COMPONENT ::bongo::logger::LogComponent::kASIO
...
#include "bongo/executor/async_timer_interface.h"
#include "bongo/executor/async_timer_mock.h"
#include "bongo/executor/connection_pool_asio.h"
...
#include "bongo/util/table_formatter.h"
...
Status NetworkInterfaceASIO::startCommand(const TaskExecutor::CallbackHandle& cbHandle,
    ...
    BONGO_ASIO_INVARIANT(onFinish, "Invalid completion function");
    {
        stdx::lock_guard<stdx::mutex> lk(_inProgressMutex);
        const auto insertResult = _inGetConnection.emplace(cbHandle);
        // We should never see the same CallbackHandle added twice
        BONGO_ASIO_INVARIANT_INLOCK(insertResult.second, "Same CallbackHandle added twice");
    }
    ...
    auto nextStep = [this, getConnectionStartTime, cbHandle, request, onFinish](
        ...
        BONGO_ASIO_INVARIANT_INLOCK(!op->canceled(), "AsyncOp has dirty canceled flag", op);
        BONGO_ASIO_INVARIANT_INLOCK(!op->timedOut(), "AsyncOp has dirty timeout flag", op);
        ...
        op->_strand.post([this, op, getConnectionStartTime] {
            ...
            if (timeout != RemoteCommandRequest::kNoTimeout) {
                ...
                BONGO_ASIO_INVARIANT(timeout > getConnectionDuration, "timeout underflowed", op);
