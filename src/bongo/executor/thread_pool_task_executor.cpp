...
#define BONGO_LOG_DEFAULT_COMPONENT bongo::logger::LogComponent::kExecutor
...
#include "bongo/util/concurrency/thread_pool_interface.h"
...
namespace {
BONGO_FP_DECLARE(scheduleIntoPoolSpinsUntilThreadPoolShutsDown);
}

class ThreadPoolTaskExecutor::CallbackState : public TaskExecutor::CallbackState {
    BONGO_DISALLOW_COPYING(CallbackState);
...
class ThreadPoolTaskExecutor::EventState : public TaskExecutor::EventState {
    BONGO_DISALLOW_COPYING(EventState);
...
void ThreadPoolTaskExecutor::scheduleIntoPool_inlock(WorkQueue* fromQueue,
    ...
    if (BONGO_FAIL_POINT(scheduleIntoPoolSpinsUntilThreadPoolShutsDown)) {
