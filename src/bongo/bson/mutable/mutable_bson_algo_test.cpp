...
using bongo::Status;
using namespace bongo::mutablebson;

class DocumentTest : public bongo::unittest::Test {
public:
    DocumentTest() : _doc() {}
...
TEST(WoLessTest, CollationAware) {
    ...
    Document less(bongo::fromjson("{ x: 'cbc' }"));
    Document greater(bongo::fromjson("{ x: 'abd' }"));
...
TEST(WoEqualTest, CollationAware) {
    ...
    Document docA(bongo::fromjson("{ x: 'not' }"));
    Document docB(bongo::fromjson("{ x: 'equal' }"));
