...
Status bsonExtractField(const BSONObj& object, StringData fieldName, BSONElement* outElement) {
    ...
    if (element.eoo())
        return Status(ErrorCodes::NoSuchKey,
                      bongoutils::str::stream() << "Missing expected field \""
                                                << fieldName.toString()
                                                << "\"");
    *outElement = element;
    return Status::OK();
...
Status bsonExtractTypedField(const BSONObj& object,
    ...
    if (type != outElement->type()) {
        return Status(ErrorCodes::TypeMismatch,
                      bongoutils::str::stream() << "\"" << fieldName
...
Status bsonExtractBooleanFieldWithDefault(const BSONObj& object,
    ...
    if (status == ErrorCodes::NoSuchKey) {
        *out = defaultValue;
        ...
        return Status(ErrorCodes::TypeMismatch,
                      bongoutils::str::stream() << "Expected boolean or number type for field \""
                                                << fieldName
                                                << "\", found "
                                                << typeName(value.type()));
    } else {
...
Status bsonExtractIntegerField(const BSONObj& object, StringData fieldName, long long* out) {
    ...
    if (!value.isNumber()) {
        return Status(ErrorCodes::TypeMismatch,
                      bongoutils::str::stream() << "Expected field \"" << fieldName
...
Status bsonExtractIntegerFieldWithDefaultIf(const BSONObj& object,
    ...
    if (!pred(*out)) {
        return Status(
            ErrorCodes::BadValue,
            bongoutils::str::stream() << "Invalid value in field \"" << fieldName << "\": " << *out
                                      << ": "
                                      << predDescription);
    }
    return Status::OK();
