...
#include "bongo/bson/inline_decls.h"
#include "bongo/platform/decimal128.h"
#include "bongo/stdx/type_traits.h"
#include "bongo/util/allocator.h"
#include "bongo/util/assert_util.h"
#include "bongo/util/shared_buffer.h"
...
class SharedBufferAllocator {
    BONGO_DISALLOW_COPYING(SharedBufferAllocator);
...
class StackAllocator {
    BONGO_DISALLOW_COPYING(StackAllocator);
...
class _BufBuilder {
    BONGO_DISALLOW_COPYING(_BufBuilder);
...
public:
    ...
    */
    ...
    void appendUChar(unsigned char j) {
        BONGO_STATIC_ASSERT(CHAR_BIT == 8);
        appendNumImpl(j);
    }
    ...
    void appendNum(short j) {
        BONGO_STATIC_ASSERT(sizeof(short) == 2);
        appendNumImpl(j);
    }
    void appendNum(int j) {
        BONGO_STATIC_ASSERT(sizeof(int) == 4);
        appendNumImpl(j);
    }
    ...
    void appendNum(double j) {
        BONGO_STATIC_ASSERT(sizeof(double) == 8);
        appendNumImpl(j);
    }
    void appendNum(long long j) {
        BONGO_STATIC_ASSERT(sizeof(long long) == 8);
        appendNumImpl(j);
    }
...
public:
    ...
    static const size_t BONGO_DBL_SIZE = 3 + DBL_MANT_DIG - DBL_MIN_EXP + 1;
    static const size_t BONGO_S32_SIZE = 12;
    static const size_t BONGO_U32_SIZE = 11;
    static const size_t BONGO_S64_SIZE = 23;
    static const size_t BONGO_U64_SIZE = 22;
    static const size_t BONGO_S16_SIZE = 7;
    static const size_t BONGO_PTR_SIZE = 19;  // Accounts for the 0x prefix

    StringBuilderImpl() {}

    StringBuilderImpl& operator<<(double x) {
        return SBNUM(x, BONGO_DBL_SIZE, "%g");
    }
    StringBuilderImpl& operator<<(int x) {
        return SBNUM(x, BONGO_S32_SIZE, "%d");
    }
    StringBuilderImpl& operator<<(unsigned x) {
        return SBNUM(x, BONGO_U32_SIZE, "%u");
    }
    StringBuilderImpl& operator<<(long x) {
        return SBNUM(x, BONGO_S64_SIZE, "%ld");
    }
    StringBuilderImpl& operator<<(unsigned long x) {
        return SBNUM(x, BONGO_U64_SIZE, "%lu");
    }
    StringBuilderImpl& operator<<(long long x) {
        return SBNUM(x, BONGO_S64_SIZE, "%lld");
    }
    StringBuilderImpl& operator<<(unsigned long long x) {
        return SBNUM(x, BONGO_U64_SIZE, "%llu");
    }
    StringBuilderImpl& operator<<(short x) {
        return SBNUM(x, BONGO_S16_SIZE, "%hd");
    }
    StringBuilderImpl& operator<<(const void* x) {
        if (sizeof(x) == 8) {
            return SBNUM(x, BONGO_PTR_SIZE, "0x%llX");
        } else {
            return SBNUM(x, BONGO_PTR_SIZE, "0x%lX");
        }
    }
