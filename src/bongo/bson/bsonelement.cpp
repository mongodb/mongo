...
string BSONElement::jsonString(JsonStringFormat format, bool includeFieldNames, int pretty) const {
    ...
    switch (type()) {
        ...
        case NumberDecimal:
            ...
            // Also, treat -NaN and +NaN as the same thing for BongoDB.
        ...
        case DBRef: {
            ...
            if (format != TenGen)
                s << "\"$id\" : ";
            s << '"' << bongo::OID::from(valuestr() + valuestrsize()) << "\" ";
            if (format == TenGen)
                s << ')';
            else
                s << '}';
...
/* wo = "well ordered"
   note: (bongodb related) : this can only change in behavior when index version # changes
*/
