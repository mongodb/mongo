...
#include "bongo/platform/strtoll.h"
...
#if 0
#define BONGO_JSON_DEBUG(message)                                                          \
...
Status JParse::value(StringData fieldName, BSONObjBuilder& builder) {
    BONGO_JSON_DEBUG("fieldName: " << fieldName);
    if (peekToken(LBRACE)) {
...
bool JParse::readField(StringData expectedField) {
    BONGO_JSON_DEBUG("expectedField: " << expectedField);
...
BSONObj fromjson(const char* jsonString, int* len) {
    BONGO_JSON_DEBUG("jsonString: " << jsonString);
    if (jsonString[0] == '\0') {
        if (len)
            *len = 0;
        return BSONObj();
