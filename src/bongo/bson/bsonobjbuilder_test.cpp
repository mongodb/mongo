...
using bongo::BSONType;
using bongo::BufBuilder;
using std::string;
...
void assertBSONTypeEquals(BSONType actual, BSONType expected, T value, int i) {
    if (expected != actual) {
        ...
        ss << "incorrect type in bson object for " << (i + 1) << "-th test value " << value
           << ". actual: " << bongo::typeName(actual)
           << "; expected: " << bongo::typeName(expected);
...
TEST(BSONObjBuilderTest, AppendUnsignedInt) {
    struct {
        unsigned int v;
        BSONType t;
    } data[] = {{0, bongo::NumberInt},
                {100, bongo::NumberInt},
                {maxEncodableInt, bongo::NumberInt},
                {maxEncodableInt + 1, bongo::NumberInt},
                {static_cast<unsigned int>(maxInt), bongo::NumberInt},
                {static_cast<unsigned int>(maxInt) + 1U, bongo::NumberInt},
                {(std::numeric_limits<unsigned int>::max)(), bongo::NumberInt},
                {0, bongo::Undefined}};
    for (int i = 0; data[i].t != bongo::Undefined; i++) {
...
TEST(BSONObjBuilderTest, AppendIntOrLL) {
    struct {
        ...
        BSONType t;
                ...
                {-100, bongo::NumberInt},
                {100, bongo::NumberInt},
                {-(maxInt / 2 - 1), bongo::NumberInt},
                {maxInt / 2 - 1, bongo::NumberInt},
                {-(maxInt / 2), bongo::NumberLong},
                {maxInt / 2, bongo::NumberLong},
                {minEncodableInt, bongo::NumberLong},
                {maxEncodableInt, bongo::NumberLong},
                {minEncodableInt - 1, bongo::NumberLong},
                {maxEncodableInt + 1, bongo::NumberLong},
                {minInt, bongo::NumberLong},
                {maxInt, bongo::NumberLong},
                {minInt - 1, bongo::NumberLong},
                {maxInt + 1, bongo::NumberLong},
                {minLongLong, bongo::NumberLong},
                {maxLongLong, bongo::NumberLong},
                {0, bongo::Undefined}};
    for (int i = 0; data[i].t != bongo::Undefined; i++) {
...
TEST(BSONObjBuilderTest, AppendNumberSizeT) {
    struct {
        ...
        BSONType t;
                ...
                {maxEncodableInt, bongo::NumberInt},
                {maxEncodableInt + 1, bongo::NumberLong},
                {size_t(maxInt), bongo::NumberLong},
                {size_t(maxInt) + 1U, bongo::NumberLong},
                {(std::numeric_limits<size_t>::max)(), bongo::NumberLong},
                {0, bongo::Undefined}};
    for (int i = 0; data[i].t != bongo::Undefined; i++) {
        size_t v = data[i].v;
        BSONObjBuilder b;
...
TEST(BSONObjBuilderTest, AppendNumberLongLong) {
    struct {
        ...
        BSONType t;
                ...
                {-100, bongo::NumberInt},
                {100, bongo::NumberInt},
                {minEncodableInt, bongo::NumberInt},
                {maxEncodableInt, bongo::NumberInt},
                {minEncodableInt - 1, bongo::NumberDouble},
                {maxEncodableInt + 1, bongo::NumberDouble},
                {minInt, bongo::NumberDouble},
                {maxInt, bongo::NumberDouble},
                {minInt - 1, bongo::NumberDouble},
                {maxInt + 1, bongo::NumberDouble},
                {minEncodableDouble, bongo::NumberDouble},
                {maxEncodableDouble, bongo::NumberDouble},
                {minEncodableDouble - 1, bongo::NumberLong},
                {maxEncodableDouble + 1, bongo::NumberLong},
                {minDouble, bongo::NumberLong},
                {maxDouble, bongo::NumberLong},
                {minDouble - 1, bongo::NumberLong},
                {maxDouble + 1, bongo::NumberLong},
                {minLongLong, bongo::NumberLong},
                {maxLongLong, bongo::NumberLong},
                {0, bongo::Undefined}};
    for (int i = 0; data[i].t != bongo::Undefined; i++) {
        ...
        ASSERT_EQUALS(o.nFields(), 1);
        BSONElement e = o.getField("a");
        if (data[i].t != bongo::NumberDouble) {
            long long n = e.numberLong();
            ASSERT_EQUALS(n, v);
        } else {
...
TEST(BSONObjBuilderTest, AppendMaxTimestampConversion) {
    ...
    bongo::Timestamp timestamp = e.timestamp();
