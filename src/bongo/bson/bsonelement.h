...
public:
    ...
    */
    std::string String() const {
        return chk(bongo::String).str();
    }
    const StringData checkAndGetStringData() const {
        return chk(bongo::String).valueStringData();
    }
    Date_t Date() const {
        return chk(bongo::Date).date();
    }
    double Number() const {
    ...
    */
    ...
    const char* valuestrsafe() const {
        return type() == bongo::String ? valuestr() : "";
    }
    /** Get the std::string value of the element.  If not a std::string returns "". */
    std::string str() const {
        return type() == bongo::String ? std::string(valuestr(), valuestrsize() - 1)
                                       : std::string();
    }
    ...
    */
    ...
    Timestamp timestamp() const {
        if (type() == bongo::Date || type() == bsonTimestamp) {
