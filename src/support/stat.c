...
__wt_stat_alloc_btree_file_stats(SESSION *session, WT_STATS **statsp)
{
	...
	WT_RET(__wt_calloc(session, 24, sizeof(WT_STATS), &stats));

	stats[WT_STAT_ALLOCSIZE].desc = "page size allocation unit";
...
__wt_stat_alloc_method_stats(SESSION *session, WT_STATS **statsp)
{
	...
	stats[WT_STAT_BTREE_SALVAGE].desc = "btree.salvage";
