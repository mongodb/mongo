...
#ifdef _DOXYGEN
/**
 * Embeddable MerizoDB Library.
 *
 * @invariant All functions in this library (those `extern "C"` functions starting with
 * `merizo_embedded_v1_` in their names) have undefined behavior unless their thread safety
 ...
 *
 * The Embedded MerizoDB Library (most `merizo_embedded_v1_` prefixed functions) uses allocated
 * objects
 * of this type to report the details of any failure, when an operation cannot be completed.
 * Several `merizo_embedded_v1_status` functions are provided which permit observing the details of
 * these failures. Further a construction function and a destruction function for these objects are
 * also provided.
 *
 * @invariant The use of `merizo_embedded_v1_status` objects from multiple threads is not threadsafe
 * unless all of the threads accessing a single `merizo_embedded_v1_status` object are passing that
 * object as a const-qualified (`const merizo_embedded_v1_status *`) pointer. If a single thread is
 * passing a `merizo_embedded_v1_status` object a function taking it by non-const-qualified
 * (`merizo_embedded_v1_status *`) pointer, then no other thread may access the
 * `merizo_embedded_v1_status` object.
 *
 * @note All `merizo_embedded_v1_` functions which take a `status` object may be passed a `NULL`
 ...
 *
 * @note All `merizo_embedded_v1_status` functions can be used before the `merizo_embedded_capi`
 * library is initialized. This facilitates detailed error reporting from all library functions.
 */
typedef struct merizo_embedded_v1_status merizo_embedded_v1_status;

/**
 * Allocate and construct an API-return-status buffer object of type `merizo_embedded_v1_status`.
 *
 * All `merizo_embedded_v1_` functions outside of the `merizo_embedded_v1_status` family accept
 * pointers to these objects (specifically a parameter of type `merizo_embedded_v1_status *`). These
 ...
 * @pre None.
 *
 * @returns A pointer to a newly allocated `merizo_embedded_v1_status` object which will hold details
 * of any failures of operations to which it was passed.
 *
 * @returns `NULL` when construction of a `merizo_embedded_v1_status` object fails.
 *
 * @invariant This function is completely threadsafe.
 *
 * @note It is possible to use the rest of the `merizo_embedded_v1` functions without status objects
 * if detailed error reporting is unnecessary; however, if allocation of status objects fails it is
 * likely that all other `merizo_embedded_v1` operations will fail as well.
 *
 * @note Allocation of an Embedded MerizoDB Status buffer should rarely fail, except for
 ...
 *
 * Frees the storage associated with a valid `merizo_embedded_v1_status` object including all shared
 * observable storage, such as strings. The only way that a `merizo_embedded_v1_status` can be
 * validly created is via `merizo_embedded_v1_status_create`, therefore the object being destroyed
 * must have been created using that function.
 *
 * @pre The specified `status` object must not be `NULL`.
 * @pre The specified `status` object must be a valid `merizo_embedded_v1_status` object.
 *
 * @param status The `merizo_embedded_v1_status` object to release.
 *
 * @invariant This function is not threadsafe unless the specified `status` object is not passed
 * concurrently to any other function. It is safe to destroy distinct `merizo_embedded_v1_status`
...
/**
 * The error codes reported by `merizo_embedded_v1` functions will be given the symbolic names as
 * mapped by this enum.
 *
 * When a `merizo_embedded_v1` function fails (and it has been documented to report errors) it will
...
/**
 * Gets an error code from a `merizo_embedded_v1_status` object.
 *
 * When a `merizo_embedded_v1` function fails (and it has been documented to report errors) it will
 * report its error in the form of an `int` status code which is stored into a supplied
 * `merizo_embedded_v1_status` object, if provided. Some of these functions may also report extra
 * information, which will be reported by other observer functions. Every `merizo_embedded_v1`
 * function which reports errors will always update the `Error` code stored in a
 * `merizo_embedded_v1_status` object, even upon success.
 *
 * @pre The specified `status` object must not be `NULL`.
 * @pre The specified `status` object must be a valid `merizo_embedded_v1_status` object.
 * @pre The specified `status` object must have been passed to a `merizo_embedded_v1` function.
 *
 ...
 *
 * @note This function will report the `merizo_embedded_v1_error` value for the failure associated
 * with `status`, therefore if the failing function returned a `merizo_embedded_v1_error` value
...
/**
 * Gets a descriptive error message from a `merizo_embedded_v1_status` object.
 *
 * Any `merizo_embedded_v1` function which reports failure must, when it fails, update the specified
 * `merizo_embedded_v1_status` object, if it exists, to contain a string indicating a user-readable
 ...
 * the next time that the specified `status` is passed to any other `merizo_embedded_v1` function
 * (including those in the `merizo_embedded_v1_status` family).
 *
 * @invariant This function is thread-safe, if the thread safety requirements specified by
 * `merizo_embedded_v1_status`'s invariants are met; however, the pointer returned by this function
 * is considered to be part of the specified `status` object for the purposes of thread safety. If
 * the `merizo_embedded_v1_status` is changed, by any thread, it will invalidate the string returned
 * by this function.
 *
 ...
 * `merizo_embedded_v1_destroy_status`.
 *
 * @note This function does not report its own failures.
 * @note This behavior of this function is undefined unless its preconditions are met.
 */
...
/**
 * Gets a status code from a `merizo_embedded_v1_status` object.
 *
 * Any `merizo_embedded_v1` function which reports failure must, when it fails, update the specified
...
/**
 * An object which describes the runtime state of the Embedded MerizoDB Library.
 *
 * The `merizo_embedded_v1` library uses allocated objects of this type to indicate the present state
 ...
 *
 * @invariant The use of `merizo_embedded_v1_lib` objects from multiple threads is not threadsafe
 * unless all of the threads accessing a single `merizo_embedded_v1_lib` object are not destroying
 * this object.  If a single thread is passing a `merizo_embedded_v1_lib` to its destruction
 * function, then no other thread may access the `merizo_embedded_v1_status` object.
 */
typedef struct merizo_embedded_v1_lib merizo_embedded_v1_lib;

/**
 * An object which describes the runtime state of the Embedded MerizoDB Library.
 *
 * The `merizo_embedded_v1` library uses structures of this type to indicate the desired
 ...
 */
typedef struct merizo_embedded_v1_init_params merizo_embedded_v1_init_params;

/**
 * Log callback. For details on what the parameters mean, see the documentation at
 * https://docs.merizodb.com/manual/reference/log-messages/
 *
...
/**
 * Valid bits for the log_flags bitfield in merizo_embedded_v1_init_params.
 */
typedef enum {
...
struct merizo_embedded_v1_init_params {
    /**
     * Optional null-terminated YAML formatted MerizoDB configuration string.
     * See documentation for valid options.
     */
    const char* yaml_config;

    /**
     * Bitfield of log destinations, accepts values from merizo_embedded_v1_log_flags.
     * Default is stdout.
     */
    uint64_t log_flags;

    /**
     * Optional log callback to the merizo_embedded_capi library, it is not legal to reenter the
     * library from the callback.
     */
    merizo_embedded_v1_log_callback log_callback;
...
/**
 * Initializes the merizo_embedded_capi library; required before any other call except as otherwise
 * noted.
 *
 * The Embedded MerizoDB Library must be initialized before it can be used. However, it is
 * permissible to create and destroy `merizo_embedded_v1_status` objects without the library having
 * been initialized. Initializing the library sets up internal state for all Embedded MerizoDB
 * Library operations, including creating embedded "server-like" instances and creating clients.
 *
 * @pre The specified `params` object must either be a valid `merizo_embedded_v1_lib_params` object
 * (in a valid state) or `NULL`.
 *
 * @pre The specified `status` object must either be a valid `merizo_embedded_v1_status` object or
 * `NULL`.
 *
 * @pre Either `merizo_embedded_v1_fini` must have never been called in this process, or it was
 * called and returned success and `merizo_embedded_v1_lib_init` was not called after this.
 *
 * @pre Either `merizo_embedded_v1_init` must have never been called in this process, or it was
 * called and then the embedded library was terminated by a successful call to
 * `merizo_embedded_v1_lib_fini`.
 *
 * @pre No valid `merizo_embedded_v1_lib` must exist.
 *
 * @param params A pointer to merizo_embedded_v1_init_params containing library initialization
 * parameters. A default configuration will be used if `params == NULL`.
 *
 * @param status A pointer to a `merizo_embedded_v1_status` object which will not be modified unless
 * this function reports a failure.
 *
 * @post Either the Embedded MerizoDB Library will be initialized, or an error will be reported.
 *
 * @returns A pointer to a `merizo_embedded_v1_lib` object on success.
 ...
 * other non-`merizo_embedded_v1_status` operations can be called on any thread.
 *
 ...
 *
 * The Embedded MerizoDB Library must be quiesced before the containg process can be safely
 ...
 * destroy `merizo_embedded_v1_status` objects after the library has been quiesced. The library may
 * be re-initialized with a potentially different configuration after it has been queisced.
 *
 * @pre All `merizo_embedded_v1_instance` objects associated with this library handle must be
 * destroyed.
 *
 * @pre The specified `lib` object must not be `NULL`.
 * @pre The specified `lib` object must be a valid `merizo_embedded_v1_lib` object.
 *
 * @pre The specified `status` object must either be a valid `merizo_embedded_v1_status` object or
 * `NULL`.
 *
 * @param lib A pointer to a `merizo_embedded_v1_lib` handle which represents this library.
 *
 * @param status A pointer to a `merizo_embedded_v1_status` object which will not be modified unless
 * this function reports a failure.
 *
 ...
 * non-`merizo_embedded_v1_status` operation.
 *
...
/**
 * An object which represents an instance of an Embedded MerizoDB Server.
 *
 * The Embedded MerizoDB Library uses allocated objects of this type (`merizo_embedded_v1_instance`)
 * to indicate the present state of a single "server-like" MerizoDB instance. Some operations which
 ...
 *
 * @invariant The use of `merizo_embedded_v1_instance` objects from multiple threads is not
 * threadsafe unless all of the threads accessing a single `merizo_embedded_v1_instance` object are
 * not destroying this object. If a single thread is passing a `merizo_embedded_v1_instance` to its
 * destruction function, then no other thread may access the `merizo_embedded_v1_instance` object.
 */
typedef struct merizo_embedded_v1_instance merizo_embedded_v1_instance;

/**
 * Creates an embedded MerizoDB instance and returns a handle with the service context.
 *
 * A `merizo_embedded_v1_instance` object which represents a single embedded "server-like" context is
 ...
 * be `NULL`.
 *
 * @pre The specified `status` object must be either a valid `merizo_embedded_v1_status` object or
 * `NULL`.
 *
 * @param lib A pointer to a `merizo_embedded_v1_lib` handle which represents the Embedded MerizoDB
 * Library.
 *
 * @param yaml_config A null-terminated YAML formatted Embedded MerizoDB Instance configuration. See
 * documentation for valid options.
 *
 * @param status A pointer to a `merizo_embedded_v1_status` object which will not be modified unless
 * this function reports a failure.
 *
 * @post Either a new Embedded MerizoDB Server will be created, or an error will be reported.
 *
 * @return A pointer to a newly constructed, valid `merizo_embedded_v1_instance`.
...
/**
 * Shuts down an embedded MerizoDB instance.
 *
 * A `merizo_embedded_v1_instance` embedded "server-like" instance can be terminated by this
 * function.
 ...
 * @pre The specified `instance` object must be a valid `merizo_embedded_v1_instance` object.
 *
 * @pre The specified `status` object must be either a valid `merizo_embedded_v1_status` object or
 * `NULL`.
 *
 * @pre All `merizo_embedded_v1_client` objects associated with this database must be destroyed.
 *
 * @param instance A pointer to a valid `merizo_embedded_v1_instance` instance to be destroyed.
 *
 * @param status A pointer to a `merizo_embedded_v1_status` object which will not be modified unless
 * this function reports a failure.
 *
...
/**
 * An object which represents "client connection" to an Embedded MerizoDB Server.
 *
 * A `merizo_embedded_v1_client` connection object is necessary to perform most database operations,
 ...
 *
 * @invariant The use of `merizo_embedded_v1_client` objects from multiple threads is not threadsafe.
 */
typedef struct merizo_embedded_v1_client merizo_embedded_v1_client;
 ...
 * `NULL`.
 *
 * @post Either a new Embedded MerizoDB Client will be created, or an error will be reported.
 *
 * @param instance The Embedded MerizoDB Server instance that will be attached to this client and
 * execute its RPC calls
 *
 * @param status A pointer to a `merizo_embedded_v1_status` object which will not be modified unless
 * this function reports a failure.
 *
 * @return A pointer to a newly constructed, valid `merizo_embedded_v1_client`.
...
/**
 * Destroys an Embedded MerizoDB Client.
 *
 ...
 * @pre The specified `client` object must be a valid `merizo_embedded_v1_client` object.
 *
 * @pre The specified `status` object must be either a valid `merizo_embedded_v1_status` object or
 * `NULL`.
 *
 ...
 * concurrently to any other function. It is safe to destroy distinct `merizo_embedded_v1_client`
 * objects on distinct threads.
 *
 * @note This function exhibits undefined behavior unless its preconditions are met.
 *
 ...
 *
 * A MerizoDB client operation is performed according to the provided BSON object specified by
 ...
 * `merizo_embedded_v1_client` object is not concurrently accessed by any other thread until after
 ...
 * `merizo_embedded_v1_client_wire_protocol_rpc` on the specified `client` object, or the `client` is
 * destroyed using `merizo_embedded_v1_client_destroy`.
 *
