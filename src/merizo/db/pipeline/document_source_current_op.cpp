...
const StringData kMerizosClientFieldName = "client_s"_sd;
...
std::unique_ptr<DocumentSourceCurrentOp::LiteParsed> DocumentSourceCurrentOp::LiteParsed::parse(
    ...
    for (auto&& elem : spec.embeddedObject()) {
        if (elem.fieldNameStringData() == "allUsers"_sd) {
            ...
            if (elem.boolean()) {
                localOps = LocalOpsMode::kLocalMerizosOps;
            }
        }
    }
...
DocumentSource::GetNextResult DocumentSourceCurrentOp::getNext() {
    ...
    if (_ops.empty()) {
        ...
        if (pExpCtx->fromMerizos) {
            ...
            uassert(40465,
                    "Aggregation request specified 'fromMerizos' but unable to retrieve shard name "
    ...
    if (_opsIter != _ops.end()) {
        ...
        for (auto&& elt : op) {
            ...
            if (fieldName == kOpIdFieldName) {
                ...
                doc.addField(kMerizosClientFieldName, Value(elt.str()));
            } else {
        ...
        if (fieldName == kIdleConnectionsFieldName) {
            ...
            uassert(ErrorCodes::FailedToParse,
                    str::stream() << "The 'localOps' parameter of the $currentOp stage must be "
                                  ...
                                  << typeName(elem.type()),
                    elem.type() == BSONType::Bool);
            showLocalOpsOnMerizoS =
                (elem.boolean() ? LocalOpsMode::kLocalMerizosOps : LocalOpsMode::kRemoteShardOps);
    ...
    return Value(Document{
        {getSourceName(),
         Document{{kIdleConnectionsFieldName,
                  ...
                  {kLocalOpsFieldName,
                   _showLocalOpsOnMerizoS == LocalOpsMode::kLocalMerizosOps ? Value(true) : Value()},
