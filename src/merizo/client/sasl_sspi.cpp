...
#include "merizo/client/sasl_sspi_options.h"
...
void HandleLastError(const sasl_utils_t* utils, DWORD errCode, const char* msg) {
    ...
    std::string buffer(merizoutils::str::stream() << "SSPI: " << msg << ": " << err);
...
(InitializerContext*) {
    ...
    if (SASL_OK != ret) {
        return Status(ErrorCodes::UnknownError,
                      merizoutils::str::stream() << "could not add SASL Client SSPI plugin "
                                                << sspiPluginName
                                                << ": "
...
(InitializerContext*) {
    ...
    if (SASL_OK != ret) {
        return Status(ErrorCodes::UnknownError,
                      merizoutils::str::stream() << "Could not add SASL Client PLAIN plugin "
                                                << sspiPluginName
                                                << ": "
                                                << sasl_errstring(ret, NULL, NULL));
    }
