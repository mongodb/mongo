...
`merizoreplay` is a traffic capture and replay tool for MerizoDB. It can be used to inspect commands being sent to a MerizoDB instance, record them, and replay them back onto another host at a later time.
##### Use cases
...
- Inspect the details of what an application is doing to a merizo cluster (i.e. a more flexible version of [merizosniff](https://docs.merizodb.org/manual/reference/program/merizosniff/))

## Quickstart

Make a recording:

    merizoreplay record -i lo0 -e "port 27017" -p playback.bson
Analyze it:

    merizoreplay stat -p playback.bson --report playback_stats.json
Replay it against another server, at 2x speed:

    merizoreplay play -p playback.bson --speed=2.0 --report replay_stats.json --host 192.168.0.4:27018

## Detailed Usage

Basic usage of `merizoreplay` works in two phases: `record` and `play`. Analyzing recordings can also be performed with the `stat` command.
* The `record` phase takes a [pcap](https://en.wikipedia.org/wiki/Pcap) file (generated by `tcpdump`) and analyzes it to produce a playback file (in BSON format). The playback file contains a list of all the requests and replies to/from the Merizo instance that were recorded in the pcap dump, along with their connection identifier, timestamp, and other metadata.
...
To create a recording of traffic, use the `record` command as follows:
    ...
    merizoreplay record -i lo0 -e "port 27017" -p recording.bson
    

This will record traffic on the network interface `lo0` targeting port 27017.
The options to `record` are:
* `-i`: The network interface to listen on, e.g. `eth0` or `lo0`. You may be required to run `merizoreplay` with root privileges for this to work.
...
Alternatively, you can capture traffic using `tcpdump` and create a recording from a static PCAP file. First, capture TCP traffic on the system where the workload you wish to record is targeting. Then, run `merizoreplay record` using the `-f` argument (instead of `-i`) to create the playback file.

    sudo tcpdump -i lo0 -n "port 27017" -w traffic.pcap

    $ ./merizoreplay record -f traffic.pcap -p playback.bson

Using the `record` command of merizoreplay, this will process the .pcap file to create a playback file. The playback file will contain everything needed to re-execute the workload.
...
The `play` command takes a playback file and executes the operations in it against a target host.
    ...
    ./merizoreplay play -p playback.bson --host merizodb://target-host.com:27017
...
To modify playback speed, add the `--speed` command line flag to the `play` command. For example, `--speed=2.0` will run playback at twice the rate of the recording, while `--speed=0.5` will run playback at half the rate of the recording.
    ...
    merizoreplay play -p workload.playback --host staging-merizo-cluster-hostname
