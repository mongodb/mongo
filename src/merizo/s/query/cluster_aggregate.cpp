...
ShardId pickMergingShard(OperationContext* opCtx,
    ...
    // If we cannot merge on merizoS, establish the merge cursor on a shard. Perform the merging
...
// "Resolve" involved namespaces into a map. We won't try to execute anything on a merizos, but we
...
// merizod. Note that this function must be called before forwarding an aggregation command on an
...
Status runPipelineOnMerizoS(const boost::intrusive_ptr<ExpressionContext>& expCtx,
    ...
    uassert(ErrorCodes::IllegalOperation,
            str::stream() << "Aggregation pipeline must be run on merizoS, but "
    ...
    // can never run on merizoS. Filter the command response and return immediately.
...
Status dispatchMergingPipeline(
    ...
    // If we are not merging on merizoS, then this is not a $changeStream aggregation, and we
...
Status ClusterAggregate::runAggregate(OperationContext* opCtx,
                                      ...
                                      const PrivilegeVector& privileges,
                                      BSONObjBuilder* result) {
    uassert(51028, "Cannot specify exchange option to a merizos", !request.getExchangeSpec());
    uassert(51143,
            "Cannot specify runtime constants option to a merizos",
...
Status ClusterAggregate::retryOnViewError(OperationContext* opCtx,
    ...
    // underlying collection namespace is used to execute the aggregation on merizoD. Any cursor
