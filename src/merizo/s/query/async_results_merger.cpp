...
int compareSortKeys(BSONObj leftSortKey, BSONObj rightSortKey, BSONObj sortKeyPattern) {
    // This does not need to sort with a collator, since merizod has already mapped strings to their
...
Status AsyncResultsMerger::_askForNextBatch(WithLock, size_t remoteIndex) {
    ...
    // If merizod returned less docs than the requested batchSize then modify the next getMore
        ...
        if (response.getLastOplogTimestamp()) {
            severe() << "Host " << remote.shardHostAndPort
                     << " returned a cursor which has an oplog timestamp but does not have a "
                        ...
                        "older version of MerizoDB. This configuration is not supported.";
