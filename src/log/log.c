    ...
    WT_MYSLOT *myslot, WT_ITEM *record, WT_LSN *lsnp)
{
	...
	WT_RUNSTAT_CONN_INCRV(session, log_bytes_written, logrec->len);
	if (lsnp != NULL) {
...
__log_release(WT_SESSION_IMPL *session, WT_LOGSLOT *slot)
{
	...
	if (FLD_ISSET(slot->slot_flags, SLOT_SYNC)) {
		WT_RUNSTAT_CONN_INCR(session, log_sync);
    ...
    uint32_t flags)
{
	...
	WT_RUNSTAT_CONN_INCR(session, log_reads);
err:
    ...
    WT_ITEM *record, WT_LSN *lsnp, void *cookie), void *cookie)
{
	...
	do {
		...
		if (reclen > log->allocsize) {
			...
			WT_RUNSTAT_CONN_INCR(session, log_scan_rereads);
		}
		/*
		 * We read in the record, verify checksum.
		 */
		...
		WT_RUNSTAT_CONN_INCR(session, log_scan_records);
		rd_lsn.offset += (off_t)rdup_len;
	} while (!done);

err:	WT_RUNSTAT_CONN_INCR(session, log_scans);
    ...
    uint32_t flags)
{
	 ...
	 */
	WT_RUNSTAT_CONN_INCRV(session, log_bytes_user, record->size);
	...
	memset(&tmp, 0, sizeof(tmp));
	WT_RUNSTAT_CONN_INCR(session, log_writes);
