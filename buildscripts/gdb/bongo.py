"""GDB Pretty-printers and utility commands for BongoDB
"""
import gdb.printing
import glob
import os
...
try:
    ...
    print("Check with the pip command if pybongo 3.x is installed.")
    bson = None
...
class StatusPrinter(object):
    """Pretty-printer for bongo::Status"""
    OK = 0  # ErrorCodes::OK
    ...
    def to_string(self):
        ...
        # Remove the bongo::ErrorCodes:: prefix. Does nothing if not a real ErrorCode.
...
class StatusWithPrinter:
    """Pretty-printer for bongo::StatusWith<>"""
    def __init__(self, val):
        self.val = val
...
class StringDataPrinter:
    """Pretty-printer for bongo::StringData"""
...
class BSONObjPrinter:
    """Pretty-printer for bongo::BSONObj"""
...
class UnorderedFastKeyTablePrinter:
    """Pretty-printer for bongo::UnorderedFastKeyTable<>"""
...
class DecorablePrinter:
    """Pretty-printer for bongo::Decorable<>"""
    ...
    def __init__(self, val):
        ...
        decinfo_t = gdb.lookup_type('bongo::DecorationRegistry::DecorationInfo')
...
class BongoSubPrettyPrinter(gdb.printing.SubPrettyPrinter):
    """Sub pretty printer managed by the pretty-printer collection"""

    def __init__(self, name, prefix, is_template, printer):
        super(BongoSubPrettyPrinter, self).__init__(name)
        self.prefix = prefix
        self.printer = printer
        self.is_template = is_template


class BongoPrettyPrinterCollection(gdb.printing.PrettyPrinter):
    """BongoDB-specific printer printer collection that ignores subtypes.
    It will match 'HashTable<T> but not 'HashTable<T>::iterator' when asked for 'HashTable'.
    """

    def __init__(self):
        super(BongoPrettyPrinterCollection, self).__init__("bongo", [])

    def add(self, name, prefix, is_template, printer):
        self.subprinters.append(BongoSubPrettyPrinter(name, prefix, is_template, printer))
...
def build_pretty_printer():
    pp = BongoPrettyPrinterCollection()
    pp.add('BSONObj', 'bongo::BSONObj', False, BSONObjPrinter)
    pp.add('Decorable', 'bongo::Decorable', True, DecorablePrinter)
    pp.add('Status', 'bongo::Status', False, StatusPrinter)
    pp.add('StatusWith', 'bongo::StatusWith', True, StatusWithPrinter)
    pp.add('StringData', 'bongo::StringData', False, StringDataPrinter)
    pp.add('UnorderedFastKeyTable', 'bongo::UnorderedFastKeyTable', True, UnorderedFastKeyTablePrinter)
    return pp
...
#
###################################################################################################
# Dictionary of commands so we can write a help function that describes the BongoDB commands.
bongo_commands = {}


def register_bongo_command(obj, name, command_class):
    """Register a command with no completer as a bongo command"""
    global bongo_commands
    gdb.Command.__init__(obj, name, command_class)

    bongo_commands[name] = obj.__doc__
...
class DumpGlobalServiceContext(gdb.Command):
    ...
    def __init__(self):
        register_bongo_command(self, "bongodb-service-context", gdb.COMMAND_DATA)

    def invoke(self, arg, _from_tty):
        gdb.execute("print *('bongo::(anonymous namespace)::globalServiceContext')")

# Register command
DumpGlobalServiceContext()


class BongoDBAnalyze(gdb.Command):
    """Analyze BongoDB process"""

    def __init__(self):
        register_bongo_command(self, "bongodb-analyze", gdb.COMMAND_STATUS)
    ...
    def invoke(self, arg, _from_tty):
        ...
        if main_binary_name.endswith('bongod'):
            self.analyze_bongod()
        else:
            print("No process specific analysis done for: %s" % (main_binary_name))

    def analyze_bongod(self):
        """GDB in-process python supplement"""

        try:

            # Call into bongod, and dump the state of lock manager
            # Note that output will go to bongod's standard output, not the debugger output window
            gdb.execute("call ('bongo::(anonymous namespace)::globalLockManager').dump()",
...
class BongoDBHelp(gdb.Command):
    """Dump list of bongodb commands"""

    def __init__(self):
        gdb.Command.__init__(self, "bongodb-help", gdb.COMMAND_DATA)

    def invoke(self, arg, _from_tty):
        print("Command - Description")
        for key in bongo_commands:
            print("%s - %s" % (key, bongo_commands[key]))
...
# Register pretty-printers, replace existing bongo printers
gdb.printing.register_pretty_printer(
    gdb.current_objfile(),
    build_pretty_printer(),
    True)

print("BongoDB GDB pretty-printers and commands loaded, run 'bongodb-help' for list of commands")
