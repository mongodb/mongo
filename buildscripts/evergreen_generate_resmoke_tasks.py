...
class ConfigOptions(object):
    ...
    def _lookup(self, config, item):
        if item not in config:
            if item in self.required_keys:
                raise KeyError("{item} must be specified in configuration.".format(item=item))
    ...
    def __repr__(self):
        ...
        required_values = [
            "{key}: {value}".format(key=key, value=self.config[key]) for key in REQUIRED_CONFIG_KEYS
        ]
        return "ConfigOptions({options})".format(options=", ".join(required_values))
