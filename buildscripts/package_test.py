...
def run_test(test: Test, client: DockerClient) -> Tuple[Test, Result]:
    ...
    commands.append(f"python3 /mnt/package_test/package_test_internal.py {log_docker_path} "
...
def get_tools_package(arch_name: str, os_name: str) -> Optional[str]:
    ...
    def rhel_major_version(name: str) -> Optional[str]:
        rhel_match = re.match(r"^rhel(\d+)", name)
        if not rhel_match:
            return None

        digits = rhel_match.group(1)
...
if args.command == "branch":
    ...
    if args.evg_build_id:
        download_dir_inner = Path(__file__).parent / "downloaded_packages"
        local_packages_path = download_packages_from_build(args.evg_build_id, download_dir_inner)
...
if all((docker_username, docker_password)):
    ...
    login_response = docker_client.login(username=docker_username, password=docker_password)
    logging.debug("Login response: %s", login_response)
else:
