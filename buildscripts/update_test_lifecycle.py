...
class JiraIssueCreator(object):
    ...
    def create_issue(  # pylint: disable=too-many-arguments
            self, evg_project, merizo_revision, model_config, added, removed, cleaned_up):
        """Create a JIRA issue for the test lifecycle tag update."""
        summary = self._get_jira_summary(evg_project)
        description = self._get_jira_description(evg_project, merizo_revision, model_config, added,
                                                 removed, cleaned_up)
    ...
    def _get_jira_description(  # pylint: disable=too-many-arguments
            project, merizo_revision, model_config, added, removed, cleaned_up):
        ...
        project_link = "[{0}|https://evergreen.merizodb.com/waterfall/{1}]".format(
            mono(project), project)
        revision_link = "[{0}|https://github.com/merizodb/merizo/commit/{1}]".format(
            mono(merizo_revision), merizo_revision)
...
class LifecycleTagsFile(object):  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(  # pylint: disable=too-many-arguments
        ...
        Arguments:
            project: The Evergreen project name, e.g. "merizodb-merizo-master".
        ...
        self.merizo_repo = git.Repository(os.getcwd())
        self.merizo_revision = self.merizo_repo.get_current_revision()
        # The branch name is the same on both repositories.
        self.merizo_branch = self.merizo_repo.get_branch_name()
    ...
    def _create_issue(self):
        ...
        issue_key = self.jira_issue_creator.create_issue(
            self.project, self.merizo_revision, self.model_config, self.changelog_lifecycle.added,
    ...
    def write(self):
        ...
        comment = ("This file was generated by {} and shouldn't be edited by hand. It was"
                   " generated against commit {} with the following options: {}.").format(
                       sys.argv[0], self.merizo_repo.get_current_revision(),
    ...
    def _ready_for_commit(self, ref_branch, references):
        ...
        if not diff:
            LOGGER.info("The local lifecycle file is identical to the the one on branch '%s'",
                        ref_branch)
            return False
        # Check that the lifecycle file has not been updated after the current merizo revision.
        update_revision = references.get("test-lifecycle", {}).get(self.project)
        if update_revision and not self.merizo_repo.is_ancestor(update_revision,
                                                               self.merizo_revision):
    ...
    def _update_and_write_references(self, references):
        ...
        references.setdefault("test-lifecycle", {})[self.project] = self.merizo_revision
...
def main():  # pylint: disable=too-many-branches,too-many-locals,too-many-statements
    ...
    commit_options.add_option(
        ...
        default="buil+testlifecycle@merizodb.com",
