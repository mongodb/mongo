...
if sys.platform == "linux":
    ...
    @unittest.skipIf(
        sys.platform != "linux",
        reason="No need to run this unittest on non linux since this is only needed for linux",
    )
    class TestSbom(unittest.TestCase):
        ...
        def test_missing_version(self):
            ...
            error_manager = sbom_linter.lint_sbom(test_file, test_file, third_party_libs, False)
            self.assert_message_in_errors(error_manager,
                                          sbom_linter.MISSING_VERSION_IN_SBOM_COMPONENT_ERROR)
        ...
        def test_missing_version_in_import_file(self):
            ...
            error_manager = sbom_linter.lint_sbom(test_file, test_file, third_party_libs, False)
            self.assert_message_in_errors(error_manager,
                                          sbom_linter.MISSING_VERSION_IN_IMPORT_FILE_ERROR)
        ...
        def test_missing_import_file(self):
            ...
            error_manager = sbom_linter.lint_sbom(test_file, test_file, third_party_libs, False)
            self.assert_message_in_errors(error_manager,
                                          sbom_linter.COULD_NOT_FIND_OR_READ_SCRIPT_FILE_ERROR)
        ...
        def test_missing_license(self):
            ...
            error_manager = sbom_linter.lint_sbom(test_file, test_file, third_party_libs, False)
            self.assert_message_in_errors(error_manager,
                                          sbom_linter.MISSING_LICENSE_IN_SBOM_COMPONENT_ERROR)
