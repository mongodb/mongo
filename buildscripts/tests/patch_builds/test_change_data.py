...
class TestGenerateRevisionMapFromManifest(unittest.TestCase):
    def test_map_can_be_created_from_evergreen_api(self):
        mock_repo_list = [create_mock_repo(os.getcwd())]
        mock_repo_list[0].head.commit.hexsha = "revision1234"

        revision_map = under_test.generate_revision_map_from_manifest(mock_repo_list)

        self.assertEqual(revision_map[mock_repo_list[0].git_dir],
                         mock_repo_list[0].head.commit.hexsha)
