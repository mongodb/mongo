...
class TestBuildShardedCluster(unittest.TestCase):
    ...
    def test_build_sharded_cluster_multiversion(self):
        ...
        self.assertEqual(
            sharded_cluster.configsvr.nodes[0].mongod_executable,
            config.DEFAULT_MONGOD_EXECUTABLE,
        )
        self.assertEqual(
            sharded_cluster.configsvr.nodes[1].mongod_executable,
            config.DEFAULT_MONGOD_EXECUTABLE,
        )
        # 1st repl set nodes are latest and last-lts (new_old)
        self.assertEqual(
            sharded_cluster.shards[0].nodes[0].mongod_executable,
            config.DEFAULT_MONGOD_EXECUTABLE,
        )
        self.assertEqual(
