...
assert.eq(
    TOP_LEVEL_VALUE,
    "top_level_import_success",
    "Top-level import() failed to load module from MONGO_PATH",
);
assert.eq(
    getTopLevelValue(),
    "top_level_function_works",
    "Function from top-level imported module doesn't work",
);
...
assert.eq(
    MONGO_PATH_TEST_COMMON,
    "loaded_from_mongo_path",
    "load() failed to load module from MONGO_PATH",
);
...
assert.eq(
    ...
    "import() failed to load module from MONGO_PATH",
);
assert.eq(
    esModule.getModuleValue(),
    "module_value_from_import",
    "Function from imported module doesn't work",
);
