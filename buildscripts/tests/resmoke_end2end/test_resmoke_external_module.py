...
from buildscripts.resmokelib import config  # pylint: disable=wrong-import-position
...
class TestExternalModule(unittest.TestCase):
    ...
    def test_builtin_prefix_with_exclude(self):
        ...
        result = self.run_resmoke(["test-discovery", "--suite", self.builtin_exclude_suite_name],
                                  env=env)
        self.assertEqual(result.returncode, 0, f"resmoke failed: {result.stderr}")
