...
class TestShardedClusterFixtureUDSEnvironmentVariables(unittest.TestCase):
    ...
    def test_fixture_type_is_set(self):
        ...
        self.assertEqual(
            ...
            "Fixture type should be ShardedClusterFixture",
        )
    ...
    def test_mongodb_uds_path_is_set(self):
        ...
        self.assertTrue(
            os.path.exists(uds_path),
            f"Primary mongos UDS path should exist: {uds_path}",
        )
    ...
    def test_mongodb_uds_paths_are_set(self):
        ...
        for uds_path in paths_list:
            ...
            self.assertIn(
                "mongodb-",
                uds_path,
                f"UDS path should contain mongodb- in filename: {uds_path}",
            )
            self.assertTrue(
                uds_path.endswith(".sock"),
                f"UDS path should end with .sock: {uds_path}",
            )
            self.assertIn(
            ...
            self.assertTrue(
                os.path.exists(uds_path),
                f"UDS path should exist on the filesystem: {uds_path}",
            )
    ...
    def test_mongodb_connection_string_is_still_set(self):
        ...
        self.assertIn(
            "localhost",
            conn_str,
            f"Connection string should contain localhost: {conn_str}",
        )
