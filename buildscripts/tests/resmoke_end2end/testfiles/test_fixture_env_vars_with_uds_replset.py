"""Test that fixture UDS environment variables are properly set for replica sets."""

import os
import unittest


class TestReplicaSetFixtureUDSEnvironmentVariables(unittest.TestCase):
    """Test case to verify fixture UDS environment variables in replica sets."""
    ...
    def test_fixture_type_is_set(self):
        ...
        self.assertEqual(
            fixture_type, "ReplicaSetFixture", "Fixture type should be ReplicaSetFixture"
        )
    ...
    def test_mongodb_uds_paths_are_set(self):
        ...
        self.assertIsNotNone(
            uds_paths, "MONGODB_UDS_PATHS environment variable should be set when UDS is configured"
        )
    ...
    def test_mongodb_primary_uds_path_is_set(self):
        """Test that MONGODB_PRIMARY_UDS_PATH is set for the primary node."""
        primary_uds = os.environ.get("MONGODB_PRIMARY_UDS_PATH")
        self.assertIsNotNone(
            primary_uds, "MONGODB_PRIMARY_UDS_PATH should be set when UDS is configured"
        )

        # Verify it exists
        self.assertTrue(
            os.path.exists(primary_uds), f"Primary UDS path should exist: {primary_uds}"
        )
