...
def _find_linter(linter, config_dict):
    ...
    # 2. The current path
    # 3. In '/opt/merizodbtoolchain/v3/bin' if virtualenv is set up.
    ...
    if sys.platform == "win32":
        ...
        # '/opt/merizodbtoolchain/v3/bin', but they may point to the wrong interpreter.
        cmd_str = os.path.join(python_dir, linter.cmd_name)
        ...
        if linter.ignore_interpreter():
            ...
            if linter.cmd_location:
                ...
                cmd_str = os.path.join('/opt/merizodbtoolchain/v3/bin', linter.cmd_name)
            cmd = [cmd_str]
        else:
    ...
    # to use the linters installed in '/opt/merizodbtoolchain/v3/bin'.
    cmd = [sys.executable, os.path.join('/opt/merizodbtoolchain/v3/bin', linter.cmd_name)]
    if _check_version(linter, cmd, linter.get_lint_version_cmd_args()):
        return base.LinterInstance(linter, cmd)
