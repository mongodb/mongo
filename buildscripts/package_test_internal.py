...
def run_mongo_query(shell, query, should_fail=False, tries=60, interval=1.0):
    ...
    while current_try <= tries:
        try:
            ...
            if (not should_fail and exec_result.returncode == 0) or (
                should_fail and exec_result.returncode != 0
            ):
                return exec_result
...
def parse_ulimits(pid: int) -> Dict[str, Tuple[int, int, Optional[str]]]:
    ulimit_line_re = re.compile(
        r"(?P<name>.*?)\s{2,}(?P<soft>\S+)\s+(?P<hard>\S+)(?:\s+(?P<units>\S+))?",
        re.MULTILINE,
    )
...
def get_test_args(package_manager: str, package_files: List[str]) -> TestArgs:
    ...
    test_args["systemd_units_dir"] = (
        run_and_log("pkg-config systemd --variable=systemdsystemunitdir")
        .stdout.decode("utf-8")
        .strip()
    )
    test_args["systemd_presets_dir"] = (
        run_and_log("pkg-config systemd --variable=systemdsystempresetdir")
        .stdout.decode("utf-8")
        .strip()
    )
...
def test_install_is_complete(test_args: TestArgs):
    ...
    if test_args["mongo_groupname"] not in mongo_user_groups:
        raise RuntimeError(
            "Required group `{}' is not in configured groups: {}".format(
    ...
    if user_info.pw_shell != test_args["mongo_user_shell"]:
        raise RuntimeError(
            "Configured user shell `{}' does not match required path `{}'".format(
...
def test_install_compass(test_args: TestArgs):
    if test_args["arch"] != "x86_64" or test_args["os_name"] not in [
        "ubuntu",
        "almalinux",
    ]:
        logging.info(
...
def test_uninstall(test_args: TestArgs):
    ...
    if test_args["package_manager"] == "apt":
        ...
        raise RuntimeError(
            "Don't know how to uninstall with package manager: {}".format(
