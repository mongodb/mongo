...
def get_pull_request(repository: Repository.Repository, base_branch: str, head_branch: str,
                     github_owner: str) -> Optional[PullRequest.PullRequest]:
    """Gets the pull request for the branch ref, if it exists."""
    pulls = repository.get_pulls(state="open", head=f"{github_owner}:{head_branch}",
                                 base=base_branch)
...
def create_branch(repository, base_branch, new_branch) -> None:
    ...
    git_ref = f"refs/heads/{new_branch}"
    try:
        ...
        repository.create_git_ref(ref=git_ref, sha=sha)
        print(f"Created branch '{new_branch}', ref: {git_ref}, sha: {sha}")
    except GithubException as err:
        if err.status == 422:
            print(f"Branch {new_branch} already exists, ref: {git_ref}")
        else:
            raise
...
def read_text_file(path: str) -> str:
    ...
    try:
        ...
        return f"ERROR: An error occurred while reading '{path}': {ex}"


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
        description=("This script checks for changes to SBOM and related files and creates a PR if "
    ...
    if not args.app_id or not args.private_key:
        parser.error("Must define --app-id or env MONGO_PR_BOT_APP_ID and --private-key or env "
    ...
    for file_path in SBOM_FILES:
        ...
        try:
            ...
            if isinstance(original_file, list):
                ORIGINAL_CONTENT = original_file[0].decoded_content.decode()
            else:
    ...
    if changed_files:
        ...
        COMMIT_MESSAGE = "Update SBOM-related files: " + ", ".join(path
