...
def convert_sbom_to_public(sbom_dict: dict):
    ...
    sbom_dict["dependencies"] = [
        d for d in sbom_dict["dependencies"] if d["ref"] in valid_refs
    ]
    for dependency in sbom_dict["dependencies"]:
        dependency["dependsOn"] = [
            d for d in dependency["dependsOn"] if d in valid_refs
        ]
...
def is_valid_purl(purl: str) -> bool:
    ...
    for purl_type, regex in REGEX_PURL.items():
        if regex.match(purl):
            logger.debug(
                "PURL: %s matched PURL type '%s' regex '%s'",
                purl,
                purl_type,
                regex.pattern,
            )
...
def read_sbom_json_file(file_path: str) -> dict | None:
    ...
    try:
        ...
        logger.info(
            "SBOM loaded from %s with %d components",
            file_path,
...
def set_dependency_version(
    ...
    logger.debug(
        "set_dependency_version: '%s' updated %d refs and %d dependsOn",
        meta_bom_ref,
        r,
        d,
    )
...
def add_component_dependsOn(
    dependencies: list, component_ref: str, depends_on_ref: str
) -> None:
