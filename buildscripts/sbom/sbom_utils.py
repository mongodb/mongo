...
def convert_sbom_to_public(sbom_dict: dict):
    ...
    internal_components = [
        c["bom-ref"] for c in sbom_dict["components"] if any(
            occurence.get("location", "").startswith("src/third_party/private")
            for occurence in c.get("evidence", {}).get("occurrences", [])) or any(
    ...
    logger.info(
        ...
        original_properties_len - sum(
            len(c.get("properties", [])) for c in sbom_dict["components"]),
    )
...
def read_sbom_json_file(file_path: str) -> Optional[dict]:
...
def set_component_version(component: dict, version: str, purl_version: Optional[str] = None,
                          cpe_version: Optional[str] = None) -> None:
    ...
    if component.get("purl"):
        component["purl"] = component["purl"].replace("{{VERSION}}",
                                                      urllib.parse.quote(purl_version))
...
def set_dependency_version(dependencies: list, meta_bom_ref: str, purl_version: str) -> None:
    ...
    logger.debug("set_dependency_version: '%s' updated %d refs and %d dependsOn", meta_bom_ref,
                 ref_count, dep_count)
