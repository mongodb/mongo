...
def get_field_type(
    field: Union[syntax.Field, syntax.Command],
    idl_file: syntax.IDLParsedSpec,
    idl_file_path: str,
) -> Optional[Union[syntax.Enum, syntax.Struct, syntax.Type]]:
        ...
        if resolved_new_chained_type is not None:
            for old_chained_type in old_reply.chained_types or []:
                ...
                ctxt.add_new_reply_chained_type_not_subset_error(
                    cmd_name,
                    new_reply.name,
                    resolved_new_chained_type.name,
                    new_idl_file_path,
                )
...
def check_param_or_type_validator(
    ...
    if new_field.validator:
        if old_field.validator:
            ...
            # guarantees.
            if new_field_name not in [
                "create-param-timeField",
                "create-param-metaField",
            ]:
            ...
            with open(old_idl_file_path) as old_file:
                ...
                for old_cmd in old_idl_file.spec.symbols.commands:
                    ...
                    check_security_access_checks(
                        ctxt,
                        old_cmd.access_check,
                        new_cmd.access_check,
                        old_cmd,
                        new_idl_file_path,
                    )
...
def check_generic_arguments_compatibility(
    ...
    ctxt = IDLCompatibilityContext(
        "old generic_argument.idl",
        "new generic_argument.idl",
        IDLCompatibilityErrorCollection(),
    )
