...
incremental_rollout_phase 'rollout' or 'release' across all IDL files under src/.
...
_HEADER = textwrap.dedent("""\
    ...
    # 'rollout' or 'release' across the IDL sources under src/. Just commit
    ...
    #   - 'release': rollout complete; retained here as a historical record.
""")
...
def _extract_phase_flags(idl_path):
    """Return (rollout_flags, release_flags) declared in one IDL file.
    ...
    for flag_name, flag_body in feature_flags.items():
        ...
        if phase == "rollout":
            ...
            release.add(flag_name)
    return rollout, release


def collect_flags(source_root):
    """Walk source_root and return (rollout_flags, release_flags) across all IDLs."""
    rollout = set()
    release = set()
...
def main(argv=None):
    ...
    try:
        rollout, release = collect_flags(args.source_root)
    except ValueError as exc:
        sys.stderr.write(f"{exc}\n")
        return 2

    with open(args.output, "w", encoding="utf-8") as f:
        f.write(render_registry(rollout, release))
    return 0
