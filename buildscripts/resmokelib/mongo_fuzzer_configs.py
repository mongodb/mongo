...
def generate_mongod_parameters(rng, mode):
    ...
    ret["storageEngineConcurrencyAdjustmentAlgorithm"] = rng.choices(
        ["throughputProbing", "fixedConcurrentTransactions"], weights=[10, 1]
    )[0]
    ...
    ret["throughputProbingMinConcurrency"] = rng.randint(
        4, ret["throughputProbingInitialConcurrency"]
    )
    ret["throughputProbingMaxConcurrency"] = rng.randint(
        ret["throughputProbingInitialConcurrency"], 128
    )
    ...
    ret["internalQueryExecYieldIterations"] = rng.choices(
        [1, rng.randint(1, 1000)], weights=[1, 10]
    )[0]
...
def fuzz_mongod_set_parameters(mode, seed, user_provided_params):
    ...
    ret = {}
    params = [
        generate_flow_control_parameters(rng),
        generate_mongod_parameters(rng, mode),
    ]
    for dct in params:
    ...
    return (
        utils.dump_yaml(ret),
        generate_eviction_configs(rng, mode),
