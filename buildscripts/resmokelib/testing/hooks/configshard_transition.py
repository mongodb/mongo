...
class _TransitionThread(threading.Thread):
    ...
    def _drain_config_for_ongoing_transition(self, num_rounds, transition_result):
        ...
        if num_rounds % 10 == 0:
            ...
            self.logger.info("Sharded collections on config server: " +
                             str({"count": len(sharded_colls), "collections": sharded_colls}))
            self.logger.info("Tracked unsharded collections on config server: " + str(
                {"count": len(tracked_unsharded_colls), "collections": tracked_unsharded_colls}))
            self.logger.info("Untracked unsharded collections on config server: " + str({
                "count": len(untracked_unsharded_colls),
                "collections": untracked_unsharded_colls,
            }))
            self.logger.info("Databases on config server: " + str({
        ...
        if should_move:
            self._move_collection_all_from_config(tracked_unsharded_colls +
                                                  untracked_unsharded_colls)
