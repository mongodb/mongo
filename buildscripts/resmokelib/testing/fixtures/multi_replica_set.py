...
class MultiReplicaSetFixture(interface.MultiClusterFixture):
    ...
    def __init__(
        ...
        **common_replica_set_options,
        ...
        self.num_replica_sets = (
            num_replica_sets if num_replica_sets else self.config.NUM_REPLSETS
        )
        ...
        self.all_nodes_electable = self.common_replica_set_options.get(
    ...
    def pids(self):
        ...
        if not out:
            self.logger.debug(
                "No replica sets when gathering multi replicaset fixture pids."
            )
        return out
    ...
    def _do_teardown(self, mode=None):
        ...
        if not running_at_start:
            self.logger.warning(
                "All replica sets were expected to be running, but weren't."
            )
