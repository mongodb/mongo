...
from typing import Any, List, Optional, NamedTuple, Set
...
class ResmokeSymbolizer:
    ...
    def make_symbolization_instructions_or_symbolize(self, test: TestCase, data: dict,
                                                     files: list[str]):
        """
        ...
        If we do not skip symbolization, the unsymbolized stacktraces are automatically symbolized and output by logs statements.        :param test: resmoke test case
        :param data
        :param files: unsymbolized stacktrace file names
    ...
    def create_unsymbolized_instructions(
        ...
        for entry in unsymbolized_stacktraces_info:
            ...
            found_process_info = self.get_value_recursively(unsymbolized_stacktrace,
                                                            PROCESS_INFO_KEY)
            if not (found_backtrace and found_process_info):
                missing_keys.append((entry["name"], entry["test_name"]))
        unsymbolizable_stacktrace_comment = (self.show_unsymbolizable_stacktraces(missing_keys)
...
If no symbolized stacktrace is created, then most likely either:
    ...
    def show_unsymbolizable_stacktraces(self, missing_keys: list[tuple[str, str]]):
        """
        Writes to the Evergreen user what files are not in proper formatting to be symbolized.
