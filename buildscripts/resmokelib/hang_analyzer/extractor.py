...
from buildscripts.resmokelib.utils.otel_thread_pool_executor import (
    OtelThreadPoolExecutor,
)
...
def download_multiversion_artifact(
    ...
    current_span = get_default_current_span(
        {
            "downloaded_artifact_type": name,
            "version": bin_version if bin_version else "current",
        }
    )
    try:
...
def post_install_gdb_optimization(download_dir: str, root_looger: Logger):
    ...
    def recalc_debuglink(file_path: str):
        ...
        process = subprocess.run(
            [f"{TOOLCHAIN_ROOT}/bin/eu-readelf", "-S", file_path],
            capture_output=True,
            text=True,
        )
