    ...
    namespace mr {
        ...
        BSONObj reduceValues( BSONList& values , Scope * s , ScriptingFunction reduce , bool final , ScriptingFunction finalize ){
            ...
            for ( ; n<values.size(); n++ ){
                ...
                uassert( 13070 , "value to large to reduce" , ee.size() < ( BSONObjMaxSize / 2 ) );

                if ( sizeSoFar + ee.size() > BSONObjMaxSize ){
        ...
        BSONObj fast_emit( const BSONObj& args ){
            ...
            uassert( 13069 , "an emit can't be more than 2mb" , args.objsize() < ( BSONObjMaxSize / 2 ) );
            _tlmr->insert( args );
            _tlmr->numEmits++;
            return BSONObj();
        }
