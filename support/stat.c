...
__wt_stat_alloc_db_dstats(ENV *env, WT_STATS **statsp)
{
	...
	if ((ret = __wt_calloc(env,
	    WT_STAT_DB_DSTATS_TOTAL + 1, sizeof(WT_STATS), &stats)) != 0)
		return (ret);
...
__wt_stat_alloc_db_hstats(ENV *env, WT_STATS **statsp)
{
	...
	if ((ret = __wt_calloc(env,
	    WT_STAT_DB_HSTATS_TOTAL + 1, sizeof(WT_STATS), &stats)) != 0)
		return (ret);
...
__wt_stat_alloc_env_hstats(ENV *env, WT_STATS **statsp)
{
	...
	if ((ret = __wt_calloc(env,
	    WT_STAT_ENV_HSTATS_TOTAL + 1, sizeof(WT_STATS), &stats)) != 0)
		return (ret);
...
__wt_stat_alloc_fh_stats(ENV *env, WT_STATS **statsp)
{
	...
	if ((ret = __wt_calloc(env,
	    WT_STAT_FH_STATS_TOTAL + 1, sizeof(WT_STATS), &stats)) != 0)
		return (ret);
