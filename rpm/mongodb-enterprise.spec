...
This metapackage will install the merizo shell, import/export tools, other client utilities, server software, default configuration, and systemd service files.
...
This package contains the MerizoDB server software, default configuration files, and systemd service files.
...
cp -v rpm/merizod.service $RPM_BUILD_ROOT/lib/systemd/system
...
if test $1 = 1
then
    /usr/bin/systemctl enable merizod
fi
if test $1 = 2
then
...
if test $1 = 0
then
  /usr/bin/systemctl disable merizod
fi

%postun server
if test $1 -ge 1
then
  /usr/bin/systemctl restart merizod >/dev/null 2>&1 || :
fi
...
%changelog
* Mon Oct 10 2016 Sam Kleinman <sam@merizodb.com>
