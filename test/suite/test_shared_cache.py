...
class test_shared_cache(wttest.WiredTigerTestCase):
    ...
    def openConnections(
            self,
            connections,
            pool_opts = ',shared_cache=(enabled=true,name=pool,size=200M,chunk=10M,reserve=30M),',
            extra_opts = '',
            add=0):
        if add == 0:
            self.conns = []
    ...
    def test_shared_cache09(self):
        nops = 1000
        self.openConnections(['WT_TEST1', 'WT_TEST2'], pool_opts=',shared_cache=(enabled=true,name=pool,size=200M)')
    ...
    def test_shared_cache10(self):
        ...
        connection.reconfigure("shared_cache=(enabled=true,size=300M)")
        self.closeConnections()

    # Test default config values
    def test_shared_cache11(self):
        nops = 1000
        self.openConnections(['WT_TEST1', 'WT_TEST2'], pool_opts=',shared_cache=(enabled=true)')
