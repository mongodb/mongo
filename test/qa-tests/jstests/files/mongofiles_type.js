...
(function() {
  ...
  var runTests = function(topology, passthrough) {
    ...
    // ensure tool runs without error with an empty --type argument
    assert.eq(runMongoProgram.apply(this, ["mongofiles", "--port", conn.port, "--type", "", "put", filesToInsert[1]].concat(passthrough.args)), 0, "put failed when it should have succeeded 2");

    fileObj = db.fs.files.findOne({
      filename: filesToInsert[1]
    });

    assert(fileObj, "did not find expected GridFS file - " + filesToInsert[1]);
