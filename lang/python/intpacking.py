...
def decompress_int(b):
	marker = ord(b[0])
	if marker < NEG_2BYTE_MARKER:
		sz = 8 - getbits(marker, 4)
		return (-1 << (sz << 3)) | get_int(b[1:], sz)
	elif marker < NEG_1BYTE_MARKER:
		return NEG_2BYTE_MIN + ((getbits(marker, 5) << 8) | ord(b[1]))
	elif marker < POS_1BYTE_MARKER:
		return NEG_1BYTE_MIN + getbits(marker, 6)
	elif marker < POS_2BYTE_MARKER:
		return getbits(marker, 6)
	elif marker < POS_MULTI_MARKER:
		return POS_1BYTE_MAX + 1 + ((getbits(marker, 5) << 8) | ord(b[1]))
	else:
		return POS_2BYTE_MAX + 1 + get_int(b[1:], getbits(marker, 4))
...
if __name__ == '__main__':
	...
	for big in (100, 10000, 1 << 40, 1 << 64):
		for i in xrange(1000):
			...
			print "\rChecking %d" % r,
			if decompress_int(compress_int(r)) != r:
		...
		for i in xrange(1000):
			...
			if cmp(r1, r2) != cmp(compress_int(r1), compress_int(r2)):
