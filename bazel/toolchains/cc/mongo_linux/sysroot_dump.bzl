...
_SYSROOT_ENV_VAR = "USE_RBE_SYSROOT"
...
def _sysroot_dump_impl(ctx):
    ...
    if ctx.os.name != "linux":
        ctx.file("BUILD.bazel", "# Sysroot dump is only supported on Linux.\n")
        ctx.file("sysroot_info.bzl", 'SYSROOT_PATH = ""\nSYSROOT_ENABLED = False\nSYSROOT_CONTAINER_URL = ""\n')
        return

    # No RBE container images exist for s390x or ppc, so skip the dump on
    # those architectures. Bazel evaluates use_rbe_sysroot_enabled to False on
    # these CPUs as well, so the toolchain template won't try to consume an
    # empty sysroot.
    arch = ctx.os.arch
    if arch in ("s390x", "ppc", "ppc64", "ppc64le"):
        ctx.file("BUILD.bazel", _NOOP_BUILD)
        ctx.file("sysroot_info.bzl", 'SYSROOT_PATH = ""\nSYSROOT_ENABLED = False\nSYSROOT_CONTAINER_URL = ""\n')
        return

    use_sysroot = ctx.os.environ.get(_SYSROOT_ENV_VAR, "0")
    if use_sysroot not in ["1", "true", "True", "yes"]:
        ctx.file("BUILD.bazel", _NOOP_BUILD)
        ctx.file("sysroot_info.bzl", 'SYSROOT_PATH = ""\nSYSROOT_ENABLED = False\nSYSROOT_CONTAINER_URL = ""\n')
        return
    ...
    if distro == None:
        print("WARNING: Could not detect host distro for sysroot dump. Skipping.")
        ctx.file("BUILD.bazel", _NOOP_BUILD)
        ctx.file("sysroot_info.bzl", 'SYSROOT_PATH = ""\nSYSROOT_ENABLED = False\nSYSROOT_CONTAINER_URL = ""\n')
        return
    ...
    # distros keep using the host's system headers and libraries; the
    # toolchain template reads SYSROOT_ENABLED = False and falls back to the
    # no-sysroot configuration even if --//bazel/config:use_rbe_sysroot is set.
    ...
    if distro not in REMOTE_EXECUTION_CONTAINERS:
        print("WARNING: No RBE container found for distro '{}'. Skipping sysroot dump.".format(distro))
        ctx.file("BUILD.bazel", _NOOP_BUILD)
        ctx.file("sysroot_info.bzl", 'SYSROOT_PATH = ""\nSYSROOT_ENABLED = False\nSYSROOT_CONTAINER_URL = ""\n')
        return
    ...
    if not docker:
        fail(
            ...
            "Either install one or unset {}=1.".format(_SYSROOT_ENV_VAR),
        )
