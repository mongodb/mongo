...
install_gdb_provider = provider(doc = "Enable installing the hermetic GDB toolchain in installed archives.", fields = ["enabled"])

install_gdb = rule(
    implementation = lambda ctx: install_gdb_provider(enabled = ctx.build_setting_value),
    build_setting = config.bool(flag = True),
)
