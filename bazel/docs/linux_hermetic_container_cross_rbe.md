...
wrapper hook warns and falls back to a native, non-containerized build before any action runs.
Containerized builds and fallback builds publish the BES keyword
`MONGO_BUILD_CONTAINERIZED=<true|false>` via `--bes_keywords` so telemetry can measure how often the
fallback occurs. The current v5 layout is required; configs from earlier layouts are rejected and
regenerated by the wrapper hook on the next normal Bazel invocation.
